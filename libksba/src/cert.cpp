...
get_name (ksba_cert_t cert, int idx, int use_subject, char **result)
{
  ...
  *result = NULL;
  ...
  if (!seqlen)
    return GPG_ERR_INV_CERT_OBJ; /* empty sequence is not allowed */
...
ksba_cert_get_cert_policies (ksba_cert_t cert, char **r_policies)
{
  ...
  *r_policies = NULL;
  ...
  if (err == GPG_ERR_EOF)
    err = 0;
  if (!*r_policies || err == GPG_ERR_NO_VALUE)
    err = GPG_ERR_NO_DATA;
...
ksba_cert_get_ext_key_usages (ksba_cert_t cert, char **result)
{
  ...
  *result = NULL;
  ...
  if (err == GPG_ERR_EOF)
    err = 0;
  if (!*result || err == GPG_ERR_NO_VALUE)
    err = GPG_ERR_NO_DATA;
                             ...
                             int *r_crit, ksba_sexp_t *r_data)
{
  ...
  *r_data = NULL;
  ...
  if (ti.length > derlen)
    return GPG_ERR_BAD_BER;
  if (ti.length != derlen)
    return GPG_ERR_INV_CERT_OBJ; /* Garbage follows. */
