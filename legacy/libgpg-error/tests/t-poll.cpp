...
static THREAD_RET_TYPE producer_thread(void *argaddr) {
...
static THREAD_RET_TYPE consumer_thread(void *argaddr) {
  struct thread_arg *arg = (thread_arg *)argaddr;
  char buf[15];
...
static void launch_thread(THREAD_RET_TYPE (*fnc)(void *),
...
static void create_pipe(estream_t *r_in, estream_t *r_out) {
  gpg_error_t err;
  int filedes[2];
  ...
  while (used || !fds[0].ignore) {
    ...
    if (debug)
      show(
          ...
          "\n",
          ret, fds[0].want_read ? 'r' : '-', fds[0].want_write ? 'w' : '-',
          fds[0].want_oob ? 'o' : '-', fds[0].want_rdhup ? 'h' : '-',
          fds[0].ignore ? '!' : '=', fds[0].got_read ? 'r' : '-',
          fds[0].got_write ? 'w' : '-', fds[0].got_oob ? 'o' : '-',
          fds[0].got_rdhup ? 'h' : '-', fds[0].got_hup ? 'H' : ' ',
          fds[0].got_err ? 'e' : ' ', fds[0].got_nval ? 'n' : ' ',

          fds[1].want_read ? 'r' : '-', fds[1].want_write ? 'w' : '-',
          fds[1].want_oob ? 'o' : '-', fds[1].want_rdhup ? 'h' : '-',
          fds[1].ignore ? '!' : '=', fds[1].got_read ? 'r' : '-',
          fds[1].got_write ? 'w' : '-', fds[1].got_oob ? 'o' : '-',
          fds[1].got_rdhup ? 'h' : '-', fds[1].got_hup ? 'H' : ' ',
          fds[1].got_err ? 'e' : ' ', fds[1].got_nval ? 'n' : ' ',

          fds[2].want_read ? 'r' : '-', fds[2].want_write ? 'w' : '-',
          fds[2].want_oob ? 'o' : '-', fds[2].want_rdhup ? 'h' : '-',
          fds[2].ignore ? '!' : '=', fds[2].got_read ? 'r' : '-',
          fds[2].got_write ? 'w' : '-', fds[2].got_oob ? 'o' : '-',
          fds[2].got_rdhup ? 'h' : '-', fds[2].got_hup ? 'H' : ' ',
          fds[2].got_err ? 'e' : ' ', fds[2].got_nval ? 'n' : ' ');
    else
      show("gpgrt_poll detected %d events\n", ret);
    ...
    if (fds[0].got_read) {
      ...
      if (used) fds[1].ignore = 0; /* Data to send.  */
    }
    if (fds[1].got_write) {
      if (used) {
        ...
        if (!ret) {
          ...
          memmove(buffer, buffer + nwritten, sizeof buffer - nwritten);
          used -= nwritten;
        }
      ...
      if (!used) fds[1].ignore = 1; /* No need to send data.  */
    }
