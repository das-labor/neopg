...
static char *get_locale_dir(void) {
  ...
  if (!GetModuleFileNameW(NULL, moddir, MAX_PATH)) *moddir = 0;
...
#define SLDIR "\\share\\locale"
  if (*moddir) {
    ...
    if (result) {
      nbytes = WideCharToMultiByte(CP_UTF8, 0, moddir, -1, result, nbytes, NULL,
                                   NULL);
      if (nbytes < 0) {
        free(result);
        result = NULL;
...
static void drop_locale_dir(char *locale_dir) { free(locale_dir); }
