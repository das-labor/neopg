...
static _gpgrt_lock_t *get_lock_object(gpgrt_lock_t *lockhd) {
  ...
  if (lock->vers != LOCK_ABI_VERSION) abort();
...
gpg_err_code_t _gpgrt_lock_init(gpgrt_lock_t *lockhd) {
  ...
  if (!lock->vers) {
    if (sizeof(gpgrt_lock_t) < sizeof(_gpgrt_lock_t)) abort();
...
gpg_err_code_t _gpgrt_lock_trylock(gpgrt_lock_t *lockhd) {
  ...
  if (!TryEnterCriticalSection(&lock->csec)) return GPG_ERR_EBUSY;
  return 0;
}
...
gpg_err_code_t _gpgrt_lock_unlock(gpgrt_lock_t *lockhd) {
  ...
  if (!lock->initdone) return GPG_ERR_INV_LOCK_OBJ;
