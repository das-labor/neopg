...
/* Return an XPG style locale name
   ...
   allocated.  */
static const char *my_nl_locale_name(const char *categoryname) {
  const char *retval;
  LCID lcid;
  LANGID langid;
  int primary, sub;
...
static GPG_ERR_INLINE uint32_t do_swap_u32(uint32_t i) {
...
void _gpg_w32__init_gettext_module(void) { module_init(); }
#endif

/* Free the domain data.  */
static void free_domain(struct loaded_domain *domain) {
...
static struct loaded_domain *load_domain(const char *filename) {
  HANDLE fh;
  DWORD size;
  ...
  switch (SWAPIT(domain->must_swap, data->revision)) {
    case MO_REVISION_NUMBER: {
      ...
      if (nstrings > 65534) goto bailout;
      domain->nstrings = nstrings;
      domain->orig_tab =
          (struct string_desc *)((char *)data + SWAPIT(domain->must_swap,
      ...
      domain->hash_tab =
          (uint32_t *)((char *)data +
                       SWAPIT(domain->must_swap, data->hash_tab_offset));
    } break;
  ...
  if (domain->mapped) return domain; /* Okay.  */
...
/* Return a malloced string encoded in the native console codepage
   ...
   is not defined. */
static char *wchar_to_native(const wchar_t *string, size_t length,
                             size_t *retlen) {
  int n;
  char *result;
...
static char *utf8_to_native(const char *string, size_t length, size_t *retlen) {
  wchar_t *wstring;
  char *result;
  size_t newlen;
...
const char *_gpg_w32_bindtextdomain(const char *domainname,
  ...
  {
    int len = (strlen(dirname) + 1 + strlen(catval) + 13 + strlen(domainname) +
               3 + 1);
    char *p;
  ...
  /* Store the domain information in the domainlist.  */
  {
    ...
    {
      for (dl = domainlist; dl; dl = dl->next)
        if (!strcmp(dl->name, domainname)) break;
...
static const char *get_plural(const char *data, size_t datalen,
  ...
  for (; idx; idx--) {
    p = strchr(data, 0) + 1;
    if (p >= data + datalen) return "ERROR in GETTEXT (bad plural entry)";
    datalen -= (p - data);
    data = p;
  }
  return data;
...
static const char *get_string(struct loaded_domain *domain, uint32_t idx,
  ...
  if (idx > 65534) return "ERROR in GETTEXT (too many strings)";
  ...
  if (tls->gt_use_utf8) {
    ...
    if (translen) translen--; /* TRANSLEN shall be the size without the nul.  */
    jnlib_free(buf);
...
static const char *do_gettext(const char *domainname, const char *msgid,
                              const char *msgid2, unsigned long nplural) {
  struct domainlist_s *dl;
  ...
  if (!domainname) domainname = current_domainname ? current_domainname : "";
  ...
  EnterCriticalSection(&domainlist_access_cs);
  {
    ...
    if (dl && !domain && !load_failed && dl->fname) {
      ...
      if (filename) strcpy(filename, dl->fname);
    }
  }
  LeaveCriticalSection(&domainlist_access_cs);
  if (!dl) goto not_found; /* DOMAINNAME not bound.  */
  if (filename) {
  ...
  if (!domain) goto not_found; /* No MO file.  */
  ...
  if (domain->hash_size > 2 && domain->hash_tab) {
    ...
    while ((nstr = SWAPIT(domain->must_swap, domain->hash_tab[idx]))) {
      ...
      if (nstr < domain->nstrings &&
          SWAPIT(domain->must_swap, domain->orig_tab[nstr].length) >= len &&
          !strcmp(msgid,
                  (domain->data +
                   SWAPIT(domain->must_swap, domain->orig_tab[nstr].offset)))) {
  ...
  while (bottom < top) {
    ...
    nstr = (bottom + top) / 2;
    cmp_val =
        strcmp(msgid, (domain->data + SWAPIT(domain->must_swap,
...
const char *_gpg_w32_textdomain(const char *domainname) {
  ...
  if (!domainname) {
    if (!current_domainname) gpg_err_set_errno(0);
  } else {
...
int _gpg_w32_gettext_use_utf8(int value) {
  ...
  if (value != -1) tls->gt_use_utf8 = value;
  return last;
}
