...
static DWORD CALLBACK reader(void *arg) {
  ...
  for (;;) {
    ...
    nbytes = (ctx->readpos + READBUF_SIZE - ctx->writepos - 1) % READBUF_SIZE;
    assert(nbytes);
    ...
    if (!SetEvent(ctx->have_data_ev))
      trace(("%p: SetEvent (%p) failed: ec=%d", ctx, ctx->have_data_ev,
             (int)GetLastError()));
    LeaveCriticalSection(&ctx->mutex);
  }
...
static struct reader_context_s *create_reader(
    estream_cookie_w32_pollable_t pcookie) {
  ...
  if (!ctx->have_data_ev || !ctx->have_space_ev || !ctx->close_ev) {
    ...
    if (ctx->have_data_ev) CloseHandle(ctx->have_data_ev);
    if (ctx->have_space_ev) CloseHandle(ctx->have_space_ev);
    if (ctx->close_ev) CloseHandle(ctx->close_ev);
    _gpgrt_free(ctx);
    return NULL;
  }
...
/* Prepare destruction of the reader thread for CTX.  Returns 0 if a
   ...
   not be called.  */
static void destroy_reader(struct reader_context_s *ctx) {
  EnterCriticalSection(&ctx->mutex);
  ctx->stop_me = 1;
  if (ctx->have_space_ev) SetEvent(ctx->have_space_ev);
  LeaveCriticalSection(&ctx->mutex);
 ...
 */
static gpgrt_ssize_t func_w32_pollable_read(void *cookie, void *buffer,
                                            size_t count) {
  ...
  nread = ctx->readpos < ctx->writepos ? ctx->writepos - ctx->readpos
  ...
  if (!SetEvent(ctx->have_space_ev)) {
    trace(("%p: SetEvent (%p) failed: ec=%d", cookie, ctx->have_space_ev,
           (int)GetLastError()));
    LeaveCriticalSection(&ctx->mutex);
    /* FIXME: Should translate the error code.  */
    _gpg_err_set_errno(EIO);
...
static DWORD CALLBACK writer(void *arg) {
...
static struct writer_context_s *create_writer(
    estream_cookie_w32_pollable_t pcookie) {
  ...
  if (ctx->have_data) ctx->is_empty = CreateEvent(&sec_attr, TRUE, TRUE, NULL);
  if (ctx->is_empty) ctx->close_ev = CreateEvent(&sec_attr, TRUE, FALSE, NULL);
  if (!ctx->have_data || !ctx->is_empty || !ctx->close_ev) {
    trace(("%p: CreateEvent failed: ec=%d", ctx, (int)GetLastError()));
    if (ctx->have_data) CloseHandle(ctx->have_data);
    if (ctx->is_empty) CloseHandle(ctx->is_empty);
    if (ctx->close_ev) CloseHandle(ctx->close_ev);
    _gpgrt_free(ctx);
    return NULL;
  }
...
static void destroy_writer(struct writer_context_s *ctx) {
  trace(("%p: enter pollable_destroy_writer", ctx));
  EnterCriticalSection(&ctx->mutex);
  trace(("%p: setting stopme", ctx));
  ctx->stop_me = 1;
  if (ctx->have_data) SetEvent(ctx->have_data);
  LeaveCriticalSection(&ctx->mutex);
 ...
 */
static gpgrt_ssize_t func_w32_pollable_write(void *cookie, const void *buffer,
                                             size_t count) {
  estream_cookie_w32_pollable_t pcookie = cookie;
  ...
  EnterCriticalSection(&ctx->mutex);
  trace(("%p: buffer: %p, count: %d, nbytes: %d", cookie, buffer, count,
         ctx->nbytes));
  ...
  if (count > WRITEBUF_SIZE) count = WRITEBUF_SIZE;
...
int _gpgrt_w32_poll(gpgrt_poll_t *fds, size_t nfds, int timeout) {
 ...
 restart:
  ...
  trace_start(("poll on [ "));
  for (i = 0; i < nwait; i++) trace_append(("%d/%c ", waitidx[i], waitinfo[i]));
  trace_finish(("]"));
  if (!any) return 0;
 ...
 */
static int func_w32_pollable_ioctl(void *cookie, int cmd, void *ptr,
                                   size_t *len) {
  estream_cookie_w32_pollable_t pcookie = cookie;
  ...
  if (func_ioctl) return func_ioctl(pcookie->next_cookie, cmd, ptr, len);
 ...
 */
static int func_w32_pollable_destroy(void *cookie) {
  estream_cookie_w32_pollable_t pcookie = cookie;

  if (cookie) {
    if (pcookie->reader) destroy_reader(pcookie->reader);
    if (pcookie->writer) destroy_writer(pcookie->writer);
...
struct cookie_io_functions_s _gpgrt_functions_w32_pollable = {
    {
        func_w32_pollable_read, func_w32_pollable_write, func_w32_pollable_seek,
