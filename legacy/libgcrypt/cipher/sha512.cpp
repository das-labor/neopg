...
typedef struct { u64 h0, h1, h2, h3, h4, h5, h6, h7; } SHA512_STATE;
...
static inline u64 ROTR(u64 x, u64 n) { return ((x >> n) | (x << (64 - n))); }

static inline u64 Ch(u64 x, u64 y, u64 z) { return ((x & y) ^ (~x & z)); }
...
static unsigned int transform_blk(SHA512_STATE *hd, const unsigned char *data) {
  ...
  for (t = 0; t < 16; t++) w[t] = buf_get_be64(data + t * 8);
...
static void sha512_final(void *context) {
  ...
  if (hd->bctx.count < 112) {              /* enough room */
    hd->bctx.buf[hd->bctx.count++] = 0x80; /* pad */
    while (hd->bctx.count < 112) hd->bctx.buf[hd->bctx.count++] = 0; /* pad */
  } else {                                 /* need one extra block */
    hd->bctx.buf[hd->bctx.count++] = 0x80; /* pad character */
    while (hd->bctx.count < 128) hd->bctx.buf[hd->bctx.count++] = 0;
    _gcry_md_block_write(context, NULL, 0); /* flush */
    ;
...
static gpg_error_t selftests_sha384(int extended,
...
failed:
  if (report) report("digest", GCRY_MD_SHA384, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}

static gpg_error_t selftests_sha512(int extended,
                                    selftest_report_func_t report) {
  const char *what;
  const char *errtxt;
...
failed:
  if (report) report("digest", GCRY_MD_SHA512, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}
