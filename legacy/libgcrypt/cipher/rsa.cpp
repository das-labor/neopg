...
static const char *rsa_names[] = {
    "rsa", "openpgp-rsa", "oid.1.2.840.113549.1.1.1", NULL,
};
 ...
 */
static gpg_error_t generate_std(RSA_secret_key *sk, unsigned int nbits,
 ...
 * Returns: 2 structures filled with all needed values
 */
static gpg_error_t generate_fips(RSA_secret_key *sk, unsigned int nbits,
  ...
  if (testparms) {
    ...
    for (idx = 0; tbl[idx].name; idx++)
      if (!*tbl[idx].value) break;
    if (tbl[idx].name) {
      /* At least one parameter is missing.  */
      for (idx = 0; tbl[idx].name; idx++) _gcry_mpi_release(*tbl[idx].value);
...
/* Variant of the standard key generation code using the algorithm
   ...
   testing.  */
static gpg_error_t generate_x931(RSA_secret_key *sk, unsigned int nbits,
                                 unsigned long e_value, gcry_sexp_t deriveparms,
                                 int *swapped) {
  gcry_mpi_t p, q; /* The two primes.  */
  ...
  *swapped = 0;
  ...
  if (nbits < 1024 || (nbits % 256)) return GPG_ERR_INV_VALUE;
  ...
  /* Point 2:  2 <= bitlength(e) < 2^{k-2}
     ...
     that limit.  */
  if (e_value < 3) return GPG_ERR_INV_VALUE;

  /* Our implementation requires E to be odd.  */
  if (!(e_value & 1)) return GPG_ERR_INV_VALUE;
  ...
  /* Compute or extract the derive parameters.  */
  {
    ...
    if (!deriveparms) {
      ...
      struct {
        const char *name;
        gcry_mpi_t *value;
      } tbl[] = {{"Xp1"}, {"Xp2"}, {"Xp"}, {"Xq1"}, {"Xq2"}, {"Xq"}, {NULL}};
      int idx;
      gcry_sexp_t oneparm;
  ...
  if (DBG_CIPHER) {
    if (*swapped) log_debug("p and q are swapped\n");
    log_mpidump("  p", p);
    log_mpidump("  q", q);
    log_mpidump("  n", n);
    log_mpidump("  e", e);
 ...
 */
static void public_x(gcry_mpi_t output, gcry_mpi_t input,
                     RSA_public_key *pkey) {
...
static void secret_core_crt(gcry_mpi_t M, gcry_mpi_t C, gcry_mpi_t D,
  ...
  /* h = u * ( m2 - m1 ) mod q */
  mpi_sub(h, m2, m1);
  if (mpi_has_sign(h)) mpi_add(h, h, Q);
  mpi_mulm(h, U, h, Q);
...
static void secret(gcry_mpi_t output, gcry_mpi_t input, RSA_secret_key *skey) {
  ...
  if (!skey->p || !skey->q || !skey->u) {
    ...
    secret_core_crt(output, input, skey->d, mpi_get_nlimbs(skey->n), skey->p,
                    skey->q, skey->u);
  }
}
...
static gpg_error_t rsa_generate(const gcry_sexp_t genparms,
                                gcry_sexp_t *r_skey) {
  gpg_error_t ec;
  unsigned int nbits;
  ...
  if (!ec) {
    ec = sexp_build(r_skey, NULL,
                    ...
                    " %S)",
                    sk.n, sk.e, sk.n, sk.e, sk.d, sk.p, sk.q, sk.u, swap_info);
  }
...
static gpg_error_t rsa_check_secret_key(gcry_sexp_t keyparms) {
  gpg_error_t rc;
  RSA_secret_key sk = {NULL, NULL, NULL, NULL, NULL, NULL};

  /* To check the key we need the optional parameters. */
  rc = sexp_extract_param(keyparms, NULL, "nedpqu", &sk.n, &sk.e, &sk.d, &sk.p,
                          &sk.q, &sk.u, NULL);
  if (rc) goto leave;
...
leave:
  ...
  _gcry_mpi_release(sk.q);
  _gcry_mpi_release(sk.u);
  if (DBG_CIPHER) log_debug("rsa_testkey    => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t rsa_encrypt(gcry_sexp_t *r_ciph, gcry_sexp_t s_data,
  ...
  if (rc) goto leave;
  if (DBG_CIPHER) log_mpidump("rsa_encrypt data", data);
  ...
  if (DBG_CIPHER) log_mpidump("rsa_encrypt  res", ciph);
...
leave:
  ...
  if (DBG_CIPHER) log_debug("rsa_encrypt    => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t rsa_decrypt(gcry_sexp_t *r_plain, gcry_sexp_t s_data,
                               gcry_sexp_t keyparms)

{
  ...
  if (rc) goto leave;
  if (DBG_CIPHER) log_printmpi("rsa_decrypt data", data);
  ...
  if (DBG_CIPHER) log_printmpi("rsa_decrypt  res", plain);
  ...
  switch (ctx.encoding) {
    ...
    case PUBKEY_ENC_OAEP:
      rc = _gcry_rsa_oaep_decode(&unpad, &unpadlen, ctx.nbits, ctx.hash_algo,
    ...
    default:
      ...
      rc = sexp_build(
          r_plain, NULL,
          (ctx.flags & PUBKEY_FLAG_LEGACYRESULT) ? "%m" : "(value %m)", plain);
      break;
  }
...
leave:
  ...
  sexp_release(l1);
  _gcry_pk_util_free_encoding_ctx(&ctx);
  if (DBG_CIPHER) log_debug("rsa_decrypt    => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t rsa_sign(gcry_sexp_t *r_sig, gcry_sexp_t s_data,
                            gcry_sexp_t keyparms) {
  gpg_error_t rc;
  struct pk_encoding_ctx ctx;
  gcry_mpi_t data = NULL;
  ...
  rc = _gcry_pk_util_data_to_mpi(s_data, &data, &ctx);
  if (rc) goto leave;
  if (DBG_CIPHER) log_printmpi("rsa_sign   data", data);
  if (mpi_is_opaque(data)) {
    rc = GPG_ERR_INV_DATA;
    goto leave;
  }
  ...
  if (DBG_CIPHER) log_printmpi("rsa_sign    res", sig);
...
leave:
  ...
  _gcry_mpi_release(data);
  _gcry_pk_util_free_encoding_ctx(&ctx);
  if (DBG_CIPHER) log_debug("rsa_sign      => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t rsa_verify(gcry_sexp_t s_sig, gcry_sexp_t s_data,
                              gcry_sexp_t keyparms) {
  gpg_error_t rc;
  struct pk_encoding_ctx ctx;
  gcry_sexp_t l1 = NULL;
  ...
  rc = _gcry_pk_util_data_to_mpi(s_data, &data, &ctx);
  if (rc) goto leave;
  if (DBG_CIPHER) log_printmpi("rsa_verify data", data);
  if (mpi_is_opaque(data)) {
    rc = GPG_ERR_INV_DATA;
    goto leave;
  }
  ...
  if (rc) goto leave;
  if (DBG_CIPHER) log_printmpi("rsa_verify  sig", sig);
  ...
  result = mpi_new(0);
  public_x(result, sig, &pk);
  if (DBG_CIPHER) log_printmpi("rsa_verify  cmp", result);
  if (ctx.verify_cmp)
...
static unsigned int rsa_get_nbits(gcry_sexp_t parms) {
  ...
  if (!l1) return 0; /* Parameter N not found.  */
...
static gpg_error_t compute_keygrip(gcry_md_hd_t md, gcry_sexp_t keyparam) {
  gcry_sexp_t l1;
  const char *data;
  size_t datalen;

  l1 = sexp_find_token(keyparam, "n", 1);
  if (!l1) return GPG_ERR_NO_OBJ;
...
static gpg_error_t selftests_rsa(selftest_report_func_t report) {
  const char *what;
  const char *errtxt;
  gpg_error_t err;
  ...
  if (!err)
    err = sexp_sscan(&pkey, NULL, sample_public_key, strlen(sample_public_key));
  if (err) {
...
failed:
  sexp_release(pkey);
  sexp_release(skey);
  if (report) report("pubkey", GCRY_PK_RSA, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}
