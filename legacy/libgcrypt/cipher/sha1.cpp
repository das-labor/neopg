 ...
 */
void _gcry_sha1_mixblock_init(SHA1_CONTEXT *hd) { sha1_init(hd, 0); }
...
static gpg_error_t selftests_sha1(int extended, selftest_report_func_t report) {
  const char *what;
  const char *errtxt;

  what = "short string";
  errtxt =
      _gcry_hash_selftest_check_one(GCRY_MD_SHA1, 0, "abc", 3,
...
failed:
  if (report) report("digest", GCRY_MD_SHA1, what, errtxt);
