...
static const char *elg_names[] = {
    "elg", "openpgp-elg", "openpgp-elg-sig", NULL,
};
...
static void progress(int c) {
  if (progress_cb) progress_cb(progress_cb_data, "pk_elg", c, 0, 0);
}
...
static int test_keys(ELG_secret_key *sk, unsigned int nbits, int nodie) {
  ...
  if (mpi_cmp(test, out2)) failed |= 1;

  sign(out1_a, out1_b, test, sk);
  if (!verify(out1_a, out1_b, test, &pk)) failed |= 2;
...
static gcry_mpi_t gen_k(gcry_mpi_t p, int small_k) {
  ...
  nbytes = (nbits + 7) / 8;
  if (DBG_CIPHER) log_debug("choosing a random k\n");
  mpi_sub_ui(p_1, p, 1);
  for (;;) {
    if (!rndbuf || nbits < 32) {
      xfree(rndbuf);
 ...
 */
static gpg_error_t generate(ELG_secret_key *sk, unsigned int nbits,
...
static gpg_error_t generate_using_x(ELG_secret_key *sk, unsigned int nbits,
  ...
  if (xbits < 64 || xbits >= nbits) return GPG_ERR_INV_VALUE;
  ...
  if (DBG_CIPHER) log_debug("using a supplied x of size %u", xbits);
...
static gpg_error_t elg_generate(const gcry_sexp_t genparms,
                                gcry_sexp_t *r_skey) {
  gpg_error_t rc;
  unsigned int nbits;
  ELG_secret_key sk;
  ...
  if (l1) {
    ...
    sexp_release(l1);
    if (!xvalue) return GPG_ERR_BAD_MPI;
  }
...
leave:
  ...
  if (factors) {
    gcry_mpi_t *mp;
    for (mp = factors; *mp; mp++) mpi_free(*mp);
    xfree(factors);
  }
...
static gpg_error_t elg_check_secret_key(gcry_sexp_t keyparms) {
  gpg_error_t rc;
...
leave:
  ...
  _gcry_mpi_release(sk.y);
  _gcry_mpi_release(sk.x);
  if (DBG_CIPHER) log_debug("elg_testkey    => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t elg_encrypt(gcry_sexp_t *r_ciph, gcry_sexp_t s_data,
  ...
  if (rc) goto leave;
  if (DBG_CIPHER) log_mpidump("elg_encrypt data", data);
...
leave:
  ...
  if (DBG_CIPHER) log_debug("elg_encrypt   => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t elg_decrypt(gcry_sexp_t *r_plain, gcry_sexp_t s_data,
                               gcry_sexp_t keyparms) {
  gpg_error_t rc;
  struct pk_encoding_ctx ctx;
  gcry_sexp_t l1 = NULL;
  ...
  if (DBG_CIPHER) log_printmpi("elg_decrypt  res", plain);
...
leave:
  ...
  sexp_release(l1);
  _gcry_pk_util_free_encoding_ctx(&ctx);
  if (DBG_CIPHER) log_debug("elg_decrypt    => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t elg_sign(gcry_sexp_t *r_sig, gcry_sexp_t s_data,
                            gcry_sexp_t keyparms) {
  gpg_error_t rc;
  struct pk_encoding_ctx ctx;
  gcry_mpi_t data = NULL;
  ...
  rc = _gcry_pk_util_data_to_mpi(s_data, &data, &ctx);
  if (rc) goto leave;
  if (DBG_CIPHER) log_mpidump("elg_sign   data", data);
  if (mpi_is_opaque(data)) {
    rc = GPG_ERR_INV_DATA;
    goto leave;
  }
...
leave:
  ...
  _gcry_mpi_release(data);
  _gcry_pk_util_free_encoding_ctx(&ctx);
  if (DBG_CIPHER) log_debug("elg_sign      => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t elg_verify(gcry_sexp_t s_sig, gcry_sexp_t s_data,
  ...
  rc = _gcry_pk_util_data_to_mpi(s_data, &data, &ctx);
  if (rc) goto leave;
  if (DBG_CIPHER) log_mpidump("elg_verify data", data);
  if (mpi_is_opaque(data)) {
    rc = GPG_ERR_INV_DATA;
    goto leave;
  }
  ...
  if (!verify(sig_r, sig_s, data, &pk)) rc = GPG_ERR_BAD_SIGNATURE;
...
static unsigned int elg_get_nbits(gcry_sexp_t parms) {
  ...
  if (!l1) return 0; /* Parameter P not found.  */
