...
static const struct whirlpool_tables_s tab = {
    /* Round constants.  */
    {
        U64_C(0x1823c6e887b8014f), U64_C(0x36a6d2f5796f9152),
        U64_C(0x60bc9b8ea30c7b35), U64_C(0x1de0d7c22e4bfe57),
        U64_C(0x157737e59ff04ada), U64_C(0x58c9290ab1a06b85),
        U64_C(0xbd5d10f4cb3e0567), U64_C(0xe427418ba77d95d8),
        U64_C(0xfbee7c66dd17479e), U64_C(0xca2dbf07ad5a8333),
    },
    /* Main lookup boxes.  */
    {{
...
static void whirlpool_add_bugemu(whirlpool_context_t *context,
  ...
  if (!buffer) return; /* Nothing to add.  */
  ...
  if (context->bugemu.count) {
    ...
    if (!buffer_n) return; /* Done.  This is the bug we emulate.  */
  }
...
static void whirlpool_final_bugemu(void *ctx) {
  ...
  memcpy(context->bctx.buf + context->bugemu.count, context->bugemu.length, 32);
...
static void whirlpool_final(void *ctx) {
  ...
  /* add the count */
  t = lsb;
  if ((lsb += context->bctx.count) < t) msb++;
  ...
  while (context->bctx.count < 32) context->bctx.buf[context->bctx.count++] = 0;
