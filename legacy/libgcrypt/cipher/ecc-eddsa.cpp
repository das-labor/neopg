...
static gpg_error_t eddsa_encodempi(gcry_mpi_t mpi, unsigned int minlen,
  ...
  if (!rawmpi) return gpg_error_from_syserror();
...
/* Encode (X,Y) using the EdDSA scheme.  MINLEN is the required length
   ...
   malloced buffer with the encoded point is stored at R_BUFFER; the
   length of this buffer is stored at R_BUFLEN.  */
static gpg_error_t eddsa_encode_x_y(gcry_mpi_t x, gcry_mpi_t y,
...
gpg_error_t _gcry_ecc_eddsa_encodepoint(mpi_point_t point, mpi_ec_t ec,
...
gpg_error_t _gcry_ecc_eddsa_ensure_compact(gcry_mpi_t value,
                                           unsigned int nbits) {
  gpg_error_t rc;
  ...
  if (!mpi_is_opaque(value)) return GPG_ERR_INV_OBJ;
  buf = (const unsigned char *)mpi_get_opaque(value, &rawmpilen);
  if (!buf) return GPG_ERR_INV_OBJ;
  ...
  if (rawmpilen > 1 && (rawmpilen % 2)) {
    if (buf[0] == 0x04) {
      /* Buffer is in SEC1 uncompressed format.  Extract y and
         compress.  */
      rc = _gcry_mpi_scan(&x, GCRYMPI_FMT_STD, buf + 1, (rawmpilen - 1) / 2,
                          NULL);
      if (rc) return rc;
      rc = _gcry_mpi_scan(&y, GCRYMPI_FMT_STD, buf + 1 + (rawmpilen - 1) / 2,
                          (rawmpilen - 1) / 2, NULL);
      if (rc) {
        mpi_free(x);
        return rc;
...
gpg_error_t _gcry_ecc_eddsa_recover_x(gcry_mpi_t x, gcry_mpi_t y, int sign,
                                      mpi_ec_t ec) {
  gpg_error_t rc = 0;
  gcry_mpi_t u, v, v3, t;
  static gcry_mpi_t p58, seven;

  if (ec->dialect != ECC_DIALECT_ED25519) return GPG_ERR_NOT_IMPLEMENTED;

  if (!p58)
    p58 = scanval(
        "0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"
        "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFD");
  if (!seven) seven = mpi_set_ui(NULL, 7);
  ...
  if (!mpi_cmp(t, u)) {
    ...
    /* -t == u ? x = x * sqrt(-1) */
    mpi_sub(t, ec->p, t);
    if (!mpi_cmp(t, u)) rc = GPG_ERR_INV_OBJ;
  }

  /* Choose the desired square root according to parity */
  if (mpi_test_bit(x, 0) != !!sign) mpi_sub(x, ec->p, x);
...
gpg_error_t _gcry_ecc_eddsa_decodepoint(gcry_mpi_t pk, mpi_ec_t ctx,
  ...
  if (mpi_is_opaque(pk)) {
    ...
    if (rawmpilen > 1 && (rawmpilen % 2)) {
      ...
      if (buf[0] == 0x04) {
        ...
        if (r_encpk) {
          rc = eddsa_encode_x_y(x, y, ctx->nbits / 8, 0, r_encpk, r_encpklen);
          if (rc) {
            mpi_free(x);
            mpi_free(y);
            return rc;
  ...
  if (r_encpk) {
    ...
    if (sign && rawmpilen) rawmpi[0] |= 0x80;
    reverse_buffer(rawmpi, rawmpilen);
    *r_encpk = rawmpi;
    if (r_encpklen) *r_encpklen = rawmpilen;
  } else
    xfree(rawmpi);
...
/* Compute the A value as used by EdDSA.  The caller needs to provide
   ...
   stored at R_DIGEST.  */
gpg_error_t _gcry_ecc_eddsa_compute_h_d(unsigned char **r_digest, gcry_mpi_t d,
                                        mpi_ec_t ec) {
  gpg_error_t rc;
  ...
  *r_digest = NULL;
  ...
  if (hashalgo != GCRY_MD_SHA512) return GPG_ERR_DIGEST_ALGO;

  b = (ec->nbits + 7) / 8;
  if (b != 256 / 8) return GPG_ERR_INTERNAL; /* We only support 256 bit. */

  /* Note that we clear DIGEST so we can use it as input to left pad
     the key with zeroes for hashing.  */
  digest = (unsigned char *)xtrycalloc_secure(2, b);
  if (!digest) return gpg_error_from_syserror();
 ...
 * to use a faster RNG.
 */
gpg_error_t _gcry_ecc_eddsa_genkey(ECC_secret_key *sk, elliptic_curve_t *E,
  ...
  if (DBG_CIPHER) log_printpnt("ecgen      pk", &Q, ctx);
 ...
 */
gpg_error_t _gcry_ecc_eddsa_sign(gcry_mpi_t input, ECC_secret_key *skey,
  ...
  if (pk) {
    ...
    if (rc) goto leave;
    if (DBG_CIPHER) log_printhex("* e_pk", encpk, encpklen);
  ...
  mlen = (tmp + 7) / 8;
  if (DBG_CIPHER) log_printhex("     m", mbuf, mlen);
 ...
 */
gpg_error_t _gcry_ecc_eddsa_verify(gcry_mpi_t input, ECC_public_key *pkey,
  ...
  rlen = (tmp + 7) / 8;
  if (DBG_CIPHER) log_printhex("     r", rbuf, rlen);
  if (rlen != b) {
    rc = GPG_ERR_INV_LENGTH;
    goto leave;
  }
     ...
     because we don't need to decode R. */
  {
    ...
    if (DBG_CIPHER) log_printhex("     s", sbuf, slen);
