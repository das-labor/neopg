...
static gpg_error_t do_twofish_setkey(TWOFISH_context *ctx, const byte *key,
...
size_t _gcry_twofish_ocb_crypt(gcry_cipher_hd_t c, void *outbuf_arg,
...

  {
    ...
    while (nblocks >= 3) {
      ...
      if (encrypt)
        twofish_amd64_ocb_enc(ctx, outbuf, inbuf, c->u_iv.iv, c->u_ctr.ctr, Ls);
      else
        twofish_amd64_ocb_dec(ctx, outbuf, inbuf, c->u_iv.iv, c->u_ctr.ctr, Ls);
  ...
  if (burn_stack_depth) _gcry_burn_stack(burn_stack_depth + 4 * sizeof(void *));
#else
  (void)c;
  (void)outbuf_arg;
  (void)inbuf_arg;
...
static const char *selftest_ctr(void) {
  ...
  return _gcry_selftest_helper_ctr("TWOFISH", &twofish_setkey, &twofish_encrypt,
                                   &_gcry_twofish_ctr_enc, nblocks, blocksize,
                                   context_size);
}
...
static const char *selftest_cbc(void) {
  ...
  return _gcry_selftest_helper_cbc("TWOFISH", &twofish_setkey, &twofish_encrypt,
                                   &_gcry_twofish_cbc_dec, nblocks, blocksize,
                                   context_size);
}
...
static const char *selftest_cfb(void) {
  ...
  return _gcry_selftest_helper_cfb("TWOFISH", &twofish_setkey, &twofish_encrypt,
                                   &_gcry_twofish_cfb_dec, nblocks, blocksize,
                                   context_size);
}
...
static const char *selftest(void) {
  ...
  static byte plaintext[16] = {0xD4, 0x91, 0xDB, 0x16, 0xE7, 0xB1, 0xC3, 0x9E,
...
int main() {
  ...
  for (i = 0; i < 125; i++) {
    ...
    for (j = 0; j < 1000; j++) twofish_encrypt(&ctx, buffer[2], buffer[2]);
    twofish_setkey(&ctx, buffer[1], sizeof(buffer[1]));
    for (j = 0; j < 1000; j++) twofish_encrypt(&ctx, buffer[3], buffer[3]);
  ...
  for (i = 0; i < 125; i++) {
    ...
    twofish_setkey(&ctx, buffer[1], sizeof(buffer[1]));
    for (j = 0; j < 1000; j++) twofish_decrypt(&ctx, buffer[3], buffer[3]);
    twofish_setkey(&ctx, buffer[0], sizeof(buffer[0]));
    for (j = 0; j < 1000; j++) twofish_decrypt(&ctx, buffer[2], buffer[2]);
  }
