...
static gpg_error_t hmac_open(gcry_mac_hd_t h) {
  gpg_error_t err;
  gcry_md_hd_t hd;
...
static gpg_error_t hmac_setkey(gcry_mac_hd_t h, const unsigned char *key,
                               size_t keylen) {
  return _gcry_md_setkey(h->u.hmac.md_ctx, key, keylen);
}

static gpg_error_t hmac_reset(gcry_mac_hd_t h) {
  _gcry_md_reset(h->u.hmac.md_ctx);
  return 0;
}

static gpg_error_t hmac_write(gcry_mac_hd_t h, const unsigned char *buf,
                              size_t buflen) {
  _gcry_md_write(h->u.hmac.md_ctx, buf, buflen);
  return 0;
}

static gpg_error_t hmac_read(gcry_mac_hd_t h, unsigned char *outbuf,
                             size_t *outlen) {
  unsigned int dlen;
...
static gpg_error_t hmac_verify(gcry_mac_hd_t h, const unsigned char *buf,
                               size_t buflen) {
  unsigned int dlen;
  const unsigned char *digest;
  ...
  if (buflen > dlen) return GPG_ERR_INV_LENGTH;
