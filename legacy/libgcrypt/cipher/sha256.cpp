...
static gpg_error_t selftests_sha224(int extended,
...
failed:
  if (report) report("digest", GCRY_MD_SHA224, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}

static gpg_error_t selftests_sha256(int extended,
                                    selftest_report_func_t report) {
  const char *what;
  const char *errtxt;
...
failed:
  if (report) report("digest", GCRY_MD_SHA256, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}
