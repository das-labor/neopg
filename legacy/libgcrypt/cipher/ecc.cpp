 ...
 */
static gpg_error_t nist_generate_key(ECC_secret_key *sk, elliptic_curve_t *E,
...
static void test_keys(ECC_secret_key *sk, unsigned int nbits) {
  ...
  if (DBG_CIPHER) log_debug("Testing key.\n");
...
static void test_ecdh_only_keys(ECC_secret_key *sk, unsigned int nbits,
  ...
  if (DBG_CIPHER) log_debug("Testing ECDH only key.\n");
...
static int check_secret_key(ECC_secret_key *sk, mpi_ec_t ec, int flags) {
  ...
  if (!mpi_cmp_ui(sk->E.G.z, 0)) {
    if (DBG_CIPHER) log_debug("Bad check: 'G' cannot be Point at Infinity!\n");
    goto leave;
  }
  ...
  if (!_gcry_ecc_compute_public(&Q, ec, &sk->E.G, sk->d)) {
    if (DBG_CIPHER) log_debug("Bad check: computation of dG failed\n");
    goto leave;
  }
...
static gpg_error_t ecc_generate(const gcry_sexp_t genparms,
                                gcry_sexp_t *r_skey) {
  gpg_error_t rc;
  unsigned int nbits;
  elliptic_curve_t E;
  ...
  if (!nbits && !curve_name) return GPG_ERR_NO_OBJ; /* No NBITS parameter. */
  ...
  if (DBG_CIPHER) {
    ...
    if (E.name) log_debug("ecgen curve used: %s\n", E.name);
  ...
  if ((sk.E.dialect == ECC_DIALECT_ED25519 || E.model == MPI_EC_MONTGOMERY) &&
    ...
    if (E.model != MPI_EC_MONTGOMERY)
      ...
      rc = _gcry_ecc_eddsa_encodepoint(
          &sk.Q, ctx, Gx, Gy, !!(flags & PUBKEY_FLAG_COMP), &encpk, &encpklen);
    else {
      encpk = _gcry_mpi_get_buffer_extra(Qx, nbits / 8, -1, &encpklen, NULL);
      if (encpk == NULL)
        ...
        ((flags & PUBKEY_FLAG_PARAM) && (flags & PUBKEY_FLAG_EDDSA))
            ...
            : ((flags & PUBKEY_FLAG_PARAM) && (flags & PUBKEY_FLAG_EDDSA))
                  ...
                  : ((flags & PUBKEY_FLAG_PARAM))
                        ? "(flags param)"
                        : ((flags & PUBKEY_FLAG_EDDSA)) ? "(flags eddsa)"
                                                        : "(flags djb-tweak)");
    if (rc) goto leave;
  }
  ...
  if ((flags & PUBKEY_FLAG_PARAM) && E.name)
    rc = sexp_build(r_skey, NULL,
                    ...
                    sk.E.n, sk.E.h, public_x, curve_info, curve_flags, sk.E.p,
                    sk.E.a, sk.E.b, base, sk.E.n, sk.E.h, public_x, secret);
  else
    rc = sexp_build(r_skey, NULL,
                    "(key-data"
                    " (public-key"
                    ...
                    "  (ecc%S%S(q%m)(d%m)))"
                    " )",
                    curve_info, curve_flags, public_x, curve_info, curve_flags,
                    public_x, secret);
  if (rc) goto leave;
...
static gpg_error_t ecc_check_secret_key(gcry_sexp_t keyparms) {
  gpg_error_t rc;
  gcry_sexp_t l1 = NULL;
  int flags = 0;
  char *curvename = NULL;
  ...
  if ((flags & PUBKEY_FLAG_PARAM))
    rc = sexp_extract_param(keyparms, NULL, "-p?a?b?g?n?h?/q?+d", &sk.E.p,
                            &sk.E.a, &sk.E.b, &mpi_g, &sk.E.n, &sk.E.h, &mpi_q,
                            &sk.d, NULL);
  else
  ...
  if (!curvename) {
    ...
    if (!sk.E.h) sk.E.h = mpi_const(MPI_C_ONE);
  }
  if (DBG_CIPHER) {
    log_debug("ecc_testkey inf: %s/%s\n", _gcry_ecc_model2str(sk.E.model),
              _gcry_ecc_dialect2str(sk.E.dialect));
    if (sk.E.name) log_debug("ecc_testkey nam: %s\n", sk.E.name);
  ...
  if (check_secret_key(&sk, ec, flags)) rc = GPG_ERR_BAD_SECKEY;
...
leave:
  ...
  xfree(curvename);
  sexp_release(l1);
  if (DBG_CIPHER) log_debug("ecc_testkey   => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t ecc_sign(gcry_sexp_t *r_sig, gcry_sexp_t s_data,
  ...
  if (rc) goto leave;
  if (DBG_CIPHER) log_mpidump("ecc_sign   data", data);
  ...
  if (!curvename) {
    sk.E.model =
        ((ctx.flags & PUBKEY_FLAG_EDDSA) ? MPI_EC_EDWARDS : MPI_EC_WEIERSTRASS);
  ...
  if (DBG_CIPHER) {
    ...
    if (sk.E.name) log_debug("ecc_sign   name: %s\n", sk.E.name);
  ...
  if ((ctx.flags & PUBKEY_FLAG_EDDSA)) {
    ...
    if (!rc)
      rc = sexp_build(r_sig, NULL, "(sig-val(eddsa(r%M)(s%M)))", sig_r, sig_s);
  } else if ((ctx.flags & PUBKEY_FLAG_GOST)) {
    rc = _gcry_ecc_gost_sign(data, &sk, sig_r, sig_s);
    if (!rc)
      rc = sexp_build(r_sig, NULL, "(sig-val(gost(r%M)(s%M)))", sig_r, sig_s);
  } else {
    rc =
        _gcry_ecc_ecdsa_sign(data, &sk, sig_r, sig_s, ctx.flags, ctx.hash_algo);
    if (!rc)
      rc = sexp_build(r_sig, NULL, "(sig-val(ecdsa(r%M)(s%M)))", sig_r, sig_s);
  }
...
leave:
  ...
  if (DBG_CIPHER) log_debug("ecc_sign      => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t ecc_verify(gcry_sexp_t s_sig, gcry_sexp_t s_data,
  ...
  rc = _gcry_pk_util_data_to_mpi(s_data, &data, &ctx);
  if (rc) goto leave;
  if (DBG_CIPHER) log_mpidump("ecc_verify data", data);
  ...
  rc = sexp_extract_param(l1, NULL,
                          (sigflags & PUBKEY_FLAG_EDDSA) ? "/rs" : "rs", &sig_r,
                          &sig_s, NULL);
  if (rc) goto leave;
  if (DBG_CIPHER) {
  ...
  if ((ctx.flags & PUBKEY_FLAG_PARAM))
    rc = sexp_extract_param(s_keyparms, NULL, "-p?a?b?g?n?h?/q", &pk.E.p,
                            &pk.E.a, &pk.E.b, &mpi_g, &pk.E.n, &pk.E.h, &mpi_q,
                            NULL);
  else
  ...
  if (!curvename) {
    pk.E.model =
        ((sigflags & PUBKEY_FLAG_EDDSA) ? MPI_EC_EDWARDS : MPI_EC_WEIERSTRASS);
    pk.E.dialect = ((sigflags & PUBKEY_FLAG_EDDSA) ? ECC_DIALECT_ED25519
                                                   : ECC_DIALECT_STANDARD);
    if (!pk.E.h) pk.E.h = mpi_const(MPI_C_ONE);
  }
  ...
  if (DBG_CIPHER) {
    ...
    if (pk.E.name) log_debug("ecc_verify name: %s\n", pk.E.name);
  ...
  if ((sigflags & PUBKEY_FLAG_EDDSA)) {
    rc = _gcry_ecc_eddsa_verify(data, &pk, sig_r, sig_s, ctx.hash_algo, mpi_q);
    ...
    if (pk.E.dialect == ECC_DIALECT_ED25519) {
      ...
      ec = _gcry_mpi_ec_p_internal_new(pk.E.model, pk.E.dialect, 0, pk.E.p,
                                       pk.E.a, pk.E.b);
    ...
    if (mpi_is_opaque(data)) {
      ...
      if (!rc) {
        if (abits > qbits) mpi_rshift(a, a, abits - qbits);
 ...
 */
static gpg_error_t ecc_encrypt_raw(gcry_sexp_t *r_ciph, gcry_sexp_t s_data,
                                   gcry_sexp_t keyparms) {
  unsigned int nbits;
  gpg_error_t rc;
  struct pk_encoding_ctx ctx;
  ...
  if ((flags & PUBKEY_FLAG_DJB_TWEAK)) {
    ...
    for (i = 0; i < mpi_get_nbits(pk.E.h) - 1; i++) mpi_clear_bit(data, i);
    mpi_set_highbit(data, mpi_get_nbits(pk.E.p) - 1);
  }
  if (DBG_CIPHER) log_mpidump("ecc_encrypt data", data);

  if (DBG_CIPHER) {
    log_debug("ecc_encrypt info: %s/%s\n", _gcry_ecc_model2str(pk.E.model),
              _gcry_ecc_dialect2str(pk.E.dialect));
    if (pk.E.name) log_debug("ecc_encrypt name: %s\n", pk.E.name);
  ...
  /* The following is false: assert( mpi_cmp_ui( R.x, 1 )==0 );, so */
  {
    ...
    if (_gcry_mpi_ec_get_affine(x, y, &R, ec)) {
       ...
       */
      if (!(flags &
            PUBKEY_FLAG_DJB_TWEAK)) { /* It's not for X25519, then, the input
                                         data was simply wrong.  */
        rc = GPG_ERR_INV_DATA;
        goto leave;
      }
    }
...
leave:
  ...
  _gcry_mpi_ec_free(ec);
  _gcry_pk_util_free_encoding_ctx(&ctx);
  if (DBG_CIPHER) log_debug("ecc_encrypt    => %s\n", gpg_strerror(rc));
  return rc;
}
 ...
 */
static gpg_error_t ecc_decrypt_raw(gcry_sexp_t *r_plain, gcry_sexp_t s_data,
                                   gcry_sexp_t keyparms) {
  unsigned int nbits;
  gpg_error_t rc;
  struct pk_encoding_ctx ctx;
  ...
  if (rc) goto leave;
  if (DBG_CIPHER) log_printmpi("ecc_decrypt  d_e", data_e);
  ...
  if (DBG_CIPHER) {
    log_debug("ecc_decrypt info: %s/%s\n", _gcry_ecc_model2str(sk.E.model),
              _gcry_ecc_dialect2str(sk.E.dialect));
    if (sk.E.name) log_debug("ecc_decrypt name: %s\n", sk.E.name);
  ...
  if (DBG_CIPHER) log_printpnt("ecc_decrypt    kG", &kG, NULL);
  ...
  if (DBG_CIPHER) log_printmpi("ecc_decrypt  res", r);

  if (!rc) rc = sexp_build(r_plain, NULL, "(value %m)", r);
...
leave:
  ...
  _gcry_mpi_ec_free(ec);
  _gcry_pk_util_free_encoding_ctx(&ctx);
  if (DBG_CIPHER) log_debug("ecc_decrypt    => %s\n", gpg_strerror(rc));
  return rc;
}
...
static unsigned int ecc_get_nbits(gcry_sexp_t parms) {
  ...
  if (!l1) { /* Parameter P not found - check whether we have "curve".  */
    l1 = sexp_find_token(parms, "curve", 5);
    if (!l1) return 0; /* Neither P nor CURVE found.  */

    curve = sexp_nth_string(l1, 1);
    sexp_release(l1);
    if (!curve) return 0; /* No curve name given (or out of core). */

    if (_gcry_ecc_fill_in_curve(0, curve, NULL, &nbits)) nbits = 0;
    xfree(curve);
  } else {
...
static gpg_error_t compute_keygrip(gcry_md_hd_t md, gcry_sexp_t keyparms) {
#define N_COMPONENTS 7
  ...
  for (idx = 0; idx < N_COMPONENTS; idx++) values[idx] = NULL;
  ...
  if (l1) {
    curvename = sexp_nth_string(l1, 1);
    if (curvename) {
      rc = _gcry_ecc_update_curve_param(curvename, &model, &dialect, &values[0],
  ...
  /* Guess required fields if a curve parameter has not been given.
     FIXME: This is a crude hacks.  We need to fix that.  */
  if (!curvename) {
    model = ((flags & PUBKEY_FLAG_EDDSA) ? MPI_EC_EDWARDS : MPI_EC_WEIERSTRASS);
    dialect = ((flags & PUBKEY_FLAG_EDDSA) ? ECC_DIALECT_ED25519
                                           : ECC_DIALECT_STANDARD);
    if (!values[5]) values[5] = mpi_const(MPI_C_ONE);
  }
  ...
  for (idx = 0; idx < N_COMPONENTS; idx++) {
    ...
    if (idx == 5) continue; /* Skip cofactor. */
...
leave:
  xfree(curvename);
  sexp_release(l1);
  for (idx = 0; idx < N_COMPONENTS; idx++) _gcry_mpi_release(values[idx]);
...
gpg_error_t _gcry_pk_ecc_get_sexp(gcry_sexp_t *r_sexp, int mode, mpi_ec_t ec) {
  ...
  if (mode == GCRY_PK_GET_SECKEY && !ec->d) return GPG_ERR_NO_SECKEY;

  /* Compute the public point if it is missing.  */
  if (!ec->Q && ec->d) ec->Q = _gcry_ecc_compute_public(NULL, ec, NULL, NULL);
...
static const char *selftest_sign(gcry_sexp_t pkey, gcry_sexp_t skey) {
  ...
  if (!err) err = _gcry_mpi_scan(&r, GCRYMPI_FMT_HEX, signature_r, 0, NULL);
  if (!err) err = _gcry_mpi_scan(&s, GCRYMPI_FMT_HEX, signature_s, 0, NULL);
...
static gpg_error_t selftests_ecdsa(selftest_report_func_t report) {
  const char *what;
  const char *errtxt;
  gpg_error_t err;
...
failed:
  sexp_release(pkey);
  sexp_release(skey);
  if (report) report("pubkey", GCRY_PK_ECC, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}

/* Run a full self-test for ALGO and return 0 on success.  */
static gpg_error_t run_selftests(int algo, int extended,
                                 selftest_report_func_t report) {
  (void)extended;

  if (algo != GCRY_PK_ECC) return GPG_ERR_PUBKEY_ALGO;
