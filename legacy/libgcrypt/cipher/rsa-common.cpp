...
static gpg_error_t octet_string_from_mpi(unsigned char **r_frame, void *space,
...
gpg_error_t _gcry_rsa_pkcs1_encode_for_sig(gcry_mpi_t *r_result,
  ...
  if (!(frame = (byte *)xtrymalloc(nframe))) return gpg_error_from_syserror();
...
static gpg_error_t mgf1(unsigned char *output, size_t outlen,
...
gpg_error_t _gcry_rsa_oaep_encode(gcry_mpi_t *r_result, unsigned int nbits,
  ...
  *r_result = NULL;
  ...
  /* Step 2e and 2f: Create maskedDB.  */
  {
    ...
    for (n = 1 + hlen, p = dmask; n < nframe; n++) frame[n] ^= *p++;
    xfree(dmask);
  }
  ...
  /* Step 2g and 2h: Create maskedSeed.  */
  {
    ...
    for (n = 1, p = smask; n < 1 + hlen; n++) frame[n] ^= *p++;
    xfree(smask);
  }
  ...
  if (!rc && DBG_CIPHER) log_mpidump("OAEP encoded data", *r_result);
  xfree(frame);
...
gpg_error_t _gcry_rsa_oaep_decode(unsigned char **r_result, size_t *r_resultlen,
  ...
  *r_result = NULL;
  ...
  if (!lhash) return gpg_error_from_syserror();
...
gpg_error_t _gcry_rsa_pss_encode(gcry_mpi_t *r_result, unsigned int nbits,
  ...
  *p++ = 0x01;
  ...
  for (n = 0, p = dbmask; n < emlen - hlen - 1; n++, p++) em[n] ^= *p;
  ...
  if (!rc && DBG_CIPHER) log_mpidump("PSS encoded data", *r_result);
