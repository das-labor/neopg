...
static const char *check_one(int algo, const void *data, size_t datalen,
  ...
  /*   printf ("HMAC algo %d\n", algo); */
  if (_gcry_md_get_algo_dlen(algo) != expectlen) return "invalid tests data";
  if (_gcry_md_open(&hd, algo, GCRY_MD_FLAG_HMAC)) return "gcry_md_open failed";
...
static gpg_err_code_t selftests_sha1(int extended,
  ...
  if (extended) {
    ...
    what = "FIPS-198a, A.3";
    for (i = 0, j = 0x50; i < 100; i++) key[i] = j++;
...
failed:
  if (report) report("hmac", GCRY_MD_SHA1, what, errtxt);
...
static gpg_err_code_t selftests_sha224(int extended,
  ...
  for (tvidx = 0; tv[tvidx].desc; tvidx++) {
    what = tv[tvidx].desc;
    errtxt = check_one(GCRY_MD_SHA224, tv[tvidx].data, strlen(tv[tvidx].data),
...
failed:
  if (report) report("hmac", GCRY_MD_SHA224, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}
...
static gpg_err_code_t selftests_sha256(int extended,
  ...
  for (tvidx = 0; tv[tvidx].desc; tvidx++) {
    ...
    what = tv[tvidx].desc;
    errtxt = check_one(GCRY_MD_SHA256, tv[tvidx].data, strlen(tv[tvidx].data),
                       tv[tvidx].key, strlen(tv[tvidx].key), tv[tvidx].expect,
                       DIM(tv[tvidx].expect));
    if (errtxt) goto failed;
...
failed:
  if (report) report("hmac", GCRY_MD_SHA256, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}
...
static gpg_err_code_t selftests_sha384(int extended,
  ...
  for (tvidx = 0; tv[tvidx].desc; tvidx++) {
    what = tv[tvidx].desc;
    errtxt = check_one(GCRY_MD_SHA384, tv[tvidx].data, strlen(tv[tvidx].data),
                       tv[tvidx].key, strlen(tv[tvidx].key), tv[tvidx].expect,
                       DIM(tv[tvidx].expect));
    if (errtxt) goto failed;
    if (!extended) break;
...
failed:
  if (report) report("hmac", GCRY_MD_SHA384, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}
...
static gpg_err_code_t selftests_sha512(int extended,
  ...
  for (tvidx = 0; tv[tvidx].desc; tvidx++) {
    what = tv[tvidx].desc;
    errtxt = check_one(GCRY_MD_SHA512, tv[tvidx].data, strlen(tv[tvidx].data),
                       tv[tvidx].key, strlen(tv[tvidx].key), tv[tvidx].expect,
                       DIM(tv[tvidx].expect));
    if (errtxt) goto failed;
    if (!extended) break;
...
failed:
  if (report) report("hmac", GCRY_MD_SHA512, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}
...
gpg_error_t _gcry_hmac_selftest(int algo, int extended,
  ...
  if (!_gcry_md_test_algo(algo)) {
    ...
    ec = GPG_ERR_DIGEST_ALGO;
    if (report) report("hmac", algo, "module", "algorithm not available");
  }
  return gpg_error(ec);
}
