...
static inline int poly1305_bytecounter_add(u32 ctr[2], size_t add) {
  ...
  if (sizeof(add) > sizeof(u32)) {
    ...
    ctr[1] += high_add;
    if (ctr[1] < high_add) overflow = 1;
  }

  ctr[0] += add;
  if (ctr[0] >= add) return overflow;
...
static gpg_error_t poly1305_set_zeroiv(gcry_cipher_hd_t c) {
  byte zero[8] = {
      0,
  };
...
gpg_error_t _gcry_cipher_poly1305_authenticate(gcry_cipher_hd_t c,
                                               const byte *aadbuf,
                                               size_t aadbuflen) {
  if (c->u_mode.poly1305.bytecount_over_limits) return GPG_ERR_INV_LENGTH;
  if (c->u_mode.poly1305.aad_finalized) return GPG_ERR_INV_STATE;
  if (c->marks.tag) return GPG_ERR_INV_STATE;

  if (!c->marks.iv) poly1305_set_zeroiv(c);
...
gpg_error_t _gcry_cipher_poly1305_encrypt(gcry_cipher_hd_t c, byte *outbuf,
  ...
  if (!c->u_mode.poly1305.aad_finalized) poly1305_aad_finish(c);
...
gpg_error_t _gcry_cipher_poly1305_decrypt(gcry_cipher_hd_t c, byte *outbuf,
                                          size_t outbuflen, const byte *inbuf,
                                          size_t inbuflen) {
  gpg_error_t err;
...
static gpg_error_t _gcry_cipher_poly1305_tag(gcry_cipher_hd_t c, byte *outbuf,
                                             size_t outbuflen, int check) {
  gpg_error_t err;

  if (outbuflen < POLY1305_TAGLEN) return GPG_ERR_BUFFER_TOO_SHORT;
  if (c->u_mode.poly1305.bytecount_over_limits) return GPG_ERR_INV_LENGTH;
...
gpg_error_t _gcry_cipher_poly1305_get_tag(gcry_cipher_hd_t c,
                                          unsigned char *outtag,
                                          size_t taglen) {
  return _gcry_cipher_poly1305_tag(c, outtag, taglen, 0);
}

gpg_error_t _gcry_cipher_poly1305_check_tag(gcry_cipher_hd_t c,
...
gpg_error_t _gcry_cipher_poly1305_setiv(gcry_cipher_hd_t c, const byte *iv,
                                        size_t ivlen) {
  byte tmpbuf[64]; /* size of ChaCha20 block */
  gpg_error_t err;

  /* IV must be 96-bits */
  if (!iv && ivlen != (96 / 8)) return GPG_ERR_INV_ARG;
