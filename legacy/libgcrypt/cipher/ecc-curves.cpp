...
static const ecc_domain_parms_t domain_parms[] = {
    {/* (-x^2 + y^2 = 1 + dx^2y^2) */
     "Ed25519", 256, 0, (enum gcry_mpi_ec_models)MPI_EC_EDWARDS,
     (ecc_dialects)ECC_DIALECT_ED25519,
    ...
    {/* (y^2 = x^3 + 486662*x^2 + x) */
     "Curve25519", 256, 0, (enum gcry_mpi_ec_models)MPI_EC_MONTGOMERY,
...
#endif /*0*/
    {"NIST P-192", 192, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"NIST P-224", 224, 1, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"NIST P-256", 256, 1, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
     ...
     "0x01"},
    {"NIST P-384", 384, 1, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
     ...
     "0x01"},
    {"NIST P-521", 521, 1, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"brainpoolP160r1", 160, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"brainpoolP192r1", 192, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"brainpoolP224r1", 224, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"brainpoolP256r1", 256, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"brainpoolP320r1", 320, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"brainpoolP384r1", 384, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"brainpoolP512r1", 512, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
     ...
     "0x01"},
    {"GOST2001-test", 256, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"GOST2001-CryptoPro-C", 256, 0,
     ...
     "0x01"},
    {"GOST2012-test", 511, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
     ...
     "0x01"},
    {"GOST2012-tc26-A", 512, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
     ...
     "0x01"},
    {"GOST2012-tc26-B", 512, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {"secp256k1", 256, 0, (enum gcry_mpi_ec_models)MPI_EC_WEIERSTRASS,
     (ecc_dialects)ECC_DIALECT_STANDARD,
    ...
    {NULL, 0, 0, (enum gcry_mpi_ec_models)0, (ecc_dialects)0, NULL, NULL, NULL,
     NULL, NULL}};
...
static gcry_mpi_t scanval(const char *string) {
  ...
  if (rc) log_fatal("scanning ECC parameter failed: %s\n", gpg_strerror(rc));
  return val;
}
...
static int find_domain_parms_idx(const char *name) {
  ...
  for (idx = 0; domain_parms[idx].desc; idx++)
    if (!strcmp(name, domain_parms[idx].desc)) return idx;

  /* If not found consult the alias table.  */
  if (!domain_parms[idx].desc) {
    for (aliasno = 0; curve_aliases[aliasno].name; aliasno++)
      if (!strcmp(name, curve_aliases[aliasno].other)) break;
...
gpg_error_t _gcry_ecc_fill_in_curve(unsigned int nbits, const char *name,
  ...
  if (name)
    ...
    for (idx = 0; domain_parms[idx].desc; idx++)
      if (nbits == domain_parms[idx].nbits &&
        ...
        break;
    if (!domain_parms[idx].desc) idx = -1;
  }
  if (idx < 0) return GPG_ERR_UNKNOWN_CURVE;
  ...
  if (curve) {
    ...
    if (!curve->p) curve->p = scanval(domain_parms[idx].p);
    if (!curve->a) {
      curve->a = scanval(domain_parms[idx].a);
      if (curve->a->sign) mpi_add(curve->a, curve->p, curve->a);
    }
    if (!curve->b) {
      curve->b = scanval(domain_parms[idx].b);
      if (curve->b->sign) mpi_add(curve->b, curve->p, curve->b);
    }
    if (!curve->n) curve->n = scanval(domain_parms[idx].n);
    if (!curve->h) curve->h = scanval(domain_parms[idx].h);
    if (!curve->G.x) curve->G.x = scanval(domain_parms[idx].g_x);
    if (!curve->G.y) curve->G.y = scanval(domain_parms[idx].g_y);
    if (!curve->G.z) curve->G.z = mpi_alloc_set_ui(1);
    if (!curve->name) curve->name = resname;
  }
...
/* Give the name of the curve NAME, store the curve parameters into P,
   ...
   they are not NULL. */
gpg_error_t _gcry_ecc_update_curve_param(const char *name,
                                         enum gcry_mpi_ec_models *model,
                                         enum ecc_dialects *dialect,
  ...
  if (model) *model = domain_parms[idx].model;
  if (dialect) *dialect = domain_parms[idx].dialect;
  if (p) {
    _gcry_mpi_release(*p);
...
const char *_gcry_ecc_get_curve(gcry_sexp_t keyparms, int iterator,
   ...
   */
  rc = sexp_extract_param(keyparms, NULL, "-pabgnh", &E.p, &E.a, &E.b, &mpi_g,
                          &E.n, &E.h, NULL);
  if (rc == GPG_ERR_NO_OBJ) {
    ...
    if (!l1) goto leave; /* No curve name parameter.  */

    name = sexp_nth_string(l1, 1);
    sexp_release(l1);
    if (!name) goto leave; /* Name missing or out of core. */
  ...
  if (mpi_g) {
    ...
    if (_gcry_ecc_os2ec(&E.G, mpi_g)) goto leave;
  }
...
static gpg_error_t mpi_from_keyparam(gcry_mpi_t *r_a, gcry_sexp_t keyparam,
                                     const char *name) {
  gpg_error_t ec = 0;
  gcry_sexp_t l1;
...
static gpg_error_t point_from_keyparam(gcry_mpi_point_t *r_a,
  ...
  if (l1) {
    ...
    if (!tmpname) return gpg_error_from_syserror();
...
/* This function creates a new context for elliptic curve operations.
   ...
   gcry_ctx_release.  */
gpg_error_t _gcry_mpi_ec_new(gcry_ctx_t *r_ctx, gcry_sexp_t keyparam,
...
gcry_sexp_t _gcry_ecc_get_param_sexp(const char *name) {
  ...
  memset(&E, 0, sizeof E);
  if (_gcry_ecc_fill_in_curve(0, name, &E, &nbits)) return NULL;

  g_x = mpi_new(0);
  g_y = mpi_new(0);
  ctx = _gcry_mpi_ec_p_internal_new(MPI_EC_WEIERSTRASS, ECC_DIALECT_STANDARD, 0,
                                    E.p, E.a, NULL);
  ...
  for (i = 0; pkey[i]; i++) _gcry_mpi_release(pkey[i]);
...
gcry_mpi_t _gcry_ecc_get_mpi(const char *name, mpi_ec_t ec, int copy) {
  ...
  /* If the base point has been requested, return it in standard
     encoding.  */
  if (!strcmp(name, "g") && ec->G) return _gcry_mpi_ec_ec2os(ec->G, ec);
  ...
  if (*name == 'q' && (!name[1] || name[1] == '@')) {
    ...
    if (!ec->Q) ec->Q = _gcry_ecc_compute_public(NULL, ec, NULL, NULL);

    if (!ec->Q) return NULL;

    if (name[1] != '@') return _gcry_mpi_ec_ec2os(ec->Q, ec);
...
gcry_mpi_point_t _gcry_ecc_get_point(const char *name, mpi_ec_t ec) {
  if (!strcmp(name, "g") && ec->G) return point_copy(ec->G);
  if (!strcmp(name, "q")) {
    /* If only the private key is given, compute the public key.  */
    if (!ec->Q) ec->Q = _gcry_ecc_compute_public(NULL, ec, NULL, NULL);

    if (ec->Q) return point_copy(ec->Q);
  }
