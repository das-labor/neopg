...
static int map_algo(int algo) { return algo; }
  ...
  for (idx = 0; (spec = cipher_list[idx]); idx++) {
    ...
    if (oid_specs) {
      for (j = 0; oid_specs[j].oid; j++)
        if (!strcasecmp(oid, oid_specs[j].oid)) return spec;
    }
  }
...
static gcry_cipher_spec_t *search_oid(const char *oid,
  ...
  if (!strncmp(oid, "oid.", 4) || !strncmp(oid, "OID.", 4)) oid += 4;

  spec = spec_from_oid(oid);
  if (spec && spec->oids) {
    for (i = 0; spec->oids[i].oid; i++)
      if (!strcasecmp(oid, spec->oids[i].oid)) {
        if (oid_spec) *oid_spec = spec->oids[i];
        return spec;
      }
  }
...
/* Return 0 if the cipher algorithm with identifier ALGORITHM is
   ...
   available.  */
static gpg_error_t check_cipher_algo(int algorithm) {
  gcry_cipher_spec_t *spec;

  spec = spec_from_algo(algorithm);
  if (spec && !spec->flags.disabled) return 0;
...
static unsigned int cipher_get_keylen(int algorithm) {
  ...
  if (spec) {
    len = spec->keylen;
    if (!len) log_bug("cipher %d w/o key length\n", algorithm);
  }
...
static unsigned int cipher_get_blocksize(int algorithm) {
  ...
  if (spec) {
    len = spec->blocksize;
    if (!len) log_bug("cipher %d w/o blocksize\n", algorithm);
  }
 ...
 */
gpg_error_t _gcry_cipher_open(gcry_cipher_hd_t *handle, int algo, int mode,
                              unsigned int flags) {
  gpg_error_t rc;
...
gpg_error_t _gcry_cipher_open_internal(gcry_cipher_hd_t *handle, int algo,
  ...
  /* check flags */
  if ((!err) && ((flags &
                  ~(0 | GCRY_CIPHER_SECURE | GCRY_CIPHER_ENABLE_SYNC |
  ...
  if (!err) switch (mode) {
      case GCRY_CIPHER_MODE_CCM:
        ...
        if (!spec->encrypt || !spec->decrypt) err = GPG_ERR_INV_CIPHER_MODE;
        break;
      ...
      case GCRY_CIPHER_MODE_STREAM:
        if (!spec->stencrypt || !spec->stdecrypt) err = GPG_ERR_INV_CIPHER_MODE;
        break;
  ...
  if (!err) {
    size_t size =
        (sizeof(*h) + 2 * spec->contextsize - sizeof(cipher_context_alignment_t)
  ...
  if (!rc) {
    ...
    switch (c->mode) {
      ...
      case GCRY_CIPHER_MODE_XTS:
        ...
        rc = c->spec->setkey(c->u_mode.xts.tweak_context, key + keylen, keylen);
        if (!rc) {
...
static gpg_error_t cipher_setiv(gcry_cipher_hd_t c, const byte *iv,
                                size_t ivlen) {
...
static gpg_error_t do_ecb_crypt(gcry_cipher_hd_t c, unsigned char *outbuf,
  ...
  if ((inbuflen % blocksize)) return GPG_ERR_INV_LENGTH;
...
static gpg_error_t do_ecb_encrypt(gcry_cipher_hd_t c, unsigned char *outbuf,
                                  size_t outbuflen, const unsigned char *inbuf,
                                  size_t inbuflen) {
  return do_ecb_crypt(c, outbuf, outbuflen, inbuf, inbuflen, c->spec->encrypt);
}

static gpg_error_t do_ecb_decrypt(gcry_cipher_hd_t c, unsigned char *outbuf,
                                  size_t outbuflen, const unsigned char *inbuf,
                                  size_t inbuflen) {
 ...
 */
static gpg_error_t cipher_encrypt(gcry_cipher_hd_t c, byte *outbuf,
  ...
  switch (c->mode) {
    ...
    case GCRY_CIPHER_MODE_AESWRAP:
      rc = _gcry_cipher_aeswrap_encrypt(c, outbuf, outbuflen, inbuf, inbuflen);
      break;
    ...
    case GCRY_CIPHER_MODE_POLY1305:
      rc = _gcry_cipher_poly1305_encrypt(c, outbuf, outbuflen, inbuf, inbuflen);
      break;
 ...
 * been requested.
 */
gpg_error_t _gcry_cipher_encrypt(gcry_cipher_hd_t h, void *out, size_t outsize,
  ...
  if (rc && out) memset(out, 0x42, outsize);
 ...
 */
static gpg_error_t cipher_decrypt(gcry_cipher_hd_t c, byte *outbuf,
                                  size_t outbuflen, const byte *inbuf,
                                  size_t inbuflen) {
  gpg_error_t rc;
  ...
  switch (c->mode) {
    ...
    case GCRY_CIPHER_MODE_AESWRAP:
      rc = _gcry_cipher_aeswrap_decrypt(c, outbuf, outbuflen, inbuf, inbuflen);
      break;
    ...
    case GCRY_CIPHER_MODE_POLY1305:
      rc = _gcry_cipher_poly1305_decrypt(c, outbuf, outbuflen, inbuf, inbuflen);
      break;
...
gpg_error_t _gcry_cipher_decrypt(gcry_cipher_hd_t h, void *out, size_t outsize,
                                 const void *in, size_t inlen) {
  if (!in) /* Caller requested in-place encryption. */
  {
    in = out;
...
static void cipher_sync(gcry_cipher_hd_t c) {
  if ((c->flags & GCRY_CIPHER_ENABLE_SYNC) && c->unused) {
    memmove(c->u_iv.iv + c->unused, c->u_iv.iv, c->spec->blocksize - c->unused);
    memcpy(c->u_iv.iv, c->lastiv + c->spec->blocksize - c->unused, c->unused);
    c->unused = 0;
  }
}

gpg_error_t _gcry_cipher_setkey(gcry_cipher_hd_t hd, const void *key,
                                size_t keylen) {
  return cipher_setkey(hd, (byte *)((void *)key), keylen);
}

gpg_error_t _gcry_cipher_setiv(gcry_cipher_hd_t hd, const void *iv,
                               size_t ivlen) {
  gpg_error_t rc = 0;
...
/* Set counter for CTR mode.  (CTR,CTRLEN) must denote a buffer of
   ...
   block. */
gpg_error_t _gcry_cipher_setctr(gcry_cipher_hd_t hd, const void *ctr,
                                size_t ctrlen) {
...
gpg_error_t _gcry_cipher_getctr(gcry_cipher_hd_t hd, void *ctr, size_t ctrlen) {
  if (ctr && ctrlen == hd->spec->blocksize)
...
gpg_error_t _gcry_cipher_authenticate(gcry_cipher_hd_t hd, const void *abuf,
  ...
  switch (hd->mode) {
    case GCRY_CIPHER_MODE_CCM:
      rc = _gcry_cipher_ccm_authenticate(hd, (const unsigned char *)(abuf),
                                         abuflen);
      break;

    case GCRY_CIPHER_MODE_CMAC:
      rc = _gcry_cipher_cmac_authenticate(hd, (const unsigned char *)(abuf),
                                          abuflen);
      break;

    case GCRY_CIPHER_MODE_GCM:
      rc = _gcry_cipher_gcm_authenticate(hd, (const unsigned char *)(abuf),
                                         abuflen);
      break;

    case GCRY_CIPHER_MODE_POLY1305:
      rc = _gcry_cipher_poly1305_authenticate(hd, (const unsigned char *)(abuf),
                                              abuflen);
      break;

    case GCRY_CIPHER_MODE_OCB:
      rc = _gcry_cipher_ocb_authenticate(hd, (const unsigned char *)(abuf),
                                         abuflen);
      break;
...
gpg_error_t _gcry_cipher_checktag(gcry_cipher_hd_t hd, const void *intag,
  ...
  switch (hd->mode) {
    case GCRY_CIPHER_MODE_CCM:
      rc = _gcry_cipher_ccm_check_tag(hd, (const unsigned char *)(intag),
                                      taglen);
      break;

    case GCRY_CIPHER_MODE_CMAC:
      rc = _gcry_cipher_cmac_check_tag(hd, (const unsigned char *)(intag),
                                       taglen);
      break;

    case GCRY_CIPHER_MODE_GCM:
      rc = _gcry_cipher_gcm_check_tag(hd, (const unsigned char *)(intag),
                                      taglen);
      break;

    case GCRY_CIPHER_MODE_POLY1305:
      rc = _gcry_cipher_poly1305_check_tag(hd, (const unsigned char *)(intag),
                                           taglen);
      break;

    case GCRY_CIPHER_MODE_OCB:
      rc = _gcry_cipher_ocb_check_tag(hd, (const unsigned char *)(intag),
                                      taglen);
      break;
...
gpg_error_t _gcry_cipher_ctl(gcry_cipher_hd_t h, int cmd, void *buffer,
  ...
  switch (cmd) {
    ...
    case GCRYCTL_FINALIZE:
      if (!h || buffer || buflen) return GPG_ERR_INV_ARG;
      h->marks.finalize = 1;
      break;
    ...
    case GCRYCTL_SET_CCM_LENGTHS: {
      ...
      if (h->mode != GCRY_CIPHER_MODE_CCM) return GPG_ERR_INV_CIPHER_MODE;

      if (!buffer || buflen != 3 * sizeof(u64)) return GPG_ERR_INV_ARG;
    ...
    case GCRYCTL_SET_TAGLEN:
      if (!h || !buffer || buflen != sizeof(int)) return GPG_ERR_INV_ARG;
      switch (h->mode) {
        case GCRY_CIPHER_MODE_OCB:
          switch (*(int *)buffer) {
            case 8:
    ...
    case GCRYCTL_DISABLE_ALGO:
      ...
      if (h || !buffer || buflen != sizeof(int)) return GPG_ERR_CIPHER_ALGO;
      disable_cipher_algo(*(int *)buffer);
      break;

    case PRIV_CIPHERCTL_DISABLE_WEAK_KEY: /* (private)  */
      if (h->spec->set_extra_info)
        rc = h->spec->set_extra_info(&h->context.c, CIPHER_INFO_NO_WEAK_KEY,
                                     NULL, 0);
      else
    ...
    case GCRYCTL_SET_SBOX:
      if (h->spec->set_extra_info)
        rc = h->spec->set_extra_info(&h->context.c, GCRYCTL_SET_SBOX, buffer,
                                     buflen);
      else
        rc = GPG_ERR_NOT_SUPPORTED;
      break;
 ...
 */
gpg_error_t _gcry_cipher_info(gcry_cipher_hd_t h, int cmd, void *buffer,
                              size_t *nbytes) {
  gpg_error_t rc = 0;
...
size_t _gcry_cipher_get_algo_keylen(int algo) {
  ...
  if (_gcry_cipher_algo_info(algo, GCRYCTL_GET_KEYLEN, NULL, &n)) n = 0;
  return n;
}
