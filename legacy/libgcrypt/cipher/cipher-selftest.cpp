...
const char *_gcry_selftest_helper_cbc(const char *cipher,
  ...
  if (!mem) return "failed to allocate memory";
  ...
  for (i = 0; i < blocksize; i++) plaintext[i] = i;
  ...
  for (i = 0; i < nblocks * blocksize; i++) plaintext[i] = i;
...
const char *_gcry_selftest_helper_ctr(const char *cipher,
  ...
  /* CTR manually.  */
  encrypt_one(ctx, ciphertext, iv);
  for (i = 0; i < blocksize; i++) ciphertext[i] ^= plaintext[i];
  ...
  for (i = 0; i < blocksize * nblocks; i++) plaintext2[i] = plaintext[i] = i;

  /* Create CTR ciphertext manually.  */
  for (i = 0; i < blocksize * nblocks; i += blocksize) {
    encrypt_one(ctx, &ciphertext[i], iv);
    for (j = 0; j < blocksize; j++) ciphertext[i + j] ^= plaintext[i + j];
  ...
  for (diff = 0; diff < nblocks; diff++) {
    ...
    for (i = 0; i < blocksize * nblocks; i++) plaintext[i] = i;
