...
static gpg_error_t camellia_setkey(void *c, const byte *key, unsigned keylen) {
  ...
  if (keylen != 16 && keylen != 24 && keylen != 32) return GPG_ERR_INV_KEYLEN;
  ...
  if (ctx->use_aesni_avx2) {
    ...
    if (did_use_aesni_avx2) {
      int avx2_burn_stack_depth =
          32 * CAMELLIA_BLOCK_SIZE + 16 + 2 * sizeof(void *) + ASM_EXTRA_STACK;
