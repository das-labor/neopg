...
static void permute(unsigned char *X, const unsigned char *buf) {
  int i, j;
  unsigned char t;

  memcpy(X + 16, buf, 16);
  for (i = 0; i < 16; i++) X[32 + i] = X[16 + i] ^ X[i];
  t = 0;
  for (i = 0; i < 18; i++) {
    for (j = 0; j < 48; j++) {
      t = X[j] ^ S[t];
...
static void md2_init(void *context, unsigned int flags) {
...
static void md2_final(void *context) {
  ...
  /* pad */
  memset(hd->bctx.buf + hd->bctx.count, 16 - hd->bctx.count,
         16 - hd->bctx.count);
  burn = transform_blk(hd, hd->bctx.buf);
  permute(hd->X, hd->C);
}

static byte *md2_read(void *context) {
...
gcry_md_spec_t _gcry_digest_spec_md2 = {GCRY_MD_MD2,
                                        {0, 0},
                                        "MD2",
                                        asn,
                                        DIM(asn),
