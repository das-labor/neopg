...
static inline void double_block(unsigned char *b) {
  ...
  for (i = 0; i < OCB_BLOCK_LEN - 1; i++) b[i] = (b[i] << 1) | (b[i + 1] >> 7);
...
static void double_block_cpy(unsigned char *d, const unsigned char *s) {
  if (d != s) buf_cpy(d, s, OCB_BLOCK_LEN);
  double_block(d);
}
...
static void ocb_get_L_big(gcry_cipher_hd_t c, u64 n, unsigned char *l_buf) {
  ...
  for (ntz -= OCB_L_TABLE_SIZE; ntz; ntz--) double_block(l_buf);
}
...
/* Set the nonce for OCB.  This requires that the key has been set.
   ...
   key.  */
gpg_error_t _gcry_cipher_ocb_set_nonce(gcry_cipher_hd_t c,
  ...
  if (c->spec->blocksize != OCB_BLOCK_LEN) return GPG_ERR_CIPHER_ALGO;
...
/* Process additional authentication data.  This implementation allows
   ...
   gcry_cipher_gettag.  */
gpg_error_t _gcry_cipher_ocb_authenticate(gcry_cipher_hd_t c,
  ...
  if (c->u_mode.ocb.aad_nleftover) {
    ...
    if (c->u_mode.ocb.aad_nleftover == OCB_BLOCK_LEN) {
      ...
      if ((c->u_mode.ocb.aad_nblocks % table_maxblks) == 0) {
        ...
        buf_cpy(l_tmp, ocb_get_l(c, c->u_mode.ocb.aad_nblocks), OCB_BLOCK_LEN);
      }
...
static void ocb_aad_finalize(gcry_cipher_hd_t c) {
  ...
  /* Check that a nonce and thus a key has been set and that we have
     ...
     finalized.  */
  if (!c->marks.iv || c->marks.tag || c->u_mode.ocb.aad_finalized) return;
  if (c->spec->blocksize != OCB_BLOCK_LEN) return; /* Ooops.  */

  /* Hash final partial block if any.  */
  if (c->u_mode.ocb.aad_nleftover) {
    /* Offset_* = Offset_m xor L_*  */
    buf_xor_1(c->u_mode.ocb.aad_offset, c->u_mode.ocb.L_star, OCB_BLOCK_LEN);
...
static gpg_error_t ocb_crypt(gcry_cipher_hd_t c, int encrypt,
  ...
  if (!c->marks.iv || c->u_mode.ocb.data_finalized) return GPG_ERR_INV_STATE;
  ...
  while (inbuflen >= OCB_BLOCK_LEN) {
    ...
    if (nblks) {
      ...
      if (!encrypt) {
        /* Checksum_i = Checksum_{i-1} xor P_i  */
        ocb_checksum(c->u_ctr.ctr, outbuf - nblks_chksum * OCB_BLOCK_LEN,
                     nblks_chksum);
      }
    }
  }
  ...
  if (c->marks.finalize) {
    ...
    nburn =
        c->spec->encrypt(&c->context.c, c->u_mode.ocb.tag, c->u_mode.ocb.tag);
    burn = nburn > burn ? nburn : burn;
...
gpg_error_t _gcry_cipher_ocb_encrypt(gcry_cipher_hd_t c, unsigned char *outbuf,
                                     size_t outbuflen,
...
/* Decrypt (INBUF,INBUFLEN) in OCB mode to OUTBUF.  OUTBUFLEN gives
   ...
   of a full block unless gcry_cipher_final has been called in which
   case the next block may have any length.  */
gpg_error_t _gcry_cipher_ocb_decrypt(gcry_cipher_hd_t c, unsigned char *outbuf,
                                     size_t outbuflen,
                                     const unsigned char *inbuf,
                                     size_t inbuflen) {
...
gpg_error_t _gcry_cipher_ocb_get_tag(gcry_cipher_hd_t c, unsigned char *outtag,
                                     size_t outtagsize) {
  if (c->u_mode.ocb.taglen > outtagsize) return GPG_ERR_BUFFER_TOO_SHORT;
...
/* Check that the tag (INTAG,TAGLEN) matches the computed tag for the
   handle C.  */
gpg_error_t _gcry_cipher_ocb_check_tag(gcry_cipher_hd_t c,
