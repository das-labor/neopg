...
#ifdef USE_AMD64_ASM
  {
    if (nblocks >= 4) burn_stack_depth += 5 * sizeof(void *);
...
static const char *selftest_ctr(void) {
  ...
  return _gcry_selftest_helper_ctr("BLOWFISH", &bf_setkey, &encrypt_block,
                                   &_gcry_blowfish_ctr_enc, nblocks, blocksize,
                                   context_size);
}
...
static const char *selftest_cbc(void) {
  ...
  return _gcry_selftest_helper_cbc("BLOWFISH", &bf_setkey, &encrypt_block,
                                   &_gcry_blowfish_cbc_dec, nblocks, blocksize,
                                   context_size);
}
...
static const char *selftest_cfb(void) {
  ...
  return _gcry_selftest_helper_cfb("BLOWFISH", &bf_setkey, &encrypt_block,
                                   &_gcry_blowfish_cfb_dec, nblocks, blocksize,
                                   context_size);
}
...
static const char *selftest(void) {
  ...
  byte plain[] = "BLOWFISH";
  byte buffer[8];
  static const byte plain3[] = {0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10};
  ...
  if (memcmp(buffer, plain, 8)) return "Blowfish selftest failed (2).";

  bf_setkey((void *)&c, key3, 8);
  encrypt_block((void *)&c, buffer, plain3);
  if (memcmp(buffer, cipher3, 8)) return "Blowfish selftest failed (3).";
  decrypt_block((void *)&c, buffer, buffer);
  if (memcmp(buffer, plain3, 8)) return "Blowfish selftest failed (4).";
...
static gpg_error_t do_bf_setkey(BLOWFISH_context *c, const byte *key,
                                unsigned keylen) {
  ...
  for (i = 0; i < BLOWFISH_ROUNDS + 2; i++) c->p[i] = ps[i];
  for (i = 0; i < 256; i++) {
    c->s0[i] = ks0[i];
    c->s1[i] = ks1[i];
    c->s2[i] = ks2[i];
  ...
  if (weak) return GPG_ERR_WEAK_KEY;

  return GPG_ERR_NO_ERROR;
}

static gpg_error_t bf_setkey(void *context, const byte *key, unsigned keylen) {
  BLOWFISH_context *c = (BLOWFISH_context *)context;
