...
gcry_mpi_t _gcry_ecc_ec2os(gcry_mpi_t x, gcry_mpi_t y, gcry_mpi_t p) {
  ...
  *buf = 04; /* Uncompressed point.  */
  ...
  if (rc) log_fatal("mpi_print failed: %s\n", gpg_strerror(rc));
  if (n < pbytes) {
  ...
  if (rc) log_fatal("mpi_scan failed: %s\n", gpg_strerror(rc));
  xfree(buf);
...
mpi_point_t _gcry_ecc_compute_public(mpi_point_t Q, mpi_ec_t ec, mpi_point_t G,
  ...
  if (!d || !G || !ec->p || !ec->a) return NULL;
  if (ec->model == MPI_EC_EDWARDS && !ec->b) return NULL;

  if (ec->dialect == ECC_DIALECT_ED25519 && (ec->flags & PUBKEY_FLAG_EDDSA)) {
    gcry_mpi_t a;
    unsigned char *digest;

    if (_gcry_ecc_eddsa_compute_h_d(&digest, d, ec)) return NULL;
    ...
    if (!Q) Q = mpi_point_new(0);
    if (Q) _gcry_mpi_ec_mul_point(Q, a, G, ec);
    mpi_free(a);
  } else {
    if (!Q) Q = mpi_point_new(0);
    if (Q) _gcry_mpi_ec_mul_point(Q, d, G, ec);
  }
...
gpg_error_t _gcry_ecc_mont_decodepoint(gcry_mpi_t pk, mpi_ec_t ctx,
  ...
  if (mpi_is_opaque(pk)) {
     ...
     */
    if (pk->nlimbs * BYTES_PER_MPI_LIMB <
        nbytes) { /*
