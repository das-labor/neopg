...
#else
typedef struct bufhelp_int_s { uintptr_t a; } bufhelp_int_t;
#endif
#endif
...
static inline void buf_cpy(void *_dst, const void *_src, size_t len) {
  ...
  if (UNLIKELY(((uintptr_t)dst | (uintptr_t)src) & longmask)) goto do_bytes;
#endif
...
static inline int buf_eq_const(const void *_a, const void *_b, size_t len) {
  ...
  for (i = 0, diff = 0; i < len; i++) diff -= !!(a[i] - b[i]);
