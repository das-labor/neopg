...
static void do_encrypt_block(CAST5_context *c, byte *outbuf,
                             const byte *inbuf) {
  u32 l, r, t;
...
static void do_decrypt_block(CAST5_context *c, byte *outbuf,
                             const byte *inbuf) {
  u32 l, r, t;
  u32 I;
  u32 *Km;
...
#ifdef USE_AMD64_ASM
  {
    if (nblocks >= 4) burn_stack_depth += 8 * sizeof(void *);
...
static const char *selftest(void) {
  CAST5_context c;
  static const byte key[16] = {0x01, 0x23, 0x45, 0x67, 0x12, 0x34, 0x56, 0x78,
                               0x23, 0x45, 0x67, 0x89, 0x34, 0x56, 0x78, 0x9A};
  static const byte plain[8] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF};
  ...
  if (memcmp(buffer, cipher, 8)) return "1";
  decrypt_block(&c, buffer, buffer);
  if (memcmp(buffer, plain, 8)) return "2";
...
static gpg_error_t do_cast_setkey(CAST5_context *c, const byte *key,
                                  unsigned keylen) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    c->Kr_arm_enc[i] =
        Kr_arm[0] | (Kr_arm[1] << 8) | (Kr_arm[2] << 16) | (Kr_arm[3] << 24);
    c->Kr_arm_dec[i] =
        Kr_arm[3] | (Kr_arm[2] << 8) | (Kr_arm[1] << 16) | (Kr_arm[0] << 24);
...
static gpg_error_t cast_setkey(void *context, const byte *key,
                               unsigned keylen) {
  CAST5_context *c = (CAST5_context *)context;
