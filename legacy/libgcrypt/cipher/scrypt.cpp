...
static void salsa20_core(u32 *dst, const u32 *src, unsigned int rounds) {
  ...
  for (i = 0; i < SALSA20_INPUT_LENGTH; i++) x[i] = LE_SWAP32(src[i]);
...
gpg_error_t _gcry_kdf_scrypt(const unsigned char *passwd, size_t passwdlen,
  ...
  if (subalgo < 1 || !iterations) return GPG_ERR_INV_VALUE;
  ...
  r128 = r * 128;
  if (r128 / 128 != r) return GPG_ERR_ENOMEM;

  nbytes = p * r128;
  if (r128 && nbytes / r128 != p) return GPG_ERR_ENOMEM;

  nbytes = N * r128;
  if (r128 && nbytes / r128 != N) return GPG_ERR_ENOMEM;

  nbytes = 64 + r128;
  if (nbytes < r128) return GPG_ERR_ENOMEM;

  B = (unsigned char *)xtrymalloc(p * r128);
  if (!B) {
    ec = gpg_error_from_syserror();
    goto leave;
  }

  tmp1 = (unsigned char *)xtrymalloc(N * r128);
  if (!tmp1) {
    ec = gpg_error_from_syserror();
    goto leave;
  }

  tmp2 = (unsigned char *)xtrymalloc(64 + r128);
  if (!tmp2) {
    ec = gpg_error_from_syserror();
    goto leave;
  }
  ...
  for (i = 0; !ec && i < p; i++) scrypt_ro_mix(r, &B[i * r128], N, tmp1, tmp2);
