...
static void prefetch_dec(void) {
  prefetch_table((const volatile byte *)((const void *)&dec_tables),
                 sizeof(dec_tables));
}
...
static gpg_error_t rijndael_setkey(void *context, const byte *key,
                                   const unsigned keylen) {
...
static unsigned int rijndael_encrypt(void *context, byte *b, const byte *a) {
  ...
  if (ctx->prefetch_enc_fn) ctx->prefetch_enc_fn();
...
void _gcry_aes_cfb_enc(void *context, unsigned char *iv, void *outbuf_arg,
  ...
  if (burn_depth) _gcry_burn_stack(burn_depth + 4 * sizeof(void *));
}
...
void _gcry_aes_cbc_enc(void *context, unsigned char *iv, void *outbuf_arg,
  ...
  if (0)
    ...
    for (; nblocks; nblocks--) {
      ...
      last_iv = outbuf;
      inbuf += BLOCKSIZE;
      if (!cbc_mac) outbuf += BLOCKSIZE;
    }

    if (last_iv != iv) buf_cpy(iv, last_iv, BLOCKSIZE);
  }
...
static unsigned int rijndael_decrypt(void *context, byte *b, const byte *a) {
  ...
  if (ctx->prefetch_dec_fn) ctx->prefetch_dec_fn();
...
/* The test vectors are from the AES supplied ones; more or less
  ...
  static const unsigned char key_128[16] = {0xE8, 0xE9, 0xEA, 0xEB, 0xED, 0xEE,
                                            0xEF, 0xF0, 0xF2, 0xF3, 0xF4, 0xF5,
                                            0xF7, 0xF8, 0xF9, 0xFA};
...
#warning debug test vectors in use
  ...
  ctx =
      (RIJNDAEL_context *)_gcry_cipher_selftest_alloc_ctx(sizeof *ctx, &ctxmem);
  if (!ctx) return "failed to allocate memory";
...
static const char *selftest_basic_192(void) {
  ...
  static unsigned char plaintext_192[16] = {0x76, 0x77, 0x74, 0x75, 0xF1, 0xF2,
                                            0xF3, 0xF4, 0xF8, 0xF9, 0xE6, 0xE7,
                                            0x77, 0x70, 0x71, 0x72};
...
static const char *selftest_basic_256(void) {
  ...
  static unsigned char plaintext_256[16] = {0x06, 0x9A, 0x00, 0x7F, 0xC7, 0x6A,
                                            0x45, 0x9F, 0x98, 0xBA, 0xF9, 0x17,
                                            0xFE, 0xDF, 0x95, 0x21};
...
static const char *selftest(void) {
  ...
  if ((r = selftest_ctr_128())) return r;

  if ((r = selftest_cbc_128())) return r;

  if ((r = selftest_cfb_128())) return r;
      ...
      const unsigned char output[16];
             ...
             {GCRY_CIPHER_MODE_OFB,
               ...
               0x0b, 0x0c, 0x0d, 0x0e, 0x0f},
              {
                  ...
                  {{0xae, 0x2d, 0x8a, 0x57, 0x1e, 0x03, 0xac, 0x9c, 0x9e, 0xb7,
                    0x6f, 0xac, 0x45, 0xaf, 0x8e, 0x51},
                   {0x77, 0x89, 0x50, 0x8d, 0x16, 0x91, 0x8f, 0x03, 0xf5, 0x3c,
                    0x52, 0xda, 0xc5, 0x4e, 0xd8, 0x25}},

                  {{0x30, 0xc8, 0x1c, 0x46, 0xa3, 0x5c, 0xe4, 0x11, 0xe5, 0xfb,
                    0xc1, 0x19, 0x1a, 0x0a, 0x52, 0xef},
                   {0x97, 0x40, 0x05, 0x1e, 0x9c, 0x5f, 0xec, 0xf6, 0x43, 0x44,
                    0xf7, 0xa8, 0x22, 0x60, 0xed, 0xcc}},

                  {{0xf6, 0x9f, 0x24, 0x45, 0xdf, 0x4f, 0x9b, 0x17, 0xad, 0x2b,
                    0x41, 0x7b, 0xe6, 0x6c, 0x37, 0x10},
                   {0x30, 0x4c, 0x65, 0x28, 0xf6, 0x59, 0xc7, 0x78, 0x66, 0xa5,
                    0x10, 0xd9, 0xc1, 0xd6, 0xae, 0x5e}},
              }}};
  unsigned char scratch[16];
  gpg_error_t err;
  int tvi, idx;
...
#define Fail(a)                \
  ...
  for (tvi = 0; tvi < DIM(tv); tvi++)
    if (tv[tvi].mode == requested_mode) break;
  if (tvi == DIM(tv)) Fail("no test data for this mode");
  ...
  if (!err) err = _gcry_cipher_setkey(hddec, tv[tvi].key, sizeof tv[tvi].key);
  if (err) Fail("set key");
  err = _gcry_cipher_setiv(hdenc, tv[tvi].iv, sizeof tv[tvi].iv);
  if (!err) err = _gcry_cipher_setiv(hddec, tv[tvi].iv, sizeof tv[tvi].iv);
  if (err) Fail("set IV");
...
static gpg_error_t selftest_fips_128(int extended,
...
failed:
  if (report) report("cipher", GCRY_CIPHER_AES128, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}

/* Complete selftest for AES-192.  */
static gpg_error_t selftest_fips_192(int extended,
                                     selftest_report_func_t report) {
  const char *what;
  const char *errtxt;
...
failed:
  if (report) report("cipher", GCRY_CIPHER_AES192, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}

/* Complete selftest for AES-256.  */
static gpg_error_t selftest_fips_256(int extended,
                                     selftest_report_func_t report) {
  const char *what;
  const char *errtxt;
...
failed:
  if (report) report("cipher", GCRY_CIPHER_AES256, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}
