...
static gpg_error_t generate(DSA_secret_key *sk, unsigned int nbits,
...
static void progress(int c) {
  if (progress_cb) progress_cb(progress_cb_data, "pk_dsa", c, 0, 0);
}
...
static gpg_error_t generate_fips186(DSA_secret_key *sk, unsigned int nbits,
                                    unsigned int qbits, gcry_sexp_t deriveparms,
...
static gpg_error_t dsa_generate(const gcry_sexp_t genparms,
                                gcry_sexp_t *r_skey) {
  gpg_error_t rc;
  unsigned int nbits;
  ...
  if (!rc) {
    ...
    if (!format)
      ...
      if (seedinfo) p = stpcpy(p, "%S");
      if (nfactors) {
        p = stpcpy(p, "(pm1-factors");
        for (i = 0; i < nfactors; i++) p = stpcpy(p, "%m");
        p = stpcpy(p, ")");
      }
      p = stpcpy(p, ")");
      ...
      if (!arg_list)
        ...
        i = 0;
        if (seedinfo) arg_list[i++] = &seedinfo;
        for (j = 0; j < nfactors; j++) arg_list[i++] = factors + j;
        arg_list[i] = NULL;
...
static gpg_error_t dsa_check_secret_key(gcry_sexp_t keyparms) {
  gpg_error_t rc;
...
leave:
  ...
  _gcry_mpi_release(sk.y);
  _gcry_mpi_release(sk.x);
  if (DBG_CIPHER) log_debug("dsa_testkey    => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t dsa_sign(gcry_sexp_t *r_sig, gcry_sexp_t s_data,
  ...
  if (rc) goto leave;
  if (DBG_CIPHER) log_mpidump("dsa_sign   data", data);
...
leave:
  ...
  if (DBG_CIPHER) log_debug("dsa_sign      => %s\n", gpg_strerror(rc));
  return rc;
}

static gpg_error_t dsa_verify(gcry_sexp_t s_sig, gcry_sexp_t s_data,
  ...
  rc = _gcry_pk_util_data_to_mpi(s_data, &data, &ctx);
  if (rc) goto leave;
  if (DBG_CIPHER) log_mpidump("dsa_verify data", data);
...
static gpg_error_t selftests_dsa_2048(selftest_report_func_t report) {
  const char *what;
  const char *errtxt;
  gpg_error_t err;
...
failed:
  sexp_release(pkey);
  sexp_release(skey);
  if (report) report("pubkey", GCRY_PK_DSA, what, errtxt);
  return GPG_ERR_SELFTEST_FAILED;
}
