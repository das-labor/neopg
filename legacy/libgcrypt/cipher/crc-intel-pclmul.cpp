...
static const byte crc32_shuf_shift[3 * 16] ALIGNED_16 = {
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0x0f, 0x0e, 0x0d, 0x0c, 0x0b, 0x0a, 0x09, 0x08,
    0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, 0x00, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
};
...
static inline void crc32_bulk(u32 *pcrc, const byte *inbuf, size_t inlen,
                              const struct crc32_consts_s *consts) {
...
static inline void crc32_less_than_16(u32 *pcrc, const byte *inbuf,
                                      size_t inlen,
                                      const struct crc32_consts_s *consts) {
  if (inlen < 4) {
    u32 crc = *pcrc;
    ...
    asm volatile(
        ...
        "movdqa %[k3k4], %%xmm6\n\t"
        :
        : [shuf] "m"(crc32_shuf_shift[32 - inlen]), [crc] "m"(*pcrc),
          [my_p] "m"(consts->my_p[0]), [k3k4] "m"(consts->k[3 - 1]));
