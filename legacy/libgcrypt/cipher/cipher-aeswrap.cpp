...
/* Perform the AES-Wrap algorithm as specified by RFC3394.  We
   ...
   blocksize 128.  */
gpg_error_t _gcry_cipher_aeswrap_encrypt(gcry_cipher_hd_t c, byte *outbuf,
...
#error Invalid block size
  ...
  if (c->spec->blocksize != 16) return GPG_ERR_INV_LENGTH;

  /* The output buffer must be able to hold the input data plus one
     additional block.  */
  if (outbuflen < inbuflen + 8) return GPG_ERR_BUFFER_TOO_SHORT;
  /* Input data must be multiple of 64 bits.  */
  if (inbuflen % 8) return GPG_ERR_INV_ARG;
...
/* Perform the AES-Unwrap algorithm as specified by RFC3394.  We
   implement this as a mode usable with any cipher algorithm of
   blocksize 128.  */
gpg_error_t _gcry_cipher_aeswrap_decrypt(gcry_cipher_hd_t c, byte *outbuf,
                                         size_t outbuflen, const byte *inbuf,
                                         size_t inbuflen) {
  int j, x;
  size_t n, i;
...
#error Invalid block size
  ...
  if (outbuflen + 8 < inbuflen) return GPG_ERR_BUFFER_TOO_SHORT;
  /* Input data must be multiple of 64 bits.  */
  if (inbuflen % 8) return GPG_ERR_INV_ARG;
  ...
  for (x = 0; x < 8 && x < sizeof(i); x++) t[7 - x] = i >> (8 * x);
  for (; x < 8; x++) t[7 - x] = 0;
