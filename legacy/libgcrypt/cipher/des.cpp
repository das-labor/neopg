...
static int working_memcmp(const void *_a, const void *_b, size_t n) {
  ...
  for (; n; n--, a++, b++)
    if (*a != *b) return (int)(*(byte *)a) - (int)(*(byte *)b);
  return 0;
}
#endif
...
static int tripledes_set2keys(struct _tripledes_ctx *, const byte *,
                              const byte *);
static int tripledes_set3keys(struct _tripledes_ctx *, const byte *,
                              const byte *, const byte *);
static int tripledes_ecb_crypt(struct _tripledes_ctx *, const byte *, byte *,
                               int);
...
static int is_weak_key(const byte *key) {
  ...
  /* clear parity bits */
  for (i = 0; i < 8; ++i) work[i] = key[i] & 0xfe;
  ...
  while (left <= right) {
    ...
    if (!(cmp_result = working_memcmp(work, weak_keys[middle], 8))) return -1;
...
static gpg_error_t bulk_selftest_setkey(void *context, const byte *__key,
                                        unsigned __keylen) {
...
static const char *selftest(void) {
   ...
   * need this.
   */
  if (sizeof(u32) != 4) return "Wrong word size for DES configured.";
   ...
   */
  {
    ...
    if (memcmp(temp3, result, 8)) return "DES maintenance test failed.";
  }
   ...
   */
  {
    ...
    if (memcmp(input, result, 8)) return "Triple-DES test failed.";
  }
   ...
   */
  {
    ...
    if (_gcry_md_open(&h, GCRY_MD_SHA1, 0)) return "SHA1 not available";

    for (i = 0; i < 64; ++i) _gcry_md_write(h, weak_keys[i], 8);
    p = _gcry_md_read(h, GCRY_MD_SHA1);
    i = memcmp(p, weak_keys_chksum, 20);
    _gcry_md_close(h);
    if (i) return "weak key table defect";

    for (i = 0; i < 64; ++i)
      if (!is_weak_key(weak_keys[i])) return "DES weak key detection failed";
  }

  if ((r = selftest_cbc())) return r;

  if ((r = selftest_cfb())) return r;

  if ((r = selftest_ctr())) return r;
...
static gpg_error_t do_tripledes_setkey(void *context, const byte *key,
  ...
  if (keylen != 24) return GPG_ERR_INV_KEYLEN;
...
static gpg_error_t do_tripledes_set_extra_info(void *context, int what,
                                               const void *buffer,
                                               size_t buflen) {
  struct _tripledes_ctx *ctx = (struct _tripledes_ctx *)context;
  gpg_error_t ec = 0;
...
static gpg_error_t do_des_setkey(void *context, const byte *key,
                                 unsigned keylen) {
  struct _des_ctx *ctx = (struct _des_ctx *)context;

  if (keylen != 8) return GPG_ERR_INV_KEYLEN;
...
static gpg_error_t selftest_fips(int extended, selftest_report_func_t report) {
  const char *what;
  const char *errtxt;
...
failed:
  if (report) report("cipher", GCRY_CIPHER_3DES, what, errtxt);
