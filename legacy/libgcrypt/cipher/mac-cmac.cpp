...
static gpg_error_t cmac_open(gcry_mac_hd_t h) {
  gpg_error_t err;
  gcry_cipher_hd_t hd;
...
static gpg_error_t cmac_setkey(gcry_mac_hd_t h, const unsigned char *key,
                               size_t keylen) {
  return _gcry_cipher_setkey(h->u.cmac.ctx, key, keylen);
}

static gpg_error_t cmac_reset(gcry_mac_hd_t h) {
  return _gcry_cipher_reset(h->u.cmac.ctx);
}

static gpg_error_t cmac_write(gcry_mac_hd_t h, const unsigned char *buf,
                              size_t buflen) {
  return _gcry_cipher_cmac_authenticate(h->u.cmac.ctx, buf, buflen);
}

static gpg_error_t cmac_read(gcry_mac_hd_t h, unsigned char *outbuf,
                             size_t *outlen) {
  if (*outlen > h->u.cmac.blklen) *outlen = h->u.cmac.blklen;
  return _gcry_cipher_cmac_get_tag(h->u.cmac.ctx, outbuf, *outlen);
}

static gpg_error_t cmac_verify(gcry_mac_hd_t h, const unsigned char *buf,
                               size_t buflen) {
...
gcry_mac_spec_t _gcry_mac_type_spec_cmac_blowfish = {
    GCRY_MAC_CMAC_BLOWFISH, {0}, "CMAC_BLOWFISH", &cmac_ops};
#endif
#if USE_DES
gcry_mac_spec_t _gcry_mac_type_spec_cmac_tripledes = {
    GCRY_MAC_CMAC_3DES, {0}, "CMAC_3DES", &cmac_ops};
#endif
#if USE_CAST5
gcry_mac_spec_t _gcry_mac_type_spec_cmac_cast5 = {
    GCRY_MAC_CMAC_CAST5, {0}, "CMAC_CAST5", &cmac_ops};
#endif
#if USE_AES
gcry_mac_spec_t _gcry_mac_type_spec_cmac_aes = {
    GCRY_MAC_CMAC_AES, {0}, "CMAC_AES", &cmac_ops};
#endif
#if USE_TWOFISH
gcry_mac_spec_t _gcry_mac_type_spec_cmac_twofish = {
    GCRY_MAC_CMAC_TWOFISH, {0}, "CMAC_TWOFISH", &cmac_ops};
#endif
#if USE_SERPENT
gcry_mac_spec_t _gcry_mac_type_spec_cmac_serpent = {
    GCRY_MAC_CMAC_SERPENT, {0}, "CMAC_SERPENT", &cmac_ops};
#endif
#if USE_RFC2268
...
gcry_mac_spec_t _gcry_mac_type_spec_cmac_camellia = {
    GCRY_MAC_CMAC_CAMELLIA, {0}, "CMAC_CAMELLIA", &cmac_ops};
#endif
#ifdef USE_IDEA
gcry_mac_spec_t _gcry_mac_type_spec_cmac_idea = {
    GCRY_MAC_CMAC_IDEA, {0}, "CMAC_IDEA", &cmac_ops};
#endif
#if USE_GOST28147
gcry_mac_spec_t _gcry_mac_type_spec_cmac_gost28147 = {
    GCRY_MAC_CMAC_GOST28147, {0}, "CMAC_GOST28147", &cmac_ops};
#endif
