...
static unsigned int do_ghash(unsigned char *hsub, unsigned char *result,
  ...
  for (i = 0; i < 16; i++) {
    for (j = 0x80; j; j >>= 1) {
      if (hsub[i] & j) buf_xor(p, p, V, 16);
      if (bshift(V)) V[0] ^= 0xe1000000;
    }
  }
...
static inline int gcm_check_datalen(u32 ctr[2]) {
  ...
  if (ctr[1] > 0xfU) return 0;
  if (ctr[1] < 0xfU) return 1;

  if (ctr[0] <= 0xffffffe0U) return 1;
...
static inline int gcm_check_aadlen_or_ivlen(u32 ctr[2]) {
  ...
  if (ctr[1] > 0x1fffffffU) return 0;
  if (ctr[1] < 0x1fffffffU) return 1;

  if (ctr[0] <= 0xffffffffU) return 1;
...
static void do_ghash_buf(gcry_cipher_hd_t c, byte *hash, const byte *buf,
  ...
  if (buflen == 0 && (unused == 0 || !do_padding)) return;
  ...
  do {
    ...
    if (!buflen) {
      ...
      while (unused < blocksize) c->u_mode.gcm.macbuf[unused++] = 0;
    }
  ...
  if (burn) _gcry_burn_stack(burn);
}

gpg_error_t _gcry_cipher_gcm_encrypt(gcry_cipher_hd_t c, byte *outbuf,
                                     size_t outbuflen, const byte *inbuf,
                                     size_t inbuflen) {
  static unsigned char zerobuf[MAX_BLOCKSIZE];
  gpg_error_t err;

  if (c->spec->blocksize != GCRY_GCM_BLOCK_LEN) return GPG_ERR_CIPHER_ALGO;
  if (outbuflen < inbuflen) return GPG_ERR_BUFFER_TOO_SHORT;
  if (c->u_mode.gcm.datalen_over_limits) return GPG_ERR_INV_LENGTH;
...
gpg_error_t _gcry_cipher_gcm_decrypt(gcry_cipher_hd_t c, byte *outbuf,
                                     size_t outbuflen, const byte *inbuf,
                                     size_t inbuflen) {
  static unsigned char zerobuf[MAX_BLOCKSIZE];
...
gpg_error_t _gcry_cipher_gcm_authenticate(gcry_cipher_hd_t c,
                                          const byte *aadbuf,
                                          size_t aadbuflen) {
  static unsigned char zerobuf[MAX_BLOCKSIZE];
...
static gpg_error_t _gcry_cipher_gcm_initiv(gcry_cipher_hd_t c, const byte *iv,
                                           size_t ivlen) {
  ...
  if (ivlen == 0) return GPG_ERR_INV_LENGTH;

  if (ivlen != GCRY_GCM_BLOCK_LEN - 4) {
    u32 iv_bytes[2] = {0, 0};
    u32 bitlengths[2][2];

    if (!c->u_mode.gcm.ghash_fn) return GPG_ERR_INV_STATE;
...
gpg_error_t _gcry_cipher_gcm_setiv(gcry_cipher_hd_t c, const byte *iv,
                                   size_t ivlen) {
  c->marks.iv = 0;
  c->marks.tag = 0;
...
static gpg_error_t _gcry_cipher_gcm_tag(gcry_cipher_hd_t c, byte *outbuf,
  ...
  if (!check) {
    if (outbuflen > GCRY_GCM_BLOCK_LEN) outbuflen = GCRY_GCM_BLOCK_LEN;
...
gpg_error_t _gcry_cipher_gcm_check_tag(gcry_cipher_hd_t c,
