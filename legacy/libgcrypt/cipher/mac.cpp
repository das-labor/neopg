...
static gpg_error_t check_mac_algo(int algorithm) {
  gcry_mac_spec_t *spec;
 ...
 */
static gpg_error_t mac_open(gcry_mac_hd_t *hd, int algo, int secure,
                            gcry_ctx_t ctx) {
  gcry_mac_spec_t *spec;
  gpg_error_t err;
  gcry_mac_hd_t h;
...
static gpg_error_t mac_reset(gcry_mac_hd_t hd) {
  if (hd->spec->ops->reset) return hd->spec->ops->reset(hd);

  return 0;
}

static void mac_close(gcry_mac_hd_t hd) {
  if (hd->spec->ops->close) hd->spec->ops->close(hd);
...
static gpg_error_t mac_setkey(gcry_mac_hd_t hd, const void *key,
                              size_t keylen) {
  if (!hd->spec->ops->setkey) return GPG_ERR_INV_ARG;
  if (keylen > 0 && !key) return GPG_ERR_INV_ARG;

  return hd->spec->ops->setkey(hd, (const unsigned char *)key, keylen);
}

static gpg_error_t mac_setiv(gcry_mac_hd_t hd, const void *iv, size_t ivlen) {
  if (!hd->spec->ops->setiv) return GPG_ERR_INV_ARG;
  if (ivlen > 0 && !iv) return GPG_ERR_INV_ARG;

  return hd->spec->ops->setiv(hd, (const unsigned char *)iv, ivlen);
}

static gpg_error_t mac_write(gcry_mac_hd_t hd, const void *inbuf,
                             size_t inlen) {
  if (!hd->spec->ops->write) return GPG_ERR_INV_ARG;
  if (inlen > 0 && !inbuf) return GPG_ERR_INV_ARG;

  return hd->spec->ops->write(hd, (const unsigned char *)inbuf, inlen);
}

static gpg_error_t mac_read(gcry_mac_hd_t hd, void *outbuf, size_t *outlen) {
...
static gpg_error_t mac_verify(gcry_mac_hd_t hd, const void *buf,
                              size_t buflen) {
  if (!buf || buflen == 0 || !hd->spec->ops->verify) return GPG_ERR_INV_ARG;
...
gpg_error_t _gcry_mac_open(gcry_mac_hd_t *h, int algo, unsigned int flags,
                           gcry_ctx_t ctx) {
  gpg_error_t rc;
...
gpg_error_t _gcry_mac_write(gcry_mac_hd_t hd, const void *inbuf, size_t inlen) {
  return mac_write(hd, inbuf, inlen);
}

gpg_error_t _gcry_mac_read(gcry_mac_hd_t hd, void *outbuf, size_t *outlen) {
  return mac_read(hd, outbuf, outlen);
}

gpg_error_t _gcry_mac_verify(gcry_mac_hd_t hd, const void *buf, size_t buflen) {
  return mac_verify(hd, buf, buflen);
}

int _gcry_mac_get_algo(gcry_mac_hd_t hd) { return hd->algo; }

unsigned int _gcry_mac_get_algo_maclen(int algo) {
  gcry_mac_spec_t *spec;

  spec = spec_from_algo(algo);
  if (!spec || !spec->ops || !spec->ops->get_maclen) return 0;
...
unsigned int _gcry_mac_get_algo_keylen(int algo) {
  ...
  spec = spec_from_algo(algo);
  if (!spec || !spec->ops || !spec->ops->get_keylen) return 0;

  return spec->ops->get_keylen(algo);
}

gpg_error_t _gcry_mac_ctl(gcry_mac_hd_t hd, int cmd, void *buffer,
                          size_t buflen) {
  gpg_error_t rc;
