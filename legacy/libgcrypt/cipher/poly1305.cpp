...
#ifndef POLY1305_USE_SSE2
static OPS_FUNC_ABI unsigned int poly1305_blocks_ref32(void *state,
                                                       const byte *m,
                                                       size_t bytes) {
  poly1305_state_ref32_t *st = (poly1305_state_ref32_t *)state;
...
#ifndef POLY1305_USE_SSE2
static OPS_FUNC_ABI unsigned int poly1305_finish_ext_ref32(
    void *state, const byte *m, size_t remaining, byte mac[POLY1305_TAGLEN]) {
  poly1305_state_ref32_t *st = (poly1305_state_ref32_t *)state;
  ...
  if (remaining) {
    ...
    size_t i;
    for (i = 0; i < remaining; i++) final[i] = m[i];
    final[remaining] = 1;
    st->final = 1;
...
void _gcry_poly1305_update(poly1305_context_t *ctx, const byte *m,
                           size_t bytes) {
  void *state = poly1305_get_state(ctx);
  unsigned int burn = 0;
  ...
  if (ctx->leftover) {
    ...
    m += want;
    ctx->leftover += want;
    if (ctx->leftover < block_size) return;
