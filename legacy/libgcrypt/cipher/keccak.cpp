...
static unsigned int keccak_absorb_lanes32bi(KECCAK_STATE *hd, int pos,
  ...
  while (nlanes) {
    keccak_absorb_lane32bi(&hd->u.state32bi[pos * 2], buf_get_le32(lanes + 0),
...
static gpg_error_t selftests_keccak(int algo, int extended,
  ...
  if (extended) {
    ...
    what = "one million \"a\"";
    errtxt = _gcry_hash_selftest_check_one(algo, 1, NULL, 0, one_million_a_hash,
                                           hash_len);
    if (errtxt) goto failed;
  }
...
failed:
  if (report) report("digest", algo, what, errtxt);
