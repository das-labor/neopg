...
gcry_mpi_t _gcry_dsa_gen_k(gcry_mpi_t q) {
  ...
  for (;;) {
    ...
    {
      if (DBG_CIPHER) log_debug("\tk too large - again\n");
      continue; /* no  */
    }
    if (!(mpi_cmp_ui(k, 0) > 0)) /* check: k > 0 */
    {
      if (DBG_CIPHER) log_debug("\tk is zero - again\n");
      continue; /* no */
    }
    break; /* okay */
  }
...
static gpg_error_t int2octets(unsigned char **r_frame, gcry_mpi_t value,
                              size_t nbytes) {
  gpg_error_t rc;
...
/* Connert the bit string BITS of length NBITS into an octet string
   ...
   R_FRAME.  */
static gpg_error_t bits2octets(unsigned char **r_frame, const void *bits,
  ...
  if (rc) return rc;
  if (nbits > qbits) mpi_rshift(z1, z1, nbits - qbits);

  /* z2 - z1 mod q */
  if (mpi_cmp(z1, q) >= 0) mpi_sub(z1, z1, q);
...
gpg_error_t _gcry_dsa_gen_rfc6979_k(gcry_mpi_t *r_k, gcry_mpi_t dsa_q,
  ...
  if (!qbits || !h1 || !hlen) return GPG_ERR_EINVAL;

  if (_gcry_md_get_algo_dlen(halgo) != hlen) return GPG_ERR_DIGEST_ALGO;
...
again:
  ...
  if (rc) goto leave;
  if (tbits > qbits) mpi_rshift(k, k, tbits - qbits);
...
gpg_error_t _gcry_dsa_normalize_hash(gcry_mpi_t input, gcry_mpi_t *out,
  ...
  if (mpi_is_opaque(input)) {
    ...
    if (rc) return rc;
    if (abits > qbits) mpi_rshift(hash, hash, abits - qbits);
  } else
    hash = input;
