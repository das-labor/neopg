...
static gpg_error_t openpgp_s2k(const void *passphrase, size_t passphraselen,
  ...
  for (pass = 0; used < keysize; pass++) {
    ...
    if (i > keysize - used) i = keysize - used;
...
gpg_error_t _gcry_kdf_pkdf2(const void *passphrase, size_t passphraselen,
  ...
  if (!salt || !iterations || !dklen) return GPG_ERR_INV_VALUE;

  hlen = _gcry_md_get_algo_dlen(hashalgo);
  if (!hlen) return GPG_ERR_DIGEST_ALGO;
 ...
 * a larger value.  */
  ...
  if (dklen > 0xffffffffU) return GPG_ERR_INV_VALUE;
#endif
  ...
  sbuf = (char *)(secmode ? xtrymalloc_secure(saltlen + 4 + hlen + hlen)
                          : xtrymalloc(saltlen + 4 + hlen + hlen));
  if (!sbuf) return gpg_error_from_syserror();
  tbuf = sbuf + saltlen + 4;
  ubuf = tbuf + hlen;

  ec = _gcry_md_open(&md, hashalgo,
  ...
  for (lidx = 1; lidx <= l; lidx++) {
    for (iter = 0; iter < iterations; iter++) {
      ...
      } else /* Compute U_(2..c):  */
      {
        ...
        memcpy(ubuf, _gcry_md_read(md, 0), hlen);
        for (i = 0; i < hlen; i++) tbuf[i] ^= ubuf[i];
      }
    }
  ...
  switch (algo) {
    ...
    case GCRY_KDF_PBKDF2:
      if (!saltlen)
        ...
        ec = _gcry_kdf_pkdf2(passphrase, passphraselen, subalgo, salt, saltlen,
...
#if USE_SCRYPT
      ec = _gcry_kdf_scrypt((const unsigned char *)(passphrase), passphraselen,
                            algo, subalgo, (const unsigned char *)(salt),
                            saltlen, iterations, keysize,
