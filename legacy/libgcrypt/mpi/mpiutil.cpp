...
const char *_gcry_mpi_get_hw_config(void) { return ""; }
...
mpi_ptr_t _gcry_mpi_alloc_limb_space(unsigned int nlimbs, int secure) {
  ...
  p = (mpi_ptr_t)(secure ? xmalloc_secure(len) : xmalloc(len));
  if (!nlimbs) *p = 0;
...
void _gcry_mpi_resize(gcry_mpi_t a, unsigned nlimbs) {
  ...
  if (a->d) {
    a->d = (mpi_limb_t *)xrealloc(a->d, nlimbs * sizeof(mpi_limb_t));
    for (i = a->alloced; i < nlimbs; i++) a->d[i] = 0;
  } else {
    if (a->flags & 1) /* Secure memory is wanted.  */
      a->d = (mpi_limb_t *)xcalloc_secure(nlimbs, sizeof(mpi_limb_t));
    else
      /* Standard memory.  */
      a->d = (mpi_limb_t *)xcalloc(nlimbs, sizeof(mpi_limb_t));
  }
  a->alloced = nlimbs;
}
...
void _gcry_mpi_free(gcry_mpi_t a) {
  ...
  if ((a->flags &
       ~(1 | 2 | 4 | 16 | GCRYMPI_FLAG_USER1 | GCRYMPI_FLAG_USER2 |
...
gcry_mpi_t _gcry_mpi_set_opaque(gcry_mpi_t a, void *p, unsigned int nbits) {
  ...
  a->flags = 4 | (a->flags & (GCRYMPI_FLAG_USER1 | GCRYMPI_FLAG_USER2 |
                              GCRYMPI_FLAG_USER3 | GCRYMPI_FLAG_USER4));
  if (_gcry_is_secure(a->d)) a->flags |= 1;
  return a;
}
...
gcry_mpi_t _gcry_mpi_copy(gcry_mpi_t a) {
  ...
  if (a && (a->flags & 4)) {
    void *p = _gcry_is_secure(a->d) ? xmalloc_secure((a->sign + 7) / 8)
                                    : xmalloc((a->sign + 7) / 8);
    if (a->d) memcpy(p, a->d, (a->sign + 7) / 8);
    b = mpi_set_opaque(NULL, p, a->sign);
    b->flags &= ~(16 | 32); /* Reset the immutable and constant flags.  */
  } else if (a) {
    b = mpi_is_secure(a) ? mpi_alloc_secure(a->nlimbs) : mpi_alloc(a->nlimbs);
    b->nlimbs = a->nlimbs;
    b->sign = a->sign;
    b->flags = a->flags;
    b->flags &= ~(16 | 32); /* Reset the immutable and constant flags.  */
...
gcry_mpi_t _gcry_mpi_set(gcry_mpi_t w, gcry_mpi_t u) {
  ...
  if (!w) w = _gcry_mpi_alloc(mpi_get_nlimbs(u));
  if (mpi_is_immutable(w)) {
    mpi_immutable_failed();
    return w;
  }
...
void _gcry_mpi_release(gcry_mpi_t a) { _gcry_mpi_free(a); }
...
void _gcry_mpi_randomize(gcry_mpi_t w, unsigned int nbits,
  ...
  if (level == GCRY_WEAK_RANDOM) {
    p = (unsigned char *)(mpi_is_secure(w) ? xmalloc_secure(nbytes)
                                           : xmalloc(nbytes));
    _gcry_create_nonce(p, nbytes);
  } else {
    p = (unsigned char *)(mpi_is_secure(w)
                              ? _gcry_random_bytes_secure(nbytes, level)
...
gcry_mpi_t _gcry_mpi_const(enum gcry_mpi_constants no) {
  if ((int)no < 0 || no > MPI_NUMBER_OF_CONSTANTS)
    log_bug("invalid mpi_const selector %d\n", no);
  if (!constants[no]) log_bug("MPI subsystem not initialized\n");
  return constants[no];
}
