...
static mpi_limb_t mul_n_basecase(mpi_ptr_t prodp, mpi_ptr_t up, mpi_ptr_t vp,
  ...
  for (i = 1; i < size; i++) {
    ...
    if (v_limb <= 1) {
      cy = 0;
      if (v_limb == 1) cy = _gcry_mpih_add_n(prodp, prodp, up, size);
    } else
...
static void mul_n(mpi_ptr_t prodp, mpi_ptr_t up, mpi_ptr_t vp, mpi_size_t size,
  ...
  if (size & 1) {
    ...
    if (cy) _gcry_mpih_add_1(prodp + size, prodp + size, size, 1);
  }
}
...
void _gcry_mpih_sqr_n_basecase(mpi_ptr_t prodp, mpi_ptr_t up, mpi_size_t size) {
  ...
  for (i = 1; i < size; i++) {
    ...
    if (v_limb <= 1) {
      cy_limb = 0;
      if (v_limb == 1) cy_limb = _gcry_mpih_add_n(prodp, prodp, up, size);
    } else
...
void _gcry_mpih_mul_karatsuba_case(mpi_ptr_t prodp, mpi_ptr_t up,
  ...
  if (!ctx->tspace || ctx->tspace_size < vsize) {
    if (ctx->tspace) _gcry_mpi_free_limb_space(ctx->tspace, ctx->tspace_nlimbs);
  ...
  if (usize >= vsize) {
    if (!ctx->tp || ctx->tp_size < vsize) {
      if (ctx->tp) _gcry_mpi_free_limb_space(ctx->tp, ctx->tp_nlimbs);
...
mpi_limb_t _gcry_mpih_mul(mpi_ptr_t prodp, mpi_ptr_t up, mpi_size_t usize,
  ...
  if (vsize < KARATSUBA_THRESHOLD) {
    ...
    for (i = 1; i < vsize; i++) {
      ...
      if (v_limb <= 1) {
        cy = 0;
        if (v_limb == 1) cy = _gcry_mpih_add_n(prodp, prodp, up, usize);
      } else
