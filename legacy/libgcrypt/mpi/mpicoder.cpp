...
static unsigned char *do_get_buffer(gcry_mpi_t a, unsigned int fill_le,
  ...
  *nbytes = a->nlimbs * BYTES_PER_MPI_LIMB;
  ...
  retbuffer = (unsigned char *)((force_secure || mpi_is_secure(a))
                                    ? xtrymalloc_secure(n2)
                                    : xtrymalloc(n2));
  if (!retbuffer) return NULL;
  if (extraalloc < 0)
...
#error please implement for this limb size.
  ...
  if (fill_le) {
    ...
    /* Pad with zeroes.  */
    for (p = buffer + length; length < fill_le; length++) *p++ = 0;
    *nbytes = length;
...
static void onecompl(gcry_mpi_t a) {
  ...
  for (i = 0; i < n; i++) ap[i] ^= (mpi_limb_t)(-1);
...
gpg_error_t _gcry_mpi_scan(struct gcry_mpi **ret_mpi,
                           enum gcry_mpi_format format, const void *buffer_arg,
  ...
  if (format == GCRYMPI_FMT_STD) {
    ...
    /* This test is not strictly necessary and an assert (!len)
       ...
       below. */
    if (len && len < 4) return GPG_ERR_TOO_SHORT;

    n = (s[0] << 24 | s[1] << 16 | s[2] << 8 | s[3]);
    s += 4;
    if (len) len -= 4;
    if (len && n > len) return GPG_ERR_TOO_LARGE;
    ...
    if (buflen) return GPG_ERR_INV_ARG;
    ...
    if (ret_mpi) {
      ...
      *ret_mpi = a;
      ...
      mpi_free(a);
    if (nscanned) *nscanned = strlen((const char *)buffer);
    return 0;
  } else
  ...
  *nwritten = 0;
  if (format == GCRYMPI_FMT_STD) {
    ...
    *nwritten = n;
    ...
    if (buffer && n > len) return GPG_ERR_TOO_SHORT;
    if (buffer) {
      unsigned char *tmp;
    ...
    *nwritten = n;
    ...
    if (buffer && n + 2 > len) return GPG_ERR_TOO_SHORT;
...
gpg_error_t _gcry_mpi_aprint(enum gcry_mpi_format format,
  ...
  *buffer = (unsigned char *)(mpi_is_secure(a) ? xtrymalloc_secure(n ? n : 1)
                                               : xtrymalloc(n ? n : 1));
...
gpg_error_t _gcry_mpi_to_octet_string(unsigned char **r_frame, void *space,
                                      gcry_mpi_t value, size_t nbytes) {
  gpg_error_t rc;
  size_t nframe, noff, n;
  unsigned char *frame;

  if (!r_frame == !space) return GPG_ERR_INV_ARG; /* Only one may be used.  */
  ...
  if (space)
    ...
    frame = (unsigned char *)(mpi_is_secure(value) ? xtrymalloc_secure(n)
                                                   : xtrymalloc(n));
    if (!frame) {
