...
void _gcry_mpi_powm(gcry_mpi_t res, gcry_mpi_t base, gcry_mpi_t expo,
  ...
  /* Main processing.  */
  {
      ...
      while (c) {
        ...
        if (esec || (mpi_limb_signed_t)e < 0) {
          ...
          if (bsize < KARATSUBA_THRESHOLD)
            ...
            _gcry_mpih_mul_karatsuba_case(xp, rp, rsize, bp, bsize, &karactx);
    ...
    if (mod_shift_cnt) _gcry_mpih_rshift(rp, rp, rsize, mod_shift_cnt);
  ...
  if (negative_result && rsize) {
    if (mod_shift_cnt) _gcry_mpih_rshift(mp, mp, msize, mod_shift_cnt);
...
void _gcry_mpi_powm(gcry_mpi_t res, gcry_mpi_t base, gcry_mpi_t expo,
  ...
  /* Main processing.  */
  {
    ...
    for (i = 1; i < (1 << (W - 1)); i++) { /* PRECOMP[i] = BASE^(2 * i + 1) */
      ...
      if (max_u_size < base_u_size) max_u_size = base_u_size;
      MPN_COPY(precomp[i], rp, rsize);
    }

    if (msize > max_u_size) max_u_size = msize;
    ...
    for (;;)
      ...
      last_step:
        ...
        for (j += W - c0; j >= 0; j--) {
          ...
          mul_mod(xp, &xsize, rp, rsize, base_u, base_u_size, mp, msize,
                  &karactx);
          tp = rp;
          rp = xp;
          xp = tp;
