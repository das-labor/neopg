...
static void point_resize(mpi_point_t p, mpi_ec_t ctx) {
  ...
  if (ctx->model != MPI_EC_MONTGOMERY) mpi_resize(p->y, nlimbs);
...
void _gcry_mpi_point_get(gcry_mpi_t x, gcry_mpi_t y, gcry_mpi_t z,
  ...
  if (y) mpi_set(y, point->y);
  if (z) mpi_set(z, point->z);
}
...
mpi_point_t _gcry_mpi_point_set(mpi_point_t point, gcry_mpi_t x, gcry_mpi_t y,
                                gcry_mpi_t z) {
  if (!point) point = mpi_point_new(0);
...
gpg_error_t _gcry_mpi_ec_p_new(gcry_ctx_t *r_ctx, enum gcry_mpi_ec_models model,
                               enum ecc_dialects dialect, int flags,
                               gcry_mpi_t p, gcry_mpi_t a, gcry_mpi_t b) {
  gcry_ctx_t ctx;
  mpi_ec_t ec;
...
gpg_error_t _gcry_mpi_ec_decode_point(mpi_point_t result, gcry_mpi_t value,
                                      mpi_ec_t ec) {
  gpg_error_t rc;
...
int _gcry_mpi_ec_get_affine(gcry_mpi_t x, gcry_mpi_t y, mpi_point_t point,
  ...
  switch (ctx->model) {
    ...
    case MPI_EC_EDWARDS: {
      ...
      if (x) ec_mulm(x, point->x, z, ctx);
      if (y) ec_mulm(y, point->y, z, ctx);
...
int _gcry_mpi_ec_curve_point(gcry_mpi_point_t point, mpi_ec_t ctx) {
  ...
  switch (ctx->model) {
    case MPI_EC_WEIERSTRASS: {
      ...
      if (_gcry_mpi_ec_get_affine(x, y, point, ctx)) goto leave;
...
#define xx y
      ...
      if (_gcry_mpi_ec_get_affine(x, NULL, point, ctx)) goto leave;
