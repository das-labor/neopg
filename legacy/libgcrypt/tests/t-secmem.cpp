...
static void test_secmem(void) {
  ...
  for (i = 0; i < DIM(a); i++) a[i] = gcry_xmalloc_secure(512);

  /* Allocating another 2k should fail for the default 16k pool.  */
  b = gcry_malloc_secure(2048);
  if (b) fail("allocation did not fail as expected\n");

  for (i = 0; i < DIM(a); i++) xfree(a[i]);
  xfree(b);
}

static void test_secmem_overflow(void) {
  void *a[150];
  int i;
...
static int outofcore_handler(void *opaque, size_t req_n, unsigned int flags) {
...
int secmem_main(int argc, char **argv) {
  ...
  if (!verbose) xgcry_control(GCRYCTL_DISABLE_SECMEM_WARN, 0);
