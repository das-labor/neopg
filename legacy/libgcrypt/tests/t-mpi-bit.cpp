...
/* Allocate a bit string consisting of '0' and '1' from the MPI
   ...
   the result. */
static char *mpi2bitstr(gcry_mpi_t a, size_t length) {
  char *p, *buf;

  buf = p = xmalloc(length + 1);
  while (length--) *p++ = gcry_mpi_test_bit(a, length) ? '1' : '0';
  *p = 0;
...
static void rshiftbitstring(char *string, size_t n) {
...
static char *lshiftbitstring(const char *string, size_t n) {
  size_t len = strlen(string);
  char *result;

  if (len + n + 1 < len) die("internal overflow\n");
...
static void one_bit_only(int highbit) {
  gcry_mpi_t a;
  char *result;
  int i;
  ...
  if (!gcry_mpi_test_bit(a, 42)) fail("failed to set a bit\n");
  gcry_mpi_clear_bit(a, 42);
  if (gcry_mpi_test_bit(a, 42)) fail("failed to clear a bit\n");
  result = mpi2bitstr(a, 70);
  assert(strlen(result) == 70);
  for (i = 0; result[i]; i++)
    if (result[i] != '0') break;
  if (result[i]) fail("spurious bits detected\n");
  xfree(result);
  gcry_mpi_release(a);
}
...
static void test_rshift(int pass) {
  gcry_mpi_t a, b;
  char *result, *result2;
  int i;
...
static void test_lshift(int pass) {
...
static void set_bit_with_resize(void) {
  gcry_mpi_t a;
  int i;
  ...
  if (!gcry_mpi_test_bit(a, 1536)) fail("failed to set a bit\n");
  for (i = 0; i < 1536; i++) {
  ...
  if (gcry_mpi_test_bit(a, 1537)) fail("more bits set than expected\n");
  gcry_mpi_release(a);
