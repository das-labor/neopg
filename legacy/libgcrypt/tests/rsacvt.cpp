...
static gcry_mpi_t read_hexmpi_line(FILE *fp, int *got_eof) {
  gpg_error_t err;
  gcry_mpi_t a;
  char *line;
...
static int skip_to_empty_line(FILE *fp) {
  char line[256];
  char *p;
...
static void print_mpi_line(const char *label, gcry_mpi_t a) {
  unsigned char *buf, *p;
  gcry_error_t err;
  int writerr = 0;

  if (with_labels && label) printf("%s = ", label);

  err = gcry_mpi_aprint(GCRYMPI_FMT_HEX, &buf, NULL, a);
  if (err) die("gcry_mpi_aprint failed: %s\n", gpg_strerror(err));

  p = buf;
  if (!keep_lz && p[0] == '0' && p[1] == '0' && p[2]) p += 2;
...
static void compute_missing(gcry_mpi_t rsa_p, gcry_mpi_t rsa_q,
                            gcry_mpi_t rsa_e) {
  ...
  if (openpgp_mode) print_mpi_line("d", rsa_d);
...
int main(int argc, char **argv) {
  ...
  if (!gcry_check_version("1.4.0")) die("Libgcrypt is not sufficient enough\n");
  xgcry_control(GCRYCTL_DISABLE_SECMEM, 0);
  xgcry_control(GCRYCTL_INITIALIZATION_FINISHED, 0);

  do {
    rsa_p = read_hexmpi_line(input, &got_eof);
    if (!rsa_p && got_eof) break;
    if (!rsa_p) die("RSA parameter 'p' missing or not properly hex encoded\n");
    rsa_q = read_hexmpi_line(input, &got_eof);
    if (!rsa_q) die("RSA parameter 'q' missing or not properly hex encoded\n");
    rsa_e = read_hexmpi_line(input, &got_eof);
    if (!rsa_e) die("RSA parameter 'e' missing or not properly hex encoded\n");
