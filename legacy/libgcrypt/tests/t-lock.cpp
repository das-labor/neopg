...
static void external_lock_test_init(int line) {
  gpg_error_t err;

  err = gcry_control(PRIV_CTL_EXTERNAL_LOCK_TEST, EXTERNAL_LOCK_TEST_INIT);
  if (err) fail("init lock failed at %d: %s", line, gpg_strerror(err));
}

static void external_lock_test_lock(int line) {
  gpg_error_t err;

  err = gcry_control(PRIV_CTL_EXTERNAL_LOCK_TEST, EXTERNAL_LOCK_TEST_LOCK);
  if (err) fail("taking lock failed at %d: %s", line, gpg_strerror(err));
}

static void external_lock_test_unlock(int line) {
  gpg_error_t err;

  err = gcry_control(PRIV_CTL_EXTERNAL_LOCK_TEST, EXTERNAL_LOCK_TEST_UNLOCK);
  if (err) fail("releasing lock failed at %d: %s", line, gpg_strerror(err));
}

static void external_lock_test_destroy(int line) {
  gpg_error_t err;

  err = gcry_control(PRIV_CTL_EXTERNAL_LOCK_TEST, EXTERNAL_LOCK_TEST_DESTROY);
  if (err) fail("destroying lock failed at %d: %s", line, gpg_strerror(err));
}

/* The nonce thread.  We simply request a couple of nonces and
   return.  */
static THREAD_RET_TYPE nonce_thread(void *argarg) {
  ...
  for (i = 0; i < N_NONCE_ITERATIONS; i++) {
    ...
    if (i && !(i % 100)) info("thread %d created %d nonces so far", arg->no, i);
  }
...
static void check_nonce_lock(void) {
  ...
  for (i = 0; i < N_NONCE_THREADS; i++) {
    ...
    if (!threads[i])
      die("error creating nonce thread %d: rc=%d", i, (int)GetLastError());
  }
  ...
  for (i = 0; i < N_NONCE_THREADS; i++) {
    ...
    if (rc == WAIT_OBJECT_0)
      ...
      fail("waiting for nonce thread %d failed: %d", i, (int)GetLastError());
  ...
  for (i = 0; i < N_NONCE_THREADS; i++) {
    ...
    if (rc)
      fail("pthread_join failed for nonce thread %d: %s", i, strerror(errno));
    else
      info("nonce thread %d has terminated", i);
  }
...
static void init_accounts(void) {
  ...
  for (i = 0; i < N_ACCOUNT; i++) account[i] = ACCOUNT_VALUE;
}
...
static void check_accounts(void) {
  ...
  sum = 0;
  for (i = 0; i < N_ACCOUNT; i++) sum += account[i];
  if (sum != N_ACCOUNT * ACCOUNT_VALUE) die("accounts out of balance");
}

static void print_accounts(void) {
  int i;

  for (i = 0; i < N_ACCOUNT; i++) printf("account %d: %6d\n", i, account[i]);
}

/* Get a a random integer value in the range 0 to HIGH.  */
static unsigned int get_rand(int high) {
  return (unsigned int)(1 +
                        (int)((double)(high + 1) * rand() / (RAND_MAX + 1.0))) -
         1;
}

/* Pick a random account.  Note that this function is not
   thread-safe. */
static int pick_account(void) { return get_rand(N_ACCOUNT - 1); }

/* Pick a random value for a transaction.  This is not thread-safe.  */
static int pick_value(void) { return get_rand(MAX_TRANSACTION_VALUE); }

/* This is the revision department.  */
static THREAD_RET_TYPE revision_thread(void *arg) {
  (void)arg;
...
static THREAD_RET_TYPE accountant_thread(void *arg) {
  int i;
  int acc1, acc2;
  int value;
...
static void run_test(void) {
  ...
  for (i = 0; i < N_ACCOUNTANTS; i++) {
    ...
    if (!athreads[i])
      die("error creating accountant thread %d: rc=%d", i, (int)GetLastError());
  }
  ...
  for (i = 0; i < N_ACCOUNTANTS; i++) {
    ...
    if (rc == WAIT_OBJECT_0)
      ...
      fail("waiting for accountant thread %d failed: %d", i,
           (int)GetLastError());
  ...
  for (i = 0; i < N_ACCOUNTANTS; i++) {
    ...
    if (rc)
      fail("pthread_join failed for accountant thread %d: %s", i,
           strerror(errno));
    else
      info("accountant thread %d has terminated", i);
  }
