...
static void mismatch(const void *expected, size_t expectedlen,
...
static void show_note(const char *format, ...) {
  va_list arg_ptr;
...
static void show_md_not_available(int algo) {
  static int list[100];
  static int listlen;
  int i;
  ...
  for (i = 0; i < listlen; i++)
    if (algo == list[i]) return; /* Note already printed.  */
  if (listlen < DIM(list)) list[listlen++] = algo;
  show_note("hash algorithm %d not available - skipping tests", algo);
}

static void show_old_hmac_not_available(int algo) {
  static int list[100];
  static int listlen;
  int i;
  ...
  if (listlen < DIM(list)) list[listlen++] = algo;
  show_note(
      "hash algorithm %d for old HMAC API not available "
      "- skipping tests",
      algo);
}

static void show_mac_not_available(int algo) {
  static int list[100];
  static int listlen;
  int i;
...
void progress_handler(void *cb_data, const char *what, int printchar,
...
static void check_cbc_mac_cipher(void) {
  static const struct tv {
    ...
    char mac[MAX_DATA_LEN];
      ...
      {GCRY_CIPHER_AES, "chicken teriyaki",
      ...
      {GCRY_CIPHER_3DES, "abcdefghABCDEFGH01234567",
       "This is a sample plaintext for CBC MAC of sixtyfour bytes.......", 0,
       "\x5c\x11\xf0\x01\x47\xbd\x3d\x3a"},
      {GCRY_CIPHER_DES, "abcdefgh",
       "This is a sample plaintext for CBC MAC of sixtyfour bytes.......", 0,
  ...
  if (verbose) fprintf(stderr, "  Starting CBC MAC checks.\n");
  ...
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    ...
    err = gcry_cipher_open(&hd, tv[i].algo, GCRY_CIPHER_MODE_CBC,
                           GCRY_CIPHER_CBC_MAC);
    if (!hd) {
    ...
    err = gcry_cipher_encrypt(hd, out, blklen, tv[i].plaintext,
                              tv[i].plaintextlen
                                  ? tv[i].plaintextlen
  ...
  if (verbose) fprintf(stderr, "  Completed CBC MAC checks.\n");
}
...
static void check_aes128_cbc_cts_cipher(void) {
  ...
  if (verbose) fprintf(stderr, "  Starting AES128 CBC CTS checks.\n");
  err = gcry_cipher_open(&hd, GCRY_CIPHER_AES, GCRY_CIPHER_MODE_CBC,
                         GCRY_CIPHER_CBC_CTS);
  if (err) {
  ...
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    ...
    err = gcry_cipher_encrypt(hd, out, MAX_DATA_LEN, plaintext, tv[i].inlen);
    if (err) {
  ...
  gcry_cipher_close(hd);
  if (verbose) fprintf(stderr, "  Completed AES128 CBC CTS checks.\n");
}
...
static void check_ctr_cipher(void) {
  static const struct tv {
    ...
    {GCRY_CIPHER_AES,
     ...
     {{"\x6b\xc1\xbe\xe2\x2e\x40\x9f\x96\xe9\x3d\x7e\x11\x73\x93\x17\x2a", 16,
      ...
      {"\xae\x2d\x8a\x57\x1e\x03\xac\x9c\x9e\xb7\x6f\xac\x45\xaf\x8e\x51", 16,
    ...
    {GCRY_CIPHER_AES,
     ...
     {{"\x6b\xc1\xbe\xe2\x2e\x40\x9f\x96\xe9\x3d\x7e\x11\x73\x93\x17\x2a", 16,
       "\x87\x4d\x61\x91\xb6\x20\xe3\x26\x1b\xef\x68\x64\x99\x0d\xb6\xce"},
      {"\xae\x2d\x8a\x57\x1e\x03\xac\x9c\x9e\xb7\x6f\xac\x45\xaf\x8e", 15,
    ...
    {GCRY_CIPHER_AES,
     "\x2b\x7e\x15\x16\x28\xae\xd2\xa6\xab\xf7\x15\x88\x09\xcf\x4f\x3c",
     "\xf0\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8\xf9\xfa\xfb\xfc\xfd\xfe\xff",
     {{"\x6b\xc1\xbe\xe2\x2e\x40\x9f\x96\xe9\x3d\x7e\x11\x73\x93\x17", 15,
    ...
    {GCRY_CIPHER_AES,
     ...
     {{"\x6b", 1, "\x87"},
      {"\xc1\xbe", 2, "\x4d\x61"},
      {"\xe2\x2e\x40", 3, "\x91\xb6\x20"},
      {"\x9f", 1, "\xe3"},
      {"\x96\xe9\x3d\x7e\x11\x73\x93\x17\x2a", 9,
       "\x26\x1b\xef\x68\x64\x99\x0d\xb6\xce"},
      {"\xae\x2d\x8a\x57\x1e\x03\xac\x9c\x9e\xb7\x6f\xac\x45\xaf\x8e", 15,
       "\x98\x06\xf6\x6b\x79\x70\xfd\xff\x86\x17\x18\x7b\xb9\xff\xfd"},
      {"\x51\x30\xc8\x1c\x46\xa3\x5c\xe4\x11", 9,
    ...
    {GCRY_CIPHER_CAST5,
     ...
     {{"\x6b\xc1\xbe\xe2\x2e\x40\x9f\x96\xe9\x3d\x7e\x11\x73\x93\x17\x2a", 16,
      ...
      {"\xae\x2d\x8a\x57\x1e\x03\xac\x9c", 8,
       "\x16\xe8\x72\x77\xb0\x98\x29\x68"},
      {"\x9e\xb7\x6f\xac\x45\xaf\x8e\x51", 8,
       "\x9a\xb3\xa8\x03\x3b\xb4\x14\xba"},
      {"\xae\x2d\x8a\x57\x1e\x03\xac\x9c\xa1\x00", 10,
  ...
  if (verbose) fprintf(stderr, "  Starting CTR cipher checks.\n");
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    if (!tv[i].algo) continue;
    ...
    if (!err) err = gcry_cipher_open(&hdd, tv[i].algo, GCRY_CIPHER_MODE_CTR, 0);
    if (err) {
    ...
    if (!err) err = gcry_cipher_setkey(hdd, tv[i].key, keylen);
    if (err) {
    ...
    if (!err) err = gcry_cipher_setctr(hdd, tv[i].ctr, blklen);
    if (err) {
    ...
    for (j = 0; tv[i].data[j].inlen; j++) {
      err = gcry_cipher_encrypt(hde, out, MAX_DATA_LEN, tv[i].data[j].plaintext,
                                tv[i].data[j].inlen == -1
      ...
      if (memcmp(tv[i].data[j].out, out, tv[i].data[j].inlen)) {
        ...
        mismatch(tv[i].data[j].out, tv[i].data[j].inlen, out,
                 tv[i].data[j].inlen);
      }
      ...
      if (memcmp(tv[i].data[j].plaintext, out, tv[i].data[j].inlen)) {
        ...
        mismatch(tv[i].data[j].plaintext, tv[i].data[j].inlen, out,
                 tv[i].data[j].inlen);
      }
    }
    ...
    /* Now check that we get valid return codes back for good and
       bad inputs.  */
    err = gcry_cipher_encrypt(hde, out, MAX_DATA_LEN, "1234567890123456", 16);
    if (err) fail("aes-ctr, encryption failed for valid input");
    ...
    err = gcry_cipher_decrypt(hde, out, MAX_DATA_LEN, "1234567890123456", 16);
    if (err) fail("aes-ctr, decryption failed for valid input");
  ...
  if (verbose) fprintf(stderr, "  Completed CTR cipher checks.\n");
}
...
static void check_cfb_cipher(void) {
  static const struct tv {
    ...
    struct data {
      ...
      {GCRY_CIPHER_AES,
       ...
       {
           {"\x87\xef\xeb\x8d\x55\x9e\xd3\x36\x77\x28", 10,
      ...
      {GCRY_CIPHER_AES192,
       ...
       {
           {"\xd3\xd8\xb9\xb9\x84\xad\xc2\x42\x37\xee", 10,
      ...
      {GCRY_CIPHER_AES256,
       ...
       {
           {"\xb0\xfe\x25\xac\x8d\x3d\x28\xa2\xf4\x71", 10,
      ...
      {GCRY_CIPHER_3DES,
       ...
       {
           {"\xdb\xe9\x15\xfc\xb3\x3b\xca\x18\xef\x14", 10,
      ...
      {GCRY_CIPHER_3DES,
       ...
       {
           {"\x2e\xa9\x56\xd4\xa2\x11\xdb\x68\x59\xb7", 10,
  ...
  if (verbose) fprintf(stderr, "  Starting CFB checks.\n");
  ...
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    ...
    if (!err) err = gcry_cipher_open(&hdd, tv[i].algo, mode, 0);
    if (err) {
    ...
    if (!err) err = gcry_cipher_setiv(hdd, tv[i].iv, blklen);
    if (err) {
  ...
  if (verbose) fprintf(stderr, "  Completed CFB checks.\n");
}
...
static void check_ofb_cipher(void) {
  ...
  if (verbose) fprintf(stderr, "  Starting OFB checks.\n");
  ...
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    ...
    if (!err) err = gcry_cipher_open(&hdd, tv[i].algo, GCRY_CIPHER_MODE_OFB, 0);
    if (err) {
    ...
    if (!err) err = gcry_cipher_reset(hdd);
    if (err) {
    ...
    for (j = 0; tv[i].data[j].inlen; j++) {
      ...
      for (byteNum = 0; byteNum < tv[i].data[j].inlen; ++byteNum) {
        err = gcry_cipher_encrypt(hde, out + byteNum, 1,
                                  (tv[i].data[j].plaintext) + byteNum, 1);
        if (err) {
          fail("aes-ofb, gcry_cipher_encrypt (%d, %d) failed: %s\n", i, j,
               gpg_strerror(err));
          gcry_cipher_close(hde);
  ...
  if (verbose) fprintf(stderr, "  Completed OFB checks.\n");
}

static void _check_gcm_cipher(unsigned int step) {
  struct tv {
    int algo;
    char key[MAX_DATA_LEN];
    char iv[MAX_DATA_LEN];
    ...
    int should_fail;
      ...
      {GCRY_CIPHER_AES,
       ...
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 12, "", 0, "", 0, "",
       "\x58\xe2\xfc\xce\xfa\x7e\x30\x61\x36\x7f\x1d\x57\xa4\xe7\x45\x5a"},
      {GCRY_CIPHER_AES,
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 12, "", 0, "", 0, "",
       "\x58\xe2\xfc\xce\xfa\x7e\x30\x61\x36\x7f\x1d\x57\xa4\xe7\x45", 15},
      {GCRY_CIPHER_AES,
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 12, "", 0, "", 0, "",
       "\x58\xe2\xfc\xce\xfa\x7e\x30\x61\x36\x7f\x1d\x57\xa4\xe7", 14},
      {GCRY_CIPHER_AES,
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 12, "", 0, "", 0, "",
       "\x58\xe2\xfc\xce\xfa\x7e\x30\x61\x36\x7f\x1d\x57\xa4", 13},
      {GCRY_CIPHER_AES,
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 12, "", 0, "", 0, "",
       "\x58\xe2\xfc\xce\xfa\x7e\x30\x61\x36\x7f\x1d\x57", 12},
      {GCRY_CIPHER_AES,
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 12, "", 0, "", 0, "",
       "\x58\xe2\xfc\xce\xfa\x7e\x30\x61\x36\x7f\x1d", 11, 1},
      {GCRY_CIPHER_AES,
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 12, "", 0, "", 0, "",
       "\x58\xe2\xfc\xce\xfa\x7e\x30\x61", 8},
      {GCRY_CIPHER_AES,
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 12, "", 0, "", 0, "",
       "\x58\xe2\xfc\xce", 4},
      ...
      {GCRY_CIPHER_AES,
       ...
       "\xca\xfe\xba\xbe\xfa\xce\xdb\xad\xde\xca\xf8\x88", 12, "", 0,
  ...
  if (verbose) fprintf(stderr, "  Starting GCM checks.\n");
  ...
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    ...
    if (!err) err = gcry_cipher_open(&hdd, tv[i].algo, GCRY_CIPHER_MODE_GCM, 0);
    if (err) {
    ...
    if (!err) err = gcry_cipher_setiv(hdd, tv[i].iv, tv[i].ivlen);
    if (err) {
    ...
    if (taglen2 != GCRY_GCM_BLOCK_LEN) {
      fail(
          "cipher-gcm, gcryctl_get_taglen returned bad length"
          " (tv %d): got=%zu want=%d\n",
    ...
    for (pos = 0; pos < tv[i].aadlen; pos += step) {
      ...
      if (err) {
        fail(
            "aes-gcm, gcry_cipher_authenticate (%d) (%lu:%d) failed: "
            "%s\n",
      ...
      if (err) {
        fail(
            "aes-gcm, de gcry_cipher_authenticate (%d) (%lu:%d) failed: "
            "%s\n",
            i, (unsigned long)pos, step, gpg_strerror(err));
        gcry_cipher_close(hde);
        gcry_cipher_close(hdd);
    ...
    for (pos = 0; pos < tv[i].inlen; pos += step) {
      ...
      err = gcry_cipher_encrypt(hde, out + pos, poslen, tv[i].plaintext + pos,
                                poslen);
      if (err) {
        fail("aes-gcm, gcry_cipher_encrypt (%d) (%lu:%d) failed: %s\n", i,
             (unsigned long)pos, step, gpg_strerror(err));
        gcry_cipher_close(hde);
        gcry_cipher_close(hdd);
        return;
      }
    ...
    if (err) {
      if (tv[i].should_fail) goto next_tv;
    ...
    for (byteNum = 0; byteNum < tv[i].aadlen; ++byteNum) {
      ...
      if (err) {
        fail(
            "aes-gcm, gcry_cipher_authenticate (%d) (byte-buf) failed: "
            "%s\n",
            i, gpg_strerror(err));
        gcry_cipher_close(hde);
        gcry_cipher_close(hdd);
      ...
      if (err) {
        fail(
            "aes-gcm, de gcry_cipher_authenticate (%d) (byte-buf) "
            "failed: %s\n",
            i, gpg_strerror(err));
        gcry_cipher_close(hde);
        gcry_cipher_close(hdd);
    ...
    if (err) {
      ...
      fail("aes-gcm, gcry_cipher_gettag(%d, %lu) (byte-buf) failed: %s\n", i,
           (unsigned long)taglen2, gpg_strerror(err));
      gcry_cipher_close(hde);
      gcry_cipher_close(hdd);
      return;
    }
    ...
    if (!err) {
      fail(
          "aes-gcm, gcry_cipher_checktag(%d) did not fail for invalid "
          " tag length of '%d'\n",
          i, 1);
      gcry_cipher_close(hde);
      gcry_cipher_close(hdd);
  ...
  if (verbose) fprintf(stderr, "  Completed GCM checks.\n");
}

static void check_gcm_cipher(void) {
...
static void _check_poly1305_cipher(unsigned int step) {
  struct tv {
    int algo;
    const char *key;
    const char *iv;
  ...
  if (verbose) fprintf(stderr, "  Starting POLY1305 checks.\n");
  ...
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    ...
    if (taglen2 != 16) {
      fail(
          "cipher-poly1305, gcryctl_get_taglen returned bad length"
          " (tv %d): got=%zu want=%d\n",
          i, taglen2, 16);
      gcry_cipher_close(hde);
      gcry_cipher_close(hdd);
    ...
    for (pos = 0; pos < tv[i].aadlen; pos += step) {
      ...
      if (err) {
        fail(
            "poly1305, gcry_cipher_authenticate (%d) (%lu:%d) failed: "
            "%s\n",
            i, (unsigned long)pos, step, gpg_strerror(err));
        gcry_cipher_close(hde);
        gcry_cipher_close(hdd);
      ...
      if (err) {
        fail(
            "poly1305, de gcry_cipher_authenticate (%d) (%lu:%d) failed: "
            "%s\n",
            i, (unsigned long)pos, step, gpg_strerror(err));
        gcry_cipher_close(hde);
        gcry_cipher_close(hdd);
    ...
    for (byteNum = 0; byteNum < tv[i].aadlen; ++byteNum) {
      ...
      if (err) {
        fail(
            "poly1305, gcry_cipher_authenticate (%d) (byte-buf) failed: "
            "%s\n",
            i, gpg_strerror(err));
        gcry_cipher_close(hde);
        gcry_cipher_close(hdd);
      ...
      if (err) {
        fail(
            "poly1305, de gcry_cipher_authenticate (%d) (byte-buf) "
            "failed: %s\n",
            i, gpg_strerror(err));
        gcry_cipher_close(hde);
        gcry_cipher_close(hdd);
  ...
  if (verbose) fprintf(stderr, "  Completed POLY1305 checks.\n");
}

static void check_poly1305_cipher(void) {
  /* Large buffers, no splitting. */
...
static void check_ccm_cipher(void) {
  static const struct tv {
    int algo;
    int keylen;
    const char *key;
    ...
    const char *ciphertext;
      ...
      {GCRY_CIPHER_AES, /* Packet Vector #1 */
       ...
       13, "\x00\x00\x00\x03\x02\x01\x00\xA0\xA1\xA2\xA3\xA4\xA5", 8,
       "\x00\x01\x02\x03\x04\x05\x06\x07", 23,
       "\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18"
       "\x19\x1A\x1B\x1C\x1D\x1E",
       31,
       "\x58\x8C\x97\x9A\x61\xC6\x63\xD2\xF0\x66\xD0\xC2\xC0\xF9\x89\x80\x6D"
       "\x5F\x6B\x61\xDA\xC3\x84\x17\xE8\xD1\x2C\xFD\xF9\x26\xE0"},
      {GCRY_CIPHER_AES, /* Packet Vector #2 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x04\x03\x02\x01\xA0\xA1\xA2\xA3\xA4\xA5", 8,
       "\x00\x01\x02\x03\x04\x05\x06\x07", 24,
       "\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18"
       "\x19\x1A\x1B\x1C\x1D\x1E\x1F",
       32,
       "\x72\xC9\x1A\x36\xE1\x35\xF8\xCF\x29\x1C\xA8\x94\x08\x5C\x87\xE3\xCC"
       "\x15\xC4\x39\xC9\xE4\x3A\x3B\xA0\x91\xD5\x6E\x10\x40\x09\x16"},
      {GCRY_CIPHER_AES, /* Packet Vector #3 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x05\x04\x03\x02\xA0\xA1\xA2\xA3\xA4\xA5", 8,
       "\x00\x01\x02\x03\x04\x05\x06\x07", 25,
       "\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18"
       "\x19\x1A\x1B\x1C\x1D\x1E\x1F\x20",
       33,
       "\x51\xB1\xE5\xF4\x4A\x19\x7D\x1D\xA4\x6B\x0F\x8E\x2D\x28\x2A\xE8\x71"
       "\xE8\x38\xBB\x64\xDA\x85\x96\x57\x4A\xDA\xA7\x6F\xBD\x9F\xB0\xC5"},
      {GCRY_CIPHER_AES, /* Packet Vector #4 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x06\x05\x04\x03\xA0\xA1\xA2\xA3\xA4\xA5", 12,
       "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B", 19,
       "\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C"
       "\x1D\x1E",
       27,
       "\xA2\x8C\x68\x65\x93\x9A\x9A\x79\xFA\xAA\x5C\x4C\x2A\x9D\x4A\x91\xCD"
       "\xAC\x8C\x96\xC8\x61\xB9\xC9\xE6\x1E\xF1"},
      {GCRY_CIPHER_AES, /* Packet Vector #5 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x07\x06\x05\x04\xA0\xA1\xA2\xA3\xA4\xA5", 12,
       "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B", 20,
       "\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C"
       "\x1D\x1E\x1F",
       28,
       "\xDC\xF1\xFB\x7B\x5D\x9E\x23\xFB\x9D\x4E\x13\x12\x53\x65\x8A\xD8\x6E"
       "\xBD\xCA\x3E\x51\xE8\x3F\x07\x7D\x9C\x2D\x93"},
      {GCRY_CIPHER_AES, /* Packet Vector #6 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x08\x07\x06\x05\xA0\xA1\xA2\xA3\xA4\xA5", 12,
       "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B", 21,
       "\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C"
       "\x1D\x1E\x1F\x20",
       29,
       "\x6F\xC1\xB0\x11\xF0\x06\x56\x8B\x51\x71\xA4\x2D\x95\x3D\x46\x9B\x25"
       "\x70\xA4\xBD\x87\x40\x5A\x04\x43\xAC\x91\xCB\x94"},
      {GCRY_CIPHER_AES, /* Packet Vector #7 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x09\x08\x07\x06\xA0\xA1\xA2\xA3\xA4\xA5", 8,
       "\x00\x01\x02\x03\x04\x05\x06\x07", 23,
       "\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18"
       "\x19\x1A\x1B\x1C\x1D\x1E",
       33,
       "\x01\x35\xD1\xB2\xC9\x5F\x41\xD5\xD1\xD4\xFE\xC1\x85\xD1\x66\xB8\x09"
       "\x4E\x99\x9D\xFE\xD9\x6C\x04\x8C\x56\x60\x2C\x97\xAC\xBB\x74\x90"},
      {GCRY_CIPHER_AES, /* Packet Vector #8 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x0A\x09\x08\x07\xA0\xA1\xA2\xA3\xA4\xA5", 8,
       "\x00\x01\x02\x03\x04\x05\x06\x07", 24,
       "\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18"
       "\x19\x1A\x1B\x1C\x1D\x1E\x1F",
       34,
       "\x7B\x75\x39\x9A\xC0\x83\x1D\xD2\xF0\xBB\xD7\x58\x79\xA2\xFD\x8F\x6C"
       "\xAE\x6B\x6C\xD9\xB7\xDB\x24\xC1\x7B\x44\x33\xF4\x34\x96\x3F\x34\xB4"},
      {GCRY_CIPHER_AES, /* Packet Vector #9 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x0B\x0A\x09\x08\xA0\xA1\xA2\xA3\xA4\xA5", 8,
       "\x00\x01\x02\x03\x04\x05\x06\x07", 25,
       "\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18"
       "\x19\x1A\x1B\x1C\x1D\x1E\x1F\x20",
       35,
       "\x82\x53\x1A\x60\xCC\x24\x94\x5A\x4B\x82\x79\x18\x1A\xB5\xC8\x4D\xF2"
       "\x1C\xE7\xF9\xB7\x3F\x42\xE1\x97\xEA\x9C\x07\xE5\x6B\x5E\xB1\x7E\x5F"
       "\x4E"},
      {GCRY_CIPHER_AES, /* Packet Vector #10 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x0C\x0B\x0A\x09\xA0\xA1\xA2\xA3\xA4\xA5", 12,
       "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B", 19,
       "\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C"
       "\x1D\x1E",
       29,
       "\x07\x34\x25\x94\x15\x77\x85\x15\x2B\x07\x40\x98\x33\x0A\xBB\x14\x1B"
       "\x94\x7B\x56\x6A\xA9\x40\x6B\x4D\x99\x99\x88\xDD"},
      {GCRY_CIPHER_AES, /* Packet Vector #11 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x0D\x0C\x0B\x0A\xA0\xA1\xA2\xA3\xA4\xA5", 12,
       "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B", 20,
       "\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C"
       "\x1D\x1E\x1F",
       30,
       "\x67\x6B\xB2\x03\x80\xB0\xE3\x01\xE8\xAB\x79\x59\x0A\x39\x6D\xA7\x8B"
       "\x83\x49\x34\xF5\x3A\xA2\xE9\x10\x7A\x8B\x6C\x02\x2C"},
      {GCRY_CIPHER_AES, /* Packet Vector #12 */
       16, "\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF",
       13, "\x00\x00\x00\x0E\x0D\x0C\x0B\xA0\xA1\xA2\xA3\xA4\xA5", 12,
       "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B", 21,
       "\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C"
       "\x1D\x1E\x1F\x20",
       31,
       "\xC0\xFF\xA0\xD6\xF0\x5B\xDB\x67\xF2\x4D\x43\xA4\x33\x8D\x2A\xA4\xBE"
       "\xD7\xB2\x0E\x43\xCD\x1A\xA3\x16\x62\xE7\xAD\x65\xD6\xDB"},
      {GCRY_CIPHER_AES, /* Packet Vector #13 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\x41\x2B\x4E\xA9\xCD\xBE\x3C\x96\x96\x76\x6C\xFA", 8,
       "\x0B\xE1\xA8\x8B\xAC\xE0\x18\xB1", 23,
       "\x08\xE8\xCF\x97\xD8\x20\xEA\x25\x84\x60\xE9\x6A\xD9\xCF\x52\x89\x05"
       "\x4D\x89\x5C\xEA\xC4\x7C",
       31,
       "\x4C\xB9\x7F\x86\xA2\xA4\x68\x9A\x87\x79\x47\xAB\x80\x91\xEF\x53\x86"
       "\xA6\xFF\xBD\xD0\x80\xF8\xE7\x8C\xF7\xCB\x0C\xDD\xD7\xB3"},
      {GCRY_CIPHER_AES, /* Packet Vector #14 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\x33\x56\x8E\xF7\xB2\x63\x3C\x96\x96\x76\x6C\xFA", 8,
       "\x63\x01\x8F\x76\xDC\x8A\x1B\xCB", 24,
       "\x90\x20\xEA\x6F\x91\xBD\xD8\x5A\xFA\x00\x39\xBA\x4B\xAF\xF9\xBF\xB7"
       "\x9C\x70\x28\x94\x9C\xD0\xEC",
       32,
       "\x4C\xCB\x1E\x7C\xA9\x81\xBE\xFA\xA0\x72\x6C\x55\xD3\x78\x06\x12\x98"
       "\xC8\x5C\x92\x81\x4A\xBC\x33\xC5\x2E\xE8\x1D\x7D\x77\xC0\x8A"},
      {GCRY_CIPHER_AES, /* Packet Vector #15 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\x10\x3F\xE4\x13\x36\x71\x3C\x96\x96\x76\x6C\xFA", 8,
       "\xAA\x6C\xFA\x36\xCA\xE8\x6B\x40", 25,
       "\xB9\x16\xE0\xEA\xCC\x1C\x00\xD7\xDC\xEC\x68\xEC\x0B\x3B\xBB\x1A\x02"
       "\xDE\x8A\x2D\x1A\xA3\x46\x13\x2E",
       33,
       "\xB1\xD2\x3A\x22\x20\xDD\xC0\xAC\x90\x0D\x9A\xA0\x3C\x61\xFC\xF4\xA5"
       "\x59\xA4\x41\x77\x67\x08\x97\x08\xA7\x76\x79\x6E\xDB\x72\x35\x06"},
      {GCRY_CIPHER_AES, /* Packet Vector #16 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\x76\x4C\x63\xB8\x05\x8E\x3C\x96\x96\x76\x6C\xFA", 12,
       "\xD0\xD0\x73\x5C\x53\x1E\x1B\xEC\xF0\x49\xC2\x44", 19,
       "\x12\xDA\xAC\x56\x30\xEF\xA5\x39\x6F\x77\x0C\xE1\xA6\x6B\x21\xF7\xB2"
       "\x10\x1C",
       27,
       "\x14\xD2\x53\xC3\x96\x7B\x70\x60\x9B\x7C\xBB\x7C\x49\x91\x60\x28\x32"
       "\x45\x26\x9A\x6F\x49\x97\x5B\xCA\xDE\xAF"},
      {GCRY_CIPHER_AES, /* Packet Vector #17 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\xF8\xB6\x78\x09\x4E\x3B\x3C\x96\x96\x76\x6C\xFA", 12,
       "\x77\xB6\x0F\x01\x1C\x03\xE1\x52\x58\x99\xBC\xAE", 20,
       "\xE8\x8B\x6A\x46\xC7\x8D\x63\xE5\x2E\xB8\xC5\x46\xEF\xB5\xDE\x6F\x75"
       "\xE9\xCC\x0D",
       28,
       "\x55\x45\xFF\x1A\x08\x5E\xE2\xEF\xBF\x52\xB2\xE0\x4B\xEE\x1E\x23\x36"
       "\xC7\x3E\x3F\x76\x2C\x0C\x77\x44\xFE\x7E\x3C"},
      {GCRY_CIPHER_AES, /* Packet Vector #18 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\xD5\x60\x91\x2D\x3F\x70\x3C\x96\x96\x76\x6C\xFA", 12,
       "\xCD\x90\x44\xD2\xB7\x1F\xDB\x81\x20\xEA\x60\xC0", 21,
       "\x64\x35\xAC\xBA\xFB\x11\xA8\x2E\x2F\x07\x1D\x7C\xA4\xA5\xEB\xD9\x3A"
       "\x80\x3B\xA8\x7F",
       29,
       "\x00\x97\x69\xEC\xAB\xDF\x48\x62\x55\x94\xC5\x92\x51\xE6\x03\x57\x22"
       "\x67\x5E\x04\xC8\x47\x09\x9E\x5A\xE0\x70\x45\x51"},
      {GCRY_CIPHER_AES, /* Packet Vector #19 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\x42\xFF\xF8\xF1\x95\x1C\x3C\x96\x96\x76\x6C\xFA", 8,
       "\xD8\x5B\xC7\xE6\x9F\x94\x4F\xB8", 23,
       "\x8A\x19\xB9\x50\xBC\xF7\x1A\x01\x8E\x5E\x67\x01\xC9\x17\x87\x65\x98"
       "\x09\xD6\x7D\xBE\xDD\x18",
       33,
       "\xBC\x21\x8D\xAA\x94\x74\x27\xB6\xDB\x38\x6A\x99\xAC\x1A\xEF\x23\xAD"
       "\xE0\xB5\x29\x39\xCB\x6A\x63\x7C\xF9\xBE\xC2\x40\x88\x97\xC6\xBA"},
      {GCRY_CIPHER_AES, /* Packet Vector #20 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\x92\x0F\x40\xE5\x6C\xDC\x3C\x96\x96\x76\x6C\xFA", 8,
       "\x74\xA0\xEB\xC9\x06\x9F\x5B\x37", 24,
       "\x17\x61\x43\x3C\x37\xC5\xA3\x5F\xC1\xF3\x9F\x40\x63\x02\xEB\x90\x7C"
       "\x61\x63\xBE\x38\xC9\x84\x37",
       34,
       "\x58\x10\xE6\xFD\x25\x87\x40\x22\xE8\x03\x61\xA4\x78\xE3\xE9\xCF\x48"
       "\x4A\xB0\x4F\x44\x7E\xFF\xF6\xF0\xA4\x77\xCC\x2F\xC9\xBF\x54\x89\x44"},
      {GCRY_CIPHER_AES, /* Packet Vector #21 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\x27\xCA\x0C\x71\x20\xBC\x3C\x96\x96\x76\x6C\xFA", 8,
       "\x44\xA3\xAA\x3A\xAE\x64\x75\xCA", 25,
       "\xA4\x34\xA8\xE5\x85\x00\xC6\xE4\x15\x30\x53\x88\x62\xD6\x86\xEA\x9E"
       "\x81\x30\x1B\x5A\xE4\x22\x6B\xFA",
       35,
       "\xF2\xBE\xED\x7B\xC5\x09\x8E\x83\xFE\xB5\xB3\x16\x08\xF8\xE2\x9C\x38"
       "\x81\x9A\x89\xC8\xE7\x76\xF1\x54\x4D\x41\x51\xA4\xED\x3A\x8B\x87\xB9"
       "\xCE"},
      {GCRY_CIPHER_AES, /* Packet Vector #22 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\x5B\x8C\xCB\xCD\x9A\xF8\x3C\x96\x96\x76\x6C\xFA", 12,
       "\xEC\x46\xBB\x63\xB0\x25\x20\xC3\x3C\x49\xFD\x70", 19,
       "\xB9\x6B\x49\xE2\x1D\x62\x17\x41\x63\x28\x75\xDB\x7F\x6C\x92\x43\xD2"
       "\xD7\xC2",
       29,
       "\x31\xD7\x50\xA0\x9D\xA3\xED\x7F\xDD\xD4\x9A\x20\x32\xAA\xBF\x17\xEC"
       "\x8E\xBF\x7D\x22\xC8\x08\x8C\x66\x6B\xE5\xC1\x97"},
      {GCRY_CIPHER_AES, /* Packet Vector #23 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\x3E\xBE\x94\x04\x4B\x9A\x3C\x96\x96\x76\x6C\xFA", 12,
       "\x47\xA6\x5A\xC7\x8B\x3D\x59\x42\x27\xE8\x5E\x71", 20,
       "\xE2\xFC\xFB\xB8\x80\x44\x2C\x73\x1B\xF9\x51\x67\xC8\xFF\xD7\x89\x5E"
       "\x33\x70\x76",
       30,
       "\xE8\x82\xF1\xDB\xD3\x8C\xE3\xED\xA7\xC2\x3F\x04\xDD\x65\x07\x1E\xB4"
       "\x13\x42\xAC\xDF\x7E\x00\xDC\xCE\xC7\xAE\x52\x98\x7D"},
      {GCRY_CIPHER_AES, /* Packet Vector #24 */
       16, "\xD7\x82\x8D\x13\xB2\xB0\xBD\xC3\x25\xA7\x62\x36\xDF\x93\xCC\x6B",
       13, "\x00\x8D\x49\x3B\x30\xAE\x8B\x3C\x96\x96\x76\x6C\xFA", 12,
       "\x6E\x37\xA6\xEF\x54\x6D\x95\x5D\x34\xAB\x60\x59", 21,
       "\xAB\xF2\x1C\x0B\x02\xFE\xB8\x8F\x85\x6D\xF4\xA3\x73\x81\xBC\xE3\xCC"
       "\x12\x85\x17\xD4",
       31,
       "\xF3\x29\x05\xB8\x8A\x64\x1B\x04\xB9\xC9\xFF\xB5\x8C\xC3\x90\x90\x0F"
       "\x3D\xA1\x2A\xB1\x6D\xCE\x9E\x82\xEF\xA1\x6D\xA6\x20\x59"},
      /* RFC 5528 */
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #1 */
       ...
       "\x19\x1A\x1B\x1C\x1D\x1E",
       31,
       "\xBA\x73\x71\x85\xE7\x19\x31\x04\x92\xF3\x8A\x5F\x12\x51\xDA\x55\xFA"
       "\xFB\xC9\x49\x84\x8A\x0D\xFC\xAE\xCE\x74\x6B\x3D\xB9\xAD"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #2 */
       ...
       "\x19\x1A\x1B\x1C\x1D\x1E\x1F",
       32,
       "\x5D\x25\x64\xBF\x8E\xAF\xE1\xD9\x95\x26\xEC\x01\x6D\x1B\xF0\x42\x4C"
       "\xFB\xD2\xCD\x62\x84\x8F\x33\x60\xB2\x29\x5D\xF2\x42\x83\xE8"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #3 */
       ...
       "\x19\x1A\x1B\x1C\x1D\x1E\x1F\x20",
       33,
       "\x81\xF6\x63\xD6\xC7\x78\x78\x17\xF9\x20\x36\x08\xB9\x82\xAD\x15\xDC"
       "\x2B\xBD\x87\xD7\x56\xF7\x92\x04\xF5\x51\xD6\x68\x2F\x23\xAA\x46"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #4 */
       ...
       "\x1D\x1E",
       27,
       "\xCA\xEF\x1E\x82\x72\x11\xB0\x8F\x7B\xD9\x0F\x08\xC7\x72\x88\xC0\x70"
       "\xA4\xA0\x8B\x3A\x93\x3A\x63\xE4\x97\xA0"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #5 */
       ...
       "\x1D\x1E\x1F",
       28,
       "\x2A\xD3\xBA\xD9\x4F\xC5\x2E\x92\xBE\x43\x8E\x82\x7C\x10\x23\xB9\x6A"
       "\x8A\x77\x25\x8F\xA1\x7B\xA7\xF3\x31\xDB\x09"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #6 */
       ...
       "\x1D\x1E\x1F\x20",
       29,
       "\xFE\xA5\x48\x0B\xA5\x3F\xA8\xD3\xC3\x44\x22\xAA\xCE\x4D\xE6\x7F\xFA"
       "\x3B\xB7\x3B\xAB\xAB\x36\xA1\xEE\x4F\xE0\xFE\x28"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #7 */
       ...
       "\x19\x1A\x1B\x1C\x1D\x1E",
       33,
       "\x54\x53\x20\x26\xE5\x4C\x11\x9A\x8D\x36\xD9\xEC\x6E\x1E\xD9\x74\x16"
       "\xC8\x70\x8C\x4B\x5C\x2C\xAC\xAF\xA3\xBC\xCF\x7A\x4E\xBF\x95\x73"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #8 */
       ...
       "\x19\x1A\x1B\x1C\x1D\x1E\x1F",
       34,
       "\x8A\xD1\x9B\x00\x1A\x87\xD1\x48\xF4\xD9\x2B\xEF\x34\x52\x5C\xCC\xE3"
       "\xA6\x3C\x65\x12\xA6\xF5\x75\x73\x88\xE4\x91\x3E\xF1\x47\x01\xF4\x41"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #9 */
       ...
       "\x19\x1A\x1B\x1C\x1D\x1E\x1F\x20",
       35,
       "\x5D\xB0\x8D\x62\x40\x7E\x6E\x31\xD6\x0F\x9C\xA2\xC6\x04\x74\x21\x9A"
       "\xC0\xBE\x50\xC0\xD4\xA5\x77\x87\x94\xD6\xE2\x30\xCD\x25\xC9\xFE\xBF"
       "\x87"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #10 */
       ...
       "\x1D\x1E",
       29,
       "\xDB\x11\x8C\xCE\xC1\xB8\x76\x1C\x87\x7C\xD8\x96\x3A\x67\xD6\xF3\xBB"
       "\xBC\x5C\xD0\x92\x99\xEB\x11\xF3\x12\xF2\x32\x37"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #11 */
       ...
       "\x1D\x1E\x1F",
       30,
       "\x7C\xC8\x3D\x8D\xC4\x91\x03\x52\x5B\x48\x3D\xC5\xCA\x7E\xA9\xAB\x81"
       "\x2B\x70\x56\x07\x9D\xAF\xFA\xDA\x16\xCC\xCF\x2C\x4E"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #12 */
       ...
       "\x1D\x1E\x1F\x20",
       31,
       "\x2C\xD3\x5B\x88\x20\xD2\x3E\x7A\xA3\x51\xB0\xE9\x2F\xC7\x93\x67\x23"
       "\x8B\x2C\xC7\x48\xCB\xB9\x4C\x29\x47\x79\x3D\x64\xAF\x75"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #13 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\xA9\x70\x11\x0E\x19\x27\xB1\x60\xB6\xA3\x1C\x1C", 8,
       "\x6B\x7F\x46\x45\x07\xFA\xE4\x96", 23,
       "\xC6\xB5\xF3\xE6\xCA\x23\x11\xAE\xF7\x47\x2B\x20\x3E\x73\x5E\xA5\x61"
       "\xAD\xB1\x7D\x56\xC5\xA3",
       31,
       "\xA4\x35\xD7\x27\x34\x8D\xDD\x22\x90\x7F\x7E\xB8\xF5\xFD\xBB\x4D\x93"
       "\x9D\xA6\x52\x4D\xB4\xF6\x45\x58\xC0\x2D\x25\xB1\x27\xEE"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #14 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\x83\xCD\x8C\xE0\xCB\x42\xB1\x60\xB6\xA3\x1C\x1C", 8,
       "\x98\x66\x05\xB4\x3D\xF1\x5D\xE7", 24,
       "\x01\xF6\xCE\x67\x64\xC5\x74\x48\x3B\xB0\x2E\x6B\xBF\x1E\x0A\xBD\x26"
       "\xA2\x25\x72\xB4\xD8\x0E\xE7",
       32,
       "\x8A\xE0\x52\x50\x8F\xBE\xCA\x93\x2E\x34\x6F\x05\xE0\xDC\x0D\xFB\xCF"
       "\x93\x9E\xAF\xFA\x3E\x58\x7C\x86\x7D\x6E\x1C\x48\x70\x38\x06"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #15 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\x5F\x54\x95\x0B\x18\xF2\xB1\x60\xB6\xA3\x1C\x1C", 8,
       "\x48\xF2\xE7\xE1\xA7\x67\x1A\x51", 25,
       "\xCD\xF1\xD8\x40\x6F\xC2\xE9\x01\x49\x53\x89\x70\x05\xFB\xFB\x8B\xA5"
       "\x72\x76\xF9\x24\x04\x60\x8E\x08",
       33,
       "\x08\xB6\x7E\xE2\x1C\x8B\xF2\x6E\x47\x3E\x40\x85\x99\xE9\xC0\x83\x6D"
       "\x6A\xF0\xBB\x18\xDF\x55\x46\x6C\xA8\x08\x78\xA7\x90\x47\x6D\xE5"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #16 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\xEC\x60\x08\x63\x31\x9A\xB1\x60\xB6\xA3\x1C\x1C", 12,
       "\xDE\x97\xDF\x3B\x8C\xBD\x6D\x8E\x50\x30\xDA\x4C", 19,
       "\xB0\x05\xDC\xFA\x0B\x59\x18\x14\x26\xA9\x61\x68\x5A\x99\x3D\x8C\x43"
       "\x18\x5B",
       27,
       "\x63\xB7\x8B\x49\x67\xB1\x9E\xDB\xB7\x33\xCD\x11\x14\xF6\x4E\xB2\x26"
       "\x08\x93\x68\xC3\x54\x82\x8D\x95\x0C\xC5"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #17 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\x60\xCF\xF1\xA3\x1E\xA1\xB1\x60\xB6\xA3\x1C\x1C", 12,
       "\xA5\xEE\x93\xE4\x57\xDF\x05\x46\x6E\x78\x2D\xCF", 20,
       "\x2E\x20\x21\x12\x98\x10\x5F\x12\x9D\x5E\xD9\x5B\x93\xF7\x2D\x30\xB2"
       "\xFA\xCC\xD7",
       28,
       "\x0B\xC6\xBB\xE2\xA8\xB9\x09\xF4\x62\x9E\xE6\xDC\x14\x8D\xA4\x44\x10"
       "\xE1\x8A\xF4\x31\x47\x38\x32\x76\xF6\x6A\x9F"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #18 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\x0F\x85\xCD\x99\x5C\x97\xB1\x60\xB6\xA3\x1C\x1C", 12,
       "\x24\xAA\x1B\xF9\xA5\xCD\x87\x61\x82\xA2\x50\x74", 21,
       "\x26\x45\x94\x1E\x75\x63\x2D\x34\x91\xAF\x0F\xC0\xC9\x87\x6C\x3B\xE4"
       "\xAA\x74\x68\xC9",
       29,
       "\x22\x2A\xD6\x32\xFA\x31\xD6\xAF\x97\x0C\x34\x5F\x7E\x77\xCA\x3B\xD0"
       "\xDC\x25\xB3\x40\xA1\xA3\xD3\x1F\x8D\x4B\x44\xB7"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #19 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\xC2\x9B\x2C\xAA\xC4\xCD\xB1\x60\xB6\xA3\x1C\x1C", 8,
       "\x69\x19\x46\xB9\xCA\x07\xBE\x87", 23,
       "\x07\x01\x35\xA6\x43\x7C\x9D\xB1\x20\xCD\x61\xD8\xF6\xC3\x9C\x3E\xA1"
       "\x25\xFD\x95\xA0\xD2\x3D",
       33,
       "\x05\xB8\xE1\xB9\xC4\x9C\xFD\x56\xCF\x13\x0A\xA6\x25\x1D\xC2\xEC\xC0"
       "\x6C\xCC\x50\x8F\xE6\x97\xA0\x06\x6D\x57\xC8\x4B\xEC\x18\x27\x68"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #20 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\x2C\x6B\x75\x95\xEE\x62\xB1\x60\xB6\xA3\x1C\x1C", 8,
       "\xD0\xC5\x4E\xCB\x84\x62\x7D\xC4", 24,
       "\xC8\xC0\x88\x0E\x6C\x63\x6E\x20\x09\x3D\xD6\x59\x42\x17\xD2\xE1\x88"
       "\x77\xDB\x26\x4E\x71\xA5\xCC",
       34,
       "\x54\xCE\xB9\x68\xDE\xE2\x36\x11\x57\x5E\xC0\x03\xDF\xAA\x1C\xD4\x88"
       "\x49\xBD\xF5\xAE\x2E\xDB\x6B\x7F\xA7\x75\xB1\x50\xED\x43\x83\xC5\xA9"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #21 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\xC5\x3C\xD4\xC2\xAA\x24\xB1\x60\xB6\xA3\x1C\x1C", 8,
       "\xE2\x85\xE0\xE4\x80\x8C\xDA\x3D", 25,
       "\xF7\x5D\xAA\x07\x10\xC4\xE6\x42\x97\x79\x4D\xC2\xB7\xD2\xA2\x07\x57"
       "\xB1\xAA\x4E\x44\x80\x02\xFF\xAB",
       35,
       "\xB1\x40\x45\x46\xBF\x66\x72\x10\xCA\x28\xE3\x09\xB3\x9B\xD6\xCA\x7E"
       "\x9F\xC8\x28\x5F\xE6\x98\xD4\x3C\xD2\x0A\x02\xE0\xBD\xCA\xED\x20\x10"
       "\xD3"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #22 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\xBE\xE9\x26\x7F\xBA\xDC\xB1\x60\xB6\xA3\x1C\x1C", 12,
       "\x6C\xAE\xF9\x94\x11\x41\x57\x0D\x7C\x81\x34\x05", 19,
       "\xC2\x38\x82\x2F\xAC\x5F\x98\xFF\x92\x94\x05\xB0\xAD\x12\x7A\x4E\x41"
       "\x85\x4E",
       29,
       "\x94\xC8\x95\x9C\x11\x56\x9A\x29\x78\x31\xA7\x21\x00\x58\x57\xAB\x61"
       "\xB8\x7A\x2D\xEA\x09\x36\xB6\xEB\x5F\x62\x5F\x5D"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #23 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\xDF\xA8\xB1\x24\x50\x07\xB1\x60\xB6\xA3\x1C\x1C", 12,
       "\x36\xA5\x2C\xF1\x6B\x19\xA2\x03\x7A\xB7\x01\x1E", 20,
       "\x4D\xBF\x3E\x77\x4A\xD2\x45\xE5\xD5\x89\x1F\x9D\x1C\x32\xA0\xAE\x02"
       "\x2C\x85\xD7",
       30,
       "\x58\x69\xE3\xAA\xD2\x44\x7C\x74\xE0\xFC\x05\xF9\xA4\xEA\x74\x57\x7F"
       "\x4D\xE8\xCA\x89\x24\x76\x42\x96\xAD\x04\x11\x9C\xE7"},
      {GCRY_CIPHER_CAMELLIA128, /* Packet Vector #24 */
       16, "\xD7\x5C\x27\x78\x07\x8C\xA9\x3D\x97\x1F\x96\xFD\xE7\x20\xF4\xCD",
       13, "\x00\x3B\x8F\xD8\xD3\xA9\x37\xB1\x60\xB6\xA3\x1C\x1C", 12,
       "\xA4\xD4\x99\xF7\x84\x19\x72\x8C\x19\x17\x8B\x0C", 21,
       "\x9D\xC9\xED\xAE\x2F\xF5\xDF\x86\x36\xE8\xC6\xDE\x0E\xED\x55\xF7\x86"
       "\x7E\x33\x33\x7D",
       31,
       "\x4B\x19\x81\x56\x39\x3B\x0F\x77\x96\x08\x6A\xAF\xB4\x54\xF8\xC3\xF0"
       "\x34\xCC\xA9\x66\x94\x5F\x1F\xCE\xA7\xE1\x1B\xEE\x6A\x2F"}};
  ...
  if (verbose) fprintf(stderr, "  Starting CCM checks.\n");
  ...
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    ...
    for (j = 0; j < sizeof(cut) / sizeof(cut[0]); j++) {
      ...
      if (tv[i].plainlen < split) continue;
      ...
      if (!err) err = gcry_cipher_setiv(hdd, tv[i].nonce, tv[i].noncelen);
      if (err) {
      ...
      if (err) {
        fail(
            "cipher-ccm, gcry_cipher_ctl GCRYCTL_SET_CCM_LENGTHS "
            "failed: %s\n",
            gpg_strerror(err));
        gcry_cipher_close(hde);
        gcry_cipher_close(hdd);
      ...
      if (taglen2 != authlen) {
        fail(
            "cipher-ccm, gcryctl_get_taglen returned bad length"
      ...
      err = gcry_cipher_authenticate(hde, tv[i].aad, tv[i].aadlen - aadsplit);
      if (!err)
        err = gcry_cipher_authenticate(hde, &tv[i].aad[tv[i].aadlen - aadsplit],
                                       aadsplit);
      if (!err)
        err = gcry_cipher_authenticate(hdd, tv[i].aad, tv[i].aadlen - aadsplit);
      if (!err)
        err = gcry_cipher_authenticate(hdd, &tv[i].aad[tv[i].aadlen - aadsplit],
                                       aadsplit);
      if (err) {
  ...
  /* Test encoding of aadlen > 0xfeff.  */
  {
    ...
    static const char iv[] = {0x10, 0x11, 0x12, 0x13, 0x14,
                              0x15, 0x16, 0x17, 0x18, 0x19};
    static const char tag[] = {0x9C, 0x76, 0xE7, 0x33, 0xD5, 0x15, 0xB3,
                               0x6C, 0xBA, 0x76, 0x95, 0xF7, 0xFB, 0x91};
    char buf[1024];
    ...
    if (err) {
      fail(
          "cipher-ccm-large, gcry_cipher_ctl GCRYCTL_SET_CCM_LENGTHS "
          "failed: %s\n",
          gpg_strerror(err));
      gcry_cipher_close(hde);
      return;
...
static void do_check_ocb_cipher(int inplace) {
  ...
  static const struct {
    ...
    const char *ciph;
      ...
      {GCRY_CIPHER_AES, 16, NULL, "BBAA99887766554433221100", "", "",
       "785407BFFFC8AD9EDCC5520AC9111EE6"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA99887766554433221101",
       "0001020304050607", "0001020304050607",
       "6820B3657B6F615A5725BDA0D3B4EB3A257C9AF1F8F03009"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA99887766554433221102",
       "0001020304050607", "", "81017F8203F081277152FADE694A0A00"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA99887766554433221103", "",
       "0001020304050607", "45DD69F8F5AAE72414054CD1F35D82760B2CD00D2F99BFA9"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA99887766554433221104",
       "000102030405060708090A0B0C0D0E0F", "000102030405060708090A0B0C0D0E0F",
       "571D535B60B277188BE5147170A9A22C3AD7A4FF3835B8C5"
       "701C1CCEC8FC3358"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA99887766554433221105",
       "000102030405060708090A0B0C0D0E0F", "",
       "8CF761B6902EF764462AD86498CA6B97"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA99887766554433221106", "",
       "000102030405060708090A0B0C0D0E0F",
       "5CE88EC2E0692706A915C00AEB8B2396F40E1C743F52436B"
       "DF06D8FA1ECA343D"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA99887766554433221107",
       "000102030405060708090A0B0C0D0E0F1011121314151617",
       "000102030405060708090A0B0C0D0E0F1011121314151617",
       "1CA2207308C87C010756104D8840CE1952F09673A448A122"
       "C92C62241051F57356D7F3C90BB0E07F"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA99887766554433221108",
       "000102030405060708090A0B0C0D0E0F1011121314151617", "",
       "6DC225A071FC1B9F7C69F93B0F1E10DE"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA99887766554433221109", "",
       "000102030405060708090A0B0C0D0E0F1011121314151617",
      ...
      {GCRY_CIPHER_AES, 16, NULL, "BBAA9988776655443322110B",
       "000102030405060708090A0B0C0D0E0F1011121314151617"
       "18191A1B1C1D1E1F",
       "", "FE80690BEE8A485D11F32965BC9D2A32"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA9988776655443322110C", "",
       "000102030405060708090A0B0C0D0E0F1011121314151617"
       "18191A1B1C1D1E1F",
       "2942BFC773BDA23CABC6ACFD9BFD5835BD300F0973792EF4"
       "6040C53F1432BCDFB5E1DDE3BC18A5F840B52E653444D5DF"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA9988776655443322110D",
       "000102030405060708090A0B0C0D0E0F1011121314151617"
       ...
       "A7035490C5769E60"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA9988776655443322110E",
       "000102030405060708090A0B0C0D0E0F1011121314151617"
       "18191A1B1C1D1E1F2021222324252627",
       "", "C5CD9D1850C141E358649994EE701B68"},
      {GCRY_CIPHER_AES, 16, NULL, "BBAA9988776655443322110F", "",
       "000102030405060708090A0B0C0D0E0F1011121314151617"
       "18191A1B1C1D1E1F2021222324252627",
  ...
  if (verbose) fprintf(stderr, "  Starting OCB checks.\n");
  ...
  for (tidx = 0; tidx < DIM(tv); tidx++) {
    ...
    if (taglen != 16) {
      err = gcry_cipher_ctl(hde, GCRYCTL_SET_TAGLEN, &taglen, sizeof taglen);
      if (err) {
    ...
    err = gcry_cipher_ctl(hdd, GCRYCTL_SET_TAGLEN, &taglen, sizeof taglen);
    if (err) {
      fail("cipher-ocb, gcryctl_set_taglen failed (tv %d): %s\n", tidx,
           gpg_strerror(err));
      gcry_cipher_close(hde);
    ...
    if (taglen2 != tv[tidx].taglen) {
      fail(
          "cipher-ocb, gcryctl_get_taglen returned bad length (tv %d): "
          "got=%zu want=%d\n",
    ...
    if (!err) err = gcry_cipher_setkey(hdd, key, keylen);
    if (err) {
    ...
    if (!err) err = gcry_cipher_setiv(hdd, nonce, noncelen);
    if (err) {
    ...
    if (memcmp(ciph + ciphlen - tv[tidx].taglen, tag, tv[tidx].taglen)) {
      mismatch(ciph + ciphlen - tv[tidx].taglen, tv[tidx].taglen, tag,
               tv[tidx].taglen);
  ...
  if (verbose) fprintf(stderr, "  Completed OCB checks.\n");
}

static void check_ocb_cipher_largebuf_split(int algo, int keylen,
                                            const char *tagexpect,
  ...
  for (i = 0; i < buflen; i++) inbuf[i] = 'a';

  err = gcry_cipher_open(&hde, algo, GCRY_CIPHER_MODE_OCB, 0);
  if (!err) err = gcry_cipher_open(&hdd, algo, GCRY_CIPHER_MODE_OCB, 0);
  if (err) {
  ...
  if (!err) err = gcry_cipher_setiv(hdd, nonce, 12);
  if (err) {
  ...
  if (!err) {
    err = gcry_cipher_final(hdd);
    if (!err) {
      err = gcry_cipher_decrypt(hdd, outbuf + splitpos, buflen - splitpos, NULL,
                                0);
    }
  }
  if (err) {
...
static void check_ocb_cipher_largebuf(int algo, int keylen,
                                      const char *tagexpect) {
  unsigned int split;
...
static void check_ocb_cipher_splitaad(void) {
  ...
  struct {
    ...
    const char *aad3;
      ...
      {"000102030405060708090A0B0C0D0E0F", "101112131415161718191A1B1C1D1E1F",
       "2021222324252627"},
      {"000102030405060708090A0B0C0D0E0F", "1011121314151617",
       "18191A1B1C1D1E1F", "2021222324252627"},
      {"000102030405060708090A0B0C0D0E0F", "101112131415161718191A1B1C1D1E1F",
       "20", "21222324252627"},
      {"000102030405060708090A0B0C0D0E0F", "101112131415161718191A1B1C1D1E1F",
       "2021", "222324252627"},
      ...
      {"0001", "02030405060708090A0B0C0D0E0F", "1011121314151617",
       "18191A1B1C1D1E1F2021222324252627"},
      {"000102030405060708090A0B0C0D", "0E0F", "1011121314151617",
       "18191A1B1C1D1E1F2021222324252627"},
      {"000102030405060708090A0B0C0D0E", "0F", "1011121314151617",
       "18191A1B1C1D1E1F2021222324252627"},
      {"000102030405060708090A0B0C0D0E",
       "0F101112131415161718191A1B1C1D1E1F20212223242526", "27"}};
  ...
  for (tidx = 0; tidx < DIM(tv); tidx++) {
    ...
    if (verbose) fprintf(stderr, "    checking OCB aad split (tv %d)\n", tidx);
...
static void check_ocb_cipher(void) {
...
static void do_check_xts_cipher(int inplace) {
  /* Note that we use hex strings and not binary strings in TV.  That
     makes it easier to maintain the test vectors.  */
  static const struct {
    int algo;
    ...
    const char *ciph;
      ...
      {GCRY_CIPHER_AES,
       ...
       "64981173159d58ac355a20120c8e81f1", "189acacee06dfa7c94484c7dae59e166",
      ...
      {GCRY_CIPHER_AES,
       ...
       "9a785634120000000000000000000000", "000102030405060708090a0b0c0d0e0f10",
  ...
  if (verbose) fprintf(stderr, "  Starting XTS checks.\n");
  ...
  for (tidx = 0; !got_err && tidx < DIM(tv); tidx++) {
    ...
    if (!err) err = gcry_cipher_setiv(hdd, iv, ivlen);
    if (err) {
  ...
  if (verbose) fprintf(stderr, "  Completed XTS checks.\n");
}

static void check_xts_cipher(void) {
...
static void check_gost28147_cipher(void) {
  ...
  static const struct {
    ...
    char out[MAX_DATA_LEN];
  } tv[] = {
      {
          ...
          "1.2.643.7.1.2.5.1.1", "\x01\x02\x03\x04\x05\x06\x07\x08", 8,
          "\xce\x5a\x5e\xd7\xe0\x57\x7a\x5f",
      },
      {
          "\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8d\x8e\x8f\x80"
          "\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xd0",
          "1.2.643.2.2.31.0", "\x01\x02\x03\x04\x05\x06\x07\x08", 8,
          "\x98\x56\xcf\x8b\xfc\xc2\x82\xf4",
      },
      {
          "\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8d\x8e\x8f\x80"
          "\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xd0",
          "1.2.643.2.2.31.1", "\x01\x02\x03\x04\x05\x06\x07\x08", 8,
          "\x66\x81\x84\xae\xdc\x48\xc9\x17",
      },
      {
          "\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8d\x8e\x8f\x80"
          "\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xd0",
          "1.2.643.2.2.31.2", "\x01\x02\x03\x04\x05\x06\x07\x08", 8,
          "\xdb\xee\x81\x14\x7b\x74\xb0\xf2",
      },
      {
          "\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8d\x8e\x8f\x80"
          "\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xd0",
          "1.2.643.2.2.31.3", "\x01\x02\x03\x04\x05\x06\x07\x08", 8,
          "\x31\xa3\x85\x9d\x0a\xee\xb8\x0e",
      },
      {
          "\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8d\x8e\x8f\x80"
          "\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xd0",
          "1.2.643.2.2.31.4", "\x01\x02\x03\x04\x05\x06\x07\x08", 8,
          "\xb1\x32\x3e\x0b\x21\x73\xcb\xd1",
      },
      {
          "\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8d\x8e\x8f\x80"
          "\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xd0",
          "1.2.643.2.2.30.0", "\x01\x02\x03\x04\x05\x06\x07\x08", 8,
          "\xce\xd5\x2a\x7f\xf7\xf2\x60\xd5",
      },
      {
          "\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8d\x8e\x8f\x80"
          "\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xd0",
          "1.2.643.2.2.30.1", "\x01\x02\x03\x04\x05\x06\x07\x08", 8,
  ...
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    err =
        gcry_cipher_open(&hde, GCRY_CIPHER_GOST28147, GCRY_CIPHER_MODE_ECB, 0);
    if (!err)
      err = gcry_cipher_open(&hdd, GCRY_CIPHER_GOST28147, GCRY_CIPHER_MODE_ECB,
                             0);
    if (err) {
    ...
    if (!err) err = gcry_cipher_set_sbox(hdd, tv[i].oid);
    if (err) {
    ...
    err = gcry_cipher_encrypt(
        hde, out, MAX_DATA_LEN, tv[i].plaintext,
        tv[i].inlen == -1 ? strlen((char *)tv[i].plaintext) : tv[i].inlen);
    if (err) {
    ...
    if (memcmp(tv[i].plaintext, out, tv[i].inlen)) {
      ...
      mismatch(tv[i].plaintext, tv[i].inlen, out, tv[i].inlen);
    }
...
static void check_stream_cipher(void) {
  static const struct tv {
    const char *name;
    int algo;
    int keylen;
      ...
      /* from draft-nir-cfrg-chacha20-poly1305-02 */
      {
          ...
          "\x07\x00\x00\x00\x40\x41\x42\x43\x44\x45\x46\x47",
          {
              {64,
               ...
               "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
               "\x00",
               "\x7b\xac\x2b\x25\x2d\xb4\x47\xaf\x09\xb6\x7a\x55\xa4\xe9\x55"
               "\x84"
               "\x0a\xe1\xd6\x73\x10\x75\xd9\xeb\x2a\x93\x75\x78\x3e\xd5\x53"
               "\xff"
               "\xa2\x7e\xcc\xde\xad\xdb\x4d\xb4\xd1\x17\x9c\xe4\xc9\x0b\x43"
               "\xd8"
               "\xbc\xb7\x94\x8c\x4b\x4b\x7d\x8b\x7d\xf6\x27\x39\x32\xa4\x69"
               "\x16"},
          },
      },
#endif /*USE_CHACHA20*/
  ...
  if (verbose) fprintf(stderr, "  Starting stream cipher checks.\n");
  ...
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    ...
    for (j = 0; tv[i].data[j].inlen; j++) {
      ...
      if (!err) err = gcry_cipher_setkey(hdd, tv[i].key, tv[i].keylen);
      if (err) {
  ...
  if (verbose) fprintf(stderr, "  Completed stream cipher checks.\n");
}

static void check_stream_cipher_large_block(void) {
  static const struct tv {
    const char *name;
    int algo;
    int keylen;
  ...
  for (i = 0; i < sizeof(tv) / sizeof(tv[0]); i++) {
    ...
    for (j = 0, p = buffer + buffersize; j < 1024; j++, p++)
      if (*p != 0x5a) die("large stream, buffer corrupted at j=%d\n", j);

    /* Now loop over all the data samples.  */
    for (j = 0; tv[i].data[j].length; j++) {
      assert(tv[i].data[j].offset + tv[i].data[j].length <= buffersize);

      if (memcmp(tv[i].data[j].result, buffer + tv[i].data[j].offset,
                 tv[i].data[j].length)) {
    ...
    for (n = 0, p = buffer, j = 0; n < buffersize; n += j, p += j) {
      ...
      if (n + j >= buffersize) j = buffersize - n;
      assert(j <= 512);
...
static void check_bulk_cipher_modes(void) {
  static const struct {
    int algo;
    int mode;
    const char *key;
    ...
    char t1_hash[20];
            ...
            {GCRY_CIPHER_AES256,
             GCRY_CIPHER_MODE_XTS,
             "abcdefghijklmnopABCDEFGHIJKLMNOP_abcdefghijklmnopABCDEFGHIJKLMNO",
             64,
             "1234567890123456",
             16,
             /*[16]*/
  ...
  if (verbose) fprintf(stderr, "Starting bulk cipher checks.\n");
  ...
  for (i = 0; i < DIM(tv); i++) {
    if (verbose)
      fprintf(stderr, "    checking bulk encryption for %s [%i], mode %d\n",
              gcry_cipher_algo_name(tv[i].algo), tv[i].algo, tv[i].mode);
    err = gcry_cipher_open(&hde, tv[i].algo, tv[i].mode, 0);
    if (!err) err = gcry_cipher_open(&hdd, tv[i].algo, tv[i].mode, 0);
    if (err) {
  ...
  if (verbose) fprintf(stderr, "Completed bulk cipher checks.\n");
leave:
  gcry_cipher_close(hde);
  gcry_cipher_close(hdd);
...
static int check_one_cipher_core_reset(gcry_cipher_hd_t hd, int algo, int mode,
                                       int pass, int nplain) {
  ...
  if (mode == GCRY_CIPHER_MODE_CCM) {
    ...
    if (err) {
      fail(
          "pass %d, algo %d, mode %d, gcry_cipher_ctl "
...
/* The core of the cipher check.  In addition to the parameters passed
   ...
   success.  */
static int check_one_cipher_core(int algo, int mode, int flags, const char *key,
                                 size_t nkey, const unsigned char *plain,
                                 size_t nplain, int bufshift, int pass) {
  gcry_cipher_hd_t hd;
  ...
  if (check_one_cipher_core_reset(hd, algo, mode, pass, nplain) < 0) return -1;
  ...
  if (err) {
    fail(
        "pass %d, algo %d, mode %d, in-place, gcry_cipher_encrypt failed:"
        " %s\n",
        pass, algo, mode, gpg_strerror(err));
    gcry_cipher_close(hd);
    return -1;
  ...
  if (err) {
    fail(
        "pass %d, algo %d, mode %d, in-place, gcry_cipher_decrypt failed:"
        " %s\n",
        pass, algo, mode, gpg_strerror(err));
    gcry_cipher_close(hd);
    return -1;
  ...
  pos = 0;
  while (pos < nplain) {
    if (piecelen > nplain - pos) piecelen = nplain - pos;

    err = gcry_cipher_encrypt(hd, out + pos, piecelen, plain + pos, piecelen);
    if (err) {
      fail(
          "pass %d, algo %d, mode %d, split-buffer (pos: %d, "
  ...
  while (pos < nplain) {
    ...
    if (err) {
      fail(
          "pass %d, algo %d, mode %d, in-place split-buffer (pos: %d, "
          "piecelen: %d), gcry_cipher_encrypt failed: %s\n",
          pass, algo, mode, pos, piecelen, gpg_strerror(err));
      gcry_cipher_close(hd);
      return -1;
  ...
  if (memcmp(plain, out, nplain))
    fail(
        "pass %d, algo %d, mode %d, in-place split-buffer, encrypt-decrypt"
        " mismatch\n",
        pass, algo, mode);
...
static void check_one_cipher(int algo, int mode, int flags) {
  ...
  for (bufshift = 0; bufshift < 4; bufshift++) {
    ...
    memcpy(key,
           "0123456789abcdef.,;/[]{}-=ABCDEF_"
           "0123456789abcdef.,;/[]{}-=ABCDEF",
           64);
    memcpy(plain, "foobar42FOOBAR17", 16);
    for (i = 16; i < 1040; i += 16) {
      memcpy(&plain[i], &plain[i - 16], 16);
      if (!++plain[i + 7]) plain[i + 6]++;
      if (!++plain[i + 15]) plain[i + 14]++;
    }

    if (check_one_cipher_core(algo, mode, flags, key, 64, plain, 1040, bufshift,
                              0 + 10 * bufshift))
      return;
...
static void check_ciphers(void) {
  ...
  if (verbose) fprintf(stderr, "Starting Cipher checks.\n");
  for (i = 0; algos[i]; i++) {
  ...
  if (verbose) fprintf(stderr, "Completed Cipher checks.\n");

  /* TODO: add some extra encryption to test the higher level functions */
}

static void check_cipher_modes(void) {
  if (verbose) fprintf(stderr, "Starting Cipher Mode checks.\n");
  ...
  if (verbose) fprintf(stderr, "Completed Cipher Mode checks.\n");
}

static void fillbuf_count(char *buf, size_t buflen, unsigned char pos) {
  while (buflen--) *((unsigned char *)(buf++)) = pos++;
}

static void check_one_md(int algo, const char *data, int len,
                         const char *expect, int elen, const char *key,
                         int klen) {
  gcry_md_hd_t hd, hd2;
  unsigned char *p;
  int mdlen;
  ...
  if ((*data == '!' && !data[1]) || /* hash one million times a "a" */
      (*data == '?' && !data[1]))   /* hash million byte data-set with byte
                                       pattern 0x00,0x01,0x02,... */
  {
    char aaa[1000];
    ...
    if (*data == '!') memset(aaa, 'a', 1000);

    /* Write in chuck with all sizes 1 to 1000 (500500 bytes)  */
    for (i = 1; i <= 1000 && left > 0; i++) {
      piecelen = i;
      if (piecelen > sizeof(aaa)) piecelen = sizeof(aaa);
      if (piecelen > left) piecelen = left;

      if (*data == '?') fillbuf_count(aaa, piecelen, 1000 * 1000 - left);
  ...
  if (!xof) {
    ...
    if (memcmp(p, expect, mdlen)) {
      printf("computed: ");
      for (i = 0; i < mdlen; i++) printf("%02x ", p[i] & 0xFF);
      printf("\nexpected: ");
      for (i = 0; i < mdlen; i++) printf("%02x ", expect[i] & 0xFF);
      printf("\n");
    ...
    if (memcmp(buf, expect, outmax)) {
      printf("computed: ");
      for (i = 0; i < outmax; i++) printf("%02x ", buf[i] & 0xFF);
      printf("\nexpected: ");
      for (i = 0; i < outmax; i++) printf("%02x ", expect[i] & 0xFF);
      printf("\n");
    ...
    if (*data == '!' && !data[1]) {
      ...
      for (i = 0; i < 1000; i++) {
        ...
        if (!err) gcry_md_write(crc1, buf, 1000);
      }
      if (err) {
        fail("algo %d, gcry_md_extract failed: %s\n", algo, gpg_strerror(err));
      }
      ...
      while (!err && left > 0) {
        ...
        if (!err) gcry_md_write(crc2, buf, piecelen);
        if (err) {
          fail("algo %d, gcry_md_extract failed: %s\n", algo,
               gpg_strerror(err));
        }
      ...
      if (memcmp(p1, p2, crclen)) {
        printf("computed: ");
        for (i = 0; i < crclen; i++) printf("%02x ", p2[i] & 0xFF);
        printf("\nexpected: ");
        for (i = 0; i < crclen; i++) printf("%02x ", p1[i] & 0xFF);
        printf("\n");
...
static void check_one_md_multi(int algo, const char *data, int len,
                               const char *expect) {
  gpg_error_t err;
  gcry_buffer_t iov[3];
  int iovcnt;
  ...
  if (*data == '!' && !data[1]) return; /* We can't do that here.  */
  if (*data == '?' && !data[1]) return; /* We can't do that here.  */
  ...
  if (memcmp(digest, expect, mdlen)) {
    printf("computed: ");
    for (i = 0; i < mdlen; i++) printf("%02x ", digest[i] & 0xFF);
    printf("\nexpected: ");
    for (i = 0; i < mdlen; i++) printf("%02x ", expect[i] & 0xFF);
    printf("\n");
...
static void check_digests(void) {
  ...
  static const struct algos {
    ...
    int keylen;
      ...
      {GCRY_MD_SHA1, "?" /* kludge for "\x00\x01\x02"..."\xfe\xff\x00\x01"...
                            (length 1000000) */
       ,
      ...
      {GCRY_MD_GOSTR3411_94, "This is message, length=32 bytes",
...
#include "./blake2b.h"
      {GCRY_MD_BLAKE2B_160, "",
       ...
       "\x6d\xe8\x55\x3d",
       20},
      {GCRY_MD_BLAKE2B_160, "\x9c\x9c\x38",
      ...
      {GCRY_MD_BLAKE2B_256, "",
       ...
       32},
      {GCRY_MD_BLAKE2B_256, "\x9c\x9c\x38",
      ...
      {GCRY_MD_BLAKE2B_384, "",
       ...
       48},
      {GCRY_MD_BLAKE2B_384, "\x9c\x9c\x38",
      ...
      {GCRY_MD_BLAKE2B_512, "",
       ...
       64},
      {GCRY_MD_BLAKE2B_512, "\x9c\x9c\x38",
...
#include "./blake2s.h"
      {GCRY_MD_BLAKE2S_128, "",
       "\x84\x89\x68\xb3\x59\x01\xe9\x57\x9a\x4d\xbf\x28\xdf\x99\xec\x23", 0,
       16, "\xea\xea\xd5\xc0\x96\x56\xec\x43\x30\x73\xa3\x17\xbb\xd3\x8e\x62",
       16},
      {GCRY_MD_BLAKE2S_128, "\x9c\x9c\x38",
       "\x2e\xbb\x18\x78\xda\x34\x05\xad\x98\x1a\x33\x06\x50\x35\xd3\x75", 3,
       16, "\xea\xea\xd5\xc0\x96\x56\xec\x43\x30\x73\xa3\x17\xbb\xd3\x8e\x62",
       16},
      {GCRY_MD_BLAKE2S_128,
       "\xab\xab\x56\x01\x58\x5a\xb3\x0d\xc1\xce\x8f\x5e\xee\x4d\x3b\x88"
       ...
       "\x68\xd6\x3f\x16\x55\x6b\xc1\x2d\xef\x1d\x0c\x29\x35\x5f\x94\xf3"
       "\x88\x7c\x04\x81\x86\x07\x8e\x95\x23\xb9\xdd\x97\x74\x0c\x80\x8c",
       "\x3c\xd4\xea\xd7\x88\x0b\x8e\x82\xde\x07\x9c\x1f\xad\x34\x17\xd4", 64,
       16, "\xea\xea\xd5\xc0\x96\x56\xec\x43\x30\x73\xa3\x17\xbb\xd3\x8e\x62",
       16},
      {GCRY_MD_BLAKE2S_128,
       ...
       "\x6b",
       "\xee\x92\xc5\x25\x4c\x29\x7a\x88\xe6\x9a\x23\x69\x56\xb6\x7c\xee", 65,
       16, "\xea\xea\xd5\xc0\x96\x56\xec\x43\x30\x73\xa3\x17\xbb\xd3\x8e\x62",
       16},
      {GCRY_MD_BLAKE2S_160, "",
       ...
       "\x6d\xe8\x55\x3d",
       20},
      {GCRY_MD_BLAKE2S_160, "\x9c\x9c\x38",
      ...
      {GCRY_MD_BLAKE2S_224, "",
       ...
       28},
      {GCRY_MD_BLAKE2S_224, "\x9c\x9c\x38",
      ...
      {GCRY_MD_BLAKE2S_256, "",
       ...
       "\xe2\xa6\x88\x2f\xb7\xe6\x9e\x85\x23\xa9\xcd\x76\x43\xb9\xfd\xb7",
       32},
      {GCRY_MD_BLAKE2S_256, "\x9c\x9c\x38",
  ...
  if (verbose) fprintf(stderr, "Starting hash checks.\n");
  ...
  for (i = 0; algos[i].md; i++) {
    ...
    if (verbose)
      fprintf(stderr, "  checking %s [%i] for length %d\n",
              gcry_md_algo_name(algos[i].md), algos[i].md,
              (!strcmp(algos[i].data, "!") || !strcmp(algos[i].data, "?"))
                  ? 1000000
                  : (int)strlen(algos[i].data));

    check_one_md(
        algos[i].md, algos[i].data,
        algos[i].datalen > 0 ? algos[i].datalen : strlen(algos[i].data),
        algos[i].expect, algos[i].expectlen, algos[i].key, algos[i].keylen);

    if (algos[i].key && algos[i].keylen) continue;
...
leave:
  if (verbose) fprintf(stderr, "Completed hash checks.\n");
}

static void check_one_hmac(int algo, const char *data, int datalen,
  ...
  p = gcry_md_read(hd2, algo);
  if (!p) fail("algo %d, hmac gcry_md_read failed\n", algo);
...
static void check_hmac(void) {
  static const struct algos {
    ...
    const char *expect;
      ...
      {GCRY_MD_SHA256, "Test Using Larger Than Block-Size Key - Hash Key First",
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
      ...
      {GCRY_MD_SHA224, "Test Using Larger Than Block-Size Key - Hash Key First",
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
      ...
      {GCRY_MD_SHA384, "Test Using Larger Than Block-Size Key - Hash Key First",
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
      ...
      {GCRY_MD_SHA512, "Test Using Larger Than Block-Size Key - Hash Key First",
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
       "\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa\xaa"
  ...
  if (verbose) fprintf(stderr, "Starting hashed MAC checks.\n");
  ...
  for (i = 0; algos[i].md; i++) {
    ...
    if (verbose)
      fprintf(stderr, "  checking %s [%i] for %d byte key and %d byte data\n",
              gcry_md_algo_name(algos[i].md), algos[i].md,
              (int)strlen(algos[i].key), (int)strlen(algos[i].data));

    check_one_hmac(algos[i].md, algos[i].data, strlen(algos[i].data),
                   algos[i].key, strlen(algos[i].key), algos[i].expect);
  }

  if (verbose) fprintf(stderr, "Completed hashed MAC checks.\n");
}

static void check_one_mac(int algo, const char *data, int datalen,
                          const char *key, int keylen, const char *iv,
                          int ivlen, const char *expect, int test_buffering) {
  gcry_mac_hd_t hd;
  unsigned char *p;
  unsigned int maclen;
  size_t macoutlen;
  ...
  if (memcmp(p, expect, maclen)) {
    printf("computed: ");
    for (i = 0; i < maclen; i++) printf("%02x ", p[i] & 0xFF);
    printf("\nexpected: ");
    for (i = 0; i < maclen; i++) printf("%02x ", expect[i] & 0xFF);
    printf("\n");
...
static void check_mac(void) {
  static const struct algos {
    ...
    unsigned int klen;
      ...
      {GCRY_MAC_HMAC_SHA3_512, "Hi There",
       "\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b"
       "\x0b\x0b\x0b",
      ...
      {GCRY_MAC_CMAC_3DES, "\x6b\xc1\xbe\xe2\x2e\x40\x9f\x96",
       "\x8a\xa8\x3b\xf8\xcb\xda\x10\x62\x0b\xc1\xbf\x19\xfb\xb6\xcd\x58"
       "\xbc\x31\x3d\x4a\x37\x1c\xa8\xb5",
      ...
      {GCRY_MAC_POLY1305, "Cryptographic Forum Research Group",
      ...
      {GCRY_MAC_POLY1305,
       ...
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", NULL,
       191, 32},
      {GCRY_MAC_POLY1305,
       ...
       "\x36\xe5\xf6\xb5\xc5\xe0\x60\x70\xf0\xef\xca\x96\x22\x7a\x86\x3e", NULL,
       0, 32},
      {GCRY_MAC_POLY1305,
       "Any submission to the IETF intended by the Contributor for "
       "publication as all or part of an IETF Internet-Draft or RFC and "
       ...
       "\x36\xe5\xf6\xb5\xc5\xe0\x60\x70\xf0\xef\xca\x96\x22\x7a\x86\x3e"
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\xf3\x47\x7e\x7c\xd9\x54\x17\xaf\x89\xa6\xb8\x79\x4c\x31\x0c\xf0", NULL,
       0, 32},
      ...
      {GCRY_MAC_POLY1305,
       ...
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x03\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", NULL,
       16, 32},
      /* TV#6 */
      {GCRY_MAC_POLY1305,
       "\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
      ...
      {GCRY_MAC_POLY1305,
       ...
       "\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x05\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", NULL,
       48, 32},
      /* TV#8 */
      {GCRY_MAC_POLY1305,
       "\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF"
      ...
      {GCRY_MAC_POLY1305,
       ...
       "\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\xFA\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF", NULL,
       16, 32},
      /* TV#10 */
      {GCRY_MAC_POLY1305,
       ...
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x14\x00\x00\x00\x00\x00\x00\x00\x55\x00\x00\x00\x00\x00\x00\x00", NULL,
       64, 32},
      /* TV#11 */
      {GCRY_MAC_POLY1305,
       "\xE3\x35\x94\xD7\x50\x5E\x43\xB9\x00\x00\x00\x00\x00\x00\x00\x00"
       ...
       "\x01\x00\x00\x00\x00\x00\x00\x00\x04\x00\x00\x00\x00\x00\x00\x00"
       "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
       "\x13\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", NULL,
       48, 32},
      /* from http://cr.yp.to/mac/poly1305-20050329.pdf */
      {GCRY_MAC_POLY1305, "\xf3\xf6",
       ...
       "\xf4\xc6\x33\xc3\x04\x4f\xc1\x45\xf8\x4f\x33\x5c\xb8\x19\x53\xde", NULL,
       0, 32},
      {GCRY_MAC_POLY1305, "",
       ...
       "\xdd\x3f\xab\x22\x51\xf1\x1a\xc7\x59\xf0\x88\x71\x29\xcc\x2e\xe7", NULL,
       0, 32},
      {GCRY_MAC_POLY1305,
      ...
      {GCRY_MAC_POLY1305_AES, "\xf3\xf6",
       ...
       "\xfb\x44\x73\x50\xc4\xe8\x68\xc5\x2a\xc3\x27\x5c\xf9\xd4\x32\x7e", 0,
       32},
      {GCRY_MAC_POLY1305_AES, "",
       "\x75\xde\xaa\x25\xc0\x9f\x20\x8e\x1d\xc4\xce\x6b\x5c\xad\x3f\xbf"
       "\xa0\xf3\x08\x00\x00\xf4\x64\x00\xd0\xc7\xe9\x07\x6c\x83\x44\x03",
       "\xdd\x3f\xab\x22\x51\xf1\x1a\xc7\x59\xf0\x88\x71\x29\xcc\x2e\xe7",
       "\x61\xee\x09\x21\x8d\x29\xb0\xaa\xed\x7e\x15\x4a\x2c\x55\x09\xcc", 0,
       32},
      {GCRY_MAC_POLY1305_AES,
       "\x66\x3c\xea\x19\x0f\xfb\x83\xd8\x95\x93\xf3\xf4\x76\xb6\xbc\x24"
       "\xd7\xe6\x79\x10\x7e\xa2\x6a\xdb\x8c\xaf\x66\x52\xd0\x65\x61\x36",
       ...
       "\xae\x21\x2a\x55\x39\x97\x29\x59\x5d\xea\x45\x8b\xc6\x21\xff\x0e", 0,
       32},
      {GCRY_MAC_POLY1305_AES,
       "\xab\x08\x12\x72\x4a\x7f\x1e\x34\x27\x42\xcb\xed\x37\x4d\x94\xd1"
       "\x36\xc6\xb8\x79\x5d\x45\xb3\x81\x98\x30\xf2\xc0\x44\x91\xfa\xf0"
       ...
       "\x12\x97\x6a\x08\xc4\x42\x6d\x0c\xe8\xa8\x24\x07\xc4\xf4\x82\x07",
       "\x51\x54\xad\x0d\x2c\xb2\x6e\x01\x27\x4f\xc5\x11\x48\x49\x1f\x1b",
       "\x9a\xe8\x31\xe7\x43\x97\x8d\x3a\x23\x52\x7c\x71\x28\x14\x9e\x3a", 0,
       32},
      {GCRY_MAC_POLY1305_AES, "?", "????????????????????????????????",
  ...
  if (verbose) fprintf(stderr, "Starting MAC checks.\n");
  ...
  for (i = 0; algos[i].algo; i++) {
    ...
    if (verbose)
      fprintf(stderr, "  checking %s [%i] for %d byte key and %d byte data\n",
              gcry_mac_algo_name(algos[i].algo), algos[i].algo,
              (int)strlen(algos[i].key),
              (!strcmp(algos[i].data, "!") || !strcmp(algos[i].data, "?"))
                  ? 1000000
                  : (int)strlen(algos[i].data));
  ...
  if (verbose) fprintf(stderr, "Completed MAC checks.\n");
}
...
static void verify_one_signature(gcry_sexp_t pkey, gcry_sexp_t hash,
                                 gcry_sexp_t badhash, gcry_sexp_t sig) {
  gcry_error_t rc;

  rc = gcry_pk_verify(sig, hash, pkey);
  if (rc) fail("gcry_pk_verify failed: %s\n", gpg_strerror(rc));
...
static void check_pubkey_sign(int n, gcry_sexp_t skey, gcry_sexp_t pkey,
  ...
  static const struct {
    ...
    int expected_rc;
      ...
      {"(data\n (flags pkcs1)\n"
       ...
       GCRY_PK_RSA, GPG_ERR_DIGEST_ALGO},
      {"(data\n (flags )\n"
  ...
  if (rc) die("converting data failed: %s\n", gpg_strerror(rc));

  for (dataidx = 0; datas[dataidx].data; dataidx++) {
    if (datas[dataidx].algo && datas[dataidx].algo != algo) continue;
    ...
    if (!rc) verify_one_signature(pkey, hash, badhash, sig);
...
static void check_pubkey_sign_ecdsa(int n, gcry_sexp_t skey, gcry_sexp_t pkey) {
  gcry_error_t rc;
  gcry_sexp_t sig, badhash, hash;
  unsigned int nbits;
  int dataidx;
  ...
  for (dataidx = 0; datas[dataidx].data; dataidx++) {
    if (datas[dataidx].nbits != nbits) continue;
    ...
    if (!rc && verbose > 1) show_sexp("ECDSA signature:\n", sig);
...
static void check_pubkey_crypt(int n, gcry_sexp_t skey, gcry_sexp_t pkey,
                               int algo) {
  gcry_error_t rc;
  ...
  static const struct {
    ...
    int special;
      ...
      {GCRY_PK_RSA,
       "(data\n (flags oaep)\n (hash-algo sha1)\n"
       " (value #11223344556677889900AA#))\n",
       "(flags oaep)(hash-algo sha1)", 1, 0, 0},
      {GCRY_PK_RSA,
       "(data\n (flags oaep)\n (hash-algo sha1)\n (label \"test\")\n"
       " (value #11223344556677889900AA#))\n",
       "(flags oaep)(hash-algo sha1)(label \"test\")", 1, 0, 0},
      {GCRY_PK_RSA,
       "(data\n (flags oaep)\n (hash-algo sha1)\n (label \"test\")\n"
       " (value #11223344556677889900AA#)\n"
       " (random-override #4253647587980912233445566778899019283747#))\n",
      ...
      {GCRY_PK_RSA,
       "(data\n (flags pkcs1)\n"
       " (hash sha1 #11223344556677889900AABBCCDDEEFF10203040#))\n",
       NULL, 0, GPG_ERR_CONFLICT, 0},
      {0,
       "(data\n (flags raw foo)\n"
       " (hash sha1 #11223344556677889900AABBCCDDEEFF10203040#))\n",
       NULL, 0, GPG_ERR_INV_FLAG, 0},
      {0,
       "(data\n (flags raw)\n"
       " (value #11223344556677889900AA#))\n",
       "(flags oaep)", 1, 0, GPG_ERR_ENCODING_PROBLEM, 1},
      {GCRY_PK_RSA,
       "(data\n (flags oaep)\n"
       " (value #11223344556677889900AA#))\n",
       "(flags pkcs1)", 1, 0, GPG_ERR_ENCODING_PROBLEM, 1},
      {0,
       "(data\n (flags pss)\n"
       " (value #11223344556677889900AA#))\n",
       NULL, 0, GPG_ERR_CONFLICT},
      {0, NULL}};
  ...
  for (dataidx = 0; datas[dataidx].data; dataidx++) {
    ...
    if (!rc) {
      ...
      if (datas[dataidx].hint) {
        ...
        if (!hint) die("can't allocate memory\n");
        hint_len = gcry_sexp_sprint(list, GCRYSEXP_FMT_CANON, hint, hint_len);
      ...
      if (!rc && datas[dataidx].special == 1) {
        ...
        fail("gcry_pk_decrypt failed: expected %d (%s), got %d (%s)\n",
             ...
             gpg_strerror(datas[dataidx].decrypt_expected_rc), rc,
             gpg_strerror(rc));
      }
        ...
        if (p1 && p2) {
          ...
          if (n1 != n2 || memcmp(s1, s2, n1)) {
            if (expect_mismatch)
              ...
              fail(
                  "gcry_pk_encrypt/gcry_pk_decrypt "
                  "do not roundtrip\n");
          }
        }
...
static void check_pubkey_grip(int n, const unsigned char *grip,
  ...
  if (memcmp(sgrip, pgrip, 20)) fail("[%i] keygrips don't match\n", n);
  if (memcmp(sgrip, grip, 20)) fail("wrong keygrip for RSA key\n");
}

static void do_check_one_pubkey(int n, gcry_sexp_t skey, gcry_sexp_t pkey,
                                const unsigned char *grip, int algo,
                                int flags) {
  if (flags & FLAG_SIGN) {
  ...
  if (flags & FLAG_CRYPT) check_pubkey_crypt(n, skey, pkey, algo);
  if (grip && (flags & FLAG_GRIP)) check_pubkey_grip(n, grip, skey, pkey, algo);
}
...
static void check_one_pubkey(int n, test_spec_pubkey_t spec) {
  ...
  err = gcry_sexp_sscan(&skey, NULL, spec.key.secret, strlen(spec.key.secret));
  if (!err)
    err =
        gcry_sexp_sscan(&pkey, NULL, spec.key.public, strlen(spec.key.public));
  if (err) die("converting sample key failed: %s\n", gpg_strerror(err));

  do_check_one_pubkey(n, skey, pkey, (const unsigned char *)spec.key.grip,
                      spec.id, spec.flags);
...
static void get_keys_new(gcry_sexp_t *pkey, gcry_sexp_t *skey) {
  ...
  int rc;
  if (verbose) fprintf(stderr, "  generating RSA key:");
...
static void check_one_pubkey_new(int n) {
  ...
  do_check_one_pubkey(n, skey, pkey, NULL, GCRY_PK_RSA, FLAG_SIGN | FLAG_CRYPT);
  gcry_sexp_release(pkey);
  gcry_sexp_release(skey);
}
...
static void check_pubkey(void) {
  ...
  if (verbose) fprintf(stderr, "Starting public key checks.\n");
  ...
  if (verbose) fprintf(stderr, "Completed public key checks.\n");
  ...
  if (verbose) fprintf(stderr, "Completed additional public key checks.\n");
}
...
int main(int argc, char **argv) {
    ...
    if (!strcmp(*argv, "--")) {
      ...
      if (argc) {
        if (gcry_control(GCRYCTL_DISABLE_HWF, *argv, NULL))
          fprintf(stderr,
                  PGM ": unknown hardware feature `%s' - option ignored\n",
                  *argv);
        argc--;
        argv++;
      }
  ...
  if (use_fips) xgcry_control(GCRYCTL_FORCE_FIPS_MODE, 0);
  ...
  if (gcry_fips_mode_active()) in_fips_mode = 1;

  if (!in_fips_mode) xgcry_control(GCRYCTL_DISABLE_SECMEM, 0);

  if (verbose) gcry_set_progress_handler(progress_handler, NULL);
  ...
  if (in_fips_mode && !selftest_only) {
    ...
    if (err)
      ...
      if (err)
        ...
        fputs(
            "Note: Two lines with error messages follow "
            "- this is expected\n",
            stderr);
        gcry_md_get_algo(md);
        gcry_md_close(md);
        if (gcry_control(GCRYCTL_OPERATIONAL_P, 0))
          ...
          if (!gcry_control(GCRYCTL_OPERATIONAL_P, 0))
            fail(
                "did not reach operational after error "
                "and self-test\n");
        }
      }
    }
    ...
    if (gcry_control(GCRYCTL_SELFTEST, 0)) fail("running self-test failed\n");
  }
