...
static struct {
  ...
  const char *h;         /* Cofactor.  */
    ...
    {"NIST P-192", "0xfffffffffffffffffffffffffffffffeffffffffffffffff",
    ...
    {"NIST P-224", "0xffffffffffffffffffffffffffffffff000000000000000000000001",
...
static void set_get_point(void) {
  ...
  if (gcry_mpi_cmp_ui(x, 0) || gcry_mpi_cmp_ui(y, 0) || gcry_mpi_cmp_ui(z, 0))
  ...
  gcry_mpi_point_get(x, y, z, point);
  if (gcry_mpi_cmp_ui(x, 17) || gcry_mpi_cmp_ui(y, 42) ||
      gcry_mpi_cmp_ui(z, 11371))
...
static int get_and_cmp_mpi(const char *name, const char *mpistring,
...
static int get_and_cmp_point(const char *name, const char *mpi_x_string,
                             const char *mpi_y_string, const char *desc,
                             gcry_ctx_t ctx) {
  gcry_mpi_point_t point;
  gcry_mpi_t x, y, z;
  int result = 0;
...
static void context_param(void) {
  ...
  err = gcry_sexp_build(&keyparam, NULL, "(public-key(ecc(curve %s)(q %m)))",
                        "NIST P-256", q);
  ...
  if (err)
    ...
    get_and_cmp_point("q", sample_p256_q_x, sample_p256_q_y, "nistp256", ctx);

    /* Delete Q.  */
    err = gcry_mpi_ec_set_mpi("q", NULL, ctx);
    if (err) fail("clearing Q for nistp256 failed: %s\n", gpg_strerror(err));
    ...
    if (err) fail("setting Q for nistp256 failed: %s\n", gpg_strerror(err));
  ...
  if (gcry_fips_mode_active()) goto cleanup;
  ...
  if (err)
    ...
    get_and_cmp_point("q", sample_ed25519_q_x, sample_ed25519_q_y, "Ed25519",
                      ctx);
    ...
    if (err) fail("setting d for Ed25519 failed: %s\n", gpg_strerror(err));
    gcry_mpi_release(d);
    ...
    if (err) fail("setting d for Ed25519 failed(2): %s\n", gpg_strerror(err));
    if (gcry_mpi_ec_get_mpi("q", ctx, 0))
    ...
    err = gcry_mpi_ec_set_mpi("q", q, ctx);
    if (err) fail("setting Q for Ed25519 failed: %s\n", gpg_strerror(err));
    gcry_mpi_release(q);
...
static void basic_ec_math(void) {
  ...
  G = make_point("188DA80EB03090F67CBF20EB43A18800F4FF0AFD82FF1012",
                 "7192B95FFC8DA78631011ED6B24CDD573F977A11E794811", "1");
...
static void twistededwards_math(void) {
  ...
  if (gcry_mpi_cmp_ui(w, 1)) fail("failed assertion: 2^{p-1} mod p == 1\n");
  ...
  if (gcry_mpi_cmp_ui(w, 1)) fail("failed assertion: 2^{n-1} mod n == 1\n");
  ...
  gcry_mpi_abs(w);
  if (gcry_mpi_cmp(w, a)) fail("failed assertion: b^{(p-1)/2} mod p == p-1\n");
