...
static void print_hex(const char *text, const void *buf, size_t n) {
  ...
  info("%s", text);
  for (; n; n--, p++) fprintf(stderr, "%02X", *p);
  putc('\n', stderr);
}
...
static int readn(int fd, void *buf, size_t buflen, size_t *ret_nread) {
  ...
  if (ret_nread) *ret_nread = buflen - nleft;
  return 0;
}
#endif /*!HAVE_W32_SYSTEM*/

/* Check that forking won't return the same random. */
static void check_forking(void) {
#ifdef HAVE_W32_SYSTEM
  if (verbose) info("check_forking skipped: not applicable on Windows\n");
  ...
  if (verbose) print_hex("initial random: ", tmp1, sizeof tmp1);

  if (pipe(rp) == -1) die("pipe failed: %s\n", strerror(errno));

  pid = fork();
  if (pid == (pid_t)(-1)) die("fork failed: %s\n", strerror(errno));
  if (!pid) {
  ...
  if (verbose) print_hex(" parent random: ", tmp1p, sizeof tmp1p);

  close(rp[1]);
  if (readn(rp[0], tmp1c, sizeof tmp1c, &nread))
    die("read failed: %s\n", strerror(errno));
  if (nread != sizeof tmp1c) die("read too short\n");
...
static void check_nonce_forking(void) {
#ifdef HAVE_W32_SYSTEM
  if (verbose) info("check_nonce_forking skipped: not applicable on Windows\n");
#else  /*!HAVE_W32_SYSTEM*/
  pid_t pid;
  int rp[2];
  int i, status;
  ...
  if (verbose) info("checking that a fork won't cause the same nonce output\n");
  ...
  if (verbose) print_hex("initial nonce: ", nonce1, sizeof nonce1);
  ...
  if (verbose) print_hex(" parent nonce: ", nonce1p, sizeof nonce1p);

  close(rp[1]);
  if (readn(rp[0], nonce1c, sizeof nonce1c, &nread))
    die("read failed: %s\n", strerror(errno));
  if (nread != sizeof nonce1c) die("read too short\n");
...
static void check_close_random_device(void) {
#ifdef HAVE_W32_SYSTEM
  if (verbose)
  ...
  if (verbose) info("checking that close_random_device works\n");

  gcry_randomize(buf, sizeof buf, GCRY_STRONG_RANDOM);
  if (verbose) print_hex("parent random: ", buf, sizeof buf);
...
static void check_rng_type_switching(void) {
  int rngtype, initial;
  char tmp1[4];

  if (verbose) info("checking whether RNG type switching works\n");

  rngtype = rng_type();
  if (debug) info("rng type: %d\n", rngtype);
  initial = rngtype;
  gcry_randomize(tmp1, sizeof tmp1, GCRY_STRONG_RANDOM);
  if (debug) print_hex("  sample: ", tmp1, sizeof tmp1);
  if (rngtype != rng_type()) die("RNG type unexpectedly changed\n");
  ...
  rngtype = rng_type();
  if (debug) info("rng type: %d\n", rngtype);
  if (rngtype != initial) die("switching to FIPS RNG unexpectedly succeeded\n");
  gcry_randomize(tmp1, sizeof tmp1, GCRY_STRONG_RANDOM);
  if (debug) print_hex("  sample: ", tmp1, sizeof tmp1);
  if (rngtype != rng_type()) die("RNG type unexpectedly changed\n");
...
static void check_early_rng_type_switching(void) {
  int rngtype, initial;
...
static void check_drbg_reinit(void) {
  static struct {
    const char *flags;
  } tv[] = {{NULL},          {""},
            {"sha1"},        {"sha1 pr"},
            {"sha256"},      {"sha256 pr"},
            {"sha512"},      {"sha512 pr"},
            {"hmac sha1"},   {"hmac sha1 pr"},
            {"hmac sha256"}, {"hmac sha256 pr"},
            {"hmac sha512"}, {"hmac sha512 pr"},
            {"aes sym128"},  {"aes sym128 pr"},
            {"aes sym192"},  {"aes sym192 pr"},
            {"aes sym256"},  {"aes sym256 pr"}};
  int tidx;
  gpg_error_t err;
  char pers_string[] = "I'm a doctor, not an engineer.";
  gcry_buffer_t pers[1];

  if (verbose) info("checking DRBG_REINIT\n");
...
static void run_all_rng_tests(const char *program) {
  static const char *options[] = {"--early-rng-check",
  ...
  if (!cmdline) die("out of core\n");

  for (idx = 0; options[idx]; idx++) {
    if (verbose) info("now running with options '%s'\n", options[idx]);
    strcpy(cmdline, program);
    strcat(cmdline, " --in-recursion");
    if (verbose) strcat(cmdline, " --verbose");
    if (debug) strcat(cmdline, " --debug");
    if (with_progress) strcat(cmdline, " --progress");
    strcat(cmdline, " ");
    strcat(cmdline, options[idx]);
    if (system(cmdline)) die("running '%s' failed\n", cmdline);
  }
...
int main(int argc, char **argv) {
  ...
  if (early_rng) {
    ...
    if (!gcry_fips_mode_active()) check_early_rng_type_switching();
  }
  ...
  if (!getenv("GCRYPT_IN_REGRESSION_TEST")) check_drbg_reinit();

  /* Don't switch RNG in fips mode.  */
  if (!gcry_fips_mode_active()) check_rng_type_switching();

  if (!in_recursion) run_all_rng_tests(program);

  if (debug) xgcry_control(GCRYCTL_DUMP_RANDOM_STATS);
