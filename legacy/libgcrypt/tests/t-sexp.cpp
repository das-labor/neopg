...
static gcry_mpi_t hex2mpiopa(const char *string) {
  char *buffer;
  size_t buflen;
  gcry_mpi_t val;

  buffer = hex2buffer(string, &buflen);
  if (!buffer) die("hex2mpiopa '%s' failed: parser error\n", string);
  val = gcry_mpi_set_opaque(NULL, buffer, buflen * 8);
  if (!buffer) die("hex2mpiopa '%s' failed: set_opaque error\n", string);
  return val;
}
...
static int cmp_bufhex(const void *a, size_t alen, const char *b) {
  void *bbuf;
  size_t blen;
  int res;
  ...
  if (!bbuf) die("cmp_bufhex: error converting hex string\n");
...
static void basic(void) {
  ...
  static struct {
    const char *token;
    const char *parm;
  } values[] = {{"public-key", NULL}, {"dsa", NULL}, {"dsa", "p"}, {"dsa", "y"},
                {"dsa", "q"},         {"dsa", "g"},  {NULL}};
  ...
  for (pass = 0;; pass++) {
    ...
    switch (pass) {
      case 0:
        string =
            ("(public-key (dsa (p #41424344#) (y this_is_y) "
      ...
      case 1:
        ...
        m = gcry_mpi_set_ui(NULL, 42);
        if (gcry_sexp_build(&sexp, NULL, string, 15, "foo\0\x01\0x02789012345",
                            m)) {
          gcry_mpi_release(m);
          fail(" scanning `%s' failed\n", string);
          return;
      ...
      case 2:
        string =
            ("(public-key (dsa (p #41424344#) (y silly_y_value) "
             "(q %b) (g %m)))");

        m = gcry_mpi_set_ui(NULL, 17);
        if (gcry_sexp_build(&sexp, NULL, string, secure_buffer_len,
                            secure_buffer, m)) {
          gcry_mpi_release(m);
          fail(" scanning `%s' failed\n", string);
          return;
      ...
      case 3: {
        ...
        if (gcry_sexp_new(&help_sexp, "(foobar-parms (xp #1234#)(xq #03#))", 0,
                          1)) {
        ...
        string =
            ("(public-key (dsa (p #41424344#) (parm %S) "
             "(y dummy)(q %b) (g %m)))");
        m = gcry_mpi_set_ui(NULL, 17);
        if (gcry_sexp_build(&sexp, NULL, string, help_sexp, secure_buffer_len,
                            secure_buffer, m)) {
          gcry_mpi_release(m);
          fail(" scanning `%s' failed\n", string);
          return;
...
static void canon_len(void) {
  ...
  for (idx = 0; values[idx].text; idx++) {
    n = gcry_sexp_canon_len((const unsigned char *)values[idx].text,
                            values[idx].textlen, &erroff, &errcode);

    if (n && n == values[idx].expected)
      ; /* success */
    else if (!n &&
             !values[idx]
                  .expected) { /* we expected an error - check that this is
                                  the right one */
      ...
      fail("canonical length test %d failed - n=%u, off=%u, err=%d\n", idx,
           (unsigned int)n, (unsigned int)erroff, errcode);
  }
}
...
static int compare_to_canon(gcry_sexp_t se, const unsigned char *canon,
                            size_t canonlen) {
  size_t n, n1;
  char *p1;
...
static void back_and_forth_one(int testno, const char *buffer, size_t length) {
  gcry_error_t rc;
  gcry_sexp_t se, se1;
  unsigned char *canon;
...
static void back_and_forth(void) {
  static struct {
    const char *buf;
    int len;
  } tests[] = {
      ...
      {"(protected-private-key \n"
       " (rsa \n"
       "  (n "
       "#00BE8A536204687149A48FF9F1715FF3530AD9A836D62102BF4065E5CF5953236DB94F"
       "1DF2FF4D525CD4CE7966DDC3C839968E8BAC2948934DF047CC65287CD79F6C23C93E55D"
       "7F9231E3942BD496DE383469977635A51ADF4AF747DB958CA02E9940DFC1DC0FC7FC755"
       "E7EB6618FEE6DA54B8A06E0CBF9D9257443F9992261435#)\n"
       "  (e #010001#)\n"
       ...
       "   "
       "#F7B0B535F8F8E22F4F3DA031224070303F82F9207D42952F1ACF21A4AB1C50304EBB25"
       "527992C7B265A9E9FF702826FB88759BDD55E4759E9FCA6C879538C9D043A9C60A326CB"
       "6681090BAA731289BD880A7D5774D9999F026E5E7963BFC8C0BDC9F061393CB734B4F25"
       "9725C0A0A0B15BA39C39146EF6A1B3DC4DF30A22EBE09FD05AE6CB0C8C6532951A925F3"
       "54F4E26A51964F5BBA50081690C421C8385C4074E9BAB9297D081B857756607EAE65241"
       "5275A741C89E815558A50AC638EDC5F5030210B4395E3E1A40FF38DCCCB333A19EA88EF"
       "E7E4D51B54128C6DF27395646836679AC21B1B25C1DA6F0A7CE9F9BE078EFC7934FA9AE"
       "202CBB0AA06C20DFAF9A66FAB7E9073FBE96B9A7F25C3BA45EC3EECA65796AEE313BA14"
       "8DE5314F30345B452B50B17C4D841A7F27397126E8C10BD0CE3B50A82C0425AAEE77980"
       "31671407B681F52916256F78CAF92A477AC27BCBE26DAFD1BCE386A853E2A036F8314BB"
       "2E8E5BB1F196434232EFB0288331C2AB16DBC5457CC295EB966CAC5CE73D5DA5D566E46"
       "9F0EFA82F9A12B8693E0#)\n"
       "  )\n"
       " )\n",
       0},
      {"((sha1 #8B98CBF4A9823CA7# \"2097\") #3B6FC9#)", 0},
      {"((4:sha18:\x8B\x98\xCB\xF4\xA9\x82\x3C\xA7"
       "4:2097)3:\x3B\x6F\xC9)",
       0},
      {"((4:sha18:\x8B\x98\xCB\x22\xA9\x82\x3C\xA7"
       "4:2097)3:\x3B\x6F\xC9)",
       0},
      {"((sha1 #64652267686970C9# \"2097\") #3B6FC9#)", 0},
      {"((4:sha18:\x64\x65\x22\x67\x68\xc3\xa4\x71"
       "4:2097)3:\x3B\x6F\xC9)",
       0},
...
static void check_sscan(void) {
  static struct {
    const char *text;
    gcry_error_t expected_err;
  } values[] = {/* Bug reported by Olivier L'Heureux 2003-10-07 */
                {"(7:sig-val(3:dsa"
  ...
  for (idx = 0; values[idx].text; idx++) {
    err = gcry_sexp_sscan(&s, NULL, values[idx].text, strlen(values[idx].text));
...
static void check_extract_param(void) {
  ...
  static struct {
    ...
    const char *exp_d;
      ...
      {
          sample1, NULL, "pabgnqd", 6, GPG_ERR_MISSING_VALUE,
      },
      {sample1, NULL, "pabgnq", 7, GPG_ERR_INV_ARG},
      {sample1, NULL, "pab'gnq", 7, GPG_ERR_SYNTAX},
      {sample1, NULL, "pab''gnq", 7, GPG_ERR_SYNTAX},
      {sample1, NULL, "pabgnqd", 7, 0, sample1_px, sample1_ax, sample1_bx,
       sample1_gx, sample1_nx, sample1_qx, sample1_d},
      {sample1, NULL, "  pab\tg nq\nd  ", 7, 0, sample1_px, sample1_ax,
       sample1_bx, sample1_gx, sample1_nx, sample1_qx, sample1_d},
      {sample1, NULL, "abg", 3, 0, sample1_ax, sample1_bx, sample1_gx},
      {sample1, NULL, "ab'g'", 3, 0, sample1_ax, sample1_bx, sample1_gx},
      {sample1, NULL, "x?abg", 4, 0, NULL, sample1_ax, sample1_bx, sample1_gx},
      {sample1, NULL, "p?abg", 4, GPG_ERR_USER_1, NULL, sample1_ax, sample1_bx,
       sample1_gx},
      {sample1, NULL, "pax?gnqd", 7, 0, sample1_px, sample1_ax, NULL,
       sample1_gx, sample1_nx, sample1_qx, sample1_d},
      {sample1, "public-key", "pabgnqd", 7,
       GPG_ERR_NO_OBJ, /* d is not in public key.  */
       sample1_px, sample1_ax, sample1_bx, sample1_gx, sample1_nx, sample1_qx,
       sample1_d},
      {sample1, "private-key", "pabgnqd", 7, 0, sample1_p, sample1_a, sample1_b,
       sample1_g, sample1_n, sample1_q, sample1_d},
      {sample1, "public-key!ecc", "pabgnq", 6, 0, sample1_px, sample1_ax,
       sample1_bx, sample1_gx, sample1_nx, sample1_qx},
      {sample1, "public-key!ecc!foo", "pabgnq", 6, GPG_ERR_NOT_FOUND},
      {sample1, "public-key!!ecc", "pabgnq", 6, GPG_ERR_NOT_FOUND},
      {sample1, "private-key", "pa/bgnqd", 7, 0, sample1_p, sample1_a,
       sample1_b, sample1_g, sample1_n, sample1_q, sample1_d},
      {sample1, "private-key", "p-a+bgnqd", 7, 0, sample1_p, "-01", sample1_b,
       sample1_g, sample1_n, sample1_q, sample1_d},
      {NULL}};
  int idx, i;
  const char *paramstr;
  ...
  for (idx = 0; tests[idx].sexp_str; idx++) {
    ...
    if (err) die("converting string to sexp failed: %s", gpg_strerror(err));
    ...
    switch (tests[idx].nparam) {
      ...
      case 7:
        err = gcry_sexp_extract_param(sxp, tests[idx].path, tests[idx].list,
                                      mpis + 0, mpis + 1, mpis + 2, mpis + 3,
                                      mpis + 4, mpis + 5, mpis + 6, NULL);
        break;
      default:
        die("test %d: internal error", idx);
    }

    if (tests[idx].expected_err && tests[idx].expected_err != GPG_ERR_USER_1) {
      if (tests[idx].expected_err != gpg_err_code(err))
        fail(
            "gcry_sexp_extract_param test %d failed: "
            "expected error '%s' - got '%s'",
            idx, gpg_strerror(tests[idx].expected_err), gpg_strerror(err));

    } else if (err) {
      fail("gcry_sexp_extract_param test %d failed: %s", idx,
           gpg_strerror(err));
    } else /* No error - check the extracted values.  */
    {
      for (paramidx = 0; paramidx < DIM(mpis); paramidx++) {
        switch (paramidx) {
          case 0:
            paramstr = tests[idx].exp_p;
            break;
          case 1:
            paramstr = tests[idx].exp_a;
            break;
          case 2:
            paramstr = tests[idx].exp_b;
            break;
          case 3:
            paramstr = tests[idx].exp_g;
            break;
          case 4:
            paramstr = tests[idx].exp_n;
            break;
          case 5:
            paramstr = tests[idx].exp_q;
            break;
          case 6:
            paramstr = tests[idx].exp_d;
            break;
          default:
            die("test %d: internal error: bad param %d", idx, paramidx);
        }

        if (tests[idx].expected_err == GPG_ERR_USER_1 && mpis[paramidx] &&
            !paramstr && paramidx == 0)
          ; /* Okay  Special case error for param 0.  */
        else if (!mpis[paramidx] && !paramstr)
          ; /* Okay.  */
        else if (!mpis[paramidx] && paramstr)
          fail("test %d: value for param %d expected but not returned", idx,
               paramidx);
        else if (mpis[paramidx] && !paramstr)
          fail("test %d: value for param %d not expected", idx, paramidx);
          ...
          fail("test %d: param %d: expected error '%s' - got 'Success'", idx,
               paramidx, gpg_strerror(tests[idx].expected_err));
      }
    }

    for (i = 0; i < DIM(mpis); i++) gcry_mpi_release(mpis[i]);
  ...
  if (err)
    ...
    if (!ioarray[1].data)
      ...
      gcry_log_debug("expected: %s\n", sample1_a);
      gcry_log_debughex("     got", (char *)ioarray[1].data + ioarray[1].off,
                        ioarray[1].len);
    }
    ...
    if (!ioarray[2].data)
      ...
      gcry_log_debug("expected: %s\n", sample1_b);
      gcry_log_debughex("     got", (char *)ioarray[2].data + ioarray[2].off,
                        ioarray[2].len);
    }
  ...
  err = gcry_sexp_extract_param(sxp, "key-data!private-key", "&'curve'+p",
...
static void bug_1594(void) {
   ...
   */
  if (!n_val) fail("extracting 1-th of 'n' list failed");
  /*gcry_log_debugsxp ("1-th", n_val); => "(#00A5...#)"  */
  gcry_sexp_release(n_val);
  n_val = gcry_sexp_nth(n, 2);
  if (n_val) fail("extracting 2-th of 'n' list did not fail");
  n_val = gcry_sexp_nth(n, 0);
  if (!n_val) fail("extracting 0-th of 'n' list failed");
