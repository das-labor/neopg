...
static const char sample_private_dsa_key_1024[] =
    ...
    "  (dsa\n"
    "   (p "
    "#00A126202D592214C5A8F6016E2C3F4256052ACB1CB17D88E64B1293FAF08F5E4685"
    ...
    "   (g "
    "#3958B34AE7747194ECBD312F8FEE8CBE3918E94DF9FD11E2912E56318F33BDC38622"
    ...
    "   (y "
    "#519E9FE9AB0545A6724E74603B7B04E48DC1437E0284A11EA605A7BA8AB1CF354FD4"
...
static const char sample_private_dsa_key_2048[] =
    ...
    "  (dsa\n"
    "   (p "
    "#00B54636673962B64F7DC23C71ACEF6E7331796F607560B194DFCC0CA370E858A365"
    ...
    "   (g "
    "#48E35DA584A089D05142AA63603FDB00D131B07A0781E2D5A8F9614D2B33D3E40A78"
    ...
    "   (y "
    "#66E0D1A69D663466F8FEF2B7C0878DAC93C36A2FB2C05E0306A53B926021D4B92A1C"
...
static const char sample_private_dsa_key_3072[] =
    ...
    "  (dsa\n"
    "   (p "
    "#00BA73E148AEA5E8B64878AF5BE712B8302B9671C5F3EEB7722A9D0D9868D048C938"
    ...
    "   (q "
    "#00BFF3F3CC18FA018A5B8155A8695E1E4939660D5E4759322C39D50F3B93E5F68B#)\n"
    "   (g "
    "#6CCFD8219F5FCE8EF2BEF3262929787140847E38674B1EF8DB20255E212CB6330EC4"
    ...
    "   (y "
    "#2320B22434C5DB832B4EC267CC52E78DD5CCFA911E8F0804E7E7F32B186B2D4167AE"
    ...
    "   (x "
    "#00A9FFFC88E67D6F7B810E291C050BAFEA7FC4A75E8D2F16CFED3416FD77607232#)\n"
    "))\n";
...
static const char sample_public_elg_key_1024[] =
    ...
    "  (elg"
    "   (p "
    "#00F7CC7C08AF096B620C545C9353B1140D698FF8BE2D97A3515C17C7F8DABCDB8FB6"
    ...
    "   (g #06#)"
    "   (y "
    "#36B38FB63E3340A0DD8A0468E9FAA512A32DA010BF7110201D0A3DF1B8FEA0E16F3C"
...
static const char sample_public_elg_key_2048[] =
    ...
    "  (elg"
    "   (p "
    "#00BE5A2BB4E562D7B644E3D01321CB818DBA27295C339FC2C47EAE9823225EE1E7B6"
    ...
    "   (g #07#)"
    "   (y "
    "#5AFF87BC23D8B97AA62897A5C1CDFFA86C59F39EDBD6012B6F333CE23D872009B8C8"
...
static const char sample_private_elg_key_2048[] =
    ...
    "D493E092AEA2371AA904009C8960B0969D12#)"
    "   (x "
    "#0628C3903972C55BDC1BC4223075616D3F3BA57D55532DDB40CB14CF72070E0D28BF"
    "D0402B9088D25ED8FC#)"
    "  ))";
...
static const char sample_public_elg_key_3072[] =
    ...
    "  (elg"
    "   (p "
    "#008EAA3497AFE3706E1A57FFA52E68C64C500731B58EBAFEB51C4A20AB15BA57FA72"
    ...
    "   (g #0B#)"
    "   (y "
    "#18E734FF645AE169079AEAFC78772371089AD3088627ECF77034AFBDF33ADF594AAF"
...
static const char sample_private_elg_key_3072[] =
    ...
    "4DDDEFAFD2C7C66E235D#)"
    "   (x "
    "#03A73F0389E470AAC831B039F8AA0C4EBD3A47DD083E32EEA08E4911236CD597C272"
    "9823D47A51C8535DA52FE6DAB3E8D1C20D#)"
    "  ))";

#define BUG()                                                \
  do {                                                       \
    fprintf(stderr, "Ooops at %s:%d\n", __FILE__, __LINE__); \
    exit(2);                                                 \
  } while (0)
...
static void show_sexp(const char *prefix, gcry_sexp_t a) {
  ...
  buf = malloc(size);
  if (!buf) die("out of core\n");

  gcry_sexp_sprint(a, GCRYSEXP_FMT_ADVANCED, buf, size);
  fprintf(stderr, "%.*s", (int)size, buf);
}

static void progress_cb(void *cb_data, const char *what, int printchar,
  ...
  if (single_char_progress) {
    ...
    fprintf(stderr, PGM ": progress (%s %c %d %d)\n", what, printchar, current,
            total);
    fflush(stderr);
  }
}

static void random_bench(int very_strong) {
  char buf[128];
  int i;
  ...
  putchar('\n');
  if (verbose) xgcry_control(GCRYCTL_DUMP_RANDOM_STATS);
}

static void md_bench(const char *algoname) {
  int algo;
  gcry_md_hd_t hd;
  int i, j, repcount;
  ...
  for (repcount = 0; repcount < hash_repetitions; repcount++)
    for (i = 0; i < 1000; i++) gcry_md_write(hd, buf, bufsize);
  gcry_md_final(hd);
  stop_timer();
  printf(" %s", elapsed_time(1));
  fflush(stdout);
  ...
  start_timer();
  for (repcount = 0; repcount < hash_repetitions; repcount++)
    for (i = 0; i < 10000; i++) gcry_md_write(hd, buf, bufsize / 10);
  gcry_md_final(hd);
  stop_timer();
  printf(" %s", elapsed_time(1));
  fflush(stdout);
  ...
  start_timer();
  for (repcount = 0; repcount < hash_repetitions; repcount++)
    for (i = 0; i < 1000000; i++) gcry_md_write(hd, buf, 1);
  gcry_md_final(hd);
  stop_timer();
  printf(" %s", elapsed_time(1));
  fflush(stdout);
  ...
  for (repcount = 0; repcount < hash_repetitions; repcount++)
    for (i = 0; i < 1000; i++)
      for (j = 0; j < bufsize; j++) gcry_md_putc(hd, buf[j]);
  gcry_md_final(hd);
  stop_timer();
  printf(" %s", elapsed_time(1));
  fflush(stdout);
...
static void mac_bench(const char *algoname) {
  int algo;
  gcry_mac_hd_t hd;
  ...
  for (repcount = 0; repcount < mac_repetitions; repcount++)
    for (i = 0; i < 1000; i++) gcry_mac_write(hd, buf, bufsize);
  macoutlen = maclen;
...
static void cipher_bench(const char *algoname) {
  ...
  static const struct {
    ...
    int doublekey;
      ...
      {GCRY_CIPHER_MODE_XTS, "      XTS", 0, NULL, GCRY_XTS_BLOCK_LEN, 0, 0, 1},
      {GCRY_CIPHER_MODE_CCM, "      CCM", 0, ccm_aead_init, GCRY_CCM_BLOCK_LEN,
       8},
      {GCRY_CIPHER_MODE_GCM, "      GCM", 0, NULL, GCRY_GCM_BLOCK_LEN,
       GCRY_GCM_BLOCK_LEN},
      {GCRY_CIPHER_MODE_OCB, "      OCB", 1, NULL, 16, 16, 15},
  ...
  if (!algoname) {
    for (i = 1; i < 400; i++)
      if (!gcry_cipher_test_algo(i)) cipher_bench(gcry_cipher_algo_name(i));
    return;
  }
  ...
  outbuf =
      (raw_outbuf + ((16 - ((size_t)raw_outbuf & 0x0f)) % buffer_alignment));
  ...
  if (!header_printed) {
    ...
    for (modeidx = 0; modes[modeidx].mode; modeidx++)
      if (*modes[modeidx].name) printf(" %-15s", modes[modeidx].name);
    putchar('\n');
    printf("%-12s", "");
    for (modeidx = 0; modes[modeidx].mode; modeidx++)
      if (*modes[modeidx].name) printf(" ---------------");
    putchar('\n');
    header_printed = 1;
  }
  ...
  for (i = 0; i < keylen * 2; i++) key[i] = i + (clock() & 0xff);
  ...
  for (modeidx = 0; modes[modeidx].mode; modeidx++) {
    ...
    if (modes[modeidx].blocked) buflen = (buflen / blklen) * blklen;
    ...
    for (i = err = 0; !err && i < repetitions; i++) {
      ...
      if (modes[modeidx].aead_init) {
        ...
        if (gpg_err_code(err) == GPG_ERR_CHECKSUM) err = 0;
      } else {
        gcry_cipher_final(hd);
        err = gcry_cipher_decrypt(hd, outbuf, buflen, buf, buflen);
      }
...
static void rsa_bench(int iterations, int print_header, int no_blinding) {
  gpg_error_t err;
  ...
  if (print_header)
    printf(
        "Algorithm         generate %4d*priv %4d*public\n"
        "------------------------------------------------\n",
  ...
  for (testno = 0; testno < DIM(p_sizes); testno++) {
    ...
    err = gcry_sexp_build(&key_spec, NULL,
                          ...
                          nbits);
    if (err) die("creating S-expression failed: %s\n", gcry_strerror(err));
    ...
    gcry_mpi_release(x);
    if (err) die("converting data failed: %s\n", gcry_strerror(err));

    start_timer();
    for (count = 0; count < iterations; count++) {
      gcry_sexp_release(sig);
      err = gcry_pk_sign(&sig, data, sec_key);
      if (err) die("signing failed (%d): %s\n", count, gpg_strerror(err));
    }
    stop_timer();
    printf("   %s", elapsed_time(1));
    fflush(stdout);
...
static void elg_bench(int iterations, int print_header) {
  gpg_error_t err;
...
static void dsa_bench(int iterations, int print_header) {
  gpg_error_t err;
  gcry_sexp_t pub_key[3], sec_key[3];
  int p_sizes[3] = {1024, 2048, 3072};
...
static void ecc_bench(int iterations, int print_header) {
#if USE_ECC
  gpg_error_t err;
  const char *p_sizes[] = {"192", "224",     "256",     "384",
                           "521", "Ed25519", "gost256", "gost512"};
  int testno;
  ...
  for (testno = 0; testno < DIM(p_sizes); testno++) {
    ...
    if (is_ed25519)
      ...
      err = gcry_sexp_build(&key_spec, NULL, "(genkey (ecdsa (curve %s)))",
                            p_size == 256 ? "GOST2001-test" : "GOST2012-test");
    else
      err = gcry_sexp_build(&key_spec, NULL, "(genkey (ECDSA (nbits %d)))",
                            p_size);
    if (err) die("creating S-expression failed: %s\n", gcry_strerror(err));
    ...
    if (verbose > 2) show_sexp("ECC key:\n", key_pair);
...
static void do_powm(const char *n_str, const char *e_str, const char *m_str) {
  ...
  start_timer();
  for (i = 0; i < 1000; i++) gcry_mpi_powm(cip, msg, e, n);
  stop_timer();
  printf(" %s", elapsed_time(1));
  fflush(stdout);
  /*    { */
...
static void mpi_bench(void) {
  printf("%-10s", "powm");
  fflush(stdout);

  do_powm(
      "20A94417D4D5EF2B2DA99165C7DC87DADB3979B72961AF90D09D59BA24CB9A10166FDCCC"
      "9C659F2B9626EC23F3FA425F564A072BA941B03FA81767CC289E4",
      "29",
      "B870187A323F1ECD5B8A0B4249507335A1C4CE8394F38FD76B08C78A42C58F6EA136ACF9"
      "0DFE8603697B1694A3D81114D6117AC1811979C51C4DD013D52F8");
  do_powm(
      "20A94417D4D5EF2B2DA99165C7DC87DADB3979B72961AF90D09D59BA24CB9A10166FDCCC"
      "9C659F2B9626EC23F3FA425F564A072BA941B03FA81767CC289E41071F0246879A442658"
      "FBD18C1771571E7073EEEB2160BA0CBFB3404D627069A6CFBD53867AD2D9D40231648000"
      "787B5C84176B4336144644AE71A403CA40716",
      "29",
      "B870187A323F1ECD5B8A0B4249507335A1C4CE8394F38FD76B08C78A42C58F6EA136ACF9"
      "0DFE8603697B1694A3D81114D6117AC1811979C51C4DD013D52F8FC4EE4BB446B83E48AB"
      "ED7DB81CBF5E81DE4759E8D68AC985846D999F96B0D8A80E5C69D272C766AB8A23B40D50"
      "A4FA889FBC2BD2624222D8EB297F4BAEF8593847");
  do_powm(
      "20A94417D4D5EF2B2DA99165C7DC87DADB3979B72961AF90D09D59BA24CB9A10166FDCCC"
      "9C659F2B9626EC23F3FA425F564A072BA941B03FA81767CC289E41071F0246879A442658"
      "FBD18C1771571E7073EEEB2160BA0CBFB3404D627069A6CFBD53867AD2D9D40231648000"
      "787B5C84176B4336144644AE71A403CA4071620A94417D4D5EF2B2DA99165C7DC87DADB3"
      "979B72961AF90D09D59BA24CB9A10166FDCCC9C659F2B9626EC23F3FA425F564A072BA94"
      "1B03FA81767CC289E41071F0246879A442658FBD18C1771571E7073EEEB2160BA0CBFB34"
      "04D627069A6CFBD53867AD2D9D40231648000787B5C84176B4336144644AE71A403CA407"
      "16",
      "29",
      "B870187A323F1ECD5B8A0B4249507335A1C4CE8394F38FD76B08C78A42C58F6EA136ACF9"
      "0DFE8603697B1694A3D81114D6117AC1811979C51C4DD013D52F8FC4EE4BB446B83E48AB"
      "ED7DB81CBF5E81DE4759E8D68AC985846D999F96B0D8A80E5C69D272C766AB8A23B40D50"
      "A4FA889FBC2BD2624222D8EB297F4BAEF8593847B870187A323F1ECD5B8A0B4249507335"
      "A1C4CE8394F38FD76B08C78A42C58F6EA136ACF90DFE8603697B1694A3D81114D6117AC1"
      "811979C51C4DD013D52F8FC4EE4BB446B83E48ABED7DB81CBF5E81DE4759E8D68AC98584"
      "6D999F96B0D8A80E5C69D272C766AB8A23B40D50A4FA889FBC2BD2624222D8EB297F4BAE"
      "F8593847");

  putchar('\n');
}

static void prime_bench(void) {
  gpg_error_t err;
  int i;
  gcry_mpi_t prime;
  ...
  if (!with_progress) printf("%-10s", "prime");
  fflush(stdout);
  start_timer();
  for (i = 0; i < 10; i++) {
    if (with_progress) fputs("primegen ", stdout);
    err = gcry_prime_generate(&prime, 1024, 0, NULL, NULL, NULL,
                              GCRY_WEAK_RANDOM, GCRY_PRIME_FLAG_SECRET);
    if (with_progress) {
      fputc('\n', stdout);
      fflush(stdout);
    }
...
int main(int argc, char **argv) {
  ...
  if (use_random_daemon) xgcry_control(GCRYCTL_USE_RANDOM_DAEMON, 1);

  if (with_progress) gcry_set_progress_handler(progress_cb, NULL);

  xgcry_control(GCRYCTL_INITIALIZATION_FINISHED, 0);

  if (cipher_repetitions < 1) cipher_repetitions = 1;
  if (hash_repetitions < 1) hash_repetitions = 1;
  if (mac_repetitions < 1) mac_repetitions = 1;
  ...
  if (!argc) {
    ...
    if (argc == 1)
      ...
      for (argc--, argv++; argc; argc--, argv++) md_bench(*argv);
  } else if (!strcmp(*argv, "mac")) {
    if (argc == 1)
      mac_bench(NULL);
    else
      for (argc--, argv++; argc; argc--, argv++) mac_bench(*argv);
  } else if (!strcmp(*argv, "cipher")) {
    if (argc == 1)
      cipher_bench(NULL);
    else
      for (argc--, argv++; argc; argc--, argv++) cipher_bench(*argv);
