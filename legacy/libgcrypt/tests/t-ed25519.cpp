...
char *prepend_srcdir(const char *fname) {
  ...
  if (!srcdir && !(srcdir = getenv("srcdir"))) srcdir = ".";
...
static char *read_textline(FILE *fp, int *lineno) {
  ...
  do {
    ...
    *p = 0;
    for (p--; p > line && my_isascii(*p) && isspace(*p); p--) *p = 0;
  } while (!*line || *line == '#');
  /* if (debug) */
  /*   info ("read line: '%s'\n", line); */
  return xstrdup(line);
...
/* Copy the data after the tag to BUFFER.  BUFFER will be allocated as
   needed.  */
static void copy_data(char **buffer, const char *line, int lineno) {
  const char *s;
...
static void hexdowncase(char *string) {
  char *p;

  for (p = string; *p; p++)
    if (my_isascii(*p)) *p = tolower(*p);
}

static void one_test(int testno, const char *sk, const char *pk,
  ...
  if (!(buffer = hex2buffer(sk, &buflen))) {
    fail("error building s-exp for test %d, %s: %s", testno, "sk",
         "invalid hex string");
    goto leave;
  }
  ...
  if (sign_with_pk)
    err = gcry_sexp_build(&s_sk, NULL,
                          ...
                          "  (q %b)"
                          "  (d %b)))",
                          (int)buflen2, buffer2, (int)buflen, buffer);
  else
    err = gcry_sexp_build(&s_sk, NULL,
                          "(private-key"
                          " (ecc"
  ...
  if (!(buffer = hex2buffer(msg, &buflen))) {
    fail("error building s-exp for test %d, %s: %s", testno, "msg",
         "invalid hex string");
    goto leave;
  }
  ...
  if (!sig_r || !sig_s)
    ...
    p = sig_rs_string;
    *p = 0;
    for (i = 0; i < sig_r_len; i++, p += 2) snprintf(p, 3, "%02x", sig_r[i]);
    for (i = 0; i < sig_s_len; i++, p += 2) snprintf(p, 3, "%02x", sig_s[i]);
    if (strcmp(sig_rs_string, sig)) {
      fail("gcry_pk_sign failed for test %d: %s", testno,
...
static void check_ed25519(const char *fname) {
  FILE *fp;
  int lineno, ntests;
  char *line;
  int testno;
  ...
  while ((line = read_textline(fp, &lineno))) {
    ...
    if (testno && sk && pk && msg && sig) {
      ...
      ntests++;
      if (!(ntests % 256)) show_note("%d of %d tests done\n", ntests, N_TESTS);
      xfree(pk);
      pk = NULL;
      xfree(sk);
      sk = NULL;
...
int main(int argc, char **argv) {
  ...
  if (gcry_fips_mode_active()) return 77;
