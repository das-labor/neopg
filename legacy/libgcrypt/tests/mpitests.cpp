...
/* 48 bytes with value 1: this results in 8 limbs for 64bit limbs, 16limb for 32
 * bit limbs */
unsigned char ones[] = {
...
/* 48 bytes with value 2: this results in 8 limbs for 64bit limbs, 16limb for 32
 * bit limbs */
unsigned char twos[] = {
...
/* 48 bytes with value 3: this results in 8 limbs for 64bit limbs, 16limb for 32
 * bit limbs */
...
/* 48 bytes with value 0x80: this results in 8 limbs for 64bit limbs, 16limb for
 * 32 bit limbs */
...
/* 48 bytes with value 0xff: this results in 8 limbs for 64bit limbs, 16limb for
 * 32 bit limbs */
...
static int test_const_and_immutable(void) {
...
static void test_opaque(void) {
  ...
  a = gcry_mpi_set_opaque(NULL, p,
                          21 * 8 + 1); /* (a non byte aligned length) */

  if (!gcry_mpi_get_flag(a, GCRYMPI_FLAG_OPAQUE)) die("opaque flag not set\n");

  p = gcry_mpi_get_opaque(a, &nbits);
  if (!p) die("gcry_mpi_get_opaque returned NULL\n");
  if (nbits != 21 * 8 + 1)
  ...
  if (debug) gcry_log_debugmpi("mpi", a);
  gcry_mpi_release(a);
...
static void test_cmp(void) {
  ...
  if (rc) die("scanning number failed at line %d", __LINE__);
  ...
  if (gcry_mpi_cmp_ui(zero, 0)) fail("mpi_cmp_ui failed at line %d", __LINE__);
  if (!(gcry_mpi_cmp_ui(zero, 1) < 0))
    fail("mpi_cmp_ui failed at line %d", __LINE__);
  if (!(gcry_mpi_cmp_ui(zero, (-1)) < 0))
    fail("mpi_cmp_ui failed at line %d", __LINE__);

  if (gcry_mpi_cmp_ui(two, 2)) fail("mpi_cmp_ui failed at line %d", __LINE__);
  ...
  if (gcry_mpi_cmp(zero, zero2)) fail("mpi_cmp failed at line %d", __LINE__);
...
static int test_add(void) {
  ...
  if (debug) gcry_log_debug("Result of one plus two:\n%s\n", pc);
  gcry_free(pc);

  gcry_mpi_add(result, ff, one);
  gcry_mpi_aprint(GCRYMPI_FMT_HEX, &pc, NULL, result);
  if (debug) gcry_log_debug("Result of ff plus one:\n%s\n", pc);
  gcry_free(pc);
...
static int test_sub(void) {
  ...
  gcry_mpi_aprint(GCRYMPI_FMT_HEX, &pc, NULL, result);
  if (debug) gcry_log_debug("Result of two minus one:\n%s\n", pc);
  gcry_free(pc);
...
static int test_mul(void) {
  ...
  gcry_mpi_aprint(GCRYMPI_FMT_HEX, &pc, NULL, result);
  if (debug) gcry_log_debug("Result of two mul three:\n%s\n", pc);
  gcry_free(pc);
...
static int test_powm(void) {
  ...
  if (gcry_mpi_cmp(res, base)) die("test_powm failed at %d\n", __LINE__);
  if (gcry_mpi_cmp_ui(exp, e_int))
    die("test_powm_ui failed for exp at %d\n", __LINE__);
  if (gcry_mpi_cmp_ui(mod, m_int))
    die("test_powm failed for mod at %d\n", __LINE__);
  ...
  if (gcry_mpi_cmp(res, exp)) die("test_powm failed at %d\n", __LINE__);
  if (gcry_mpi_cmp_ui(base, b_int))
    die("test_powm failed for base at %d\n", __LINE__);
  if (gcry_mpi_cmp_ui(mod, m_int))
    die("test_powm failed for mod at %d\n", __LINE__);
  ...
  if (gcry_mpi_cmp(res, mod)) die("test_powm failed at %d\n", __LINE__);
  if (gcry_mpi_cmp_ui(base, b_int))
    die("test_powm failed for base at %d\n", __LINE__);
  if (gcry_mpi_cmp_ui(exp, e_int))
    die("test_powm_ui failed for exp at %d\n", __LINE__);
