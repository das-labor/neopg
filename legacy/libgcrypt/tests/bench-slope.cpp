...
static void get_nsec_time(struct nsec_time *t) {
  BOOL ok;
...
static double get_time_nsec_diff(struct nsec_time *start,
                                 struct nsec_time *end) {
...
double get_slope(double (*const get_x)(unsigned int idx, void *priv),
  ...
  if (overhead) *overhead = a; /* nsecs */

  return b; /* nsecs per byte */
}

double get_bench_obj_point_x(unsigned int idx, void *priv) {
  struct bench_obj *obj = priv;
  return (double)(obj->min_bufsize + (idx * obj->step_size));
}

unsigned int get_num_measurements(struct bench_obj *obj) {
  unsigned int buf_range = obj->max_bufsize - obj->min_bufsize;
  unsigned int num = buf_range / obj->step_size + 1;

  while (obj->min_bufsize + (num * obj->step_size) > obj->max_bufsize) num--;
...
double do_bench_obj_measurement(struct bench_obj *obj, void *buffer,
                                size_t buflen, double *measurement_raw,
  ...
  if (num_repetitions < 1 || loop_iterations < 1) return 0.0;

  for (rep = 0; rep < num_repetitions; rep++) {
    get_nsec_time(&start);

    for (loop = 0; loop < loop_iterations; loop++) do_run(obj, buffer, buflen);
  ...
  if (num_repetitions % 2 == 1) return measurement_raw[num_repetitions / 2];
...
unsigned int adjust_loop_iterations_to_timer_accuracy(struct bench_obj *obj,
                                                      void *buffer,
  ...
  if (test_bufsize == 0) test_bufsize += obj->step_size;

  loop_iterations = 0;
  do {
    /* Increase loop iterations until we get other results than zero.  */
    nsecs = do_bench_obj_measurement(obj, buffer, test_bufsize, measurement_raw,
                                     ++loop_iterations);
  } while (nsecs < 1.0 - 0.1);
  do {
    /* Increase loop iterations until we get reasonable increase for elapsed
     * time.  */
    tmp = do_bench_obj_measurement(obj, buffer, test_bufsize, measurement_raw,
                                   ++loop_iterations);
...
double do_slope_benchmark(struct bench_obj *obj) {
  ...
  if (!measurements) goto err_free;

  measurement_raw =
      calloc(obj->num_measure_repetitions, sizeof(*measurement_raw));
  if (!measurement_raw) goto err_free;
  ...
  if (!real_buffer) goto err_free;
  /* Get aligned buffer */
  buffer = real_buffer;
  buffer += 128 - ((real_buffer - (unsigned char *)0) & (128 - 1));
  if (unaligned_mode) buffer += unaligned_mode; /* Make buffer unaligned */

  for (i = 0; i < obj->max_bufsize; i++) buffer[i] = 0x55 ^ (-i);

  /* Adjust number of loop iterations up to timer accuracy.  */
  loop_iterations =
      adjust_loop_iterations_to_timer_accuracy(obj, buffer, measurement_raw);

  /* Perform measurements */
  for (midx = 0, cur_bufsize = obj->min_bufsize;
       cur_bufsize <= obj->max_bufsize; cur_bufsize += obj->step_size, midx++) {
    measurements[midx] = do_bench_obj_measurement(
        obj, buffer, cur_bufsize, measurement_raw, loop_iterations);
    measurements[midx] /= loop_iterations;
  }

  assert(midx == num_measurements);

  slope = get_slope(&get_bench_obj_point_x, obj, measurements, num_measurements,
                    &overhead);
...
err_free:
  if (measurement_raw) free(measurement_raw);
...
static void double_to_str(char *out, size_t outlen, double value) {
  const char *fmt;
...
static void bench_print_result_csv(double nsecs_per_byte) {
  ...
  *cpbyte_buf = 0;
  ...
  mbytes_per_sec = (1000.0 * 1000.0 * 1000.0) / (nsecs_per_byte * 1024 * 1024);
  ...
  printf("%s,%s,%s,,,%s,ns/B,%s,MiB/s,%s,c/B\n", current_section_name,
         ...
         current_mode_name ? current_mode_name : "", nsecpbyte_buf, mbpsec_buf,
         cpbyte_buf);
}

static void bench_print_result_std(double nsecs_per_byte) {
  double cycles_per_byte, mbytes_per_sec;
  char nsecpbyte_buf[16];
  char mbpsec_buf[16];
  char cpbyte_buf[16];
...
static void bench_print_section(const char *section_name,
...
static void bench_print_header(int algo_width, const char *algo_name) {
  if (csv_mode) {
...
static void bench_print_algo(int algo_width, const char *algo_name) {
  if (csv_mode) {
    gcry_free(current_algo_name);
    current_algo_name = gcry_xstrdup(algo_name);
  } else {
...
static void bench_print_mode(int width, const char *mode_name) {
  if (csv_mode) {
...
static void bench_print_footer(int algo_width) {
  if (!csv_mode) printf(" %-*s =\n", algo_width, "");
}
...
static int bench_encrypt_init(struct bench_obj *obj) {
  ...
  if (keylen) {
    ...
    for (i = 0; i < keylen; i++) key[i] = 0x33 ^ (11 - i);
...
static void bench_encrypt_free(struct bench_obj *obj) {
  gcry_cipher_hd_t hd = obj->priv;

  gcry_cipher_close(hd);
}

static void bench_encrypt_do_bench(struct bench_obj *obj, void *buf,
                                   size_t buflen) {
  gcry_cipher_hd_t hd = obj->priv;
  int err;
...
static void bench_decrypt_do_bench(struct bench_obj *obj, void *buf,
                                   size_t buflen) {
  gcry_cipher_hd_t hd = obj->priv;
  int err;
...
static struct bench_ops encrypt_ops = {&bench_encrypt_init, &bench_encrypt_free,
                                       &bench_encrypt_do_bench};

static struct bench_ops decrypt_ops = {&bench_encrypt_init, &bench_encrypt_free,
                                       &bench_decrypt_do_bench};

static int bench_xts_encrypt_init(struct bench_obj *obj) {
  struct bench_cipher_mode *mode = obj->priv;
  gcry_cipher_hd_t hd;
  int err, keylen;
...
static void bench_xts_encrypt_do_bench(struct bench_obj *obj, void *buf,
                                       size_t buflen) {
  gcry_cipher_hd_t hd = obj->priv;
  unsigned int pos;
  static const char tweak[16] = {
      0xff, 0xff, 0xfe,
  };
...
static void bench_xts_decrypt_do_bench(struct bench_obj *obj, void *buf,
                                       size_t buflen) {
  gcry_cipher_hd_t hd = obj->priv;
  unsigned int pos;
  static const char tweak[16] = {
...
static struct bench_ops xts_encrypt_ops = {
    &bench_xts_encrypt_init, &bench_encrypt_free, &bench_xts_encrypt_do_bench};

static struct bench_ops xts_decrypt_ops = {
    &bench_xts_encrypt_init, &bench_encrypt_free, &bench_xts_decrypt_do_bench};

static void bench_ccm_encrypt_do_bench(struct bench_obj *obj, void *buf,
                                       size_t buflen) {
  gcry_cipher_hd_t hd = obj->priv;
  int err;
  char tag[8];
...
static void bench_ccm_decrypt_do_bench(struct bench_obj *obj, void *buf,
                                       size_t buflen) {
  gcry_cipher_hd_t hd = obj->priv;
  int err;
  char tag[8] = {
  ...
  if (gpg_err_code(err) == GPG_ERR_CHECKSUM) err = gpg_error(GPG_ERR_NO_ERROR);
  if (err) {
    fprintf(stderr, PGM ": gcry_cipher_gettag failed: %s\n", gpg_strerror(err));
    gcry_cipher_close(hd);
    exit(1);
...
static void bench_ccm_authenticate_do_bench(struct bench_obj *obj, void *buf,
                                            size_t buflen) {
  gcry_cipher_hd_t hd = obj->priv;
  int err;
  char tag[8] = {
...
static struct bench_ops ccm_encrypt_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_ccm_encrypt_do_bench};

static struct bench_ops ccm_decrypt_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_ccm_decrypt_do_bench};

static struct bench_ops ccm_authenticate_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_ccm_authenticate_do_bench};

static void bench_aead_encrypt_do_bench(struct bench_obj *obj, void *buf,
...
static void bench_aead_decrypt_do_bench(struct bench_obj *obj, void *buf,
                                        size_t buflen, const char *nonce,
                                        size_t noncelen) {
  gcry_cipher_hd_t hd = obj->priv;
  int err;
...
static void bench_aead_authenticate_do_bench(struct bench_obj *obj, void *buf,
                                             size_t buflen, const char *nonce,
                                             size_t noncelen) {
  gcry_cipher_hd_t hd = obj->priv;
  int err;
...
static void bench_gcm_encrypt_do_bench(struct bench_obj *obj, void *buf,
                                       size_t buflen) {
...
static void bench_gcm_decrypt_do_bench(struct bench_obj *obj, void *buf,
                                       size_t buflen) {
  char nonce[12] = {0xca, 0xfe, 0xba, 0xbe, 0xfa, 0xce,
                    0xdb, 0xad, 0xde, 0xca, 0xf8, 0x88};
...
static void bench_gcm_authenticate_do_bench(struct bench_obj *obj, void *buf,
                                            size_t buflen) {
  char nonce[12] = {0xca, 0xfe, 0xba, 0xbe, 0xfa, 0xce,
                    0xdb, 0xad, 0xde, 0xca, 0xf8, 0x88};
...
static struct bench_ops gcm_encrypt_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_gcm_encrypt_do_bench};

static struct bench_ops gcm_decrypt_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_gcm_decrypt_do_bench};

static struct bench_ops gcm_authenticate_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_gcm_authenticate_do_bench};

static void bench_ocb_encrypt_do_bench(struct bench_obj *obj, void *buf,
                                       size_t buflen) {
  char nonce[15] = {0xca, 0xfe, 0xba, 0xbe, 0xfa, 0xce, 0xdb, 0xad,
                    0xde, 0xca, 0xf8, 0x88, 0x00, 0x00, 0x01};
  bench_aead_encrypt_do_bench(obj, buf, buflen, nonce, sizeof(nonce));
}

static void bench_ocb_decrypt_do_bench(struct bench_obj *obj, void *buf,
                                       size_t buflen) {
  char nonce[15] = {0xca, 0xfe, 0xba, 0xbe, 0xfa, 0xce, 0xdb, 0xad,
                    0xde, 0xca, 0xf8, 0x88, 0x00, 0x00, 0x01};
  bench_aead_decrypt_do_bench(obj, buf, buflen, nonce, sizeof(nonce));
...
static void bench_ocb_authenticate_do_bench(struct bench_obj *obj, void *buf,
                                            size_t buflen) {
  char nonce[15] = {0xca, 0xfe, 0xba, 0xbe, 0xfa, 0xce, 0xdb, 0xad,
                    0xde, 0xca, 0xf8, 0x88, 0x00, 0x00, 0x01};
  bench_aead_authenticate_do_bench(obj, buf, buflen, nonce, sizeof(nonce));
...
static struct bench_ops ocb_encrypt_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_ocb_encrypt_do_bench};

static struct bench_ops ocb_decrypt_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_ocb_decrypt_do_bench};

static struct bench_ops ocb_authenticate_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_ocb_authenticate_do_bench};

static void bench_poly1305_encrypt_do_bench(struct bench_obj *obj, void *buf,
                                            size_t buflen) {
  char nonce[8] = {0xca, 0xfe, 0xba, 0xbe, 0xfa, 0xce, 0xdb, 0xad};
  bench_aead_encrypt_do_bench(obj, buf, buflen, nonce, sizeof(nonce));
}

static void bench_poly1305_decrypt_do_bench(struct bench_obj *obj, void *buf,
                                            size_t buflen) {
  char nonce[8] = {0xca, 0xfe, 0xba, 0xbe, 0xfa, 0xce, 0xdb, 0xad};
  bench_aead_decrypt_do_bench(obj, buf, buflen, nonce, sizeof(nonce));
}

static void bench_poly1305_authenticate_do_bench(struct bench_obj *obj,
...
static struct bench_ops poly1305_encrypt_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_poly1305_encrypt_do_bench};

static struct bench_ops poly1305_decrypt_ops = {
    &bench_encrypt_init, &bench_encrypt_free, &bench_poly1305_decrypt_do_bench};

static struct bench_ops poly1305_authenticate_ops = {
    &bench_encrypt_init, &bench_encrypt_free,
...
static void cipher_bench_one(int algo, struct bench_cipher_mode *pmode) {
  ...
  if (mode.mode == GCRY_CIPHER_MODE_CCM && blklen != GCRY_CCM_BLOCK_LEN) return;

  /* GCM has restrictions for block-size */
  if (mode.mode == GCRY_CIPHER_MODE_GCM && blklen != GCRY_GCM_BLOCK_LEN) return;

  /* XTS has restrictions for block-size */
  if (mode.mode == GCRY_CIPHER_MODE_XTS && blklen != GCRY_XTS_BLOCK_LEN) return;

  /* Our OCB implementation has restrictions for block-size.  */
  if (mode.mode == GCRY_CIPHER_MODE_OCB && blklen != GCRY_OCB_BLOCK_LEN) return;
...
static void _cipher_bench(int algo) {
  const char *algoname;
  int i;
...
void cipher_bench(char **argv, int argc) {
  int i, algo;
  ...
  if (argv && argc) {
    for (i = 0; i < argc; i++) {
      ...
      if (algo) _cipher_bench(algo);
    }
  } else {
    for (i = 1; i < 400; i++)
      if (!gcry_cipher_test_algo(i)) _cipher_bench(i);
  }
}
...
static int bench_hash_init(struct bench_obj *obj) {
...
static void bench_hash_free(struct bench_obj *obj) {
  gcry_md_hd_t hd = obj->priv;

  gcry_md_close(hd);
}

static void bench_hash_do_bench(struct bench_obj *obj, void *buf,
                                size_t buflen) {
  gcry_md_hd_t hd = obj->priv;
...
static struct bench_ops hash_ops = {&bench_hash_init, &bench_hash_free,
                                    &bench_hash_do_bench};
...
static void hash_bench_one(int algo, struct bench_hash_mode *pmode) {
...
static void _hash_bench(int algo) {
  int i;

  for (i = 0; hash_modes[i].name; i++) hash_bench_one(algo, &hash_modes[i]);
}

void hash_bench(char **argv, int argc) {
  int i, algo;
  ...
  if (argv && argc) {
    for (i = 0; i < argc; i++) {
      ...
      if (algo) _hash_bench(algo);
    }
  } else {
    for (i = 1; i < 400; i++)
      if (!gcry_md_test_algo(i)) _hash_bench(i);
  }
...
static int bench_mac_init(struct bench_obj *obj) {
...
static void bench_mac_free(struct bench_obj *obj) {
  gcry_mac_hd_t hd = obj->priv;

  gcry_mac_close(hd);
}

static void bench_mac_do_bench(struct bench_obj *obj, void *buf,
                               size_t buflen) {
  gcry_mac_hd_t hd = obj->priv;
  size_t bs;
  char b;
...
static struct bench_ops mac_ops = {&bench_mac_init, &bench_mac_free,
                                   &bench_mac_do_bench};
...
static void mac_bench_one(int algo, struct bench_mac_mode *pmode) {
...
static void _mac_bench(int algo) {
  int i;

  for (i = 0; mac_modes[i].name; i++) mac_bench_one(algo, &mac_modes[i]);
}

void mac_bench(char **argv, int argc) {
  int i, algo;
  ...
  if (argv && argc) {
    for (i = 0; i < argc; i++) {
      ...
      if (algo) _mac_bench(algo);
    }
  } else {
    for (i = 1; i < 600; i++)
      if (!gcry_mac_test_algo(i)) _mac_bench(i);
  }
...
static int bench_kdf_init(struct bench_obj *obj) {
...
static void bench_kdf_free(struct bench_obj *obj) { (void)obj; }

static void bench_kdf_do_bench(struct bench_obj *obj, void *buf,
                               size_t buflen) {
  struct bench_kdf_mode *mode = obj->priv;
  char keybuf[16];
...
static struct bench_ops kdf_ops = {&bench_kdf_init, &bench_kdf_free,
                                   &bench_kdf_do_bench};

static void kdf_bench_one(int algo, int subalgo) {
  ...
  *algo_name = 0;
  ...
  if (csv_mode) {
    printf("%s,%s,%s,,,,,,,,,%s,ns/iter,%s,c/iter\n", current_section_name,
           current_algo_name ? current_algo_name : "",
           current_mode_name ? current_mode_name : "", nsecpiter_buf,
           cpiter_buf);
  } else {
...
void kdf_bench(char **argv, int argc) {
  char algo_name[32];
  int i, j;
  ...
  if (argv && argc) {
    for (i = 0; i < argc; i++) {
      for (j = 1; j < 400; j++) {
        if (gcry_md_test_algo(j)) continue;

        snprintf(algo_name, sizeof(algo_name), "PBKDF2-HMAC-%s",
                 gcry_md_algo_name(j));

        if (!strcmp(argv[i], algo_name)) kdf_bench_one(GCRY_KDF_PBKDF2, j);
      }
    }
  } else {
    for (i = 1; i < 400; i++)
      if (!gcry_md_test_algo(i)) kdf_bench_one(GCRY_KDF_PBKDF2, i);
  }
...
void print_help(void) {
  static const char *help_lines[] = {
      ...
      "                             for benchmarking.",
      "   --repetitions <n>         Use N repetitions (default " STR2(
  ...
  for (line = help_lines; *line; line++) fprintf(stdout, "%s\n", *line);
}
...
int main(int argc, char **argv) {
  ...
  if (!argc && getenv("srcdir") && getenv("GCRYPT_NO_BENCHMARKS")) exit(77);
    ...
    if (!strcmp(*argv, "--")) {
      ...
      if (argc) {
        ...
        if (num_measurement_repetitions < 2) {
          fprintf(stderr,
                  PGM ": value for --repetitions too small - using %d\n",
                  NUM_MEASUREMENT_REPETITIONS);
          num_measurement_repetitions = NUM_MEASUREMENT_REPETITIONS;
        }
        argc--;
