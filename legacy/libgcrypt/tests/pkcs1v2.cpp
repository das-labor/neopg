...
static int extract_cmp_data(gcry_sexp_t sexp, const char *name,
...
#include "pkcs1v2-oaep.h"
  ...
  for (tno = 0; tno < DIM(tbl); tno++) {
    ...
    err = gcry_sexp_build(
        &sec_key, NULL, "(private-key (rsa (n %b)(e %b)(d %b)))",
        (int)rsa_n_len, rsa_n, (int)rsa_e_len, rsa_e, (int)rsa_d_len, rsa_d);
    if (err) die("constructing private key failed: %s\n", gpg_strerror(err));
    err = gcry_sexp_build(&pub_key, NULL, "(public-key (rsa (n %b)(e %b)))",
                          (int)rsa_n_len, rsa_n, (int)rsa_e_len, rsa_e);
    if (err) die("constructing public key failed: %s\n", gpg_strerror(err));
    gcry_free(rsa_n);
    gcry_free(rsa_e);
    gcry_free(rsa_d);
    ...
    for (mno = 0; mno < DIM(tbl[0].m); mno++) {
      ...
      if (verbose) info("running test: %s\n", tbl[tno].m[mno].desc);
      ...
      if (err) die("constructing plain data failed: %s\n", gpg_strerror(err));
      gcry_free(mesg);
      gcry_free(seed);
      ...
      if (err) die("constructing cipher data failed: %s\n", gpg_strerror(err));
      gcry_free(encr);
      gcry_free(seed);
...
#include "pkcs1v2-pss.h"
  ...
  for (tno = 0; tno < DIM(tbl); tno++) {
    ...
    for (mno = 0; mno < DIM(tbl[0].m); mno++) {
      ...
      if (err) die("constructing sig template failed: %s\n", gpg_strerror(err));
      gcry_free(mesg);
      gcry_free(salt);
      ...
      err = gcry_sexp_build(&sig, NULL, "(sig-val(rsa(s %b)))", (int)sign_len,
                            sign);
      if (err) die("constructing verify data failed: %s\n", gpg_strerror(err));
      err = gcry_sexp_build(&sigtmpl, NULL,
                            "(data (flags pss)"
                            "(hash sha1 %b)"
                            "(random-override %b))",
                            20, mhash, (int)salt_len, salt);
      if (err) die("constructing verify tmpl failed: %s\n", gpg_strerror(err));
      gcry_free(sign);
      gcry_free(salt);
...
static void check_v15crypt(void) {
...
static void check_v15sign(void) {
...
int main(int argc, char **argv) {
  ...
  if (!gcry_check_version("1.5.0")) die("version mismatch\n");
  xgcry_control(GCRYCTL_INITIALIZATION_FINISHED, 0);
  if (debug) xgcry_control(GCRYCTL_SET_DEBUG_FLAGS, 1u, 0);
  /* No valuable keys are create, so we can speed up our RNG. */
  xgcry_control(GCRYCTL_ENABLE_QUICK_RANDOM, 0);

  if (run_oaep) check_oaep();
  if (run_pss) check_pss();
  if (run_v15c) check_v15crypt();
  if (run_v15s) check_v15sign();
