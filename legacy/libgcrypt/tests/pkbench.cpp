...
static void *read_file(const char *fname, size_t *r_length) {
  FILE *fp;
  struct stat st;
  char *buf;
  size_t buflen;
  ...
  if (r_length) *r_length = buflen;
  return buf;
}

static void benchmark(work_t worker, context_t context) {
...
static int work_encrypt(context_t context, unsigned int final) {
...
static int work_decrypt(context_t context, unsigned int final) {
  gcry_error_t err = GPG_ERR_NO_ERROR;
  int ret = 1;
  ...
  if (!context->data_encrypted)
    ...
    err = gcry_pk_decrypt(&data_decrypted, context->data_encrypted,
                          context->key_secret);
    assert(!err);
    if (final) {
...
static int work_sign(context_t context, unsigned int final) {
  gcry_error_t err = GPG_ERR_NO_ERROR;
...
static int work_verify(context_t context, unsigned int final) {
  gcry_error_t err = GPG_ERR_NO_ERROR;
  int ret = 1;

  if (!context->data_signed) return 0;

  err =
      gcry_pk_verify(context->data_signed, context->data, context->key_public);
  if (err) {
...
static void process_key_pair(context_t context) {
  struct {
    work_t worker;
...
static void context_init(context_t context, gcry_sexp_t key_secret,
                         gcry_sexp_t key_public) {
  gcry_error_t err = GPG_ERR_NO_ERROR;
  ...
  err =
      gcry_sexp_build(&data_sexp, NULL, "(data (flags raw) (value %m))", data);
  assert(!err);
...
static void process_key_pair_file(const char *key_pair_file) {
  gcry_error_t err = GPG_ERR_NO_ERROR;
  ...
  if (!key_pair_buffer) die("failed to open `%s'\n", key_pair_file);

  err = gcry_sexp_sscan(&key_pair_sexp, NULL, key_pair_buffer, file_length);
  if (err) die("gcry_sexp_sscan failed\n");
...
static void generate_key(const char *algorithm, const char *key_size) {
  ...
  if (isdigit((unsigned int)*key_size))
    err = gcry_sexp_build(&key_spec, NULL, "(genkey (%s (nbits %s)))",
                          algorithm, key_size);
  else
    err = gcry_sexp_build(&key_spec, NULL, "(genkey (%s (curve %s)))",
                          algorithm, key_size);
  if (err) die("sexp_build failed: %s\n", gpg_strerror(err));
  ...
  key_pair_buffer_size =
      gcry_sexp_sprint(key_pair, GCRYSEXP_FMT_ADVANCED, NULL, 0);
  key_pair_buffer = gcry_xmalloc(key_pair_buffer_size);

  gcry_sexp_sprint(key_pair, GCRYSEXP_FMT_ADVANCED, key_pair_buffer,
                   key_pair_buffer_size);
...
int main(int argc, char **argv) {
  ...
  if (fips_mode) xgcry_control(GCRYCTL_FORCE_FIPS_MODE, 0);
  ...
  if (genkey_mode && argc == 2) {
    ...
    for (i = 0; i < argc; i++) process_key_pair_file(argv[i]);
  } else {
