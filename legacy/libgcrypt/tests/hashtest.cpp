...
static void showhex(const void *buffer, size_t buflen, const char *format,
                    ...) {
  va_list arg_ptr;
...
static void run_selftest(int algo) {
  gpg_error_t err;
  size_t n;
  ...
  if (err && gpg_err_code(err) != GPG_ERR_NOT_IMPLEMENTED)
    fail("extended selftest for %s (%d) failed: %s", gcry_md_algo_name(algo),
         algo, gpg_strerror(err));
  else if (err && verbose)
    info("extended selftest for %s (%d) not implemented",
         gcry_md_algo_name(algo), algo);
  else if (verbose)
    info("extended selftest for %s (%d) passed", gcry_md_algo_name(algo), algo);
}
...
static int cmp_digest(const unsigned char *digest, size_t digestlen, int algo,
                      int gigs, int bytes) {
  int idx;
  ...
  for (idx = 0; testvectors[idx].algo; idx++) {
    if (testvectors[idx].algo == algo && testvectors[idx].gigs == gigs &&
        testvectors[idx].bytes == bytes)
      break;
  }
  if (!testvectors[idx].algo) {
    info("%d GiB %+3d %-10s warning: %s", gigs, bytes, gcry_md_algo_name(algo),
         "no test vector");
  ...
  {
    fail("%d GiB %+3d %-10s error: %s", gigs, bytes, gcry_md_algo_name(algo),
...
static void run_longtest(int algo, int gigs) {
  gpg_error_t err;
  gcry_md_hd_t hd;
  ...
  if (err) {
    fail("gcry_md_open failed for %s (%d): %s", gcry_md_algo_name(algo), algo,
         gpg_strerror(err));
    return;
  }
  ...
  for (g = 0; g < gigs; g++) {
    if (g == gigs - 1) {
      ...
      for (i = 0; i < 1023; i++) gcry_md_write(hd, pattern, sizeof pattern);
      err = gcry_md_copy(&hd_pre, hd);
      if (!err) err = gcry_md_copy(&hd_pre2, hd);
      if (err)
        die("gcry_md_copy failed for %s (%d): %s", gcry_md_algo_name(algo),
            algo, gpg_strerror(err));
      gcry_md_write(hd, pattern, sizeof pattern);
    } else {
  ...
  if (g >= 16) show_note("%d GiB hashed with %s", g, gcry_md_algo_name(algo));

  err = gcry_md_copy(&hd_post, hd);
  if (err)
    die("gcry_md_copy failed for %s (%d): %s", gcry_md_algo_name(algo), algo,
        gpg_strerror(err));
...
int main(int argc, char **argv) {
  ...
  for (idx = 0; idx < argc; idx++) {
    ...
    if (!algo) fail("invalid algorithm '%s'", argv[idx]);
  }
  ...
  if (!argc) {
    ...
    for (idx = 0; idx < argc; idx++) {
      algo = gcry_md_map_name(argv[idx]);
      if (!algo) die("invalid algorithm '%s'", argv[idx]);
  ...
  if (missing_test_vectors) fail("Some test vectors are missing");
