...
static void show_mpi(const char *prefix, gcry_mpi_t a) {
  char *buf;
  void *bufaddr = &buf;
  gcry_error_t rc;
...
static void check_generated_rsa_key(gcry_sexp_t key, unsigned long expected_e) {
  ...
  if (!pkey)
    ...
    if (!list || !(e = gcry_sexp_nth_mpi(list, 1, 0)))
      ...
      if (verbose) show_mpi("public exponent: ", e);
  ...
  if (!skey)
    ...
    if (rc) fail("gcry_pk_testkey failed: %s\n", gpg_strerror(rc));
...
static void check_rsa_keys(void) {
  ...
  if (verbose) info("creating 2048 bit RSA key\n");
  ...
  if (verbose) info("creating 1024 bit RSA key\n");
  rc = gcry_sexp_new(&keyparm,
                     "(genkey\n"
                     " (rsa\n"
                     "  (nbits 4:1024)\n"
  ...
  if (!rc) {
    if (verbose > 1) show_sexp("1024 bit RSA key:\n", key);
    check_generated_rsa_key(key, 65537);
  }
  gcry_sexp_release(key);

  if (verbose) info("creating 2048 bit RSA key with e=65539\n");
  rc = gcry_sexp_new(&keyparm,
                     "(genkey\n"
                     " (rsa\n"
                     "  (nbits 4:2048)\n"
  ...
  rc = gcry_pk_genkey(&key, keyparm);
  gcry_sexp_release(keyparm);
  if (rc) fail("error generating RSA key: %s\n", gpg_strerror(rc));

  if (!rc) check_generated_rsa_key(key, 65539);
  gcry_sexp_release(key);

  if (verbose) info("creating 512 bit RSA key with e=257\n");
  rc = gcry_sexp_new(&keyparm,
                     "(genkey\n"
                     " (rsa\n"
                     "  (nbits 3:512)\n"
  ...
  if (!rc) check_generated_rsa_key(key, 257);
  gcry_sexp_release(key);

  if (verbose) info("creating 512 bit RSA key with default e\n");
  rc = gcry_sexp_new(&keyparm,
                     "(genkey\n"
                     " (rsa\n"
                     "  (nbits 3:512)\n"
...
static void check_elg_keys(void) {
  gcry_sexp_t keyparm, key;
  int rc;

  if (verbose) info("creating 1024 bit Elgamal key\n");
  rc = gcry_sexp_new(&keyparm,
                     "(genkey\n"
                     " (elg\n"
                     "  (nbits 4:1024)\n"
  ...
  if (verbose > 1) show_sexp("1024 bit Elgamal key:\n", key);
  gcry_sexp_release(key);
}

static void check_dsa_keys(void) {
  gcry_sexp_t keyparm, key;
  int rc;
  int i;
  ...
  /* Check that DSA generation works and that it can grok the qbits
     argument. */
  if (verbose) info("creating 5 1024 bit DSA keys\n");
  for (i = 0; i < 5; i++) {
    rc = gcry_sexp_new(&keyparm,
                       "(genkey\n"
                       " (dsa\n"
    ...
    if (!i && verbose > 1) show_sexp("1024 bit DSA key:\n", key);
    gcry_sexp_release(key);
  }

  if (verbose) info("creating 1536 bit DSA key\n");
  rc = gcry_sexp_new(&keyparm,
                     "(genkey\n"
                     " (dsa\n"
                     "  (nbits 4:1536)\n"
  ...
  if (verbose > 1) show_sexp("1536 bit DSA key:\n", key);
  gcry_sexp_release(key);

  if (verbose) info("creating 3072 bit DSA key\n");
  rc = gcry_sexp_new(&keyparm,
                     "(genkey\n"
                     " (dsa\n"
                     "  (nbits 4:3072)\n"
  ...
  if (verbose > 1) show_sexp("3072 bit DSA key:\n", key);
  gcry_sexp_release(key);

  if (verbose) info("creating 2048/256 bit DSA key\n");
  rc = gcry_sexp_new(&keyparm,
                     "(genkey\n"
                     " (dsa\n"
                     "  (nbits 4:2048)\n"
  ...
  gcry_sexp_release(keyparm);
  if (rc) die("error generating DSA key: %s\n", gpg_strerror(rc));
  if (verbose > 1) show_sexp("2048 bit DSA key:\n", key);
  gcry_sexp_release(key);

  if (verbose) info("creating 2048/224 bit DSA key\n");
  rc = gcry_sexp_new(&keyparm,
                     "(genkey\n"
                     " (dsa\n"
                     "  (nbits 4:2048)\n"
...
static void check_generated_ecc_key(gcry_sexp_t key) {
     ...
     S-expression.  */
  {
    ...
    if (rc) fail("gcry_pk_testkey failed on key pair: %s\n", gpg_strerror(rc));
  }
}

static void check_ecc_keys(void) {
  const char *curves[] = {"NIST P-521", "NIST P-384", "NIST P-256", "Ed25519",
                          NULL};
  int testno;
  gcry_sexp_t keyparm, key;
  int rc;
  ...
  for (testno = 0; curves[testno]; testno++) {
    if (verbose) info("creating ECC key using curve %s\n", curves[testno]);
    if (!strcmp(curves[testno], "Ed25519")) {
      /* Ed25519 isn't allowed in fips mode */
      if (in_fips_mode) continue;
    ...
    if (verbose > 1) show_sexp("ECC key:\n", key);
  ...
  if (verbose) info("creating ECC key using curve Ed25519 for ECDSA\n");
  ...
  if (verbose) info("creating ECC key using curve NIST P-384 for ECDSA\n");
  ...
  if (verbose)
    info(
        "creating ECC key using curve Ed25519 for ECDSA "
...
static void check_nonce(void) {
  char a[32], b[32];
  int i, j;
  int oops = 0;

  if (verbose) info("checking gcry_create_nonce\n");

  gcry_create_nonce(a, sizeof a);
  for (i = 0; i < 10; i++) {
    gcry_create_nonce(b, sizeof b);
    if (!memcmp(a, b, sizeof a)) die("identical nonce found\n");
  }
  for (i = 0; i < 10; i++) {
    gcry_create_nonce(a, sizeof a);
    if (!memcmp(a, b, sizeof a)) die("identical nonce found\n");
...
again:
  for (i = 1, j = 0; i < sizeof a; i++)
    if (a[0] == a[i]) j++;
  if (j + 1 == sizeof(a)) {
    if (oops) die("impossible nonce found\n");
    oops++;
    gcry_create_nonce(a, sizeof a);
    goto again;
  }
