...
static void check_one_mac(int algo, const void *key, size_t keylen,
...
static void check_hmac(void) {
  ...
  if (verbose) fprintf(stderr, "checking FIPS-198a, A.1\n");
  ...
  if (verbose) fprintf(stderr, "checking FIPS-198a, A.2\n");
  ...
  if (verbose) fprintf(stderr, "checking FIPS-198a, A.3\n");
  ...
  if (verbose) fprintf(stderr, "checking FIPS-198a, A.4\n");
...
static void check_hmac_multi(void) {
  gpg_error_t err;
  unsigned char key[128];
  ...
  if (verbose) fprintf(stderr, "checking HMAC using multiple buffers\n");
  for (i = 0; i < 64; i++) key[i] = i;
  ...
  if (memcmp(digest, mac, maclen)) {
    printf("computed: ");
    for (i = 0; i < maclen; i++) printf("%02x ", digest[i] & 0xFF);
    printf("\nexpected: ");
    for (i = 0; i < maclen; i++) printf("%02x ", mac[i] & 0xFF);
    printf("\n");
