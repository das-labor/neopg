...
static void check_primes(void) {
  ...
  struct prime_spec {
    ...
    unsigned int flags;
      ...
      {1024, 100, GCRY_PRIME_FLAG_SPECIAL_FACTOR}, {128, 0, 0}, {0},
  };

  for (i = 0; prime_specs[i].prime_bits; i++) {
    err = gcry_prime_generate(&prime, prime_specs[i].prime_bits,
                              prime_specs[i].factor_bits, &factors, NULL, NULL,
...
static void print_mpi(const char *name, gcry_mpi_t a) {
  ...
  if (writerr) die("writing output failed\n");
  gcry_free(buf);
}

/* Create the key for our public standard dummy CA.  */
static void create_42prime(void) {
  gcry_error_t err;
  char string[128 * 2 + 1];
  int i;
  ...
  if (err) die("gcry_mpi_scan failed: %s\n", gcry_strerror(err));
  fputs("start:", stderr);
  gcry_mpi_dump(start);
  putc('\n', stderr);
  ...
  while (gcry_prime_check(p, 0)) gcry_mpi_sub_ui(p, p, 2);
  fputs("    p:", stderr);
  gcry_mpi_dump(p);
  putc('\n', stderr);
  q = gcry_mpi_copy(start);
  gcry_mpi_add_ui(q, q, 1);
  while (gcry_prime_check(q, 0)) gcry_mpi_add_ui(q, q, 2);
  fputs("    q:", stderr);
  gcry_mpi_dump(q);
  putc('\n', stderr);
  ...
  if (!gcry_mpi_gcd(t1, e, phi)) die("Oops: E is not a generator\n");
  fputs("    e:", stderr);
  gcry_mpi_dump(e);
  putc('\n', stderr);
