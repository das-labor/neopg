...
static void showhex(const char *prefix, const void *buffer, size_t length) {
  const unsigned char *p = buffer;

  if (prefix) fprintf(stderr, PGM ": %s: ", prefix);
  while (length--) fprintf(stderr, "%02X", *p++);
  if (prefix) putc('\n', stderr);
}
...
static int hex2bin(const char *string, void *buffer, size_t length) {
  int i;
  const char *s = string;
...
static char *read_textline(FILE *fp) {
  char line[256];
  char *p;
  int any = 0;
  ...
  do {
    do {
      ...
      for (; p > line; p--)
        if (my_isascii(*p) && isspace(*p)) *p = 0;
    } while (!any && !*line);
    any = 1;
  } while (*line == '#'); /* Always skip comment lines.  */
  if (verbose > 1) fprintf(stderr, PGM ": received line: %s\n", line);
  return gcry_xstrdup(line);
}

static char *read_hexline(FILE *fp, size_t *retlen) {
  char *line, *p;

  line = read_textline(fp);
  if (!line) return NULL;
  p = hex2buffer(line, retlen);
  if (!p) die("error decoding hex string on input\n");
  gcry_free(line);
  return p;
}

static void skip_to_empty_line(FILE *fp) {
  char line[256];
  char *p;
...
static size_t base64_decode(char *buffer, size_t length) {
  ...
  for (d = buffer; length; length--, s++) {
    if (lfseen && *s == '-' && length > 9 && !memcmp(s, "-----END ", 9)) break;
    if ((lfseen = (*s == '\n')) || *s == ' ' || *s == '\r' || *s == '\t')
      continue;
    if (*s == '=') {
      /* Pad character: stop */
...
/* Parse the buffer at the address BUFFER which consists of the number
   ...
   buffer. */
static int parse_tag(unsigned char const **buffer, size_t *buflen,
                     struct tag_info *ti) {
  int c;
  unsigned long tag;
  ...
  /* Get the tag */
  if (!length) return -1; /* Premature EOF.  */
  c = *buf++;
  length--;
  ti->nhdr++;
  ...
  if (ti->class == UNIVERSAL && !ti->tag) ti->length = 0;

  if (ti->length > length) return -1; /* Data larger than buffer.  */
...
/* Read the file FNAME assuming it is a PEM encoded private key file
   ...
   printed.  */
static gcry_sexp_t read_private_key_file(const char *fname, int show) {
  gcry_error_t err;
  FILE *fp;
  char *buffer;
  size_t buflen;
  ...
  if (!fp) die("can't open `%s': %s\n", fname, strerror(errno));
  buffer = read_file(fp, 0, &buflen);
  if (!buffer) die("error reading `%s'\n", fname);
  fclose(fp);
  ...
  derlen = buflen;
  if (parse_tag(&der, &derlen, &ti) || ti.tag != TAG_SEQUENCE || ti.class ||
      !ti.cons || ti.ndef)
    goto bad_asn1;
  if (parse_tag(&der, &derlen, &ti) || ti.tag != TAG_INTEGER || ti.class ||
      ti.cons || ti.ndef)
    goto bad_asn1;
  if (ti.length != 1 || *der)
  ...
  if (idx != n_keyparms) die("not enough RSA key parameters\n");
  ...
  err = gcry_sexp_build(&s_key, NULL,
                        ...
                        /**/ "(d%m)(p%m)(q%m)(u%m)))",
                        keyparms[0], keyparms[1], keyparms[2], keyparms[3],
                        keyparms[4], keyparms[7]);
  if (err) die("error building S-expression: %s\n", gpg_strerror(err));
...
/* Read the file FNAME assuming it is a PEM encoded public key file
   and return an S-expression.  With SHOW set, the key parameters are
   printed.  */
static gcry_sexp_t read_public_key_file(const char *fname, int show) {
  gcry_error_t err;
  FILE *fp;
  char *buffer;
  size_t buflen;
  ...
  if (parse_tag(&der, &derlen, &ti) || ti.tag != TAG_BIT_STRING || ti.class ||
      ti.cons || ti.ndef)
    goto bad_asn1;
  if (ti.length < 1 || *der)
  ...
  /* Build the S-expression.  */
  err = gcry_sexp_build(&s_key, NULL, "(public-key(rsa(n%m)(e%m)))",
...
static gcry_sexp_t read_sig_file(const char *fname) {
  gcry_error_t err;
  FILE *fp;
  char *buffer;
  size_t buflen;
  ...
  if (!err) err = gcry_sexp_build(&s_sig, NULL, "(sig-val(rsa(s %m)))", tmpmpi);
  if (err) die("error building S-expression: %s\n", gpg_strerror(err));
...
static gcry_sexp_t read_sexp_from_file(const char *fname) {
  gcry_error_t err;
  FILE *fp;
  char *buffer;
  size_t buflen;
  ...
  if (!buffer) die("error reading `%s'\n", fname);
  fclose(fp);
  if (!buflen) die("error: file `%s' is empty\n", fname);

  err = gcry_sexp_create(&sexp, buffer, buflen, 1, gcry_free);
  if (err) die("error parsing `%s': %s\n", fname, gpg_strerror(err));

  return sexp;
}

static void print_buffer(const void *buffer, size_t length) {
  int writerr = 0;
  ...
  if (base64_output) {
    ...
    for (p = buffer; length; p++, length--) {
      ...
      if (idx > 2) {
        ...
        outbuf[1] =
            bintoasc[(((*inbuf << 4) & 060) | ((inbuf[1] >> 4) & 017)) & 077];
        outbuf[2] =
            bintoasc[(((inbuf[1] << 2) & 074) | ((inbuf[2] >> 6) & 03)) & 077];
        outbuf[3] = bintoasc[inbuf[2] & 077];
        if (fwrite(outbuf, 4, 1, stdout) != 1) writerr = 1;
        idx = 0;
        if (++quads >= (64 / 4)) {
          if (fwrite("\n", 1, 1, stdout) != 1) writerr = 1;
          quads = 0;
        }
      }
    }
    ...
    if (fwrite(buffer, length, 1, stdout) != 1) writerr++;
  } else {
    const unsigned char *p = buffer;

    if (verbose > 1) showhex("sent line", buffer, length);
    while (length-- && !ferror(stdout)) printf("%02X", *p++);
    if (ferror(stdout)) writerr++;
  }
  if (!writerr && fflush(stdout) == EOF) writerr++;
  if (writerr) {
...
static void print_mpi_line(gcry_mpi_t a, int no_lz) {
  unsigned char *buf, *p;
  gcry_error_t err;
  int writerr = 0;

  err = gcry_mpi_aprint(GCRYMPI_FMT_HEX, &buf, NULL, a);
  if (err) die("gcry_mpi_aprint failed: %s\n", gpg_strerror(err));

  p = buf;
  if (no_lz && p[0] == '0' && p[1] == '0' && p[2]) p += 2;

  printf("%s\n", p);
  if (ferror(stdout)) writerr++;
  if (!writerr && fflush(stdout) == EOF) writerr++;
  if (writerr) die("writing output failed: %s\n", strerror(errno));
  gcry_free(buf);
}

/* Print some data on hex format on a line.  */
static void print_data_line(const void *data, size_t datalen) {
  const unsigned char *p = data;
  int writerr = 0;

  while (data && datalen-- && !ferror(stdout)) printf("%02X", *p++);
  putchar('\n');
  if (ferror(stdout)) writerr++;
  if (!writerr && fflush(stdout) == EOF) writerr++;
  if (writerr) die("writing output failed: %s\n", strerror(errno));
...
static void print_sexp(gcry_sexp_t a, FILE *fp) {
  char *buf;
  size_t size;
...
static gcry_error_t init_external_rng_test(void **r_context, unsigned int flags,
                                           const void *key, size_t keylen,
                                           const void *seed, size_t seedlen,
                                           const void *dt, size_t dtlen) {
  return gcry_control(PRIV_CTL_INIT_EXTRNG_TEST, r_context, flags, key, keylen,
                      seed, seedlen, dt, dtlen);
}

static gcry_error_t run_external_rng_test(void *context, void *buffer,
                                          size_t buflen) {
  return gcry_control(PRIV_CTL_RUN_EXTRNG_TEST, context, buffer, buflen);
}

static void deinit_external_rng_test(void *context) {
...
/* Given an OpenSSL cipher name NAME, return the Libgcrypt algirithm
   ...
   error.  */
static int map_openssl_cipher_name(const char *name, int *r_mode) {
  static struct {
    const char *name;
    int algo;
    int mode;
  } table[] = {{"bf-cbc", GCRY_CIPHER_BLOWFISH, GCRY_CIPHER_MODE_CBC},
...
static void run_encrypt_decrypt(int encrypt_mode, int cipher_algo,
                                int cipher_mode, const void *iv_buffer,
                                size_t iv_buflen, const void *key_buffer,
                                size_t key_buflen, const void *data,
                                size_t datalen, FILE *fp) {
  gpg_error_t err;
  gcry_cipher_hd_t hd;
  void *outbuf;
  size_t outbuflen;
  ...
  do {
    if (inbuf) {
      ...
      if (nread < (int)datalen && ferror(fp)) die("error reading input\n");
      data = inbuf;
      inbuflen = nread;
    } else
      inbuflen = datalen;
...
static void get_current_iv(gcry_cipher_hd_t hd, void *buffer, size_t buflen) {
...
static void run_cipher_mct_loop(int encrypt_mode, int cipher_algo,
                                int cipher_mode, const void *iv_buffer,
                                size_t iv_buflen, const void *key_buffer,
                                size_t key_buflen, const void *data,
                                size_t datalen, int iterations) {
  gpg_error_t err;
  gcry_cipher_hd_t hd;
  size_t blocklen;
  int count;
  ...
  for (count = 0; count < iterations; count++) {
    ...
    if (encrypt_mode && (cipher_mode == GCRY_CIPHER_MODE_CFB ||
      ...
      int i;
      for (i = 0; i < blocklen; i++) input[i] ^= output[i];
    } else
  ...
  putchar('\n');
  if (verbose > 1) showhex("sent line", "", 0);
  putchar('\n');
  fflush(stdout);
...
static void run_digest(int digest_algo, const void *data, size_t datalen) {
  gpg_error_t err;
  gcry_md_hd_t hd;
...
static void run_hmac(int digest_algo, const void *key, size_t keylen,
...
static void run_rsa_derive(const void *data, size_t datalen) {
  gpg_error_t err;
  ...
  if (err) die("gcry_pk_genkey failed for RSA: %s\n", gpg_strerror(err));
...
static void run_rsa_keygen(const void *data, size_t datalen, int test) {
  gpg_error_t err;
  gcry_sexp_t s_keyspec, s_key, s_top, l1;
  gcry_mpi_t mpi;
  const char *parmlist;
...
static size_t compute_tag_length(size_t n) {
  int needed = 0;
...
static unsigned char *store_tag_length(unsigned char *p, int tag, size_t n) {
  if (tag == TAG_SEQUENCE) tag |= 0x20; /* constructed */
...
*/
static void run_rsa_gen(int keysize, int pubexp) {
  gpg_error_t err;
  ...
  err =
      gcry_sexp_build(&keyspec, NULL, "(genkey (rsa (nbits %d)(rsa-use-e %d)))",
                      keysize, pubexp);
  if (err)
  ...
  if (!l1) die("private key not found in genkey result\n");
  gcry_sexp_release(key);
  key = l1;

  l1 = gcry_sexp_find_token(key, "rsa", 0);
  if (!l1) die("returned private key not formed as expected\n");
  gcry_sexp_release(key);
  key = l1;
  ...
  for (idx = 0; idx < DIM(keyparms); idx++) {
    ...
    if (!l1) die("no %c parameter in returned private key\n", keyelems[idx]);
  ...
  for (idx = 0; idx < DIM(keyparms); idx++) {
    ...
    if (err) die("error formatting parameter: %s\n", gpg_strerror(err));
    keyparmslen[idx] = n;
  ...
  *der++ = 0;
  for (idx = 0; idx < DIM(keyparms); idx++) {
    ...
    err = gcry_mpi_print(GCRYMPI_FMT_STD, der, keyparmslen[idx], NULL,
                         keyparms[idx]);
    if (err) die("error formatting parameter: %s\n", gpg_strerror(err));
    der += keyparmslen[idx];
  }
  ...
  /* Print the stuff.  */
  for (idx = 0; idx < DIM(keyparms); idx++) gcry_mpi_release(keyparms[idx]);

  assert(der - derbuf == derlen);

  if (base64_output) puts("-----BEGIN RSA PRIVATE KEY-----");
  print_buffer(derbuf, derlen);
  if (base64_output) puts("-----END RSA PRIVATE KEY-----");
...
static void run_rsa_sign(const void *data, size_t datalen, int hashalgo,
                         int pkcs1, int pss, const char *keyfile)

{
  ...
  if (pkcs1) {
    ...
    err = gcry_sexp_build(&s_data, NULL, "(data (flags pkcs1)(hash %s %b))",
                          gcry_md_algo_name(hashalgo), (int)hashsize, hash);
  } else if (pss) {
    unsigned char hash[64];
    unsigned int hashsize;
    ...
    if (!err) {
      err = gcry_sexp_build(&s_data, NULL, "(data (flags raw)(value %m))", tmp);
  ...
  if (!sig_mpi) die("no value in returned S-expression\n");
...
static void run_rsa_verify(const void *data, size_t datalen, int hashalgo,
                           int pkcs1, int pss, const char *keyfile,
                           const char *sigfile)
...
/* Generate a DSA key of size KEYSIZE and return the complete
   S-expression.  */
static gcry_sexp_t dsa_gen(int keysize) {
  gpg_error_t err;
  ...
  err = gcry_pk_genkey(&key, keyspec);
  if (err) die("gcry_pk_genkey failed for DSA: %s\n", gpg_strerror(err));
...
/* Generate a DSA key of size KEYSIZE and return the complete
   S-expression.  */
static gcry_sexp_t dsa_gen_with_seed(int keysize, const void *seed,
                                     size_t seedlen) {
  gpg_error_t err;
  gcry_sexp_t keyspec, key;
...
/* Generate an ECDSA key on the specified curve and return the complete
   S-expression. */
static gcry_sexp_t ecdsa_gen_key(const char *curve) {
  gpg_error_t err;
  gcry_sexp_t keyspec, key;
  ...
  if (err)
    die("gcry_sexp_build failed for ECDSA key generation: %s\n",
        gpg_strerror(err));
  err = gcry_pk_genkey(&key, keyspec);
  if (err) die("gcry_pk_genkey failed for ECDSA: %s\n", gpg_strerror(err));
...
/* Print the domain parameter as well as the derive information.  KEY
   ...
   g, seed, counter, h. */
static void print_dsa_domain_parameters(gcry_sexp_t key) {
  gcry_sexp_t l1, l2;
  gcry_mpi_t mpi;
  int idx;
  const void *data;
  ...
  if (!l1) die("public key not found in genkey result\n");

  l2 = gcry_sexp_find_token(l1, "dsa", 0);
  if (!l2) die("returned public key not formed as expected\n");
  gcry_sexp_release(l1);
  l1 = l2;
  ...
  if (!l1) die("misc-key-info not found in genkey result\n");

  l2 = gcry_sexp_find_token(l1, "seed-values", 0);
  if (!l2) die("no seed-values in returned key\n");
  gcry_sexp_release(l1);
  l1 = l2;

  l2 = gcry_sexp_find_token(l1, "seed", 0);
  if (!l2) die("no seed value in returned key\n");
  data = gcry_sexp_nth_data(l2, 1, &datalen);
  if (!data) die("no seed value in returned key\n");
  if (standalone_mode) printf("Seed = ");
  print_data_line(data, datalen);
  gcry_sexp_release(l2);

  l2 = gcry_sexp_find_token(l1, "counter", 0);
  if (!l2) die("no counter value in returned key\n");
  string = gcry_sexp_nth_string(l2, 1);
  if (!string) die("no counter value in returned key\n");
  if (standalone_mode)
  ...
  if (!l2) die("no n value in returned key\n");
  mpi = gcry_sexp_nth_mpi(l2, 1, GCRYMPI_FMT_USG);
  if (!mpi) die("no h value in returned key\n");
  if (standalone_mode) printf("H = ");
  print_mpi_line(mpi, 1);
  gcry_mpi_release(mpi);
  gcry_sexp_release(l2);
...
static void print_ecdsa_dq(gcry_sexp_t key) {
  gcry_sexp_t l1, l2;
  gcry_mpi_t mpi;
  int idx;
  ...
  if (!l2) die("returned private key not formed as expected\n");
  gcry_sexp_release(l1);
  l1 = l2;
  ...
  for (idx = 0; "dq"[idx]; idx++) {
    ...
    if (!mpi)
      die("no value for %c parameter in returned private key\n", "dq"[idx]);
    gcry_sexp_release(l2);
    if (standalone_mode) printf("%c = ", "dQ"[idx]);
    print_mpi_line(mpi, 1);
    gcry_mpi_release(mpi);
  }
...
static void run_dsa_pqg_gen(int keysize, const void *seed, size_t seedlen) {
  gcry_sexp_t key;
...
/* Generate a DSA key of size of KEYSIZE and write the private key to
   ...
   run_dsa_pqg_gen.  */
static void run_dsa_gen(int keysize, const char *filename) {
  gcry_sexp_t key, private_key;
  FILE *fp;

  key = dsa_gen(keysize);
  private_key = gcry_sexp_find_token(key, "private-key", 0);
  if (!private_key) die("private key not found in genkey result\n");
  print_dsa_domain_parameters(key);

  fp = fopen(filename, "wb");
  if (!fp) die("can't create `%s': %s\n", filename, strerror(errno));
...
/* Sign DATA of length DATALEN using the key taken from the S-expression
   encoded KEYFILE. */
static void run_dsa_sign(const void *data, size_t datalen, const char *keyfile)

{
  ...
  if (!err) {
    err =
        gcry_sexp_build(&s_data, NULL, "(data (flags raw)(value %m))", tmpmpi);
    gcry_mpi_release(tmpmpi);
  }
  if (err)
  ...
  if (!s_tmp) die("private key part not found in provided key\n");

  s_tmp2 = gcry_sexp_find_token(s_tmp, "dsa", 0);
  if (!s_tmp2) die("private key part is not a DSA key\n");
  gcry_sexp_release(s_tmp);

  s_tmp = gcry_sexp_find_token(s_tmp2, "y", 0);
  tmpmpi = gcry_sexp_nth_mpi(s_tmp, 1, GCRYMPI_FMT_USG);
  if (!tmpmpi) die("no y parameter in DSA key\n");
  ...
  s_tmp = gcry_sexp_find_token(s_sig, "sig-val", 0);
  if (!s_tmp) die("no sig-val element in returned S-expression\n");

  gcry_sexp_release(s_sig);
  s_sig = s_tmp;
  s_tmp = gcry_sexp_find_token(s_sig, "dsa", 0);
  if (!s_tmp) die("no dsa element in returned S-expression\n");
  ...
  tmpmpi = gcry_sexp_nth_mpi(s_tmp, 1, GCRYMPI_FMT_USG);
  if (!tmpmpi) die("no r parameter in returned S-expression\n");
  print_mpi_line(tmpmpi, 1);
  gcry_mpi_release(tmpmpi);
  gcry_sexp_release(s_tmp);
  ...
  s_tmp = gcry_sexp_find_token(s_sig, "s", 0);
  tmpmpi = gcry_sexp_nth_mpi(s_tmp, 1, GCRYMPI_FMT_USG);
  if (!tmpmpi) die("no s parameter in returned S-expression\n");
  print_mpi_line(tmpmpi, 1);
  gcry_mpi_release(tmpmpi);
  gcry_sexp_release(s_tmp);
...
static void run_dsa_verify(const void *data, size_t datalen,
...
/* Sign DATA of length DATALEN using the key taken from the S-expression
   encoded KEYFILE. */
static void run_ecdsa_sign(const void *data, size_t datalen,
...

{
  ...
  if (!err) {
    err = gcry_sexp_build(&s_data, NULL, "(data (flags raw)(hash %s %M))",
  ...
  if (!s_tmp) die("no ecdsa element in returned S-expression\n");
...
/* Verify DATA of length DATALEN using the public key taken from the
   S-expression in KEYFILE against the S-expression formatted
   signature in SIGFILE.  */
static void run_ecdsa_verify(const void *data, size_t datalen,
                             const char *keyfile, const int algo,
                             const char *sigfile)
...
static void run_ecdsa_gen_key(const char *curve) {
  gcry_sexp_t key;
...
static void usage(int show_help) {
  if (!show_help) {
    fputs("usage: " PGM " [OPTION] [FILE] (try --help for more information)\n",
          stderr);
    exit(2);
  }
  fputs("Usage: " PGM
...
int main(int argc, char **argv) {
  ...
  if (!argc || argc > 2) usage(0);

  mode_string = *argv;

  if (use_pkcs1 && use_pss) die("Only one of --pkcs or --pss may be given\n");

  if (!strcmp(mode_string, "rsa-derive")) binary_input = 1;

  if (argc == 2 && strcmp(argv[1], "-")) {
    input = fopen(argv[1], binary_input ? "rb" : "r");
    if (!input) die("can't open `%s': %s\n", argv[1], strerror(errno));
  } else
    input = stdin;

#ifndef HAVE_W32_SYSTEM
  if (loop_mode) signal(SIGPIPE, SIG_IGN);
#endif

  if (verbose) fprintf(stderr, PGM ": started (mode=%s)\n", mode_string);

  xgcry_control(GCRYCTL_SET_VERBOSITY, (int)verbose);
  if (!no_fips) xgcry_control(GCRYCTL_FORCE_FIPS_MODE, 0);
  if (!gcry_check_version("1.4.3")) die("Libgcrypt is not sufficient enough\n");
  if (verbose)
    fprintf(stderr, PGM ": using Libgcrypt %s\n", gcry_check_version(NULL));
  if (no_fips) xgcry_control(GCRYCTL_DISABLE_SECMEM, 0);
  xgcry_control(GCRYCTL_INITIALIZATION_FINISHED, 0);

  /* Most operations need some input data.  */
  if (!chunksize && !mct_server && strcmp(mode_string, "random") &&
      strcmp(mode_string, "rsa-gen") && strcmp(mode_string, "rsa-keygen") &&
      strcmp(mode_string, "rsa-keygen-kat") && strcmp(mode_string, "dsa-gen") &&
  ...
  if (!strcmp(mode_string, "encrypt") || !strcmp(mode_string, "decrypt")) {
    ...
    if (mct_server) {
      ...
      for (;;) {
        ...
        run_cipher_mct_loop((*mode_string == 'e'), cipher_algo, cipher_mode,
                            iv_buffer, iv_buflen, key_buffer, key_buflen, data,
                            datalen, iterations);
      }
    } else {
      if (cipher_mode != GCRY_CIPHER_MODE_ECB) {
        if (!iv_string) die("option --iv is required in this mode\n");
        iv_buffer = hex2buffer(iv_string, &iv_buflen);
        if (!iv_buffer) die("invalid value for IV\n");
      } else {
        iv_buffer = NULL;
        iv_buflen = 0;
      }
      ...
      if (!key_buffer) die("invalid value for KEY\n");

      run_encrypt_decrypt((*mode_string == 'e'), cipher_algo, cipher_mode,
                          iv_buffer, iv_buflen, key_buffer, key_buflen, data,
                          data ? datalen : chunksize, input);
    }
    gcry_free(key_buffer);
    gcry_free(iv_buffer);
    ...
    if (!algo) die("digest algorithm `%s' is not supported\n", algo_string);
    ...
    if (err) die("init external RNG test failed: %s\n", gpg_strerror(err));

    do {
      err = run_external_rng_test(context, buffer, sizeof buffer);
      if (err) die("running external RNG test failed: %s\n", gpg_strerror(err));
    ...
    if (!binary_output) base64_output = 1;
    ...
    if (!curve_string)
      die("option --curve containing name of the specified curve is required "
          "in this mode\n");
