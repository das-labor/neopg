...
void _gcry_set_log_verbosity(int level) { verbosity_level = level; }

int _gcry_log_verbosity(int level) { return verbosity_level >= level; }
...
char **_gcry_strtokenize(const char *string, const char *delim) {
  ...
  for (n = 0, p = buffer; (pend = strpbrk(p, delim)); p = pend + 1) {
    *pend = 0;
    while (strchr(ws, *(byte *)p)) p++;
    for (px = pend - 1; px >= p && strchr(ws, *(byte *)px); px--) *px = 0;
    result[n++] = p;
  }
  while (*p && strchr(ws, *(byte *)p)) p++;
...
void __gcry_burn_stack(unsigned int bytes) {
  ...
  if (bytes > sizeof buf) _gcry_burn_stack(bytes - sizeof buf);
#endif
}
...
void _gcry_divide_by_zero(void) {
  ...
  _gcry_fatal_error(gpg_error_from_errno(errno), "divide by zero");
}
