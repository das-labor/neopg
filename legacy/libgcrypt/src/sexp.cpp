...
static gpg_error_t do_vsexp_sscan(gcry_sexp_t *retsexp, size_t *erroff,
                                  const char *buffer, size_t length,
                                  int argflag, void **arg_list,
                                  va_list arg_ptr);

static gpg_error_t do_sexp_sscan(gcry_sexp_t *retsexp, size_t *erroff,
...
gpg_error_t _gcry_sexp_create(gcry_sexp_t *retsexp, void *buffer, size_t length,
  ...
  if (!retsexp) return GPG_ERR_INV_ARG;
  *retsexp = NULL;
  if (autodetect < 0 || autodetect > 1 || !buffer) return GPG_ERR_INV_ARG;

  if (!length && !autodetect) { /* What a brave caller to assume that there is
                                   really a canonical
                                   encoded S-expression in buffer */
    length = _gcry_sexp_canon_len((const unsigned char *)(buffer), 0, NULL,
                                  &errcode);
    if (!length) return errcode;
  } else if (!length && autodetect) { /* buffer is a string */
...
gcry_sexp_t _gcry_sexp_nth(const gcry_sexp_t list, int number) {
  ...
  if (*p == ST_DATA) {
    ...
    newlist =
        (gcry_sexp_t)xtrymalloc(sizeof *newlist + 1 + 1 + sizeof n + n + 1);
...
gcry_mpi_t _gcry_sexp_nth_mpi(gcry_sexp_t list, int number, int mpifmt) {
  ...
  if (mpifmt == GCRYMPI_FMT_OPAQUE) {
    ...
    if (_gcry_mpi_scan(&a, (gcry_mpi_format)(mpifmt), s, n, NULL)) return NULL;
  }
...
static gpg_error_t make_space(struct make_space_ctx *c, size_t n) {
  ...
  if (used + n + sizeof(DATALEN) + 1 >= c->allocated) {
    ...
    if (!newsexp) return gpg_error_from_errno(errno);
    c->allocated = newsize;
    newhead = newsexp->d;
    c->pos = newhead + used;
    c->sexp = newsexp;
...
static gpg_error_t do_vsexp_sscan(gcry_sexp_t *retsexp, size_t *erroff,
  ...
  *retsexp = NULL;
  ...
  if (!buffer) return GPG_ERR_INV_ARG;
    ...
    if (quoted) {
      ...
      if (*p == 'm' || *p == 'M') {
        ...
        *c.pos++ = ST_DATA;
        ...
        MAKE_SPACE(alen);
        if (alen && !_gcry_is_secure(c.sexp->d) && _gcry_is_secure(astr)) {
          /* We have to switch to secure allocation.  */
          gcry_sexp_t newsexp;
          byte *newhead;

          newsexp =
              (gcry_sexp_t)xtrymalloc_secure(sizeof *newsexp + c.allocated - 1);
          if (!newsexp) {
            err = gpg_error_from_errno(errno);
            goto leave;
          }
...
gpg_error_t _gcry_sexp_build(gcry_sexp_t *retsexp, size_t *erroff,
  ...
  rc =
      do_vsexp_sscan(retsexp, erroff, format, strlen(format), 1, NULL, arg_ptr);
  va_end(arg_ptr);
...
static int suitable_encoding(const unsigned char *buffer, size_t length) {
  ...
  for (s = buffer; length; s++, length--) {
    if ((*s < 0x20 || (*s >= 0x7f && *s <= 0xa0)) &&
        !strchr("\b\t\v\n\f\r\"\'\\", *s))
      return 0; /*binary*/
    if (maybe_token && !alphap(s) && !digitp(s) && !strchr(TOKEN_SPECIALS, *s))
      maybe_token = 0;
  }
  s = buffer;
  if (maybe_token && !digitp(s)) return 2;
  return 1;
}
...
static int convert_to_hex(const unsigned char *src, size_t len, char *dest) {
  ...
  if (dest) {
    *dest++ = '#';
    for (i = 0; i < len; i++, dest += 2) sprintf(dest, "%02X", src[i]);
    *dest++ = '#';
  }
  return len * 2 + 2;
}
...
gpg_error_t _gcry_sexp_vextract_param(gcry_sexp_t sexp, const char *path,
  ...
  for (s = list, idx = 0; *s && idx < DIM(array); s++) {
    if (*s == '&' || *s == '+' || *s == '-' || *s == '/' || *s == '?')
      ...
      if (!array[idx]) return GPG_ERR_MISSING_VALUE; /* NULL pointer given.  */
      idx++;
    }
  }
  if (*s) return GPG_ERR_LIMIT_REACHED; /* Too many list elements.  */
