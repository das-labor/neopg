...
#define R(a, b, c, d, e, f, g, h, k, w)                    \
  ...
  for (i = 0; i < 64; i++) R(a, b, c, d, e, f, g, h, K[i], w[i]);
...
static void finalize(hmac256_context_t hd) {
  ...
  if (hd->finalized) return; /* Silently ignore a finalized context.  */
...
void _gcry_hmac256_release(hmac256_context_t ctx) {
  if (ctx) {
    ...
    if (ctx->use_hmac) my_wipememory(ctx->opad, 64);
    free(ctx);
  }
}
...
void _gcry_hmac256_update(hmac256_context_t hd, const void *buffer,
  ...
  if (!inbuf) return; /* Only flushing was requested. */
  if (hd->count) {
    for (; length && hd->count < 64; length--) hd->buf[hd->count++] = *inbuf++;
    _gcry_hmac256_update(hd, NULL, 0); /* Flush.  */
    if (!length) return;
  }
...
int main(int argc, char **argv) {
...
#ifdef __WIN32
  if (use_binary) setmode(fileno(stdout), O_BINARY);
#endif
  ...
  for (; argc || use_stdin; argv++, argc--) {
    ...
    if (use_binary) {
      ...
      for (idx = 0; idx < dlen; idx++) printf("%02x", digest[idx]);
      _gcry_hmac256_release(hd);
      if (use_stdin) {
        putchar('\n');
        break;
