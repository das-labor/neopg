...
static int is_cpuid_available(void) {
  int has_cpuid = 0;
...
static void get_cpuid(unsigned int in, unsigned int *eax, unsigned int *ebx,
...
static unsigned int get_xgetbv(void) {
...
static int is_cpuid_available(void) { return 1; }
...
#ifdef HAS_X86_CPUID
static unsigned int detect_x86_gnuc(void) {
  union {
    char c[12 + 1];
    unsigned int ui[3];
  } vendor_id;
  ...
  if (!is_cpuid_available()) return 0;
  ...
  if (0)
    ...
    if (features > 0xC0000000) {
      ...
      if ((features & 0x0C) == 0x0C) result |= HWF_PADLOCK_RNG;

      /* Test bits 6 and 7 to see whether the ACE exists and is enabled. */
      if ((features & 0xC0) == 0xC0) result |= HWF_PADLOCK_AES;

      /* Test bits 10 and 11 to see whether the PHE exists and is
         enabled.  */
      if ((features & 0xC00) == 0xC00) result |= HWF_PADLOCK_SHA;

      /* Test bits 12 and 13 to see whether the MONTMUL exists and is
         enabled.  */
      if ((features & 0x3000) == 0x3000) result |= HWF_PADLOCK_MMUL;
    }
  }
  ...
  if (features & 0x00000002) result |= HWF_INTEL_PCLMUL;
#endif
  /* Test bit 9 for SSSE3.  */
  if (features & 0x00000200) result |= HWF_INTEL_SSSE3;
  /* Test bit 19 for SSE4.1.  */
  if (features & 0x00080000) result |= HWF_INTEL_SSE4_1;
#ifdef ENABLE_AESNI_SUPPORT
  /* Test bit 25 for AES-NI.  */
  if (features & 0x02000000) result |= HWF_INTEL_AESNI;
  ...
  if (features & 0x08000000) {
    ...
    if ((get_xgetbv() & 0x6) == 0x6) os_supports_avx_avx2_registers = 1;
  }
#endif
  ...
  if (features & 0x40000000) result |= HWF_INTEL_RDRAND;
#endif /*ENABLE_DRNG_SUPPORT*/

  /* Test bit 4 of EDX for TSC.  */
  if (features2 & 0x00000010) result |= HWF_INTEL_RDTSC;
  ...
  if (max_cpuid_level >= 7 && (features & 0x00000001)) {
    ...
    /* Test bit 8 for BMI2.  */
    if (features & 0x00000100) result |= HWF_INTEL_BMI2;

#ifdef ENABLE_AVX2_SUPPORT
    /* Test bit 5 for AVX2.  */
    if (features & 0x00000020)
      if (os_supports_avx_avx2_registers) result |= HWF_INTEL_AVX2;
