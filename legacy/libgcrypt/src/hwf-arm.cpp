...
static unsigned int detect_arm_at_hwcap(void) {
  ...
  if (get_hwcap(&hwcap, &hwcap2) < 0) return features;
...
static unsigned int detect_arm_proc_cpuinfo(unsigned int *broken_hwfs) {
  ...
  fclose(f);
  if (readlen <= 0 || readlen > sizeof(buf)) return 0;
  ...
  if (!str_features) return stored_cpuinfo_features;
