...
leave:
  if (!no_fips_mode_required) {
    ...
    if (fp) {
      ...
      if (fgets(line, sizeof line, fp) && atoi(line)) enforced_fips_mode = 1;
      fclose(fp);
    }
...
int _gcry_enforced_fips_mode(void) {
  if (!_gcry_fips_mode()) return 0;
  return enforced_fips_mode;
}

/* Set a flag telling whether we are in the enforced fips mode.  */
void _gcry_set_enforced_fips_mode(void) { enforced_fips_mode = 1; }
...
int _gcry_fips_test_error_or_operational(void) {
  ...
  if (!fips_mode())
    ...
    lock_fsm();
    result =
        (current_state == STATE_OPERATIONAL || current_state == STATE_ERROR);
    unlock_fsm();
  }
  return result;
}
...
static void reporter(const char *domain, int algo, const char *what,
                     const char *errtxt) {
  if (!errtxt && !_gcry_log_verbosity(2)) return;
...
static int run_hmac_selftests(int extended) { return 0; }
        ...
        *p = '.';
        ...
        if (!fp)
          ...
          if (n == 64 || (n == 65 && buffer[64] == '\n') ||
            ...
            buffer[64] = 0;
            for (n = 0, s = buffer;
                 n < 32 && loxdigit_p(s) && loxdigit_p(s + 1); n++, s += 2)
              buffer[n] = loxtoi_2(s);
            if (n == 32 && !memcmp(digest, buffer, 32)) err = 0;
          }
          fclose(fp);
        }
      }
...
gpg_error_t _gcry_fips_run_selftests(int extended) {
  ...
  if (run_cipher_selftests(extended)) goto leave;
  ...
  if (check_binary_integrity()) goto leave;
...
leave:
  if (fips_mode()) fips_new_state(result);
...
void _gcry_fips_signal_error(const char *srcfile, int srcline,
  ...
  if (!fips_mode()) return; /* Not required.  */
...
static void fips_new_state(enum module_states new_state) {
  ...
  switch (current_state) {
    ...
    case STATE_FATALERROR:
      if (new_state == STATE_SHUTDOWN) ok = 1;
      break;
  ...
  if (!ok || _gcry_log_verbosity(2))
    log_info("libgcrypt state transition %s => %s %s\n", state2str(last_state),
             state2str(new_state), ok ? "granted" : "denied");
...
#ifdef HAVE_SYSLOG
    syslog(LOG_USER | LOG_WARNING,
           "Libgcrypt notice: state transition %s => %s", state2str(last_state),
           state2str(new_state));
#endif /*HAVE_SYSLOG*/
  }
}
