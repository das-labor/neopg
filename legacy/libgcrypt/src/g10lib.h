...
#define gcry_assert(expr)                                                      \
  (LIKELY(expr) ? (void)0 : _gcry_assert_failed(STR(expr), __FILE__, __LINE__, \
                                                __FUNCTION__))
...
gpg_error_t _gcry_generate_elg_prime(int mode, unsigned int pbits,
                                     ...
                                     gcry_mpi_t *r_prime, gcry_mpi_t **factors);
...
gpg_error_t _gcry_fips186_4_prime_check(const gcry_mpi_t x, unsigned int bits);
...
typedef struct fast_wipememory_s { FASTWIPE_T a; } fast_wipememory_t;
#endif
...
#define fast_wipememory2(_vptr, _vset, _vlen)             \
  do {                                                    \
    ...
    if (_vlen < sizeof(FASTWIPE_T)) break;                \
