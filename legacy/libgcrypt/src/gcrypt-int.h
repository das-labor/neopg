...
gpg_error_t _gcry_pk_encrypt(gcry_sexp_t *result, gcry_sexp_t data,
                             gcry_sexp_t pkey);
gpg_error_t _gcry_pk_decrypt(gcry_sexp_t *result, gcry_sexp_t data,
                             gcry_sexp_t skey);
gpg_error_t _gcry_pk_sign(gcry_sexp_t *result, gcry_sexp_t data,
                          gcry_sexp_t skey);
gpg_error_t _gcry_pk_verify(gcry_sexp_t sigval, gcry_sexp_t data,
                            gcry_sexp_t pkey);
...
gpg_error_t _gcry_pk_algo_info(int algo, int what, void *buffer,
                               size_t *nbytes);
...
gpg_error_t _gcry_md_ctl(gcry_md_hd_t hd, int cmd, void *buffer, size_t buflen);
...
#define _gcry_md_putc(h, c)                                              \
  do {                                                                   \
    gcry_md_hd_t h__ = (h);                                              \
    if ((h__)->bufpos == (h__)->bufsize) _gcry_md_write((h__), NULL, 0); \
    (h__)->buf[(h__)->bufpos++] = (c)&0xff;                              \
  } while (0)

gpg_error_t _gcry_mac_open(gcry_mac_hd_t *handle, int algo, unsigned int flags,
                           gcry_ctx_t ctx);
...
gpg_error_t _gcry_mac_algo_info(int algo, int what, void *buffer,
                                size_t *nbytes);
gpg_error_t _gcry_mac_setkey(gcry_mac_hd_t hd, const void *key, size_t keylen);
gpg_error_t _gcry_mac_setiv(gcry_mac_hd_t hd, const void *iv, size_t ivlen);
...
gpg_error_t _gcry_sexp_create(gcry_sexp_t *retsexp, void *buffer, size_t length,
...
gpg_error_t _gcry_sexp_extract_param(gcry_sexp_t sexp, const char *path,
                                     const char *list, ...)
...
gpg_error_t _gcry_mpi_get_ui(gcry_mpi_t w, unsigned long *u);
...
gpg_error_t _gcry_mpi_print(enum gcry_mpi_format format, unsigned char *buffer,
