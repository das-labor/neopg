...
gpg_error_t gcry_pubkey_get_sexp(gcry_sexp_t *r_sexp, int mode, gcry_ctx_t ctx);
...
gpg_error_t gcry_md_algo_info(int algo, int what, void *buffer, size_t *nbytes);
...
#define gcry_md_putc(h, c)                                              \
  do {                                                                  \
    gcry_md_hd_t h__ = (h);                                             \
    if ((h__)->bufpos == (h__)->bufsize) gcry_md_write((h__), NULL, 0); \
...
gpg_error_t gcry_mac_ctl(gcry_mac_hd_t h, int cmd, void *buffer, size_t buflen);
...
gpg_error_t gcry_mac_write(gcry_mac_hd_t hd, const void *buffer, size_t length);
