...
gpg_error_t _gcry_disable_hw_feature(const char *name) {
  ...
  while (name && *name) {
    ...
    if (!n1)
      ...
      if (!(i < DIM(hwflist))) return GPG_ERR_INV_NAME;
    }
    name += n1;
    if (*name) name++; /* Skip delimiter ':' or ','.  */
  }
  return 0;
}
...
unsigned int _gcry_get_hw_features(void) { return hw_features; }
...
const char *_gcry_enum_hw_features(int idx, unsigned int *r_feature) {
  if (idx < 0 || idx >= DIM(hwflist)) return NULL;
  if (r_feature) *r_feature = hwflist[idx].flag;
...
static void parse_hwf_deny_file(void) {
  ...
  for (;;) {
    if (!fgets(buffer, sizeof buffer, fp)) {
      if (!feof(fp)) {
        ...
        syslog(LOG_USER | LOG_WARNING,
               "Libgcrypt warning: error reading '%s', line %d", fname, lnr);
#endif /*HAVE_SYSLOG*/
      }
      fclose(fp);
      return;
    ...
    for (; pend > p; pend--)
      if (my_isascii(*pend) && isspace(*pend)) *pend = 0;
    if (!*p || *p == '#') continue;

    if (_gcry_disable_hw_feature(p) == GPG_ERR_INV_NAME) {
#ifdef HAVE_SYSLOG
      syslog(LOG_USER | LOG_WARNING,
             "Libgcrypt warning: unknown feature in '%s', line %d", fname, lnr);
#endif /*HAVE_SYSLOG*/
    }
  }
}
