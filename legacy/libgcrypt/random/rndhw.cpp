...
static size_t poll_padlock(void (*add)(const void *, size_t,
  ...
  while (nbytes < 64) {
    ...
    if ((status & (1 << 6))      /* RNG still enabled.  */
      ...
      nbytes += (status & 0x1f);
      if (fast) break; /* Don't get into the loop with the fast flag set.  */
      p += (status & 0x1f);
    } else {
...
static size_t poll_drng(add_fn_t add, enum random_origins origin, int fast) {
  ...
  if (!rdrand_nlong(buffer, DIM(buffer))) return 0;
  (*add)((void *)buffer, nbytes, origin);
  return nbytes;
}
#endif /*USE_DRNG*/

int _gcry_rndhw_failed_p(void) { return rng_failed; }
...
#ifdef USE_DRNG
  if ((_gcry_get_hw_features() & HWF_INTEL_RDRAND)) poll_drng(add, origin, 1);
#endif
#ifdef USE_PADLOCK
  if ((_gcry_get_hw_features() & HWF_PADLOCK_RNG)) poll_padlock(add, origin, 1);
#endif
}
