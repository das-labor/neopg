...
#if USE_RNDLINUX
  rc =
      _gcry_rndlinux_gather_random(read_cb, (random_origins)(0), length, level);
#elif USE_RNDUNIX
...
void _gcry_rngsystem_dump_stats(void) { /* Not yet implemented.  */
}
...
void _gcry_rngsystem_randomize(void *buffer, size_t length,
  ...
  if (level != GCRY_VERY_STRONG_RANDOM) level = GCRY_STRONG_RANDOM;
