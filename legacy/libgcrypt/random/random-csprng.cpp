...
static void initialize(void) {
  ...
  if (!rndpool) {
    ...
    rndpool =
        (unsigned char *)(secure_alloc ? xcalloc_secure(1, POOLSIZE + BLOCKLEN)
...
void _gcry_rngcsprng_dump_stats(void) {
  ...
  log_info(
      ...
      rndstats.getbytes2, _gcry_rndhw_failed_p() ? " (hwrng failed)" : "");
}
...
/* This function should be called during initialization and before
   ...
   memory.  */
void _gcry_rngcsprng_secure_alloc(void) { secure_alloc = 1; }
...
void _gcry_rngcsprng_enable_quick_gen(void) { quick_test = 1; }

void _gcry_rngcsprng_set_daemon_socket(const char *socketname) {
#ifdef USE_RANDOM_DAEMON
  if (daemon_socket_name) BUG();
...
gpg_error_t _gcry_rngcsprng_add_bytes(const void *buf, size_t buflen,
  ...
  if (!buf) return GPG_ERR_INV_ARG;

  if (!buflen || quality < 10) return 0; /* Take a shortcut. */
  ...
  while (buflen) {
    ...
    lock_pool();
    if (rndpool) add_randomness(bufptr, nbytes, RANDOM_ORIGIN_EXTERNAL);
    unlock_pool();
    bufptr += nbytes;
    buflen -= nbytes;
  }
...
void _gcry_rngcsprng_randomize(void *buffer, size_t length,
  ...
  if (quick_test && level > GCRY_STRONG_RANDOM) level = GCRY_STRONG_RANDOM;
...
#error must have a digest length of 20 for SHA-1
  ...
  if (failsafe_digest_valid && pool == rndpool) {
    for (i = 0; i < DIGESTLEN; i++) pool[i] ^= failsafe_digest[i];
  }
...
retry:
  ...
  my_pid2 = getpid();
  if (my_pid == (pid_t)(-1)) my_pid = my_pid2;
  if (my_pid != my_pid2) {
  ...
  while (length--) {
    ...
    if (pool_writepos >= POOLSIZE) {
      ...
      if (origin >= RANDOM_ORIGIN_SLOWPOLL && !pool_filled) {
        ...
        count = 0;
        if (pool_filled_counter >= POOLSIZE) pool_filled = 1;
      }
      pool_writepos = 0;
      mix_pool(rndpool);
      rndstats.mixrnd++;
...
static void do_fast_random_poll(void) {
  ...
  if (fast_gather_fnc) fast_gather_fnc(add_randomness, RANDOM_ORIGIN_FASTPOLL);
