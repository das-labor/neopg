...
static void unlock_rng(void) {
  ...
  if (rc) log_fatal("failed to release the RNG lock: %s\n", gpg_strerror(rc));
}

static void setup_guards(rng_context_t rng_ctx) {
...
static void check_guards(rng_context_t rng_ctx) {
  if (rng_ctx->guard_0[0] != 17 || rng_ctx->guard_1[0] != 42 ||
...
static void x931_get_dt(unsigned char *buffer, size_t length,
                        rng_context_t rng_ctx) {
  ...
  if (rng_ctx->test_dt_ptr && rng_ctx != nonce_context &&
...
static void xor_buffer(unsigned char *r, const unsigned char *a,
                       const unsigned char *b, size_t length) {
  for (; length; length--, a++, b++, r++) *r = (*a ^ *b);
}

/* Encrypt LENGTH bytes of INPUT to OUTPUT using KEY.  LENGTH
   needs to be 16. */
static void encrypt_aes(gcry_cipher_hd_t key, unsigned char *output,
  ...
  if (err) log_fatal("AES encryption in RNG failed: %s\n", gpg_strerror(err));
}
...
static void x931_aes(unsigned char result_R[16], unsigned char datetime_DT[16],
                     unsigned char seed_V[16], gcry_cipher_hd_t key,
...
static int x931_aes_driver(unsigned char *output, size_t length,
                           rng_context_t rng_ctx) {
  ...
  while (length) {
    ...
    x931_aes(result_buffer, datetime_DT, rng_ctx->seed_V, rng_ctx->cipher_hd,
             intermediate_I, temp_buffer);
    rng_ctx->use_counter++;

    if (rng_ctx->test_no_dup_check && rng_ctx->test_dt_ptr &&
        rng_ctx != nonce_context && rng_ctx != std_rng_context &&
...
/* Callback for x931_generate_key. Note that this callback uses the
   ...
   gathering module. */
static void entropy_collect_cb(const void *buffer, size_t length,
...
static void *get_entropy(size_t nbytes) {
  void *result;
  int rc;
  ...
  rc = _gcry_rndlinux_gather_random(entropy_collect_cb, (random_origins)(0),
                                    X931_AES_KEYLEN, GCRY_VERY_STRONG_RANDOM);
#elif USE_RNDW32
  do {
    rc = _gcry_rndw32_gather_random(entropy_collect_cb, 0, X931_AES_KEYLEN,
...
static gcry_cipher_hd_t x931_generate_key(int for_nonce) {
  gcry_cipher_hd_t hd;
  gpg_error_t rc;
  void *buffer;
...
static void x931_generate_seed(unsigned char *seed_buffer, size_t length) {
  void *buffer;
...
static void get_random(void *buffer, size_t length, rng_context_t rng_ctx) {
  ...
  if (!rng_ctx->cipher_hd) {
    ...
    if (!rng_ctx->cipher_hd) goto bailout;
    rng_ctx->key_init_pid = getpid();
  }

  /* Initialize the seed value if needed.  */
  if (!rng_ctx->is_seeded) x931_reseed(rng_ctx);

  if (rng_ctx->key_init_pid != getpid() || rng_ctx->seed_init_pid != getpid()) {
    ...
    fips_signal_error(
        "fork without proper re-initialization "
        "detected in RNG");
    goto bailout;
  }

  if (x931_aes_driver((unsigned char *)(buffer), length, rng_ctx)) goto bailout;
...
void _gcry_rngfips_initialize(int full) {
  ...
  if (!tempvalue_for_x931_aes_driver) {
    ...
    strong_rng_context =
        (rng_context_t)xcalloc_secure(1, sizeof *strong_rng_context);
...
void _gcry_rngfips_dump_stats(void) { /* Not yet implemented.  */
}
...
gpg_error_t _gcry_rngfips_add_bytes(const void *buf, size_t buflen,
                                    int quality) {
  (void)buf;
  (void)buflen;
  (void)quality;
...
/* Run a Know-Answer-Test using a dedicated test context.  Note that
   ...
   vectors. */
static gpg_error_t selftest_kat(selftest_report_func_t report) {
  static struct {
    const unsigned char key[16];
    const unsigned char dt[16];
    const unsigned char v[16];
    const unsigned char r[3][16];
  } tv[] = {{{0xb9, 0xca, 0x7f, 0xd6, 0xa0, 0xf5, 0xd3, 0x42, 0x19, 0x6d, 0x84,
              0x91, 0x76, 0x1c, 0x3b, 0xbe},
             {0x48, 0xb2, 0x82, 0x98, 0x68, 0xc2, 0x80, 0x00, 0x00, 0x00, 0x28,
              0x18, 0x00, 0x00, 0x25, 0x00},
             {0x52, 0x17, 0x8d, 0x29, 0xa2, 0xd5, 0x84, 0x12, 0x9d, 0x89, 0x9a,
              0x45, 0x82, 0x02, 0xf7, 0x77},
             {{0x42, 0x9c, 0x08, 0x3d, 0x82, 0xf4, 0x8a, 0x40, 0x66, 0xb5, 0x49,
               0x27, 0xab, 0x42, 0xc7, 0xc3},
              {0x0e, 0xb7, 0x61, 0x3c, 0xfe, 0xb0, 0xbe, 0x73, 0xf7, 0x6e, 0x6d,
               0x6f, 0x1d, 0xa3, 0x14, 0xfa},
              {0xbb, 0x4b, 0xc1, 0x0e, 0xc5, 0xfb, 0xcd, 0x46, 0xbe, 0x28, 0x61,
               0xe7, 0x03, 0x2b, 0x37, 0x7d}}},
            {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
             ...
             {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
              0x00, 0x00, 0x00, 0x00, 0x00},
             {{0xf7, 0x95, 0xbd, 0x4a, 0x52, 0xe2, 0x9e, 0xd7, 0x13, 0xd3, 0x13,
               0xfa, 0x20, 0xe9, 0x8d, 0xbc},
              {0xc8, 0xd1, 0xe5, 0x11, 0x59, 0x52, 0xf7, 0xfa, 0x37, 0x38, 0xb4,
               0xc5, 0xce, 0xb2, 0xb0, 0x9a},
              {0x0d, 0x9c, 0xc5, 0x0d, 0x16, 0xe1, 0xbc, 0xed, 0xcf, 0x60, 0x62,
               0x09, 0x9d, 0x20, 0x83, 0x7e}}},
            {{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a,
              0x0b, 0x0c, 0x0d, 0x0e, 0x0f},
             {0x80, 0x00, 0x81, 0x01, 0x82, 0x02, 0x83, 0x03, 0xa0, 0x20, 0xa1,
              0x21, 0xa2, 0x22, 0xa3, 0x23},
             {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
              0xff, 0xff, 0xff, 0xff, 0xff},
             {{0x96, 0xed, 0xcc, 0xc3, 0xdd, 0x04, 0x7f, 0x75, 0x63, 0x19, 0x37,
               0x6f, 0x15, 0x22, 0x57, 0x56},
              {0x7a, 0x14, 0x76, 0x77, 0x95, 0x17, 0x7e, 0xc8, 0x92, 0xe8, 0xdd,
               0x15, 0xcb, 0x1f, 0xbc, 0xb1},
              {0x25, 0x3e, 0x2e, 0xa2, 0x41, 0x1b, 0xdd, 0xf5, 0x21, 0x48, 0x41,
               0x71, 0xb3, 0x8d, 0x2f, 0x4c}}}};
  int tvidx, ridx;
  ...
  for (tvidx = 0; tvidx < DIM(tv); tvidx++) {
    /* Setup the key.  */
    rc = _gcry_cipher_open(&test_ctx->cipher_hd, GCRY_CIPHER_AES128,
    ...
    test_ctx->test_dt_counter =
        ((tv[tvidx].dt[12] << 24) | (tv[tvidx].dt[13] << 16) |
         (tv[tvidx].dt[14] << 8) | (tv[tvidx].dt[15]));
...
/* Run the self-tests.  */
gpg_error_t _gcry_rngfips_selftest(selftest_report_func_t report) {
  gpg_error_t ec;
...
gpg_error_t _gcry_rngfips_init_external_test(void **r_context,
                                             unsigned int flags,
  ...
  if (!r_context || !key || keylen != 16 || !seed || seedlen != 16 || !dt ||
      dtlen != 16)
    return GPG_ERR_INV_ARG;

  test_ctx = (rng_context_t)xtrycalloc(1, sizeof *test_ctx + dtlen);
  if (!test_ctx) return gpg_error_from_syserror();
  setup_guards(test_ctx);
  ...
  test_ctx->test_dt_counter =
      ((test_ctx->test_dt_ptr[12] << 24) | (test_ctx->test_dt_ptr[13] << 16) |
       (test_ctx->test_dt_ptr[14] << 8) | (test_ctx->test_dt_ptr[15]));

  if ((flags & 1)) test_ctx->test_no_dup_check = 1;
...
gpg_error_t _gcry_rngfips_run_external_test(void *context, char *buffer,
                                            size_t buflen) {
  rng_context_t test_ctx = (rng_context_t)context;

  if (!test_ctx || !buffer || buflen != 16) return GPG_ERR_INV_ARG;
