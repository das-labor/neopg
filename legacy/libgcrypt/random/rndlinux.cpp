...
static int open_device(const char *name, int retry) {
  ...
  if (retry) _gcry_random_progress("open_dev_random", 'X', 1, 0);
again:
...
int _gcry_rndlinux_gather_random(void (*add)(const void *, size_t,
  ...
  if ((_gcry_get_hw_features() & HWF_INTEL_RDRAND)) {
    if (n_hw > length / 4) n_hw = length / 4;
  } else {
    if (n_hw > length / 2) n_hw = length / 2;
  }
  if (length > 1) length -= n_hw;
 ...
 * select(2) to have a timeout. */
    ...
    if (fd == fd_urandom) {
      ...
      else { /* The syscall is supported.  Some sanity checks.  */
        if (ret == -1)
          log_fatal("unexpected error from getrandom: %s\n", strerror(errno));
        else if (ret != nbytes)
          log_fatal(
              "getrandom returned only"
    ...
    if (any_need_entropy || last_so_far != (want - length)) {
      ...
      _gcry_random_progress("need_entropy", 'X', (int)last_so_far, (int)want);
      any_need_entropy = 1;
    }
