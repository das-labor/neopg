...
static unsigned char *read_file(const char *fname, size_t *r_length) {
  FILE *fp;
  struct stat st;
  char *buf;
  size_t buflen;
...
ksba_cert_t get_one_cert(const char *fname) {
  gpg_error_t err;
  FILE *fp;
  ksba_reader_t r;
  ksba_cert_t cert;
...
/* Create a request for the DER encoded certificate in the file
   ...
   ISSUER_CERT_FNAME. */
void one_request(const char *cert_fname, const char *issuer_cert_fname) {
  gpg_error_t err;
  ...
  if (!no_nonce) ksba_ocsp_set_nonce(ocsp, "ABCDEFGHIJKLMNOP", 16);
  ...
  printf("OCSP request of length %u created\n", (unsigned int)requestlen);
  {
    ...
    if (!fp) fail("can't create output file `a.req'");
    if (fwrite(request, requestlen, 1, fp) != 1) fail("can't write output");
    fclose(fp);
  }
...
void one_response(const char *cert_fname, const char *issuer_cert_fname,
  ...
  switch (response_status) {
    ...
    case KSBA_OCSP_RSPSTATUS_NONE:
      ...
      break;
    default:
      fail("impossible response_status");
      break;
  }
  ...
  if (response_status == KSBA_OCSP_RSPSTATUS_SUCCESS ||
...

    {
      ...
      for (idx = 0; !(err = ksba_ocsp_get_extension(ocsp, NULL, idx, &oid,
                                                    &crit, &der, &derlen));
           idx++) {
        const char *s = get_oid_desc(oid);
        printf("%sresp-extn ..%s: %s%s%s%s  (", crit ? "crit. " : "",
               crit ? "" : "......", s ? "(" : "", s ? s : "", s ? ") " : "",
               oid);
      ...
      if (err && gpg_err_code(err) != GPG_ERR_EOF) fail_if_err(err);

      for (idx = 0; !(err = ksba_ocsp_get_extension(ocsp, cert, idx, &oid,
                                                    &crit, &der, &derlen));
           idx++) {
        const char *s = get_oid_desc(oid);
        printf("%ssngl-extn ..%s: %s%s%s%s  (", crit ? "crit. " : "",
               crit ? "" : "......", s ? "(" : "", s ? s : "", s ? ") " : "",
               oid);
        print_hex(der, derlen);
        putchar(')');
...
static gpg_error_t my_hash_buffer(void *arg, const char *oid,
  ...
  if (resultsize < 20) return gpg_error(GPG_ERR_BUFFER_TOO_SHORT);
...
/* ( printf "POST / HTTP/1.0\r\nContent-Type:
   ...
   '`\r\n\r\n"; cat a.req ) |  nc -v ocsp.openvalidation.org 8088   | sed
   '1,/^\r$/d' >a.rsp
...
int main(int argc, char **argv) {
  ...
  while (argc && last_argc != argc) {
    ...
    if (!strcmp(*argv, "--help")) {
      puts(
          ...
          "       ./t-ocsp [options] --response {CERTFILE ISSUERCERTFILE "
          "RESPONSEFILE}\n"
          "\n"
  ...
  if (response_mode) {
    ...
    if (argc) fputs("warning: extra argument ignored\n", stderr);
  } else if (argc) {
    for (; argc > 1; argc -= 2, argv += 2) one_request(*argv, argv[1]);
    if (argc) fputs("warning: extra argument ignored\n", stderr);
  } else {
    struct {
