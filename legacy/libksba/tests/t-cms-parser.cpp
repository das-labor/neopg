...
void dummy_hash_fnc(void *arg, const void *buffer, size_t length) {
  (void)arg;
  (void)buffer;
  (void)length;
}
...
static void one_file(const char *fname) {
  ...
  switch (ksba_cms_identify(r)) {
    case KSBA_CT_DATA:
      s = "data";
      break;
    case KSBA_CT_SIGNED_DATA:
      s = "signed data";
      break;
    case KSBA_CT_ENVELOPED_DATA:
      s = "enveloped data";
      break;
    case KSBA_CT_DIGESTED_DATA:
      s = "digested data";
      break;
    case KSBA_CT_ENCRYPTED_DATA:
      s = "encrypted data";
      break;
    case KSBA_CT_AUTH_DATA:
      s = "auth data";
      break;
    default:
      s = "unknown";
  ...
  if (ksba_cms_get_content_type(cms, 0) == KSBA_CT_ENVELOPED_DATA) {
    ...
    for (idx = 0; idx < 1; idx++) {
      ...
      if (err && err != -1) fail_if_err2(fname, err);
      if (err != -1) {
        char *tmp;
