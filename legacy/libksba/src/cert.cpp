...
gpg_error_t ksba_cert_new(ksba_cert_t *acert) {
  *acert = (ksba_cert_t)xtrycalloc(1, sizeof **acert);
  if (!*acert) return gpg_error_from_errno(errno);
  (*acert)->ref_count++;
...
void ksba_cert_release(ksba_cert_t cert) {
  ...
  if (cert->udata) {
    ...
    do {
      ...
      if (ud->data && ud->data != ud->databuf) xfree(ud->data);
      xfree(ud);
      ud = ud2;
    } while (ud);
  }
  ...
  if (cert->cache.extns_valid) {
    for (i = 0; i < cert->cache.n_extns; i++) xfree(cert->cache.extns[i].oid);
...
gpg_error_t ksba_cert_set_user_data(ksba_cert_t cert, const char *key,
  ...
  if (!cert || !key || !*key) return GPG_ERR_INV_VALUE;

  for (ud = cert->udata; ud; ud = ud->next)
    if (!strcmp(ud->key, key)) break;
  if (ud) /* Update the data stored under this key or reuse this item. */
  {
    ...
    if (data && datalen <= sizeof ud->databuf) {
      ...
      if (!ud->data) return gpg_error_from_errno(errno);
  ...
  } else if (data) /* Insert as a new item. */
  {
    ...
    if (!ud) return gpg_error_from_errno(errno);
    strcpy(ud->key, key);
...
gpg_error_t ksba_cert_get_user_data(ksba_cert_t cert, const char *key,
  ...
  for (ud = cert->udata; ud; ud = ud->next)
    if (!strcmp(ud->key, key)) break;
  if (!ud || !ud->data) return GPG_ERR_NOT_FOUND;
  if (datalen) *datalen = ud->datalen;
  if (buffer) {
    if (ud->datalen > bufferlen) return GPG_ERR_BUFFER_TOO_SHORT;
...
gpg_error_t ksba_cert_read_der(ksba_cert_t cert, ksba_reader_t reader) {
  ...
  if (!cert || !reader) return GPG_ERR_INV_VALUE;
  if (cert->initialized)
    return GPG_ERR_CONFLICT; /* Fixme: should remove the old one */
  ...
  err = _ksba_ber_decoder_decode(decoder, "TMTTv2.Certificate", 0, &cert->root,
                                 &cert->image, &cert->imagelen);
  if (!err) cert->initialized = 1;
...
gpg_error_t ksba_cert_init_from_mem(ksba_cert_t cert, const void *buffer,
                                    size_t length) {
  gpg_error_t err;
...
const unsigned char *ksba_cert_get_image(ksba_cert_t cert, size_t *r_length) {
  ...
  if (!cert) return NULL;
  if (!cert->initialized) return NULL;
  ...
  if (r_length) *r_length = n->nhdr + n->len;
...
gpg_error_t ksba_cert_hash(ksba_cert_t cert, int what,
                           void (*hasher)(void *, const void *, size_t length),
                           void *hasher_arg) {
  AsnNode n;

  if (!cert /*|| !hasher*/) return GPG_ERR_INV_VALUE;
  if (!cert->initialized) return GPG_ERR_NO_DATA;

  n = _ksba_asn_find_node(
      cert->root, what == 1 ? "Certificate.tbsCertificate" : "Certificate");
  if (!n) return GPG_ERR_NO_VALUE; /* oops - should be there */
  if (n->off == -1) {
...
const char *ksba_cert_get_digest_algo(ksba_cert_t cert) {
  ...
  if (!cert) return NULL; /* Ooops (can't set cert->last_error :-().  */

  if (!cert->initialized) {
    cert->last_error = GPG_ERR_NO_DATA;
    return NULL;
  }

  if (cert->cache.digest_algo) return cert->cache.digest_algo;

  /*   n = _ksba_asn_find_node (cert->root, */
  /*                            "Certificate.signatureAlgorithm.algorithm"); */
  /*   algo = _ksba_oid_node_to_str (cert->image, n); */
  /*   if (!algo) */
  /*     cert->last_error = GPG_ERR_UNKNOWN_ALGORITHM; */
  /*   else  */
...
ksba_sexp_t ksba_cert_get_serial(ksba_cert_t cert) {
  ...
  if (!cert || !cert->initialized) return NULL;

  n = _ksba_asn_find_node(cert->root,
                          "Certificate.tbsCertificate.serialNumber");
  if (!n) return NULL; /* oops - should be there */
...
gpg_error_t _ksba_cert_get_serial_ptr(ksba_cert_t cert,
  ...
  if (!cert || !cert->initialized || !ptr || !length) return GPG_ERR_INV_VALUE;
  n = _ksba_asn_find_node(cert->root,
                          "Certificate.tbsCertificate.serialNumber");
...
gpg_error_t _ksba_cert_get_subject_dn_ptr(ksba_cert_t cert,
  ...
  if (!n || !n->down) return GPG_ERR_NO_VALUE; /* oops - should be there */
  n = n->down;                                 /* dereference the choice node */
  if (n->off == -1) return GPG_ERR_NO_VALUE;
...
static gpg_error_t get_name(ksba_cert_t cert, int idx, int use_subject,
                            char **result) {
  gpg_error_t err;
  char *p;
  int i;
  ...
  if (!cert || !cert->initialized || !result) return GPG_ERR_INV_VALUE;
  if (idx < 0) return GPG_ERR_INV_INDEX;

  *result = NULL;
  if (!idx) { /* Get the required DN */
    AsnNode n;

    n = _ksba_asn_find_node(
        cert->root, (use_subject ? "Certificate.tbsCertificate.subject"
  ...
  for (i = 0;
       !(err = ksba_cert_get_extension(cert, i, &oid, NULL, &off, &derlen));
       i++) {
    if (!strcmp(oid,
                (use_subject ? oidstr_subjectAltName : oidstr_issuerAltName)))
      break;
  }
  if (err) return err; /* no alt name or error*/
  ...
  seqlen = ti.length;
  if (seqlen > derlen) return GPG_ERR_BAD_BER;
  if (!seqlen) return GPG_ERR_INV_CERT_OBJ; /* empty sequence is not allowed */
  ...
  while (seqlen) {
    ...
    if (ti.ndef) return GPG_ERR_NOT_DER_ENCODED;
    if (seqlen < ti.nhdr) return GPG_ERR_BAD_BER;
    seqlen -= ti.nhdr;
    if (seqlen < ti.length) return GPG_ERR_BAD_BER;
    seqlen -= ti.length;
    ...
    if (!(ti.tag == 1 || ti.tag == 2 || ti.tag == 6))
    ...
    else if (ti.tag == 1) { /* rfc822Name - this is an implicit IA5_STRING */
      ...
      *result = p;
      return 0;
               ...
               ti.tag ==
                   6) { /* dNSName or URI - this are implicit IA5_STRINGs */
...
gpg_error_t ksba_cert_get_validity(ksba_cert_t cert, int what,
  ...
  if (!cert || what < 0 || what > 1) return GPG_ERR_INV_VALUE;
  *timebuf = 0;
  if (!cert->initialized) return GPG_ERR_NO_DATA;
  ...
  if (!n) return 0; /* no value available */
  ...
  return _ksba_asntime_to_iso((const char *)(cert->image + n->off + n->nhdr),
                              n->len, n->type == TYPE_UTC_TIME, timebuf);
}
...
ksba_sexp_t ksba_cert_get_public_key(ksba_cert_t cert) {
  ...
  if (!n) {
    cert->last_error = GPG_ERR_NO_VALUE;
    return NULL;
  }

  err = _ksba_keyinfo_to_sexp(cert->image + n->off, n->nhdr + n->len, &string);
  if (err) {
    cert->last_error = err;
    return NULL;
  }
...
gpg_error_t _ksba_cert_get_public_key_ptr(ksba_cert_t cert,
  ...
  if (!n || !n->down || !n->down->right)
    return GPG_ERR_NO_VALUE; /* oops - should be there */
  n = n->down->right;
  if (n->off == -1) return GPG_ERR_NO_VALUE;
  *ptr = cert->image + n->off + n->nhdr;
  *length = n->len;
...
static gpg_error_t read_extensions(ksba_cert_t cert) {
  ...
  for (count = 0, n = start; n; n = n->right) count++;
  if (!count) {
  ...
  cert->cache.extns =
      (cert_extn_info *)xtrycalloc(count, sizeof *cert->cache.extns);
  if (!cert->cache.extns) return GPG_ERR_ENOMEM;
  cert->cache.n_extns = count;

  {
    for (count = 0; start; start = start->right, count++) {
      n = start->down;
      if (!n || n->type != TYPE_OBJECT_ID) goto no_value;

      cert->cache.extns[count].oid = _ksba_oid_node_to_str(cert->image, n);
      if (!cert->cache.extns[count].oid) goto no_value;
      ...
      if (!n || n->type != TYPE_OCTET_STRING || n->off == -1) goto no_value;
...
gpg_error_t ksba_cert_get_extension(ksba_cert_t cert, int idx,
  ...
  if (!cert) return GPG_ERR_INV_VALUE;
  if (!cert->initialized) return GPG_ERR_NO_DATA;
  ...
  if (idx == cert->cache.n_extns) return GPG_ERR_EOF; /* No more extensions. */

  if (idx < 0 || idx >= cert->cache.n_extns) return GPG_ERR_INV_INDEX;

  if (r_oid) *r_oid = cert->cache.extns[idx].oid;
  if (r_crit) *r_crit = cert->cache.extns[idx].crit;
  if (r_deroff) *r_deroff = cert->cache.extns[idx].off;
  if (r_derlen) *r_derlen = cert->cache.extns[idx].len;
  return 0;
}
...
gpg_error_t ksba_cert_is_ca(ksba_cert_t cert, int *r_ca, int *r_pathlen) {
  ...
  /* set default values */
  if (r_ca) *r_ca = 0;
  if (r_pathlen) *r_pathlen = -1;
  for (idx = 0;
       !(err = ksba_cert_get_extension(cert, idx, &oid, &crit, &off, &derlen));
       idx++) {
    if (!strcmp(oid, oidstr_basicConstraints)) break;
  }
  if (err == GPG_ERR_EOF) return 0; /* no such constraint */
  if (err) return err;

  /* check that there is only one */
  for (idx++;
       !(err = ksba_cert_get_extension(cert, idx, &oid, NULL, NULL, NULL));
       idx++) {
    if (!strcmp(oid, oidstr_basicConstraints)) return GPG_ERR_DUP_VALUE;
  }
  ...
  if (ti.klasse == CLASS_UNIVERSAL && ti.tag == TYPE_BOOLEAN) {
    if (ti.length != 1) return GPG_ERR_ENCODING_PROBLEM;
    if (r_ca) *r_ca = !!*der;
    der++;
    derlen--;
    if (!seqlen) return 0; /* ready (no pathlength) */
...
gpg_error_t ksba_cert_get_key_usage(ksba_cert_t cert, unsigned int *r_flags) {
  ...
  if (!r_flags) return GPG_ERR_INV_VALUE;
  *r_flags = 0;
  for (idx = 0;
       !(err = ksba_cert_get_extension(cert, idx, &oid, &crit, &off, &derlen));
       idx++) {
    if (!strcmp(oid, oidstr_keyUsage)) break;
  }
  if (err == GPG_ERR_EOF || err == GPG_ERR_NO_VALUE)
    return GPG_ERR_NO_DATA; /* no key usage */
  if (err) return err;
  ...
  for (idx++;
       !(err = ksba_cert_get_extension(cert, idx, &oid, NULL, NULL, NULL));
       idx++) {
    if (!strcmp(oid, oidstr_keyUsage)) return GPG_ERR_DUP_VALUE;
  }
  ...
  if (!ti.length || ti.length > derlen)
    return GPG_ERR_ENCODING_PROBLEM; /* number of unused bits missing */
  unused = *der++;
  derlen--;
  ti.length--;
  ...
  /* the first octet */
  if (!ti.length) return 0; /* no bits set */
  bits = *der++;
  derlen--;
  ti.length--;
  if (full)
  ...
  if (bits & 0x80) *r_flags |= KSBA_KEYUSAGE_DIGITAL_SIGNATURE;
  if (bits & 0x40) *r_flags |= KSBA_KEYUSAGE_NON_REPUDIATION;
  if (bits & 0x20) *r_flags |= KSBA_KEYUSAGE_KEY_ENCIPHERMENT;
  if (bits & 0x10) *r_flags |= KSBA_KEYUSAGE_DATA_ENCIPHERMENT;
  if (bits & 0x08) *r_flags |= KSBA_KEYUSAGE_KEY_AGREEMENT;
  if (bits & 0x04) *r_flags |= KSBA_KEYUSAGE_KEY_CERT_SIGN;
  if (bits & 0x02) *r_flags |= KSBA_KEYUSAGE_CRL_SIGN;
  if (bits & 0x01) *r_flags |= KSBA_KEYUSAGE_ENCIPHER_ONLY;
  ...
  if (bits & 0x80) *r_flags |= KSBA_KEYUSAGE_DECIPHER_ONLY;

  return 0;
}

/* Note, that this helper is also used for ext_key_usage. */
static gpg_error_t append_cert_policy(char **policies, const char *oid,
                                      int crit) {
  char *p;

  if (!*policies) {
    *policies = (char *)xtrymalloc(strlen(oid) + 4);
    if (!*policies) return GPG_ERR_ENOMEM;
    p = *policies;
  } else {
    char *tmp =
        (char *)xtryrealloc(*policies, strlen(*policies) + 1 + strlen(oid) + 4);
    if (!tmp) return GPG_ERR_ENOMEM;
    *policies = tmp;
...
gpg_error_t ksba_cert_get_cert_policies(ksba_cert_t cert, char **r_policies) {
  ...
  if (!cert || !r_policies) return GPG_ERR_INV_VALUE;
  *r_policies = NULL;
  ...
  if (err == GPG_ERR_EOF) err = 0;
  if (!*r_policies || err == GPG_ERR_NO_VALUE) err = GPG_ERR_NO_DATA;
...
gpg_error_t ksba_cert_get_ext_key_usages(ksba_cert_t cert, char **result) {
  ...
  if (!cert || !result) return GPG_ERR_INV_VALUE;
  *result = NULL;
  ...
  if (err == GPG_ERR_EOF) err = 0;
  if (!*result || err == GPG_ERR_NO_VALUE) err = GPG_ERR_NO_DATA;
...
static gpg_error_t parse_distribution_point(const unsigned char *der,
                                            size_t derlen,
                                            ksba_name_t *distpoint,
                                            ksba_name_t *issuer,
  ...
  if (ti.tag == 0 && derlen) { /* distributionPointName */
    err = _ksba_ber_parse_tl(&der, &derlen, &ti);
    if (err) return err;
    if (ti.klasse != CLASS_CONTEXT)
      return GPG_ERR_INV_CERT_OBJ; /* we expected a tag */
    if (ti.ndef) return GPG_ERR_NOT_DER_ENCODED;
    if (derlen < ti.nhdr) return GPG_ERR_BAD_BER;
    if (derlen < ti.length) return GPG_ERR_BAD_BER;
  ...
  if (ti.tag == 1 && derlen) { /* reasonFlags */
    unsigned int bits, mask;
    int i, unused, full;

    if (!ti.length || ti.length > derlen) return GPG_ERR_ENCODING_PROBLEM;
    unused = *der++;
    derlen--;
    ti.length--;
    if ((!ti.length && unused) || unused / 8 > ti.length)
    ...
    if (ti.length && reason) {
      ...
      if (bits & 0x80) *reason |= KSBA_CRLREASON_UNSPECIFIED;
      if (bits & 0x40) *reason |= KSBA_CRLREASON_KEY_COMPROMISE;
      if (bits & 0x20) *reason |= KSBA_CRLREASON_CA_COMPROMISE;
      if (bits & 0x10) *reason |= KSBA_CRLREASON_AFFILIATION_CHANGED;
      if (bits & 0x08) *reason |= KSBA_CRLREASON_SUPERSEDED;
      if (bits & 0x04) *reason |= KSBA_CRLREASON_CESSATION_OF_OPERATION;
      if (bits & 0x02) *reason |= KSBA_CRLREASON_CERTIFICATE_HOLD;
    }
  ...
  if (derlen) return GPG_ERR_INV_CERT_OBJ;
...
gpg_error_t ksba_cert_get_crl_dist_point(ksba_cert_t cert, int idx,
  ...
  if (r_distpoint) *r_distpoint = NULL;
  if (r_issuer) *r_issuer = NULL;
  if (r_reason) *r_reason = 0;
  ...
  for (myidx = 0; !(err = ksba_cert_get_extension(cert, myidx, &oid, &crit,
    ...
    if (!strcmp(oid, oidstr_crlDistributionPoints)) {
      ...
      while (seqlen) {
        ...
        if (idx) { /* skip because we are not yet at the desired index */
          der += ti.length;
          derlen -= ti.length;
          seqlen -= ti.length;
          idx--;
        ...
        if (!ti.length) return 0;

        err = parse_distribution_point(der, ti.length, r_distpoint, r_issuer,
                                       r_reason);
        if (err && r_distpoint) {
...
/* Return the authorityKeyIdentifier in R_NAME and R_SERIAL or/and in
   ...
   is NULL. */
gpg_error_t ksba_cert_get_auth_key_id(ksba_cert_t cert, ksba_sexp_t *r_keyid,
                                      ksba_name_t *r_name,
  ...
  if (r_keyid) *r_keyid = NULL;
  if (!r_name || !r_serial) return GPG_ERR_INV_VALUE;
  *r_name = NULL;
  *r_serial = NULL;
  ...
  for (idx = 0;
       !(err = ksba_cert_get_extension(cert, idx, &oid, &crit, &off, &derlen));
       idx++) {
    if (!strcmp(oid, oidstr_authorityKeyIdentifier)) break;
  }
  if (err == GPG_ERR_EOF || err == GPG_ERR_NO_VALUE)
  ...
  for (idx++;
       !(err = ksba_cert_get_extension(cert, idx, &oid, NULL, NULL, NULL));
       idx++) {
    if (!strcmp(oid, oidstr_authorityKeyIdentifier)) return GPG_ERR_DUP_VALUE;
  }
  ...
  if (!(ti.klasse == CLASS_UNIVERSAL && ti.tag == TYPE_SEQUENCE &&
    ...
    return GPG_ERR_INV_CERT_OBJ;
  if (ti.ndef) return GPG_ERR_NOT_DER_ENCODED;
  if (ti.length > derlen) return GPG_ERR_BAD_BER;
  ...
  if (ti.tag == 0) { /* keyIdentifier:  Save it away and skip over it. */
    keyid_der = der;
    ...
    if (r_keyid && !derlen) goto build_keyid;
    if (!derlen) return GPG_ERR_NO_DATA; /* not available */
    err = _ksba_ber_parse_tl(&der, &derlen, &ti);
    if (err) return err;
    if (ti.klasse != CLASS_CONTEXT)
      return GPG_ERR_INV_CERT_OBJ; /* we expected a tag */
  ...
  if (ti.tag != 1 || !derlen) return GPG_ERR_INV_CERT_OBJ;
...
build_keyid:
  if (r_keyid && keyid_der && keyid_derlen) {
    ...
    *r_keyid = (ksba_sexp_t)xtrymalloc(numbuflen + keyid_derlen + 2);
    if (!*r_keyid) return GPG_ERR_ENOMEM;
...
static gpg_error_t get_simple_octet_string_ext(ksba_cert_t cert,
  ...
  if (!r_data) return GPG_ERR_INV_VALUE;
  *r_data = NULL;

  for (idx = 0; !(err = ksba_cert_get_extension(cert, idx, &tmpoid, &crit, &off,
                                                &derlen));
       idx++) {
    if (!strcmp(tmpoid, oid)) break;
  }
  if (err) {
    if (err == GPG_ERR_EOF || err == GPG_ERR_NO_VALUE) return GPG_ERR_NO_DATA;
    return err;
  }

  /* Check that there is only one */
  for (idx++;
       !(err = ksba_cert_get_extension(cert, idx, &tmpoid, NULL, NULL, NULL));
       idx++) {
    if (!strcmp(tmpoid, oid)) return GPG_ERR_DUP_VALUE;
  }
...
gpg_error_t ksba_cert_get_subj_key_id(ksba_cert_t cert, int *r_crit,
                                      ksba_sexp_t *r_keyid) {
  return get_simple_octet_string_ext(cert, oidstr_subjectKeyIdentifier, r_crit,
                                     r_keyid);
}
 ...
 */
static gpg_error_t get_info_access(ksba_cert_t cert, int idx, int mode,
  ...
  *location = NULL;
  ...
  if (!cert || !cert->initialized) return GPG_ERR_INV_VALUE;
  if (idx < 0) return GPG_ERR_INV_INDEX;
  ...
  for (myidx = 0; !(err = ksba_cert_get_extension(cert, myidx, &oid, &crit,
       ...
       myidx++) {
    if (!strcmp(oid,
                (mode == 0) ? oidstr_authorityInfoAccess
      ...
      while (seqlen) {
        ...
        *method = ksba_oid_to_str((const char *)(der), ti.length);
        if (!*method) return GPG_ERR_ENOMEM;
        der += ti.length;
        derlen -= ti.length;
...
gpg_error_t ksba_cert_get_authority_info_access(ksba_cert_t cert, int idx,
  ...
  if (!r_method || !r_location) return GPG_ERR_INV_VALUE;
