...
gpg_error_t ksba_certreq_new(ksba_certreq_t *r_cr) {
  *r_cr = (ksba_certreq_t)xtrycalloc(1, sizeof **r_cr);
  if (!*r_cr) return gpg_error_from_errno(errno);
...
gpg_error_t ksba_certreq_set_writer(ksba_certreq_t cr, ksba_writer_t w) {
  if (!cr || !w) return GPG_ERR_INV_VALUE;
  cr->writer = w;
  return 0;
}
...
gpg_error_t ksba_certreq_set_serial(ksba_certreq_t cr, ksba_const_sexp_t sn) {
  ...
  if (!cr || !sn || !p || *p != '(') return GPG_ERR_INV_VALUE;

  p++;
  n = strtoul(p, &endp, 10);
  p = endp;
  if (*p++ != ':' || !n) return GPG_ERR_INV_VALUE;

  /* Remove invalid leading zero bytes.  */
  for (; n > 1 && !*p && !(p[1] & 0x80); n--, p++)
    ;

  if (cr->x509.serial.der) return GPG_ERR_CONFLICT; /* Already set */
  cr->x509.serial.der = (char *)xtrymalloc(n);
  if (!cr->x509.serial.der) return gpg_error_from_syserror();
...
gpg_error_t ksba_certreq_set_issuer(ksba_certreq_t cr, const char *name) {
  if (!cr || !name) return GPG_ERR_INV_VALUE;
  if (cr->x509.issuer.der) return GPG_ERR_CONFLICT; /* Already set */
  return _ksba_dn_from_str(name, &cr->x509.issuer.der, &cr->x509.issuer.derlen);
}
...
gpg_error_t ksba_certreq_set_validity(ksba_certreq_t cr, int what,
  ...
  if (!cr || what < 0 || what > 1 || !timebuf ||
      _ksba_assert_time_format(timebuf))
...
gpg_error_t ksba_certreq_set_siginfo(ksba_certreq_t cr,
  ...
  if (!cr || !siginfo) return GPG_ERR_INV_VALUE;
  xfree(cr->x509.siginfo.der);
...
gpg_error_t ksba_certreq_add_subject(ksba_certreq_t cr, const char *name) {
  ...
  namelen = strlen(name);
  if (*name == '<' && name[namelen - 1] == '>' && namelen >= 4 &&
      strchr(name, '@')) {
    name++;
    namelen -= 2;
    tag = 1; /* rfc822Name */
    ...
    name = endp;
    if (!namelen || *name != ':') return GPG_ERR_INV_SEXP;
    name++;
  ...
  if (!gn) return gpg_error_from_errno(errno);
  gn->tag = tag;
  gn->datalen = n1;
...
static gpg_error_t add_general_names_to_extn(ksba_certreq_t cr,
                                             struct general_names_s *gnames,
                                             const char *oid) {
  ...
  n1 = 0;
  for (g = gnames; g; g = g->next) n1 += g->datalen;
  ...
  if (!e) return gpg_error_from_errno(errno);
  e->oid = oid;
  e->critical = 0;
  e->derlen = n2;
  der = e->der;
  n = _ksba_ber_encode_tl(der, TYPE_SEQUENCE, CLASS_UNIVERSAL, 1, n1);
  if (!n) return GPG_ERR_BUG; /* (no need to cleanup after a bug) */
  der += n;
...
gpg_error_t ksba_certreq_set_public_key(ksba_certreq_t cr,
  ...
  if (!cr) return GPG_ERR_INV_VALUE;
  xfree(cr->key.der);
  cr->key.der = NULL;
...
/* Generic function to add an extension to a certificate request.  The
   ...
   critical. */
gpg_error_t ksba_certreq_add_extension(ksba_certreq_t cr, const char *oid,
                                       int is_crit, const void *der,
                                       size_t derlen) {
  size_t oidlen;
  struct extn_list_s *e;

  if (!cr || !oid || !*oid || !der || !derlen) return GPG_ERR_INV_VALUE;
...
gpg_error_t ksba_certreq_set_sig_val(ksba_certreq_t cr,
  ...
  s++;
  if (n != 7 || memcmp(s, "sig-val", 7)) return GPG_ERR_UNKNOWN_SEXP;
  s += 7;
  ...
  if (n == 3 && s[0] == 'r' && s[1] == 's' &&
    ...
    if (!cr->sig_val.algo) return GPG_ERR_ENOMEM;
  } else {
  ...
  if (!n || *s != ':') return GPG_ERR_INV_SEXP;
  s++;
  if (n > 1 && !*s) { /* We might have a leading zero due to the way we encode
  ...
  if (!cr->sig_val.value) return GPG_ERR_ENOMEM;
...
static gpg_error_t build_extensions(ksba_certreq_t cr, int certmode,
  ...
  *r_derlen = 0;
  ...
  for (e = cr->extn_list; e; e = e->next) {
    ...
    if (!err) err = ksba_writer_write(w, p, n);
    xfree(p);

    if (e->critical) {
      err = _ksba_ber_write_tl(w, TYPE_BOOLEAN, CLASS_UNIVERSAL, 0, 1);
      if (!err) err = ksba_writer_write(w, "\xff", 1);
      if (err) goto leave;
    }

    err =
        _ksba_ber_write_tl(w, TYPE_OCTET_STRING, CLASS_UNIVERSAL, 0, e->derlen);
    if (!err) err = ksba_writer_write(w, e->der, e->derlen);
    if (err) goto leave;
    ...
    if (!err) err = ksba_writer_write(writer, p, n);
    xfree(p);
    p = NULL;
    if (err) goto leave;
  }
  ...
  if (err) goto leave;
  err = _ksba_ber_write_tl(writer, TYPE_SEQUENCE, CLASS_UNIVERSAL, 1, valuelen);
  if (!err) err = ksba_writer_write(writer, value, valuelen);
  if (err) goto leave;
  ...
  if (!certmode) {
    ...
    if (err) goto leave;
    err =
        _ksba_ber_write_tl(writer, TYPE_SEQUENCE, CLASS_UNIVERSAL, 1, valuelen);
    if (!err) err = ksba_writer_write(writer, value, valuelen);
    if (err) goto leave;
...
static gpg_error_t build_cri(ksba_certreq_t cr) {
  ...
  if (certmode) {
    ...
    if (!err)
      err = ksba_writer_write(writer, cr->x509.serial.der,
                              cr->x509.serial.derlen);
    if (err) goto leave;
    ...
    if (!cr->x509.siginfo.der)
      ...
      err = ksba_writer_write(writer, cr->x509.siginfo.der,
                              cr->x509.siginfo.derlen);
    if (err) goto leave;
    ...
    if (cr->x509.issuer.der)
      err = ksba_writer_write(writer, cr->x509.issuer.der,
  ...
  if (cr->extn_list) {
    ...
    if (err) goto leave;
    err = _ksba_ber_write_tl(writer, certmode ? 3 : 0, CLASS_CONTEXT, 1,
                             valuelen);
    if (!err) err = ksba_writer_write(writer, value, valuelen);
    if (err) goto leave;
  } else { /* We can't write an object of length zero using our ber_write
              function.  So we must open encode it. */
    err = ksba_writer_write(writer, certmode ? "\xa3\x02\x30" : "\xa0\x02\x30",
                            4);
    if (err) goto leave;
  }
  ...
  cr->cri.der =
      (unsigned char *)ksba_writer_snatch_mem(writer, &cr->cri.derlen);
  if (!cr->cri.der) err = GPG_ERR_ENOMEM;
...
static gpg_error_t hash_cri(ksba_certreq_t cr) {
  if (!cr->hash_fnc) return GPG_ERR_MISSING_ACTION;
  if (!cr->cri.der) return GPG_ERR_INV_STATE;
...
static gpg_error_t sign_and_write(ksba_certreq_t cr) {
  ...
  if (!cr->sig_val.algo) return GPG_ERR_MISSING_VALUE;
  err = _ksba_der_write_algorithm_identifier(writer, cr->sig_val.algo, NULL, 0);
  if (err) goto leave;
...
gpg_error_t ksba_certreq_build(ksba_certreq_t cr,
  ...
  enum { sSTART, sHASHING, sGOTSIG, sERROR } state = sERROR;
  gpg_error_t err = 0;
  ksba_stop_reason_t stop_reason;

  if (!cr || !r_stopreason) return GPG_ERR_INV_VALUE;

  if (!cr->any_build_done) { /* first time initialization of the stop reason */
