...
AsnNode _ksba_asn_new_node(node_type_t type) { return add_node(type); }
...
static AsnNode copy_node(const AsnNode s) {
  ...
  if (s->name) d->name = xstrdup(s->name);
  d->flags = s->flags;
  copy_value(d, s);
  return d;
}
...
void _ksba_asn_set_name(AsnNode node, const char *name) {
  ...
  if (name && *name) node->name = xstrdup(name);
}
...
static AsnNode find_node(AsnNode root, const char *name, int resolve) {
  ...
  /* find the first part */
  s = name;
  for (i = 0; *s && *s != '.' && i < DIM(buf) - 1; s++) buf[i++] = *s;
  buf[i] = 0;
  ...
  while (p && *s) {
    ...
    if (!p->down) return NULL; /* not found */
    p = p->down;
    ...
    if (!*buf) {
      ...
      for (; p; p = p->right) {
        if (p->name && !strcmp(p->name, buf)) break;
        if (resolve && p->name && p->type == TYPE_IDENTIFIER) {
          AsnNode p2;

          p2 = resolve_identifier(root, p, 0);
          if (p2 && p2->name && !strcmp(p2->name, buf)) break;
        }
      }
...
void _ksba_asn_node_dump(AsnNode p, FILE *fp) {
  ...
  switch (p->type) {
    ...
    case TYPE_UTF8_STRING:
      ...
      break;
    case TYPE_NUMERIC_STRING:
      typestr = "NUMERIC_STRING";
      break;
    case TYPE_PRINTABLE_STRING:
      typestr = "PRINTABLE_STRING";
      break;
    case TYPE_TELETEX_STRING:
      typestr = "TELETEX_STRING";
      break;
    case TYPE_IA5_STRING:
      typestr = "IA5_STRING";
      break;
  ...
  if (p->flags.explicit_x) fputs(",explicit", fp);
  if (p->flags.implicit) fputs(",implicit", fp);
  if (p->flags.is_implicit) fputs(",is_implicit", fp);
  if (p->flags.has_tag) fputs(",tag", fp);
  if (p->flags.has_default) fputs(",default", fp);
  if (p->flags.is_true) fputs(",true", fp);
  if (p->flags.is_false) fputs(",false", fp);
  if (p->flags.has_list) fputs(",list", fp);
  if (p->flags.has_min_max) fputs(",min_max", fp);
  if (p->flags.is_optional) fputs(",optional", fp);
  if (p->flags.one_param) fputs(",1_param", fp);
  if (p->flags.has_size) fputs(",size", fp);
  if (p->flags.has_defined_by) fputs(",def_by", fp);
  if (p->flags.has_imports) fputs(",imports", fp);
  if (p->flags.assignment) fputs(",assign", fp);
  if (p->flags.in_set) fputs(",in_set", fp);
  if (p->flags.in_choice) fputs(",in_choice", fp);
  if (p->flags.in_array) fputs(",in_array", fp);
  if (p->flags.not_used) fputs(",not_used", fp);
  if (p->flags.skip_this) fputs(",[skip]", fp);
  if (p->flags.is_any) fputs(",is_any", fp);
  if (p->off != -1) fprintf(fp, " %d.%d.%d", p->off, p->nhdr, p->len);
}
...
void ksba_asn_tree_dump(ksba_asn_tree_t tree, const char *name, FILE *fp) {
  ...
  if (!tree || !tree->parse_tree) return;
  ...
  if (expand) root = _ksba_asn_expand_tree(root, NULL);

  p = root;
  while (p) {
    for (k = 0; k < indent; k++) fprintf(fp, " ");
    _ksba_asn_node_dump(p, fp);
    putc('\n', fp);
  ...
  if (expand) _ksba_asn_release_nodes(root);
}

int _ksba_asn_delete_structure(AsnNode root) {
  AsnNode p, p2, p3;

  if (root == NULL) return GPG_ERR_ELEMENT_NOT_FOUND;
...
int _ksba_asn_check_identifier(AsnNode node) {
  ...
  if (!node) return GPG_ERR_ELEMENT_NOT_FOUND;
  ...
  for (p = node; p; p = _ksba_asn_walk_tree(node, p)) {
    if (p->type == TYPE_IDENTIFIER && p->valuetype == VALTYPE_CSTR) {
      ...
      /*            fputs (")\n", stdout); */
               ...
               p->flags.assignment) { /* an object ID in an assignment */
      p2 = p->down;
      if (p2 && (p2->type == TYPE_CONSTANT)) {
        if (p2->valuetype == VALTYPE_CSTR &&
            !isdigit(
                p2->value
                    .v_cstr[0])) { /* the first constand below is a reference */
          if (strlen(node->name) + strlen(p->value.v_cstr) + 2 > DIM(name2))
            return GPG_ERR_BUG; /* well identifier too long */
          strcpy(name2, node->name);
          strcat(name2, ".");
          ...
          if (!p2) {
            ...
            return GPG_ERR_IDENTIFIER_NOT_FOUND;
          } else if (p2->type != TYPE_OBJECT_ID || !p2->flags.assignment) {
            fprintf(stderr, "`%s' is not an object id\n", name2);
            return GPG_ERR_IDENTIFIER_NOT_FOUND;
          }
          /*                    fprintf (stdout,"found objid reference for `%s'
           * (", name2); */
          /*                    print_node (p2, stdout); */
          /*                    fputs (")\n", stdout); */
        }
...
int _ksba_asn_change_integer_value(AsnNode node) {
  ...
  if (node == NULL) return GPG_ERR_ELEMENT_NOT_FOUND;
...
int _ksba_asn_expand_object_id(AsnNode node) {
  ...
  if (!node) return GPG_ERR_ELEMENT_NOT_FOUND;
  if (!node->name) return GPG_ERR_INV_VALUE;
  if (strlen(node->name) >= DIM(name_root) - 1) return GPG_ERR_GENERAL;
    ...
    if (p->type == TYPE_OBJECT_ID && p->flags.assignment) {
      ...
      if (p2 && p2->type == TYPE_CONSTANT) {
        if (p2->valuetype == VALTYPE_CSTR && !isdigit(p2->value.v_cstr[0])) {
          ...
          if (!p3 || p3->type != TYPE_OBJECT_ID || !p3->flags.assignment)
          ...
          while (p4) {
            if (p4->type == TYPE_CONSTANT) {
              ...
              _ksba_asn_set_value(p5, VALTYPE_CSTR, p4->value.v_cstr, 0);
              if (p2 == p) {
                set_right(p5, p->down);
                set_down(p, p5);
              } else {
...
void _ksba_asn_set_default_tag(AsnNode node) {
  ...
  for (p = node; p; p = _ksba_asn_walk_tree(node, p)) {
    if (p->type == TYPE_TAG && !p->flags.explicit_x && !p->flags.implicit) {
...
void _ksba_asn_type_set_config(AsnNode node) {
  ...
  for (p = node; p; p = _ksba_asn_walk_tree(node, p)) {
    if (p->type == TYPE_SET) {
      ...
      for (p2 = p->down; p2; p2 = p2->right) p2->flags.in_array = 1;
    } else if (p->type ==
               TYPE_ANY) { /* Help the DER encoder to track ANY tags */
      p->flags.is_any = 1;
    }
  }
}
...
static AsnNode copy_tree(AsnNode src_root, AsnNode s) {
  ...
  for (; s; s = s->right) {
    ...
    if (down) {
      ...
      if (tmp) {
        if (link_nextp) *link_nextp = tmp;
        link_nextp = &tmp->link_next;
        while (*link_nextp) link_nextp = &(*link_nextp)->link_next;
      }

      if (d->down && tmp) { /* Need to merge it with the existing down */
        AsnNode x;
...
static AsnNode do_expand_tree(AsnNode src_root, AsnNode s, int depth) {
  ...
  for (; s; s = depth ? s->right : NULL) {
    ...
    if (s->type == TYPE_IDENTIFIER) {
      ...
      if (link_nextp) *link_nextp = d;
      link_nextp = &d->link_next;
      if (s->flags.is_optional) d->flags.is_optional = 1;
      if (s->flags.in_choice) d->flags.in_choice = 1;
      if (s->flags.in_array) d->flags.in_array = 1;
      if (s->flags.is_implicit) d->flags.is_implicit = 1;
      if (s->flags.is_any) d->flags.is_any = 1;
...
AsnNode _ksba_asn_find_type_value(const unsigned char *image, AsnNode root,
                                  int idx, const void *oidbuf, size_t oidlen) {
  AsnNode n, noid;

  if (!image || !root) return NULL;

  for (n = root; n; n = _ksba_asn_walk_tree(root, n)) {
    if (n->type == TYPE_SEQUENCE && n->down &&
        n->down->type == TYPE_OBJECT_ID) {
      noid = n->down;
      if (noid->off != -1 && noid->len == oidlen &&
          !memcmp(image + noid->off + noid->nhdr, oidbuf, oidlen) &&
          noid->right) {
        if (!idx--) return noid->right;
      }
    }
  }
  return NULL;
