...
static size_t insert_string(const char *name) {
  ...
  if (string_table_offset && !*name) return 0;

  for (item = string_table, off = 0; item; item = item->next) {
    for (n = 0; item->name[n]; n++)
      if (!strcmp(item->name + n, name)) return off + n;
...
static void sort_string_table(void) {
  ...
  if (!string_table || !string_table->next) return; /* Nothing to sort.  */

  for (item = string_table, arraylen = 0; item; item = item->next) arraylen++;
  ...
  for (i = 0; i < arraylen; i++) insert_string(array[i]->name);
  xfree(array);
  /* for (item = string_table,arraylen = 0; item; item = item->next) */
  /*   fprintf (stderr, "  `%s'\n", item->name); */
}
...
static struct name_list_s *create_static_structure(AsnNode pointer,
  ...
  for (p = pointer; p; p = _ksba_asn_walk_tree(pointer, p)) {
    ...
    if (p->valuetype == VALTYPE_CSTR)
      fprintf(fp, ",%u", (unsigned int)insert_string(p->value.v_cstr));
    else if (p->valuetype == VALTYPE_LONG && p->type == TYPE_INTEGER &&
             p->flags.assignment) {
...
int main(int argc, char **argv) {
  ...
  if (!argc)
    ...
    for (i = 0; i < argc; i++) one_file(argv[i], &count, nullfp);
    fclose(nullfp);
