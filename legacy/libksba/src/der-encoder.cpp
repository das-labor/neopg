...
static gpg_error_t store_value(AsnNode node, const void *buffer,
                               size_t length) {
...
void _ksba_der_encoder_release(DerEncoder d) { xfree(d); }
...
gpg_error_t _ksba_der_encoder_set_writer(DerEncoder d, ksba_writer_t w) {
  if (!d || !w) return GPG_ERR_INV_VALUE;
  if (d->writer) return GPG_ERR_CONFLICT; /* reader already set */
...
gpg_error_t _ksba_der_write_algorithm_identifier(ksba_writer_t w,
  ...
  if (!err) err = ksba_writer_write(w, buf, len);
  if (err) goto leave;
  ...
  if (no_null)
    ...
    err = _ksba_ber_write_tl(w, TYPE_OCTET_STRING, CLASS_UNIVERSAL, 0, parmlen);
    if (!err) err = ksba_writer_write(w, parm, parmlen);
  } else {
...
/* Copy all values from the tree SRC (with values store in SRCIMAGE)
   to the tree DST */
gpg_error_t _ksba_der_copy_tree(AsnNode dst_root, AsnNode src_root,
  ...
  while (s && d && (s->type == d->type || d->flags.is_any)) {
    if (d->flags.is_any) d->type = s->type;

    if (s->flags.in_array && s->right) {
      if (!_ksba_asn_insert_copy(d)) return GPG_ERR_ENOMEM;
    }
...
gpg_error_t _ksba_der_store_time(AsnNode node, const ksba_isotime_t atime) {
  ...
  if (node->type == TYPE_ANY)
    node->type = need_gen ? TYPE_GENERALIZED_TIME : TYPE_UTC_TIME;
           ...
           TYPE_CHOICE) { /* find a suitable choice to store the value */
    AsnNode n;
  ...
  if (node->type == TYPE_GENERALIZED_TIME || node->type == TYPE_UTC_TIME) {
...
gpg_error_t _ksba_der_store_oid(AsnNode node, const char *oid) {
  ...
  if (node->type == TYPE_ANY) node->type = TYPE_OBJECT_ID;
...
gpg_error_t _ksba_der_store_octet_string(AsnNode node, const char *buf,
                                         size_t len) {
  if (node->type == TYPE_ANY) node->type = TYPE_OCTET_STRING;
...
gpg_error_t _ksba_der_store_sequence(AsnNode node, const unsigned char *buf,
                                     size_t len) {
  if (node->type == TYPE_ANY) node->type = TYPE_PRE_SEQUENCE;
...
gpg_error_t _ksba_der_store_null(AsnNode node) {
  if (node->type == TYPE_ANY) node->type = TYPE_NULL;
...
static void set_nhdr_and_len(AsnNode node, unsigned long length) {
  ...
  if (!node->type /*&& !class*/)
    ...
    buflen +=
        (length <= 0xff ? 2
                        : length <= 0xffff ? 3 : length <= 0xffffff ? 4 : 5);
  }
...
static size_t copy_nhdr_and_len(unsigned char *buffer, AsnNode node) {
  ...
  if (tag < 0x1f) {
    *p = (klasse << 6) | tag;
    if (!_ksba_asn_is_primitive((node_type_t)(tag))) *p |= 0x20;
    p++;
  } else {
...
static unsigned long sum_up_lengths(AsnNode root) {
  ...
  if (root->type == TYPE_NULL) return root->nhdr;

  if (!(n = root->down) || _ksba_asn_is_primitive(root->type))
    len = root->len;
  else {
    for (; n; n = n->right) len += sum_up_lengths(n);
  }
  if (!_ksba_asn_is_primitive(root->type) && root->type != TYPE_CHOICE && len &&
      !root->flags.is_implicit) { /* this is a constructed one */
...
gpg_error_t _ksba_der_encode_tree(AsnNode root, unsigned char **r_image,
  ...
  for (n = root; n; n = _ksba_asn_walk_tree(root, n)) {
    if (_ksba_asn_is_primitive(n->type) && !n->flags.is_implicit &&
  ...
  if (!image) return GPG_ERR_ENOMEM;
  len = 0;
  for (n = root; n; n = _ksba_asn_walk_tree(root, n)) {
    size_t nbytes;
    ...
    len += nbytes;
    if (_ksba_asn_is_primitive(n->type) && n->valuetype == VALTYPE_MEM &&
        n->value.v_mem.len) {
  ...
  *r_image = image;
  if (r_imagelen) *r_imagelen = imagelen;
  return 0;
}
