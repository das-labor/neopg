...
static void parse_skip(unsigned char const **buf, size_t *len,
                       struct tag_info *ti) {
  if (ti->length) {
...
static gpg_error_t parse_sequence(unsigned char const **buf, size_t *len,
                                  struct tag_info *ti) {
  gpg_error_t err;
...
static gpg_error_t parse_enumerated(unsigned char const **buf, size_t *len,
...
static gpg_error_t parse_integer(unsigned char const **buf, size_t *len,
                                 struct tag_info *ti) {
  gpg_error_t err;
...
static gpg_error_t parse_octet_string(unsigned char const **buf, size_t *len,
                                      struct tag_info *ti) {
  gpg_error_t err;
...
static gpg_error_t parse_optional_boolean(unsigned char const **buf,
  ...
  if (err)
    ...
    err = GPG_ERR_BAD_BER;
           ...
           !ti.is_constructed) {
    if (ti.length != 1) err = GPG_ERR_BAD_BER;
    *r_bool = !!**buf;
...
static gpg_error_t parse_object_id_into_str(unsigned char const **buf,
                                            size_t *len, char **oid) {
  struct tag_info ti;
  gpg_error_t err;
...
static gpg_error_t parse_asntime_into_isotime(unsigned char const **buf,
                                              size_t *len,
...
static gpg_error_t parse_context_tag(unsigned char const **buf, size_t *len,
 ...
 */
gpg_error_t ksba_ocsp_new(ksba_ocsp_t *r_ocsp) {
  *r_ocsp = (ksba_ocsp_t)xtrycalloc(1, sizeof **r_ocsp);
  if (!*r_ocsp) return gpg_error_from_syserror();
  return 0;
}
...
gpg_error_t ksba_ocsp_set_digest_algo(ksba_ocsp_t ocsp, const char *oid) {
  if (!ocsp || !oid || !*oid) return GPG_ERR_INV_VALUE;
  if (ocsp->digest_oid) xfree(ocsp->digest_oid);
  ocsp->digest_oid = xtrystrdup(oid);
  if (!ocsp->digest_oid) return gpg_error_from_syserror();
  return 0;
}
...
gpg_error_t ksba_ocsp_add_target(ksba_ocsp_t ocsp, ksba_cert_t cert,
                                 ksba_cert_t issuer_cert) {
  struct ocsp_reqitem_s *ri;

  if (!ocsp || !cert || !issuer_cert) return GPG_ERR_INV_VALUE;

  ri = (ocsp_reqitem_s *)xtrycalloc(1, sizeof *ri);
  if (!ri) return gpg_error_from_syserror();
  ksba_cert_ref(cert);
  ri->cert = cert;
...
size_t ksba_ocsp_set_nonce(ksba_ocsp_t ocsp, unsigned char *nonce,
                           size_t noncelen) {
  if (!ocsp) return 0;
  if (!nonce) return sizeof ocsp->nonce;
  if (noncelen > sizeof ocsp->nonce) noncelen = sizeof ocsp->nonce;
  if (noncelen) {
...
/* Compute the SHA-1 nameHash for the certificate CERT and put it in
   ...
   20 bytes. */
static gpg_error_t issuer_name_hash(ksba_cert_t cert,
                                    unsigned char *sha1_buffer) {
  gpg_error_t err;
  ...
  if (!err) {
    ...
    if (!err && dummy != 20) err = GPG_ERR_BUG;
  }
  return err;
}
...
/* Compute the SHA-1 hash of the public key of CERT and put it in teh
   ...
   bytes. */
static gpg_error_t issuer_key_hash(ksba_cert_t cert,
                                   unsigned char *sha1_buffer) {
  gpg_error_t err;
  const unsigned char *ptr;
  size_t length, dummy;
...
static gpg_error_t write_request_extensions(ksba_ocsp_t ocsp,
                                            ksba_writer_t wout) {
  gpg_error_t err;
  unsigned char *buf;
  size_t buflen;
  ...
  if (!ocsp->noncelen) return 0; /* We do only support the nonce extension.  */

  /* Create writer objects for construction of the extension. */
  err = ksba_writer_new(&w2);
  if (!err) err = ksba_writer_set_mem(w2, 256);
  if (!err) err = ksba_writer_new(&w1);
  if (!err) err = ksba_writer_set_mem(w1, 256);
  if (err) goto leave;
  ...
  if (!err) err = ksba_writer_write(w1, buf, buflen);
  xfree(buf);
  buf = NULL;
  ...
  if (!err)
    err = _ksba_ber_write_tl(w1, TYPE_OCTET_STRING, CLASS_UNIVERSAL, 0,
                             ocsp->noncelen);
  if (!err) err = ksba_writer_write(w1, ocsp->nonce, ocsp->noncelen);
  ...
  if (!err) err = ksba_writer_write(w2, p, derlen);
  xfree(p);
  p = NULL;
  ...
  if (!err) err = ksba_writer_write(w1, p, derlen);
  xfree(p);
  p = NULL;
  ...
  if (!err) err = ksba_writer_write(wout, p, derlen);
  xfree(p);
  p = NULL;
...
gpg_error_t ksba_ocsp_prepare_request(ksba_ocsp_t ocsp) {
  ...
  if (!ocsp) return GPG_ERR_INV_VALUE;

  xfree(ocsp->request_buffer);
  ocsp->request_buffer = NULL;
  ocsp->request_buflen = 0;

  if (!ocsp->requestlist) return GPG_ERR_MISSING_ACTION;

  /* Create three writer objects for construction of the request. */
  err = ksba_writer_new(&w3);
  if (!err) err = ksba_writer_set_mem(w3, 2048);
  if (!err) err = ksba_writer_new(&w2);
  if (!err) err = ksba_writer_new(&w1);
  if (err) goto leave;
  ...
  for (ri = ocsp->requestlist; ri; ri = ri->next) {
    ...
    if (!err) err = ksba_writer_write(w1, ri->issuer_name_hash, 20);
    if (err) goto leave;
    ...
    if (!err)
      err = _ksba_ber_write_tl(w1, TYPE_OCTET_STRING, CLASS_UNIVERSAL, 0, 20);
    if (!err) err = ksba_writer_write(w1, ri->issuer_key_hash, 20);
    if (err) goto leave;
    ...
    if (!err) err = ksba_writer_write(w1, der, derlen);
    if (err) goto leave;
    xfree(ri->serialno);
    ...
    err = _ksba_ber_write_tl(w3, TYPE_SEQUENCE, CLASS_UNIVERSAL, 1, derlen);
    if (!err) err = ksba_writer_write(w3, p, derlen);
    xfree(p);
    p = NULL;
    if (err) goto leave;
  ...
  if (!err) err = ksba_writer_set_mem(w5, 2048);
  if (err) goto leave;
  ...
  err = _ksba_ber_write_tl(w4, TYPE_SEQUENCE, CLASS_UNIVERSAL, 1, derlen);
  if (!err) err = ksba_writer_write(w4, p, derlen);
  xfree(p);
  p = NULL;
  if (err) goto leave;
  ...
  err = _ksba_ber_write_tl(w5, TYPE_SEQUENCE, CLASS_UNIVERSAL, 1, derlen);
  if (!err) err = ksba_writer_write(w5, p, derlen);
  xfree(p);
  p = NULL;
  if (err) goto leave;
  ...
  if (!err) err = ksba_writer_set_mem(w7, 2048);
  if (err) goto leave;
  ...
  err = _ksba_ber_write_tl(w6, TYPE_SEQUENCE, CLASS_UNIVERSAL, 1, derlen);
  if (!err) err = ksba_writer_write(w6, p, derlen);
  xfree(p);
  p = NULL;
  if (err) goto leave;
 ...
 */
gpg_error_t ksba_ocsp_build_request(ksba_ocsp_t ocsp, unsigned char **r_buffer,
                                    size_t *r_buflen) {
  gpg_error_t err;

  if (!ocsp || !r_buffer || !r_buflen) return GPG_ERR_INV_VALUE;
  *r_buffer = NULL;
  *r_buflen = 0;
...
static int parse_response_extensions(ksba_ocsp_t ocsp,
  ...
  while (length) {
    ...
    if (!strcmp(oid, oidstr_ocsp_nonce)) {
      err = parse_octet_string(&data, &datalen, &ti);
      if (err) goto leave;
      if (ocsp->noncelen != ti.length || memcmp(ocsp->nonce, data, ti.length))
        ocsp->bad_nonce = 1;
      else
        ocsp->good_nonce = 1;
    }
...
*/
static int parse_single_extensions(struct ocsp_reqitem_s *ri,
...
static gpg_error_t parse_response_status(ksba_ocsp_t ocsp,
                                         unsigned char const **data,
                                         size_t *datalen, size_t *rlength) {
  gpg_error_t err;
  struct tag_info ti;
  char *oid;
  ...
  *rlength = 0;
  ...
  switch (**data) {
    case 0:
      ocsp->response_status = KSBA_OCSP_RSPSTATUS_SUCCESS;
      break;
    case 1:
      ocsp->response_status = KSBA_OCSP_RSPSTATUS_MALFORMED;
      break;
    case 2:
      ocsp->response_status = KSBA_OCSP_RSPSTATUS_INTERNAL;
      break;
    case 3:
      ocsp->response_status = KSBA_OCSP_RSPSTATUS_TRYLATER;
      break;
    case 5:
      ocsp->response_status = KSBA_OCSP_RSPSTATUS_SIGREQUIRED;
      break;
    case 6:
      ocsp->response_status = KSBA_OCSP_RSPSTATUS_UNAUTHORIZED;
      break;
    default:
      ocsp->response_status = KSBA_OCSP_RSPSTATUS_OTHER;
      break;
  }
...
static gpg_error_t parse_single_response(ksba_ocsp_t ocsp,
                                         unsigned char const **data,
                                         size_t *datalen) {
  gpg_error_t err;
  struct tag_info ti;
  ...
  *datalen -= n;
  ...
  /*   putc ('\n', stderr); */
  if (ti.length != 20) look_for_request = 0; /* Can't be a SHA-1 digest. */
  parse_skip(data, datalen, &ti);
  ...
  if (look_for_request) {
    for (request_item = ocsp->requestlist; request_item;
         request_item = request_item->next)
  ...
  *revocation_time = 0;
  ...
  if (ti.length > *datalen)
  ...
  else if (ti.klasse == CLASS_CONTEXT && ti.tag == 0 &&
           !ti.is_constructed) { /* good */
    if (!ti.length)
      ;                             /* Cope with zero length objects. */
    else if (*datalen && !**data) { /* Skip the NULL. */
      (*datalen)--;
      (*data)++;
    } else
      return GPG_ERR_INV_OBJ;

    if (request_item) request_item->status = KSBA_STATUS_GOOD;
  } else if (ti.klasse == CLASS_CONTEXT && ti.tag == 1 &&
             ti.is_constructed) { /* revoked */
      ...
      *datalen = savedatalen;
      ...
      switch (**data) {
        ...
        case 9:
          ...
          break;
        case 10:
          reason = KSBA_CRLREASON_AA_COMPROMISE;
          break;
        default:
          reason = KSBA_CRLREASON_OTHER;
          break;
      }
      parse_skip(data, datalen, &ti);
    }
    ...
    } else /* The comment indicates that an enumeration may come here. */
    {
      ...
      if (err) return err;
      fprintf(stderr,
              "libksba: unknownReason with an enum of "
              "length %u detected\n",
              (unsigned int)ti.length);
      parse_skip(data, datalen, &ti);
    }
    if (request_item) request_item->status = KSBA_STATUS_UNKNOWN;
  } else
    err = GPG_ERR_INV_OBJ;
  ...
  if (request_item) _ksba_copy_time(request_item->this_update, this_update);

  /* nextUpdate is optional. */
  if (*data >= endptr) return 0;
  *next_update = 0;
  err = _ksba_ber_parse_tl(data, datalen, &ti);
  if (err) return err;
  if (ti.length > *datalen)
    return GPG_ERR_BAD_BER;
           ...
           ti.is_constructed) { /* have nextUpdate */
    err = parse_asntime_into_isotime(data, datalen, next_update);
    if (err) return err;
    /*       fprintf (stderr, "nextUpdate=%s\n", next_update); */
    if (request_item) _ksba_copy_time(request_item->next_update, next_update);
  } else if (ti.klasse == CLASS_CONTEXT && ti.tag == 1 &&
             ti.is_constructed) { /* Undo that read. */
    *data -= ti.nhdr;
    *datalen += ti.nhdr;
  } else
    err = GPG_ERR_INV_OBJ;
  ...
  err = _ksba_ber_parse_tl(data, datalen, &ti);
  if (err) return err;
  if (ti.length > *datalen) return GPG_ERR_BAD_BER;
...
*/
static gpg_error_t parse_response_data(ksba_ocsp_t ocsp,
                                       unsigned char const **data,
                                       size_t *datalen) {
  gpg_error_t err;
  struct tag_info ti;
  ...
  if (ti.length > *datalen)
    return GPG_ERR_BAD_BER;
           ...
           ti.is_constructed) { /* byName. */
    err = _ksba_derdn_to_str(*data, ti.length, &ocsp->responder_id.name);
    if (err) return err;
    parse_skip(data, datalen, &ti);
  } else if (ti.klasse == CLASS_CONTEXT && ti.tag == 2 &&
             ti.is_constructed) { /* byKey. */
    err = parse_octet_string(data, datalen, &ti);
    if (err) return err;
    if (!ti.length) return GPG_ERR_INV_OBJ; /* Zero length key id.  */
    ocsp->responder_id.keyid = (char *)xtrymalloc(ti.length);
    if (!ocsp->responder_id.keyid) return gpg_error_from_syserror();
...
/* Parse the entire response message pointed to by MSG of length
   MSGLEN. */
static gpg_error_t parse_response(ksba_ocsp_t ocsp, const unsigned char *msg,
                                  size_t msglen) {
  gpg_error_t err;
  struct tag_info ti;
  ...
  */
  ...
  if (msg >= endptr) return 0; /* It's optional, so stop now. */
  err = parse_context_tag(&msg, &msglen, &ti, 0);
  if (err == GPG_ERR_INV_OBJ) return 0; /* Not the right tag. Stop here. */
  if (err) return err;
  err = parse_sequence(&msg, &msglen, &ti);
  if (err) return err;
  if (ti.ndef) return GPG_ERR_UNSUPPORTED_ENCODING;

  {
    ...
    while (msg < endptr) {
      ...
      if (err) return err;
      err = ksba_cert_init_from_mem(cert, msg - ti.nhdr, ti.nhdr + ti.length);
      if (err) {
...
gpg_error_t ksba_ocsp_parse_response(
    ksba_ocsp_t ocsp, const unsigned char *msg, size_t msglen,
    ksba_ocsp_response_status_t *response_status) {
  gpg_error_t err;
  struct ocsp_reqitem_s *ri;

  if (!ocsp || !msg || !msglen || !response_status) return GPG_ERR_INV_VALUE;
...
gpg_error_t ksba_ocsp_hash_response(
    ksba_ocsp_t ocsp, const unsigned char *msg, size_t msglen,
    void (*hasher)(void *, const void *, size_t length), void *hasher_arg)

{
  if (!ocsp || !msg || !hasher) return GPG_ERR_INV_VALUE;
  if (!ocsp->hash_length) return GPG_ERR_MISSING_ACTION;
  if (ocsp->hash_offset + ocsp->hash_length >= msglen) return GPG_ERR_CONFLICT;
...
ksba_sexp_t ksba_ocsp_get_sig_val(ksba_ocsp_t ocsp,
  ...
  if (produced_at) *produced_at = 0;
  if (!ocsp || !ocsp->sigval) return NULL;

  if (produced_at) _ksba_copy_time(produced_at, ocsp->produced_at);
...
gpg_error_t ksba_ocsp_get_responder_id(ksba_ocsp_t ocsp, char **r_name,
                                       ksba_sexp_t *r_keyid) {
  ...
  if (ocsp->responder_id.name && r_name) {
    *r_name = xtrystrdup(ocsp->responder_id.name);
    if (!*r_name) return gpg_error_from_syserror();
    ...
    *r_keyid =
        (ksba_sexp_t)xtrymalloc(numbuflen + ocsp->responder_id.keyidlen + 2);
    if (!*r_keyid) return gpg_error_from_syserror();
    strcpy((char *)(*r_keyid), numbuf);
    memcpy(*r_keyid + numbuflen, ocsp->responder_id.keyid,
           ocsp->responder_id.keyidlen);
...
gpg_error_t ksba_ocsp_get_status(ksba_ocsp_t ocsp, ksba_cert_t cert,
  ...
  if (!ocsp || !cert || !r_status) return GPG_ERR_INV_VALUE;
  if (!ocsp->requestlist) return GPG_ERR_MISSING_ACTION;
  ...
  for (ri = ocsp->requestlist; ri; ri = ri->next)
    if (ri->cert == cert) break;
  if (!ri) return GPG_ERR_NOT_FOUND;
  if (r_status) *r_status = ri->status;
  if (r_this_update) _ksba_copy_time(r_this_update, ri->this_update);
  if (r_next_update) _ksba_copy_time(r_next_update, ri->next_update);
  if (r_revocation_time)
    _ksba_copy_time(r_revocation_time, ri->revocation_time);
  if (r_reason) *r_reason = ri->revocation_reason;
  return 0;
}

/* WARNING: The returned values ares only valid as long as no other
   ocsp function is called on the same context.  */
gpg_error_t ksba_ocsp_get_extension(ksba_ocsp_t ocsp, ksba_cert_t cert, int idx,
  ...
  if (cert) {
    ...
    for (ex = ri->single_extensions; ex && idx; ex = ex->next, idx--)
      ;
    if (!ex) return GPG_ERR_EOF; /* No more extensions. */
  } else {
  ...
  if (r_oid) *r_oid = ex->data;
  if (r_crit) *r_crit = ex->crit;
  if (r_der) *r_der = (const unsigned char *)ex->data + ex->off;
  if (r_derlen) *r_derlen = ex->len;
