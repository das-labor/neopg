...
static const struct {
  ...
  const char *oidstr; /* OID as dotted string.  */
    ...
    {"DC", 1, "domainComponent", 10, "\x09\x92\x26\x89\x93\xF2\x2C\x64\x01\x19",
     "0.9.2342.19200300.100.1.25"},
    {"UID", 1, "userid", 10, "\x09\x92\x26\x89\x93\xF2\x2C\x64\x01\x01",
     "0.9.2342.19200300.100.1.1 "},
    {"EMAIL", 3, "emailAddress", 9, "\x2A\x86\x48\x86\xF7\x0D\x01\x09\x01",
...
static unsigned char charclasses[128] = {
    ...
    N, N, N, N, N, N, N, N, N, N, P, N, N, N, N, N, N, P, P, P, N, P,
    P, P, P, P, P, P, P, P, P, P, P, P, P, P, P, N, N, P, N, P, N, P,
    P, P, P, P, P, P, P, P, P, P, P, P, P, P, P, P, P, P, P, P, P, P,
    P, P, P, N, N, N, N, N, N, P, P, P, P, P, P, P, P, P, P, P, P, P,
    P, P, P, P, P, P, P, P, P, P, P, P, P, N, N, N, N, N};
#undef N
#undef P
...
static void put_stringbuf_mem_skip(struct stringbuf *sb, const char *text,
                                   size_t n, int skip) {
  char *p;
...
static void append_quoted(struct stringbuf *sb, const unsigned char *value,
...
static void append_utf8_value(const unsigned char *value, size_t length,
                              struct stringbuf *sb) {
  unsigned char tmp[6];
  ...
  for (s = value, n = 0;;) {
    for (value = s; n < length && !(*s & 0x80); n++, s++)
      ;
    if (s != value) append_quoted(sb, value, s - value, 0);
    if (n == length) return; /* ready */
    if (!(*s & 0x80))
...
/* Append VALUE of LENGTH and TYPE to SB.  Do character set conversion
   and quoting */
static void append_latin1_value(const unsigned char *value, size_t length,
                                struct stringbuf *sb) {
  unsigned char tmp[2];
  const unsigned char *s;
  size_t n;
...
/* Append VALUE of LENGTH and TYPE to SB.  Do UCS-4 to utf conversion
   and and quoting */
static void append_ucs4_value(const unsigned char *value, size_t length,
                              struct stringbuf *sb) {
  unsigned char tmp[7];
  const unsigned char *s;
  size_t n;
  ...
  for (s = value, n = 0;;) {
    for (value = s; n + 3 < length && !s[0] && !s[1] && !s[2] && !(s[3] & 0x80);
         n += 4, s += 4)
      ;
    if (s != value) append_quoted(sb, value, s - value, 3);
    if (n >= length) return; /* ready */
    if (n < 4) { /* This is an invalid encoding - better stop after adding
...
/* Append VALUE of LENGTH and TYPE to SB.  Do UCS-2 to utf conversion
   and and quoting */
static void append_ucs2_value(const unsigned char *value, size_t length,
                              struct stringbuf *sb) {
  unsigned char tmp[3];
  const unsigned char *s;
  size_t n;
  ...
  for (s = value, n = 0;;) {
    for (value = s; n + 1 < length && !s[0] && !(s[1] & 0x80); n += 2, s += 2)
      ;
    if (s != value) append_quoted(sb, value, s - value, 1);
    if (n >= length) return; /* ready */
    if (n < 2) { /* This is an invalid encoding - better stop after adding
                    one impossible characater */
      put_stringbuf_mem(sb, "\xff", 1);
      return;
    }
...
static gpg_error_t append_atv(const unsigned char *image, AsnNode root,
                              struct stringbuf *sb) {
  ...
  if (!node || node->type != TYPE_OBJECT_ID) return GPG_ERR_UNEXPECTED_TAG;
  if (node->off == -1)
    return GPG_ERR_NO_VALUE; /* Hmmm, this might lead to misunderstandings */

  name = NULL;
  for (i = 0; oid_name_tbl[i].name; i++) {
    if (oid_name_tbl[i].source == 1 && node->len == oid_name_tbl[i].oidlen &&
        !memcmp(image + node->off + node->nhdr, oid_name_tbl[i].oid,
                node->len)) {
  ...
  else { /* No name for the OID in the table; at least not DER encoded.
            Now convert the OID to a string, try to find it in the table
            again and use the string as last resort.  */
    char *p;

    p = ksba_oid_to_str((const char *)(image + node->off + node->nhdr),
                        node->len);
    if (!p) return GPG_ERR_ENOMEM;

    for (i = 0; *p && oid_name_tbl[i].name; i++) {
      if (oid_name_tbl[i].source == 1 && !strcmp(p, oid_name_tbl[i].oidstr)) {
        name = oid_name_tbl[i].name;
        break;
      }
    }
  ...
  put_stringbuf(sb, "=");
  node = node->right;
  if (!node || node->off == -1) return GPG_ERR_NO_VALUE;
...
static gpg_error_t dn_to_str(const unsigned char *image, AsnNode root,
                             struct stringbuf *sb) {
  gpg_error_t err;
  AsnNode nset;

  if (!root) return 0; /* empty DN */
  nset = root->down;
  if (!nset) return 0; /* consider this as empty */
  if (nset->type != TYPE_SET_OF) return GPG_ERR_UNEXPECTED_TAG;

  /* output in reverse order */
  while (nset->right) nset = nset->right;
  ...
  for (;;) {
    ...
    for (nseq = nset->down; nseq; nseq = nseq->right) {
      if (nseq->type != TYPE_SEQUENCE) return GPG_ERR_UNEXPECTED_TAG;
      if (nseq != nset->down) put_stringbuf(sb, "+");
      err = append_atv(image, nseq, sb);
      if (err) return err;
    }
    if (nset == root->down) break;
    put_stringbuf(sb, ",");
    nset = nset->left;
  }
...
gpg_error_t _ksba_dn_to_str(const unsigned char *image, AsnNode node,
  ...
  *r_string = NULL;
  if (!node || node->type != TYPE_SEQUENCE_OF) return GPG_ERR_INV_VALUE;

  init_stringbuf(&sb, 100);
  err = dn_to_str(image, node, &sb);
  if (!err) {
    *r_string = get_stringbuf(&sb);
    if (!*r_string) err = GPG_ERR_ENOMEM;
  }
  deinit_stringbuf(&sb);
 ...
 */
static const char *count_quoted_string(const char *string, size_t *result,
...
static gpg_error_t write_escaped(ksba_writer_t w, const unsigned char *buffer,
                                 size_t nbytes) {
  const unsigned char *s;
  gpg_error_t err;
...
static gpg_error_t parse_rdn(const unsigned char *string, const char **endp,
  ...
  if (!roff) roff = &dummy_roff;
  if (!rlen) rlen = &dummy_rlen;

  *roff = *rlen = 0;

  if (!string) return GPG_ERR_INV_VALUE;
  while (*string == ' ') string++;
  *roff = string - orig_string;
  if (!*string) return GPG_ERR_SYNTAX; /* empty elements are not allowed */
  s = string;
  ...
  *roff = string - orig_string;
  ...
  } else if (*s == '\"') { /* old style quotation */
    string = s + 1;
    s = (const unsigned char *)count_quoted_string((const char *)(string), &n,
                                                   1, &valuetype);
    if (!s || *s != '\"') {
      *rlen = s - orig_string;
  ...
  *endp = (const char *)(*s ? (s + 1) : s);
  ...
  if (writer) { /* write out the data */
    ...
    /* the OBJECT ID */
    err =
        _ksba_ber_write_tl(writer, TYPE_OBJECT_ID, CLASS_UNIVERSAL, 0, oidlen);
    ...
    err = _ksba_ber_write_tl(writer, valuetype, CLASS_UNIVERSAL, 0, valuelen);
    if (!err)
...
gpg_error_t _ksba_dn_from_str(const char *string, char **rbuf,
                              size_t *rlength) {
  gpg_error_t err;
  ...
  *rlength = 0;
  ...
  if (!err) err = ksba_writer_set_mem(writer, 1024);
  if (err) return err;
  ...
  for (nparts = 0, s = string; s && *s;) {
    ...
    if (nparts >= part_array_size) {
      ...
      part_array_size += 2;
      tmp = (const char **)(part_array_size
                                ? xtryrealloc(part_array,
  ...
  while (--nparts >= 0) {
    err = parse_rdn((const unsigned char *)(part_array[nparts]), &endp, writer,
                    NULL, NULL);
    if (err) goto leave;
  }
  ...
  err = _ksba_ber_write_tl(writer, TYPE_SEQUENCE, CLASS_UNIVERSAL, 1, buflen);
  if (err) goto leave;
...
gpg_error_t ksba_dn_der2str(const void *der, size_t derlen, char **rstring) {
  return _ksba_derdn_to_str((const unsigned char *)(der), derlen, rstring);
}

gpg_error_t ksba_dn_str2der(const char *string, unsigned char **rder,
                            size_t *rderlen) {
...
gpg_error_t ksba_dn_teststr(const char *string, int seq, size_t *rerroff,
                            size_t *rerrlen) {
  ...
  if (!rerroff) rerroff = &dummy_erroff;
  if (!rerrlen) rerrlen = &dummy_errlen;
  ...
  *rerrlen = *rerroff = 0;
  ...
  if (!nparts) return GPG_ERR_SYNTAX;
  return 0;
}
