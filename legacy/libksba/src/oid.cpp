...
char *_ksba_oid_node_to_str(const unsigned char *image, AsnNode node) {
  ...
  return ksba_oid_to_str((const char *)(image + node->off + node->nhdr),
                         node->len);
}
...
gpg_error_t ksba_oid_from_str(const char *string, unsigned char **rbuf,
  ...
  if (!string || !rbuf || !rlength) return GPG_ERR_INV_VALUE;
  *rbuf = NULL;
  *rlength = 0;
  ...
  do {
    ...
    if (arcno == 1) {
      if (val > 2) break; /* not allowed, error catched below */
      val1 = val;
    } else if (arcno ==
               2) { /* need to combine the first to arcs in one octet */
      if (val1 < 2) {
        if (val > 39) {
          xfree(buf);
          return GPG_ERR_INV_OID_STRING;
  ...
  if (arcno == 1) { /* it is not possible to encode only the first arc */
    xfree(buf);
    return GPG_ERR_INV_OID_STRING;
  }
