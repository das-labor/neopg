...
static gpg_error_t create_and_run_decoder(ksba_reader_t reader,
  ...
  err = _ksba_ber_decoder_decode(decoder, elem_name, flags, r_root, r_image,
                                 r_imagelen);
...
static gpg_error_t parse_content_info(ksba_reader_t reader,
  ...
  if (!content_ndef && content_len < 3)
    return GPG_ERR_TOO_SHORT; /* to encode an OID */
  ...
  if (!content_ndef) {
    if (content_len < ti.nhdr)
      return GPG_ERR_BAD_BER; /* triplet header larger that sequence */
    content_len -= ti.nhdr;
    if (content_len < ti.length)
      return GPG_ERR_BAD_BER; /* triplet larger that sequence */
    content_len -= ti.length;
  }

  if (ti.length >= DIM(oidbuf)) return GPG_ERR_TOO_LARGE;
  err = read_buffer(reader, (char *)(oidbuf), ti.length);
  if (err) return err;
  oid = ksba_oid_to_str((const char *)(oidbuf), ti.length);
  if (!oid) return GPG_ERR_ENOMEM;

  if (!content_ndef && !content_len) { /* no data */
    *has_content = 0;
  } else { /* now read the explicit tag 0 which is optional */
    err = _ksba_ber_read_tl(reader, &ti);
    if (err) {
      xfree(oid);
      return err;
    ...
    if (ti.klasse == CLASS_CONTEXT && ti.tag == 0 && ti.is_constructed) {
      *has_content = 1;
    } else if (ti.klasse == CLASS_UNIVERSAL && ti.tag == 0 &&
               !ti.is_constructed) {
...
/* Parse this structure and return the oid of the content as well as
   ...
   by the parameters.
*/
static gpg_error_t parse_encrypted_content_info(
    ksba_reader_t reader, unsigned long *r_len, int *r_ndef, char **r_cont_oid,
    char **r_algo_oid, char **r_algo_parm, size_t *r_algo_parmlen,
    int *has_content) {
  struct tag_info ti;
  gpg_error_t err;
  int content_ndef;
  ...
  err = _ksba_parse_algorithm_identifier2(tmpbuf, ti.nhdr + ti.length, &nread,
                                          &algo_oid, &algo_parm, &algo_parmlen);
  if (err) return err;
  ...
  *has_content = 0;
  if (content_ndef || content_len) { /* now read the implicit tag 0.  Actually
                                        this is optional but
                                        in that case we don't expect to have a
                                        content_len - well, it
                                        may be the end tag */
    err = _ksba_ber_read_tl(reader, &ti);
    if (err) {
    ...
    if (ti.klasse == CLASS_CONTEXT && ti.tag == 0) {
      *has_content = 1;
      if (!content_ndef) {
        if (content_len < ti.nhdr) return GPG_ERR_BAD_BER;
        content_len -= ti.nhdr;
        if (!ti.ndef && content_len < ti.length) return GPG_ERR_BAD_BER;
      }
...
gpg_error_t _ksba_cms_parse_content_info(ksba_cms_t cms) {
  ...
  err = parse_content_info(cms->reader, &content_len, &content_ndef, &oid,
                           &has_content);
  if (err) { /* return a more meaningful error message.  This way the caller
                ...
                Error. */
    if (err == GPG_ERR_BAD_BER || err == GPG_ERR_INV_CMS_OBJ ||
        err == GPG_ERR_TOO_SHORT)
      err = GPG_ERR_NO_CMS_OBJ;
    return err;
  }
  if (!has_content) return GPG_ERR_NO_CMS_OBJ; /* It is not optional here */
...
static gpg_error_t parse_cms_version(ksba_reader_t reader, int *r_version,
                                     unsigned long *r_len, int *r_ndef) {
  struct tag_info ti;
  gpg_error_t err;
  ...
  if (!data_ndef && data_len < 3)
    return GPG_ERR_TOO_SHORT; /*to encode the version*/
  ...
  if (ti.length != 1) return GPG_ERR_UNSUPPORTED_CMS_VERSION;
  if ((c = read_byte(reader)) == -1) {
    err = ksba_reader_error(reader);
    return err ? err : GPG_ERR_GENERAL;
  }
  if (!(c == 0 || c == 1 || c == 2 || c == 3 || c == 4))
    return GPG_ERR_UNSUPPORTED_CMS_VERSION;
  *r_version = c;
  *r_len = data_len;
  *r_ndef = data_ndef;
  return 0;
...
gpg_error_t _ksba_cms_parse_signed_data_part_1(ksba_cms_t cms) {
  ...
  err = parse_cms_version(cms->reader, &cms->cms_version, &signed_data_len,
                          &signed_data_ndef);
  if (err) return err;
  ...
  if (!(ti.klasse == CLASS_UNIVERSAL && ti.tag == TYPE_SET &&
        ti.is_constructed))
    return GPG_ERR_INV_CMS_OBJ; /* not the expected SET tag */
  if (!signed_data_ndef) {
  ...
  while (algo_set_len) {
    ...
    err = _ksba_parse_algorithm_identifier((const unsigned char *)(p),
                                           algo_set_len, &nread, &oid);
    if (err) {
      xfree(buffer);
      return err;
    }
  ...
  err = parse_content_info(cms->reader, &encap_cont_len, &encap_cont_ndef, &oid,
                           &has_content);
  if (err) return err;
  ...
  if (!signed_data_ndef) {
    ...
    if (signed_data_len < len)
      return GPG_ERR_BAD_BER; /* parsed content info larger that sequence */
    signed_data_len -= len;
...
gpg_error_t _ksba_cms_parse_signed_data_part_2(ksba_cms_t cms) {
  ...
  if (ti.klasse == CLASS_CONTEXT && ti.tag == 0 &&
      ti.is_constructed) { /* Implicit SET OF certificateSet with elements of
                              CHOICE, but
                              we assume the first choice which is a Certificate;
                              all other
                              choices are obsolete.  We are now parsing a set of
                              certificates which we do by utilizing the
                              ksba_cert code. */
    ksba_cert_t cert;
    int expect_endtag;
  ...
  if (ti.klasse == CLASS_CONTEXT && ti.tag == 1 &&
      ti.is_constructed) { /* implicit SET OF certificateList.  We should
                              delegate the
                              parsing to a - not yet existing - ksba_crl module.
                              CRLs are
                              quite important for other applications too so we
                              should
  ...
  while (ti.length) {
    ...
    err = create_and_run_decoder(cms->reader,
                                 "CryptographicMessageSyntax.SignerInfo", 0,
...
gpg_error_t _ksba_cms_parse_enveloped_data_part_1(ksba_cms_t cms) {
  ...
  /* get the version */
  err = parse_cms_version(cms->reader, &cms->cms_version, &env_data_len,
                          &env_data_ndef);
  if (err) return err;
  ...
  if (ti.klasse == CLASS_CONTEXT && ti.tag == 0 &&
      ti.is_constructed) { /* originatorInfo - but we skip it for now */
    /* well, raise an error */
  ...
  if (ti.ndef) {
    for (;;) {
      ...
      if (!ti2.klasse && !ti2.tag) break; /* End tag found: ready.  */
      ...
      if (!vt) return gpg_error_from_syserror();

      err = create_and_run_decoder(
          cms->reader, "CryptographicMessageSyntax.KeyTransRecipientInfo",
          BER_DECODER_FLAG_FAST_STOP, &vt->root, &vt->image, &vt->imagelen);
      if (err) {
        xfree(vt);
        return err;
      }
    ...
    while (ti.length) {
      ...
      err = create_and_run_decoder(
          cms->reader, "CryptographicMessageSyntax.KeyTransRecipientInfo", 0,
  ...
  off = ksba_reader_tell(cms->reader);
  err = parse_encrypted_content_info(cms->reader, &encr_cont_len,
                                     &encr_cont_ndef, &cont_oid, &algo_oid,
                                     &algo_parm, &algo_parmlen, &has_content);
  if (err) return err;
