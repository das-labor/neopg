...
static inline void do_hash(ksba_crl_t crl, const void *buffer, size_t length) {
  while (length) {
    size_t n = length;
    ...
    if (crl->hashbuf.used == sizeof crl->hashbuf.buffer) {
      if (crl->hash_fnc)
        crl->hash_fnc(crl->hash_fnc_arg, crl->hashbuf.buffer,
                      crl->hashbuf.used);
      crl->hashbuf.used = 0;
    }
...
gpg_error_t ksba_crl_new(ksba_crl_t *r_crl) {
  *r_crl = (ksba_crl_t)xtrycalloc(1, sizeof **r_crl);
  if (!*r_crl) return gpg_error_from_errno(errno);
  return 0;
}
...
gpg_error_t ksba_crl_set_reader(ksba_crl_t crl, ksba_reader_t r) {
  if (!crl || !r) return GPG_ERR_INV_VALUE;
...
gpg_error_t ksba_crl_get_issuer(ksba_crl_t crl, char **r_issuer) {
  ...
  if (!crl || !r_issuer) return GPG_ERR_INV_VALUE;
  if (!crl->issuer.root) return GPG_ERR_NO_DATA;
...
gpg_error_t ksba_crl_get_extension(ksba_crl_t crl, int idx, char const **oid,
                                   int *critical, unsigned char const **der,
                                   size_t *derlen) {
  crl_extn_t e;

  if (!crl) return GPG_ERR_INV_VALUE;
  if (idx < 0) return GPG_ERR_INV_INDEX;

  for (e = crl->extension_list; e && idx; e = e->next, idx--)
    ;
  if (!e) return GPG_ERR_EOF;

  if (oid) *oid = e->oid;
  if (critical) *critical = e->critical;
  if (der) *der = e->der;
  if (derlen) *derlen = e->derlen;
...
*/
gpg_error_t ksba_crl_get_auth_key_id(ksba_crl_t crl, ksba_sexp_t *r_keyid,
                                     ksba_name_t *r_name,
  ...
  if (!crl || !r_name || !r_serial) return GPG_ERR_INV_VALUE;
  *r_name = NULL;
  *r_serial = NULL;

  for (e = crl->extension_list; e; e = e->next)
    if (!strcmp(e->oid, oidstr_authorityKeyIdentifier)) break;
  if (!e) return GPG_ERR_NO_DATA; /* not available */
  ...
  if (ti.tag == 0) { /* keyIdentifier:  Just save it away for later use. */
    keyid_der = der;
  ...
  if (ti.tag != 1 || !derlen) return GPG_ERR_INV_CRL_OBJ;
...
gpg_error_t ksba_crl_get_crl_number(ksba_crl_t crl, ksba_sexp_t *number) {
  ...
  if (!crl || !number) return GPG_ERR_INV_VALUE;
  *number = NULL;

  for (e = crl->extension_list; e; e = e->next)
    if (!strcmp(e->oid, oidstr_crlNumber)) break;
  if (!e) return GPG_ERR_NO_DATA; /* not available */
  ...
  /* Check that there is only one. */
  {
    ...
    for (e2 = e->next; e2; e2 = e2->next)
      if (!strcmp(e2->oid, oidstr_crlNumber)) return GPG_ERR_DUP_VALUE;
  }
  ...
  *number = (ksba_sexp_t)xtrymalloc(numbuflen + ti.length + 2);
  if (!*number) return gpg_error_from_errno(errno);
 ...
 * Return value: 0 on success or an error code
 **/
gpg_error_t ksba_crl_get_update_times(ksba_crl_t crl, ksba_isotime_t this_x,
                                      ksba_isotime_t next) {
  if (this_x) *this_x = 0;
  if (next) *next = 0;
  if (!crl) return GPG_ERR_INV_VALUE;
  if (!*crl->this_update) return GPG_ERR_INV_TIME;
  if (this_x) _ksba_copy_time(this_x, crl->this_update);
  if (next) _ksba_copy_time(next, crl->next_update);
  return 0;
}
 ...
 **/
gpg_error_t ksba_crl_get_item(ksba_crl_t crl, ksba_sexp_t *r_serial,
                              ksba_isotime_t r_revocation_date,
                              ksba_crl_reason_t *r_reason) {
  if (r_revocation_date) *r_revocation_date = 0;

  if (!crl) return GPG_ERR_INV_VALUE;

  if (r_serial) {
    if (!crl->item.serial) return GPG_ERR_NO_DATA;
  ...
  if (r_reason) *r_reason = crl->item.reason;
  return 0;
}
...
static int read_buffer(ksba_reader_t reader, char *buffer, size_t count) {
  ...
  while (count) {
    if (ksba_reader_read(reader, buffer, count, &nread)) return -1;
    buffer += nread;
    count -= nread;
  }
  return 0;
...
static gpg_error_t create_and_run_decoder(ksba_reader_t reader,
  ...
  err = _ksba_ber_decoder_decode(decoder, elem_name, 0, r_root, r_image,
                                 r_imagelen);
...
static gpg_error_t parse_one_extension(const unsigned char *der, size_t derlen,
                                       char **oid, int *critical, size_t *off,
                                       size_t *len) {
  gpg_error_t err;
  struct tag_info ti;
...
static gpg_error_t store_one_extension(ksba_crl_t crl, const unsigned char *der,
                                       size_t derlen) {
  gpg_error_t err;
  char *oid;
  int critical;
...
static gpg_error_t parse_to_next_update(ksba_crl_t crl) {
  ...
  outer_len = ti.length;
  outer_ndef = ti.ndef;
  if (!outer_ndef && outer_len < 10) return GPG_ERR_TOO_SHORT;
  ...
  tbs_len = ti.length;
  tbs_ndef = ti.ndef;
  if (!tbs_ndef && tbs_len < 10) return GPG_ERR_TOO_SHORT;
  ...
  if (ti.klasse == CLASS_UNIVERSAL && ti.tag == TYPE_INTEGER) {
    if (ti.is_constructed || !ti.length) return GPG_ERR_INV_CRL_OBJ;
    HASH(ti.buf, ti.nhdr);
    if (!tbs_ndef) {
      if (tbs_len < ti.nhdr) return GPG_ERR_BAD_BER;
      tbs_len -= ti.nhdr;
      if (tbs_len < ti.length) return GPG_ERR_BAD_BER;
      tbs_len -= ti.length;
    }
    ...
    if (ti.length != 1) return GPG_ERR_UNSUPPORTED_CRL_VERSION;
    if ((c = read_byte(crl->reader)) == -1) {
      err = ksba_reader_error(crl->reader);
      return err ? err : GPG_ERR_GENERAL;
    }
    if (!(c == 0 || c == 1)) return GPG_ERR_UNSUPPORTED_CRL_VERSION;
    {
      unsigned char tmp = c;
      HASH(&tmp, 1);
    }
  ...
  if (!tbs_ndef) {
    if (tbs_len < ti.nhdr) return GPG_ERR_BAD_BER;
    tbs_len -= ti.nhdr;
    if (!ti.ndef && tbs_len < ti.length) return GPG_ERR_BAD_BER;
    tbs_len -= ti.length;
  }
  if (ti.nhdr + ti.length >= DIM(tmpbuf)) return GPG_ERR_TOO_LARGE;
  ...
  err = _ksba_parse_algorithm_identifier2(tmpbuf, ti.nhdr + ti.length, &nread,
                                          &crl->algo.oid, &crl->algo.parm,
                                          &crl->algo.parmlen);
  if (err) return err;
  assert(nread <= ti.nhdr + ti.length);
  if (nread < ti.nhdr + ti.length) return GPG_ERR_TOO_SHORT;

  /* read the name */
  {
    unsigned long n = ksba_reader_tell(crl->reader);
    err = create_and_run_decoder(
        crl->reader, "TMTTv2.CertificateList.tbsCertList.issuer",
        &crl->issuer.root, &crl->issuer.image, &crl->issuer.imagelen);
    if (err) return err;
    ...
    if (n > crl->issuer.imagelen) return GPG_ERR_BUG;
    HASH(crl->issuer.image, n);

    if (!tbs_ndef) {
      if (tbs_len < n) return GPG_ERR_BAD_BER;
      tbs_len -= n;
    }
  }
  ...
  if (tbs_ndef || tbs_len) {
    if (ti.klasse == CLASS_UNIVERSAL && ti.tag == TYPE_SEQUENCE &&
        ti.is_constructed) { /* yes, there is one */
      HASH(ti.buf, ti.nhdr);
      if (!tbs_ndef) {
        if (tbs_len < ti.nhdr) return GPG_ERR_BAD_BER;
        tbs_len -= ti.nhdr;
...
static gpg_error_t store_one_entry_extension(ksba_crl_t crl,
                                             const unsigned char *der,
                                             size_t derlen) {
  gpg_error_t err;
  char *oid;
  ...
  if (!strcmp(oid, oidstr_crlReason)) {
    ...
    switch (*buf) {
      case 0:
        crl->item.reason |= KSBA_CRLREASON_UNSPECIFIED;
        break;
      case 1:
        crl->item.reason |= KSBA_CRLREASON_KEY_COMPROMISE;
        break;
      case 2:
        crl->item.reason |= KSBA_CRLREASON_CA_COMPROMISE;
        break;
      case 3:
        crl->item.reason |= KSBA_CRLREASON_AFFILIATION_CHANGED;
        break;
      case 4:
        crl->item.reason |= KSBA_CRLREASON_SUPERSEDED;
        break;
      case 5:
        crl->item.reason |= KSBA_CRLREASON_CESSATION_OF_OPERATION;
        break;
      case 6:
        crl->item.reason |= KSBA_CRLREASON_CERTIFICATE_HOLD;
        break;
      case 8:
        crl->item.reason |= KSBA_CRLREASON_REMOVE_FROM_CRL;
        break;
      case 9:
        crl->item.reason |= KSBA_CRLREASON_PRIVILEGE_WITHDRAWN;
        break;
      case 10:
        crl->item.reason |= KSBA_CRLREASON_AA_COMPROMISE;
        break;
      default:
        crl->item.reason |= KSBA_CRLREASON_OTHER;
        break;
    }
  }
...
static gpg_error_t parse_crl_entry(ksba_crl_t crl, int *got_entry) {
  ...
  if (!crl->state.have_seqseq) return 0; /* ready (no entries at all) */

  if (!seqseq_ndef && !seqseq_len) return 0; /* ready */
  ...
  HASH(ti.buf, ti.nhdr);
  if (!seqseq_ndef) {
    if (seqseq_len < ti.nhdr) return GPG_ERR_BAD_BER;
    seqseq_len -= ti.nhdr;
    if (!ti.ndef && seqseq_len < ti.length) return GPG_ERR_BAD_BER;
  ...
  if (!(ti.klasse == CLASS_UNIVERSAL && ti.tag == TYPE_INTEGER &&
    ...
    return GPG_ERR_INV_CRL_OBJ;
  if (!ndef) {
    if (len < ti.nhdr) return GPG_ERR_BAD_BER;
    len -= ti.nhdr;
    if (!ti.ndef && len < ti.length) return GPG_ERR_BAD_BER;
    len -= ti.length;
  }
  if (ti.nhdr + ti.length >= DIM(tmpbuf)) return GPG_ERR_TOO_LARGE;
  memcpy(tmpbuf, ti.buf, ti.nhdr);
  ...
  if (!crl->item.serial) return GPG_ERR_ENOMEM;
  ...
  if (ndef)
    ...
    if (len < ti.nhdr) return GPG_ERR_BAD_BER;
    len -= ti.nhdr;
    if (len < ti.length) return GPG_ERR_BAD_BER;
...
static gpg_error_t parse_crl_extensions(ksba_crl_t crl) {
  ...
  if (ti.ndef) return GPG_ERR_UNSUPPORTED_ENCODING;
  HASH(ti.buf, ti.nhdr);
  if (ext_len < ti.nhdr) return GPG_ERR_BAD_BER;
  ext_len -= ti.nhdr;
  if (ext_len < ti.length) return GPG_ERR_BAD_BER;
  len = ti.length;
...
static gpg_error_t parse_signature(ksba_crl_t crl) {
  ...
  if (ti.ndef) return GPG_ERR_UNSUPPORTED_ENCODING;
  n = ti.nhdr + ti.length;
  if (n >= DIM(tmpbuf)) return GPG_ERR_TOO_LARGE;
  memcpy(tmpbuf, ti.buf, ti.nhdr);
  err = read_buffer(crl->reader, (char *)(tmpbuf + ti.nhdr), ti.length);
  if (err) return err;
  ...
  if (n + n2 >= DIM(tmpbuf)) return GPG_ERR_TOO_LARGE;
...
gpg_error_t ksba_crl_parse(ksba_crl_t crl, ksba_stop_reason_t *r_stopreason) {
  enum { sSTART, sCRLENTRY, sCRLEXT, sERROR } state = sERROR;
  ksba_stop_reason_t stop_reason;
  gpg_error_t err = 0;
  int got_entry = 0;

  if (!crl || !r_stopreason) return GPG_ERR_INV_VALUE;

  if (!crl->any_parse_done) { /* first time initialization of the stop reason */
