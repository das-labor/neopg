...
gpg_error_t _ksba_asntime_to_iso(const char *buffer, size_t length,
  ...
  *timebuf = 0;
  for (s = buffer, n = 0; n < length && digitp(s); n++, s++)
    ;
  if (is_utctime) {
    if ((n != 10 && n != 12) || *s != 'Z') return GPG_ERR_INV_TIME;
...
gpg_error_t _ksba_assert_time_format(const ksba_isotime_t atime) {
  ...
  for (s = atime, i = 0; i < 8; i++, s++)
    if (!digitp(s)) return GPG_ERR_BUG;
  if (*s != 'T') return GPG_ERR_BUG;
  for (s++, i = 9; i < 15; i++, s++)
    if (!digitp(s)) return GPG_ERR_BUG;
  if (*s) return GPG_ERR_BUG;
  return 0;
}
...
void _ksba_current_time(ksba_isotime_t timebuf) {
  ...
  sprintf(timebuf, "%04d%02d%02dT%02d%02d%02d", 1900 + tp->tm_year,
