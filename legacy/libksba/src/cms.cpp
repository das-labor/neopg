...
static struct {
  ...
  gpg_error_t (*build_handler)(ksba_cms_t);
    ...
    {"1.2.840.113549.1.7.1", KSBA_CT_DATA, ct_parse_data, ct_build_data},
    {"1.2.840.113549.1.7.2", KSBA_CT_SIGNED_DATA, ct_parse_signed_data,
     ct_build_signed_data},
    {"1.2.840.113549.1.7.3", KSBA_CT_ENVELOPED_DATA, ct_parse_enveloped_data,
     ct_build_enveloped_data},
    {"1.2.840.113549.1.7.5", KSBA_CT_DIGESTED_DATA, ct_parse_digested_data,
     ct_build_digested_data},
    {"1.2.840.113549.1.7.6", KSBA_CT_ENCRYPTED_DATA, ct_parse_encrypted_data,
...
static const char oid_messageDigest[9] = {
    '\x2A', '\x86', '\x48', '\x86', '\xF7', '\x0D', '\x01', '\x09', '\x04'};

static const char oidstr_signingTime[] = "1.2.840.113549.1.9.5";
static const char oid_signingTime[9] = {'\x2A', '\x86', '\x48', '\x86', '\xF7',
                                        '\x0D', '\x01', '\x09', '\x05'};
...
static gpg_error_t read_hash_block(ksba_cms_t cms, unsigned long nleft) {
  ...
  while (nleft) {
    ...
    if (err) return err;
    nleft -= nread;
    if (cms->hash_fnc) cms->hash_fnc(cms->hash_fnc_arg, buffer, nread);
    if (cms->writer) err = ksba_writer_write(cms->writer, buffer, nread);
    if (err) return err;
  }
  return 0;
}
...
static gpg_error_t read_and_hash_cont(ksba_cms_t cms) {
  ...
  if (cms->inner_cont_ndef) {
    for (;;) {
      ...
      } else if (ti.klasse == CLASS_UNIVERSAL && !ti.tag && !ti.is_constructed)
        return 0; /* ready */
      else
        return GPG_ERR_ENCODING_PROBLEM;
    }
    ...
    err = _ksba_ber_read_tl(cms->reader, &ti);
    if (err) return err;
    if (nleft < ti.nhdr) return GPG_ERR_ENCODING_PROBLEM;
    nleft -= ti.nhdr;
...
static gpg_error_t write_encrypted_cont(ksba_cms_t cms) {
  ...
  while (
      !(err = ksba_reader_read(cms->reader, buffer, sizeof buffer, &nread))) {
    err = _ksba_ber_write_tl(cms->writer, TYPE_OCTET_STRING, CLASS_UNIVERSAL, 0,
                             nread);
    if (!err) err = ksba_writer_write(cms->writer, buffer, nread);
  }
...
ksba_content_type_t ksba_cms_identify(ksba_reader_t reader) {
  ...
  if (!reader) return KSBA_CT_NONE; /* oops */
  ...
  */
  ...
  for (count = sizeof buffer; count; count -= n) {
    if (ksba_reader_read(reader, (char *)(buffer + sizeof(buffer) - count),
                         count, &n))
      return KSBA_CT_NONE; /* too short */
  }
  n = sizeof buffer;
  if (ksba_reader_unread(reader, buffer, n)) return KSBA_CT_NONE; /* oops */

  p = buffer;
  if (_ksba_ber_parse_tl(&p, &n, &ti)) return KSBA_CT_NONE;
  ...
  if (!oid) return KSBA_CT_NONE; /* out of core */
  for (i = 0; content_handlers[i].oid; i++) {
    if (!strcmp(content_handlers[i].oid, oid)) break;
  }
  if (!content_handlers[i].oid) return KSBA_CT_NONE; /* unknown */
...
gpg_error_t ksba_cms_new(ksba_cms_t *r_cms) {
  *r_cms = (ksba_cms_t)xtrycalloc(1, sizeof **r_cms);
  if (!*r_cms) return gpg_error_from_errno(errno);
  return 0;
}
...
gpg_error_t ksba_cms_set_reader_writer(ksba_cms_t cms, ksba_reader_t r,
                                       ksba_writer_t w) {
  if (!cms || !(r || w)) return GPG_ERR_INV_VALUE;
...
gpg_error_t ksba_cms_parse(ksba_cms_t cms, ksba_stop_reason_t *r_stopreason) {
  ...
  if (!cms || !r_stopreason) return GPG_ERR_INV_VALUE;

  *r_stopreason = KSBA_SR_RUNNING;
  if (!cms->stop_reason) { /* Initial state: start parsing */
    err = _ksba_cms_parse_content_info(cms);
    if (err) return err;
    for (i = 0; content_handlers[i].oid; i++) {
      if (!strcmp(content_handlers[i].oid, cms->content.oid)) break;
    }
    if (!content_handlers[i].oid) return GPG_ERR_UNKNOWN_CMS_OBJ;
    if (!content_handlers[i].parse_handler) return GPG_ERR_UNSUPPORTED_CMS_OBJ;
    ...
    return GPG_ERR_UNSUPPORTED_CMS_OBJ;
...
gpg_error_t ksba_cms_build(ksba_cms_t cms, ksba_stop_reason_t *r_stopreason) {
  ...
  *r_stopreason = KSBA_SR_RUNNING;
  if (!cms->stop_reason) { /* Initial state: check that the content handler is
                              known */
    if (!cms->writer) return GPG_ERR_MISSING_ACTION;
    if (!cms->content.handler) return GPG_ERR_MISSING_ACTION;
    if (!cms->inner_cont_oid) return GPG_ERR_MISSING_ACTION;
    cms->stop_reason = KSBA_SR_GOT_CONTENT;
  } else if (cms->content.handler) {
    err = cms->content.handler(cms);
    if (err) return err;
...
ksba_content_type_t ksba_cms_get_content_type(ksba_cms_t cms, int what) {
  ...
  if (!cms) return (ksba_content_type_t)0;
  if (!what) return cms->content.ct;
...
const char *ksba_cms_get_content_oid(ksba_cms_t cms, int what) {
  if (!cms) return NULL;
  if (!what) return cms->content.oid;
  if (what == 1) return cms->inner_cont_oid;
  if (what == 2) return cms->encr_algo_oid;
  return NULL;
}
...
gpg_error_t ksba_cms_get_content_enc_iv(ksba_cms_t cms, void *iv,
  ...
  if (!cms || !iv || !ivlen) return GPG_ERR_INV_VALUE;
  if (!cms->encr_ivlen) return GPG_ERR_NO_DATA;
  if (cms->encr_ivlen > maxivlen) return GPG_ERR_BUFFER_TOO_SHORT;
 ...
 * all.
 **/
gpg_error_t ksba_cms_get_issuer_serial(ksba_cms_t cms, int idx, char **r_issuer,
  ...
  if (!cms) return GPG_ERR_INV_VALUE;
  if (idx < 0) return GPG_ERR_INV_INDEX;
  ...
  if (cms->signer_info) {
    ...
    serial_path =
        "KeyTransRecipientInfo.rid.issuerAndSerialNumber.serialNumber";
  ...
  if (r_issuer) {
    ...
    if (!n || !n->down) return GPG_ERR_NO_VALUE;
...
const char *ksba_cms_get_digest_algo(ksba_cms_t cms, int idx) {
  ...
  if (!cms) return NULL;
  if (!cms->signer_info) return NULL;
  ...
  if (si->cache.digest_algo) return si->cache.digest_algo;
...
gpg_error_t ksba_cms_get_message_digest(ksba_cms_t cms, int idx,
  ...
  if (!cms || !r_digest || !r_digest_len) return GPG_ERR_INV_VALUE;
  if (!cms->signer_info) return GPG_ERR_NO_DATA;
  if (idx < 0) return GPG_ERR_INV_INDEX;
  ...
  *r_digest_len = 0;
  ...
  if (!nsiginfo) return GPG_ERR_BUG;

  n = _ksba_asn_find_type_value(si->image, nsiginfo, 0, oid_messageDigest,
                                DIM(oid_messageDigest));
  if (!n) return 0; /* this is okay, because the element is optional */

  /* check that there is only one */
  if (_ksba_asn_find_type_value(si->image, nsiginfo, 1, oid_messageDigest,
                                DIM(oid_messageDigest)))
  ...
  if (!(n->type == TYPE_SET_OF && n->down &&
    ...
    return GPG_ERR_INV_CMS_OBJ;
  n = n->down;
  if (n->off == -1) return GPG_ERR_BUG;

  *r_digest_len = n->len;
  *r_digest = (char *)xtrymalloc(n->len);
  if (!*r_digest) return GPG_ERR_ENOMEM;
...
gpg_error_t ksba_cms_get_signing_time(ksba_cms_t cms, int idx,
  ...
  *r_sigtime = 0;
  ...
  n = _ksba_asn_find_type_value(si->image, nsiginfo, 0, oid_signingTime,
                                DIM(oid_signingTime));
  if (!n) return 0; /* This is okay because signing time is optional. */

  /* check that there is only one */
  if (_ksba_asn_find_type_value(si->image, nsiginfo, 1, oid_signingTime,
                                DIM(oid_signingTime)))
    return GPG_ERR_DUP_VALUE;
  ...
  return _ksba_asntime_to_iso((const char *)(si->image + n->off + n->nhdr),
                              n->len, n->type == TYPE_UTC_TIME, r_sigtime);
}
...
gpg_error_t ksba_cms_get_sigattr_oids(ksba_cms_t cms, int idx,
  ...
  if (!cms || !r_value) return GPG_ERR_INV_VALUE;
  if (!cms->signer_info) return GPG_ERR_NO_DATA;
  if (idx < 0) return GPG_ERR_INV_INDEX;
  *r_value = NULL;
  ...
  for (si = cms->signer_info; si && idx; si = si->next, idx--)
    ;
  if (!si) return -1; /* no more signers */
  ...
  for (i = 0; (n = _ksba_asn_find_type_value(si->image, nsiginfo, i, reqoidbuf,
                                             reqoidlen));
       i++) {
    char *line, *p;
    ...
    if (!retstr)
      ...
      char *tmp =
          (char *)xtryrealloc(retstr, strlen(retstr) + 1 + strlen(p) + 2);
      if (!tmp)
        line = NULL;
      else {
        retstr = tmp;
  ...
  xfree(reqoidbuf);
  if (!n && !i) return -1; /* no such attribute */
  *r_value = retstr;
  return 0;
}
...
ksba_sexp_t ksba_cms_get_enc_val(ksba_cms_t cms, int idx) {
  ...
  if (!cms) return NULL;
  if (!cms->recp_info) return NULL;
  if (idx < 0) return NULL;

  for (vt = cms->recp_info; vt && idx; vt = vt->next, idx--)
    ;
  if (!vt) return NULL; /* No value at this IDX */
...
gpg_error_t ksba_cms_hash_signed_attrs(ksba_cms_t cms, int idx) {
  ...
  if (!cms) return GPG_ERR_INV_VALUE;
  if (!cms->hash_fnc) return GPG_ERR_MISSING_ACTION;
  if (idx < 0) return -1;
  ...
  if (!n || n->off == -1) return GPG_ERR_NO_VALUE;

  /* We don't hash the implicit tag [0] but a SET tag */
  cms->hash_fnc(cms->hash_fnc_arg, "\x31", 1);
  cms->hash_fnc(cms->hash_fnc_arg, si->image + n->off + 1,
                n->nhdr + n->len - 1);
...
gpg_error_t ksba_cms_set_content_type(ksba_cms_t cms, int what,
  ...
  if (!cms || what < 0 || what > 1) return GPG_ERR_INV_VALUE;

  for (i = 0; content_handlers[i].oid; i++) {
    if (content_handlers[i].ct == type) break;
  }
  if (!content_handlers[i].oid) return GPG_ERR_UNKNOWN_CMS_OBJ;
  if (!content_handlers[i].build_handler) return GPG_ERR_UNSUPPORTED_CMS_OBJ;
  oid = xtrystrdup(content_handlers[i].oid);
  if (!oid) return GPG_ERR_ENOMEM;
...
gpg_error_t ksba_cms_add_digest_algo(ksba_cms_t cms, const char *oid) {
  ...
  if (!cms || !oid) return GPG_ERR_INV_VALUE;

  ol = (oidlist_s *)xtrymalloc(sizeof *ol);
  if (!ol) return GPG_ERR_ENOMEM;
...
gpg_error_t ksba_cms_add_cert(ksba_cms_t cms, ksba_cert_t cert) {
  ...
  if (!cms || !cert) return GPG_ERR_INV_VALUE;

  /* first check whether this is a duplicate. */
  for (cl = cms->cert_info_list; cl; cl = cl->next) {
    if (!_ksba_cert_cmp(cert, cl->cert)) return 0; /* duplicate */
  }
...
gpg_error_t ksba_cms_add_smime_capability(ksba_cms_t cms, const char *oid,
  ...
  if (!opl) return gpg_error_from_errno(errno);
  opl->next = NULL;
  ...
  opl->parmlen = derlen;
  if (der) memcpy(opl->parm, der, derlen);
...
gpg_error_t ksba_cms_set_message_digest(ksba_cms_t cms, int idx,
  ...
  if (!cms || !digest) return GPG_ERR_INV_VALUE;
  if (!digest_len || digest_len > DIM(cl->msg_digest)) return GPG_ERR_INV_VALUE;
  if (idx < 0) return GPG_ERR_INV_INDEX;

  for (cl = cms->cert_list; cl && idx; cl = cl->next, idx--)
    ;
  if (!cl) return GPG_ERR_INV_INDEX; /* no certificate to store it */
 ...
 */
gpg_error_t ksba_cms_set_sig_val(ksba_cms_t cms, int idx,
  ...
  if (!cms) return GPG_ERR_INV_VALUE;
  if (idx < 0) return GPG_ERR_INV_INDEX; /* only one signer for now */

  s = sigval;
  if (*s != '(') return GPG_ERR_INV_SEXP;
  s++;

  for (i = 0, sv_tail = &cms->sig_val; *sv_tail;
       sv_tail = &(*sv_tail)->next, i++)
    ;
  if (i != idx) return GPG_ERR_INV_INDEX;

  if (!(n = snext(&s))) return GPG_ERR_INV_SEXP;
  if (!smatch(&s, 7, "sig-val")) return GPG_ERR_UNKNOWN_SEXP;
  if (*s != '(') return digitp(s) ? GPG_ERR_UNKNOWN_SEXP : GPG_ERR_INV_SEXP;
  s++;
  ...
  if (!sv) return GPG_ERR_ENOMEM;
  if (n == 3 && s[0] == 'r' && s[1] == 's' &&
      s[2] == 'a') { /* kludge to allow "rsa" to be passed as algorithm name */
  ...
  if (n > 1 &&
      !*s) { /* We might have a leading zero due to the way we encode
...
gpg_error_t ksba_cms_set_content_enc_algo(ksba_cms_t cms, const char *oid,
  ...
  if (!cms->encr_algo_oid) return GPG_ERR_ENOMEM;

  if (iv) {
    cms->encr_iv = (char *)xtrymalloc(ivlen);
    if (!cms->encr_iv) return GPG_ERR_ENOMEM;
 ...
 * Note the <algo> must be given as a stringified OID or the special
 * string "rsa" */
gpg_error_t ksba_cms_set_enc_val(ksba_cms_t cms, int idx,
  ...
  for (cl = cms->cert_list; cl && idx; cl = cl->next, idx--)
    ;
  if (!cl) return GPG_ERR_INV_INDEX; /* no certificate to store the value */
  ...
  s++;
  if (n != 7 || memcmp(s, "enc-val", 7)) return GPG_ERR_UNKNOWN_SEXP;
  s += 7;
  if (*s != '(') return digitp(s) ? GPG_ERR_UNKNOWN_SEXP : GPG_ERR_INV_SEXP;
  s++;
  ...
  if (n == 3 && s[0] == 'r' && s[1] == 's' &&
    ...
    if (!cl->enc_val.algo) return GPG_ERR_ENOMEM;
  } else {
  ...
  n = strtoul(s, (char **)&endp, 10);
  s = endp;
  if (!n || *s != ':') return GPG_ERR_INV_SEXP;
  s++;
  s += n; /* ignore the name of the parameter */
  ...
  if (!cl->enc_val.value) return GPG_ERR_ENOMEM;
...
static gpg_error_t ct_parse_signed_data(ksba_cms_t cms) {
  enum { sSTART, sGOT_HASH, sIN_DATA, sERROR } state = sERROR;
  ...
  if (state == sSTART) {
    if (cms->detached_data && !cms->data.digest) { /* We use this stop reason to
                                                      inform the caller about a
                                                      detached signatures.
                                                      Actually there is no need
                                                      for him
                                                      to hash the data now, he
                                                      can do this also later. */
      stop_reason = KSBA_SR_NEED_HASH;
    } else { /* The user must now provide a hash function so that we can
...
static gpg_error_t ct_parse_enveloped_data(ksba_cms_t cms) {
  enum { sSTART, sREST, sINDATA, sERROR } state = sERROR;
  ksba_stop_reason_t stop_reason = cms->stop_reason;
  gpg_error_t err = 0;
  ...
  if (state == sSTART) {
    stop_reason =
        cms->detached_data ? KSBA_SR_DETACHED_DATA : KSBA_SR_BEGIN_DATA;
  } else if (state == sINDATA)
    stop_reason = KSBA_SR_END_DATA;
  else if (state == sREST)
    stop_reason = KSBA_SR_READY;
...
static gpg_error_t build_signed_data_header(ksba_cms_t cms) {
  ...
  if (err) return err;
  err =
      _ksba_ber_write_tl(cms->writer, TYPE_OBJECT_ID, CLASS_UNIVERSAL, 0, len);
  if (!err) err = ksba_writer_write(cms->writer, buf, len);
  xfree(buf);
  if (err) return err;
  ...
  /* SET OF DigestAlgorithmIdentifier */
  {
    ...
    err =
        _ksba_ber_write_tl(cms->writer, TYPE_SET, CLASS_UNIVERSAL, 1, valuelen);
    if (!err) err = ksba_writer_write(cms->writer, value, valuelen);
    xfree(value);
    if (err) return err;
  }
  ...
  if (!cms->detached_data) { /* write the tag */
    err = _ksba_ber_write_tl(cms->writer, 0, CLASS_CONTEXT, 1, 0);
    if (err) return err;
  }
...
static gpg_error_t set_issuer_serial(AsnNode info, ksba_cert_t cert, int mode) {
  ...
  if (!info || !cert) return GPG_ERR_INV_VALUE;

  src = _ksba_asn_find_node(cert->root,
                            "Certificate.tbsCertificate.serialNumber");
  dst = _ksba_asn_find_node(info,
                            mode ? "rid.issuerAndSerialNumber.serialNumber"
                                 : "sid.issuerAndSerialNumber.serialNumber");
  err = _ksba_der_copy_tree(dst, src, cert->image);
  if (err) return err;

  src = _ksba_asn_find_node(cert->root, "Certificate.tbsCertificate.issuer");
  dst = _ksba_asn_find_node(info,
                            mode ? "rid.issuerAndSerialNumber.issuer"
...
static gpg_error_t store_smime_capability_sequence(
    AsnNode node, struct oidparmlist_s *capabilities) {
  gpg_error_t err;
  ...
  for (cap = capabilities; cap; cap = cap->next) {
    ...
    for (cap2 = capabilities; cap2 != cap; cap2 = cap2->next) {
      if (!strcmp(cap->oid, cap2->oid) && cap->parmlen &&
    ...
    if (cap2 == cap) {
      ...
      err = _ksba_der_write_algorithm_identifier(
          tmpwrt, cap->oid, cap->parmlen ? cap->parm : (const void *)"",
          cap->parmlen);
      if (err) {
        ksba_writer_release(tmpwrt);
        return err;
  ...
  value = (unsigned char *)ksba_writer_snatch_mem(tmpwrt, &valuelen);
  if (!value) err = GPG_ERR_ENOMEM;
  if (!err) err = _ksba_der_store_sequence(node, value, valuelen);
  xfree(value);
  ksba_writer_release(tmpwrt);
  return err;
}
...
static gpg_error_t build_signed_data_attributes(ksba_cms_t cms) {
  ...
  if (cms->signer_info)
    return GPG_ERR_CONFLICT; /* This list must be empty at
                                            this point. */
  ...
  if (cms->cert_info_list) {
    ...
    for (certlist = cms->cert_info_list; certlist; certlist = certlist->next) {
      if (!ksba_cert_get_image(certlist->cert, &n))
        return GPG_ERR_GENERAL; /* User passed an
                                               unitialized cert */
      totallen += n;
    }
    ...
    for (certlist = cms->cert_info_list; certlist; certlist = certlist->next) {
      if (!(der = ksba_cert_get_image(certlist->cert, &n))) return GPG_ERR_BUG;
  ...
  if (!certlist) {
    err = GPG_ERR_MISSING_VALUE; /* oops */
    goto leave;
  }
  ...
  for (signer = 0; certlist;
    ...
    if (!n || !n->down) return GPG_ERR_ELEMENT_NOT_FOUND;
    ...
    /* Arggh.  That silly ASN.1 DER encoding rules: We need to sort
       the SET values. */
    qsort(attrarray, attridx, sizeof(struct attrarray_s), compare_attrarray);
...
static gpg_error_t build_signed_data_rest(ksba_cms_t cms) {
  ...
  for (signer = 0; certlist; signer++, certlist = certlist->next,
      digestlist = digestlist->next, si = si->next, sv = sv->next) {
    AsnNode n, n2;
    unsigned char *image;
    size_t imagelen;
    ...
    err = _ksba_der_store_integer(
        n, (const unsigned char *)("\x00\x00\x00\x01\x01"));
    if (err) goto leave;
    ...
    err = _ksba_der_store_octet_string(n, (const char *)(sv->value),
                                       sv->valuelen);
    if (err) goto leave;
  ...
  /* Write 3 end tags */
  err = _ksba_ber_write_tl(cms->writer, 0, (tag_class)(0), 0, 0);
  if (!err) err = _ksba_ber_write_tl(cms->writer, 0, (tag_class)(0), 0, 0);
  if (!err) err = _ksba_ber_write_tl(cms->writer, 0, (tag_class)(0), 0, 0);
...
static gpg_error_t ct_build_signed_data(ksba_cms_t cms) {
  enum { sSTART, sDATAREADY, sGOTSIG, sERROR } state = sERROR;
  ...
  if (stop_reason == KSBA_SR_GOT_CONTENT) {
    ...
    if (!cms->sig_val)
      err = GPG_ERR_MISSING_ACTION; /* No ksba_cms_set_sig_val () called */
    state = sGOTSIG;
  } else if (stop_reason == KSBA_SR_RUNNING)
    err = GPG_ERR_INV_STATE;
  else if (stop_reason)
  ...
  if (state == sSTART) {
    ...
    if (!cms->detached_data)
      err = _ksba_ber_write_tl(cms->writer, 0, (tag_class)(0), 0, 0);
    if (!err) err = build_signed_data_attributes(cms);
  ...
  if (state == sSTART) {
    ...
    stop_reason = cms->detached_data ? KSBA_SR_END_DATA : KSBA_SR_BEGIN_DATA;
  } else if (state == sDATAREADY)
...
static gpg_error_t build_enveloped_data_header(ksba_cms_t cms) {
  ...
  */
  ...
  for (recpno = 0; certlist; recpno++, certlist = certlist->next) {
    ...
    err = _ksba_der_store_integer(
        n, (const unsigned char *)("\x00\x00\x00\x01\x00"));
    if (err) goto leave;
    ...
    n = _ksba_asn_find_node(
        root, "RecipientInfo.ktri.keyEncryptionAlgorithm.algorithm");
    if (!n) {
      err = GPG_ERR_ELEMENT_NOT_FOUND;
      goto leave;
    }
    ...
    if (err) goto leave;
    n = _ksba_asn_find_node(
        root, "RecipientInfo.ktri.keyEncryptionAlgorithm.parameters");
    if (!n) {
      err = GPG_ERR_ELEMENT_NOT_FOUND;
      goto leave;
    }
    ...
    err = _ksba_der_store_octet_string(
        n, (const char *)(certlist->enc_val.value), certlist->enc_val.valuelen);
    if (err) goto leave;
  ...
  err = _ksba_der_write_algorithm_identifier(cms->writer, cms->encr_algo_oid,
                                             cms->encr_iv, cms->encr_ivlen);
  if (err) return err;
...
static gpg_error_t ct_build_enveloped_data(ksba_cms_t cms) {
  enum { sSTART, sINDATA, sREST, sERROR } state = sERROR;
  ksba_stop_reason_t stop_reason;
  gpg_error_t err = 0;
  ...
  /* Calculate new stop reason */
  if (state == sSTART) { /* user should now write the encrypted data */
    stop_reason = KSBA_SR_BEGIN_DATA;
  } else if (state == sINDATA) { /* tell the user that we wrote everything */
    stop_reason = KSBA_SR_END_DATA;
  } else if (state == sREST) {
    stop_reason = KSBA_SR_READY;
  }
