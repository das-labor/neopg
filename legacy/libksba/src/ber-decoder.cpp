...
static inline int sum_a1_a2_gt_b(size_t a1, size_t a2, size_t b) {
  size_t sum = a1 + a2;
  return (sum < a1 || sum > b);
}

/* Evaluate with overflow check:  A1 + A2 >= B  */
static inline int sum_a1_a2_ge_b(size_t a1, size_t a2, size_t b) {
  size_t sum = a1 + a2;
  return (sum < a1 || sum >= b);
}
...
static void release_decoder_state(DECODER_STATE ds) { xfree(ds); }
...
static void dump_decoder_state(DECODER_STATE ds) {
  ...
  for (i = 0; i < ds->idx; i++) {
    ...
    fprintf(stderr, ") %s%d (%d)%s\n", ds->stack[i].ndef_length ? "ndef " : "",
            ds->stack[i].length, ds->stack[i].nread,
...
static int eof_or_error(BerDecoder d, int premature) {
  ...
  if (premature) return set_error(d, NULL, "premature EOF");
  return GPG_ERR_EOF;
}

static const char *universal_tag_name(unsigned long no) {
  static const char *const names[31] = {"[End Tag]",         "BOOLEAN",
                                        "INTEGER",           "BIT STRING",
                                        "OCTECT STRING",     "NULL",
                                        "OBJECT IDENTIFIER", "ObjectDescriptor",
                                        "EXTERNAL",          "REAL",
                                        "ENUMERATED",        "EMBEDDED PDV",
                                        "UTF8String",        "RELATIVE-OID",
                                        "[UNIVERSAL 14]",    "[UNIVERSAL 15]",
                                        "SEQUENCE",          "SET",
                                        ...
                                        "UniversalString",   "CHARACTER STRING",
                                        "BMPString"};
...
static void dump_tlv(const struct tag_info *ti, FILE *fp) {
  ...
  if (ti->klasse == CLASS_UNIVERSAL) tagname = universal_tag_name(ti->tag);

  if (tagname)
    fputs(tagname, fp);
  else
    fprintf(fp, "[%s %lu]",
            ti->klasse == CLASS_UNIVERSAL
                ? "UNIVERSAL"
                : ti->klasse == CLASS_APPLICATION
                      ? "APPLICATION"
                      : ti->klasse == CLASS_CONTEXT ? "CONTEXT-SPECIFIC"
                                                    : "PRIVATE",
            ti->tag);
  fprintf(fp, " %c hdr=%lu len=", ti->is_constructed ? 'c' : 'p',
...
static void prepare_copied_tree(AsnNode node) {
  ...
  for (p = node; p; p = _ksba_asn_walk_tree(node, p)) p->off = -1;
}
...
static void fixup_type_any(AsnNode node) {
  ...
  for (p = node; p; p = _ksba_asn_walk_tree(node, p)) {
    if (p->type == TYPE_ANY && p->off != -1) p->type = p->actual_type;
  }
}
...
void _ksba_ber_decoder_release(BerDecoder d) { xfree(d); }
...
gpg_error_t _ksba_ber_decoder_set_module(BerDecoder d, ksba_asn_tree_t module) {
  if (!d || !module) return GPG_ERR_INV_VALUE;
  if (d->module) return GPG_ERR_CONFLICT; /* module already set */
...
gpg_error_t _ksba_ber_decoder_set_reader(BerDecoder d, ksba_reader_t r) {
  if (!d || !r) return GPG_ERR_INV_VALUE;
  if (d->reader) return GPG_ERR_CONFLICT; /* reader already set */
...
static int read_buffer(ksba_reader_t reader, char *buffer, size_t count) {
  ...
  if (buffer) {
    ...
    while (count) {
      ...
      if (ksba_reader_read(reader, dummy, n, &nread)) return -1;
      count -= nread;
    }
  }
  return 0;
...
static int cmp_tag(AsnNode node, const struct tag_info *ti) {
  ...
  if (node->type == ti->tag) return 1;
  if (ti->klasse == CLASS_UNIVERSAL) {
    if (node->type == TYPE_SEQUENCE_OF && ti->tag == TYPE_SEQUENCE) return 1;
    if (node->type == TYPE_SET_OF && ti->tag == TYPE_SET) return 1;
...
static AsnNode find_anchor_node(AsnNode root, const struct tag_info *ti) {
  ...
  while (node) {
    ...
    if (node->down)
      ...
      do {
        while (node->left && node->left->right == node) node = node->left;
        node = node->left;
...
static int match_der(AsnNode root, const struct tag_info *ti, DECODER_STATE ds,
                     AsnNode *retnode, int debug) {
  int rc;
  AsnNode node;
  ...
  *retnode = NULL;
  ...
  if (!node) {
    ...
    {
      if (debug) fputs("  going up after choice - get next\n", stderr);
      while (node->left && node->left->right == node) node = node->left;
      node = node->left; /* this is the up pointer */
      if (node) node = node->right;
    }
    ...
    if (debug) {
      fprintf(stderr, "  prepare for seq/set_of (%d %d)  ", ds->cur.length,
              ds->cur.nread);
      fprintf(stderr, "  cur: (");
    ...
    } else if (ds->cur.node->flags.in_array && ds->cur.went_up) {
      if (debug) fputs("  Reiterating\n", stderr);
      node = _ksba_asn_insert_copy(node);
      if (node) prepare_copied_tree(node);
    } else
      node = node->down;
  } else /* constructed */
  {
    if (debug) {
      fprintf(stderr, "  prepare for constructed (%d %d) ", ds->cur.length,
              ds->cur.nread);
      fprintf(stderr, "  cur: (");
      _ksba_asn_node_dump(node, stderr);
      fprintf(stderr, ")\n");
    ...
    if (ds->cur.node->flags.in_array && ds->cur.went_up) {
      if (debug) fputs("  Reiterating this\n", stderr);
      node = _ksba_asn_insert_copy(node);
      if (node) prepare_copied_tree(node);
    } else if (ds->cur.went_up || ds->cur.next_tag ||
               ds->cur.node->flags.skip_this) {
      if (node->right)
        node = node->right;
      else {
        for (;;) {
  ...
  if (node->flags.skip_this) {
    if (debug) fprintf(stderr, "   skipping this\n");
    return 1;
  }

  if (node->type == TYPE_SIZE) {
    if (debug) fprintf(stderr, "   skipping size tag\n");
    return 1;
  }
  if (node->type == TYPE_DEFAULT) {
    if (debug) fprintf(stderr, "   skipping default tag\n");
    return 1;
  }

  if (node->flags.is_implicit) {
    if (debug) fprintf(stderr, "   dummy accept for implicit tag\n");
    return 1; /* again */
  }
  ...
  if (node->type == TYPE_CHOICE) {
    if (debug) fprintf(stderr, "   testing choice...\n");
  ...
  if (node->flags.in_choice) {
    if (debug) fprintf(stderr, "   skipping non matching choice\n");
    return 1;
  }

  if (node->flags.is_optional) {
    if (debug) fprintf(stderr, "   skipping optional element\n");
    if (node->type == TYPE_TAG) ds->cur.next_tag = 1;
    return 1;
  }

  if (node->flags.has_default) {
    if (debug) fprintf(stderr, "   use default value\n");
    if (node->type == TYPE_TAG) ds->cur.next_tag = 1;
    *retnode = node;
    return 2;
  }
...
static void decoder_deinit(BerDecoder d) {
  ...
  d->ds = NULL;
  d->val.node = NULL;
  if (d->debug) fprintf(stderr, "DECODER_DEINIT\n");
}
...
static gpg_error_t decoder_next(BerDecoder d) {
  ...
  if (err) {
    ...
    if (err == GPG_ERR_BAD_BER && d->ignore_garbage && ti.err_string &&
        !strcmp(ti.err_string, "premature EOF"))
      err = GPG_ERR_EOF;
    return err;
  }
  ...
  if (d->use_image) {
    if (!d->image.buf) {
      ...
      if (d->image.length < ti.length) return GPG_ERR_BAD_BER;
      if (d->image.length > MAX_IMAGE_LENGTH) return GPG_ERR_TOO_LARGE;
      d->image.buf = (unsigned char *)xtrycalloc(1, d->image.length);
      if (!d->image.buf) return GPG_ERR_ENOMEM;
    }
  ...
  if (!d->bypass) {
    ...
    do {
      again = endtag = 0;
      switch (ds->cur.in_any
                  ? 4
                  : (ti.klasse == CLASS_UNIVERSAL && !ti.tag)
                        ? (endtag = 1, 5)
        ...
        case -1:
          ...
          break;
        case 0:
          if (debug) fputs("  End of description\n", stderr);
          d->bypass = 1;
          break;
        case 1: /* again */
          if (debug) fprintf(stderr, "  Again\n");
          again = 1;
          break;
        case 2: /* Use default value +  again */
          if (debug) fprintf(stderr, "  Using default\n");
          again = 1;
          break;
        case 4: /* Match of ANY on a constructed type */
          if (debug) fprintf(stderr, "  ANY");
          ds->cur.in_any = 1;
        case 3: /* match */
        case 5: /* end tag */
          if (debug) {
          ...
          if (!ti.is_constructed) ds->cur.nread += ti.length;
          ...
          do {
            if (debug)
              fprintf(stderr, "  (length %d nread %d) %s\n",
                      ...
                      ds->cur.nread, ti.is_constructed ? "con" : "pri");
            if (d->outer_sequence_length && ds->idx == 1 &&
                ds->cur.nread == d->outer_sequence_length) {
              if (debug) fprintf(stderr, "  Need to stop now\n");
              d->ignore_garbage = 1;
            }

            if (ds->idx && !ds->stack[ds->idx - 1].ndef_length &&
                (ds->cur.nread > ds->stack[ds->idx - 1].length)) {
              fprintf(stderr,
                      "ksba: ERROR: object length field "
                      "%d octects too large\n",
                      ds->cur.nread - ds->cur.length);
              ds->cur.nread = ds->cur.length;
            }
            if (ds->idx && (endtag || (!ds->stack[ds->idx - 1].ndef_length &&
                                       (ds->cur.nread >=
            ...
            endtag = 0;
          } while (ds->idx && !ds->stack[ds->idx - 1].ndef_length &&
                   (ds->cur.nread >= ds->stack[ds->idx - 1].length));
  ...
  if (debug) dump_decoder_state(ds);
...
static gpg_error_t decoder_skip(BerDecoder d) {
  if (d->val.primitive) {
    if (read_buffer(d->reader, NULL, d->val.length)) return eof_or_error(d, 1);
  }
  return 0;
}
...
gpg_error_t _ksba_ber_decoder_dump(BerDecoder d, FILE *fp) {
  ...
  while (!(err = decoder_next(d))) {
    node = d->val.node;
    if (node) depth = distance(d->root, node);

    fprintf(fp, "%4lu %4lu:%*s", ksba_reader_tell(d->reader) - d->val.nhdr,
            (unsigned long)d->val.length, depth * 2, "");
    if (node)
    ...
    if (node && d->val.primitive) {
      ...
      if (!buf || buflen < d->val.length) {
        ...
        if (buflen < d->val.length)
          ...
          if (!buf) err = gpg_error_from_syserror();
        }
      }

      for (n = 0; !err && n < d->val.length; n++) {
        if ((c = read_byte(d->reader)) == -1) err = eof_or_error(d, 1);
        buf[n] = c;
      }
      if (err) break;
      fputs("  (", fp);
...
gpg_error_t _ksba_ber_decoder_decode(BerDecoder d, const char *start_name,
  ...
  while (!(err = decoder_next(d))) {
    ...
    if (d->use_image) {
      if (node &&
          !d->val.is_endtag) { /* We don't have nodes for the end tag - so don't
                                  store it */
        node->off = (ksba_reader_tell(d->reader) - d->val.nhdr - startoff);
        node->nhdr = d->val.nhdr;
        node->len = d->val.length;
        if (node->type == TYPE_ANY) node->actual_type = (node_type_t)d->val.tag;
      }
      if (sum_a1_a2_gt_b(d->image.used, d->val.length, d->image.length))
        ...
        if (read_buffer(d->reader, (char *)(d->image.buf + d->image.used),
                        d->val.length))
  ...
  if (err) xfree(d->image.buf);

  if (r_root && !err) {
    if (!d->image.buf) { /* Not even the first node available - return eof */
