...
static void test_hex2bin(void) {
  ...
  static const char *invalid[] = {
      ...
      NULL};
  static const char *valid2[] = {"00", "00 x", NULL};
  static const char *invalid2[] = {"", "0", "00:", "00x", " 00", NULL};
  ...
  for (i = 0; valid[i]; i++) {
    ...
    if (memcmp(buffer, ("\x00\x11\x22\x33\x44\x55\x66\x77\x88\x99\xaa"
                        "\xbb\xcc\xdd\xee\xff\x11\x22\x33\x44"),
               20))
      fail(i);
  }
  if (hex2bin(valid[0], buffer, sizeof buffer) != 40) fail(0);
  if (hex2bin(valid[2], buffer, sizeof buffer) != 41) fail(0);
  ...
  for (i = 0; valid2[i]; i++) {
    ...
    if (len < 0) fail(i);
    if (memcmp(buffer, "\x00", 1)) fail(i);
  }
  if (hex2bin(valid2[0], buffer, 1) != 2) fail(0);
  if (hex2bin(valid2[1], buffer, 1) != 3) fail(0);
...
static void test_hexcolon2bin(void) {
  static const char *valid[] = {
      "00112233445566778899aabbccddeeff11223344",
      "00112233445566778899AABBCCDDEEFF11223344",
  ...
  static const char *invalid[] = {
      ...
      NULL};
  static const char *valid2[] = {"00", "00 x", NULL};
  static const char *invalid2[] = {"",    "0",   "00:", ":00",
                                   "0:0", "00x", " 00", NULL};
  unsigned char buffer[20];
  int len;
  int i;
  ...
  if (hexcolon2bin(valid[0], buffer, sizeof buffer) != 40) fail(0);
  if (hexcolon2bin(valid[3], buffer, sizeof buffer) != 41) fail(0);
  ...
  if (hexcolon2bin(valid2[0], buffer, 1) != 2) fail(0);
  if (hexcolon2bin(valid2[1], buffer, 1) != 3) fail(0);
...
static void test_bin2hex(void) {
  ...
  if (!p) fail(0);
  if (p != buffer) fail(0);
  if (strcmp(buffer, hexstuff)) fail(0);
...
static void test_bin2hexcolon(void) {
  char stuff[20 + 1] =
      ("\x00\x11\x22\x33\x44\x55\x66\x77\x88\x99\xaa"
       "\xbb\xcc\xdd\xee\xff\x01\x10\x02\xa3");
  char hexstuff[] =
...
static void test_hex2str(void) {
  static struct {
    ...
    int no_alloc_test;
      ...
      {"6162636465666768696A6b6c6D6e6f70717273", "abcdefghijklmnopqrs", 19, 38},
      {"6162636465666768696A6b6c6D6e6f7071727300", "abcdefghijklmnopqrs", 20,
       40},
      {"6162636465666768696A6b6c6D6e6f7071727374", NULL, 0, 0, 1},
      {"6162636465666768696A6b6c6D6e6f707172737400", NULL, 0, 0, 1},
      {"6162636465666768696A6b6c6D6e6f707172737475", NULL, 0, 0, 1},
  ...
  for (idx = 0; tests[idx].hex; idx++) {
    ...
    if (tests[idx].str) {
      ...
      /* Bad case test.  */
      if (tail) fail(idx);
      if (strcmp(tmpbuf, tests[idx].hex)) fail(idx); /* Buffer was modified.  */
    }
  }

  /* Test the allocation variant.  */
  for (idx = 0; tests[idx].hex; idx++) {
    if (tests[idx].no_alloc_test) continue;
