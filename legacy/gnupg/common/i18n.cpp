...
#ifdef HAVE_LANGINFO_CODESET
  if (!orig_codeset) orig_codeset = nl_langinfo(CODESET);
#endif
  if (orig_codeset) { /* We only switch when we are able to restore the codeset
                         later.
                         Note that bind_textdomain_codeset does only return on
                         memory
                         errors but not if a codeset is not available.  Thus we
                         don't
...
const char *i18n_localegettext(const char *lc_messages, const char *string) {
  ...
  if (!lc_messages) goto leave;

  /* Lookup in the cache.  */
  for (mh = msgcache; mh; mh = mh->next)
    if (!strcmp(mh->lc_messages, lc_messages)) break;
  if (mh) {
  ...
  if (!saved) goto leave;
  if (!setlocale(LC_MESSAGES, lc_messages)) goto leave;
