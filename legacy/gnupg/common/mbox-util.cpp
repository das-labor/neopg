...
static int string_count_chr(const char *string, int c) {
  int count;
...
static int mem_count_chr(const void *buffer, int c, size_t length) {
...
static int string_has_ctrl_or_space(const char *string) {
  for (; *string; string++)
    if (!(*string & 0x80) && *string <= 0x20) return 1;
  return 0;
}

/* Return true if STRING has two consecutive '.' after an '@'
   sign.  */
static int has_dotdot_after_at(const char *string) {
  string = strchr(string, '@');
  if (!string) return 0; /* No at-sign.  */
  string++;
...
int has_invalid_email_chars(const void *buffer, size_t length) {
  ...
  for (; length && *s; length--, s++) {
    if ((*s & 0x80)) continue; /* We only care about ASCII.  */
    if (*s == '@')
      at_seen = 1;
    else if (!at_seen &&
             !(strchr(valid_chars, *s) || strchr("!#$%&'*+/=?^`{|}~", *s)))
      return 1;
...
/* Same as is_valid_mailbox (see below) but operates on non-nul
   terminated buffer.  */
int is_valid_mailbox_mem(const void *name_arg, size_t namelen) {
  const char *name = (const char *)name_arg;

  return !(!name || !namelen || has_invalid_email_chars(name, namelen) ||
           mem_count_chr(name, '@', namelen) != 1 || *name == '@' ||
...
char *mailbox_from_userid(const char *userid) {
  ...
  if (s) {
    ...
    if (s_end && s_end > s) {
      ...
      if (!result) return NULL; /* Ooops - out of core.  */
      ...
      if (string_count_chr(result, '@') != 1 /* Need exactly one '@.  */
          ...
          || string_has_ctrl_or_space(result) || has_dotdot_after_at(result)) {
        xfree(result);
        result = NULL;
        errno = EINVAL;
      }
