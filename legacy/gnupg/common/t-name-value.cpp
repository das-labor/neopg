...
void test_getting_values(nvc_t pk) {
  nve_t e;
...
void test_key_extraction(nvc_t pk) {
  gpg_error_t err;
  gcry_sexp_t key;
...
void test_iteration(nvc_t pk) {
  int i;
  nve_t e;

  i = 0;
  for (e = nvc_first(pk); e; e = nve_next(e)) i++;
  assert(i == 4);

  i = 0;
  for (e = nvc_lookup(pk, "Comment:"); e; e = nve_next_value(e, "Comment:"))
    i++;
  assert(i == 3);
}

void test_whitespace(nvc_t pk) {
  nve_t e;
...
static char *nvc_to_string(nvc_t pk) {
  gpg_error_t err;
  char *buf;
  size_t len;
  estream_t sink;
...
void *dummy_realloc(void *p, size_t s) {
  (void)s;
  return p;
}
...
void run_tests(void) {
  ...
  for (i = 0; i < DIM(tests); i++) {
    ...
    source =
        es_mopen(tests[i].value, len, len, 0, dummy_realloc, dummy_free, "r");
    assert(source);
    ...
    if (tests[i].test_func) tests[i].test_func(pk);

    nvc_release(pk);
  }
}

void run_modification_tests(void) {
  gpg_error_t err;
  nvc_t pk;
  gcry_sexp_t key;
  char *buf;
  ...
  buf = nvc_to_string(pk);
  assert(strcmp(buf,
                "Foo: A really long value spanning across multiple"
...
void convert(const char *fname) {
  ...
  if (source == NULL) goto leave;

  if (fstat(es_fileno(source), &st)) goto leave;

  buflen = st.st_size;
  buf = xtrymalloc(buflen + 1);
  assert(buf);

  if (es_fread(buf, buflen, 1, source) != 1) goto leave;
