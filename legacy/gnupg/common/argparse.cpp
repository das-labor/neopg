...
static void initialize(ARGPARSE_ARGS *arg, const char *filename,
  ...
  if (!(arg->flags & (1 << 15))) {
    ...
    if (*arg->argc < 0) log_bug("invalid argument for arg_parse\n");
  }
...
int optfile_parse(FILE *fp, const char *filename, unsigned *lineno,
  ...
  if (!*lineno) {
    ...
    if (unread_buf[0] != 0xef || unread_buf[1] != 0xbb || unread_buf[2] != 0xbf)
      unread_buf_count = 3;
  }
      ...
      if (state == -1)
        ...
        if (in_alias) {
          ...
          if (buffer) {
            ...
            if (!set_opt_arg(arg, opts[idx].flags, p)) xfree(buffer);
          }
        }
        break;
      } else if (c == EOF) {
...
static int find_long_option(ARGPARSE_ARGS *arg, const ARGPARSE_OPTS *opts,
                            const char *keyword) {
  int i;
  size_t n;
...
static size_t long_opt_strlen(const ARGPARSE_OPTS *o) {
 ...
 */
static void show_help(const ARGPARSE_OPTS *opts, unsigned int flags) {
  const char *s;
  char tmp[2];
