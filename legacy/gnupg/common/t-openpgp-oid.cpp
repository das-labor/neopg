...
#define fail(a, e)                                                           \
  do {                                                                       \
    fprintf(stderr, "%s:%d: test %d failed (%s)\n", __FILE__, __LINE__, (a), \
            gpg_strerror(e));                                                \
    exit(1);                                                                 \
  } while (0)
...
static void test_openpgp_oid_from_str(void) {
  static char *sample_oids[] = {
      "0.0",          "1.0",          "1.2.3", "1.2.840.10045.3.1.7",
      "1.3.132.0.34", "1.3.132.0.35", NULL};
  gpg_error_t err;
  gcry_mpi_t a;
  int idx;
  char *string;
  ...
  if (gpg_err_code(err) != GPG_ERR_INV_VALUE) fail(0, err);
  gcry_mpi_release(a);

  err = openpgp_oid_from_str(".", &a);
  if (gpg_err_code(err) != GPG_ERR_INV_OID_STRING) fail(0, err);
  gcry_mpi_release(a);
  ...
  for (idx = 0; sample_oids[idx]; idx++) {
    ...
    if (!string) fail(idx, gpg_error_from_syserror());
    if (strcmp(string, sample_oids[idx])) fail(idx, 0);
    xfree(string);

    p = gcry_mpi_get_opaque(a, &nbits);
    length = (nbits + 7) / 8;
    if (!p || !length || p[0] != length - 1) fail(idx, 0);
...
static void test_openpgp_oid_to_str(void) {
  static struct {
    ...
    unsigned char der[10];
      ...
      {"1.3.132.0.34", {5, 0x2B, 0x81, 0x04, 0x00, 0x22}},

      {"1.3.132.0.35", {5, 0x2B, 0x81, 0x04, 0x00, 0x23}},

      {BADOID, {9, 0x80, 0x02, 0x70, 0x50, 0x25, 0x46, 0xfd, 0x0c, 0xc0}},
  ...
  for (idx = 0; samples[idx].string; idx++) {
    ...
    if (!a) fail(idx, gpg_error_from_syserror());

    string = openpgp_oid_to_str(a);
    if (!string) fail(idx, gpg_error_from_syserror());
    if (strcmp(string, samples[idx].string)) fail(idx, 0);
    xfree(string);
    gcry_mpi_release(a);
  }
}

static void test_openpgp_oid_is_ed25519(void) {
  static struct {
    int yes;
    const char *oidstr;
  } samples[] = {
  ...
  for (idx = 0; samples[idx].oidstr; idx++) {
    ...
    if (openpgp_oid_is_ed25519(a) != samples[idx].yes) fail(idx, 0);

    gcry_mpi_release(a);
  }
}

static void test_openpgp_enum_curves(void) {
  int iter = 0;
  const char *name;
  int p256 = 0;
  int p384 = 0;
