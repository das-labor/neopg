...
/* Helper */
static inline gpg_error_t my_error_from_syserror(void) {
  return gpg_error(gpg_err_code_from_syserror());
}

static inline gpg_error_t my_error(int errcode) { return gpg_error(errcode); }
...
static char *build_w32_commandline_copy(char *buffer, const char *string) {
  char *p = buffer;
  const char *s;
...
static gpg_error_t build_w32_commandline(const char *pgmname,
  ...
  *cmdline = NULL;
  ...
  for (; *s; s++)
    if (*s == '\"') n++; /* Need to double inner quotes.  */
  for (i = 0; (s = argv[i]); i++) {
    n += strlen(s) + 1 + 2; /* (1 space, 2 quoting */
    for (; *s; s++)
      if (*s == '\"') n++; /* Need to double inner quotes.  */
  ...
  buf = p = xtrymalloc(n);
  if (!buf) return my_error_from_syserror();
...
static int create_inheritable_pipe(HANDLE filedes[2], int flags) {
  HANDLE r, w;
  ...
  if (!CreatePipe(&r, &w, &sec_attr, 0)) return -1;

  if ((flags & INHERIT_READ) == 0)
    if (!SetHandleInformation(r, HANDLE_FLAG_INHERIT, 0)) goto fail;

  if ((flags & INHERIT_WRITE) == 0)
    if (!SetHandleInformation(w, HANDLE_FLAG_INHERIT, 0)) goto fail;
...
static HANDLE w32_open_null(int for_write) {
  HANDLE hfile;

  hfile = CreateFileW(L"nul", for_write ? GENERIC_WRITE : GENERIC_READ,
...
gpg_error_t gnupg_spawn_process(const char *pgmname, const char *argv[],
  ...
  HANDLE nullhd[3] = {INVALID_HANDLE_VALUE, INVALID_HANDLE_VALUE,
  ...
  *pid = (pid_t)(-1); /* Always required.  */
  ...
  if (r_outfp) {
    ...
    if (!outfp) {
      ...
      if (infp)
        ...
        CloseHandle(inpipe[1]);
      if (inpipe[0] != INVALID_HANDLE_VALUE) CloseHandle(inpipe[0]);
      return err;
    }
  }
  ...
  if (r_errfp) {
    ...
    if (!errfp) {
      ...
      if (outfp)
        ...
        CloseHandle(outpipe[0]);
      if (outpipe[1] != INVALID_HANDLE_VALUE) CloseHandle(outpipe[1]);
      if (infp)
        es_fclose(infp);
      else if (inpipe[1] != INVALID_HANDLE_VALUE)
        CloseHandle(inpipe[1]);
  ...
  if (inpipe[0] == INVALID_HANDLE_VALUE) nullhd[0] = w32_open_null(0);
  if (outpipe[1] == INVALID_HANDLE_VALUE) nullhd[1] = w32_open_null(1);
  if (errpipe[1] == INVALID_HANDLE_VALUE) nullhd[2] = w32_open_null(1);
  ...
  cr_flags = (CREATE_DEFAULT_ERROR_MODE |
              ...
              GetPriorityClass(GetCurrentProcess()) | CREATE_SUSPENDED);
  ...
  if (!CreateProcess(pgmname,   /* Program to start.  */
    ...
    if (errfp)
      ...
      CloseHandle(errpipe[0]);
    if (errpipe[1] != INVALID_HANDLE_VALUE) CloseHandle(errpipe[1]);
  ...
  for (i = 0; i < DIM(nullhd); i++)
    if (nullhd[i] != INVALID_HANDLE_VALUE) CloseHandle(nullhd[i]);
  ...
  if (r_infp) *r_infp = infp;
  if (r_outfp) *r_outfp = outfp;
  if (r_errfp) *r_errfp = errfp;
...
gpg_error_t gnupg_spawn_process_fd(const char *pgmname, const char *argv[],
  ...
  *pid = (pid_t)(-1);
   ...
   */
  if (!CreateProcess(
          pgmname,   /* Program to start.  */
          cmdline,   /* Command line arguments.  */
          &sec_attr, /* Process security attributes.  */
          &sec_attr, /* Thread security attributes.  */
          TRUE,      /* Inherit handles.  */
          (CREATE_DEFAULT_ERROR_MODE | GetPriorityClass(GetCurrentProcess()) |
  ...
  xfree(cmdline);
  for (i = 0; i < 3; i++)
    if (stdhd[i] != INVALID_HANDLE_VALUE) CloseHandle(stdhd[i]);
  if (err) return err;
...
gpg_error_t gnupg_wait_processes(const char **pgmnames, pid_t *pids,
  ...
  if (procs == NULL) return my_error_from_syserror();

  for (i = 0; i < count; i++) {
    if (r_exitcodes) r_exitcodes[i] = -1;

    if (pids[i] == (pid_t)(-1)) return my_error(GPG_ERR_INV_VALUE);
    ...
    case WAIT_OBJECT_0:
      for (i = 0; i < count; i++) {
        ...
        if (!GetExitCodeProcess(procs[i], &exc)) {
          ...
          if (r_exitcodes) r_exitcodes[i] = 0;
        }
      }
      break;
...
gpg_error_t gnupg_spawn_process_detached(const char *pgmname,
  ...
  if (access(pgmname, X_OK)) return my_error_from_syserror();
