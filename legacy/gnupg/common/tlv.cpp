...
static const unsigned char *do_find_tlv(const unsigned char *buffer,
                                        size_t length, int tag, size_t *nbytes,
                                        int nestlevel) {
  ...
  for (;;) {
    ...
    if (!*s || *s == 0xff) { /* Skip optional filler between TLV objects. */
      s++;
      n--;
      continue;
    }
    composite = !!(*s & 0x20);
    if ((*s & 0x1f) == 0x1f) { /* more tag bytes to follow */
      s++;
      n--;
      if (n < 2)
        return NULL; /* buffer definitely too short for tag and length. */
      if ((*s & 0x1f) == 0x1f) return NULL; /* We support only up to 2 bytes. */
    ...
    if (len < 0x80)
      ;
    else if (len == 0x81) { /* One byte length follows. */
      if (!n) return NULL;  /* we expected 1 more bytes with the length. */
      len = s[0];
      s++;
      n--;
    } else if (len == 0x82) { /* Two byte length follows. */
      if (n < 2) return NULL; /* We expected 2 more bytes with the length. */
    ...
    if (composite &&
        nestlevel <
            100) { /* Dive into this composite DO after checking for a too deep
                      nesting. */
    ...
    if (len > n) return NULL; /* Buffer too short to skip to the next tag. */
    s += len;
    n -= len;
  }
}
...
gpg_error_t parse_ber_header(unsigned char const **buffer, size_t *size,
                             int *r_class, int *r_tag, int *r_constructed,
  ...
  *r_tag = tag;
  ...
  if (*r_class == CLASS_UNIVERSAL && !*r_tag) *r_length = 0;
...
/* Return the next token of an canonical encoded S-expression.  BUF
   ...
   if (err)
     handle_error ();
 */
gpg_error_t parse_sexp(unsigned char const **buf, size_t *buflen, int *depth,
                       unsigned char const **tok, size_t *toklen) {
  ...
  *tok = NULL;
  *toklen = 0;
  if (!n) return *depth ? GPG_ERR_INV_SEXP : 0;
  if (*s == '(') {
    s++;
    n--;
    (*depth)++;
  ...
  if (*s == ')') {
    if (!*depth) return GPG_ERR_INV_SEXP;
    *toklen = 1;
    s++;
    n--;
    (*depth)--;
  ...
  s++;
  n--;
  if (vlen > n) return GPG_ERR_INV_SEXP;
  *tok = s;
  *toklen = vlen;
  s += vlen;
  n -= vlen;
