...
static char *sexp_to_string(gcry_sexp_t sexp) {
  size_t n;
  char *result;
...
char *canon_sexp_to_string(const unsigned char *canon, size_t canonlen) {
  ...
  if (!n) return NULL;
  if (gcry_sexp_sscan(&sexp, NULL, (const char *)(canon), n)) return NULL;
...
/* Print the canonical encoded S-expression in SEXP in advanced
   ...
   entire debug line. */
void log_printcanon(const char *text, const unsigned char *sexp,
                    size_t sexplen) {
...
/* Helper function to create a canonical encoded S-expression from a
   ...
   R_BUFLEN.  */
gpg_error_t make_canon_sexp(gcry_sexp_t sexp, unsigned char **r_buffer,
                            size_t *r_buflen) {
  size_t len;
  unsigned char *buf;

  *r_buffer = NULL;
  if (r_buflen) *r_buflen = 0;
  ;

  len = gcry_sexp_sprint(sexp, GCRYSEXP_FMT_CANON, NULL, 0);
  if (!len) return GPG_ERR_BUG;
...
gpg_error_t make_canon_sexp_pad(gcry_sexp_t sexp, int secure,
  ...
  *r_buffer = NULL;
  ...
  if (!len) return GPG_ERR_BUG;
  len += (8 - len % 8) % 8;
  buf = (unsigned char *)(secure ? xtrycalloc_secure(1, len)
                                 : xtrycalloc(1, len));
  if (!buf) return gpg_error_from_syserror();
  if (!gcry_sexp_sprint(sexp, GCRYSEXP_FMT_CANON, buf, len)) return GPG_ERR_BUG;
...
gpg_error_t keygrip_from_canon_sexp(const unsigned char *key, size_t keylen,
  ...
  if (!grip) return GPG_ERR_INV_VALUE;
  err = gcry_sexp_sscan(&sexp, NULL, (const char *)key, keylen);
  if (err) return err;
  if (!gcry_pk_get_keygrip(sexp, grip)) err = GPG_ERR_INTERNAL;
  gcry_sexp_release(sexp);
  return err;
}
...
/* Compare two simple S-expressions like "(3:foo)".  Returns 0 if they
   ...
   be used for sorting. */
int cmp_simple_canon_sexp(const unsigned char *a_orig,
  ...
  if (!a && !b) return 0; /* Both are NULL, they are identical. */
  if (!a || !b) return 1; /* One is NULL, they are not identical. */
  if (*a != '(' || *b != '(')
  ...
  if (*a != ':' || *b != ':')
    log_bug("invalid S-exp in cmp_simple_canon_sexp\n");
  if (n1 != n2) return 1; /* Not the same. */

  for (a++, b++; n1; n1--, a++, b++)
    if (*a != *b) return 1; /* Not the same. */
  return 0;
}
...
int hash_algo_from_sigval(const unsigned char *sigval) {
  ...
  if (!s || *s != '(') return 0; /* Invalid S-expression.  */
  s++;
  n = snext(&s);
  if (!n) return 0;                        /* Invalid S-expression.  */
  if (!smatch(&s, n, "sig-val")) return 0; /* Not a sig-val.  */
  if (*s != '(') return 0;                 /* Invalid S-expression.  */
  s++;
  /* Skip over the algo+parameter list.  */
  depth = 1;
  if (sskip(&s, &depth) || depth) return 0; /* Invalid S-expression.  */
  if (*s != '(') return 0;                  /* No further list.  */
  /* Check whether this is (hash ALGO).  */
  s++;
  n = snext(&s);
  if (!n) return 0;                     /* Invalid S-expression.  */
  if (!smatch(&s, n, "hash")) return 0; /* Not a "hash" keyword.  */
  n = snext(&s);
...
unsigned char *make_canon_sexp_from_rsa_pk(const void *m_arg, size_t mlen,
  ...
  /* Insert a leading zero if the number would be zero or interpreted
     as negative.  */
  if (!mlen || (m[0] & 0x80)) m_extra = 1;
  if (!elen || (e[0] & 0x80)) e_extra = 1;
  ...
  keybuf = (unsigned char *)xtrymalloc(
      strlen(part1) + strlen(mlen_str) + mlen + m_extra + strlen(part2) +
      strlen(elen_str) + elen + e_extra + strlen(part3) + 1);
...
gpg_error_t get_rsa_pk_from_canon_sexp(
    const unsigned char *keydata, size_t keydatalen, unsigned char const **r_n,
    size_t *r_nlen, unsigned char const **r_e, size_t *r_elen) {
  gpg_error_t err;
  ...
  *r_elen = 0;
  ...
  if (!rsa_n || !rsa_n_len || !rsa_e || !rsa_e_len) return GPG_ERR_BAD_PUBKEY;
