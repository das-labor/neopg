...
time_t gnupg_get_time() {
  ...
  if (current == (time_t)(-1)) log_fatal("time() failed\n");
...
struct tm *gnupg_gmtime(const time_t *timep, struct tm *result) {
  ...
  tp = gmtime(timep);
  if (tp) memcpy(result, tp, sizeof *result);
  return tp;
#endif
}
...
void gnupg_get_isotime(gnupg_isotime_t timebuf) {
  ...
  if (!tp)
    *timebuf = 0;
    ...
    snprintf(timebuf, 16, "%04d%02d%02dT%02d%02d%02d", 1900 + tp->tm_year,
...
int gnupg_faked_time_p(void) { return timemode; }
...
u32 scan_isodatestr(const char *string) {
  ...
  if (strlen(string) != 10 || string[4] != '-' || string[7] != '-') return 0;
  for (i = 0; i < 4; i++)
    if (!digitp(string + i)) return 0;
  if (!digitp(string + 5) || !digitp(string + 6)) return 0;
  if (!digitp(string + 8) || !digitp(string + 9)) return 0;
  year = atoi(string);
  month = atoi(string + 5);
  day = atoi(string + 8);
  /* some basic checks */
...
int isotime_human_p(const char *string, int date_only) {
  ...
  if (date_only) return 1; /* Okay; only date was requested.  */
  s++;
...
size_t string2isotime(gnupg_isotime_t atime, const char *string) {
  ...
  if (!isotime_human_p(string, 0)) return 0;
  atime[0] = string[0];
  atime[1] = string[1];
  atime[2] = string[2];
  atime[3] = string[3];
...
time_t isotime2epoch(const char *string) {
  ...
  if (!isotime_p(string)) return (time_t)(-1);
...
int isodate_human_to_tm(const char *string, struct tm *t) {
  ...
  if (!isotime_human_p(string, 1)) return -1;
...
static time_t _win32_timegm(struct tm *tm) {
  ...
  cnsecs = (((unsigned long long)ft.dwHighDateTime << 32) | ft.dwLowDateTime);
...
time_t parse_timestamp(const char *timestamp, char **endp) {
  ...
  while (*timestamp && *timestamp == ' ') timestamp++;
  ...
  if (strlen(timestamp) >= 15 && timestamp[8] == 'T') {
    ...
    if (year < 1900) return (time_t)(-1);

    if (endp) *endp = (char *)(timestamp + 15);
...
u32 add_days_to_timestamp(u32 stamp, u16 days) { return stamp + days * 86400L; }

/****************
 * Return a string with a time value in the form: x Y, n D, n H
 */

const char *strtimevalue(u32 value) {
  ...
  if (years) return buffer;
  if (days) return strchr(buffer, 'y') + 1;
...
char *elapsed_time_string(time_t since, time_t now) {
  ...
  if (!now) now = gnupg_get_time();

  diff = difftime(now, since);
  if (diff < 0) return xtrystrdup("time-warp");
...
const char *strtimestamp(u32 stamp) {
  ...
  if (IS_INVALID_TIME_T(atime)) {
    ...
    tp = gmtime(&atime);
    snprintf(buffer, sizeof buffer, "%04d-%02d-%02d", 1900 + tp->tm_year,
...
gpg_error_t check_isotime(const gnupg_isotime_t atime) {
  ...
  if (!*atime) return GPG_ERR_NO_VALUE;

  for (s = atime, i = 0; i < 8; i++, s++)
    if (!digitp(s)) return GPG_ERR_INV_TIME;
  if (*s != 'T') return GPG_ERR_INV_TIME;
  for (s++, i = 9; i < 15; i++, s++)
    if (!digitp(s)) return GPG_ERR_INV_TIME;
  return 0;
}
...
void dump_isotime(const gnupg_isotime_t t) {
  if (!t || !*t)
    log_printf("%s", _("[none]"));
  else
    log_printf("%.4s-%.2s-%.2s %.2s:%.2s:%s", t, t + 4, t + 6, t + 9, t + 11,
               t + 13);
}
...
gpg_error_t add_seconds_to_isotime(gnupg_isotime_t atime, int nseconds) {
  ...
  if (nseconds < 0 || nseconds >= (0x7fffffff - 61)) return GPG_ERR_INV_VALUE;
  ...
  snprintf(atime, 16, "%04d%02d%02dT%02d%02d%02d", year, month, day, hour,
           minute, sec);
  return 0;
}
...
gpg_error_t add_days_to_isotime(gnupg_isotime_t atime, int ndays) {
  ...
  if (ndays < 0 || ndays >= 9999 * 366) return GPG_ERR_INV_VALUE;
