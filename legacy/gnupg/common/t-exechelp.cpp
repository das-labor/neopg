...
static void print_open_fds(int *array) {
  int n;
...
static int *xget_all_open_fds(void) {
  int *array;
...
static void test_close_all_fds(void) {
  ...
  if (verbose) printf("max. file descriptors: %d\n", max_fd);
  array = xget_all_open_fds();
  print_open_fds(array);
  for (initial_count = n = 0; array[n] != -1; n++) initial_count++;
  free(array);
  ...
  array = xget_all_open_fds();
  if (verbose) print_open_fds(array);
  for (count = n = 0; array[n] != -1; n++) count++;
  ...
  if (get_max_fds() > 32) {
    ...
    for (n = initial_count; n < 31; n++) dup(1);
    array = xget_all_open_fds();
    if (verbose) print_open_fds(array);
    free(array);
    for (n = 0; n < 5; n++) {
