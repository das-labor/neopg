...
static void test_zb32enc(void) {
  static struct {
    ...
    const char *expected;
      ...
      /* More tests.  */
      {160,
       "\x80\x61\x58\x70\xF5\xBA\xD6\x90\x33\x36"
  ...
  for (tidx = 0; tidx < DIM(tests); tidx++) {
    ...
    /* puts (output); */
    if (strcmp(output, tests[tidx].expected)) fail(tidx);
    xfree(output);
  }
}
...
static void endecode_file(const char *fname, int decode) {
  char *buffer;
  size_t buflen;
  char *result;
...
#ifdef HAVE_DOSISH_SYSTEM
  if (decode) setmode(fileno(stdout), O_BINARY);
#endif
