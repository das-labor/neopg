...
char *read_w32_registry_string(const char *root, const char *dir,
  ...
  if (!(root_key = get_root_key(root))) return NULL;

  if (RegOpenKeyEx(root_key, dir, 0, KEY_READ, &key_handle)) {
    if (root) return NULL; /* No need for a RegClose, so return immediately. */
  ...
  nbytes = 1;
  if (RegQueryValueEx(key_handle, name, 0, NULL, NULL, &nbytes)) goto leave;
