...
void *gcry_malloc(size_t n) { return malloc(n); }
...
char *gcry_strdup(const char *string) {
  ...
  if (p) strcpy(p, string);
  return p;
}

void *gcry_realloc(void *a, size_t n) { return realloc(a, n); }
...
void *gcry_calloc(size_t n, size_t m) { return calloc(n, m); }
...
int gpg_err_code_from_errno(int err) {
  (void)err;
...
#ifndef GPG_ERROR_H /* Don't do this if gpg-error.h has been included.  */
int gpg_err_code_from_syserror(void) {
  assert(!"stub function");
  return -1;
}
#endif /*GPG_ERROR_H*/
