...
static void read_and_log_stderr(read_and_log_buffer_t *state,
                                es_poll_t *fderr) {
  gpg_error_t err;
  int c;
  ...
  if (!fderr) {
    ...
    if (state->used) {
      ...
      if (state->status_cb && !strncmp(state->buffer, "[GNUPG:] ", 9) &&
        ...
        state->status_cb(state->status_cb_value, state->buffer + 9, rest);
      } else if (!state->cont && !strncmp(state->buffer, pname, len) &&
...
static void copy_buffer_init(struct copy_buffer *c) {
  c->writep = c->buffer;
  c->nread = 0;
}

/* Securely wipe a copy buffer.  */
static void copy_buffer_shred(struct copy_buffer *c) {
  if (c == NULL) return;
...
static gpg_error_t copy_buffer_do_copy(struct copy_buffer *c, estream_t source,
                                       estream_t sink) {
  gpg_error_t err;
  size_t nwritten = 0;
  ...
  if (c->nread == 0) {
    ...
    if (es_read(source, c->buffer, sizeof c->buffer, &c->nread)) {
      err = gpg_error_from_syserror();
      if (err == GPG_ERR_EAGAIN) return 0; /* We will just retry next time.  */
  ...
  if (c->nread == 0) return 0; /* Done copying.  */
...
static gpg_error_t copy_buffer_flush(struct copy_buffer *c, estream_t sink) {
  gpg_error_t err = 0;
  size_t nwritten = 0;
  ...
  if (es_fflush(sink)) err = gpg_error_from_syserror();
...
/* Now find the argument marker and replace by the pipe's fd.
  ...
  err = gnupg_spawn_process(pgmname, argv, exceptclose, NULL,
                            GNUPG_SPAWN_NONBLOCK, input ? &infp : NULL, &outfp,
                            &errfp, &pid);
  if (extrapipe[0] != -1) close(extrapipe[0]);
  if (argsave) argv[argsaveidx] = argsave;
  if (err) {
  ...
  while (!(fds[1].ignore && fds[2].ignore)) {
    ...
    if (fds[0].got_write) {
      ...
      if (es_feof(input)) {
        ...
        if (err == GPG_ERR_EAGAIN) continue; /* Retry next time.  */
        if (err) {
          log_error("error feeding data to '%s': %s\n", pgmname,
                    gpg_strerror(err));
          goto leave;
    ...
    if (fds[2].got_read) read_and_log_stderr(&fderrstate, fds + 2);
  }
...
leave:
  if (err && pid != (pid_t)-1) gnupg_kill_process(pid);

  es_fclose(infp);
  es_fclose(extrafp);
  es_fclose(outfp);
  es_fclose(errfp);
  if (pid != (pid_t)(-1)) gnupg_wait_process(pgmname, pid, 1, NULL);
...
static void nop_free(void *ptr) { (void)ptr; }
...
gpg_error_t gnupg_exec_tool(const char *pgmname, const char *argv[],
  ...
  *result = NULL;
  ...
  if (input_string) {
    ...
    input = es_mopen((char *)input_string, len, len, 0 /* don't grow */, NULL,
                     nop_free, "rb");
    if (!input) return gpg_error_from_syserror();
  }
  ...
  *result = (char *)xtrymalloc(len + 1);
  ...
  if (len) {
    ...
    if (nread != len) log_fatal("%s: short read from memstream\n", __func__);
  }
  (*result)[len] = 0;
