...
static int fd_cache_strcmp(const char *a, const char *b) {
 ...
 */
static int fd_cache_invalidate(const char *fname) {
  close_cache_t cc;
  int rc = 0;

  assert(fname);
  if (DBG_IOBUF) log_debug("fd_cache_invalidate (%s)\n", fname);

  for (cc = close_cache; cc; cc = cc->next) {
    if (cc->fp != GNUPG_INVALID_FD && !fd_cache_strcmp(cc->fname, fname)) {
      if (DBG_IOBUF) log_debug("                did (%s)\n", cc->fname);
#ifdef HAVE_W32_SYSTEM
      if (!CloseHandle(cc->fp)) rc = -1;
#else
      rc = close(cc->fp);
#endif
...
/* Try to sync changes to the disk.  This is to avoid data loss during
   ...
   systems.  */
static int fd_cache_synchronize(const char *fname) {
  int err = 0;

#ifdef HAVE_FSYNC
  close_cache_t cc;

  if (DBG_IOBUF) log_debug("fd_cache_synchronize (%s)\n", fname);
...
static gnupg_fd_t direct_open(const char *fname, const char *mode,
                              int mode700) {
#ifdef HAVE_W32_SYSTEM
   ...
   */
  if (strchr(mode, '+')) {
    if (fd_cache_invalidate(fname)) return GNUPG_INVALID_FD;
  ...
  if (!mode700) cflag |= S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH;
...
#ifdef O_BINARY
  if (strchr(mode, 'b')) oflag |= O_BINARY;
#endif
 ...
 */
static void fd_cache_close(const char *fname, gnupg_fd_t fp) {
  close_cache_t cc;
  ...
  if (!fname || !*fname) {
    ...
    close(fp);
#endif
    if (DBG_IOBUF) log_debug("fd_cache_close (%d) real\n", (int)fp);
    return;
  }
  /* try to reuse a slot */
  for (cc = close_cache; cc; cc = cc->next) {
  ...
  /* add a new one */
  if (DBG_IOBUF) log_debug("fd_cache_close (%s) new slot created\n", fname);
 ...
 */
static gnupg_fd_t fd_cache_open(const char *fname, const char *mode) {
  close_cache_t cc;
  ...
  for (cc = close_cache; cc; cc = cc->next) {
    if (cc->fp != GNUPG_INVALID_FD && !fd_cache_strcmp(cc->fname, fname)) {
      gnupg_fd_t fp = cc->fp;
      cc->fp = GNUPG_INVALID_FD;
      if (DBG_IOBUF) log_debug("fd_cache_open (%s) using cached fp\n", fname);
#ifdef HAVE_W32_SYSTEM
  ...
  if (DBG_IOBUF) log_debug("fd_cache_open (%s) not cached\n", fname);
  return direct_open(fname, mode, 0);
}

static int file_filter(void *opaque, int control, iobuf_t chain, byte *buf,
                       size_t *ret_len) {
  ...
  if (control == IOBUFCTRL_UNDERFLOW) {
    ...
    *ret_len = nbytes;
    ...
    if (f != FD_FOR_STDIN && f != FD_FOR_STDOUT) {
      if (DBG_IOBUF) log_debug("%s: close fd/handle %d\n", a->fname, FD2INT(f));
      if (!a->keep_open) fd_cache_close(a->no_cache ? NULL : a->fname, f);
    }
...
static int file_es_filter(void *opaque, int control, iobuf_t chain, byte *buf,
                          size_t *ret_len) {
  ...
  if (control == IOBUFCTRL_UNDERFLOW) {
    ...
    *ret_len = nbytes;
    ...
    if (f != es_stdin && f != es_stdout) {
      if (DBG_IOBUF) log_debug("%s: es_fclose %p\n", a->fname, f);
      if (!a->keep_open) es_fclose(f);
    }
    f = NULL;
    xfree(a); /* We can free our context now. */
  }
...
static int sock_filter(void *opaque, int control, iobuf_t chain, byte *buf,
                       size_t *ret_len) {
  ...
  if (control == IOBUFCTRL_UNDERFLOW) {
    ...
    *ret_len = nbytes;
    ...
    if (!a->keep_open) closesocket(a->sock);
    xfree(a); /* we can free our context now */
  }
  return rc;
}
 ...
 * without a filter
 */
static int block_filter(void *opaque, int control, iobuf_t chain, byte *buffer,
                        size_t *ret_len) {
      ...
      while (!rc && size && a->size) {
        ...
        if (c < needed) {
          ...
          log_error("block_filter %p: read error (size=%lu,a->size=%lu)\n", a,
                    (unsigned long)size + c, (unsigned long)a->size + c);
          rc = GPG_ERR_BAD_DATA;
        } else {
          size -= c;
          a->size -= c;
    ...
    *ret_len = n;
    ...
    if (a->partial) { /* the complicated openpgp scheme */
      ...
      if (nbytes < OP_MIN_PARTIAL_CHUNK) {
        ...
        if (!a->buffer) a->buffer = (char *)xmalloc(OP_MIN_PARTIAL_CHUNK);
        ...
        do {
          ...
          for (blen = OP_MIN_PARTIAL_CHUNK * 2,
              c = OP_MIN_PARTIAL_CHUNK_2POW + 1;
               blen <= nbytes; blen *= 2, c++)
            ;
          blen /= 2;
          c--;
          ...
          if ((n = nbytes) > blen) n = blen;
          if (n && iobuf_write(chain, p, n)) rc = gpg_error_from_syserror();
          p += n;
          nbytes -= n;
    ...
    if (DBG_IOBUF) log_debug("init block_filter %p\n", a);
    if (a->partial)
      a->count = 0;
    ...
    if (a->use == IOBUF_OUTPUT) { /* write the end markers */
      if (a->partial) {
        u32 len;
        ...
        if (!rc && len) rc = iobuf_write(chain, a->buffer, len);
        if (rc) {
 ...
 */
static const char *iobuf_desc(iobuf_t a, byte *buf) {
...
static void print_chain(iobuf_t a) {
  if (!DBG_IOBUF) return;
  for (; a; a = a->chain) {
    byte desc[MAX_IOBUF_DESC];

    log_debug("iobuf chain: %d.%d '%s' filter_eof=%d start=%d len=%d\n", a->no,
              a->subno, iobuf_desc(a, desc), a->filter_eof, (int)a->d.start,
              (int)a->d.len);
  }
}
...
iobuf_t iobuf_alloc(int use, size_t bufsize) {
  ...
  assert(use == IOBUF_INPUT || use == IOBUF_INPUT_TEMP || use == IOBUF_OUTPUT ||
...
int iobuf_close(iobuf_t a) {
  ...
  for (; a; a = a_chain) {
    ...
    if (a->filter && (rc2 = a->filter(a->filter_ov, IOBUFCTRL_FREE, a->chain,
                                      NULL, &dummy_len)))
...
iobuf_t iobuf_temp_with_content(const char *buffer, size_t length) {
  ...
  for (i = 0; i < length; i++) a->d.buf[i] = buffer[i];
  a->d.len = length;
...
int iobuf_is_pipe_filename(const char *fname) {
  if (!fname || (*fname == '-' && !fname[1])) return 1;
  return 0;
}

static iobuf_t do_open(const char *fname, int special_filenames, int use,
                       const char *opentype, int mode700) {
  iobuf_t a;
  gnupg_fd_t fp;
  ...
  if (special_filenames
    ...
    if (fp == GNUPG_INVALID_FD) return NULL;
  }
  ...
  if (!print_only) a->real_fname = xstrdup(fname);
  a->filter = file_filter;
  a->filter_ov = fcx;
  file_filter(fcx, IOBUFCTRL_INIT, NULL, NULL, &len);
  if (DBG_IOBUF)
    log_debug("iobuf-%d.%d: open '%s' desc=%s fd=%d\n", a->no, a->subno, fname,
              iobuf_desc(a, desc), FD2INT(fcx->fp));
...
static iobuf_t do_iobuf_fdopen(int fd, const char *mode, int keep_open) {
  iobuf_t a;
  gnupg_fd_t fp;
  file_filter_ctx_t *fcx;
  size_t len;
  ...
  if (cmd == IOBUF_IOCTL_KEEP_OPEN) {
    ...
    if (DBG_IOBUF)
      log_debug("iobuf-%d.%d: ioctl '%s' keep_open=%d\n", a ? a->no : -1,
                a ? a->subno : -1, iobuf_desc(a, desc), intval);
    for (; a; a = a->chain)
    ...
    if (DBG_IOBUF)
      log_debug("iobuf-*.*: ioctl '%s' invalidate\n",
                ptrval ? (char *)ptrval : "?");
    if (!a && !intval && ptrval) {
      if (fd_cache_invalidate((const char *)(ptrval))) return -1;
      return 0;
    }
...
int iobuf_push_filter2(iobuf_t a,
  ...
  if (a->use == IOBUF_OUTPUT && (rc = filter_flush(a))) return rc;
  ...
  */
  ...
  if (a->filter && (rc = a->filter(a->filter_ov, IOBUFCTRL_INIT, a->chain, NULL,
                                   &dummy_len)))
...
int iobuf_pop_filter(iobuf_t a, int (*f)(void *opaque, int control,
  ...
  for (b = a; b; b = b->chain)
    if (b->filter == f && (!ov || b->filter_ov == ov)) break;
  if (!b) log_bug("iobuf_pop_filter(): filter function not found\n");
  ...
  if (b->filter && (rc = b->filter(b->filter_ov, IOBUFCTRL_FREE, b->chain, NULL,
                                   &dummy_len))) {
  ...
  if (a == b && !b->chain)
    ...
    memcpy(a, b, sizeof *a);
    xfree(b);
    if (DBG_IOBUF) log_debug("iobuf-%d.%d: popped filter\n", a->no, a->subno);
...
static int underflow_target(iobuf_t a, int clear_pending_eof, size_t target) {
  ...
  if (DBG_IOBUF)
    log_debug(
        "iobuf-%d.%d: underflow: buffer size: %d; still buffered: %d => space "
        "for %d bytes\n",
        a->no, a->subno, (int)a->d.size, (int)(a->d.len - a->d.start),
     ...
     didn't get an EOF or an error.  Try to fill the buffer.  */
  {
    ...
    if (DBG_IOBUF)
      log_debug(
          "iobuf-%d.%d: A->FILTER() returned rc=%d (%s), read %lu bytes\n",
          a->no, a->subno, rc,
          rc == 0 ? "ok" : rc == -1 ? "EOF" : gpg_strerror(rc),
          (unsigned long)len);
    /*  	    if( a->no == 1 ) */
    ...
    /* EOF.  */
    {
      ...
      if ((rc = a->filter(a->filter_ov, IOBUFCTRL_FREE, a->chain, NULL,
                          &dummy_len)))
        log_error("IOBUFCTRL_FREE failed: %s\n", gpg_strerror(rc));

      /* Free everything except for the internal buffer.  */
      if (a->filter_ov && a->filter_ov_owner) xfree(a->filter_ov);
      a->filter_ov = NULL;
      a->filter = NULL;
      a->filter_eof = 1;
        ...
        Unlink this filter.  */
      {
        ...
        if (DBG_IOBUF)
          log_debug(
              "iobuf-%d.%d: pop in underflow (nothing buffered, got EOF)\n",
              a->no, a->subno);
        xfree(a->d.buf);
        xfree(a->real_fname);
        memcpy(a, b, sizeof *a);
  ...
  assert(a->d.start <= a->d.len);
  if (a->d.start < a->d.len) return a->d.buf[a->d.start++];
...
static int filter_flush(iobuf_t a) {
  ...
  if (a->use == IOBUF_OUTPUT_TEMP) { /* increase the temp buffer */
...
int iobuf_readbyte(iobuf_t a) {
  ...
  if (a->nlimit && a->nbytes >= a->nlimit) return -1; /* forced EOF */
...
int iobuf_peek(iobuf_t a, byte *buf, unsigned buflen) {
  ...
  if (buflen > a->d.size) /* We can't peek more than we can buffer.  */
    buflen = a->d.size;
...
int iobuf_writebyte(iobuf_t a, unsigned int c) {
  ...
  if (a->d.len == a->d.size)
    if ((rc = filter_flush(a))) return rc;
...
size_t iobuf_temp_to_buffer(iobuf_t a, byte *buffer, size_t buflen) {
  ...
  while (1) {
    ...
    if (rc)
      log_bug(
          "Flushing iobuf %d.%d (%s) from iobuf_temp_to_buffer failed.  "
          "Ignoring.\n",
          a->no, a->subno, iobuf_desc(a, desc));
    if (!a->chain) break;
    a = a->chain;
...
void iobuf_flush_temp(iobuf_t temp) {
  ...
  while (temp->chain) iobuf_pop_filter(temp, temp->filter, NULL);
}
...
off_t iobuf_get_filelength(iobuf_t a, int *overflow) {
  ...
  if (a->filter != file_filter) return 0;

  {
    ...
    if (!get_file_size_ex_initialized) {
      ...
      if (handle) {
        ...
        if (!get_file_size_ex) dlclose(handle);
      }
    ...
    if (get_file_size_ex) {
      ...
      if (get_file_size_ex(fp, &exsize)) {
        if (!exsize.u.HighPart) return exsize.u.LowPart;
        if (overflow) *overflow = 1;
        return 0;
      }
    } else {
      if ((size = GetFileSize(fp, NULL)) != 0xffffffff) return size;
    }
...
#else  /*!HAVE_W32_SYSTEM*/
    {
      ...
      if (!fstat(FD2INT(fp), &st)) return st.st_size;
...
int iobuf_get_fd(iobuf_t a) {
  ...
  if (a->filter != file_filter) return -1;
...
off_t iobuf_tell(iobuf_t a) { return a->ntotal + a->nbytes; }
...
int iobuf_seek(iobuf_t a, off_t newpos) {
  ...
  while (a->chain) iobuf_pop_filter(a, a->filter, NULL);

  return 0;
}

const char *iobuf_get_real_fname(iobuf_t a) {
  if (a->real_fname) return a->real_fname;
...
void iobuf_set_partial_body_length_mode(iobuf_t a, size_t len) {
  ...
  /* Disable partial body length mode.  */
  {
    if (a->use == IOBUF_INPUT)
      log_debug(
          "iobuf_pop_filter called in set_partial_block_mode"
          " - please report\n");
...
unsigned int iobuf_read_line(iobuf_t a, byte **addr_of_buffer,
