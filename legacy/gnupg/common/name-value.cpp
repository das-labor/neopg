...
nvc_t nvc_new_private_key(void) {
  nvc_t nvc = nvc_new();
  if (nvc) nvc->private_key_mode = 1;
  return nvc;
}

static void nve_release(nve_t entry, int private_key_mode) {
...
static int valid_name(const char *name) {
  ...
  if (!alphap(name) || len == 0 || name[len - 1] != ':') return 0;

  for (i = 1; i < len - 1; i++)
    if (!alnump(&name[i]) && name[i] != '-') return 0;

  return 1;
}

/* Makes sure that ENTRY has a RAW_VALUE.  */
static gpg_error_t assert_raw_value(nve_t entry) {
  gpg_error_t err = 0;
  size_t len, offset;
#define LINELEN 70
  char buf[LINELEN + 3];

  if (entry->raw_value) return 0;
  ...
  while (len) {
    ...
    if (len <= linelen)
      ...
      for (i = linelen - 1; linelen - i < 30 && linelen - i > offset; i--)
        if (ascii_isspace(entry->value[i])) break;
...
static size_t continuation_length(const char *s, int *swallow_ws,
                                  const char **start) {
  size_t len;
  ...
  len = strlen(s);
  while (len > 0 && ascii_isspace(s[len - 1])) len--;
...
static gpg_error_t assert_value(nve_t entry) {
  size_t len;
  int swallow_ws;
  strlist_t s;
  char *p;
  ...
  if (entry->value == NULL) return gpg_error_from_syserror();
...
/* Get the name.  */
char *nve_name(nve_t pke) { return pke->name; }

/* Get the value.  */
char *nve_value(nve_t pke) {
  if (assert_value(pke)) return NULL;
  return pke->value;
}
...
static gpg_error_t _nvc_add(nvc_t pk, char *name, char *value,
                            strlist_t raw_value, int preserve_order) {
  gpg_error_t err = 0;
  nve_t e;
  ...
  if (name && pk->private_key_mode && !ascii_strcasecmp(name, "Key:") &&
      nvc_lookup(pk, "Key:")) {
    err = GPG_ERR_INV_NAME;
    goto leave;
  }
...
leave:
  if (err) {
    xfree(name);
    if (value) wipememory(value, strlen(value));
    xfree(value);
...
gpg_error_t nvc_add(nvc_t pk, const char *name, const char *value) {
  ...
  k = xtrystrdup(name);
  if (k == NULL) return gpg_error_from_syserror();
...
gpg_error_t nvc_set(nvc_t pk, const char *name, const char *value) {
  ...
  if (!valid_name(name)) return GPG_ERR_INV_NAME;
  ...
  if (e) {
    ...
    v = xtrystrdup(value);
    if (v == NULL) return gpg_error_from_syserror();

    free_strlist_wipe(e->raw_value);
    e->raw_value = NULL;
    if (e->value) wipememory(e->value, strlen(e->value));
    xfree(e->value);
    e->value = v;
...
void nvc_delete(nvc_t pk, nve_t entry) {
  if (entry->prev)
...
nve_t nvc_lookup(nvc_t pk, const char *name) {
  nve_t entry;
  for (entry = pk->first; entry; entry = entry->next)
    if (entry->name && ascii_strcasecmp(entry->name, name) == 0) return entry;
  return NULL;
}
...
gpg_error_t nvc_get_private_key(nvc_t pk, gcry_sexp_t *retsexp) {
  ...
  if (e == NULL) return GPG_ERR_MISSING_KEY;
...
gpg_error_t nvc_set_private_key(nvc_t pk, gcry_sexp_t sexp) {
  ...
  if (!pk->private_key_mode) return GPG_ERR_MISSING_KEY;

  len = gcry_sexp_sprint(sexp, GCRYSEXP_FMT_ADVANCED, NULL, 0);

  raw = (char *)xtrymalloc(len);
  if (raw == NULL) return gpg_error_from_syserror();
...
static gpg_error_t do_nvc_parse(nvc_t *result, int *errlinep, estream_t stream,
                                int for_private_key) {
  gpg_error_t err = 0;
  gpgrt_ssize_t len;
  char *buf = NULL;
  ...
  *result = for_private_key ? nvc_new_private_key() : nvc_new();
  if (*result == NULL) return gpg_error_from_syserror();

  if (errlinep) *errlinep = 0;
  while ((len = es_read_line(stream, &buf, &buf_len, NULL)) > 0) {
    char *p;
    if (errlinep) *errlinep += 1;

    /* Skip any whitespace.  */
    for (p = buf; *p && ascii_isspace(*p); p++) /* Do nothing.  */
      ;
  ...
  /* Add the final entry.  */
  if (raw_value) err = _nvc_add(*result, name, NULL, raw_value, 1);
...
gpg_error_t nvc_write(nvc_t pk, estream_t stream) {
  ...
  for (entry = pk->first; entry; entry = entry->next) {
    if (entry->name) es_fputs(entry->name, stream);

    err = assert_raw_value(entry);
    if (err) return err;

    for (s = entry->raw_value; s; s = s->next) es_fputs(s->d, stream);

    if (es_ferror(stream)) return gpg_error_from_syserror();
  }
