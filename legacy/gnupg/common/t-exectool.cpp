...
static void test_executing_true(void) {
  gpg_error_t err;
  ...
  if (err) fail("gnupg_exec_tool", err);
...
static void test_executing_false(void) {
  gpg_error_t err;
...
static void test_executing_cat(const char *vector) {
  gpg_error_t err;
...
static void test_catting_cat(void) {
  gpg_error_t err;
  ...
  for (p = reference; p - reference < reference_len;) {
    ...
    if (err) {
      fprintf(stderr, "error reading %s: %s", argv[1], gpg_strerror(err));
      exit(1);
    }
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < 255 /* one less */; i++) binjunk[i] = i + 1; /* avoid 0 */
  binjunk[255] = 0;
