...
static void test_b64enc_pgp(const char *string) {
  gpg_error_t err;
...
static void test_b64enc_file(const char *fname) {
  gpg_error_t err;
  struct b64state state;
  FILE *fp;
  char buffer[50];
...
static void test_b64dec_file(const char *fname) {
  gpg_error_t err;
  struct b64state state;
  FILE *fp;
  char buffer[50];
  ...
  while ((nread = fread(buffer, 1, sizeof buffer, fp))) {
    ...
    if (err) {
      if (gpg_err_code(err) == GPG_ERR_EOF) break;
      fail(2);
    } else if (nbytes)
