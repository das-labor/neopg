...
int gnupg_pk_is_compliant(enum gnupg_compliance_mode compliance, int algo,
  ...
  if (compliance == CO_DE_VS) {
    ...
    switch (algotype) {
      ...
      case is_rsa:
        result = (keylength == 2048 || keylength == 3072 || keylength == 4096);
        break;
      ...
      case is_ecc:
        ...
        result = (curvename &&
                  (algo == PUBKEY_ALGO_ECDH || algo == PUBKEY_ALGO_ECDSA) &&
      ...
      switch (algo) {
        ...
        case PUBKEY_ALGO_RSA_S:
          switch (use) {
            ...
            case PK_USE_SIGNING:
              return (keylength == 2048 || keylength == 3072 ||
                      keylength == 4096);
            case PK_USE_VERIFICATION:
              return (keylength == 2048 || keylength == 3072 ||
                      keylength == 4096 || keylength < 2048);
            default:
              log_assert(!"reached");
          }
          log_assert(!"reached");
        ...
        case PUBKEY_ALGO_DSA:
          if (key) {
            size_t P = gcry_mpi_get_nbits(key[0]);
            size_t Q = gcry_mpi_get_nbits(key[1]);
            return ((use == PK_USE_SIGNING && Q == 256 &&
                     (P == 2048 || P == 3072)) ||
                    (use == PK_USE_VERIFICATION && P < 2048));
          } else
            return 0;
          log_assert(!"reached");
...
int gnupg_cipher_is_allowed(enum gnupg_compliance_mode compliance, int producer,
                            cipher_algo_t cipher, enum gcry_cipher_modes mode) {
  if (!initialized) return 1;
...
const char *gnupg_status_compliance_flag(
...
int gnupg_parse_compliance_option(const char *string,
                                  ...
                                  size_t length, int quiet) {
  size_t i;

  if (!ascii_strcasecmp(string, "help")) {
    log_info(_("valid values for option '%s':\n"), "--compliance");
    for (i = 0; i < length; i++) log_info("  %s\n", options[i].keyword);
    return -1;
  }

  for (i = 0; i < length; i++)
    if (!ascii_strcasecmp(string, options[i].keyword)) return options[i].value;

  log_error(_("invalid value for option '%s'\n"), "--compliance");
  if (!quiet) log_info(_("(use \"help\" to list choices)\n"));
  return -1;
}

/* Return the command line option for the given COMPLIANCE mode.  */
const char *gnupg_compliance_option_string(
    enum gnupg_compliance_mode compliance) {
  switch (compliance) {
    case CO_GNUPG:
    ...
    case CO_PGP7:
      return "--compliance=pgp7";
    case CO_PGP8:
      return "--compliance=pgp8";
    case CO_DE_VS:
      return "--compliance=de-vs";
  }
