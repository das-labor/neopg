...
gpg_error_t classify_user_id(const char *name, KEYDB_SEARCH_DESC *desc,
  ...
  switch (*s) {
    ...
    case ':': /* Unified fingerprint. */
    {
      ...
      for (i = 0, si = s; si < se; i++, si += 2) desc->u.fpr[i] = hextobyte(si);
      for (; i < 20; i++) desc->u.fpr[i] = 0;
    ...
    default:
      ...
      if (desc->exact) hexlength--; /* Remove the bang.  */

      if ((hexlength == 8 && (s[hexlength] == 0 || (s[hexlength] == '!' &&
                                                    s[hexlength + 1] == 0))) ||
        ...
        if (hexlength == 59 && (!s[hexlength] || spacep(s + hexlength))) {
          ...
          if (i == 20) mode = KEYDB_SEARCH_MODE_FPR20;
        }
        if (!mode) {
