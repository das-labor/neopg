...
void close_all_fds(int first, int *except) {
  ...
  if (except) {
    ...
    for (fd = first; fd < max_fd; fd++) {
      ...
      if (except[i] == -1) close(fd);
    }
  } else {
    for (fd = first; fd < max_fd; fd++) close(fd);
  }
...
static void do_exec(const char *pgmname, const char *argv[], int fd_in,
                    int fd_out, int fd_err, int *except,
                    void (*preexec)(void)) {
  char **arg_list;
  int i, j;
  int fds[3];
  ...
  if (argv)
    for (i = 0, j = 1; argv[i]; i++, j++) arg_list[j] = (char *)argv[i];
...
static gpg_error_t do_create_pipe(int filedes[2]) {
  gpg_error_t err = 0;
...
gpg_error_t gnupg_spawn_process(const char *pgmname, const char *argv[],
  ...
  *pid = fork();
  ...
  if (!*pid) {
    ...
    es_fclose(outfp);
    es_fclose(errfp);
    do_exec(pgmname, argv, inpipe[0], outpipe[1], errpipe[1], except, preexec);
    /*NOTREACHED*/
  }
...
static gpg_error_t store_result(pid_t pid, int exitcode) {
  struct terminated_child *c;

  c = (terminated_child *)xtrymalloc(sizeof *c);
  if (c == NULL) return gpg_error_from_syserror();
...
gpg_error_t gnupg_wait_processes(const char **pgmnames, pid_t *pids,
  ...
  if (r_exitcodes == NULL) {
    ...
    if (dummy == NULL) return gpg_error_from_syserror();
  }

  for (i = 0, left = count; i < count; i++) {
    int status = -1;

    if (pids[i] == (pid_t)(-1)) return GPG_ERR_INV_VALUE;

    /* See if there was a previously stored result for this pid.  */
    if (get_result(pids[i], &status)) left -= 1;
...
void gnupg_release_process(pid_t pid) { (void)pid; }
...
gpg_error_t gnupg_spawn_process_detached(const char *pgmname,
  ...
  if (!pid) {
    ...
    if (pid2 == (pid_t)(-1)) _exit(1);
    if (pid2) _exit(0); /* Let the parent exit immediately. */

    if (envp)
      for (i = 0; envp[i]; i++) putenv(xstrdup(envp[i]));
