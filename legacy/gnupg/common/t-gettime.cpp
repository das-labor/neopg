...
static void test_isotime2epoch(void) {
  struct {
    const char *string;
    time_t expected;
  } array[] = {
  ...
  for (idx = 0; array[idx].string; idx++) {
    ...
    if (val != array[idx].expected) {
      ...
      if (verbose)
        fprintf(stderr, "string '%s' exp: %ld got: %ld\n", array[idx].string,
                (long)array[idx].expected, (long)val);
    }
    if (array[idx].expected != INVALID) {
      ...
      if (strncmp(array[idx].string, tbuf, 15)) fail(idx);
    }
  }
}

static void test_string2isotime(void) {
  struct {
    const char *string;
    size_t result;
    const char *expected;
  } array[] = {{"19700101T000001", 15, "19700101T000001"},
...
static void test_isodate_human_to_tm(void) {
  struct {
    const char *string;
    int okay;
    int year, mon, mday;
  } array[] = {{"1970-01-01", 1, 1970, 1, 1},
    ...
    if (okay != array[idx].okay) {
      ...
      if (verbose)
        fprintf(stderr, "string '%s' returned %04d-%02d-%02d\n",
                array[idx].string, tmbuf.tm_year + 1900, tmbuf.tm_mon + 1,
                tmbuf.tm_mday);
