...
static const unsigned char oid_ed25519[] = {0x09, 0x2b, 0x06, 0x01, 0x04,
                                            0x01, 0xda, 0x47, 0x0f, 0x01};

/* The OID for Curve25519 in OpenPGP format.  */
static const unsigned char oid_cv25519[] = {0x0a, 0x2b, 0x06, 0x01, 0x04, 0x01,
                                            0x97, 0x55, 0x01, 0x05, 0x01};
...
gpg_error_t openpgp_oid_from_str(const char *string, gcry_mpi_t *r_mpi) {
  ...
  *r_mpi = NULL;
  ...
  if (!string || !*string) return GPG_ERR_INV_VALUE;
  ...
  do {
    ...
    if (arcno == 1) {
      if (val > 2) break; /* Not allowed, error caught below.  */
      val1 = val;
    } else if (arcno ==
               2) { /* Need to combine the first two arcs in one octet.  */
      if (val1 < 2) {
        if (val > 39) {
          xfree(buf);
          return GPG_ERR_INV_OID_STRING;
  ...
  if (arcno == 1 || buflen < 2 ||
      buflen > 254) { /* It is not possible to encode only the first arc.  */
    xfree(buf);
    return GPG_ERR_INV_OID_STRING;
  }
...
int openpgp_oid_is_ed25519(gcry_mpi_t a) {
  ...
  if (!a || !gcry_mpi_get_flag(a, GCRYMPI_FLAG_OPAQUE)) return 0;

  buf = (const unsigned char *)gcry_mpi_get_opaque(a, &nbits);
  n = (nbits + 7) / 8;
  return (n == DIM(oid_ed25519) && !memcmp(buf, oid_ed25519, DIM(oid_ed25519)));
}
...
int openpgp_oid_is_cv25519(gcry_mpi_t a) {
  ...
  buf = (const unsigned char *)gcry_mpi_get_opaque(a, &nbits);
  n = (nbits + 7) / 8;
  return (n == DIM(oid_cv25519) && !memcmp(buf, oid_cv25519, DIM(oid_cv25519)));
}
...
static int curve_supported_p(const char *name) {
  int result = 0;
...
const char *openpgp_enum_curves(int *iterp) {
  int idx = *iterp;
...
const char *openpgp_is_curve_supported(const char *name, int *r_algo,
  ...
  for (idx = 0; idx < DIM(oidtable) && oidtable[idx].name; idx++) {
    if ((!strcmp(name, oidtable[idx].name) ||
      ...
      if (r_algo) *r_algo = oidtable[idx].pubkey_algo;
      if (r_nbits) *r_nbits = oidtable[idx].nbits;
