...
static void my_gcry_logger(void *dummy, int level, const char *fmt,
                           va_list arg_ptr) {
  (void)dummy;
  ...
  switch (level) {
    case GCRY_LOG_CONT:
      level = GPGRT_LOG_CONT;
      break;
    case GCRY_LOG_INFO:
      level = GPGRT_LOG_INFO;
      break;
    case GCRY_LOG_WARN:
      level = GPGRT_LOG_WARN;
      break;
    case GCRY_LOG_ERROR:
      level = GPGRT_LOG_ERROR;
      break;
    case GCRY_LOG_FATAL:
      level = GPGRT_LOG_FATAL;
      break;
    case GCRY_LOG_BUG:
      level = GPGRT_LOG_BUG;
      break;
    case GCRY_LOG_DEBUG:
      level = GPGRT_LOG_DEBUG;
      break;
    default:
      level = GPGRT_LOG_ERROR;
      break;
...
static int my_gcry_outofcore_handler(void *opaque, size_t req_n,
                                     unsigned int flags) {
...
const char *print_fname_stdin(const char *s) {
  if (!s || (*s == '-' && !s[1])) return "[stdin]";
  return s;
}

static int do_print_utf8_buffer(estream_t stream, const void *buffer,
                                size_t length, const char *delimiters,
...
void print_utf8_buffer3(estream_t stream, const void *p, size_t n,
                        const char *delim) {
  do_print_utf8_buffer(stream, p, n, delim, NULL);
}

void print_utf8_buffer2(estream_t stream, const void *p, size_t n, int delim) {
  char tmp[2];
...
void print_utf8_buffer(estream_t stream, const void *p, size_t n) {
...
char *try_make_printable_string(const void *p_arg, size_t n, int delim) {
...
char *make_printable_string(const void *p, size_t n, int delim) {
  ...
  if (!string) xoutofcore();
  return string;
}

/* Check whether (BUF,LEN) is valid header for an OpenPGP compressed
 * packet.  LEN should be at least 6.  */
static int is_openpgp_compressed_packet(unsigned char *buf, size_t len) {
  int c, ctb, pkttype;
  int lenbytes;

  ctb = *buf++;
  len--;
  if (!(ctb & 0x80)) return 0; /* Invalid packet.  */

  if ((ctb & 0x40)) /* New style (OpenPGP) CTB.  */
  {
    pkttype = (ctb & 0x3f);
    if (!len) return 0; /* Expected first length octet missing.  */
    c = *buf++;
    len--;
    if (c < 192)
      ;
    else if (c < 224) {
      if (!len) return 0; /* Expected second length octet missing. */
    } else if (c == 255) {
      if (len < 4) return 0; /* Expected length octets missing */
    }
  } else /* Old style CTB.  */
  {
    pkttype = (ctb >> 2) & 0xf;
    lenbytes = ((ctb & 3) == 3) ? 0 : (1 << (ctb & 3));
    if (len < lenbytes) return 0; /* Not enough length bytes.  */
  }
...

{
  ...
  if (!string) {
    if (debugvar) {
      ...
      for (i = 0; flags[i].name; i++)
        if ((*debugvar & flags[i].flag)) log_printf(" %s", flags[i].name);
      log_printf("\n");
    }
    return 0;
  }
  ...
  if (!strcmp(string, "?") || !strcmp(string, "help")) {
    ...
    if (flags[i].flag != 77) exit(0);
  } else if (digitp(string)) {
    errno = 0;
    result = strtoul(string, NULL, 0);
    if (result == ULONG_MAX && errno == ERANGE) return -1;
  } else {
    char **words;
