...
int agent_pkdecrypt(ctrl_t ctrl, const char *desc_text,
  ...
  *r_padding = -1;
  ...
  rc = agent_key_from_file(ctrl, NULL, desc_text, ctrl->keygrip, &shadow_info,
  ...
  if (shadow_info) { /* divert operation to the smartcard */
    ...
    rc = divert_pkdecrypt(ctrl, desc_text, ciphertext, shadow_info, &buf, &len,
                          r_padding);
    if (rc) {
    ...
    put_membuf(outbuf, ")", 2);
  } else { /* No smartcard, but a private key */
           /*       if (DBG_CRYPTO ) */
           /*         { */
           /*           log_debug ("skey: "); */
