...
static gpg_error_t get_keygrip(int pubkey_algo, const char *curve,
                               gcry_mpi_t *pkey, unsigned char *grip) {
  gpg_error_t err;
  ...
  switch (pubkey_algo) {
    case GCRY_PK_DSA:
      err = gcry_sexp_build(&s_pkey, NULL,
                            "(public-key(dsa(p%m)(q%m)(g%m)(y%m)))", pkey[0],
                            pkey[1], pkey[2], pkey[3]);
      break;

    case GCRY_PK_ELG:
      err = gcry_sexp_build(&s_pkey, NULL, "(public-key(elg(p%m)(g%m)(y%m)))",
                            pkey[0], pkey[1], pkey[2]);
      break;

    case GCRY_PK_RSA:
      err = gcry_sexp_build(&s_pkey, NULL, "(public-key(rsa(n%m)(e%m)))",
                            pkey[0], pkey[1]);
      break;
  ...
  if (!err && !gcry_pk_get_keygrip(s_pkey, grip)) err = GPG_ERR_INTERNAL;
...
static gpg_error_t convert_secret_key(gcry_sexp_t *r_key, int pubkey_algo,
                                      gcry_mpi_t *skey, const char *curve) {
  gpg_error_t err;
  ...
  *r_key = NULL;
  ...
  switch (pubkey_algo) {
    ...
    case GCRY_PK_ELG_E:
      err = gcry_sexp_build(&s_skey, NULL,
                            "(private-key(elg(p%m)(g%m)(y%m)(x%m)))", skey[0],
    ...
    case GCRY_PK_RSA_S:
      err = gcry_sexp_build(
          &s_skey, NULL, "(private-key(rsa(n%m)(e%m)(d%m)(p%m)(q%m)(u%m)))",
...
static gpg_error_t convert_transfer_key(gcry_sexp_t *r_key, int pubkey_algo,
                                        gcry_mpi_t *skey, const char *curve,
    ...
    case GCRY_PK_ECC:
      if (!curve)
        ...
        if (!strcmp(curve, "Ed25519"))
          ...
          format =
              "(protected-private-key(ecc(curve %s)(flags djb-tweak)(q%m)"
              "(protected openpgp-native%S)))";
        else
          format =
        ...
        err = gcry_sexp_build(&s_skey, NULL, format, curve, skey[0],
                              transfer_key);
      }
      break;
...
static gpg_error_t hash_passphrase_and_set_key(const char *passphrase,
                                               gcry_cipher_hd_t hd,
                                               int protect_algo, int s2k_mode,
                                               int s2k_algo, byte *s2k_salt,
                                               u32 s2k_count) {
  gpg_error_t err;
  unsigned char *key;
  size_t keylen;
  ...
  if (!keylen) return GPG_ERR_INTERNAL;

  key = (unsigned char *)xtrymalloc_secure(keylen);
  if (!key) return gpg_error_from_syserror();

  err = s2k_hash_passphrase(passphrase, s2k_algo, s2k_mode, s2k_salt, s2k_count,
                            key, keylen);
  if (!err) err = gcry_cipher_setkey(hd, key, keylen);

  xfree(key);
  return err;
}

static u16 checksum(const unsigned char *p, unsigned int n) {
  u16 a;
...
static void get_npkey_nskey(int pubkey_algo, size_t *npkey, size_t *nskey) {
  switch (pubkey_algo) {
    case GCRY_PK_RSA:
      *npkey = 2;
      *nskey = 6;
...
/* Helper for do_unprotect.  PUBKEY_ALOGO is the gcrypt algo number.
   ...
   SKEY.  */
static int prepare_unprotect(int pubkey_algo, gcry_mpi_t *skey, size_t skeysize,
                             int s2k_mode, unsigned int *r_npkey,
                             unsigned int *r_nskey, unsigned int *r_skeylen) {
  ...
  for (skeylen = 0; skey[skeylen]; skeylen++)
    ;
  for (i = skeylen; i < skeysize; i++) skey[i] = NULL;
  ...
  if (!npkey || !nskey || npkey >= nskey) return GPG_ERR_INTERNAL;
  if (skeylen <= npkey) return GPG_ERR_MISSING_VALUE;
  if (nskey + 1 >= skeysize) return GPG_ERR_BUFFER_TOO_SHORT;
  ...
  if (r_npkey) *r_npkey = npkey;
  if (r_nskey) *r_nskey = nskey;
  if (r_skeylen) *r_skeylen = skeylen;
  return 0;
}
...
static int do_unprotect(const char *passphrase, int pkt_version,
                        int pubkey_algo, int is_protected, const char *curve,
                        gcry_mpi_t *skey, size_t skeysize, int protect_algo,
                        void *protect_iv, size_t protect_ivlen, int s2k_mode,
                        int s2k_algo, byte *s2k_salt, u32 s2k_count,
  ...
  *r_key = NULL;
  ...
  err = prepare_unprotect(pubkey_algo, skey, skeysize, s2k_mode, &npkey, &nskey,
                          &skeylen);
  if (err) return err;
  ...
  if (!is_protected) {
    ...
    for (i = npkey; i < nskey; i++) {
      ...
      if (gcry_mpi_get_flag(skey[i], GCRYMPI_FLAG_OPAQUE)) {
        ...
        err = gcry_mpi_aprint(GCRYMPI_FMT_PGP, &buffer, &nbytes, skey[i]);
        if (!err) actual_csum += checksum(buffer, nbytes);
        xfree(buffer);
      }
      if (err) return err;
    }

    if (actual_csum != desired_csum) return GPG_ERR_CHECKSUM;
  ...
  if (gcry_cipher_test_algo(protect_algo)) {
    ...
    log_info(_("protection algorithm %d (%s) is not supported\n"), protect_algo,
             gnupg_cipher_algo_name(protect_algo));
  ...
  err = gcry_cipher_open(&cipher_hd, protect_algo, GCRY_CIPHER_MODE_CFB,
                         (GCRY_CIPHER_SECURE |
                          (protect_algo >= 100 ? 0 : GCRY_CIPHER_ENABLE_SYNC)));
  if (err) {
  ...
  if (pkt_version >= 4) {
    ...
    if (ndata > 1) csum_pgp7 = buf16_to_u16(p + ndata - 2);
  ...
  if (actual_csum != desired_csum) return GPG_ERR_BAD_PASSPHRASE;
...
/* Callback function to try the unprotection from the passphrase query
   code.  */
static gpg_error_t try_do_unprotect_cb(struct pin_entry_info_s *pi) {
  gpg_error_t err;
  struct try_do_unprotect_arg_s *arg =
      (try_do_unprotect_arg_s *)pi->check_cb_arg;

  err = do_unprotect(pi->pin, arg->is_v4 ? 4 : 3, arg->pubkey_algo,
                     arg->is_protected, arg->curve, arg->skey, arg->skeysize,
                     arg->protect_algo, arg->iv, arg->ivlen, arg->s2k_mode,
                     arg->s2k_algo, arg->s2k_salt, arg->s2k_count,
                     arg->desired_csum, arg->r_key);
  /* SKEY may be modified now, thus we need to re-compute SKEYIDX.  */
  for (arg->skeyidx = 0;
       (arg->skeyidx < arg->skeysize && arg->skey[arg->skeyidx]);
       arg->skeyidx++)
    ;
  return err;
}
...
/* See convert_from_openpgp for the core of the description.  This
   ...
   NULL in this mode.  */
static gpg_error_t convert_from_openpgp_main(
    ctrl_t ctrl, gcry_sexp_t s_pgp, int dontcare_exist, unsigned char *grip,
    const char *prompt, const char *cache_nonce, const char *passphrase,
  ...
  *r_key = NULL;
  ...
  if (is_protected) {
    ...
    if (!string) goto bad_seckey;
    protect_algo = gcry_cipher_map_name(string);
    xfree(string);

    value = gcry_sexp_nth_data(list, 3, &valuelen);
    if (!value || !valuelen || valuelen > sizeof iv) goto bad_seckey;
    ...
    if (!value || !valuelen || valuelen > sizeof s2k_salt) goto bad_seckey;
  ...
  get_npkey_nskey(pubkey_algo, &npkey, &nskey);
  if (!npkey || !nskey || npkey >= nskey) goto bad_seckey;
  ...
  for (idx = 0;;) {
    ...
    if (!value && skeyidx >= npkey) break; /* Ready.  */
  ...
  if (unattended && !from_native) {
    err = prepare_unprotect(pubkey_algo, skey, DIM(skey), s2k_mode, NULL, NULL,
                            NULL);
    if (err) goto leave;
    ...
    pi = (pin_entry_info_s *)Botan::allocate_memory(
    ...
    if (!is_protected) {
      ...
      if (cache_value) {
        if (strlen(cache_value) < pi->max_length) strcpy(pi->pin, cache_value);
        xfree(cache_value);
      }
      if (*pi->pin) err = try_do_unprotect_cb(pi);
    } else if (from_native) {
      if (strlen(passphrase) < pi->max_length) strcpy(pi->pin, passphrase);
      err = try_do_unprotect_cb(pi);
    }
  ...
  for (idx = 0; idx < skeyidx; idx++) gcry_mpi_release(skey[idx]);
  skeyidx = 0;
...
gpg_error_t convert_from_openpgp(ctrl_t ctrl, gcry_sexp_t s_pgp,
                                 int dontcare_exist, unsigned char *grip,
                                 const char *prompt, const char *cache_nonce,
                                 unsigned char **r_key, char **r_passphrase) {
  return convert_from_openpgp_main(ctrl, s_pgp, dontcare_exist, grip, prompt,
                                   cache_nonce, NULL, r_key, r_passphrase);
}
...
/* This function is called by agent_unprotect to re-protect an
   ...
   protection format.  */
gpg_error_t convert_from_openpgp_native(ctrl_t ctrl, gcry_sexp_t s_pgp,
                                        const char *passphrase,
                                        unsigned char **r_key) {
  gpg_error_t err;
  unsigned char grip[20];

  if (!passphrase) return GPG_ERR_INTERNAL;

  err = convert_from_openpgp_main(ctrl, s_pgp, 0, grip, NULL, NULL, passphrase,
                                  r_key, NULL);
  ...
  if (!err) {
    if (*passphrase) {
      ...
      agent_write_private_key(grip, *r_key,
                              gcry_sexp_canon_len(*r_key, 0, NULL, NULL), 1);
    }
  }
...
static gpg_error_t apply_protection(gcry_mpi_t *array, int npkey, int nskey,
                                    const char *passphrase, int protect_algo,
                                    void *protect_iv, size_t protect_ivlen,
                                    int s2k_mode, int s2k_algo, byte *s2k_salt,
                                    u32 s2k_count) {
  gpg_error_t err;
  int i, j;
  ...
  if (!data) {
    err = gpg_error_from_syserror();
    for (i = 0; i < (nskey - npkey); i++) xfree(bufarr[i]);
    return err;
  }
  p = data;
  ...
  if (!err)
    err = hash_passphrase_and_set_key(passphrase, cipherhd, protect_algo,
                                      s2k_mode, s2k_algo, s2k_salt, s2k_count);
  if (!err) err = gcry_cipher_setiv(cipherhd, protect_iv, protect_ivlen);
  if (!err) err = gcry_cipher_encrypt(cipherhd, data, ndata, NULL, 0);
 ...
 */
gpg_error_t extract_private_key(gcry_sexp_t s_key, int req_private_key_data,
                                const char **r_algoname, int *r_npkey,
                                int *r_nskey, const char **r_elems,
  ...
  *r_flags = NULL;
  ...
  if (!req_private_key_data) {
    ...
    if (!list) list = gcry_sexp_find_token(s_key, "protected-private-key", 0);
    if (!list) list = gcry_sexp_find_token(s_key, "private-key", 0);
  } else
  ...
  if (!strcmp(name, "rsa")) {
    ...
    npkey = 2;
    nskey = 6;
    err = gcry_sexp_extract_param(list, NULL, format, array + 0, array + 1,
    ...
    err =
        gcry_sexp_extract_param(list, NULL, format, array + 0, array + 1, NULL);
  } else {
    err = GPG_ERR_PUBKEY_ALGO;
  }
  xfree(name);
...
gpg_error_t convert_to_openpgp(ctrl_t ctrl, gcry_sexp_t s_key,
  ...
  *r_transferkey = NULL;
  ...
  for (i = 0; i < DIM(array); i++) array[i] = NULL;

  err = extract_private_key(s_key, 1, &algoname, &npkey, &nskey, NULL, array,
                            DIM(array), &curve, &flags);
  if (err) return err;
  ...
  err = apply_protection(array, npkey, nskey, passphrase, GCRY_CIPHER_AES,
                         protect_iv, sizeof protect_iv, 3, GCRY_MD_SHA1,
                         (byte *)(salt), s2k_count);
  ...
  if (!err) {
    ...
    if (!err)
      err = gcry_sexp_build(&tmpsexp, NULL,
                            ...
                            algoname, curve, tmpkey, (int)sizeof protect_iv,
                            protect_iv, (int)sizeof salt, salt, countbuf);
  ...
  for (i = 0; i < DIM(array); i++) gcry_mpi_release(array[i]);
  gcry_sexp_release(curve);
  gcry_sexp_release(flags);
