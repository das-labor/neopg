...
static const char headerblurb[] =
    ...
    "# which may be one of 'P', 'S' or '*' and optionally followed by a list "
    "of\n"
    "# other flags.  The fingerprint may be prefixed with a '!' to mark the\n"
    "# key as not trusted.  You should give the gpg-agent a HUP \n"
...
static inline void clear_trusttable(void) {
  xfree(trusttable);
  trusttable = NULL;
  trusttablesize = 0;
}

static gpg_error_t read_one_trustfile(const char *fname, int allow_include,
  ...
  while (es_fgets(line, DIM(line) - 1, fp)) {
    ...
    if (!n || line[n - 1] != '\n') {
      ...
      while ((c = es_getc(fp)) != EOF && c != '\n')
        ;
      err = *line ? GPG_ERR_LINE_TOO_LONG : GPG_ERR_INCOMPLETE_LINE;
      log_error(_("file '%s', line %d: %s\n"), fname, lnr, gpg_strerror(err));
      continue;
    }
    line[--n] = 0;                               /* Chop the LF. */
    if (n && line[n - 1] == '\r') line[--n] = 0; /* Chop an optional CR. */
    ...
    if (!strncmp(p, "include-default", 15) && (!p[15] || spacep(p + 15))) {
      char *etcname;
      gpg_error_t err2;
      ...
      if (!strcmp(etcname, fname)) /* Same file. */
        ...
        err2 = read_one_trustfile(etcname, 0, &table, &tablesize, &tableidx);
        if (err2) err = err2;
      }
      xfree(etcname);
    ...
    while (*p) {
      ...
      if (p[n] == '=') {
        log_error(
            "assigning a value to a flag is not yet supported; "
            "in '%s', line %d\n",
            fname, lnr);
        err = GPG_ERR_BAD_DATA;
        p++;
...
static gpg_error_t read_trustfiles(void) {
  gpg_error_t err;
  trustitem_t *table, *ti;
  int tableidx;
  size_t tablesize;
  ...
  if (!table) return gpg_error_from_syserror();
  tableidx = 0;
  ...
  err = read_one_trustfile(fname, allow_include, &table, &tablesize, &tableidx);
  xfree(fname);
  ...
  ti = (trustitem_t *)xtryrealloc(table,
                                  (tableidx ? tableidx : 1) * sizeof *table);
  if (!ti) {
    err = gpg_error_from_syserror();
    xfree(table);
    return err;
...
static gpg_error_t istrusted_internal(ctrl_t ctrl, const char *fpr,
                                      int *r_disabled, int already_locked) {
  gpg_error_t err = 0;
  ...
  if (trusttable) {
    for (ti = trusttable, len = trusttablesize; len; ti++, len--)
      if (!memcmp(ti->fpr, fprbin, 20)) {
        if (ti->flags.disabled && r_disabled) *r_disabled = 1;
        ...
        if (!err) err = ti->flags.disabled ? GPG_ERR_NOT_TRUSTED : 0;
        goto leave;
      }
  }
...
gpg_error_t agent_listtrusted(void *assuan_context) {
  ...
  if (trusttable) {
    for (ti = trusttable, len = trusttablesize; len; ti++, len--) {
      if (ti->flags.disabled) continue;
      bin2hex(ti->fpr, 20, key);
      key[40] = ' ';
      key[41] =
          ((ti->flags.for_smime && ti->flags.for_pgp)
               ? '*'
...
/* Create a copy of string with colons inserted after each two bytes.
   ...
   NULL is returned.  */
static char *insert_colons(const char *string) {
  char *buffer, *p;
...
static char *reformat_name(const char *name, const char *replstring) {
  const char *s;
  char *newname;
  char *d;
  size_t count;
  ...
  if (*name != '/') return xtrystrdup(name);
...
gpg_error_t agent_marktrusted(ctrl_t ctrl, const char *name, const char *fpr,
  ...
  if (!opt.allow_mark_trusted) return GPG_ERR_NOT_SUPPORTED;
  ...
  if (!nameformatted) return gpg_error_from_syserror();
