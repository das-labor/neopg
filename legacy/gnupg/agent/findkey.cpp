...
static gpg_error_t write_extended_private_key(char *fname, estream_t fp,
                                              int update, const void *buf,
                                              size_t len) {
  gpg_error_t err;
  nvc_t pk = NULL;
...
/* Write an S-expression formatted key to our key storage.  With FORCE
   ...
   overwritten.  */
int agent_write_private_key(const unsigned char *grip, const void *buffer,
  ...
  fname = make_filename(gnupg_homedir(), GNUPG_PRIVATE_KEYS_DIR, hexgrip, NULL);
  ...
  if (!fp) {
    ...
    if (force && tmperr == GPG_ERR_ENOENT) {
      ...
      if (!fp) tmperr = gpg_error_from_syserror();
    }
    if (!fp) {
...
/* Callback function to try the unprotection from the passphrase query
   code. */
static gpg_error_t try_unprotect_cb(struct pin_entry_info_s *pi) {
  ...
  if (!*protected_at) {
    ...
    desc =
        xtrystrdup(L_("Note: This passphrase has never been changed.%0A"
                      "Please change it now."));
    if (!desc) return gpg_error_from_syserror();
  } else {
    ...
    if (strcmp(now, tmptime) > 0) {
      /* Passphrase "expired".  */
      desc = xtryasprintf(L_("This passphrase has not been changed%%0A"
  ...
  if (desc) {
    ...
    if (opt.enforce_passphrase_constraints) {
      err =
          agent_get_confirmation(ctrl, desc, L_("Change passphrase"), NULL, 0);
      if (!err) arg->change_required = 1;
    } else {
      err = agent_get_confirmation(ctrl, desc, L_("Change passphrase"),
                                   L_("I'll change it later"), 0);
      if (!err)
        arg->change_required = 1;
      else if (err == GPG_ERR_CANCELED || err == GPG_ERR_FULLY_CANCELED)
        err = 0;
    }
    xfree(desc);
  }
...
/* Unprotect the canconical encoded S-expression key in KEYBUF.  GRIP
   ...
   passphrase. */
static int unprotect(ctrl_t ctrl, const char *cache_nonce,
                     const char *desc_text, unsigned char **keybuf,
                     const unsigned char *grip, cache_mode_t cache_mode,
                     lookup_ttl_t lookup_ttl, char **r_passphrase) {
  ...
  retry:
    ...
    if (pw) {
      rc = agent_unprotect(ctrl, *keybuf, pw, NULL, &result, &resultlen);
      if (!rc) {
        if (cache_mode == CACHE_MODE_NORMAL) agent_store_cache_hit(hexgrip);
        if (r_passphrase)
          *r_passphrase = pw;
        else
          xfree(pw);
  ...
  if (!rc) {
    ...
    if (arg.change_required) {
      ...
      rc = gcry_sexp_sscan(&s_skey, &erroff, (char *)arg.unprotected_key,
                           canlen);
      if (rc) {
      ...
      if (r_passphrase && *pi->pin) *r_passphrase = xtrystrdup(pi->pin);
    }
    xfree(*keybuf);
...
static gpg_error_t remove_key_file(const unsigned char *grip) {
  gpg_error_t err = 0;
  char *fname;
  char hexgrip[40 + 4 + 1];
  ...
  strcpy(hexgrip + 40, ".key");
  fname = make_filename(gnupg_homedir(), GNUPG_PRIVATE_KEYS_DIR, hexgrip, NULL);
  if (gnupg_remove(fname)) err = gpg_error_from_syserror();
  xfree(fname);
  return err;
}
...
/* Return the secret key as an S-Exp in RESULT after locating it using
   ...
   returned passphrase.   */
gpg_error_t agent_key_from_file(ctrl_t ctrl, const char *cache_nonce,
                                const char *desc_text,
                                const unsigned char *grip,
                                unsigned char **shadow_info,
                                cache_mode_t cache_mode,
                                lookup_ttl_t lookup_ttl, gcry_sexp_t *result,
                                char **r_passphrase) {
  int rc;
  unsigned char *buf;
  ...
  *result = NULL;
  if (shadow_info) *shadow_info = NULL;
  if (r_passphrase) *r_passphrase = NULL;

  rc = read_key_file(grip, &s_skey);
  if (rc) {
    if (rc == GPG_ERR_ENOENT) rc = GPG_ERR_NO_SECKEY;
    return rc;
  }
  ...
  switch (agent_private_key_type(buf)) {
    ...
    case PRIVATE_KEY_PROTECTED: {
         ...
         ignored.  */
      {
        ...
        if (comment_sexp) comment = gcry_sexp_nth_string(comment_sexp, 1);
    ...
    case PRIVATE_KEY_SHADOWED:
      if (shadow_info) {
        ...
        if (rc) log_error("get_shadow_info failed: %s\n", gpg_strerror(rc));
      } else
...
/* Return the string name from the S-expression S_KEY as well as a
   ...
   this case.  */
static gpg_error_t key_parms_from_sexp(gcry_sexp_t s_key, gcry_sexp_t *r_list,
  ...
  if (r_algoname) {
    if (strlen(algoname) >= algonamesize) return GPG_ERR_BUFFER_TOO_SHORT;
    strcpy(r_algoname, algoname);
  }
  if (r_elems) {
    if (strlen(elems) >= elemssize) return GPG_ERR_BUFFER_TOO_SHORT;
...
static int is_eddsa(gcry_sexp_t keyparms) {
  int result = 0;
  gcry_sexp_t list;
  const char *s;
  size_t n;
  ...
  for (i = list ? gcry_sexp_length(list) - 1 : 0; i > 0; i--) {
    ...
    if (!s) continue; /* Not a data element. */
...
/* Return the public key for the keygrip GRIP.  The result is stored
   ...
   at RESULT. */
gpg_error_t agent_public_key_from_file(ctrl_t ctrl, const unsigned char *grip,
                                       gcry_sexp_t *result) {
  gpg_error_t err;
  int i, idx;
  gcry_sexp_t s_skey;
  ...
  char *format, *p;
  void *
      args[2 + 7 + 2 + 2 + 1]; /* Size is 2 + max. # of elements + 2 for uri + 2
  ...
  *result = NULL;
  ...
  err = extract_private_key(s_skey, 0, &algoname, &npkey, NULL, &elems, array,
  ...
  if (uri_sexp) uri = gcry_sexp_nth_data(uri_sexp, 1, &uri_length);
  ...
  if (!format) {
    err = gpg_error_from_syserror();
    for (i = 0; array[i]; i++) gcry_mpi_release(array[i]);
...
gpg_error_t agent_key_info_from_file(ctrl_t ctrl, const unsigned char *grip,
  ...
  if (r_keytype) *r_keytype = PRIVATE_KEY_UNKNOWN;
  if (r_shadow_info) *r_shadow_info = NULL;
  ...
  if (!err && r_keytype) *r_keytype = keytype;
...
gpg_error_t agent_delete_key(ctrl_t ctrl, const char *desc_text,
  ...
  err = read_key_file(grip, &s_skey);
  if (err == GPG_ERR_ENOENT) err = GPG_ERR_NO_SECKEY;
  if (err) goto leave;
  ...
  switch (key_type) {
    ...
    case PRIVATE_KEY_PROTECTED:
      ...
      if (!force) {
        if (!desc_text) {
          default_desc = xtryasprintf(L_("Do you really want to delete the key "
                                         "identified by keygrip%%0A"
                                         "  %s%%0A  %%C%%0A?"),
                                      hexgrip);
        ...
        err = agent_get_confirmation(ctrl, desc_text_final, L_("Delete key"),
                                     L_("No"), 0);
        if (err) goto leave;
      }
...
gpg_error_t agent_write_shadow_key(const unsigned char *grip,
  ...
  if (!shadow_info) return gpg_error_from_syserror();
  ...
  xfree(shdkey);
  if (err) log_error("error writing key: %s\n", gpg_strerror(err));
