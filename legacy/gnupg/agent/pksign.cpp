...
static int do_encode_md(const byte *md, size_t mdlen, int algo,
                        gcry_sexp_t *r_hash, int raw_value) {
  gcry_sexp_t hash;
  int rc;
  ...
  if (!raw_value) {
    ...
    if (s && strlen(s) < 16) {
      for (i = 0; i < strlen(s); i++) tmp[i] = tolower(s[i]);
      tmp[i] = '\0';
    }

    rc = gcry_sexp_build(&hash, NULL, "(data (flags pkcs1) (hash %s %b))", tmp,
                         (int)mdlen, md);
  } else {
    gcry_mpi_t mpi;
    ...
    if (!rc) {
      rc = gcry_sexp_build(&hash, NULL, "(data (flags raw) (value %m))", mpi);
...
static unsigned int get_dsa_qbits(gcry_sexp_t key) {
  ...
  if (!l1) l1 = gcry_sexp_find_token(key, "protected-private-key", 0);
  if (!l1) l1 = gcry_sexp_find_token(key, "shadowed-private-key", 0);
  if (!l1) l1 = gcry_sexp_find_token(key, "public-key", 0);
  if (!l1) return 0; /* Does not contain a key object.  */
  l2 = gcry_sexp_cadr(l1);
  gcry_sexp_release(l1);
  l1 = gcry_sexp_find_token(l2, "q", 1);
  gcry_sexp_release(l2);
  if (!l1) return 0; /* Invalid object.  */
  q = gcry_sexp_nth_mpi(l1, 1, GCRYMPI_FMT_USG);
  gcry_sexp_release(l1);
  if (!q) return 0; /* Missing value.  */
...
static const char *rfc6979_hash_algo_string(size_t mdlen) {
  switch (mdlen) {
    case 20:
      return "sha1";
    case 28:
...
/* Encode a message digest for use with the EdDSA algorithm
   (i.e. curve Ed25519). */
static gpg_error_t do_encode_eddsa(const byte *md, size_t mdlen,
                                   gcry_sexp_t *r_hash) {
  gpg_error_t err;
  gcry_sexp_t hash;
...
static gpg_error_t do_encode_dsa(const byte *md, size_t mdlen, int pkalgo,
                                 gcry_sexp_t pkey, gcry_sexp_t *r_hash) {
  gpg_error_t err;
  gcry_sexp_t hash;
  unsigned int qbits;
  ...
  *r_hash = NULL;
  ...
  if (pkalgo == GCRY_PK_DSA && (qbits % 8)) {
    /* FIXME: We check the QBITS but print a message about the hash
       length.  */
    log_error(
        _("DSA requires the hash length to be a"
          " multiple of 8 bits\n"));
  ...
  /* ECDSA 521 is special has it is larger than the largest hash
     ...
     processing to 512.  */
  if (pkalgo == GCRY_PK_ECDSA && qbits > 512) qbits = 512;

  /* Check if we're too short.  Too long is safe as we'll
     automatically left-truncate.  */
  if (mdlen < qbits / 8) {
    log_error(_("a %zu bit hash is not valid for a %u bit %s key\n"), mdlen * 8,
              gcry_pk_get_nbits(pkey), gcry_pk_algo_name(pkalgo));
    return GPG_ERR_INV_LENGTH;
  }

  /* Truncate.  */
  if (mdlen > qbits / 8) mdlen = qbits / 8;

  /* Create the S-expression.  */
  err = gcry_sexp_build(&hash, NULL, "(data (flags rfc6979) (hash %s %b))",
                        rfc6979_hash_algo_string(mdlen), (int)mdlen, md);
  if (!err) *r_hash = hash;
  return err;
}
...
static int do_encode_raw_pkcs1(const byte *md, size_t mdlen, unsigned int nbits,
                               gcry_sexp_t *r_hash) {
  int rc;
  gcry_sexp_t hash;
  unsigned char *frame;
  ...
  /* Create the S-expression.  */
  rc = gcry_sexp_build(&hash, NULL, "(data (flags raw) (value %b))",
                       (int)nframe, frame);
  xfree(frame);
...
int agent_pksign_do(ctrl_t ctrl, const char *cache_nonce, const char *desc_text,
                    gcry_sexp_t *signature_sexp, cache_mode_t cache_mode,
                    lookup_ttl_t lookup_ttl, const void *overridedata,
                    size_t overridedatalen) {
  ...
  if (!ctrl->have_keygrip) return GPG_ERR_NO_SECKEY;

  rc = agent_key_from_file(ctrl, cache_nonce, desc_text, ctrl->keygrip,
                           &shadow_info, cache_mode, lookup_ttl, &s_skey, NULL);
  if (rc) {
    if (rc != GPG_ERR_NO_SECKEY) log_error("failed to read the secret key\n");
    goto leave;
  }
  ...
  if (shadow_info) {
...

    {
      ...
      if (desc_text) agent_modify_description(desc_text, NULL, s_skey, &desc2);

      rc = divert_pksign(ctrl, desc2 ? desc2 : desc_text, data, datalen,
                         ctrl->digest.algo, shadow_info, &buf, &len);
      xfree(desc2);
    }
    if (rc) {
    ...
    if (is_RSA) {
      ...
      rc = gcry_sexp_build(&s_sig, NULL, "(sig-val(rsa(s%b)))", (int)len, buf);
    } else if (is_EdDSA) {
      ...
      rc = gcry_sexp_build(&s_sig, NULL, "(sig-val(ecdsa(r%b)(s%b)))", r_buflen,
                           r_buf, s_buflen, s_buf);
      xfree(r_buf_allocated);
    ...
    if (rc) {
      log_error(
          "failed to convert sigbuf returned by divert_pksign "
          "into S-Exp: %s",
          gpg_strerror(rc));
      goto leave;
    }
    ...
    if (agent_is_eddsa_key(s_skey))
      rc = do_encode_eddsa(data, datalen, &s_hash);
    else if (ctrl->digest.algo == MD_USER_TLS_MD5SHA1)
      rc = do_encode_raw_pkcs1(data, datalen, gcry_pk_get_nbits(s_skey),
                               &s_hash);
    else if ((dsaalgo = agent_is_dsa_key(s_skey)))
      rc = do_encode_dsa(data, datalen, dsaalgo, s_skey, &s_hash);
    else
...
int agent_pksign(ctrl_t ctrl, const char *cache_nonce, const char *desc_text,
