...
static void release_keypair_info(KEYPAIR_INFO info) {
  while (info) {
...
static void release_certinfo(CERTINFO info) {
  while (info) {
...
static void release_sinfo(SINFO info) {
  while (info) {
    SINFO tmp = info->next;
    xfree(info);
    info = tmp;
...
static void kpinfo_cb(void *opaque, const char *line) {
  struct kpinfo_cb_parm_s *parm = (kpinfo_cb_parm_s *)opaque;
  KEYPAIR_INFO item;
  char *p;

  if (parm->error) return; /* no need to gather data after an error occurred */

  if ((parm->error = agent_write_status(parm->ctrl, "PROGRESS", "learncard",
                                        "k", "0", "0", NULL)))
    return;
  ...
  if (p == item->hexgrip && *p == 'X' && spacep(p + 1)) {
    item->no_cert = 1;
    p++;
  } else if ((p - item->hexgrip) != 40 ||
             !spacep(p)) { /* not a 20 byte hex keygrip or not followed by a
                              space */
  ...
  *p++ = 0;
  ...
  item->id = p;
  while (*p && !spacep(p)) p++;
  if (p == item->id) { /* invalid ID string */
    parm->error = GPG_ERR_INV_RESPONSE;
    xfree(item);
    return;
  }
...
static void certinfo_cb(void *opaque, const char *line) {
...
static void sinfo_cb(void *opaque, const char *keyword, size_t keywordlen,
                     const char *data) {
  struct sinfo_cb_parm_s *sparm = (sinfo_cb_parm_s *)opaque;
  SINFO item;

  if (sparm->error) return; /* no need to gather data after an error occurred */
...
static int send_cert_back(ctrl_t ctrl, const char *id, void *assuan_context) {
  int rc;
  char *derbuf;
  size_t derbuflen;
  ...
  xfree(derbuf);
  if (!rc) rc = assuan_send_data((assuan_context_t)(assuan_context), NULL, 0);
  if (!rc) rc = assuan_write_line((assuan_context_t)(assuan_context), "END");
  if (rc) {
...
int agent_handle_learn(ctrl_t ctrl, int send, void *assuan_context, int force) {
  ...
  rc = agent_card_learn(ctrl, kpinfo_cb, &parm, certinfo_cb, &cparm, sinfo_cb,
                        &sparm);
  ...
  if (assuan_context) {
    for (sitem = sparm.info; sitem; sitem = sitem->next) {
      if (!strcmp(sitem->keyword, "SERIALNO")) serialno = sitem->data;
      assuan_write_status((assuan_context_t)(assuan_context), sitem->keyword,
                          sitem->data);
    }
  }
  ...
  for (i = 0; certtype_list[i] != -1; i++) {
    ...
    for (citem = cparm.info; citem; citem = citem->next) {
      if (certtype_list[i] != citem->type) continue;
  ...
  for (item = parm.info; item; item = item->next) {
    ...
    if (item->no_cert) continue; /* No public key yet available. */

    if (assuan_context) {
      agent_write_status(ctrl, "KEYPAIRINFO", item->hexgrip, item->id, NULL);
    }

    for (p = item->hexgrip, i = 0; i < 20; p += 2, i++) grip[i] = xtoi_2(p);
    ...
    if (assuan_context && send) {
      ...
      for (citem = cparm.info; citem; citem = citem->next) {
        if (!strcmp(citem->id, item->id)) break;
      }
      if (!citem) {
