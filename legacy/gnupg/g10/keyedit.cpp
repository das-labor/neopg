...
static void show_names(ctrl_t ctrl, estream_t fp, kbnode_t keyblock,
                       PKT_public_key *pk, unsigned int flag, int with_prefs);
static void show_key_with_all_names(ctrl_t ctrl, estream_t fp, KBNODE keyblock,
                                    int only_marked, int with_revoker,
                                    int with_fpr, int with_subkeys,
                                    int with_prefs, int nowarn);
static void show_key_and_fingerprint(ctrl_t ctrl, kbnode_t keyblock,
                                     int with_subkeys);
...
/* TODO: Fix duplicated code between here and the check-sigs/list-sigs
   code in keylist.c. */
static int print_and_check_one_sig_colon(ctrl_t ctrl, kbnode_t keyblock,
                                         kbnode_t node, int *inv_sigs,
                                         int *no_key, int *oth_err,
                                         int *is_selfsig,
  ...
  if (sigrc != '?' || print_without_key) {
    es_printf("sig:%c::%d:%08lX%08lX:%lu:%lu:", sigrc, sig->pubkey_algo,
              (unsigned long)sig->keyid[0], (unsigned long)sig->keyid[1],
              (unsigned long)sig->timestamp, (unsigned long)sig->expiredate);
    ...
    if (opt.show_subpackets) print_subpackets_colon(sig);
  }
 ...
 * always be printed.
 */
int keyedit_print_one_sig(ctrl_t ctrl, int rc, kbnode_t keyblock, kbnode_t node,
  ...
  if (sigrc != '?' || print_without_key) {
    tty_printf("%s%c%c %c%c%c%c%c%c %s %s", is_rev ? "rev" : "sig", sigrc,
               (sig->sig_class - 0x10 > 0 && sig->sig_class - 0x10 < 4)
                   ? '0' + sig->sig_class - 0x10
                   : ' ',
               sig->flags.exportable ? ' ' : 'L',
               sig->flags.revocable ? ' ' : 'R',
               sig->flags.policy_url ? 'P' : ' ',
               sig->flags.notation ? 'N' : ' ', sig->flags.expired ? 'X' : ' ',
               (sig->trust_depth > 9)
                   ? 'T'
    ...
    if (sigrc == '%')
      ...
      if (extended && sig->flags.chosen_selfsig) tty_printf("*");
    } else {
      size_t n;
      char *p = get_user_id(ctrl, sig->keyid, &n);
      tty_print_utf8_string2(
          NULL, (const unsigned char *)(p), n,
...
static int print_and_check_one_sig(ctrl_t ctrl, kbnode_t keyblock,
  ...
  rc = check_key_signature(ctrl, keyblock, node, is_selfsig);
  return keyedit_print_one_sig(ctrl, rc, keyblock, node, inv_sigs, no_key,
                               oth_err, *is_selfsig, print_without_key,
                               extended);
}

static int sign_mk_attrib(PKT_signature *sig, void *opaque) {
  ...
  if (attrib->reason) revocation_reason_build_cb(sig, attrib->reason);
  ...
  if (attrib->trust_depth) {
    ...
    if (attrib->trust_regexp)
      build_sig_subpkt(
          sig, (sigsubpkttype_t)(SIGSUBPKT_FLAG_CRITICAL | SIGSUBPKT_REGEXP),
...
static void trustsig_prompt(byte *trust_value, byte *trust_depth,
                            char **regexp) {
  char *p;
  ...
  *regexp = NULL;
  ...
  tty_printf(
      _("Please decide how far you trust this user to correctly verify"
  ...
  tty_printf(
      _("Please enter the depth of this trust signature.\n"
  ...
  tty_printf(
      _("Please enter a domain to restrict this signature, "
        "or enter for none.\n"));
 ...
 */
static int sign_uids(
    ctrl_t ctrl, estream_t fp, kbnode_t keyblock,
    const std::vector<std::pair<std::string, unsigned int>> &locusr,
    int *ret_modified, int local, int nonrevocable, int trust, int interactive,
    int quick) {
  int rc = 0;
  ...
  for (sk_rover = sk_list; sk_rover; sk_rover = sk_rover->next) {
    ...
    for (node = keyblock; node; node = node->next) {
      if (node->pkt->pkttype == PKT_PUBLIC_KEY) {
        ...
        if (sk_keyid[0] == node->pkt->pkt.signature->keyid[0] &&
          ...
          snprintf(buf, sizeof buf, "%08lX%08lX", (unsigned long)pk->keyid[0],
    ...
    if (primary_pk->expiredate && !selfsig) {
      ...
      if (primary_pk->expiredate <= now) {
        ...
        if (opt.ask_cert_expire && !quick) {
          char *answer =
              cpr_get("sign_uid.expire", _("Do you want your signature to "
    ...
    if (selfsig)
      ...
      if (opt.batch || !opt.ask_cert_level || quick)
        ...
        tty_fprintf(fp, _("How carefully have you verified the key you are "
    ...
    if (opt.batch && opt.answer_yes)
      ...
      ;
    else if (!cpr_get_answer_is_yes("sign_uid.okay", _("Really sign? (y/N) ")))
      continue;
  ...
  reloop: /* (Must use this, because we are modifying the list.)  */
      ...
      if (node->pkt->pkttype == PKT_PUBLIC_KEY)
        ...
        if (selfsig)
          rc = make_keysig_packet(ctrl, &sig, primary_pk,
                                  node->pkt->pkt.user_id, NULL, pk, 0x13, 0, 0,
                                  0, keygen_add_std_prefs, primary_pk, NULL);
        else
          rc = make_keysig_packet(
              ctrl, &sig, primary_pk, node->pkt->pkt.user_id, NULL, pk, klasse,
              0, timestamp, duration, sign_mk_attrib, &attrib, NULL);
        if (rc) {
    ...
    for (node = keyblock; node; node = node->next)
      if (node->flag & NODFLG_DELSIG) delete_kbnode(node);
 ...
 */
static gpg_error_t change_passphrase(ctrl_t ctrl, kbnode_t keyblock) {
  gpg_error_t err;
  kbnode_t node;
  PKT_public_key *pk;
  int any;
  ...
  if (!any) {
    tty_printf(
        _("Key has only stub or on-card key items - "
  ...
  for (node = keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_PUBLIC_KEY ||
      ...
      err = agent_passwd(ctrl, hexgrip, desc, 0, &cache_nonce, &passwd_nonce);
      xfree(desc);

      if (err)
        log_log((err == GPG_ERR_CANCELED || err == GPG_ERR_FULLY_CANCELED)
                    ? GPGRT_LOG_INFO
                    : GPGRT_LOG_ERROR,
                _("key %s: error changing passphrase: %s\n"),
                keystr_with_sub(keyid, subid), gpg_strerror(err));
      if (err == GPG_ERR_FULLY_CANCELED) break;
    }
  }
...
static int fix_keyblock(ctrl_t ctrl, kbnode_t *keyblockp) {
  int changed = 0;

  if (collapse_uids(keyblockp)) changed++;
  if (key_check_all_keysigs(ctrl, *keyblockp, 0, 1)) changed++;
  reorder_keyblock(*keyblockp);
  /* If we modified the keyblock, make sure the flags are right. */
  if (changed) merge_keys_and_selfsig(ctrl, *keyblockp);

  return changed;
}

static int parse_sign_type(const char *str, int *localsig, int *nonrevokesig,
                           int *trustsig) {
  const char *p = str;
...
void keyedit_menu(
    ctrl_t ctrl, const char *username,
    const std::vector<std::pair<std::string, unsigned int>> &locusr,
    const std::vector<std::string> &commands, int quiet, int seckey_check) {
  ...
  if (fix_keyblock(ctrl, &keyblock)) modified++;

  /* See whether we have a matching secret key.  */
  if (seckey_check) {
    have_seckey = !agent_probe_any_secret_key(ctrl, keyblock);
    if (have_seckey && !quiet) tty_printf(_("Secret key is available.\n"));
  }
  ...
  for (;;) {
    ...
    if (run_subkey_warnings) {
      ...
      if (!count_selected_keys(keyblock)) subkey_expire_warning(keyblock);
    }
    ...
    switch (cmd) {
      ...
      case cmdFPR:
        show_key_and_fingerprint(
            ctrl, keyblock,
            (*arg_string == '*' && (!arg_string[1] || spacep(arg_string + 1))));
        break;
      ...
      case cmdSELUID:
        if (strlen(arg_string) == NAMEHASH_LEN * 2)
          ...
          if (*arg_string == '*' && (!arg_string[1] || spacep(arg_string + 1)))
      ...
      case cmdSELKEY: {
        if (*arg_string == '*' && (!arg_string[1] || spacep(arg_string + 1)))
          arg_number = -1; /* Select all. */
        if (menu_select_key(keyblock, arg_number, p)) redisplay = 1;
      } break;

      case cmdCHECK:
        if (key_check_all_keysigs(ctrl, keyblock, count_selected_uids(keyblock),
      ...
      case cmdSIGN: {
        ...
        if (count_uids(keyblock) > 1 && !count_selected_uids(keyblock)) {
          ...
          if (opt.only_sign_text_ids)
            result = cpr_get_answer_is_yes(
                "keyedit.sign_all.okay", _("Really sign all user IDs? (y/N) "));
          else
            result = cpr_get_answer_is_yes(
        ...
        sign_uids(ctrl, NULL, keyblock, locusr, &modified, localsig,
                  nonrevokesig, trustsig, interactive, 0);
      } break;
      ...
      case cmdADDREVOKER: {
        ...
        if (ascii_strcasecmp(arg_string, "sensitive") == 0) sensitive = 1;
        ...
        if (!(n1 = count_selected_keys(keyblock))) {
          if (cpr_get_answer_is_yes("keyedit.revoke.subkey.okay",
                                    _("Do you really want to revoke"
                                      " the entire key? (y/N) "))) {
            if (menu_revkey(ctrl, keyblock)) modified = 1;

            redisplay = 1;
          }
        } else if (cpr_get_answer_is_yes(
                       "keyedit.revoke.subkey.okay",
                       n1 > 1 ? _("Do you really want to revoke"
                                  " the selected subkeys? (y/N) ")
                              : _("Do you really want to revoke"
                                  " this subkey? (y/N) "))) {
          if (menu_revsubkey(ctrl, keyblock)) modified = 1;

          redisplay = 1;
        }

        if (modified) merge_keys_and_selfsig(ctrl, keyblock);
      } break;
      ...
      case cmdTRUST:
        if (opt.trust_model == TM_EXTERNAL) {
          tty_printf(
              _("Owner trust may not be set while "
      ...
      case cmdCLEAN:
        if (menu_clean(ctrl, keyblock, 0)) redisplay = modified = 1;
        break;

      case cmdMINIMIZE:
        if (menu_clean(ctrl, keyblock, 1)) redisplay = modified = 1;
        break;

      case cmdQUIT:
        if (have_commands) goto leave;
        if (!modified && !sec_shadowing) goto leave;
...
static gpg_error_t quick_find_keyblock(ctrl_t ctrl, const char *username,
                                       KEYDB_HANDLE *r_kdbhd,
                                       kbnode_t *r_keyblock) {
  gpg_error_t err;
  KEYDB_HANDLE kdbhd = NULL;
  kbnode_t keyblock = NULL;
  ...
  *r_keyblock = NULL;
  ...
  if (!err) err = keydb_search(kdbhd, &desc, 1, NULL);
  if (!err) {
...
void keyedit_quick_adduid(ctrl_t ctrl, const char *username,
  ...
  if (menu_adduid(ctrl, keyblock, uidstring)) {
    ...
    if (update_trust) revalidation_mark(ctrl);
  }
...
void keyedit_quick_revuid(ctrl_t ctrl, const char *username,
  ...
  for (node = keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_USER_ID &&
      ...
      if (valid_uids == 1 && !node->pkt->pkt.user_id->flags.revoked &&
          !node->pkt->pkt.user_id->flags.expired) {
...
leave:
  if (err) log_error(_("revoking the user ID failed: %s\n"), gpg_strerror(err));
  release_kbnode(keyblock);
  keydb_release(kdbhd);
}
...
void keyedit_quick_set_primary(ctrl_t ctrl, const char *username,
  ...
  any = 0;
  for (node = keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_USER_ID && !any &&
        !node->pkt->pkt.user_id->flags.revoked &&
        !node->pkt->pkt.user_id->flags.expired &&
...
static gpg_error_t find_by_primary_fpr(ctrl_t ctrl, const char *fpr,
                                       kbnode_t *r_keyblock,
                                       KEYDB_HANDLE *r_kdbhd) {
  gpg_error_t err;
  kbnode_t keyblock = NULL;
  ...
  *r_kdbhd = NULL;
  ...
  if (fprlen == 16 && desc.mode == KEYDB_SEARCH_MODE_FPR16 &&
    ...
    ;
           ...
           !memcmp(fprbin, desc.u.fpr, 16) && !desc.u.fpr[16] &&
           !desc.u.fpr[17] && !desc.u.fpr[18] && !desc.u.fpr[19])
    ;
...
void keyedit_quick_sign(
    ctrl_t ctrl, const char *fpr,
  ...
  /* We require a fingerprint because only this uniquely identifies a
     ...
     signing.  */
  if (find_by_primary_fpr(ctrl, fpr, &keyblock, &kdbhd)) goto leave;

  if (fix_keyblock(ctrl, &keyblock)) modified++;

  /* Give some info in verbose.  */
  if (opt.verbose) {
    show_key_with_all_names(ctrl, es_stdout, keyblock, 0, 1 /*with_revoker*/,
                            1 /*with_fingerprint*/, 0, 0, 1);
    es_fflush(es_stdout);
  }
  ...
  for (auto sl : uids) {
    ...
    for (node = keyblock; node; node = node->next) {
      if (node->pkt->pkttype == PKT_USER_ID) {
        ...
        if (uid->attrib_data)
          ;
        else if (*name == '=' && strlen(name + 1) == uid->len &&
                 !memcmp(uid->name, name + 1, uid->len)) { /* Exact match - we
                                                            * don't do a check
                                                            * for ambiguity
                                                            * in this case.  */
          node->flag |= NODFLG_SELUID;
  ...
  for (auto &sl : uids)
    if (!(sl.second & 1)) any = -1; /* That user id was not found.  */
    ...
    for (auto &sl : uids) {
      ...
      if ((flags & 2))
        log_info(_("Invalid user ID '%s': %s\n"), uid,
                 gpg_strerror(GPG_ERR_AMBIGUOUS_NAME));
      else if (!(flags & 1))
        log_info(_("Invalid user ID '%s': %s\n"), uid,
 ...
 *
 */
void keyedit_quick_set_expire(ctrl_t ctrl, const char *fpr,
  ...
  if (expire) expire += make_timestamp();
...
leave:
  release_kbnode(keyblock);
  keydb_release(kdbhd);
  if (err) write_status_error("set_expire", err);
}

static void tty_print_notations(int indent, PKT_signature *sig) {
  int first = 1;
  ...
  for (nd = notation; nd; nd = nd->next) {
    ...
    tty_printf("=");
    tty_print_utf8_string((const unsigned char *)(nd->value),
                          strlen(nd->value));
    tty_printf("\n");
  }
...
static void show_key_with_all_names_colon(ctrl_t ctrl, estream_t fp,
                                          kbnode_t keyblock) {
  KBNODE node;
  int i, j, ulti_hack = 0;
  byte pk_version = 0;
  ...
  for (node = keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_PUBLIC_KEY ||
      ...
      if (!pk->flags.valid)
        ...
        if (trust == 'u') ulti_hack = 1;
        es_putc(trust, fp);
      }

      es_fprintf(fp, ":%u:%d:%08lX%08lX:%lu:%lu::", nbits_from_pk(pk),
                 pk->pubkey_algo, (unsigned long)keyid[0],
                 (unsigned long)keyid[1], (unsigned long)pk->timestamp,
                 (unsigned long)pk->expiredate);
      if (node->pkt->pkttype == PKT_PUBLIC_KEY)
      ...
      es_putc(':', fp);
      /* Print capabilities.  */
      if ((pk->pubkey_usage & PUBKEY_USAGE_ENC)) es_putc('e', fp);
      if ((pk->pubkey_usage & PUBKEY_USAGE_SIG)) es_putc('s', fp);
      if ((pk->pubkey_usage & PUBKEY_USAGE_CERT)) es_putc('c', fp);
      if ((pk->pubkey_usage & PUBKEY_USAGE_AUTH)) es_putc('a', fp);
      es_putc('\n', fp);
  ...
  for (node = keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_USER_ID) {
      ...
      /* flags */
      es_fprintf(fp, "%d,", i);
      if (uid->flags.primary) es_putc('p', fp);
      if (uid->flags.revoked) es_putc('r', fp);
      if (uid->flags.expired) es_putc('e', fp);
      if ((node->flag & NODFLG_SELUID)) es_putc('s', fp);
      if ((node->flag & NODFLG_MARK_A)) es_putc('m', fp);
      es_putc(':', fp);
      /* TOFU */
      es_putc(':', fp);
      es_putc('\n', fp);
    ...
    if (node->pkt->pkttype == PKT_USER_ID && !is_deleted_kbnode(node)) {
      ...
      if (!flag || (flag && (node->flag & flag))) {
        ...
        if (flag & NODFLG_MARK_A)
          ...
          tty_fprintf(fp, "(%d)  ", i);
        tty_print_utf8_string2(fp, (const unsigned char *)(uid->name), uid->len,
                               0);
        tty_fprintf(fp, "\n");
        if (with_prefs && pk) {
          if (pk->version > 3 || uid->selfsigversion > 3) {
            ...
            for (signode = node->next;
              ...
              if (signode->pkt->pkt.signature->flags.chosen_selfsig) {
...
static void show_key_with_all_names(ctrl_t ctrl, estream_t fp, KBNODE keyblock,
  ...
  for (node = keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_PUBLIC_KEY ||
      ...
      if (node->pkt->pkttype == PKT_PUBLIC_KEY) {
        ...
        /* Show a warning once */
        if (!did_warn && (get_validity(ctrl, keyblock, pk, NULL, NULL, 0) &
      ...
      if (pk->flags.revoked) {
        ...
        tty_fprintf(fp, _("The following key was revoked on"
                          " %s by %s key %s\n"),
                    revokestr_from_pk(pk), gcry_pk_algo_name(pk->revoked.algo),
                    user);
        xfree(user);
      }
      ...
      if (with_revoker) {
        if (!pk->revkey && pk->numrevkeys)
          ...
          for (i = 0; i < pk->numrevkeys; i++) {
            ...
            keyid_from_fingerprint(ctrl, pk->revkey[i].fpr, MAX_FINGERPRINT_LEN,
                                   r_keyid);

            user = get_user_id_string_native(ctrl, r_keyid);
            tty_fprintf(fp, _("This key may be revoked by %s key %s"),
                        algo ? algo : "?", user);
      ...
      tty_fprintf(fp, "%s%c %s/%s",
                  node->pkt->pkttype == PKT_PUBLIC_KEY && have_seckey
                      ? "sec"
      ...
      if (serialno) {
        ...
        tty_fprintf(fp, "%*s%s", 5, "", _("card-no: "));
      ...
      } else if (pk->seckey_info && pk->seckey_info->is_protected &&
                 pk->seckey_info->s2k.mode == 1002) {
        /* FIXME: Check whether this code path is still used.  */
        tty_fprintf(fp, "%*s%s", 5, "", _("card-no: "));
        if (pk->seckey_info->ivlen == 16 &&
            !memcmp(pk->seckey_info->iv, "\xD2\x76\x00\x01\x24\x01", 6)) {
          /* This is an OpenPGP card. */
          for (i = 8; i < 14; i++) {
            if (i == 10) tty_fprintf(fp, " ");
  ...
  show_names(ctrl, fp, keyblock, primary, only_marked ? NODFLG_MARK_A : 0,
             with_prefs);
  ...
  for (node = keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_PUBLIC_KEY ||
      ...
      tty_printf(
          ...
          pubkey_string(pk, pkstrbuf, sizeof pkstrbuf), keystr_from_pk(pk));
...
static void show_key_and_fingerprint(ctrl_t ctrl, kbnode_t keyblock,
  ...
  tty_printf("\n");
  if (pk) print_fingerprint(ctrl, NULL, pk, 2);
  if (with_subkeys) {
    for (node = keyblock; node; node = node->next) {
      if (node->pkt->pkttype == PKT_PUBLIC_SUBKEY) {
        pk = node->pkt->pkt.public_key;
...
/* Show a warning if no uids on the key have the primary uid flag
   set. */
static void no_primary_warning(KBNODE keyblock) {
  KBNODE node;
  ...
  if (uid_count > 1 && !have_primary)
    log_info(
        _("WARNING: no user ID has been marked as primary.  This command"
...
static void subkey_expire_warning(kbnode_t keyblock) {
  ...
  for (node = keyblock; node; node = node->next) {
    ...
    if (node->pkt->pkttype != PKT_PUBLIC_SUBKEY) continue;
    pk = node->pkt->pkt.public_key;

    if (!pk->flags.valid) continue;
    if (pk->flags.revoked) continue;
    if (pk->timestamp > curtime) continue; /* Ignore future keys.  */
  ...
  if (!subexpire) return; /* No valid subkey with an expiration time.  */
 ...
 * user id.
 */
static int menu_adduid(ctrl_t ctrl, kbnode_t pub_keyblock,
    ...
    if (node->pkt->pkttype == PKT_USER_ID) {
      ...
      if (selected) {
        /* Only cause a trust update if we delete a
           non-revoked user id */
        if (!node->pkt->pkt.user_id->flags.revoked) update_trust = 1;
        delete_kbnode(node);
      }
...
static int menu_delsig(ctrl_t ctrl, kbnode_t pub_keyblock) {
  ...
  for (node = pub_keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_USER_ID) {
      ...
      if (opt.with_colons)
        valid =
            print_and_check_one_sig_colon(ctrl, pub_keyblock, node, &inv_sig,
                                          &no_key, &other_err, &selfsig, 1);
      else
        valid = print_and_check_one_sig(ctrl, pub_keyblock, node, &inv_sig,
                                        &no_key, &other_err, &selfsig, 1, 0);

      if (valid) {
        okay = cpr_get_answer_yes_no_quit(
            "keyedit.delsig.valid", _("Delete this good signature? (y/N/q)"));

        /* Only update trust if we delete a good signature.
           The other two cases do not affect trust. */
        if (okay) update_trust = 1;
      ...
      if (okay == -1) break;
      if (okay && selfsig &&
          !cpr_get_answer_is_yes("keyedit.delsig.selfsig",
...
static void menu_delkey(KBNODE pub_keyblock) {
  ...
  for (node = pub_keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_PUBLIC_SUBKEY) {
      ...
      if (selected) delete_kbnode(node);
    } else if (selected && node->pkt->pkttype == PKT_SIGNATURE)
      delete_kbnode(node);
    else
      selected = 0;
...
static int menu_addrevoker(ctrl_t ctrl, kbnode_t pub_keyblock, int sensitive) {
  ...
  if (pk->numrevkeys == 0 && pk->version == 3) {
    ...
    if (opt.expert) {
      tty_printf(
          _("WARNING: This is a PGP 2.x-style key.  "
      ...
      tty_printf(
          _("You may not add a designated revoker to "
            "a PGP 2.x-style key.\n"));
      return 0;
    }
  }
  ...
  for (;;) {
    ...
    if (fprlen != 20) {
      log_error(
          _("cannot appoint a PGP 2.x style key as a "
            "designated revoker\n"));
      continue;
    }

    revkey.klasse = 0x80;
    if (sensitive) revkey.klasse |= 0x40;
    ...
    if (cmp_public_keys(revoker_pk, pk) == 0) {
      ...
      log_error(
          _("you cannot appoint a key as its own "
            "designated revoker\n"));
    ...
    if (!pk->revkey && pk->numrevkeys)
      ...
      for (i = 0; i < pk->numrevkeys; i++) {
        if (memcmp(&pk->revkey[i], &revkey, sizeof(struct revocation_key)) ==
            0) {
          char buf[50];

          log_error(
              _("this key has already been designated "
                "as a revoker\n"));
    ...
    tty_printf(
        _("WARNING: appointing a key as a designated revoker "
          "cannot be undone!\n"));
  ...
  for (node = pub_keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_PUBLIC_KEY) {
      ...
      if (keyid[0] == sig->keyid[0] && keyid[1] == sig->keyid[1] &&
          ((mainkey && uid && uid->created && (sig->sig_class & ~3) == 0x10) ||
        ...
        if (mainkey)
          ...
          rc = update_keysig_packet(ctrl, &newsig, sig, main_pk, NULL, sub_pk,
                                    main_pk, keygen_add_key_expire, sub_pk);
        if (rc) {
          log_error("make_keysig_packet failed: %s\n", gpg_strerror(rc));
          if (rc == GPG_ERR_TRUE) rc = GPG_ERR_GENERAL;
          return rc;
        }
      ...
      if (keyid[0] == sig->keyid[0] && keyid[1] == sig->keyid[1] &&
        ...
        if (mainkey)
          main_pk->pubkey_usage =
              ask_key_flags(main_pk->pubkey_algo, 0, main_pk->pubkey_usage);
        else
          sub_pk->pubkey_usage =
              ask_key_flags(sub_pk->pubkey_algo, 1, sub_pk->pubkey_usage);

        if (mainkey)
          rc = update_keysig_packet(ctrl, &newsig, sig, main_pk, uid, NULL,
                                    main_pk, keygen_add_key_flags, main_pk);
        else
          rc = update_keysig_packet(ctrl, &newsig, sig, main_pk, NULL, sub_pk,
...
static int menu_backsign(ctrl_t ctrl, kbnode_t pub_keyblock) {
  ...
  for (node = pub_keyblock; node; node = node->next) {
    ...
    for (node2 = node->next; node2 && node2->pkt->pkttype == PKT_SIGNATURE;
         node2 = node2->next)
    ...
    rc = make_backsig(ctrl, sig_pk->pkt->pkt.signature, main_pk, sub_pk, sub_pk,
                      timestamp, NULL);
    if (!rc) {
      PKT_signature *newsig;
      PACKET *newpkt;

      rc = update_keysig_packet(ctrl, &newsig, sig_pk->pkt->pkt.signature,
                                main_pk, NULL, sub_pk, main_pk, NULL, NULL);
      if (!rc) {
...
static int change_primary_uid_cb(PKT_signature *sig, void *opaque) {
  byte buf[1];
    ...
    if (node->pkt->pkttype == PKT_PUBLIC_KEY) {
      ...
      if (keyid[0] == sig->keyid[0] && keyid[1] == sig->keyid[1] &&
        ...
        if (sig->version < 4) {
          ...
          if (!p)
            p = parse_sig_subpkt(sig->unhashed, SIGSUBPKT_PRIMARY_UID, NULL);
          if (p && *p) /* yes */
            action = selected ? 0 : -1;
          else /* no */
            action = selected ? 1 : 0;
          ...
          if (action) {
            int rc = update_keysig_packet(ctrl, &newsig, sig, main_pk, uid,
                                          NULL, main_pk, change_primary_uid_cb,
    ...
    if (node->pkt->pkttype == PKT_PUBLIC_KEY) {
      ...
      if (keyid[0] == sig->keyid[0] && keyid[1] == sig->keyid[1] &&
        ...
        if (sig->version < 4) {
          ...
          rc = update_keysig_packet(ctrl, &newsig, sig, main_pk, uid, NULL,
                                    main_pk, keygen_upd_std_prefs, NULL);
          if (rc) {
            log_error("update_keysig_packet failed: %s\n", gpg_strerror(rc));
            return 0;
          }
...
static int menu_set_notation(ctrl_t ctrl, const char *string,
  ...
  if (string)
    ...
    answer = cpr_get_utf8("keyedit.add_notation", _("Enter the notation: "));
    if (answer[0] == '\0' || answer[0] == CONTROL_D) {
      xfree(answer);
      return 0;
    }
  ...
  if (!ascii_strcasecmp(answer, "none") || !ascii_strcasecmp(answer, "-"))
      ...
      if (keyid[0] == sig->keyid[0] && keyid[1] == sig->keyid[1] &&
        ...
        if (sig->version < 4)
          ...
          if (notation) {
            ...
            if (notation->flags.ignore && !deleting) continue;
          } else {
          ...
          rc = update_keysig_packet(ctrl, &newsig, sig, main_pk, uid, NULL,
                                    main_pk, keygen_add_notations, notation);
          if (rc) {
            log_error("update_keysig_packet failed: %s\n", gpg_strerror(rc));
            free_notation(notation);
            return 0;
...
static int menu_select_uid_namehash(KBNODE keyblock, const char *namehash) {
  ...
  for (i = 0; i < NAMEHASH_LEN; i++) hash[i] = hextobyte(&namehash[i * 2]);
    ...
    for (node = keyblock; node; node = node->next)
      if (node->pkt->pkttype == PKT_PUBLIC_SUBKEY ||
        ...
        if (strlen(p) == 8 || strlen(p) == 16) {
          ...
          format_keyid(kid, strlen(p) == 8 ? KF_SHORT : KF_LONG, kid_str,
                       sizeof(kid_str));

          if (strcmp(p, kid_str) == 0) match = 1;
        } else {
          char fp[2 * MAX_FINGERPRINT_LEN + 1];
          hexfingerprint(node->pkt->pkt.public_key, fp, sizeof(fp));
          if (strcmp(fp, p) == 0) match = 1;
        }
...
static int count_uids_with_flag(KBNODE keyblock, unsigned flag) {
  ...
  for (node = keyblock; node; node = node->next)
    if (node->pkt->pkttype == PKT_USER_ID && (node->flag & flag)) i++;
  return i;
}
...
static int count_uids(KBNODE keyblock) {
  ...
  for (node = keyblock; node; node = node->next)
    if (node->pkt->pkttype == PKT_USER_ID) i++;
  return i;
}
 ...
 */
static void ask_revoke_sig(ctrl_t ctrl, kbnode_t keyblock, kbnode_t node) {
  int doit = 0;
  PKT_user_id *uid;
  PKT_signature *sig = node->pkt->pkt.signature;
...
static int menu_revsig(ctrl_t ctrl, kbnode_t keyblock) {
  ...
  for (node = keyblock; node; node = node->next) {
    ...
    if (node->pkt->pkttype == PKT_USER_ID) {
      ...
      if ((sig->sig_class & ~3) == 0x10) {
        ...
        if (sig->flags.revocable) node->flag |= NODFLG_SELSIG;
  ...
  /* ask */
  for (node = keyblock; node; node = node->next) {
    if (!(node->flag & NODFLG_SELSIG)) continue;
  ...
  any = 0;
  for (node = keyblock; node; node = node->next) {
    if (!(node->flag & NODFLG_MARK_A)) continue;
    if (!any) {
      any = 1;
...
reloop: /* (must use this, because we are modifying the list) */
  ...
  for (node = keyblock; node; node = node->next) {
    ...
    if (!(node->flag & NODFLG_MARK_A) || node->pkt->pkttype != PKT_SIGNATURE)
      continue;
...
static int core_revuid(ctrl_t ctrl, kbnode_t keyblock, KBNODE node,
  ...
  if (node->pkt->pkttype != PKT_USER_ID) {
    ...
    if (uid->flags.revoked) {
      ...
      rc = make_keysig_packet(ctrl, &sig, pk, uid, NULL, pk, 0x30, 0, timestamp,
                              0, sign_mk_attrib, &attrib, NULL);
      if (rc) {
        write_status_error("keysig", rc);
        log_error(_("signing failed: %s\n"), gpg_strerror(rc));
        return 1;
        ...
        if (!update_trust && ((get_validity(ctrl, keyblock, pk, uid, NULL, 0) &
                               TRUST_MASK) >= TRUST_UNDEFINED))
          update_trust = 1;
#endif /*!NO_TRUST_MODELS*/
...
/* Revoke a user ID (i.e. revoke a user ID selfsig).  Return true if
   keyblock changed.  */
static int menu_revuid(ctrl_t ctrl, kbnode_t pub_keyblock) {
...
reloop: /* (better this way because we are modifying the keyring) */
  ...
  if (changed) commit_kbnode(&pub_keyblock);
...
static int menu_revkey(ctrl_t ctrl, kbnode_t pub_keyblock) {
  ...
  rc = make_keysig_packet(ctrl, &sig, pk, NULL, NULL, pk, 0x20, 0, 0, 0,
...
static int menu_revsubkey(ctrl_t ctrl, kbnode_t pub_keyblock) {
  ...
  reason = ask_revocation_reason(1, 0, 0);
  if (!reason) return 0; /* User decided to cancel.  */
...
reloop: /* (better this way because we are modifying the keyring) */
  ...
  for (node = pub_keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_PUBLIC_SUBKEY &&
      ...
      node->flag &= ~NODFLG_SELKEY;
      rc = make_keysig_packet(ctrl, &sig, mainpk, NULL, subpk, mainpk, 0x28, 0,
                              0, 0, sign_mk_attrib, &attrib, NULL);
      if (rc) {
        write_status_error("keysig", rc);
        log_error(_("signing failed: %s\n"), gpg_strerror(rc));
        release_revocation_reason_info(reason);
...
static int enable_disable_key(ctrl_t ctrl, kbnode_t keyblock, int disable) {
  ...
  if (disable) newtrust |= TRUST_FLAG_DISABLED;
  if (trust == newtrust) return 0; /* already in that state */
