...
static void init_compress(compress_filter_context_t *zfx, bz_stream *bzs) {
  int rc;
  int level;
...
static int do_compress(compress_filter_context_t *zfx, bz_stream *bzs,
                       int flush, IOBUF a) {
  int rc;
  int zrc;
  unsigned n;
  ...
  do {
    ...
    if (DBG_FILTER)
      log_debug(
          ...
          (unsigned)bzs->avail_in, (unsigned)bzs->avail_out, (unsigned)n, zrc);
...
static void init_uncompress(compress_filter_context_t *zfx, bz_stream *bzs) {
  int rc;
...
static int do_uncompress(compress_filter_context_t *zfx, bz_stream *bzs,
  ...
  do {
    if (bzs->avail_in < zfx->inbufsize && refill) {
      n = bzs->avail_in;
      if (!n) bzs->next_in = (char *)zfx->inbuf;
    ...
    else if (zrc == BZ_OK && eofseen && !bzs->avail_in && bzs->avail_out > 0) {
...
int compress_filter_bz2(void *opaque, int control, IOBUF a, byte *buf,
  ...
  bz_stream *bzs = (bz_stream *)zfx->opaque;
  int rc = 0;

  if (control == IOBUFCTRL_UNDERFLOW) {
    if (!zfx->status) {
      bzs = (bz_stream *)xmalloc_clear(sizeof *bzs);
      zfx->opaque = bzs;
      init_uncompress(zfx, bzs);
      zfx->status = 1;
    }

    bzs->next_out = (char *)buf;
    bzs->avail_out = size;
    ...
    if (!zfx->status) {
      ...
      if (zfx->algo != COMPRESS_ALGO_BZIP2) BUG();
