...
/* Returns 1 if CTB is a new format ctb and 0 if CTB is an old format
   ctb.  */
static int ctb_new_format_p(int ctb) {
...
static int ctb_pkttype(int ctb) {
  if (ctb_new_format_p(ctb)) /* Bits 0 through 5 are the packet type.  */
    return (ctb & ((1 << 6) - 1));
  else
...
int build_packet(IOBUF out, PACKET *pkt) {
  int rc = 0;
  int new_ctb = 0;
  int ctb, pkttype;

  if (DBG_PACKET) log_debug("build_packet() type=%d\n", pkt->pkttype);
  log_assert(pkt->pkt.generic);

  switch ((pkttype = pkt->pkttype)) {
    case PKT_PUBLIC_KEY:
      if (pkt->pkt.public_key->seckey_info) pkttype = PKT_SECRET_KEY;
      break;
    case PKT_PUBLIC_SUBKEY:
      if (pkt->pkt.public_key->seckey_info) pkttype = PKT_SECRET_SUBKEY;
      break;
    case PKT_PLAINTEXT:
    ...
    case PKT_COMPRESSED:
      ...
      break;
    case PKT_USER_ID:
      if (pkt->pkt.user_id->attrib_data) pkttype = PKT_ATTRIBUTE;
      break;
    default:
      break;
  }
...
gpg_error_t build_packet_and_meta(iobuf_t out, PACKET *pkt) {
  ...
  if (err)
    ...
    if (sig->flags.checked) {
      rt.sigcache = 1;
      if (sig->flags.valid) rt.sigcache |= 2;
    }
    err = do_ring_trust(out, &rt);
  } else if (pkt->pkttype == PKT_USER_ID || pkt->pkttype == PKT_ATTRIBUTE) {
    ...
    err = do_ring_trust(out, &rt);
    rt.url = NULL;
  } else if (pkt->pkttype == PKT_PUBLIC_KEY || pkt->pkttype == PKT_SECRET_KEY) {
...
gpg_error_t gpg_mpi_write(iobuf_t out, gcry_mpi_t a) {
  ...
  if (gcry_mpi_get_flag(a, GCRYMPI_FLAG_OPAQUE)) {
    ...
    if (!rc && p) rc = iobuf_write(out, p, (nbits + 7) / 8);
  } else {
    char buffer[(MAX_EXTERN_MPI_BITS + 7) / 8 +
                2]; /* 2 is for the mpi length. */
    size_t nbytes;

    nbytes = DIM(buffer);
    rc = gcry_mpi_print(GCRYMPI_FMT_PGP, (unsigned char *)(buffer), nbytes,
                        &nbytes, a);
    if (!rc)
...
static gpg_error_t write_fake_data(IOBUF out, gcry_mpi_t a) {
  unsigned int n;
  void *p;
...
static gpg_error_t do_ring_trust(iobuf_t out, PKT_ring_trust *rt) {
  ...
  if (rt->subtype == RING_TRUST_KEY || rt->subtype == RING_TRUST_UID) {
    if (rt->url) namelen = strlen(rt->url);
  ...
  if (rt->subtype == RING_TRUST_KEY || rt->subtype == RING_TRUST_UID) {
    ...
    if (namelen) iobuf_write(out, rt->url, namelen);
  }
...
static int do_key(iobuf_t out, int ctb, PKT_public_key *pk) {
  ...
  if (pk->seckey_info) {
    ...
    if (ski->is_protected) {
      ...
      if (ski->s2k.mode == 3) iobuf_put(a, ski->s2k.count);
    ...
    if (ski->s2k.mode == 1001)
      ...
      if (p) iobuf_write(a, p, (ndatabits + 7) / 8);
    } else {
      /* Non-protected key. */
      for (; i < nskey; i++)
        if ((err = gpg_mpi_write(a, pk->pkey[i]))) goto leave;
      write_16(a, ski->csum);
    }
  }
...
static int do_symkey_enc(IOBUF out, int ctb, PKT_symkey_enc *enc) {
  ...
  if (enc->s2k.mode == 1 || enc->s2k.mode == 3) {
    ...
    if (enc->s2k.mode == 3) iobuf_put(a, enc->s2k.count);
  }
  if (enc->seskeylen) iobuf_write(a, enc->seskey, enc->seskeylen);
...
static int do_pubkey_enc(IOBUF out, int ctb, PKT_pubkey_enc *enc) {
  ...
  if (!n) write_fake_data(a, enc->data[0]);
 ...
 * was deleted.
 */
int delete_sig_subpkt(subpktarea_t *area, sigsubpkttype_t reqtype) {
  int buflen;
...
void build_sig_subpkt(PKT_signature *sig, sigsubpkttype_t type,
  ...
  if (parse_one_sig_subpkt(buffer, buflen, type) < 0) BUG();
  ...
  if (critical) type = (sigsubpkttype_t)(type | SIGSUBPKT_FLAG_CRITICAL);
...
void build_sig_subpkt_from_sig(PKT_signature *sig, PKT_public_key *pksk) {
  ...
  if (sig->expiredate) {
    ...
    build_sig_subpkt(
        sig, (sigsubpkttype_t)(SIGSUBPKT_SIG_EXPIRE | SIGSUBPKT_FLAG_CRITICAL),
        buf, 4);
  }
}

void build_attribute_subpkt(PKT_user_id *uid, byte type, const void *buf,
                            u32 buflen, const void *header, u32 headerlen) {
  byte *attrib;
  int idx;
  ...
  uid->attrib_data = (byte *)xrealloc(
      uid->attrib_data, uid->attrib_len + idx + 1 + headerlen + buflen);
  ...
  if (idx == 5) {
    ...
    attrib[0] =
        1 + headerlen + buflen; /* Good luck finding a JPEG this small! */

  attrib[idx++] = type;

  /* Tack on our data at the end */

  if (headerlen > 0) memcpy(&attrib[idx], header, headerlen);
...
static char *notation_value_to_human_readable_string(
  ...
  /* Binary data.  */
  {
    ...
    return xasprintf(_("[ not human readable (%zu bytes: %s%s) ]"), len,
                     preview, i < len ? "..." : "");
  } else
...
struct notation *string_to_notation(const char *string, int is_utf8) {
  ...
  for (s = string; *s != '='; s++) {
    ...
    if (!*s && notation->flags.ignore) break;

    if (!*s || !isascii(*s) || (!isgraph(*s) && !isspace(*s))) {
      log_error(
          _("a notation name must have only printable characters"
  ...
  if (saw_at > 1) {
    log_error(
        _("a notation name must not contain more than"
          " one '@' character\n"));
    goto fail;
  }
  ...
  if (*s) {
    ...
    for (s++; *s; s++) {
      if (!isascii(*s))
        ...
        log_error(
            _("a notation value must not use any"
              " control characters\n"));
        goto fail;
      }
    }
...
/* Like string_to_notation, but store opaque data rather than human
   readable data.  */
struct notation *blob_to_notation(const char *name, const char *data,
                                  size_t len) {
  const char *s;
  int saw_at = 0;
  struct notation *notation;
...
static int do_signature(IOBUF out, int ctb, PKT_signature *sig) {
  ...
  if (sig->version < 4) iobuf_put(a, 5); /* Constant */
  ...
  if (sig->version >= 4) {
    ...
    write_16(a, nn);
    if (nn) iobuf_write(a, sig->hashed->data, nn);
    nn = sig->unhashed ? sig->unhashed->len : 0;
    write_16(a, nn);
    if (nn) iobuf_write(a, sig->unhashed->data, nn);
  }
  iobuf_put(a, sig->digest_start[0]);
  iobuf_put(a, sig->digest_start[1]);
  n = pubkey_get_nsig((pubkey_algo_t)(sig->pubkey_algo));
  if (!n) write_fake_data(a, sig->data[0]);
  for (i = 0; i < n && !rc; i++) rc = gpg_mpi_write(a, sig->data[i]);
...
static int write_16(IOBUF out, u16 a) {
  iobuf_put(out, a >> 8);
  if (iobuf_put(out, a)) return -1;
  return 0;
}

/* Write a 32-bit quantity to OUT in big endian order.  */
static int write_32(IOBUF out, u32 a) {
  iobuf_put(out, a >> 24);
  iobuf_put(out, a >> 16);
  iobuf_put(out, a >> 8);
 ...
 */
static int write_header2(IOBUF out, int ctb, u32 len, int hdrlen) {
  if (ctb_new_format_p(ctb)) return write_new_header(out, ctb, len, hdrlen);
  ...
  {
    if (hdrlen == 2 && len < 256) /* 00 => 1 byte length.  */
      ;
    ...
    if (!len) /* 11 => Indeterminate length.  */
      ctb |= 3;
    else if (len < 256)
      /* 00 => 1 byte length.  */
      ;
  ...
  if (iobuf_put(out, ctb)) return -1;

  if (len || hdrlen) {
    if (ctb & 2) {
      if (iobuf_put(out, len >> 24)) return -1;
      if (iobuf_put(out, len >> 16)) return -1;
    }

    if (ctb & 3)
      if (iobuf_put(out, len >> 8)) return -1;

    if (iobuf_put(out, len)) return -1;
  }
...
static int write_new_header(IOBUF out, int ctb, u32 len, int hdrlen) {
  if (hdrlen) log_bug("can't cope with hdrlen yet\n");
  ...
  if (!len) {
    ...
    if (len < 192) {
      ...
      len -= 192;
      if (iobuf_put(out, (len / 256) + 192)) return -1;
      if (iobuf_put(out, (len % 256))) return -1;
    } else {
      if (iobuf_put(out, 0xff)) return -1;
      if (iobuf_put(out, (len >> 24) & 0xff)) return -1;
      if (iobuf_put(out, (len >> 16) & 0xff)) return -1;
      if (iobuf_put(out, (len >> 8) & 0xff)) return -1;
      if (iobuf_put(out, len & 0xff)) return -1;
    }
  }
  return 0;
}
