...
unsigned char encode_s2k_iterations(int iterations) {
  ...
  if (iterations <= 1024) return 0; /* Command line arg compatibility.  */
...
int have_static_passphrase() { return !!fd_passwd; }
...
const char *get_static_passphrase(void) { return fd_passwd; }
 ...
 */
static char *passphrase_get(int nocache, const char *cacheid, int repeat,
  ...
  if (tryagain_text) tryagain_text = _(tryagain_text);
  ...
  if (!rc)
    ;
  else if (rc == GPG_ERR_CANCELED || rc == GPG_ERR_FULLY_CANCELED) {
...
void passphrase_clear_cache(const char *cacheid) {
  ...
  if (rc) log_error(_("problem with the agent: %s\n"), gpg_strerror(rc));
}
...
DEK *passphrase_to_dek(int cipher_algo, STRING2KEY *s2k, int create,
  ...
  if (!canceled) canceled = &dummy_canceled;
  *canceled = 0;
  ...
  if (create && (s2k->mode == 1 || s2k->mode == 3)) {
    ...
    if (s2k->mode == 3) {
      ...
      if (!opt.s2k_count) opt.s2k_count = encode_s2k_iterations(0);
  ...
  if (!next_pw && is_status_enabled()) {
    ...
    snprintf(buf, sizeof buf, "%d %d %d", cipher_algo, s2k->mode,
             s2k->hash_algo);
  ...
  if ((!pw || !*pw) && create)
    ...
    if (!(dek->keylen > 0 && dek->keylen <= DIM(dek->key))) BUG();
...
void emit_status_need_passphrase(ctrl_t ctrl, u32 *keyid, u32 *mainkeyid,
  ...
  snprintf(buf, sizeof buf, "%08lX%08lX %08lX%08lX %d 0",
           ...
           (unsigned long)(mainkeyid ? mainkeyid[1] : keyid[1]), pubkey_algo);
...
char *gpg_format_keydesc(ctrl_t ctrl, PKT_public_key *pk, int mode,
  ...
  is_subkey =
      (pk->main_keyid[0] && pk->main_keyid[1] &&
       pk->keyid[0] != pk->main_keyid[0] && pk->keyid[1] != pk->main_keyid[1]);
  algo_name = openpgp_pk_algo_name((pubkey_algo_t)(pk->pubkey_algo));
  ...
  switch (mode) {
    ...
    case FORMAT_KEYDESC_IMPORT:
      prompt =
          _("Please enter the passphrase to import the"
            " OpenPGP secret key:");
      break;
    case FORMAT_KEYDESC_EXPORT:
      if (is_subkey)
        prompt =
            _("Please enter the passphrase to export the"
    ...
    case FORMAT_KEYDESC_DELKEY:
      if (is_subkey)
        prompt =
            _("Do you really want to permanently delete the"
  ...
  desc = xtryasprintf(_("%s\n"
                        ...
                        "%u-bit %s key, ID %s,\n"
                        "created %s%s.\n%s"),
                      prompt, (int)uidlen, uid, nbits_from_pk(pk), algo_name,
                      keystr(pk->keyid), timestr, maink ? maink : "", trailer);
  xfree(maink);
  xfree(uid);
