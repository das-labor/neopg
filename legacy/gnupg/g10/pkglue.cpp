 ...
 */
int pk_verify(pubkey_algo_t pkalgo, gcry_mpi_t hash, gcry_mpi_t *data,
              gcry_mpi_t *pkey) {
  ...
  if (pkalgo == PUBKEY_ALGO_DSA) {
    rc = gcry_sexp_build(&s_pkey, NULL, "(public-key(dsa(p%m)(q%m)(g%m)(y%m)))",
                         pkey[0], pkey[1], pkey[2], pkey[3]);
  } else if (pkalgo == PUBKEY_ALGO_ELGAMAL_E || pkalgo == PUBKEY_ALGO_ELGAMAL) {
    rc = gcry_sexp_build(&s_pkey, NULL, "(public-key(elg(p%m)(g%m)(y%m)))",
  ...
  if (rc) BUG(); /* gcry_sexp_build should never fail.  */
  ...
  if (pkalgo == PUBKEY_ALGO_DSA) {
    if (!data[0] || !data[1])
      ...
      rc = gcry_sexp_build(&s_sig, NULL, "(sig-val(dsa(r%m)(s%m)))", data[0],
                           data[1]);
  } else if (pkalgo == PUBKEY_ALGO_ECDSA) {
    if (!data[0] || !data[1])
      rc = GPG_ERR_BAD_MPI;
    else
      rc = gcry_sexp_build(&s_sig, NULL, "(sig-val(ecdsa(r%m)(s%m)))", data[0],
                           data[1]);
  } else if (pkalgo == PUBKEY_ALGO_EDDSA) {
    gcry_mpi_t r = data[0];
    gcry_mpi_t s = data[1];
    ...
    if (!r || !s)
      ...
      if (rlen < neededfixedlen && !gcry_mpi_get_flag(r, GCRYMPI_FLAG_OPAQUE) &&
          !(rc = gcry_mpi_print(GCRYMPI_FMT_USG, (unsigned char *)(buf),
                                sizeof buf, &n, r))) {
      ...
      if (slen < neededfixedlen && !gcry_mpi_get_flag(s, GCRYMPI_FLAG_OPAQUE) &&
          !(rc = gcry_mpi_print(GCRYMPI_FMT_USG, (unsigned char *)(buf),
                                sizeof buf, &n, s))) {
        log_assert(n < neededfixedlen);
        memmove(buf + (neededfixedlen - n), buf, n);
      ...
      if (!rc)
        rc = gcry_sexp_build(&s_sig, NULL, "(sig-val(eddsa(r%M)(s%M)))", r, s);

      if (r != data[0]) gcry_mpi_release(r);
      if (s != data[1]) gcry_mpi_release(s);
    }
  } else if (pkalgo == PUBKEY_ALGO_ELGAMAL || pkalgo == PUBKEY_ALGO_ELGAMAL_E) {
    if (!data[0] || !data[1])
      rc = GPG_ERR_BAD_MPI;
    else
      rc = gcry_sexp_build(&s_sig, NULL, "(sig-val(elg(r%m)(s%m)))", data[0],
                           data[1]);
  } else if (pkalgo == PUBKEY_ALGO_RSA || pkalgo == PUBKEY_ALGO_RSA_S) {
    if (!data[0])
      rc = GPG_ERR_BAD_MPI;
  ...
  if (!rc) rc = gcry_pk_verify(s_sig, s_hash, s_pkey);
...
int pk_encrypt(pubkey_algo_t algo, gcry_mpi_t *resarr, gcry_mpi_t data,
  ...
  if (algo == PUBKEY_ALGO_ELGAMAL || algo == PUBKEY_ALGO_ELGAMAL_E) {
    ...
    if (!rc) rc = gcry_sexp_build(&s_data, NULL, "%m", data);
  ...
  if (!rc) rc = gcry_pk_encrypt(&s_ciph, s_data, s_pkey);
  ...
  if (rc)
    ...
    if (fpn != 20)
      ...
      rc = pk_ecdh_encrypt_with_shared_point(1 /*=encrypton*/, shared, fp, data,
                                             pkey, &result);
...
int pk_check_secret_key(pubkey_algo_t pkalgo, gcry_mpi_t *skey) {
  gcry_sexp_t s_skey;
  int rc;

  if (pkalgo == PUBKEY_ALGO_DSA) {
    rc = gcry_sexp_build(&s_skey, NULL,
                         "(private-key(dsa(p%m)(q%m)(g%m)(y%m)(x%m)))", skey[0],
                         skey[1], skey[2], skey[3], skey[4]);
  } else if (pkalgo == PUBKEY_ALGO_ELGAMAL || pkalgo == PUBKEY_ALGO_ELGAMAL_E) {
    rc =
        gcry_sexp_build(&s_skey, NULL, "(private-key(elg(p%m)(g%m)(y%m)(x%m)))",
