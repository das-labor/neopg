...
#define S2K_DIGEST_ALGO \
  (opt.s2k_digest_algo ? opt.s2k_digest_algo : DEFAULT_S2K_DIGEST_ALGO)
...
#define openpgp_cipher_open(_a, _b, _c, _d)                               \
  gcry_cipher_open((_a), map_cipher_openpgp_to_gcry((cipher_algo_t)(_b)), \
                   (_c), (_d))
#define openpgp_cipher_get_algo_keylen(_a) \
  gcry_cipher_get_algo_keylen(             \
      (cipher_algo_t)map_cipher_openpgp_to_gcry((cipher_algo_t)(_a)))
...
void write_status_strings(int no, const char *text, ...) GPGRT_ATTR_SENTINEL(0);
void write_status_buffer(int no, const char *buffer, size_t len, int wrap);
void write_status_text_and_buffer(int no, const char *text, const char *buffer,
                                  size_t len, int wrap);
...
int cpr_get_answer_okay_cancel(const char *keyword, const char *prompt,
                               int def_answer);
...
/*-- delkey.c --*/
gpg_error_t delete_keys(ctrl_t ctrl, const std::vector<std::string> &names,
...
int open_outfile(int inp_fd, const char *iname, int mode, int restrictedperm,
                 iobuf_t *a);
...
gcry_mpi_t encode_md_value(PKT_public_key *pk, gcry_md_hd_t md, int hash_algo);
...
int gen_standard_revoke(ctrl_t ctrl, PKT_public_key *psk,
                        const char *cache_nonce);
int gen_revoke(ctrl_t ctrl, const char *uname);
int gen_desig_revoke(
    ctrl_t ctrl, const char *uname,
    const std::vector<std::pair<std::string, unsigned int>> &locusr);
int revocation_reason_build_cb(PKT_signature *sig, void *opaque);
struct revocation_reason_info *ask_revocation_reason(int key_rev, int cert_rev,
                                                     int hint);
