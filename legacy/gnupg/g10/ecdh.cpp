...
gcry_mpi_t pk_ecdh_default_params(unsigned int qbits) {
  ...
  for (i = 0; i < DIM(kek_params_table); i++) {
    if (kek_params_table[i].qbits >= qbits || i + 1 == DIM(kek_params_table)) {
...
gpg_error_t pk_ecdh_encrypt_with_shared_point(
  ...
  *r_result = NULL;
  ...
  if (DBG_CRYPTO) log_printhex("ecdh KDF params:", kek_params, kek_params_size);
  ...
  /* And, finally, aeswrap with key secret_x.  */
  {
    ...
    if (is_encrypt) {
      ...
      /* Write data MPI into the end of data_buf. data_buf is size
         aeswrap data.  */
      err = gcry_mpi_print(GCRYMPI_FMT_USG, in, data_buf_size, &nbytes,
                           data /*in*/);
      if (err) {
      ...
      *r_result = result;
      ...
      if (DBG_CRYPTO) log_printhex("ecdh decrypted to :", in, data_buf_size);
...
static gcry_mpi_t gen_k(unsigned nbits) {
  gcry_mpi_t k;
  ...
  if (DBG_CRYPTO) {
    ...
    if (gcry_mpi_aprint(GCRYMPI_FMT_HEX, &buffer, NULL, k)) BUG();
...
int pk_ecdh_decrypt(gcry_mpi_t *result, const byte sk_fp[MAX_FINGERPRINT_LEN],
  ...
  if (!data) return GPG_ERR_BAD_MPI;
  return pk_ecdh_encrypt_with_shared_point(0 /*=decryption*/, shared, sk_fp,
                                           data /*encr data as an MPI*/, skey,
                                           result);
}
