...
static int import(ctrl_t ctrl, IOBUF inp, const char *fname,
                  struct import_stats_s *stats, unsigned char **fpr,
                  size_t *fpr_len, unsigned int options,
                  import_screener_t screener, void *screener_arg);
static int read_block(IOBUF a, int with_meta, PACKET **pending_pkt,
                      kbnode_t *ret_root, int *r_v3keys);
static void revocation_present(ctrl_t ctrl, kbnode_t keyblock);
static int import_one(ctrl_t ctrl, kbnode_t keyblock,
                      struct import_stats_s *stats, unsigned char **fpr,
                      size_t *fpr_len, unsigned int options, int from_sk,
                      int silent, import_screener_t screener,
                      void *screener_arg);
static int import_secret_one(ctrl_t ctrl, kbnode_t keyblock,
                             struct import_stats_s *stats, int batch,
                             unsigned int options, int for_migration,
                             import_screener_t screener, void *screener_arg);
static int import_revoke_cert(ctrl_t ctrl, kbnode_t node,
                              struct import_stats_s *stats);
static int chk_self_sigs(ctrl_t ctrl, kbnode_t keyblock, u32 *keyid,
                         int *non_self);
static int delete_inv_parts(ctrl_t ctrl, kbnode_t keyblock, u32 *keyid,
                            unsigned int options);
static int any_uid_left(kbnode_t keyblock);
static int merge_blocks(ctrl_t ctrl, kbnode_t keyblock_orig, kbnode_t keyblock,
...
static void release_import_filter(import_filter_t filt) {
...
static void cleanup_import_globals(void) {
...
int parse_import_options(char *str, unsigned int *options, int noisy) {
  struct parse_options import_opts[] = {
      ...
      {"import-show", IMPORT_SHOW, NULL, N_("show key during import")},
      ...
      {"repair-keys", IMPORT_REPAIR_KEYS, NULL, N_("repair keys on import")},
  ...
  if (rc && (*options & IMPORT_RESTORE)) {
    ...
    *options &= ~(IMPORT_MINIMAL | IMPORT_CLEAN | IMPORT_REPAIR_PKS_SUBKEY_BUG |
                  IMPORT_MERGE_ONLY);
  }
  return rc;
}
...
void import_release_stats_handle(import_stats_t p) { xfree(p); }
  ...
  *r_keyblock = NULL;
  ...
  if (err) {
    log_error(_("can't open '%s': %s\n"),
              ...
              gpg_strerror(err));
    if (err == GPG_ERR_ENOENT) err = GPG_ERR_NO_PUBKEY;
    goto leave;
  }
  ...
  while (!(err = read_block(inp, 0, &pending_pkt, &keyblock, &v3keys))) {
    if (keyblock->pkt->pkttype == PKT_PUBLIC_KEY) break;
    log_info(_("skipping block of type %d\n"), keyblock->pkt->pkttype);
    release_kbnode(keyblock);
    keyblock = NULL;
  }
  if (err) {
    if (err != GPG_ERR_INV_KEYRING)
 ...
 */
static int import_keys_internal(ctrl_t ctrl, iobuf_t inp, char **fnames,
                                int nnames, import_stats_t stats_handle,
                                unsigned char **fpr, size_t *fpr_len,
                                unsigned int options,
                                import_screener_t screener,
                                void *screener_arg) {
  int i;
  int rc = 0;
  struct import_stats_s *stats = stats_handle;

  if (!stats) stats = import_new_stats_handle();

  if (inp) {
    rc = import(ctrl, inp, "[stream]", stats, fpr, fpr_len, options, screener,
                screener_arg);
  } else {
    if (!fnames && !nnames)
    ...
    for (i = 0; i < nnames; i++) {
      ...
      if (!inp2)
        ...
        rc = import(ctrl, inp2, fname, stats, fpr, fpr_len, options, screener,
                    screener_arg);
        iobuf_close(inp2);
  ...
  if (!(options & IMPORT_FAST)) check_or_update_trustdb(ctrl);
...
void import_keys(ctrl_t ctrl, char **fnames, int nnames,
  ...
  import_keys_internal(ctrl, NULL, fnames, nnames, stats_handle, NULL, NULL,
                       options, NULL, NULL);
}

int import_keys_es_stream(ctrl_t ctrl, estream_t fp,
                          import_stats_t stats_handle, unsigned char **fpr,
                          size_t *fpr_len, unsigned int options,
                          import_screener_t screener, void *screener_arg) {
  int rc;
  iobuf_t inp;
  ...
  rc = import_keys_internal(ctrl, inp, NULL, 0, stats_handle, fpr, fpr_len,
                            options, screener, screener_arg);
...
static int import(ctrl_t ctrl, IOBUF inp, const char *fname,
  ...
  while (!(rc = read_block(inp, !!(options & IMPORT_RESTORE), &pending_pkt,
                           &keyblock, &v3keys))) {
    stats->v3keys += v3keys;
    if (keyblock->pkt->pkttype == PKT_PUBLIC_KEY)
      rc = import_one(ctrl, keyblock, stats, fpr, fpr_len, options, 0, 0,
                      screener, screener_arg);
    else if (keyblock->pkt->pkttype == PKT_SECRET_KEY)
      rc = import_secret_one(ctrl, keyblock, stats, opt.batch, options, 0,
                             screener, screener_arg);
...
gpg_error_t import_old_secring(ctrl_t ctrl, const char *fname) {
  ...
  while (!(err = read_block(inp, 0, &pending_pkt, &keyblock, &v3keys))) {
    if (keyblock->pkt->pkttype == PKT_SECRET_KEY)
      err = import_secret_one(ctrl, keyblock, stats, 1, 0, 1, NULL, NULL);
    release_kbnode(keyblock);
    if (err) break;
  }
  import_release_stats_handle(stats);
  if (err == -1)
    err = 0;
  else if (err && err != GPG_ERR_INV_KEYRING)
    log_error(_("error reading '%s': %s\n"), fname, gpg_strerror(err));
  else if (err)
    log_error("import from '%s' failed: %s\n", fname, gpg_strerror(err));
...
void import_print_stats(import_stats_t stats) {
  if (!opt.quiet) {
  ...
  if (is_status_enabled()) {
    ...
    snprintf(buf, sizeof buf,
             ...
             stats->count + stats->v3keys, stats->no_user_id, stats->imported,
             stats->unchanged, stats->n_uids, stats->n_subk, stats->n_sigs,
             stats->n_revoc, stats->secret_read, stats->secret_imported,
             stats->secret_dups, stats->skipped_new_keys, stats->not_imported,
             stats->v3keys);
...
static int valid_keyblock_packet(int pkttype) {
  switch (pkttype) {
    case PKT_PUBLIC_KEY:
...
static int read_block(IOBUF a, int with_meta, PACKET **pending_pkt,
  ...
  *r_v3keys = 0;
  ...
  if (!with_meta) parsectx.skip_meta = 1;
  in_v3key = 0;
  while ((rc = parse_packet(&parsectx, pkt)) != -1) {
    if (rc && (rc == GPG_ERR_LEGACY_KEY && (pkt->pkttype == PKT_PUBLIC_KEY ||
    ...
    switch (pkt->pkttype) {
      case PKT_COMPRESSED:
        if (check_compress_algo(pkt->pkt.compressed->algorithm)) {
          ...
          compress_filter_context_t *cfx =
              (compress_filter_context_t *)xmalloc_clear(sizeof *cfx);
      ...
      case PKT_RING_TRUST:
        ...
        if ((opt.import_options & IMPORT_RESTORE)) goto x_default;
        free_packet(pkt, &parsectx);
        init_packet(pkt);
        break;
...
static int fix_pks_corruption(ctrl_t ctrl, kbnode_t keyblock) {
  int changed = 0;
  int keycount = 0;
  kbnode_t node;
  kbnode_t last = NULL;
  ...
  for (node = keyblock; node; last = node, node = node->next) {
    if (node->pkt->pkttype == PKT_PUBLIC_SUBKEY) {
      keycount++;
      if (!sknode) sknode = node;
    } else if (node->pkt->pkttype == PKT_SIGNATURE &&
               node->pkt->pkt.signature->sig_class == 0x18 && keycount >= 2 &&
               !node->next) {
...
static int fix_bad_direct_key_sigs(ctrl_t ctrl, kbnode_t keyblock, u32 *keyid) {
  gpg_error_t err;
  kbnode_t node;
  int count = 0;

  for (node = keyblock->next; node; node = node->next) {
    if (node->pkt->pkttype == PKT_USER_ID) break;
    if (node->pkt->pkttype == PKT_SIGNATURE &&
        IS_KEY_SIG(node->pkt->pkt.signature)) {
      err = check_key_signature(ctrl, keyblock, node, NULL);
      if (err && err != GPG_ERR_PUBKEY_ALGO) {
...
static void print_import_ok(PKT_public_key *pk, unsigned int reason) {
  ...
  s = array;
  for (i = 0; i < n; i++, s++, p += 2) sprintf(p, "%02X", *s);

  write_status_text(STATUS_IMPORT_OK, buf);
}

static void print_import_check(PKT_public_key *pk, PKT_user_id *id) {
  char *buf;
  byte fpr[24];
  u32 keyid[2];
  size_t i, n;
  ...
  for (i = 0; i < n; i++, pos += 2) sprintf(buf + pos, "%02X", fpr[i]);
  strcat(buf, " ");
  strcat(buf, id->name);
...
static void check_prefs_warning(PKT_public_key *pk) {
  log_info(_("WARNING: key %s contains preferences for unavailable\n"
             "algorithms on these user IDs:\n"),
           keystr_from_pk(pk));
}

static void check_prefs(ctrl_t ctrl, kbnode_t keyblock) {
  kbnode_t node;
  PKT_public_key *pk;
  int problem = 0;
  ...
  if (problem) {
    log_info(
        _("it is strongly suggested that you update"
          " your preferences and\n"));
    log_info(
        _("re-distribute this key to avoid potential algorithm"
          " mismatch problems\n"));
    ...
    if (!opt.batch) {
      ...
      for (i = 0; i < fprlen; i++, p++) sprintf(username + 2 * i, "%02X", *p);
 ...
 */
static void apply_keep_uid_filter(ctrl_t ctrl, kbnode_t keyblock,
  ...
  for (node = keyblock->next; node; node = node->next) {
    if (node->pkt->pkttype == PKT_USER_ID) {
      ...
      if (!recsel_select(selector, impex_filter_getval, &parm)) {
        ...
        delete_kbnode(node);
        for (; node->next && node->next->pkt->pkttype != PKT_USER_ID &&
 ...
 * KEYBLOCK must not have any blocks marked as deleted.
 */
static void apply_drop_sig_filter(ctrl_t ctrl, kbnode_t keyblock,
                                  recsel_expr_t selector) {
  kbnode_t node;
  int active = 0;
  u32 main_keyid[2];
  ...
  for (node = keyblock->next; node; node = node->next) {
    ...
    if (node->pkt->pkttype == PKT_USER_ID ||
      ...
      active = 1;
    if (!active) continue;
    if (node->pkt->pkttype != PKT_SIGNATURE) continue;
...
static int import_one(ctrl_t ctrl, kbnode_t keyblock,
  ...
  for (an = fpr2len; an < MAX_FINGERPRINT_LEN; an++) fpr2[an] = 0;
  keyid_from_pk(pk, keyid);
  uidnode = find_next_kbnode(keyblock, PKT_USER_ID);

  if (opt.verbose && !opt.interactive && !silent) {
    log_info("pub  %s/%s %s  ", pubkey_string(pk, pkstrbuf, sizeof pkstrbuf),
  ...
  if (!uidnode) {
    if (!silent) log_error(_("key %s: no user ID\n"), keystr_from_pk(pk));
    return 0;
  }
  ...
  if (opt.interactive && !silent) {
    if (is_status_enabled()) print_import_check(pk, uidnode->pkt->pkt.user_id);
    merge_keys_and_selfsig(ctrl, keyblock);
    tty_printf("\n");
  ...
  if (options & IMPORT_CLEAN)
    clean_key(ctrl, keyblock, opt.verbose, (options & IMPORT_MINIMAL), NULL,
              NULL);

  clear_kbnode_flags(keyblock);

  if ((options & IMPORT_REPAIR_PKS_SUBKEY_BUG) &&
      fix_pks_corruption(ctrl, keyblock) && opt.verbose)
  ...
  } else if (rc) /* Insert this key. */
  {
    ...
    if (rc)
      ...
      if (non_self) revalidation_mark(ctrl);
    }
    keydb_release(hd);
  ...
  } else /* merge */
  {
    ...
    if (n_sigs_cleaned) commit_kbnode(&keyblock_orig);

    /* and try to merge the block */
    clear_kbnode_flags(keyblock_orig);
    clear_kbnode_flags(keyblock);
    n_uids = n_sigs = n_subk = n_uids_cleaned = 0;
    rc = merge_blocks(ctrl, keyblock_orig, keyblock, keyid, &n_uids, &n_sigs,
                      &n_subk);
    if (rc) {
      keydb_release(hd);
      goto leave;
    ...
    if (n_uids || n_sigs || n_subk || n_sigs_cleaned || n_uids_cleaned) {
      ...
      if (is_status_enabled() && !silent)
        print_import_ok(
            pk, ((n_uids ? 2 : 0) | (n_sigs ? 4 : 0) | (n_subk ? 8 : 0)));
    } else {
      same_key = 1;
      if (is_status_enabled()) print_import_ok(pk, 0);
...
gpg_error_t transfer_secret_keys(ctrl_t ctrl, struct import_stats_s *stats,
  ...
  while ((node = walk_kbnode(sec_keyblock, &ctx, 0))) {
    ...
    if (ski->is_protected) {
      ...
      err = gcry_sexp_build(
          &prot, NULL, " (protection %s %s %b %d %s %b %s)\n",
          ski->sha1chk ? "sha1" : "sum",
          openpgp_cipher_algo_name((cipher_algo_t)(ski->algo)),
          ski->ivlen ? (int)ski->ivlen : 1,
          ski->ivlen ? ski->iv : (const unsigned char *)"X", ski->s2k.mode,
          openpgp_md_algo_name(ski->s2k.hash_algo), (int)sizeof(ski->s2k.salt),
          ski->s2k.salt, countbuf);
    } else
    ...
    if (!err)
      err = gcry_sexp_build(
          ...
          " (csum %d)\n"
          " %S)\n",
          pk->version, openpgp_pk_algo_name((pubkey_algo_t)(pk->pubkey_algo)),
          curve, skey, (int)(unsigned long)ski->csum, prot);
    ...
    /* Send the key to the agent.  */
    {
      ...
      err = agent_import_key(ctrl, desc, &cache_nonce, transferkey,
                             transferkeylen, batch, force);
      xfree(desc);
    }
    if (!err) {
      if (opt.verbose)
      ...
      if (stats) stats->secret_imported++;
    } else if (err == GPG_ERR_EEXIST) {
      if (opt.verbose)
        log_info(_("key %s: secret key already exists\n"),
                 keystr_from_pk_with_sub(main_pk, pk));
      err = 0;
      if (stats) stats->secret_dups++;
    } else {
      log_error(_("key %s: error sending to agent: %s\n"),
                keystr_from_pk_with_sub(main_pk, pk), gpg_strerror(err));
      if (err == GPG_ERR_CANCELED || err == GPG_ERR_FULLY_CANCELED)
...
static kbnode_t sec_to_pub_keyblock(kbnode_t sec_keyblock) {
...
static int import_secret_one(ctrl_t ctrl, kbnode_t keyblock,
  ...
  if (opt.verbose && !for_migration) {
    log_info("sec  %s/%s %s   ", pubkey_string(pk, pkstrbuf, sizeof pkstrbuf),
             keystr_from_pk(pk), datestr_from_pk(pk));
    if (uidnode)
      print_utf8_buffer(log_get_stream(), uidnode->pkt->pkt.user_id->name,
                        uidnode->pkt->pkt.user_id->len);
  ...
  if (!pub_keyblock)
    ...
    import_one(ctrl, pub_keyblock, stats, NULL, NULL, options, 1, for_migration,
               screener, screener_arg);
    ...
    if (stats->skipped_new_keys <= nr_prev) {
      ...
      if (!node)
        ...
        err = transfer_secret_keys(ctrl, &subkey_stats, keyblock, batch, 0);
        if (err == GPG_ERR_NOT_PROCESSED) {
        ...
        if (!err) {
          ...
          if (subkey_stats.secret_dups) stats->secret_dups += 1;

          if (is_status_enabled()) print_import_ok(pk, status);
...
static int import_revoke_cert(ctrl_t ctrl, kbnode_t node,
...

  {
    ...
    while (an < MAX_FINGERPRINT_LEN) afp[an++] = 0;
  ...
  if (get_ownertrust(ctrl, pk) == TRUST_ULTIMATE) clear_ownertrusts(ctrl, pk);
...
static int chk_self_sigs(ctrl_t ctrl, kbnode_t keyblock, u32 *keyid,
  ...
  for (n = keyblock; (n = find_next_kbnode(n, 0));) {
    ...
    if (n->pkt->pkttype != PKT_SIGNATURE) continue;
    ...
    if (!opt.no_sig_cache) check_key_signature(ctrl, keyblock, n, NULL);
...
static int delete_inv_parts(ctrl_t ctrl, kbnode_t keyblock, u32 *keyid,
  ...
  for (node = keyblock->next; node; node = node->next) {
    if (node->pkt->pkttype == PKT_USER_ID) {
      ...
      if ((node->flag & NODE_BAD_SELFSIG) ||
          !(node->flag & NODE_GOOD_SELFSIG)) {
        if (opt.verbose) log_info(_("key %s: skipped subkey\n"), keystr(keyid));
        ...
        subkey_seen = 1;
               ...
               openpgp_pk_test_algo(
                   (pubkey_algo_t)(node->pkt->pkt.signature->pubkey_algo)) &&
...
static int any_uid_left(kbnode_t keyblock) {
  ...
  for (node = keyblock->next; node; node = node->next)
    if (node->pkt->pkttype == PKT_USER_ID) return 1;
  return 0;
}
...
int collapse_uids(kbnode_t *keyblock) {
  ...
  for (uid1 = *keyblock; uid1; uid1 = uid1->next) {
    ...
    if (is_deleted_kbnode(uid1)) continue;

    if (uid1->pkt->pkttype != PKT_USER_ID) continue;

    for (uid2 = uid1->next; uid2; uid2 = uid2->next) {
      if (is_deleted_kbnode(uid2)) continue;

      if (uid2->pkt->pkttype != PKT_USER_ID) continue;

      if (cmp_user_ids(uid1->pkt->pkt.user_id, uid2->pkt->pkt.user_id) == 0) {
        ...
        for (last = uid2; last->next; last = last->next) {
          if (is_deleted_kbnode(last)) continue;
        ...
        for (sig1 = uid1->next; sig1; sig1 = sig1->next) {
          ...
          if (is_deleted_kbnode(sig1)) continue;
          ...
          if (sig1->pkt->pkttype != PKT_SIGNATURE) continue;

          for (sig2 = sig1->next, last = sig1; sig2;
               last = sig2, sig2 = sig2->next) {
            if (is_deleted_kbnode(sig2)) continue;
            ...
            if (sig2->pkt->pkttype != PKT_SIGNATURE) continue;
...
static void revocation_present(ctrl_t ctrl, kbnode_t keyblock) {
  ...
  for (onode = keyblock->next; onode; onode = onode->next) {
    ...
    if (onode->pkt->pkttype == PKT_USER_ID) break;
    ...
    if (onode->pkt->pkttype == PKT_SIGNATURE &&
      ...
      for (idx = 0; idx < sig->numrevkeys; idx++) {
        ...
        keyid_from_fingerprint(ctrl, sig->revkey[idx].fpr, MAX_FINGERPRINT_LEN,
                               keyid);

        for (inode = keyblock->next; inode; inode = inode->next) {
          /* If we reach user IDs, we're done. */
          if (inode->pkt->pkttype == PKT_USER_ID) break;
          ...
          if (inode->pkt->pkttype == PKT_SIGNATURE &&
            ...
            rc = get_pubkey_byfprint_fast(NULL, sig->revkey[idx].fpr,
                                          MAX_FINGERPRINT_LEN);
            if (rc == GPG_ERR_NO_PUBKEY || rc == GPG_ERR_UNUSABLE_PUBKEY) {
              ...
              if ((opt.keyserver_options.options &
                ...
                keyserver_import_fprint(ctrl, sig->revkey[idx].fpr,
                                        MAX_FINGERPRINT_LEN, opt.keyserver, 0);
...
static int append_uid(kbnode_t keyblock, kbnode_t node, int *n_sigs) {
  ...
  while (node) {
    ...
    if (n->pkt->pkttype == PKT_SIGNATURE) ++*n_sigs;

    node = node->next;
    if (node && node->pkt->pkttype != PKT_SIGNATURE) break;
  }
