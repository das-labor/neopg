...
int overwrite_filep(const char *fname) {
  ...
  if (access(fname, F_OK)) return 1; /* Does not exist.  */

  if (!compare_filenames(fname, NAME_OF_DEV_NULL))
    return 1; /* Does not do any harm.  */

  if (opt.answer_yes) return 1;
  if (opt.answer_no || opt.batch) return 0; /* Do not overwrite.  */

  tty_printf(_("File '%s' exists. "), fname);
  if (cpr_enabled()) tty_printf("\n");
  if (cpr_get_answer_is_yes("openfile.overwrite.okay", _("Overwrite? (y/N) ")))
    return 1;
  return 0;
}
...
char *make_outfile_name(const char *iname) {
  ...
  if (iobuf_is_pipe_filename(iname)) return xstrdup("-");
...
int open_outfile(int inp_fd, const char *iname, int mode, int restrictedperm,
  ...
  *a = NULL;
  ...
  if (*a) iobuf_ioctl(*a, IOBUF_IOCTL_NO_CACHE, 1, NULL);
...
char *get_matching_datafile(const char *sigfilename) {
  ...
  if (iobuf_is_pipe_filename(sigfilename)) return NULL;
...
iobuf_t open_sigfile(const char *sigfilename, progress_filter_context_t *pfx) {
  ...
  if (buf) {
    ...
    if (a) log_info(_("assuming signed data in '%s'\n"), buf);
    if (a && pfx) handle_progress(pfx, a, buf);
    xfree(buf);
  }
