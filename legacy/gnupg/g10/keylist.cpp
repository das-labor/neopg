...
static void list_one(ctrl_t ctrl, const std::vector<std::string> &names,
                     int secret, int mark_secret);
...
static void list_keyblock(ctrl_t ctrl, kbnode_t keyblock, int secret,
...
static void keylist_context_release(struct keylist_context *listctx) {
...
/* List the keys.  If list is NULL, all available keys are listed.
   ...
   key.  */
void public_key_list(ctrl_t ctrl, const std::vector<std::string> &list,
                     int locate_mode) {
  ...
  if (opt.with_colons) {
    ...
    read_trust_options(ctrl, &trust_model, &created, &nextcheck, &marginals,
                       &completes, &cert_depth, &min_cert_level);

    es_fprintf(es_stdout, "tru:");

    if (nextcheck && nextcheck <= make_timestamp()) es_fprintf(es_stdout, "o");
    if (trust_model != opt.trust_model) es_fprintf(es_stdout, "t");
    if (opt.trust_model == TM_PGP || opt.trust_model == TM_CLASSIC) {
      if (marginals != opt.marginals_needed) es_fprintf(es_stdout, "m");
      if (completes != opt.completes_needed) es_fprintf(es_stdout, "c");
      if (cert_depth != opt.max_cert_depth) es_fprintf(es_stdout, "d");
      if (min_cert_level != opt.min_cert_level) es_fprintf(es_stdout, "l");
    }
...
void print_card_key_info(estream_t fp, kbnode_t keyblock) {
  kbnode_t node;
  char *hexgrip;
  char *serialno;
  int s2k_char;
  ...
  for (node = keyblock; node; node = node->next) {
    if (node->pkt->pkttype == PKT_PUBLIC_KEY ||
      ...
      tty_fprintf(fp, "%s%c  %s/%s  %n",
                  ...
                  s2k_char, pubkey_string(pk, pkstrbuf, sizeof pkstrbuf),
                  keystr_from_pk(pk), &indent);
...
static void status_one_subpacket(sigsubpkttype_t type, size_t len, int flags,
                                 const byte *buf) {
  char status[40];
  ...
  snprintf(status, sizeof status, "%d %u %u ", type, flags, (unsigned int)len);

  write_status_text_and_buffer(STATUS_SIG_SUBPACKET, status,
...
void show_policy_url(PKT_signature *sig, int indent, int mode) {
  ...
  while (
      (p = enum_sig_subpkt(sig->hashed, SIGSUBPKT_POLICY, &len, &seq, &crit))) {
    if (mode != 2) {
      const char *str;
...
void show_notation(PKT_signature *sig, int indent, int mode, int which) {
  ...
  for (nd = notations; nd; nd = nd->next) {
    if (mode != 2) {
      ...
      if ((which & 1 && !has_at) || (which & 2 && has_at)) {
        ...
        if (mode > 0)
          ...
          tty_fprintf(fp, "%s", str);
        /* This is all UTF8 */
        tty_print_utf8_string2(fp, (const unsigned char *)(nd->name),
                               strlen(nd->name), 0);
        tty_fprintf(fp, "=");
        tty_print_utf8_string2(fp, (const unsigned char *)(nd->value),
                               strlen(nd->value), 0);
    ...
    if (mode > 0) {
      write_status_buffer(STATUS_NOTATION_NAME, nd->name, strlen(nd->name), 0);
      if (nd->flags.critical || nd->flags.human)
        write_status_text(STATUS_NOTATION_FLAGS,
                          nd->flags.critical && nd->flags.human
                              ? "1 1"
                              : nd->flags.critical ? "1 0" : "0 1");
      write_status_buffer(STATUS_NOTATION_DATA, nd->value, strlen(nd->value),
                          50);
    }
  }
...
static void print_signature_stats(struct keylist_context *s) {
  if (!s->check_sigs) return; /* Signature checking was not requested.  */
...
/* List all keys.  If SECRET is true only secret keys are listed.  If
   ...
   listing.  */
static void list_all(ctrl_t ctrl, int secret, int mark_secret) {
  KEYDB_HANDLE hd;
  ...
  if (opt.check_sigs) listctx.check_sigs = 1;
  ...
  do {
    ...
    if (rc) {
      if (rc == GPG_ERR_LEGACY_KEY) continue; /* Skip legacy keys.  */
    ...
    if (secret && !any_secret)
      ...
      if (!opt.with_colons) {
        ...
        if (lastresname != resname) {
          ...
          for (i = strlen(resname); i; i--) es_putc('-', es_stdout);
          es_putc('\n', es_stdout);
          lastresname = resname;
        }
      }
  ...
  if (opt.check_sigs && !opt.with_colons) print_signature_stats(&listctx);
...
static void list_one(ctrl_t ctrl, const std::vector<std::string> &names,
  ...
  memset(&listctx, 0, sizeof(listctx));
  if (!secret && opt.check_sigs) listctx.check_sigs = 1;
  ...
  do {
    ...
    list_keyblock(ctrl, keyblock, secret, mark_secret, opt.fingerprint,
                  &listctx);
    release_kbnode(keyblock);
...
static void print_key_data(PKT_public_key *pk) {
...
static void print_capabilities(ctrl_t ctrl, PKT_public_key *pk,
                               KBNODE keyblock) {
  unsigned int use = pk->pubkey_usage;
  int c_printed = 0;

  if (use & PUBKEY_USAGE_ENC) es_putc('e', es_stdout);
  ...
  if ((use & PUBKEY_USAGE_CERT) && !c_printed) es_putc('c', es_stdout);

  if ((use & PUBKEY_USAGE_AUTH)) es_putc('a', es_stdout);

  if ((use & PUBKEY_USAGE_UNKNOWN)) es_putc('?', es_stdout);
  ...
  if (keyblock) {
    ...
    for (k = keyblock; k; k = k->next) {
      if (k->pkt->pkttype == PKT_PUBLIC_KEY ||
        ...
        if (pk->flags.primary) disabled = pk_is_disabled(pk);

        if (pk->flags.valid && !pk->flags.revoked && !pk->has_expired) {
          if (pk->pubkey_usage & PUBKEY_USAGE_ENC) enc = 1;
          if (pk->pubkey_usage & PUBKEY_USAGE_SIG) {
            sign = 1;
            if (pk->flags.primary) cert = 1;
          }
          if (pk->pubkey_usage & PUBKEY_USAGE_CERT) cert = 1;
          if ((pk->pubkey_usage & PUBKEY_USAGE_AUTH)) auth = 1;
        }
      }
    }
    if (enc) es_putc('E', es_stdout);
    if (sign) es_putc('S', es_stdout);
    if (cert) es_putc('C', es_stdout);
    if (auth) es_putc('A', es_stdout);
    if (disabled) es_putc('D', es_stdout);
  }
...
/* FLAGS: 0x01 hashed
          0x02 critical  */
static void print_one_subpacket(sigsubpkttype_t type, size_t len, int flags,
                                const byte *buf) {
  size_t i;
...
void print_subpackets_colon(PKT_signature *sig) {
  ...
  for (i = opt.show_subpackets; *i; i++) {
    ...
    while ((p = enum_sig_subpkt(sig->hashed, (sigsubpkttype_t)(*i), &len, &seq,
                                &crit)))
      print_one_subpacket((sigsubpkttype_t)(*i), len, 0x01 | (crit ? 0x02 : 0),
                          p);

    seq = 0;

    while ((p = enum_sig_subpkt(sig->unhashed, (sigsubpkttype_t)(*i), &len,
                                &seq, &crit)))
      print_one_subpacket((sigsubpkttype_t)(*i), len, 0x00 | (crit ? 0x02 : 0),
                          p);
  }
}
...
void dump_attribs(const PKT_user_id *uid, PKT_public_key *pk) {
  ...
  for (i = 0; i < uid->numattribs; i++) {
    if (is_status_enabled()) {
      ...
      p = array;
      for (j = 0; j < n; j++, p++) sprintf(buf + 2 * j, "%02X", *p);

      sprintf(
          buf + strlen(buf), " %lu %u %u %u %lu %lu %u",
...
static void list_keyblock_print(ctrl_t ctrl, kbnode_t keyblock, int secret,
  ...
  if (secret || opt.with_keygrip) {
    rc = hexkeygrip_from_pk(pk, &hexgrip);
    if (rc) log_error("error computing a keygrip: %s\n", gpg_strerror(rc));
  }
  ...
  if (!listctx->no_validity) check_trustdb_stale(ctrl);

  /* Print the "pub" line and in KF_NONE mode the fingerprint.  */
  print_key_line(ctrl, es_stdout, pk, secret);

  if (fpr) print_fingerprint(ctrl, NULL, pk, 0);

  if (opt.with_keygrip && hexgrip)
    es_fprintf(es_stdout, "      Keygrip = %s\n", hexgrip);

  if (serialno) print_card_serialno(serialno);

  if (opt.with_key_data) print_key_data(pk);
  ...
  for (kbctx = NULL; (node = walk_kbnode(keyblock, &kbctx, 0));) {
    if (node->pkt->pkttype == PKT_USER_ID) {
      ...
      if (attrib_fp && uid->attrib_data != NULL) dump_attribs(uid, pk);
      ...
      if ((uid->flags.revoked || uid->flags.expired) ||
        ...
        indent = ((kl + 11) - atoi(uid_trust_string_fixed(ctrl, NULL, NULL)));
        if (indent < 0 || indent > 40) indent = 0;
      ...
      if (opt.with_keygrip && hexgrip)
        es_fprintf(es_stdout, "      Keygrip = %s\n", hexgrip);
      if (opt.with_key_data) print_key_data(pk2);
      ...
      es_fprintf(
          es_stdout, "%c%c %c%c%c%c%c%c %s %s", sigrc,
          (sig->sig_class - 0x10 > 0 && sig->sig_class - 0x10 < 4)
              ? '0' + sig->sig_class - 0x10
              : ' ',
          sig->flags.exportable ? ' ' : 'L', sig->flags.revocable ? ' ' : 'R',
          sig->flags.policy_url ? 'P' : ' ', sig->flags.notation ? 'N' : ' ',
          sig->flags.expired ? 'X' : ' ',
          (sig->trust_depth > 9)
              ? 'T'
...
void print_revokers(estream_t fp, PKT_public_key *pk) {
  ...
  if (!pk->revkey && pk->numrevkeys)
    ...
    for (i = 0; i < pk->numrevkeys; i++) {
      ...
      p = pk->revkey[i].fpr;
      for (j = 0; j < 20; j++, p++) es_fprintf(fp, "%02X", *p);
      es_fprintf(fp, ":%02x%s:\n", pk->revkey[i].klasse,
...
static void print_compliance_flags(PKT_public_key *pk, unsigned int keylength,
                                   const char *curvename) {
  int any = 0;

  if (!keylength) keylength = nbits_from_pk(pk);

  if (pk->version == 5) {
    es_fputs(gnupg_status_compliance_flag(CO_GNUPG), es_stdout);
    any++;
  }
  if (gnupg_pk_is_compliant(CO_DE_VS, pk->pubkey_algo, pk->pkey, keylength,
                            curvename)) {
...
static void list_keyblock_colon(ctrl_t ctrl, kbnode_t keyblock, int secret,
                                int has_secret) {
  int rc;
  KBNODE kbctx;
  KBNODE node;
  ...
  if (trustletter_print) es_putc(trustletter_print, es_stdout);
  es_fprintf(es_stdout, ":%u:%d:%08lX%08lX:%s:%s::", keylength, pk->pubkey_algo,
             (unsigned long)keyid[0], (unsigned long)keyid[1],
             colon_datestr_from_pk(pk), colon_strtime(pk->expiredate));

  if (ownertrust_print) es_putc(ownertrust_print, es_stdout);
  es_putc(':', es_stdout);
  ...
  if (hexgrip) es_fprintf(es_stdout, "grp:::::::::%s:\n", hexgrip);
  if (opt.with_key_data) print_key_data(pk);
  ...
  for (kbctx = NULL; (node = walk_kbnode(keyblock, &kbctx, 0));) {
    if (node->pkt->pkttype == PKT_USER_ID) {
      ...
      if (uid_validity) es_putc(uid_validity, es_stdout);
      es_fputs("::::", es_stdout);
      ...
      for (i = 0; i < 20; i++) es_fprintf(es_stdout, "%02X", uid->namehash[i]);
      ...
      xfree(serialno);
      serialno = NULL;
      if (need_hexgrip || secret || has_secret || opt.with_keygrip ||
          opt.with_key_data) {
      ...
      if (!pk2->flags.valid)
        ...
        if (trustletter) es_fprintf(es_stdout, "%c", trustletter);
      }
      keylength = nbits_from_pk(pk2);
      es_fprintf(es_stdout, ":%u:%d:%08lX%08lX:%s:%s:::::", keylength,
                 pk2->pubkey_algo, (unsigned long)keyid2[0],
                 (unsigned long)keyid2[1], colon_datestr_from_pk(pk2),
      ...
      if (opt.check_sigs) {
        ...
        rc = check_key_signature2(ctrl, keyblock, node, NULL, signer_pk, NULL,
                                  NULL, NULL);
        switch (rc) {
          case 0:
            sigrc = '!';
      ...
      if (sig->trust_regexp)
        es_write_sanitized(es_stdout, sig->trust_regexp,
                           strlen((const char *)(sig->trust_regexp)), ":",
                           NULL);
      es_fprintf(es_stdout, ":");
      ...
      if (opt.no_sig_cache && opt.check_sigs && fprokay) {
        for (i = 0; i < fplen; i++) es_fprintf(es_stdout, "%02X", fparray[i]);
      }
 ...
 * function.  */
static void do_reorder_keyblock(KBNODE keyblock, int attr) {
  ...
  if (!primary) return; /* No primary key flag found (should not happen).  */
  ...
  if (node == primary) return; /* Already the first one.  */
...
static void list_keyblock(ctrl_t ctrl, KBNODE keyblock, int secret,
                          int has_secret, int fpr,
                          struct keylist_context *listctx) {
...
static void print_icao_hexdigit(estream_t fp, int c) {
 ...
 */
void print_fingerprint(ctrl_t ctrl, estream_t override_fp, PKT_public_key *pk,
                       int mode) {
  ...
  if (!opt.fingerprint && !opt.with_fingerprint && opt.with_subkey_fingerprint)
    compact = 1;
  ...
  if (!primary && (mode == 1 || mode == 2)) {
    PKT_public_key *primary_pk =
        (PKT_public_key *)xmalloc_clear(sizeof(*primary_pk));
...
static void print_card_serialno(const char *serialno) {
  if (!serialno) return;
  if (opt.with_colons) return; /* Handled elsewhere. */
...
void print_key_line(ctrl_t ctrl, estream_t fp, PKT_public_key *pk, int secret) {
  ...
  tty_fprintf(fp, "%s%c  %s",
              ...
              secret == 2 ? '#' : secret == 3 ? '>' : ' ',
              pubkey_string(pk, pkstrbuf, sizeof pkstrbuf));
  if (opt.keyid_format != KF_NONE) tty_fprintf(fp, "/%s", keystr_from_pk(pk));
  ...
  if (pk->flags.primary && !opt.fingerprint && !opt.with_fingerprint)
...
void set_attrib_fd(int fd) {
  ...
  if (fd != -1 && last_fd == fd) return;

  /* Fixme: Do we need to check for the log stream here?  */
  if (attrib_fp && attrib_fp != log_get_stream()) es_fclose(attrib_fp);
  attrib_fp = NULL;
  if (fd == -1) return;
