...
void make_session_key(DEK *dek) {
  ...
  rc = openpgp_cipher_open(
      &chd, dek->algo, GCRY_CIPHER_MODE_CFB,
      (GCRY_CIPHER_SECURE | (dek->algo >= 100 ? 0 : GCRY_CIPHER_ENABLE_SYNC)));
  if (rc) {
 ...
 * gcry_mpi_release().  */
gcry_mpi_t encode_session_key(int openpgp_pk_algo, DEK *dek,
                              unsigned int nbits) {
  ...
  csum = 0;
  for (p = dek->key, i = 0; i < dek->keylen; i++) csum += *p++;
  ...
  if (openpgp_pk_algo == PUBKEY_ALGO_ECDH) {
    ...
    if (DBG_CRYPTO)
      log_debug(
          ...
          (int)nframe, frame[0], frame[1], frame[2], frame[nframe - 3],
...
static gcry_mpi_t do_encode_md(gcry_md_hd_t md, int algo, size_t len,
                               unsigned nbits, const byte *asn, size_t asnlen) {
  size_t nframe = (nbits + 7) / 8;
  byte *frame;
  int i, n;
  gcry_mpi_t a;
...
gcry_mpi_t encode_md_value(PKT_public_key *pk, gcry_md_hd_t md, int hash_algo) {
  ...
  if (pk->pubkey_algo == PUBKEY_ALGO_EDDSA) {
    ...
    /* pkey[1] is Q for ECDSA, which is an uncompressed point,
       i.e.  04 <x> <y>  */
    if (pk->pubkey_algo == PUBKEY_ALGO_ECDSA) qbits = ecdsa_qbits_from_Q(qbits);
    ...
    /* ECDSA 521 is special has it is larger than the largest hash
       ...
       processing to 512.  */
    if (pk->pubkey_algo == PUBKEY_ALGO_ECDSA && qbits > 512) qbits = 512;
    ...
    if (mdlen < qbits / 8) {
      log_error(_("%s key %s requires a %zu bit or larger hash "
                  "(hash is %s)\n"),
                openpgp_pk_algo_name((pubkey_algo_t)(pk->pubkey_algo)),
                keystr_from_pk(pk), qbits, gcry_md_algo_name(hash_algo));
      return NULL;
    }

    /* Note that we do the truncation by passing QBITS/8 as length to
       mpi_scan.  */
    if (gcry_mpi_scan(&frame, GCRYMPI_FMT_USG, gcry_md_read(md, hash_algo),
                      qbits / 8, NULL))
      BUG();
  } else {
    gpg_error_t rc;
    ...
    if (!asn) return NULL;
    if (gcry_md_algo_info(hash_algo, GCRYCTL_GET_ASNOID, asn, &asnlen)) BUG();
