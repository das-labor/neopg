 ...
 * are ignored.
 */
static int trust_letter(unsigned int value) {
 ...
 */
static int get_ownertrust_with_min(ctrl_t ctrl, PKT_public_key *pk,
                                   int no_create) {
#ifdef NO_TRUST_MODELS
  (void)pk;
  return TRUST_UNKNOWN;
 ...
 */
void update_ownertrust(ctrl_t ctrl, PKT_public_key *pk,
...
leave:
  ...
  if (main_pk->flags.revoked) validity |= TRUST_FLAG_REVOKED;
  if (main_pk != pk && pk->flags.revoked) validity |= TRUST_FLAG_SUB_REVOKED;
  /* Note: expiration is a trust value and not a flag - don't know why
   * I initially designed it that way.  */
  if (main_pk->has_expired || pk->has_expired)
    validity =
        ((validity & (~TRUST_MASK | TRUST_FLAG_PENDING_CHECK)) | TRUST_EXPIRED);

  if (main_pk != pk && !kb) free_public_key(main_pk);
  return validity;
}
...
int get_validity_info(ctrl_t ctrl, kbnode_t kb, PKT_public_key *pk,
  ...
  if (!pk && kb) pk = kb->pkt->pkt.public_key;
  if (!pk) return '?'; /* Just in case a NULL PK is passed.  */

  trustlevel = get_validity(ctrl, kb, pk, uid, NULL, 0);
  if ((trustlevel & TRUST_FLAG_REVOKED)) return 'r';
...
const char *get_validity_string(ctrl_t ctrl, PKT_public_key *pk,
  ...
  if (!pk) return "err"; /* Just in case a NULL PK is passed.  */

  trustlevel = get_validity(ctrl, NULL, pk, uid, NULL, 0);
  if ((trustlevel & TRUST_FLAG_REVOKED)) return _("revoked");
...
void mark_usable_uid_certs(ctrl_t ctrl, kbnode_t keyblock, kbnode_t uidnode,
  ...
  for (node = uidnode->next; node; node = node->next) {
    ...
    if (main_kid && sig->keyid[0] == main_kid[0] &&
        sig->keyid[1] == main_kid[1])
    ...
    if ((rc = check_key_signature(ctrl, keyblock, node, NULL))) {
      /* we ignore anything that won't verify, but tag the
         no_pubkey case */
      if (rc == GPG_ERR_NO_PUBKEY) node->flag |= 1 << 12;
      continue;
    }
    node->flag |= 1 << 9;
  }
  ...
  for (node = uidnode->next; node; node = node->next) {
    ...
    if (node->pkt->pkttype == PKT_PUBLIC_SUBKEY ||
        node->pkt->pkttype == PKT_SECRET_SUBKEY)
      break;
    if (!(node->flag & (1 << 9))) continue; /* not a node to look at */
    if ((node->flag & (1 << 10)))
    ...
    for (n = uidnode->next; n; n = n->next) {
      if (n->pkt->pkttype == PKT_PUBLIC_SUBKEY ||
        ...
        break;
      if (!(n->flag & (1 << 9))) continue;
      if ((n->flag & (1 << 10)))
        continue; /* shortcut already processed signatures */
      sig = n->pkt->pkt.signature;
      if (kid[0] != sig->keyid[0] || kid[1] != sig->keyid[1]) continue;
    ...
    if (IS_UID_SIG(
            sig)) { /* this seems to be a usable one which is not revoked.
...
static int clean_sigs_from_uid(ctrl_t ctrl, kbnode_t keyblock, kbnode_t uidnode,
  ...
  for (node = uidnode->next; node && node->pkt->pkttype == PKT_SIGNATURE;
    ...
    if ((node->flag & (1 << 8)) && keep) continue;

    /* ... and usable revocations... */
    if ((node->flag & (1 << 11)) && keep) continue;
...
static int clean_uid_from_key(kbnode_t keyblock, kbnode_t uidnode, int noisy) {
  kbnode_t node;
...
void clean_one_uid(ctrl_t ctrl, kbnode_t keyblock, kbnode_t uidnode, int noisy,
  ...
  if (!uids_cleaned) uids_cleaned = &dummy;

  if (!sigs_cleaned) sigs_cleaned = &dummy;
  ...
  *uids_cleaned += clean_uid_from_key(keyblock, uidnode, noisy);
  if (!uidnode->pkt->pkt.user_id->flags.compacted)
    *sigs_cleaned +=
        clean_sigs_from_uid(ctrl, keyblock, uidnode, noisy, self_only);
}
...
void clean_key(ctrl_t ctrl, kbnode_t keyblock, int noisy, int self_only,
  ...
  for (node = keyblock->next; node &&
                              !(node->pkt->pkttype == PKT_PUBLIC_SUBKEY ||
                                node->pkt->pkttype == PKT_SECRET_SUBKEY);
       node = node->next) {
    if (node->pkt->pkttype == PKT_USER_ID)
      clean_one_uid(ctrl, keyblock, node, noisy, self_only, uids_cleaned,
                    sigs_cleaned);
  }
