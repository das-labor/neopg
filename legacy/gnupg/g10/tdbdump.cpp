...
void list_trustdb(ctrl_t ctrl, estream_t fp, const char *username) {
  ...
  if (1) {
    ...
    for (i = 9 + strlen(tdbio_get_dbname()); i > 0; i--) es_fputc('-', fp);
    es_putc('\n', fp);
...
void export_ownertrust(ctrl_t ctrl) {
  ...
  for (recnum = 0; !tdbio_read_record(recnum, &rec, 0); recnum++) {
    if (rec.rectype == RECTYPE_TRUST) {
      if (!rec.r.trust.ownertrust) continue;
      p = rec.r.trust.fingerprint;
      for (i = 0; i < 20; i++, p++) es_printf("%02X", *p);
...
void import_ownertrust(ctrl_t ctrl, const char *fname) {
  ...
  while (es_fgets(line, DIM(line) - 1, fp)) {
    ...
    if (!otrust) continue; /* no otrust defined - no need to update or insert */
    /* convert the ascii fingerprint to binary */
    for (p = line, fprlen = 0; fprlen < 20 && *p != ':'; p += 2)
      fpr[fprlen++] = HEXTOBIN(p[0]) * 16 + HEXTOBIN(p[1]);
    while (fprlen < 20) fpr[fprlen++] = 0;
    ...
    if (!rc) { /* found: update */
      ...
      if (!opt.quiet) log_info("inserting ownertrust of %u\n", otrust);
  ...
  if (!is_stdin) es_fclose(fp);

  if (any) {
    revalidation_mark(ctrl);
    rc = tdbio_sync();
    if (rc) log_error(_("trustdb: sync failed: %s\n"), gpg_strerror(rc));
  }
}
