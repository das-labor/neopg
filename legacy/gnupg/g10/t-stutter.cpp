...
static void log_hexdump(byte *buffer, int length) {
  int written = 0;
...
static char *hexstr(const byte *bytes) {
  static int i;
...
static byte *bufxor2(const byte *a, const byte *b) {
  static int i;
...
/* Decode the session key, which is in the format output by gpg
   --show-session-key.  */
static void parse_session_key(char *session_key) {
  char *tail;
  char *p = session_key;
  ...
  if (strlen(p) % 2 != 0)
    log_fatal(
        "Session key must consist of an even number of hexadecimal "
        "characters.\n");
...
/* Return the ith block from TEXT.  The first block is labeled 1.
   ...
   (i=2) is just 2 bytes.  */
static byte *block(byte *text, int len, int i) {
...
/* Return true if the quick integrity check passes.  Also, if
   ...
   *CFBP.  */
static int oracle(int debug, byte *ciphertext, int len, byte **plaintextp,
                  byte **cfbp) {
  int rc = 0;
  unsigned nprefix;
  ...
  rc = openpgp_cipher_open(&cipher_hd, dek.algo, GCRY_CIPHER_MODE_CFB,
                           (!sync /* ed->mdc_method || dek.algo >= 100 */
                                ? 0
                                : GCRY_CIPHER_ENABLE_SYNC));
  if (rc) log_fatal("Failed to open cipher: %s\n", gpg_strerror(rc));
  ...
  gcry_cipher_decrypt(cipher_hd, plaintext, blocksize + 2, ciphertext,
                      blocksize + 2);
  gcry_cipher_sync(cipher_hd);
  if (len > blocksize + 2)
    gcry_cipher_decrypt(cipher_hd, &plaintext[blocksize + 2],
                        len - (blocksize + 2), &ciphertext[blocksize + 2],
                        len - (blocksize + 2));
  ...
  if (cfbp || debug) {
    int i;
    cfb = xmalloc(len);
    for (i = 0; i < len; i++) cfb[i] = plaintext[i] ^ ciphertext[i];
    ...
    if (debug) {
      ...
      log_debug("E_k([C_1]_{1,2}) = C_2 xor R (%s xor %s) = %s\n",
                hexstr(&ciphertext[blocksize]), hexstr(&plaintext[blocksize]),
                hexstr(bufxor2(&ciphertext[blocksize], &plaintext[blocksize])));
      if (len >= blocksize + 4)
        log_debug(
            "D = Ek([C1]_{3-b} || C_2)_{1-2} (%s) xor C2 (%s) xor "
            "E_k(0)_{b-1,b} (%s) = %s\n",
            hexstr(&cfb[blocksize + 2]), hexstr(&ciphertext[blocksize]),
            hexstr(&cfb[blocksize - 2]),
            hexstr(bufxor2(bufxor2(&cfb[blocksize + 2], &ciphertext[blocksize]),
                           &cfb[blocksize - 2])));
    }
  }
...
static int oracle_test(unsigned int d, int b, int debug) {
  byte probe[32 + 2];
  ...
  if (debug) log_debug("oracle (0x%04X):\n", d);
...
static void do_test(int argc, char *argv[]) {
  ...
  if (!blocksize && !filename &&
      (filename = prepend_srcdir("t-stutter-data.asc")))
  /* Try defaults.  */
  {
    parse_session_key(
  ...
  /* Parse the packet's header.  */
  {
    ...
    if (debug)
      log_debug("ctb = %x; %s format, hdrlen: %d, packet: %s\n", ctb,
                new_format ? "new" : "old", hdrlen, pkttype_str(pkttype));
...

  {
    ...
    if (oracle(debug, msg, msg_len, &msg_plaintext, &msg_cfb) == 0) {
      if (debug) log_debug("Session key appears to be good.\n");
    } else
    ...
    if (debug)
      log_debug("First two bytes of plaintext are: %02X (%c) %02X (%c)\n",
                m1[0], isprint(m1[0]) ? m1[0] : '?', m1[1],
                isprint(m1[1]) ? m1[1] : '?');

    for (d = 0; d < 256 * 256; d++)
      if ((found = oracle_test(d, 1, 0))) break;

    if (!found) log_fatal("Failed to find d!\n");

    if (debug) oracle_test(d, 1, 1);

    if (debug) log_debug("D = %d (%x) looks good.\n", d, d);

    {
      ...
      memcpy(e_k_zero, bufxor2(bufxor2(c2, D), bufxor2(c3, m1)),
             sizeof(e_k_zero));
    ...
    for (i = 1; block_offset(i + 3) + 2 <= msg_len; i++) {
      ...
      if (!found) log_fatal("Failed to find a valid d for block %d\n", i);
      ...
      {
        byte D[2] = {d >> 8, d & 0xFF};
        memcpy(e_k_prime, bufxor2(bufxor2(&ct[blocksize - 2], D), e_k_zero),
               sizeof(e_k_prime));

        memcpy(m, bufxor2(e_k_prime, block(msg, msg_len, i + 3)), sizeof(m));
      }
      ...
      if (debug)
        log_debug(
            ...
            "%s (%c%c)\n",
            i, (size_t)pt - (size_t)msg_plaintext, hexstr(m),
            isprint(m[0]) ? m[0] : '?', isprint(m[1]) ? m[1] : '?');

      if (m[0] != pt[0] || m[1] != pt[1]) {
        log_debug("oracle attack failed!  Expected %s (%c%c), got %s\n",
                  hexstr(pt), isprint(pt[0]) ? pt[0] : '?',
                  isprint(pt[1]) ? pt[1] : '?', hexstr(m));
        tests_failed++;
      }
    }

    if (i == 1) log_fatal("Message is too short, nothing to test.\n");
  }
