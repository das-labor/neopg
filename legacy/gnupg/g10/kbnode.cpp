...
static kbnode_t alloc_node(void) {
  kbnode_t n;
...
static void free_node(KBNODE n) {
  if (n) xfree(n);
}
 ...
 */
void delete_kbnode(KBNODE node) { node->private_flag |= 1; }
...
find_prev_kbnode(KBNODE root, KBNODE node, int pkttype) {
  ...
  for (n1 = NULL; root && root != node; root = root->next) {
    if (!pkttype || root->pkt->pkttype == pkttype) n1 = root;
  }
  return n1;
}
...
find_next_kbnode(KBNODE node, int pkttype) {
  for (node = node->next; node; node = node->next) {
    if (!pkttype)
      return node;
    else if (pkttype == PKT_USER_ID && (node->pkt->pkttype == PKT_PUBLIC_KEY ||
...
find_kbnode(KBNODE node, int pkttype) {
  for (; node; node = node->next) {
    if (node->pkt->pkttype == pkttype) return node;
  }
  return NULL;
}
...
void dump_kbnode(KBNODE node) {
  for (; node; node = node->next) {
    ...
    log_debug("node %p %02x/%02x type=%s", node, node->flag, node->private_flag,
              s);
    if (node->pkt->pkttype == PKT_USER_ID) {
      PKT_user_id *uid = node->pkt->pkt.user_id;
      log_printf("  \"");
      es_write_sanitized(log_get_stream(), uid->name, uid->len, NULL, NULL);
      log_printf("\" %c%c%c%c\n", uid->flags.expired ? 'e' : '.',
                 uid->flags.revoked ? 'r' : '.', uid->created ? 'v' : '.',
                 uid->flags.primary ? 'p' : '.');
      ...
      log_printf("  keyid=%08lX a=%d u=%d %c%c%c%c\n",
                 (unsigned long)keyid_from_pk(pk, NULL), pk->pubkey_algo,
                 pk->pubkey_usage, pk->has_expired ? 'e' : '.',
                 pk->flags.revoked ? 'r' : '.', pk->flags.valid ? 'v' : '.',
                 pk->flags.mdc ? 'm' : '.');
    } else
      log_printf("\n");
