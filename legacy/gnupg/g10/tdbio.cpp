 ...
 *          actually been taken.
 */
static int take_write_lock(void) {
  if (!lockhandle) lockhandle = dotlock_create(db_name, 0);
  if (!lockhandle) log_fatal(_("can't create lock for '%s'\n"), db_name);
...
static void release_write_lock(void) {
  if (!dotlock_release(lockhandle)) is_locked = 0;
}
 ...
 * a cache miss.
 */
static const char *get_record_from_cache(unsigned long recno) {
  CACHE_CTRL r;

  for (r = cache_list; r; r = r->next) {
    if (r->flags.used && r->recno == recno) return r->data;
  }
  return NULL;
}
 ...
 */
static int write_cache_item(CACHE_CTRL r) {
  gpg_error_t err;
  int n;
 ...
 * Returns: 0 on success or an error code.
 */
static int put_record_into_cache(unsigned long recno, const char *data) {
  CACHE_CTRL r, unused;
  int dirty_count = 0;
  int clean_count = 0;
...
int tdbio_is_dirty() { return cache_is_dirty; }
...
int tdbio_sync() {
  ...
  if (db_fd == -1) open_db();
  if (in_transaction) log_bug("tdbio: syncing while in transaction\n");

  if (!cache_is_dirty) return 0;

  if (!take_write_lock()) did_lock = 1;
  ...
  cache_is_dirty = 0;
  if (did_lock) release_write_lock();
 ...
 * Returns: 0 on success or an error code.
 */
static int create_version_record(ctrl_t ctrl) {
  TRUSTREC rec;
  int rc;
  ...
  if (!rc) create_hashtable(ctrl, &rec, 0);
...
int tdbio_set_dbname(ctrl_t ctrl, const char *new_dbname, int create,
  ...
  *r_nofile = 0;
  ...
  if (!new_dbname) {
    fname = make_filename(gnupg_homedir(), "trustdb" EXTSEP_S GPGEXT_GPG, NULL);
  ...
  *p = save_slash;
  ...
  if (access(fname, R_OK) || stat(fname, &statbuf) || statbuf.st_size == 0) {
    ...
    if (is_secured_filename(fname)) {
      ...
      fp = fopen(fname, "wb");
    umask(oldmask);
    if (!fp) log_fatal(_("can't create '%s': %s\n"), fname, strerror(errno));
    fclose(fp);
    ...
    if (rc)
      log_fatal(_("%s: failed to create version record: %s"), fname,
                gpg_strerror(rc));
    ...
    if (!opt.quiet) log_info(_("%s: trustdb created\n"), db_name);
  }
 ...
 */
const char *tdbio_get_dbname() { return db_name; }
...
static void open_db() {
  ...
  if (db_fd == -1 && (errno == EACCES
    ...
    if (db_fd != -1 && !opt.quiet) log_info(_("Note: trustdb not writable\n"));
  }
  if (db_fd == -1)
    log_fatal(_("can't open '%s': %s\n"), db_name, strerror(errno));
...
static void create_hashtable(ctrl_t ctrl, TRUSTREC *vr, int type) {
  ...
  if (!type) vr->r.ver.trusthashtbl = recnum;
 ...
 */
unsigned long tdbio_read_nextcheck() {
  TRUSTREC vr;
  int rc;
...
int tdbio_write_nextcheck(ctrl_t ctrl, unsigned long stamp) {
  ...
  if (vr.r.ver.nextcheck == stamp) return 0;
 ...
 */
static unsigned long get_trusthashrec(void) {
 ...
 */
static int upd_hashtable(ctrl_t ctrl, unsigned long table, byte *key,
                         int keylen, unsigned long newrecnum) {
  ...
  } else if (item != newrecnum) /* Must do an update.  */
  {
    ...
    } else if (rec.rectype == RECTYPE_TRUST) /* Insert a list record.  */
    {
      ...
      log_error("hashtbl %lu: %lu/%d points to an invalid record %lu\n", table,
                hashrec, (msb % ITEMS_PER_HTBL_RECORD), item);
      if (opt.verbose > 1) list_trustdb(ctrl, es_stderr, NULL);
      return GPG_ERR_TRUSTDB;
    }
  }
 ...
 * Return: 0 on success or an error code.
 */
static int drop_from_hashtable(ctrl_t ctrl, unsigned long table, byte *key,
                               int keylen, unsigned long recnum) {
  TRUSTREC rec;
  unsigned long hashrec, item;
  int msb;
  int level = 0;
...
next_level:
  ...
  item = rec.r.htbl.item[msb % ITEMS_PER_HTBL_RECORD];
  if (!item) return 0; /* Not found - forget about it.  */
  ...
  log_error("hashtbl %lu: %lu/%d points to wrong record %lu\n", table, hashrec,
            (msb % ITEMS_PER_HTBL_RECORD), item);
  return GPG_ERR_TRUSTDB;
}
 ...
 */
static gpg_error_t lookup_hashtable(unsigned long table, const byte *key,
                                    size_t keylen,
                                    int (*cmpfnc)(const void *,
                                                  const TRUSTREC *),
...
next_level:
  ...
  item = rec->r.htbl.item[msb % ITEMS_PER_HTBL_RECORD];
  if (!item) return GPG_ERR_NOT_FOUND;
  ...
  if ((*cmpfnc)(cmpdata, rec)) return 0; /* really found */
 ...
 * Return: 0 on success or an error code.
 */
static int update_trusthashtbl(ctrl_t ctrl, TRUSTREC *tr) {
  return upd_hashtable(ctrl, get_trusthashrec(), tr->r.trust.fingerprint, 20,
                       tr->recnum);
}
  ...
  switch (rec->rectype) {
    ...
    case RECTYPE_VER:
      es_fprintf(
          ...
          rec->r.ver.trusthashtbl, rec->r.ver.firstfree, rec->r.ver.marginals,
          rec->r.ver.completes, rec->r.ver.cert_depth, rec->r.ver.trust_model,
          rec->r.ver.min_cert_level, rec->r.ver.nextcheck,
    ...
    case RECTYPE_VALID:
      es_fprintf(fp, "valid ");
      for (i = 0; i < 20; i++) es_fprintf(fp, "%02X", rec->r.valid.namehash[i]);
  ...
  switch (rec->rectype) {
    ...
    default:
      log_error("%s: invalid record type %d at recnum %lu\n", db_name,
                rec->rectype, (unsigned long)recnum);
      err = GPG_ERR_TRUSTDB;
      break;
  }
  ...
  switch (rec->rectype) {
    ...
    case RECTYPE_VER: /* version record */
      ...
      *p++ = rec->r.ver.min_cert_level;
      p += 2;
      ulongtobuf(p, rec->r.ver.created);
      p += 4;
      ulongtobuf(p, rec->r.ver.nextcheck);
      p += 4;
      p += 4;
      p += 4;
      ulongtobuf(p, rec->r.ver.firstfree);
      p += 4;
      p += 4;
      ulongtobuf(p, rec->r.ver.trusthashtbl);
      p += 4;
      break;

    case RECTYPE_FREE:
      ulongtobuf(p, rec->r.free.next);
      p += 4;
      break;

    case RECTYPE_HTBL:
      for (i = 0; i < ITEMS_PER_HTBL_RECORD; i++) {
        ulongtobuf(p, rec->r.htbl.item[i]);
        p += 4;
      }
      break;

    case RECTYPE_HLST:
      ulongtobuf(p, rec->r.hlst.next);
      p += 4;
      for (i = 0; i < ITEMS_PER_HLST_RECORD; i++) {
        ulongtobuf(p, rec->r.hlst.rnum[i]);
        p += 4;
      }
      break;

    case RECTYPE_TRUST:
      memcpy(p, rec->r.trust.fingerprint, 20);
      p += 20;
      *p++ = rec->r.trust.ownertrust;
      *p++ = rec->r.trust.depth;
      *p++ = rec->r.trust.min_ownertrust;
      p++;
      ulongtobuf(p, rec->r.trust.validlist);
      p += 4;
      break;

    case RECTYPE_VALID:
      memcpy(p, rec->r.valid.namehash, 20);
      p += 20;
      *p++ = rec->r.valid.validity;
      ulongtobuf(p, rec->r.valid.next);
      p += 4;
...
int tdbio_delete_record(ctrl_t ctrl, unsigned long recnum) {
  ...
  if (rc)
    ...
    rc = drop_from_hashtable(ctrl, get_trusthashrec(), rec.r.trust.fingerprint,
                             20, rec.recnum);
  }
  ...
  rc = tdbio_write_record(ctrl, &rec);
  if (!rc) rc = tdbio_write_record(ctrl, &vr);
...
static int cmp_trec_fpr(const void *fpr, const TRUSTREC *rec) {
...
gpg_error_t tdbio_search_trust_byfpr(const byte *fingerprint, TRUSTREC *rec) {
  ...
  rc = lookup_hashtable(get_trusthashrec(), fingerprint, 20, cmp_trec_fpr,
                        fingerprint, rec);
  return rc;
}
...
gpg_error_t tdbio_search_trust_bypk(PKT_public_key *pk, TRUSTREC *rec) {
  ...
  for (; fingerlen < 20; fingerlen++) fingerprint[fingerlen] = 0;
