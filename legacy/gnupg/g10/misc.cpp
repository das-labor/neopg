...
int is_secured_file(int fd) {
  ...
  /*              (unsigned long)buf.st_dev, (unsigned long)buf.st_ino); */
  for (sf = secured_files; sf; sf = sf->next) {
    if (sf->ino == buf.st_ino && sf->dev == buf.st_dev) return 1; /* Yes.  */
  }
#else       /*!ENABLE_SELINUX_HACKS*/
  (void)fd;
#endif      /*!ENABLE_SELINUX_HACKS*/
...
int is_secured_filename(const char *fname) {
  ...
  if (iobuf_is_pipe_filename(fname) || !*fname) return 0;

  /* Note that we print out a error here and claim that a file is
     secure if something went wrong. */
  if (stat(fname, &buf)) {
    if (errno == ENOENT || errno == EPERM || errno == EACCES) return 0;
    log_error(_("fstat of '%s' failed in %s: %s\n"), fname,
 ...
 * printed if ERR is 0.  */
void print_reported_error(gpg_error_t err, gpg_error_t ignore_ec) {
 ...
 */
void print_further_info(const char *format, ...) {
  va_list arg_ptr;
...
static cipher_algo_t map_cipher_gcry_to_openpgp(enum gcry_cipher_algos algo) {
  switch (algo) {
    case GCRY_CIPHER_NONE:
      return CIPHER_ALGO_NONE;
    case GCRY_CIPHER_IDEA:
      return CIPHER_ALGO_IDEA;
    case GCRY_CIPHER_3DES:
      return CIPHER_ALGO_3DES;
    case GCRY_CIPHER_CAST5:
      return CIPHER_ALGO_CAST5;
    case GCRY_CIPHER_BLOWFISH:
      return CIPHER_ALGO_BLOWFISH;
    case GCRY_CIPHER_AES:
      return CIPHER_ALGO_AES;
    case GCRY_CIPHER_AES192:
      return CIPHER_ALGO_AES192;
    case GCRY_CIPHER_AES256:
      return CIPHER_ALGO_AES256;
    case GCRY_CIPHER_TWOFISH:
      return CIPHER_ALGO_TWOFISH;
    case GCRY_CIPHER_CAMELLIA128:
      return CIPHER_ALGO_CAMELLIA128;
    case GCRY_CIPHER_CAMELLIA192:
      return CIPHER_ALGO_CAMELLIA192;
    case GCRY_CIPHER_CAMELLIA256:
      return CIPHER_ALGO_CAMELLIA256;
    default:
      return (cipher_algo_t)0;
  }
}
...
pubkey_algo_t map_pk_gcry_to_openpgp(enum gcry_pk_algos algo) {
  switch (algo) {
    ...
    default:
      return (pubkey_algo_t)(algo < 110 ? algo : 0);
  }
}
...
int openpgp_cipher_test_algo(cipher_algo_t algo) {
  ...
  if (!ga) return GPG_ERR_CIPHER_ALGO;
...
int openpgp_pk_test_algo2(pubkey_algo_t algo, unsigned int use) {
  ...
  if (!ga) return GPG_ERR_PUBKEY_ALGO;
...
int openpgp_md_test_algo(digest_algo_t algo) {
  ...
  if (!ga) return GPG_ERR_DIGEST_ALGO;
...
static unsigned long get_signature_count(PKT_public_key *pk) {
...
char *pct_expando(const char *string, struct expando_args *args) {
  ...
  if (args->pk) keyid_from_pk(args->pk, pk_keyid);

  if (args->pksk) keyid_from_pk(args->pksk, sk_keyid);

  /* This is used so that %k works in photoid command strings in
     --list-secret-keys (which of course has a sk, but no pk). */
  if (!args->pk && args->pksk) keyid_from_pk(args->pksk, pk_keyid);

  while (*ch != '\0') {
    if (!done) {
      /* 8192 is way bigger than we'll need here */
      if (maxlen >= 8192) goto fail;
    ...
    if (*ch == '%') {
      switch (*(ch + 1)) {
        ...
        case 'S': /* long key id */
          if (idx + 16 < maxlen) {
            sprintf(&ret[idx], "%08lX%08lX", (unsigned long)sk_keyid[0],
                    (unsigned long)sk_keyid[1]);
            idx += 16;
            done = 1;
          }
          break;
        ...
        case 'K': /* long key id */
          if (idx + 16 < maxlen) {
            sprintf(&ret[idx], "%08lX%08lX", (unsigned long)pk_keyid[0],
                    (unsigned long)pk_keyid[1]);
            idx += 16;
            done = 1;
          }
          break;
        ...
        case 'g': /* Fingerprint of the key making the signature.  */
        {
          ...
          if ((*(ch + 1)) == 'f' && args->pk)
            ...
            else if (args->pksk->main_keyid[0] || args->pksk->main_keyid[1]) {
...
void deprecated_warning(const char *configname, unsigned int configlineno,
                        const char *option, const char *repl1,
                        const char *repl2) {
  if (configname) {
    if (strncmp("--", option, 2) == 0) option += 2;

    if (strncmp("--", repl1, 2) == 0) repl1 += 2;
...
int string_to_cipher_algo(const char *string) {
  ...
  val = map_cipher_gcry_to_openpgp(
      (gcry_cipher_algos)(gcry_cipher_map_name(string)));
...
/* Return the length of the initial token, leaving off any
   argument. */
static size_t optlen(const char *s) {
  ...
  while ((tok = optsep(&str))) {
    ...
    for (i = 0; opts[i].name; i++) {
      ...
      if (ascii_strncasecmp(opts[i].name, tok, toklen) == 0) {
        ...
        if (toklen != strlen(opts[i].name)) {
          ...
          for (j = i + 1; opts[j].name; j++) {
            if (ascii_strncasecmp(opts[j].name, tok, toklen) == 0) {
              if (noisy) log_info(_("ambiguous option '%s'\n"), otok);
              return 0;
            }
          }
        }
        ...
        if (rev) {
          *options &= ~opts[i].bit;
          if (opts[i].value) *opts[i].value = NULL;
        } else {
          *options |= opts[i].bit;
          if (opts[i].value) *opts[i].value = argsplit(tok);
        }
        break;
      }
    }
    ...
    if (!opts[i].name) {
      if (noisy) log_info(_("unknown option '%s'\n"), otok);
      return 0;
    }
  }
...
/* Temporary helper. */
unsigned int pubkey_nbits(int algo, gcry_mpi_t *key) {
  int rc, nbits;
  gcry_sexp_t sexp;

  if (algo == PUBKEY_ALGO_DSA && key[0] && key[1] && key[2] && key[3]) {
    rc = gcry_sexp_build(&sexp, NULL, "(public-key(dsa(p%m)(q%m)(g%m)(y%m)))",
                         key[0], key[1], key[2], key[3]);
  } else if ((algo == PUBKEY_ALGO_ELGAMAL || algo == PUBKEY_ALGO_ELGAMAL_E) &&
             key[0] && key[1] && key[2]) {
    rc = gcry_sexp_build(&sexp, NULL, "(public-key(elg(p%m)(g%m)(y%m)))",
                         key[0], key[1], key[2]);
  } else if (is_RSA(algo) && key[0] && key[1]) {
    rc = gcry_sexp_build(&sexp, NULL, "(public-key(rsa(n%m)(e%m)))", key[0],
                         key[1]);
...
int mpi_print(estream_t fp, gcry_mpi_t a, int mode) {
  int n = 0;
  size_t nwritten;

  if (!a) return es_fprintf(fp, "[MPI_NULL]");
  if (!mode) {
    unsigned int n1;
...
unsigned int ecdsa_qbits_from_Q(unsigned int qbits) {
  if ((qbits % 8) > 3) {
    log_error(
        _("ECDSA public key is expected to be in SEC encoding "
          "multiple of 8 bits\n"));
    return 0;
  }
  qbits -= qbits % 8;
