...
int pubkey_letter(int algo) {
  switch (algo) {
    ...
    case PUBKEY_ALGO_ECDH:
      return 'e'; /* ECC DH (encrypt only) */
    case PUBKEY_ALGO_ECDSA:
      return 'E'; /* ECC DSA (sign only)   */
    case PUBKEY_ALGO_EDDSA:
      return 'E'; /* ECC EdDSA (sign only) */
    default:
      return '?';
  }
}
...
void hash_public_key(gcry_md_hd_t md, PKT_public_key *pk) {
  ...
  if (npkey == 0 && pk->pkey[0] &&
      gcry_mpi_get_flag(pk->pkey[0], GCRYMPI_FLAG_OPAQUE)) {
    if (pp[0]) gcry_md_write(md, pp[0], nn[0]);
  } else {
    for (i = 0; i < npkey; i++) {
      if (pp[i]) gcry_md_write(md, pp[i], nn[i]);
      xfree(pp[i]);
    }
  }
}

static gcry_md_hd_t do_fingerprint_md(PKT_public_key *pk) {
  gcry_md_hd_t md;

  if (gcry_md_open(&md, DIGEST_ALGO_SHA1, 0)) BUG();
...
u32 v3_keyid(gcry_mpi_t a, u32 *ki) {
  ...
  if (gcry_mpi_print(GCRYMPI_FMT_USG, NULL, 0, &nbytes, a)) BUG();
  /* fixme: allocate it on the stack */
  buffer = (byte *)xmalloc(nbytes);
  if (gcry_mpi_print(GCRYMPI_FMT_USG, buffer, nbytes, NULL, a)) BUG();
  if (nbytes < 8) /* oops */
    ki[0] = ki[1] = 0;
  else {
    p = buffer + nbytes - 8;
...
char *format_keyid(u32 *keyid, int format, char *buffer, int len) {
  ...
  if (format == KF_DEFAULT) format = opt.keyid_format;
  if (format == KF_DEFAULT) format = KF_NONE;
  ...
  switch (format) {
    ...
    case KF_LONG:
      snprintf(buffer, len, "%08lX%08lX", (unsigned long)keyid[0],
    ...
    case KF_0xLONG:
      snprintf(buffer, len, "0x%08lX%08lX", (unsigned long)keyid[0],
               (unsigned long)keyid[1]);
      break;
  ...
  if (buffer == tmp) return xstrdup(buffer);
  return buffer;
}
...
const char *keystr(u32 *keyid) {
  ...
  if (format == KF_DEFAULT) format = KF_NONE;
  if (format == KF_NONE) format = KF_LONG;
...
const char *keystr_from_pk_with_sub(PKT_public_key *main_pk,
  ...
  if (sub_pk) keyid_from_pk(sub_pk, NULL);
...
/* Return PK's key id as a string using the default format.  PK owns
   the storage.  */
const char *pk_keyid_str(PKT_public_key *pk) { return keystr(pk_keyid(pk)); }
...
byte *namehash_from_uid(PKT_user_id *uid) {
  if (!uid->namehash) {
    std::unique_ptr<Botan::HashFunction> rmd160(
        Botan::HashFunction::create_or_throw("RIPEMD-160"));
 ...
 */
unsigned int nbits_from_pk(PKT_public_key *pk) {
  return pubkey_nbits(pk->pubkey_algo, pk->pkey);
}

static const char *mk_datestr(char *buffer, time_t atime) {
  struct tm *tp;
...
const char *expirestr_from_pk(PKT_public_key *pk) {
  ...
  if (!pk->expiredate) return _("never     ");
  atime = pk->expiredate;
  return mk_datestr(buffer, atime);
}
...
const char *expirestr_from_sig(PKT_signature *sig) {
  ...
  if (!sig->expiredate) return _("never     ");
...
const char *revokestr_from_pk(PKT_public_key *pk) {
  ...
  if (!pk->revoked.date) return _("never     ");
...
const char *usagestr_from_pk(PKT_public_key *pk, int fill) {
  ...
  if (use & PUBKEY_USAGE_SIG) buffer[i++] = 'S';

  if (use & PUBKEY_USAGE_CERT) buffer[i++] = 'C';

  if (use & PUBKEY_USAGE_ENC) buffer[i++] = 'E';

  if ((use & PUBKEY_USAGE_AUTH)) buffer[i++] = 'A';

  while (fill && i < 4) buffer[i++] = ' ';
 ...
 */
byte *fingerprint_from_pk(PKT_public_key *pk, byte *array, size_t *ret_len) {
  const byte *dp;
  size_t len;
  gcry_md_hd_t md;
  ...
  if (!array) array = (byte *)xmalloc(len);
  memcpy(array, dp, len);
...
char *format_hexfingerprint(const char *fingerprint, char *buffer,
                            size_t buflen) {
...
gpg_error_t keygrip_from_pk(PKT_public_key *pk, unsigned char *array) {
  ...
  if (DBG_PACKET) log_debug("get_keygrip for public key\n");
  ...
  if (!gcry_pk_get_keygrip(s_pkey, array)) {
    ...
    if (DBG_PACKET) log_printhex("keygrip=", array, 20);
