...
#define IS_CERT(s)                                                        \
  (IS_KEY_SIG(s) || IS_UID_SIG(s) || IS_SUBKEY_SIG(s) || IS_KEY_REV(s) || \
   IS_UID_REV(s) || IS_SUBKEY_REV(s))
...
DEK *passphrase_to_dek_ext(u32 *keyid, int pubkey_algo, int cipher_algo,
...
char *gpg_format_keydesc(ctrl_t ctrl, PKT_public_key *pk, int mode,
                         int escaped);
...
static int GPGRT_ATTR_UNUSED keyid_cmp(const u32 *a, const u32 *b) {
  if (a[0] < b[0]) return -1;
  if (a[0] > b[0]) return 1;
  if (a[1] < b[1]) return -1;
  if (a[1] > b[1]) return 1;
...
static int GPGRT_ATTR_UNUSED pk_is_primary(PKT_public_key *pk) {
