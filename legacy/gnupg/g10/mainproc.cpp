...
void reset_literals_seen(void) { literals_seen = 0; }
...
static int add_gpg_control(CTX c, PACKET *pkt) {
...
static int add_user_id(CTX c, PACKET *pkt) {
  if (!c->list) {
...
static int add_subkey(CTX c, PACKET *pkt) {
  if (!c->list) {
...
static int add_ring_trust(CTX c, PACKET *pkt) {
  if (!c->list) {
...
static int add_signature(CTX c, PACKET *pkt) {
  kbnode_t node;
...
static int symkey_decrypt_seskey(DEK *dek, byte *seskey, size_t slen) {
  gcry_cipher_hd_t hd;
  ...
  if (openpgp_cipher_open(&hd, dek->algo, GCRY_CIPHER_MODE_CFB, 1)) BUG();
  if (gcry_cipher_setkey(hd, dek->key, dek->keylen)) BUG();
...
static void proc_symkey_enc(CTX c, PACKET *pkt) {
  PKT_symkey_enc *enc;
...
static void proc_pubkey_enc(ctrl_t ctrl, CTX c, PACKET *pkt) {
  PKT_pubkey_enc *enc;
  int result = 0;
  ...
  if (opt.verbose) log_info(_("public key is %s\n"), keystr(enc->keyid));
  ...
  if (is_status_enabled()) {
    ...
    snprintf(buf, sizeof buf, "%08lX%08lX %d 0", (unsigned long)enc->keyid[0],
             (unsigned long)enc->keyid[1], enc->pubkey_algo);
  ...
  if (!opt.list_only && !opt.override_session_key.empty()) {
    ...
    if (!c->dek && ((!enc->keyid[0] && !enc->keyid[1]) || opt.try_all_secrets ||
 ...
 * not decrypt.
 */
static void print_pkenc_list(ctrl_t ctrl, struct kidlist_item *list,
                             int failed) {
  for (; list; list = list->next) {
    ...
    if (list->reason == GPG_ERR_NO_SECKEY) {
      if (is_status_enabled()) {
        char buf[20];
        snprintf(buf, sizeof buf, "%08lX%08lX", (unsigned long)list->kid[0],
                 (unsigned long)list->kid[1]);
...
static void proc_encrypted(CTX c, PACKET *pkt) {
  ...
  if (opt.list_only)
    ...
    if (!opt.override_session_key.empty()) {
      ...
      if (algo)
        ...
        algo = opt.def_cipher_algo;
        if (!algo) algo = opt.s2k_cipher_algo;
  ...
  if (!result && is_status_enabled()
    ...
    if (compliant)
      write_status_strings(STATUS_DECRYPTION_COMPLIANCE_MODE,
                           gnupg_status_compliance_flag(CO_DE_VS), NULL);
  }
  ...
  if (result == -1)
    ...
    if (result == GPG_ERR_BAD_KEY && *c->dek->s2k_cacheid != '\0') {
      if (opt.debug)
...
static void proc_plaintext(CTX c, PACKET *pkt) {
  ...
  if (gcry_md_open(&c->mfx.md, 0, 0)) BUG();
  ...
  if (DBG_HASHING) {
    ...
    if (c->mfx.md2) gcry_md_debug(c->mfx.md2, "verify2");
  }
  ...
  if (!rc) {
    ...
    rc = handle_plaintext(
        pt, &c->mfx, (opt.outfp || opt.outfile) ? 0 : c->sigs_only, clearsig);
...
static int proc_compressed_cb(iobuf_t a, void *info) {
  if (((CTX)info)->signed_data.used && ((CTX)info)->signed_data.data_fd != -1)
...
static int proc_encrypt_cb(iobuf_t a, void *info) {
  CTX c = (CTX)info;
  return proc_encryption_packets(c->ctrl, info, a);
}

static int proc_compressed(CTX c, PACKET *pkt) {
  ...
  if (rc == GPG_ERR_BAD_DATA) {
    if (!c->any.uncompress_failed) {
      ...
      for (cc = c; cc; cc = cc->anchor) cc->any.uncompress_failed = 1;
 ...
 */
static int do_check_sig(CTX c, kbnode_t node, int *is_selfsig, int *is_expkey,
                        int *is_revkey, PKT_public_key **r_pk) {
  PKT_signature *sig;
  ...
  if (sig->sig_class == 0x00) {
    if (c->mfx.md) {
      if (gcry_md_copy(&md, c->mfx.md)) BUG();
    } else /* detached signature */
    {
      /* check_signature() will enable the md. */
      if (gcry_md_open(&md, 0, 0)) BUG();
    }
    ...
    if (c->mfx.md) {
      if (gcry_md_copy(&md, c->mfx.md)) BUG();
      if (c->mfx.md2 && gcry_md_copy(&md2, c->mfx.md2)) BUG();
    } else /* detached signature */
    {
      log_debug("Do we really need this here?");
      /* check_signature() will enable the md*/
      if (gcry_md_open(&md, 0, 0)) BUG();
      if (gcry_md_open(&md2, 0, 0)) BUG();
    }
  } else if ((sig->sig_class & ~3) == 0x10 || sig->sig_class == 0x18 ||
             sig->sig_class == 0x1f || sig->sig_class == 0x20 ||
             sig->sig_class == 0x28 || sig->sig_class == 0x30) {
    if (c->list->pkt->pkttype == PKT_PUBLIC_KEY ||
        c->list->pkt->pkttype == PKT_PUBLIC_SUBKEY) {
      return check_key_signature(c->ctrl, c->list, node, is_selfsig);
    } else if (sig->sig_class == 0x20) {
      log_error(
          _("standalone revocation - "
  ...
  if (md_good) {
    ...
    sig->digest_len =
        gcry_md_get_algo_dlen(map_md_openpgp_to_gcry((digest_algo_t)(algo)));
...
static void print_userid(PACKET *pkt) {
  if (!pkt) BUG();
  ...
  if (opt.with_colons) {
    if (pkt->pkt.user_id->attrib_data)
      es_printf("%u %lu", pkt->pkt.user_id->numattribs,
                pkt->pkt.user_id->attrib_len);
    else
      es_write_sanitized(es_stdout, pkt->pkt.user_id->name,
                         pkt->pkt.user_id->len, ":", NULL);
  } else
    print_utf8_buffer(es_stdout, pkt->pkt.user_id->name, pkt->pkt.user_id->len);
}
...
static void list_node(CTX c, kbnode_t node) {
  if (!node)
    ...
    if (opt.with_colons) {
      ...
      if (pk->flags.primary)
        c->trustletter = get_validity_info(
            c->ctrl, node->pkt->pkttype == PKT_PUBLIC_KEY ? node : NULL, pk,
            NULL);
      es_printf("%s:", pk->flags.primary ? "pub" : "sub");
      if (c->trustletter) es_putc(c->trustletter, es_stdout);
      es_printf(":%u:%d:%08lX%08lX:%s:%s::", nbits_from_pk(pk), pk->pubkey_algo,
      ...
      for (node = node->next; node; node = node->next) {
        if (node->pkt->pkttype == PKT_SIGNATURE) {
          ...
          if (opt.with_colons)
            ...
            es_printf("uid%*s", kl + 11, "");
          print_userid(node->pkt);
          if (opt.with_colons) es_putc(':', es_stdout);
          es_putc('\n', es_stdout);
    ...
    if (opt.with_colons) {
      ...
      es_printf("%c       %s %s   ", sigrc, keystr(sig->keyid),
    ...
    if (sigrc == '%')
      ...
      es_write_sanitized(es_stdout, p, n, opt.with_colons ? ":" : NULL, NULL);
      xfree(p);
    }
    if (opt.with_colons)
...
int proc_signature_packets(
  ...
  if (!rc && c->anchor && c->any.sig_seen) c->anchor->any.sig_seen = 1;

  xfree(c);
  return rc;
}

int proc_signature_packets_by_fd(ctrl_t ctrl, void *anchor, iobuf_t a,
                                 int signed_data_fd) {
  int rc;
  CTX c;
...
static int check_nesting(CTX c) {
  int level;

  for (level = 0; c; c = c->anchor) level++;
...
static int do_proc_packets(ctrl_t ctrl, CTX c, iobuf_t a) {
  ...
  while ((rc = parse_packet(&parsectx, pkt)) != -1) {
    ...
    if (rc) {
      ...
      if (rc == GPG_ERR_INV_PACKET && opt.list_packets == 0) break;
      continue;
    }
    newpkt = -1;
    if (opt.list_packets) {
      ...
      switch (pkt->pkttype) {
        ...
        case PKT_ONEPASS_SIG:
          newpkt = add_onepass_sig(c, pkt);
          break;
        case PKT_GPG_CONTROL:
          newpkt = add_gpg_control(c, pkt);
          break;
        default:
          newpkt = 0;
          break;
  ...
  if (rc == GPG_ERR_INV_PACKET) write_status_text(STATUS_NODATA, "3");
...
static char *issuer_fpr_string(PKT_signature *sig) {
  ...
  if (p && n == 21 && p[0] == 4) return bin2hex(p + 1, n - 1, NULL);
  return NULL;
}
...
static int check_sig_and_print(CTX c, kbnode_t node) {
   ...
   */
  {
    ...
    if (n->pkt->pkttype == PKT_SIGNATURE) {
      ...
      for (n = n->next; n; n = n->next)
        if (n->pkt->pkttype != PKT_SIGNATURE) break;
      if (!n)
        ; /* Okay, this is a detached signature.  */
      else if (n->pkt->pkttype == PKT_GPG_CONTROL &&
               (n->pkt->pkt.gpg_control->control == CTRLPKT_PLAINTEXT_MARK)) {
        if (n->next) goto ambiguous; /* We only allow one P packet. */
      } else
        goto ambiguous;
    } else if (n->pkt->pkttype == PKT_ONEPASS_SIG) {
      /* This is the "O{1,n} P S{1,n}" case (standard signature). */
      for (n_onepass = 1, n = n->next; n && n->pkt->pkttype == PKT_ONEPASS_SIG;
           n = n->next)
        n_onepass++;
      if (!n ||
          !(n->pkt->pkttype == PKT_GPG_CONTROL &&
            (n->pkt->pkt.gpg_control->control == CTRLPKT_PLAINTEXT_MARK)))
        goto ambiguous;
      for (n_sig = 0, n = n->next; n && n->pkt->pkttype == PKT_SIGNATURE;
           n = n->next)
        n_sig++;
      if (!n_sig) goto ambiguous;

      if (n_onepass != n_sig) {
        log_info(
            "number of one-pass packets does not match "
  ...
  if (!rc || rc == GPG_ERR_BAD_SIGNATURE) {
    ...
    for (un = keyblock; un; un = un->next) {
      ...
      if (!un->pkt->pkt.user_id->created) continue;
      if (un->pkt->pkt.user_id->flags.revoked) continue;
      if (un->pkt->pkt.user_id->flags.expired) continue;
      if (!un->pkt->pkt.user_id->flags.primary) continue;
      /* We want the textual primary user ID here */
      if (un->pkt->pkt.user_id->attrib_data) continue;
      ...
      if ((opt.verify_options & VERIFY_SHOW_UID_VALIDITY))
        valid = get_validity(c->ctrl, keyblock, mainpk, un->pkt->pkt.user_id,
                             NULL, 0);
      else
        valid = 0; /* Not used.  */
    ...
    if (!count) {
      ...
      if (!un) {
        for (un = keyblock; un; un = un->next) {
          if (un->pkt->pkttype == PKT_USER_ID) break;
        }
      }
        ...
        if ((opt.verify_options & VERIFY_SHOW_UID_VALIDITY)) {
          ...
          if (un->pkt->pkt.user_id->flags.revoked)
            ...
            /* Since this is just informational, don't
               ...
               information.  */
            valid = (trust_value_to_string(get_validity(
                c->ctrl, keyblock, mainpk, un->pkt->pkt.user_id, NULL, 0)));
          log_printf(" [%s]\n", valid);
        } else
          log_printf("\n");
      }
    ...
    if (!rc && is_status_enabled() && pk) {
      ...
      write_status_printf(
          STATUS_VALIDSIG, "%s %s %lu %lu %d 0 %d %d %02X %s", pkhex,
          strtimestamp(sig->timestamp), (unsigned long)sig->timestamp,
          (unsigned long)sig->expiredate, sig->version, sig->pubkey_algo,
          sig->digest_algo, sig->sig_class, mainpkhex);
    }
    ...
    snprintf(buf, sizeof buf, "%08lX%08lX %d %d %02x %lu %d",
             (unsigned long)sig->keyid[0], (unsigned long)sig->keyid[1],
             sig->pubkey_algo, sig->digest_algo, sig->sig_class,
             (unsigned long)sig->timestamp, rc);
...
static void proc_tree(CTX c, kbnode_t node) {
  ...
  if (opt.list_packets || opt.list_only) return;
  ...
  while (node && node->pkt->pkttype == PKT_GPG_CONTROL &&
  ...
  if (node->pkt->pkttype == PKT_PUBLIC_KEY ||
    ...
    if (!c->any.data) {
      ...
      if (c->sigs_only) {
        if (c->signed_data.used && c->signed_data.data_fd != -1)
          rc = hash_datafile_by_fd(c->mfx.md, NULL, c->signed_data.data_fd,
                                   use_textmode);
        else
          rc = hash_datafiles(c->mfx.md, NULL, c->signed_data.data_names,
                              c->sigfilename, use_textmode);
      } else {
    ...
    if (n1) {
      ...
      for (; n1; (n1 = find_next_kbnode(n1, PKT_SIGNATURE))) {
        ...
        if (n1->pkt->pkt.signature->sig_class != klasse ||
            (c->any.data && n1->pkt->pkt.signature->digest_algo != hash)) {
          multiple_ok = 0;
          log_info(
              _("WARNING: multiple signatures detected.  "
    ...
    if (sig->sig_class != 0x00 && sig->sig_class != 0x01) {
        ...
        for (n1 = node; (n1 = find_next_kbnode(n1, PKT_SIGNATURE));)
          if (!openpgp_md_test_algo(
            ...
            gcry_md_enable(c->mfx.md,
                           map_md_openpgp_to_gcry((digest_algo_t)(
      ...
      if (c->sigs_only) {
        if (c->signed_data.used && c->signed_data.data_fd != -1)
          rc =
              hash_datafile_by_fd(c->mfx.md, c->mfx.md2, c->signed_data.data_fd,
                                  (sig->sig_class == 0x01));
        else
          rc = hash_datafiles(c->mfx.md, c->mfx.md2, c->signed_data.data_names,
                              c->sigfilename, (sig->sig_class == 0x01));
      } else {
        rc = ask_for_detached_datafile(c->mfx.md, c->mfx.md2,
                                       iobuf_get_real_fname(c->iobuf),
                                       (sig->sig_class == 0x01));
