...
static gpg_error_t get_it(ctrl_t ctrl, PKT_pubkey_enc *k, DEK *dek,
                          PKT_public_key *sk, u32 *keyid);

/* Check that the given algo is mentioned in one of the valid user-ids. */
static int is_algo_in_prefs(kbnode_t keyblock, preftype_t type, int algo) {
  kbnode_t k;
...
gpg_error_t get_session_key(ctrl_t ctrl, PKT_pubkey_enc *k, DEK *dek) {
  ...
  if (DBG_CLOCK) log_clock("get_session_key enter");
  ...
  if ((k->keyid[0] || k->keyid[1]) && !opt.try_all_secrets) {
    ...
    if (!(rc = get_seckey(ctrl, sk, k->keyid))) {
      ...
      if (!gnupg_pk_is_allowed(opt.compliance, PK_USE_DECRYPTION,
                               sk->pubkey_algo, sk->pkey, nbits_from_pk(sk),
                               NULL)) {
  ...
  else /* Anonymous receiver: Try all available secret keys.  */
  {
    ...
    for (;;) {
      ...
      if (sk->pubkey_algo != k->pubkey_algo) continue;
      if (!(sk->pubkey_usage & PUBKEY_USAGE_ENC)) continue;
...
leave:
  free_public_key(sk);
  if (DBG_CLOCK) log_clock("get_session_key leave");
  return rc;
}

static gpg_error_t get_it(ctrl_t ctrl, PKT_pubkey_enc *enc, DEK *dek,
                          PKT_public_key *sk, u32 *keyid) {
  gpg_error_t err;
  byte *frame = NULL;
  unsigned int n;
  ...
  if (sk->pubkey_algo == PUBKEY_ALGO_ELGAMAL ||
    ...
    if (!enc->data[0])
      ...
      err = gcry_sexp_build(&s_data, NULL, "(enc-val(rsa(a%m)))", enc->data[0]);
  ...
  err = agent_pkdecrypt(NULL, keygrip, desc, sk->keyid, sk->main_keyid,
                        sk->pubkey_algo, s_data, &frame, &nframe, &padding);
  xfree(desc);
   ...
   * CSUM
   */
  if (DBG_CRYPTO) log_printhex("DEK frame:", frame, nframe);
  n = 0;
  ...
  for (csum2 = 0, n = 0; n < dek->keylen; n++) csum2 += dek->key[n];
  if (csum != csum2) {
    err = GPG_ERR_WRONG_SECKEY;
    goto leave;
  }
  if (DBG_CLOCK) log_clock("decryption ready");
  if (DBG_CRYPTO) log_printhex("DEK is:", dek->key, dek->keylen);
   ...
   * expired.  Also print a status line with the key's fingerprint.  */
  {
    ...
    if (!pkb) {
      ...
      log_error("oops: public key not found for preference check\n");
               ...
               dek->algo != CIPHER_ALGO_3DES && !opt.quiet &&
    ...
    if (is_status_enabled() && pk && mainpk) {
       ...
       * value is irrelevant.  */
      write_status_printf(STATUS_DECRYPTION_KEY, "%s %s %c", pkhex, mainpkhex,
...
gpg_error_t get_override_session_key(DEK *dek, const char *string) {
  ...
  if (!string) return GPG_ERR_BAD_KEY;
  dek->algo = atoi(string);
  if (dek->algo < 1) return GPG_ERR_BAD_KEY;
  if (!(s = strchr(string, ':'))) return GPG_ERR_BAD_KEY;
  s++;
  for (i = 0; i < DIM(dek->key) && *s; i++, s += 2) {
    int c = hextobyte(s);
    if (c == -1) return GPG_ERR_BAD_KEY;
    dek->key[i] = c;
  }
  if (*s) return GPG_ERR_BAD_KEY;
  dek->keylen = i;
  return 0;
}
