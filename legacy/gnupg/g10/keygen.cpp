...
static gpg_error_t parse_algo_usage_expire(
    ctrl_t ctrl, int for_subkey, const char *algostr, const char *usagestr,
    const char *expirestr, int *r_algo, unsigned int *r_usage, u32 *r_expire,
...
static gpg_error_t gen_card_key(int keyno, int algo, int is_primary,
                                ...
                                u32 expireval);
static unsigned int get_keysize_range(int algo, unsigned int *min,
                                      unsigned int *max);

/* Return the algo string for a default new key.  */
const char *get_default_pubkey_algo(void) {
  if (opt.def_new_key_algo) {
    if (opt.def_new_key_algo->length() &&
        !strchr(opt.def_new_key_algo->c_str(), ':'))
...
static void print_status_key_created(int letter, PKT_public_key *pk,
                                     const char *handle) {
  ...
  buf = (char *)xmalloc(MAX_FINGERPRINT_LEN * 2 + 31 + strlen(handle) + 1);
    ...
    *p++ = letter;
    if (pk) {
      *p++ = ' ';
      ...
      s = array;
      /* Fixme: Use bin2hex */
      for (i = 0; i < n; i++, s++, p += 2) snprintf(p, 3, "%02X", *s);
    }
  }
  if (*handle) {
    *p++ = ' ';
  ...
  *p = 0;
  write_status_text(
      (letter || pk) ? STATUS_KEY_CREATED : STATUS_KEY_NOT_CREATED, buf);
  xfree(buf);
}

static void print_status_key_not_created(const char *handle) {
  print_status_key_created(0, NULL, handle);
}

static void write_uid(KBNODE root, const char *s) {
  PACKET *pkt = (PACKET *)xmalloc_clear(sizeof *pkt);
  size_t n = strlen(s);

  pkt->pkttype = PKT_USER_ID;
  pkt->pkt.user_id = (PKT_user_id *)xmalloc_clear(sizeof *pkt->pkt.user_id + n);
...
static void do_add_key_flags(PKT_signature *sig, unsigned int use) {
  byte buf[1];

  buf[0] = 0;

  /* The spec says that all primary keys MUST be able to certify. */
  if (sig->sig_class != 0x18) buf[0] |= 0x01;

  if (use & PUBKEY_USAGE_SIG) buf[0] |= 0x02;
  if (use & PUBKEY_USAGE_ENC) buf[0] |= 0x04 | 0x08;
  if (use & PUBKEY_USAGE_AUTH) buf[0] |= 0x20;

  build_sig_subpkt(sig, SIGSUBPKT_KEY_FLAGS, buf, 1);
}

int keygen_add_key_expire(PKT_signature *sig, void *opaque) {
  PKT_public_key *pk = (PKT_public_key *)opaque;
  byte buf[8];
  u32 u;
...
static int keygen_add_key_flags_and_expire(PKT_signature *sig, void *opaque) {
  struct opaque_data_usage_and_pk *oduap = (opaque_data_usage_and_pk *)opaque;

  do_add_key_flags(sig, oduap->usage);
  return keygen_add_key_expire(sig, oduap->pk);
}

static int set_one_pref(int val, int type, const char *item, byte *buf,
                        int *nbuf) {
  int i;
  ...
  if (!string || !ascii_strcasecmp(string, "default")) {
    if (opt.def_preference_list)
      ...
      */
      ...
      if (!any_compress) strcat(dummy_string, "Z0 ");
  ...
  if (strlen(string)) {
    ...
    while ((tok = gpg_strsep(&prefstring, " ,"))) {
      if ((val = string_to_cipher_algo(tok))) {
        if (set_one_pref(val, 1, tok, sym, &nsym)) rc = -1;
      } else if ((val = string_to_digest_algo(tok))) {
        if (set_one_pref(val, 2, tok, hash, &nhash)) rc = -1;
      } else if ((val = string_to_compress_algo(tok)) > -1) {
        if (set_one_pref(val, 3, tok, zip, &nzip)) rc = -1;
...
PKT_user_id *keygen_get_std_prefs(void) {
  int i, j = 0;
  PKT_user_id *uid = (PKT_user_id *)xmalloc_clear(sizeof(PKT_user_id));

  if (!prefs_initialized) keygen_set_std_prefs(NULL, 0);
...
static void add_feature_mdc(PKT_signature *sig, int enabled) {
  const byte *s;
  size_t n;
  int i;
  char *buf;
  ...
  if (s && n && ((enabled && (s[0] & 0x01)) || (!enabled && !(s[0] & 0x01))))
    return;

  if (!s || !n) { /* create a new one */
    n = 1;
    buf = (char *)xmalloc_clear(n);
  } else {
    buf = (char *)xmalloc(n);
    memcpy(buf, s, n);
  }
...
static void add_keyserver_modify(PKT_signature *sig, int enabled) {
  const byte *s;
  size_t n;
  int i;
  char *buf;
  ...
  /* Already set or cleared */
  if (s && n && ((enabled && (s[0] & 0x80)) || (!enabled && !(s[0] & 0x80))))
    return;
...
int keygen_add_notations(PKT_signature *sig, void *opaque) {
  ...
  for (notation = (struct notation *)opaque; notation;
    ...
    if (!notation->flags.ignore) {
      ...
      buf = (unsigned char *)xmalloc(8 + n1 + n2);
...
int keygen_add_revkey(PKT_signature *sig, void *opaque) {
  struct revocation_key *revkey = (revocation_key *)opaque;
...
gpg_error_t make_backsig(ctrl_t ctrl, PKT_signature *sig, PKT_public_key *pk,
  ...
  err = make_keysig_packet(ctrl, &backsig, pk, NULL, sub_pk, sub_psk, 0x19, 0,
...
static gpg_error_t write_direct_sig(ctrl_t ctrl, kbnode_t root,
                                    PKT_public_key *psk,
                                    struct revocation_key *revkey,
                                    u32 timestamp, const char *cache_nonce) {
  gpg_error_t err;
  ...
  /* Make the signature.  */
  err = make_keysig_packet(ctrl, &sig, pk, NULL, NULL, psk, 0x1F, 0, timestamp,
                           0, keygen_add_revkey, revkey, cache_nonce);
  if (err) {
...
/* Write a self-signature to the first user id in ROOT using the key
   ...
   signature.  */
static gpg_error_t write_selfsigs(ctrl_t ctrl, kbnode_t root,
                                  PKT_public_key *psk, unsigned int use,
                                  u32 timestamp, const char *cache_nonce) {
  gpg_error_t err;
  PACKET *pkt;
  PKT_signature *sig;
  ...
  if (!node) BUG(); /* No user id packet in tree.  */
  ...
  /* Make the signature.  */
  err = make_keysig_packet(ctrl, &sig, pk, uid, NULL, psk, 0x13, 0, timestamp,
                           0, keygen_add_std_prefs, pk, cache_nonce);
  if (err) {
    log_error("make_keysig_packet failed: %s\n", gpg_strerror(err));
    return err;
  }
...
static int write_keybinding(ctrl_t ctrl, kbnode_t root, PKT_public_key *pri_psk,
                            PKT_public_key *sub_psk, unsigned int use,
                            u32 timestamp, const char *cache_nonce) {
  gpg_error_t err;
  PACKET *pkt;
  PKT_signature *sig;
  ...
  oduap.usage = use;
  oduap.pk = sub_pk;
  err = make_keysig_packet(ctrl, &sig, pri_pk, NULL, sub_pk, pri_psk, 0x18, 0,
                           timestamp, 0, keygen_add_key_flags_and_expire,
                           &oduap, cache_nonce);
  if (err) {
  ...
  if (use & PUBKEY_USAGE_SIG) {
    err = make_backsig(ctrl, sig, pri_pk, sub_pk, sub_psk, timestamp,
                       cache_nonce);
    if (err) return err;
  }
...
static gpg_error_t ecckey_from_sexp(gcry_mpi_t *array, gcry_sexp_t sexp,
                                    int algo) {
  gpg_error_t err;
  gcry_sexp_t list, l2;
  char *curve = NULL;
...
static int key_from_sexp(gcry_mpi_t *array, gcry_sexp_t sexp,
...
static int do_create_from_keygrip(ctrl_t ctrl, int algo, const char *hexkeygrip,
                                  kbnode_t pub_root, u32 timestamp,
  ...
  pk->version = 4;
  if (expireval) pk->expiredate = pk->timestamp + expireval;
  pk->pubkey_algo = algo;
...
static int common_gen(const char *keyparms, int algo, const char *algoelem,
                      kbnode_t pub_root, u32 timestamp, u32 expireval,
                      int is_subkey, int keygen_flags, const char *passphrase,
  ...
  err = agent_genkey(NULL, cache_nonce_addr, passwd_nonce_addr, keyparms,
                     !!(keygen_flags & KEYGEN_FLAG_NO_PROTECTION), passphrase,
                     &s_key);
  if (err) {
 ...
 */
static int gen_elg(int algo, unsigned int nbits, KBNODE pub_root, u32 timestamp,
                   u32 expireval, int is_subkey, int keygen_flags,
                   const char *passphrase, char **cache_nonce_addr,
  ...
  if (!keyparms)
    ...
    err = common_gen(keyparms, algo, "pgy", pub_root, timestamp, expireval,
                     is_subkey, keygen_flags, passphrase, cache_nonce_addr,
                     passwd_nonce_addr);
    xfree(keyparms);
  }
 ...
 * Generate an DSA key
 */
static gpg_error_t gen_dsa(unsigned int nbits, KBNODE pub_root, u32 timestamp,
                           u32 expireval, int is_subkey, int keygen_flags,
                           const char *passphrase, char **cache_nonce_addr,
                           char **passwd_nonce_addr) {
  int err;
  ...
  */
  ...
  if (qbits != 160)
    log_info(
        _("WARNING: some OpenPGP programs can't"
          " handle a DSA key with this digest size\n"));

  snprintf(nbitsstr, sizeof nbitsstr, "%u", nbits);
  snprintf(qbitsstr, sizeof qbitsstr, "%u", qbits);
  keyparms =
      xtryasprintf("(genkey(dsa(nbits %zu:%s)(qbits %zu:%s)%s))",
                   strlen(nbitsstr), nbitsstr, strlen(qbitsstr), qbitsstr,
                   ((keygen_flags & KEYGEN_FLAG_TRANSIENT_KEY) &&
                    (keygen_flags & KEYGEN_FLAG_NO_PROTECTION))
                       ? "(transient-key)"
                       : "");
  if (!keyparms)
    ...
    err = common_gen(keyparms, PUBKEY_ALGO_DSA, "pqgy", pub_root, timestamp,
                     expireval, is_subkey, keygen_flags, passphrase,
 ...
 * Generate an ECC key
 */
static gpg_error_t gen_ecc(int algo, const char *curve, kbnode_t pub_root,
  ...
  log_assert(algo == PUBKEY_ALGO_ECDSA || algo == PUBKEY_ALGO_EDDSA ||
             algo == PUBKEY_ALGO_ECDH);

  if (!curve || !*curve) return GPG_ERR_UNKNOWN_CURVE;

  /* Note that we use the "comp" flag with EdDSA to request the use of
     a 0x40 compression prefix octet.  */
  if (algo == PUBKEY_ALGO_EDDSA)
    keyparms = xtryasprintf("(genkey(ecc(curve %zu:%s)(flags eddsa comp%s)))",
                            strlen(curve), curve,
                            (((keygen_flags & KEYGEN_FLAG_TRANSIENT_KEY) &&
                              (keygen_flags & KEYGEN_FLAG_NO_PROTECTION))
                                 ? " transient-key"
    ...
    keyparms = xtryasprintf(
        "(genkey(ecc(curve %zu:%s)(flags djb-tweak comp%s)))", strlen(curve),
        curve, (((keygen_flags & KEYGEN_FLAG_TRANSIENT_KEY) &&
                 (keygen_flags & KEYGEN_FLAG_NO_PROTECTION))
                    ? " transient-key"
                    : ""));
  else
    keyparms = xtryasprintf("(genkey(ecc(curve %zu:%s)(flags nocomp%s)))",
                            strlen(curve), curve,
                            (((keygen_flags & KEYGEN_FLAG_TRANSIENT_KEY) &&
                              (keygen_flags & KEYGEN_FLAG_NO_PROTECTION))
                                 ? " transient-key"
  ...
  if (!keyparms)
    ...
    err = common_gen(keyparms, algo, "", pub_root, timestamp, expireval,
                     is_subkey, keygen_flags, passphrase, cache_nonce_addr,
                     passwd_nonce_addr);
    xfree(keyparms);
  }
 ...
 * Generate an RSA key.
 */
static int gen_rsa(int algo, unsigned int nbits, KBNODE pub_root, u32 timestamp,
                   u32 expireval, int is_subkey, int keygen_flags,
                   const char *passphrase, char **cache_nonce_addr,
                   char **passwd_nonce_addr) {
  int err;
  ...
  if (!nbits) nbits = get_keysize_range(algo, NULL, NULL);
  ...
  snprintf(nbitsstr, sizeof nbitsstr, "%u", nbits);
  keyparms =
      xtryasprintf("(genkey(rsa(nbits %zu:%s)%s))", strlen(nbitsstr), nbitsstr,
                   ((keygen_flags & KEYGEN_FLAG_TRANSIENT_KEY) &&
                    (keygen_flags & KEYGEN_FLAG_NO_PROTECTION))
                       ? "(transient-key)"
                       : "");
  if (!keyparms)
    ...
    err = common_gen(keyparms, algo, "ne", pub_root, timestamp, expireval,
                     is_subkey, keygen_flags, passphrase, cache_nonce_addr,
                     passwd_nonce_addr);
    xfree(keyparms);
  }
 ...
 */
static int check_valid_days(const char *s) {
  if (!digitp(s)) return 0;
  for (s++; *s; s++)
    if (!digitp(s)) break;
  if (!*s) return 1;
  if (s[1]) return 0; /* e.g. "2323wc" */
  if (*s == 'd' || *s == 'D') return 1;
  if (*s == 'w' || *s == 'W') return 7;
  if (*s == 'm' || *s == 'M') return 30;
  if (*s == 'y' || *s == 'Y') return 365;
  return 0;
}

static void print_key_flags(int flags) {
  if (flags & PUBKEY_USAGE_SIG) tty_printf("%s ", _("Sign"));

  if (flags & PUBKEY_USAGE_CERT) tty_printf("%s ", _("Certify"));

  if (flags & PUBKEY_USAGE_ENC) tty_printf("%s ", _("Encrypt"));

  if (flags & PUBKEY_USAGE_AUTH) tty_printf("%s ", _("Authenticate"));
}
...
unsigned int ask_key_flags(int algo, int subkey, unsigned int current) {
  ...
  */
  ...
  if (subkey) possible &= ~PUBKEY_USAGE_CERT;
  ...
  for (;;) {
    ...
    tty_printf(_("Possible actions for a %s key: "),
               (algo == PUBKEY_ALGO_ECDSA || algo == PUBKEY_ALGO_EDDSA)
                   ? "ECDSA/EdDSA"
    ...
    else if (*answer == '\0' || *answer == togglers[6] ||
             *answer == togglers[7])
      break;
...
static int check_keygrip(ctrl_t ctrl, const char *hexgrip) {
  gpg_error_t err;
  unsigned char *public_x;
  size_t publiclen;
  int algo;

  if (hexgrip[0] == '&') hexgrip++;
 ...
 * it. */
static int ask_algo(ctrl_t ctrl, int addmode, int *r_subkey_algo,
  ...
  if (!r_subkey_algo) r_subkey_algo = &dummy_algo;
  ...
  if (opt.compliance != CO_DE_VS) tty_printf(_("   (%d) DSA (sign only)\n"), 3);
#if GPG_USE_RSA
  ...
  if (opt.expert && !addmode) tty_printf(_("   (%d) ECC and ECC\n"), 9);
  if (opt.expert) tty_printf(_("  (%d) ECC (sign only)\n"), 10);
  if (opt.expert) tty_printf(_("  (%d) ECC (set your own capabilities)\n"), 11);
  if (opt.expert && addmode) tty_printf(_("  (%d) ECC (encrypt only)\n"), 12);
#endif

  if (opt.expert && r_keygrip) tty_printf(_("  (%d) Existing key\n"), 13);
  ...
  for (;;) {
    ...
    *r_subkey_algo = 0;
    ...
    if (opt.compliance == CO_DE_VS &&
      ...
      *r_usage = ask_key_flags(algo, addmode, 0);
      break;
    } else if ((algo == 9 || !strcmp(answer, "ecc+ecc")) && opt.expert &&
               !addmode) {
      ...
      *r_usage = ask_key_flags(algo, addmode, 0);
      break;
    } else if ((algo == 12 || !strcmp(answer, "ecc/e")) && opt.expert &&
               addmode) {
      algo = PUBKEY_ALGO_ECDH;
      *r_usage = PUBKEY_USAGE_ENC;
      break;
    } else if ((algo == 13 || !strcmp(answer, "keygrip")) && opt.expert &&
               r_keygrip) {
      for (;;) {
        xfree(answer);
  ...
  xfree(answer);
  if (r_keygrip) *r_keygrip = keygrip;
  return algo;
}
...
static unsigned int fixup_keysize(unsigned int nbits, int algo, int silent) {
  if (algo == PUBKEY_ALGO_DSA && (nbits % 64)) {
    nbits = ((nbits + 63) / 64) * 64;
    if (!silent) tty_printf(_("rounded up to %u bits\n"), nbits);
    ...
    if (nbits != 255 && nbits != 441) {
      if (nbits < 256)
        ...
        nbits = 441;
      if (!silent) tty_printf(_("rounded to %u bits\n"), nbits);
    }
...
/* Ask for the key size.  ALGO is the algorithm.  If PRIMARY_KEYSIZE
   ...
   subkey. */
static unsigned ask_keysize(int algo, unsigned int primary_keysize) {
  unsigned int nbits;
...
static char *ask_curve(int *algo, int *subkey_algo) {
...
#undef MY_USE_ECDSADH
  ...
  for (idx = 0; idx < DIM(curves); idx++) {
    ...
    if (!curves[idx].supported) continue;

    if (opt.compliance == CO_DE_VS) {
      if (!curves[idx].de_vs) continue; /* Not allowed.  */
    ...
    rc = gcry_sexp_build(&keyparms, NULL, "(public-key(ecc(curve %s)))",
                         curves[idx].eddsa_curve ? curves[idx].eddsa_curve
                                                 /**/
                                                 : curves[idx].name);
    if (rc) continue;
    if (!gcry_pk_get_curve(keyparms, 0, NULL)) continue;
    ...
    tty_printf(
        "   (%d) %s\n", idx + 1,
        curves[idx].pretty_name ? curves[idx].pretty_name : curves[idx].name);
  }
  gcry_sexp_release(keyparms);
  ...
  if (!result) result = xstrdup(curves[0].name);
...
static u32 parse_creation_string(const char *string) {
  u32 seconds;
...
u32 ask_expire_interval(int object, const char *def_expire) {
  ...
  switch (object) {
    case 0:
      if (def_expire) BUG();
      tty_printf(
          _("Please specify how long the key should be valid.\n"
    ...
    case 1:
      if (!def_expire) BUG();
      tty_printf(
          _("Please specify how long the signature should be valid.\n"
  ...
  for (;;) {
    ...
    if (object == 0)
      ...
      if (*answer == '\0') answer = xstrdup(def_expire);
    }
    cpr_kill_prompt();
    trim_spaces(answer);
...
static PKT_user_id *uid_from_string(const char *string) {
  size_t n;
  PKT_user_id *uid;

  n = strlen(string);
  uid = (PKT_user_id *)xmalloc_clear(sizeof *uid + n);
  uid->len = n;
...
static int uid_already_in_keyblock(kbnode_t keyblock, const char *uid) {
  PKT_user_id *uidpkt = uid_from_string(uid);
  kbnode_t node;
  int result = 0;

  for (node = keyblock; node && !result; node = node->next)
    if (!is_deleted_kbnode(node) && node->pkt->pkttype == PKT_USER_ID &&
...
static char *ask_user_id(int mode, int full, KBNODE keyblock) {
  char *answer;
  ...
  if (!mode) {
    ...
    if (!strcmp(s1, s2)) {
      ...
      const char *s4 = _(s3);
      if (strcmp(s3, s4)) s2 = s3; /* A translation exists - use it. */
    }
    tty_printf("%s", s2);
  }
  ...
  for (;;) {
    ...
    if (!aname) {
      for (;;) {
        ...
        if (opt.allow_freeform_uid) break;
    ...
    if (!*amail && !opt.allow_freeform_uid &&
      ...
      fail = 1;
      tty_printf(
          _("Please don't put the email address "
    ...
    for (;;) {
      ...
      if (cpr_enabled()) {
        ...
        answer = cpr_get(
            "keygen.userid.cmd",
            fail ? _("Change (N)ame, (C)omment, (E)mail or (Q)uit? ")
                 : _("Change (N)ame, (C)omment, (E)mail or (O)kay/(Q)uit? "));
        cpr_kill_prompt();
      } else {
        answer =
            cpr_get("keygen.userid.cmd",
                    fail ? _("Change (N)ame, (E)mail, or (Q)uit? ")
...
static int do_create(int algo, unsigned int nbits, const char *curve,
                     KBNODE pub_root, u32 timestamp, u32 expiredate,
                     int is_subkey, int keygen_flags, const char *passphrase,
                     char **cache_nonce_addr, char **passwd_nonce_addr) {
  gpg_error_t err;
  ...
  if (algo == PUBKEY_ALGO_ELGAMAL_E)
    err =
        gen_elg(algo, nbits, pub_root, timestamp, expiredate, is_subkey,
                keygen_flags, passphrase, cache_nonce_addr, passwd_nonce_addr);
  else if (algo == PUBKEY_ALGO_DSA)
    err =
        gen_dsa(nbits, pub_root, timestamp, expiredate, is_subkey, keygen_flags,
                passphrase, cache_nonce_addr, passwd_nonce_addr);
  else if (algo == PUBKEY_ALGO_ECDSA || algo == PUBKEY_ALGO_EDDSA ||
           algo == PUBKEY_ALGO_ECDH)
    err =
        gen_ecc(algo, curve, pub_root, timestamp, expiredate, is_subkey,
                keygen_flags, passphrase, cache_nonce_addr, passwd_nonce_addr);
  else if (algo == PUBKEY_ALGO_RSA)
    err =
        gen_rsa(algo, nbits, pub_root, timestamp, expiredate, is_subkey,
                keygen_flags, passphrase, cache_nonce_addr, passwd_nonce_addr);
  else
    BUG();
...
/* Generate a new user id packet or return NULL if canceled.  If
   ...
   etc.  */
PKT_user_id *generate_user_id(KBNODE keyblock, const char *uidstr) {
  PKT_user_id *uid;
  char *p;
 ...
 * subkey.  */
static gpg_error_t parse_key_parameter_part(char *string, int for_subkey,
  ...
  if (!string || !*string) return 0; /* Success.  */
  ...
  if (algo) {
    if (!string[3])
      ...
      if (size < 512 || size > 16384 || *endp) return GPG_ERR_INV_VALUE;
    }
  ...
  if (flags) {
    ...
    for (i = 0; (s = tokens[i]); i++) {
      ...
      else if (!ascii_strcasecmp(s, "encrypt") || !ascii_strcasecmp(s, "encr"))
  ...
  if (!for_subkey) keyuse |= PUBKEY_USAGE_CERT;

  /* Check that usage is actually possible.  */
  if (/**/ ((keyuse &
             (PUBKEY_USAGE_SIG | PUBKEY_USAGE_AUTH | PUBKEY_USAGE_CERT)) &&
  ...
  if (r_keyuse) *r_keyuse = keyuse;
  if (r_curve) *r_curve = curve;
 ...
 *
 */
gpg_error_t parse_key_parameter_string(const char *string, int part,
                                       int *r_algo, unsigned int *r_size,
                                       unsigned *r_keyuse, char const **r_curve,
  ...
  if (r_algo) *r_algo = 0;
  if (r_size) *r_size = 0;
  if (r_keyuse) *r_keyuse = 0;
  if (r_curve) *r_curve = NULL;
  if (r_subalgo) *r_subalgo = 0;
  if (r_subsize) *r_subsize = 0;
  if (r_subkeyuse) *r_subkeyuse = 0;
  if (r_subcurve) *r_subcurve = NULL;

  if (!string || !*string || !strcmp(string, "default") || !strcmp(string, "-"))
  ...
  if (part == -1 || part == 0) {
    err =
        parse_key_parameter_part(primary, 0, r_algo, r_size, r_keyuse, r_curve);
    if (!err && part == -1)
    ...
    err = parse_key_parameter_part(secondary ? secondary : primary, 1, r_algo,
                                   r_size, r_keyuse, r_curve);
  }
...
static void append_to_parameter(struct para_data_s *para,
                                struct para_data_s *r) {
  log_assert(para);
  while (para->next) para = para->next;
  para->next = r;
}
...
static struct para_data_s *get_parameter(struct para_data_s *para,
                                         enum para_name key) {
  struct para_data_s *r;
...
static const char *get_parameter_value(struct para_data_s *para,
                                       enum para_name key) {
...
static const char *get_parameter_passphrase(struct para_data_s *para) {
  struct para_data_s *r = get_parameter(para, pPASSPHRASE);
  return r ? r->u.value : NULL;
}

static int get_parameter_algo(struct para_data_s *para, enum para_name key,
                              int *r_default) {
  int i;
  struct para_data_s *r = get_parameter(para, key);

  if (r_default) *r_default = 0;
  ...
  if (!ascii_strcasecmp(r->u.value, "default")) {
    ...
    parse_key_parameter_string(NULL, 0, &i, NULL, NULL, NULL, NULL, NULL, NULL,
                               NULL);

    if (r_default) *r_default = 1;
  } else if (digitp(r->u.value))
    i = atoi(r->u.value);
  else if (!strcmp(r->u.value, "ELG-E") || !strcmp(r->u.value, "ELG"))
...
static int parse_usagestr(const char *usagestr) {
  gpg_error_t err;
  char **tokens = NULL;
  const char *s;
  int i;
 ...
 */
static int parse_parameter_usage(const char *fname, struct para_data_s *para,
                                 enum para_name key) {
  struct para_data_s *r = get_parameter(para, key);
  int i;

  if (!r) return 0; /* none (this is an optional parameter)*/
...
static int parse_revocation_key(const char *fname, struct para_data_s *para,
                                enum para_name key) {
  struct para_data_s *r = get_parameter(para, key);
  struct revocation_key revkey;
  char *pn;
  ...
  /* skip to the tag */
  while (*pn && *pn != 's' && *pn != 'S') pn++;

  if (ascii_strcasecmp(pn, "sensitive") == 0) revkey.klasse |= 0x40;
...
static u32 get_parameter_u32(struct para_data_s *para, enum para_name key) {
  struct para_data_s *r = get_parameter(para, key);

  if (!r) return 0;
  if (r->key == pKEYCREATIONDATE) return r->u.creation;
  if (r->key == pKEYEXPIRE || r->key == pSUBKEYEXPIRE) return r->u.expire;
  if (r->key == pKEYUSAGE || r->key == pSUBKEYUSAGE) return r->u.usage;

  return (unsigned int)strtoul(r->u.value, NULL, 10);
}

static unsigned int get_parameter_uint(struct para_data_s *para,
                                       enum para_name key) {
  return get_parameter_u32(para, key);
}

static struct revocation_key *get_parameter_revkey(struct para_data_s *para,
                                                   enum para_name key) {
  struct para_data_s *r = get_parameter(para, key);
  return r ? &r->u.revkey : NULL;
}

static int proc_parameter_file(ctrl_t ctrl, struct para_data_s *para,
                               const char *fname,
                               struct output_control_s *outctrl, int card) {
  struct para_data_s *r;
  const char *s1, *s2, *s3;
  ...
  if (!err) {
    ...
    r = (para_data_s *)xmalloc_clear(sizeof(*r));
    r->key = pKEYUSAGE;
    r->u.usage = (is_default ? (PUBKEY_USAGE_CERT | PUBKEY_USAGE_SIG)
  ...
  if (r) {
    ...
    if (!err) {
      ...
      r->key = pSUBKEYUSAGE;
      r->u.usage =
          (is_default ? PUBKEY_USAGE_ENC : openpgp_pk_algo_usage(algo));
      append_to_parameter(para, r);
    } else if (err == -1)
      return -1;
    else {
      ...
      if (r && (r->u.usage & ~openpgp_pk_algo_usage(algo))) {
        log_error(
            "%s:%d: specified Subkey-Usage not allowed"
            " for algo %d\n",
            fname, r->lnr, algo);
        return -1;
      }
  ...
  if (get_parameter_value(para, pUSERID))
    ...
    if (s1 || s2 || s3) {
      ...
      r = (para_data_s *)xmalloc_clear(sizeof *r + n + 20);
      r->key = pUSERID;
      p = r->u.value;
      if (s1) p = stpcpy(p, s1);
      if (s2) p = stpcpy(stpcpy(stpcpy(p, " ("), s2), ")");
      if (s3) p = stpcpy(stpcpy(stpcpy(p, " <"), s3), ">");
      append_to_parameter(para, r);
      have_user_id = 1;
    }
  }
  ...
  if (r && *r->u.value) {
    ...
    r = (para_data_s *)xmalloc_clear(sizeof *r + 20);
    r->key = pSUBKEYEXPIRE;
    r->u.expire = seconds;
    append_to_parameter(para, r);
  }
 ...
 */
static void read_parameter_file(ctrl_t ctrl, const char *fname) {
  static struct {
    const char *name;
    enum para_name key;
  } keywords[] = {{"Key-Type", (para_name)pKEYTYPE},
                  {"Key-Length", (para_name)pKEYLENGTH},
                  {"Key-Curve", (para_name)pKEYCURVE},
                  {"Key-Usage", (para_name)pKEYUSAGE},
                  {"Subkey-Type", (para_name)pSUBKEYTYPE},
                  {"Subkey-Length", (para_name)pSUBKEYLENGTH},
                  {"Subkey-Curve", (para_name)pSUBKEYCURVE},
                  {"Subkey-Usage", (para_name)pSUBKEYUSAGE},
                  {"Name-Real", (para_name)pNAMEREAL},
                  {"Name-Email", (para_name)pNAMEEMAIL},
                  {"Name-Comment", (para_name)pNAMECOMMENT},
                  {"Expire-Date", (para_name)pEXPIREDATE},
                  {"Creation-Date", (para_name)pCREATIONDATE},
                  {"Passphrase", (para_name)pPASSPHRASE},
  ...
  while (iobuf_read_line(fp, &line, &nline, &maxlen)) {
    ...
    for (p = (char *)line; isspace(*(byte *)p); p++)
      ;
    if (!*p || *p == '#') continue;
    keyword = p;
    if (*keyword == '%') {
    ...
    for (i = 0; keywords[i].name; i++) {
      if (!ascii_strcasecmp(keywords[i].name, keyword)) break;
    }
    if (!keywords[i].name) {
      err = "unknown keyword";
      break;
    ...
    r = (para_data_s *)xmalloc_clear(sizeof *r + strlen(value));
    r->lnr = lnr;
...
static struct para_data_s *quickgen_set_para(struct para_data_s *para,
                                             int for_subkey, int algo,
                                             int nbits, const char *curve,
                                             unsigned int use) {
  struct para_data_s *r;

  r = (para_data_s *)xmalloc_clear(sizeof *r + 30);
  ...
  if (curve) {
    r = (para_data_s *)xmalloc_clear(sizeof *r + strlen(curve));
...
void quick_generate_keypair(ctrl_t ctrl, const char *uid, const char *algostr,
  ...
  use_tty = (!opt.batch && !opt.answer_yes && !*algostr && !*usagestr &&
             !*expirestr && !cpr_enabled() && gnupg_isatty(fileno(stdin)) &&
             gnupg_isatty(fileno(stdout)) && gnupg_isatty(fileno(stderr)));

  r = (para_data_s *)xmalloc_clear(sizeof *r + strlen(uid));
  r->key = pUSERID;
  ...
  if (use_tty) {
    ...
    if (!cpr_get_answer_is_yes_def("quick_keygen.okay", _("Continue? (Y/n) "),
                                   1))
      goto leave;
  }
  ...
  if ((!*algostr || !strcmp(algostr, "default") ||
      ...
      (!*usagestr || !strcmp(usagestr, "default") || !strcmp(usagestr, "-"))) {
    ...
    err = parse_key_parameter_string(algostr, -1, &algo, &size, &keyuse, &curve,
                                     &subalgo, &subsize, &subkeyuse, &subcurve);
    if (err) {
      log_error(_("Key generation failed: %s\n"), gpg_strerror(err));
      goto leave;
    }
    ...
    if (subalgo)
      para = quickgen_set_para(para, 1, subalgo, subsize, subcurve, subkeyuse);
    ...
    err = parse_algo_usage_expire(ctrl, 0, algostr, usagestr, expirestr, &algo,
                                  &use, &expire, &nbits, &curve);
    if (err) {
      log_error(_("Key generation failed: %s\n"), gpg_strerror(err));
      goto leave;
    }
...
void generate_keypair(ctrl_t ctrl, int full, const char *fname,
  ...
  if (card_serialno) {
    ...
    r = (para_data_s *)xcalloc(1, sizeof *r + strlen(card_serialno));
    r->key = pSERIALNO;
    strcpy(r->u.value, card_serialno);
    r->next = para;
    para = r;

    r = (para_data_s *)xcalloc(1, sizeof *r + 20);
    r->key = pKEYTYPE;
    ...
    if (info.key_attr[1].algo == PUBKEY_ALGO_RSA) {
      ...
      r = (para_data_s *)xcalloc(1, sizeof *r + strlen(info.key_attr[1].curve));
      r->key = pSUBKEYCURVE;
    ...
    if (card_backup_key) {
      r = (para_data_s *)xcalloc(1, sizeof *r + 1);
  ...
  } else if (full) /* Full featured key generation.  */
  {
    ...
    if (key_from_hexgrip) {
      ...
      if (use) {
        r = (para_data_s *)xmalloc_clear(sizeof *r + 25);
        r->key = pKEYUSAGE;
        sprintf(r->u.value, "%s%s%s", (use & PUBKEY_USAGE_SIG) ? "sign " : "",
      ...
      r = (para_data_s *)xmalloc_clear(sizeof *r + 40);
      r->key = pKEYGRIP;
      ...
      if (subkey_algo) {
        ...
        if (algo == PUBKEY_ALGO_ECDSA || algo == PUBKEY_ALGO_EDDSA ||
            algo == PUBKEY_ALGO_ECDH) {
          if (algo == PUBKEY_ALGO_EDDSA && subkey_algo == PUBKEY_ALGO_ECDH) {
  ...
  } else /* Default key generation.  */
  {
    ...
    tty_printf(_("Note: Use \"%s %s\""
               ...
               GPG_NAME "2", "--full-generate-key");

    err = parse_key_parameter_string(NULL, -1, &algo, &size, &keyuse, &curve,
                                     &subalgo, &subsize, &subkeyuse, &subcurve);
    if (err) {
      log_error(_("Key generation failed: %s\n"), gpg_strerror(err));
      return;
  ...
  r = (para_data_s *)xcalloc(1, sizeof *r + strlen(uid));
  r->key = pUSERID;
  strcpy(r->u.value, uid);
  r->next = para;
  para = r;
...
static void start_tree(KBNODE *tree) {
  PACKET *pkt;
...
static gpg_error_t card_write_key_to_backup_file(PKT_public_key *sk,
  ...
  if (rc) {
    ...
    write_status_text_and_buffer(STATUS_BACKUP_KEY_CREATED, fprbuf, fname,
                                 strlen(fname), 0);
    xfree(fprbuf);
  }
...
static gpg_error_t card_store_key_with_backup(ctrl_t ctrl,
                                              PKT_public_key *sub_psk,
                                              const char *backup_dir) {
  PKT_public_key *sk;
  ...
  if (!sk) return gpg_error_from_syserror();
  ...
  err = receive_seckey_from_agent(ctrl, 0, &cache_nonce, hexgrip, sk);
  if (err) {
...
static void do_generate_keypair(ctrl_t ctrl, struct para_data_s *para,
  ...
  if (!timestamp) timestamp = make_timestamp();
  ...
  if (key_from_hexgrip)
    err = do_create_from_keygrip(ctrl, algo, key_from_hexgrip, pub_root,
                                 timestamp, expire, 0);
  else if (!card)
    err = do_create(algo, get_parameter_uint(para, pKEYLENGTH),
                    get_parameter_value(para, pKEYCURVE), pub_root, timestamp,
                    expire, 0, outctrl->keygen_flags,
                    get_parameter_passphrase(para), &cache_nonce, NULL);
  else
    err = gen_card_key(1, algo, 1, pub_root, &timestamp, expire);
  ...
  if (!err && (revkey = get_parameter_revkey(para, pREVOKER)))
    err = write_direct_sig(ctrl, pub_root, pri_psk, revkey, timestamp,
                           cache_nonce);
  ...
  if (!err && card && get_parameter(para, pAUTHKEYTYPE)) {
    err =
        gen_card_key(3, get_parameter_algo(para, pAUTHKEYTYPE, NULL), 0,
                     pub_root, &timestamp, get_parameter_u32(para, pKEYEXPIRE));
    if (!err)
      err = write_keybinding(ctrl, pub_root, pri_psk, NULL, PUBKEY_USAGE_AUTH,
  ...
  if (!err && get_parameter(para, pSUBKEYTYPE)) {
    ...
    if (!card || (s = get_parameter_value(para, pCARDBACKUPKEY))) {
      err = do_create(get_parameter_algo(para, pSUBKEYTYPE, NULL),
                      ...
                      get_parameter_value(para, pSUBKEYCURVE), pub_root,
                      timestamp, get_parameter_u32(para, pSUBKEYEXPIRE), 1,
      ...
      if (!err) {
        ...
        if (s) err = card_store_key_with_backup(ctrl, sub_psk, gnupg_homedir());
      }
    } else {
      err = gen_card_key(2, get_parameter_algo(para, pSUBKEYTYPE, NULL), 0,
                         pub_root, &timestamp,
    ...
    if (!err)
      err = write_keybinding(ctrl, pub_root, pri_psk, sub_psk,
                             get_parameter_uint(para, pSUBKEYUSAGE), timestamp,
                             cache_nonce);
    did_sub = 1;
  }
  ...
  } else if (!err) /* Write to the standard keyrings.  */
  {
    ...
    if (!err) {
      ...
      no_enc_rsa =
          ((get_parameter_algo(para, pKEYTYPE, NULL) == PUBKEY_ALGO_RSA) &&
      ...
      update_ownertrust(ctrl, pk, ((get_ownertrust(ctrl, pk) & ~TRUST_MASK) |
                                   TRUST_ULTIMATE));
      ...
      if (!opt.batch &&
        ...
        tty_printf(
            _("Note that this key cannot be used for "
  ...
  if (err) {
    ...
    PKT_public_key *pk =
        find_kbnode(pub_root, PKT_PUBLIC_KEY)->pkt->pkt.public_key;
...
static gpg_error_t parse_algo_usage_expire(
  ...
  *r_curve = NULL;
  ...
  err = parse_key_parameter_string(algostr, for_subkey ? 1 : 0, &algo, &nbits,
                                   &use, &curve, NULL, NULL, NULL, NULL);
  if (err) return err;

  /* Parse the usage string.  */
  if (!usagestr || !*usagestr || !strcmp(usagestr, "default") ||
      !strcmp(usagestr, "-"))
  ...
  if (!for_subkey) use |= PUBKEY_USAGE_CERT;
  ...
  if (/**/ (
          ...
          !pubkey_get_nsig((pubkey_algo_t)(algo))) ||
      ((use & PUBKEY_USAGE_ENC) && !pubkey_get_nenc((pubkey_algo_t)(algo))) ||
      (for_subkey && (use & PUBKEY_USAGE_CERT)))
    return GPG_ERR_WRONG_KEY_USAGE;

  /* Parse the expire string.  */
  expire = parse_expire_string(expirestr);
  if (expire == (u32)-1) return GPG_ERR_INV_VALUE;

  if (curve) {
    *r_curve = xtrystrdup(curve);
    if (!*r_curve) return gpg_error_from_syserror();
  }
  *r_algo = algo;
  *r_usage = use;
  *r_expire = expire;
...
gpg_error_t generate_subkeypair(ctrl_t ctrl, kbnode_t keyblock,
  ...
  if (pri_psk->version < 4) {
    log_info(
        _("Note: creating subkeys for v3 keys "
  ...
  if (interactive) {
    ...
    if (!cpr_enabled() &&
        !cpr_get_answer_is_yes("keygen.sub.okay", _("Really create? (y/N) "))) {
      err = GPG_ERR_CANCELED;
      goto leave;
    }
  } else /* Unattended mode.  */
  {
    err = parse_algo_usage_expire(ctrl, 1, algostr, usagestr, expirestr, &algo,
                                  &use, &expire, &nbits, &curve);
    if (err) goto leave;
  }
   ...
   * that of the primary key.  */
  {
    ...
    err = agent_passwd(ctrl, hexgrip, desc, 1 /*=verify*/, &cache_nonce,
                       &passwd_nonce);
    xfree(desc);
    if (err) goto leave;
  }
  ...
  /* Start creation.  */
  if (key_from_hexgrip) {
    err = do_create_from_keygrip(ctrl, algo, key_from_hexgrip, keyblock,
                                 cur_time, expire, 1);
  } else {
    const char *passwd;
    ...
    err = do_create(algo, nbits, curve, keyblock, cur_time, expire, 1, 0,
...
leave:
  ...
  xfree(cache_nonce);
  xfree(passwd_nonce);
  if (err) log_error(_("Key generation failed: %s\n"), gpg_strerror(err));
  return err;
}
...
gpg_error_t generate_card_subkeypair(ctrl_t ctrl, kbnode_t pub_keyblock,
  ...
  para = (para_data_s *)xtrycalloc(1, sizeof *para + strlen(serialno));
  if (!para) {
    err = gpg_error_from_syserror();
    goto leave;
  }
  ...
  if (keyno == 1)
    ...
    use = PUBKEY_USAGE_AUTH;
  if (!cpr_enabled() &&
      !cpr_get_answer_is_yes("keygen.cardsub.okay",
                             _("Really create? (y/N) "))) {
    err = GPG_ERR_CANCELED;
    goto leave;
  }
  ...
  if (!err) {
    for (node = pub_keyblock; node; node = node->next)
      if (node->pkt->pkttype == PKT_PUBLIC_SUBKEY)
        sub_pk = node->pkt->pkt.public_key;
    log_assert(sub_pk);
    err = write_keybinding(ctrl, pub_keyblock, pri_pk, sub_pk, use, cur_time,
                           NULL);
  }
...
static gpg_error_t gen_card_key(int keyno, int algo, int is_primary,
  ...
  pk = (PKT_public_key *)xtrycalloc(1, sizeof *pk);
  if (!pk) return gpg_error_from_syserror();
  pkt = (PACKET *)xtrycalloc(1, sizeof *pkt);
  if (!pkt) {
    xfree(pk);
