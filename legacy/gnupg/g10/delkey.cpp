 ...
 */
static gpg_error_t do_delete_key(ctrl_t ctrl, const char *username, int secret,
                                 int force, int *r_sec_avail) {
  gpg_error_t err;
  ...
  *r_sec_avail = 0;
  ...
  if (!err) err = keydb_search(hd, &desc, 1, NULL);
  if (err) {
  ...
  if (okay) {
    if (secret) {
      ...
      for (kbctx = NULL; (node = walk_kbnode(keyblock, &kbctx, 0));) {
        ...
        prompt = gpg_format_keydesc(ctrl, node->pkt->pkt.public_key,
        ...
        if (!err) err = agent_delete_key(NULL, hexgrip, prompt, opt.answer_yes);
        xfree(prompt);
        xfree(hexgrip);
        if (err) {
          if (err == GPG_ERR_KEY_ON_CARD)
            write_status_text(STATUS_DELETE_PROBLEM, "1");
          log_error(
              _("deleting secret %s failed: %s\n"),
              (node->pkt->pkttype == PKT_PUBLIC_KEY ? _("key") : _("subkey")),
              gpg_strerror(err));
    ...
    if (!secret && pk && clear_ownertrusts(ctrl, pk)) {
      if (opt.verbose) log_info(_("ownertrust information cleared\n"));
    }
  }
  ...
  for (auto &name : names) {
    ...
    if (err && avail) {
      if (allow_both) {
        ...
        if (!err) err = do_delete_key(ctrl, name.c_str(), 0, 0, &avail);
      } else {
        log_error(_("there is a secret key for public key \"%s\"!\n"),
                  name.c_str());
        log_info(
            _("use option \"--delete-secret-keys\" to delete"
              " it first.\n"));
