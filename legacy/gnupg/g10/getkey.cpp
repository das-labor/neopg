...
static kbnode_t finish_lookup(kbnode_t keyblock, unsigned int req_usage,
                              int want_exact, unsigned int *r_flags);
...
void cache_public_key(PKT_public_key *pk) {
  ...
  if (pk_cache_disabled) return;

  if (pk->flags.dont_cache) return;

  if (is_ELGAMAL(pk->pubkey_algo) || pk->pubkey_algo == PUBKEY_ALGO_DSA ||
      pk->pubkey_algo == PUBKEY_ALGO_ECDSA ||
      pk->pubkey_algo == PUBKEY_ALGO_EDDSA ||
      pk->pubkey_algo == PUBKEY_ALGO_ECDH || is_RSA(pk->pubkey_algo)) {
  ...
  for (ce = pk_cache; ce; ce = ce->next)
    if (ce->keyid[0] == keyid[0] && ce->keyid[1] == keyid[1]) {
      if (DBG_CACHE) log_debug("cache_public_key: already in cache\n");
      return;
    }
...
static const char *get_primary_uid(KBNODE keyblock, size_t *uidlen) {
  KBNODE k;
  const char *s;
...
static void release_keyid_list(keyid_list_t k) {
  while (k) {
 ...
 */
static void cache_user_id(KBNODE keyblock) {
  user_id_db_t r;
  const char *uid;
  size_t uidlen;
      ...
      for (r = user_id_db; r; r = r->next) {
        ...
        for (b = r->keyids; b; b = b->next) {
          if (!memcmp(b->fpr, a->fpr, MAX_FINGERPRINT_LEN)) {
            if (DBG_CACHE) log_debug("cache_user_id: already in cache\n");
...
gpg_error_t get_pubkeys(ctrl_t ctrl, char *search_terms, int use,
                        int include_unusable, char *source,
                        int warn_possibly_ambiguous, pubkey_t *r_keys) {
  ...
  if (DBG_LOOKUP) {
    log_debug("\n");
    log_debug("%s: Checking %s=%s\n", __func__, source ? source : "user input",
              search_terms);
  }

  if (*r_keys) log_bug("%s: KEYS should be NULL!\n", __func__);
  ...
  /* No match.  */
  {
    if (DBG_LOOKUP) log_debug("%s: '%s' not found.\n", __func__, search_terms);
  ...
  for (r = results; r; r = r->next) {
    ...
    if (dups) {
      hexfingerprint(r->keyblock->pkt->pkt.public_key, fingerprint,
                     sizeof fingerprint);
      if (key_dups.count(fingerprint) == 0) {
        ...
        log_info(_("Warning: %s appears in the keyring %d times\n"),
                 format_hexfingerprint(fingerprint, fingerprint_formatted,
...
static void pk_from_block(PKT_public_key *pk, kbnode_t keyblock,
                          kbnode_t found_key) {
...
leave:
  if (!rc) cache_public_key(pk);
  if (internal) free_public_key(pk);
  return rc;
}
...
#if MAX_PK_CACHE_ENTRIES
  {
    ...
    for (ce = pk_cache; ce; ce = ce->next) {
      if (ce->keyid[0] == keyid[0] && ce->keyid[1] == keyid[1]
        ...
        if (pk) copy_public_key(pk, ce->pk);
        return 0;
      }
    }
  }
...
gpg_error_t get_seckey(ctrl_t ctrl, PKT_public_key *pk, u32 *keyid) {
  ...
  ctx.not_allocated = 1;
  ctx.kr_handle = keydb_new();
  if (!ctx.kr_handle) return gpg_error_from_syserror();
  ctx.items.emplace_back();
  ctx.items.back().mode = KEYDB_SEARCH_MODE_LONG_KID;
  ctx.items.back().u.kid[0] = keyid[0];
  ctx.items.back().u.kid[1] = keyid[1];
  ...
  if (!err) {
    ...
    if (err) release_public_key_parts(pk);
  }
...
static int skip_unusable(void *opaque, u32 *keyid, int uid_no) {
  ...
  if (pk->flags.revoked || pk->has_expired) unusable = 1;
  ...
  if (!unusable && uid_no) {
    ...
    for (node = keyblock; node; node = node->next) {
      if (node->pkt->pkttype == PKT_USER_ID) {
        ...
        uids_seen++;
        if (uids_seen != uid_no) continue;

        if (user_id->flags.revoked || user_id->flags.expired) unusable = 1;
  ...
  if (!unusable) unusable = pk_is_disabled(pk);
...
static int key_byname(ctrl_t ctrl, GETKEY_CTX *retctx,
                      const std::vector<std::string> &namelist,
                      PKT_public_key *pk, int want_secret, int include_unusable,
  ...
  /* No search terms: iterate over the whole DB.  */
  {
    ...
    for (auto &name : namelist) {
      ...
      if (ctx->items[n].exact) ctx->exact = 1;
      if (err) {
        xfree(ctx);
        return err;
      }
...
int get_pubkey_byname(ctrl_t ctrl, GETKEY_CTX *retctx, PKT_public_key *pk,
  ...
  if (!no_akl) {
    ...
    for (akl = opt.auto_key_locate; akl; akl = akl->next)
      if (akl->type != AKL_NODEFAULT) {
        if (akl->type == AKL_LOCAL) anylocalfirst = 1;
        break;
      }
  }
  ...
  if (nodefault && is_mbox) {
    ...
    rc = key_byname(ctrl, retctx, namelist, pk, 0, include_unusable,
    ...
    for (akl = opt.auto_key_locate; akl; akl = akl->next) {
      ...
      switch (akl->type) {
        ...
        case AKL_LOCAL:
          ...
          namelist.emplace(namelist.begin(), name);
          rc = key_byname(ctrl, anylocalfirst ? retctx : NULL, namelist, pk, 0,
      ...
      } else if (!rc && !fpr && !did_akl_local) { /* The acquisition method said
                                                   * no failure occurred, but
                                                   * it didn't return a
                                                   * fingerprint.  That's a
                                                   * failure.  */
        no_fingerprint = 1;
        rc = GPG_ERR_NO_PUBKEY;
      }
      ...
      if (!rc &&
          !did_akl_local) { /* There was no error and we didn't do a local
                             * lookup.
...
static int key_is_ok(const PKT_public_key *key) {
  return (!key->has_expired && !key->flags.revoked && key->flags.valid &&
          !key->flags.disabled);
}

static int uid_is_ok(const PKT_public_key *key, const PKT_user_id *uid) {
  return key_is_ok(key) && !uid->flags.revoked;
}

static int subkey_is_ok(const PKT_public_key *sub) {
...
static int pubkey_cmp(ctrl_t ctrl, const char *name,
                      struct pubkey_cmp_cookie *old,
                      struct pubkey_cmp_cookie *neu, KBNODE new_keyblock) {
  kbnode_t n;

  neu->creation_time = 0;
  for (n = find_next_kbnode(new_keyblock, PKT_PUBLIC_SUBKEY); n;
       n = find_next_kbnode(n, PKT_PUBLIC_SUBKEY)) {
    PKT_public_key *sub = n->pkt->pkt.public_key;

    if ((sub->pubkey_usage & PUBKEY_USAGE_ENC) == 0) continue;

    if (!subkey_is_ok(sub)) continue;

    if (sub->timestamp > neu->creation_time)
      neu->creation_time = sub->timestamp;
  }

  for (n = find_next_kbnode(new_keyblock, PKT_USER_ID); n;
    ...
    if (!old->valid) return -1; /* No OLD key.  */
 ...
 * result is returned.  */
int get_best_pubkey_byname(ctrl_t ctrl, GETKEY_CTX *retctx, PKT_public_key *pk,
                           const char *name, KBNODE *ret_keyblock,
  ...
  rc = get_pubkey_byname(ctrl, &ctx, pk, name, ret_keyblock, NULL,
                         include_unusable, no_akl);
  if (rc) {
 ...
 * free_public_key).
 */
gpg_error_t get_pubkey_fromfile(ctrl_t ctrl, PKT_public_key *pk,
                                const char *fname) {
  gpg_error_t err;
  kbnode_t keyblock;
  kbnode_t found_key;
...
int get_pubkey_byfprint(ctrl_t ctrl, PKT_public_key *pk, kbnode_t *r_keyblock,
  ...
  if (fprint_len == 20 || fprint_len == 16) {
    ...
    ctx.items.emplace_back();
    ctx.items.back().mode =
        fprint_len == 16 ? KEYDB_SEARCH_MODE_FPR16 : KEYDB_SEARCH_MODE_FPR20;
...
int get_pubkey_byfprint_fast(PKT_public_key *pk, const byte *fprint,
                             size_t fprint_len) {
  int rc = 0;
  KEYDB_HANDLE hd;
  KBNODE keyblock;
  ...
  for (i = 0; i < MAX_FINGERPRINT_LEN && i < fprint_len; i++)
    fprbuf[i] = fprint[i];
  while (i < MAX_FINGERPRINT_LEN) fprbuf[i++] = 0;
  ...
  if (pk) copy_public_key(pk, keyblock->pkt->pkt.public_key);
  release_kbnode(keyblock);
...
const char *parse_def_secret_key(ctrl_t ctrl) {
  ...
  for (auto &t : opt.def_secret_key) {
    ...
    if (err == GPG_ERR_NOT_FOUND) continue;
    ...
    do {
      ...
      if (!(pk->pubkey_usage & PUBKEY_USAGE_SIG)) continue;

      /* Check if the key is valid.  */
      if (pk->flags.revoked) {
        if (DBG_LOOKUP)
          log_debug("not using %s as default key, %s", keystr_from_pk(pk),
                    "revoked");
        continue;
      }
      if (pk->has_expired) {
    ...
    if (err) {
      if (!warned && !opt.quiet) {
        log_info(_("Warning: not using '%s' as default key: %s\n"), str,
                 gpg_strerror(GPG_ERR_NO_SECKEY));
 ...
 * returned.  In particular, GPG_ERR_NO_PUBKEY or GPG_ERR_NO_SECKEY
 * (if want_secret is set) is returned if the key is not found.  */
gpg_error_t getkey_bynames(ctrl_t ctrl, getkey_ctx_t *retctx,
                           PKT_public_key *pk,
                           const std::vector<std::string> &names,
                           int want_secret, kbnode_t *ret_keyblock) {
  return key_byname(ctrl, retctx, names, pk, want_secret, 1, ret_keyblock,
                    NULL);
}
...
gpg_error_t getkey_byname(ctrl_t ctrl, getkey_ctx_t *retctx, PKT_public_key *pk,
  ...
  if (want_secret && !name) def_secret_key = parse_def_secret_key(ctrl);
 ...
 * The self-signed data has already been merged into the public key
 * using merge_selfsigs.  */
gpg_error_t getkey_next(ctrl_t ctrl, getkey_ctx_t ctx, PKT_public_key *pk,
                        kbnode_t *ret_keyblock) {
   ...
   * keyblock.  */
  if (pk && ret_keyblock == NULL) ret_keyblock = &keyblock;

  rc =
      lookup(ctrl, ctx, ctx->want_secret, ret_keyblock, pk ? &found_key : NULL);
  if (!rc && pk) {
...
void getkey_end(ctrl_t ctrl, getkey_ctx_t ctx) {
  if (ctx) {
    ...
    if (!ctx->not_allocated) delete (ctx);
  }
}
...
void setup_main_keyids(kbnode_t keyblock) {
  ...
  if (keyblock->pkt->pkttype != PKT_PUBLIC_KEY) BUG();
  pk = keyblock->pkt->pkt.public_key;
...
static int parse_key_usage(PKT_signature *sig) {
  int key_usage = 0;
  const byte *p;
  size_t n;
  byte flags;
  ...
  if (p && n) {
    ...
    if (flags) key_usage |= PUBKEY_USAGE_UNKNOWN;

    if (!key_usage) key_usage |= PUBKEY_USAGE_NONE;
 ...
 */
static void fixup_uidnode(KBNODE uidnode, KBNODE signode, u32 keycreated) {
  ...
  if (p && n && (p[0] & 0x01)) uid->flags.mdc = 1;

  /* And the keyserver modify flag.  */
  uid->flags.ks_modify = 1;
  p = parse_sig_subpkt(sig->hashed, SIGSUBPKT_KS_FLAGS, &n);
  if (p && n && (p[0] & 0x80)) uid->flags.ks_modify = 0;
}

static void sig_to_revoke_info(PKT_signature *sig, struct revoke_info *rinfo) {
...
/* Given a keyblock, parse the key block and extract various pieces of
  ...
  0.  */
static void merge_selfsigs_main(ctrl_t ctrl, kbnode_t keyblock, int *r_revoked,
  ...
  *r_revoked = 0;
  ...
  for (k = keyblock;
       ...
       k->pkt->pkttype != PKT_ATTRIBUTE && k->pkt->pkttype != PKT_PUBLIC_SUBKEY;
       k = k->next) {
        ...
        if (check_key_signature(ctrl, keyblock, k, NULL))
          ...
          *r_revoked = 1;
          ...
          if (sig->revkey) {
            ...
            pk->revkey = (revocation_key *)xrealloc(
                pk->revkey,
                sizeof(struct revocation_key) *
                    (pk->numrevkeys + sig->numrevkeys));

            for (i = 0; i < sig->numrevkeys; i++)
              memcpy(&pk->revkey[pk->numrevkeys++], &sig->revkey[i],
                     sizeof(struct revocation_key));
          }
          ...
          /* This is the latest signature so far.  */
          {
            if (sig->flags.expired)
              ...
              signode = k;
              if (sig->version > sigversion) sigversion = sig->version;
            }
          }
        }
      }
  ...
  if (pk->revkey) {
    ...
    if (changed)
      pk->revkey = (revocation_key *)xrealloc(
          pk->revkey, pk->numrevkeys * sizeof(struct revocation_key));
  }
      ...
      if (k->pkt->pkttype == PKT_USER_ID)
        ...
        if (sig->keyid[0] != kid[0] || sig->keyid[1] != kid[1]) {
          ...
          if (get_pubkey_fast(ultimate_pk, sig->keyid) == 0 &&
              check_key_signature2(ctrl, keyblock, k, ultimate_pk, NULL, NULL,
                                   NULL, NULL) == 0 &&
      ...
      if (uid->flags.primary) {
        if (uid->created > uiddate) {
          ...
          /* The dates are equal, so we need to do a
             ...
             the same primary. */
          if (cmp_user_ids(uid, uidnode->pkt->pkt.user_id) > 0) uidnode = k;
        }
      } else {
        if (uid->created > uiddate2) {
          ...
          if (cmp_user_ids(uid, uidnode2->pkt->pkt.user_id) > 0) uidnode2 = k;
        }
      }
    }
  }
  if (uidnode) {
    for (k = keyblock; k && k->pkt->pkttype != PKT_PUBLIC_SUBKEY; k = k->next) {
      if (k->pkt->pkttype == PKT_USER_ID && !k->pkt->pkt.user_id->attrib_data) {
        PKT_user_id *uid = k->pkt->pkt.user_id;
        if (k != uidnode) uid->flags.primary = 0;
      }
    }
  } else if (uidnode2) {
    ...
    for (k = keyblock; k && k->pkt->pkttype != PKT_PUBLIC_SUBKEY; k = k->next) {
      if (k->pkt->pkttype == PKT_USER_ID && !k->pkt->pkt.user_id->attrib_data) {
        if (!uidnode) {
          ...
          if (cmp_user_ids(k->pkt->pkt.user_id, uidnode->pkt->pkt.user_id) >
              0) {
...
static PKT_signature *buf_to_sig(const byte *buf, size_t len) {
...
/* Use the self-signed data to fill in various fields in subkeys.
  ...
  */
static void merge_selfsigs_subkey(ctrl_t ctrl, kbnode_t keyblock,
                                  kbnode_t subnode) {
  ...
  if (subnode->pkt->pkttype != PKT_PUBLIC_SUBKEY) BUG();
  mainpk = keyblock->pkt->pkt.public_key;
  if (mainpk->version < 4) return; /* (actually this should never happen) */
  ...
  /* Algo doesn't exist.  */
  if (openpgp_pk_test_algo((pubkey_algo_t)(subpk->pubkey_algo))) return;
  ...
  if (!subpk->flags.backsig) {
    ...
    while (
        (p = enum_sig_subpkt(sig->hashed, SIGSUBPKT_SIGNATURE, &n, &seq, NULL)))
      if (n > 3 &&
          ((p[0] == 3 && p[2] == 0x19) || (p[0] == 4 && p[1] == 0x19))) {
        PKT_signature *tempsig = buf_to_sig(p, n);
        if (tempsig) {
          if (tempsig->timestamp > sigdate) {
            if (backsig) free_seckey_enc(backsig);
    ...
    while ((p = enum_sig_subpkt(sig->unhashed, SIGSUBPKT_SIGNATURE, &n, &seq,
                                NULL)))
      if (n > 3 &&
          ((p[0] == 3 && p[2] == 0x19) || (p[0] == 4 && p[1] == 0x19))) {
        PKT_signature *tempsig = buf_to_sig(p, n);
...
static void merge_selfsigs(ctrl_t ctrl, kbnode_t keyblock) {
  ...
  if (keyblock->pkt->pkttype != PKT_PUBLIC_KEY) {
    if (keyblock->pkt->pkttype == PKT_SECRET_KEY) {
      log_error(
          "expected public key but found secret key "
          "- must stop\n");
  ...
  if (revoked || main_pk->has_expired || !main_pk->flags.valid) {
    ...
    for (k = keyblock; k; k = k->next) {
      if (k->pkt->pkttype == PKT_PUBLIC_KEY ||
        ...
        if (!main_pk->flags.valid) pk->flags.valid = 0;
        if (revoked && !pk->flags.revoked) {
          pk->flags.revoked = revoked;
          memcpy(&pk->revoked, &rinfo, sizeof(rinfo));
        }
        if (main_pk->has_expired) pk->has_expired = main_pk->has_expired;
      }
    }
    return;
  }
...
#define USAGE_MASK (PUBKEY_USAGE_SIG | PUBKEY_USAGE_ENC | PUBKEY_USAGE_CERT)
  ...
  if (req_prim || (foundk && foundk->pkt->pkttype != PKT_PUBLIC_SUBKEY))
    ...
    for (k = foundk ? foundk : keyblock; k; k = nextk) {
      ...
      if (k->pkt->pkttype != PKT_PUBLIC_SUBKEY) continue;
      ...
      if (!pk->flags.valid) {
        if (DBG_LOOKUP) log_debug("\tsubkey not valid\n");
        continue;
      }
      ...
      n_subkeys++;
      if (pk->flags.revoked) {
        if (DBG_LOOKUP) log_debug("\tsubkey has been revoked\n");
        n_revoked_or_expired++;
        continue;
      }
      if (pk->has_expired) {
        if (DBG_LOOKUP) log_debug("\tsubkey has expired\n");
        n_revoked_or_expired++;
        continue;
      }
      if (pk->timestamp > curtime && !opt.ignore_valid_from) {
        if (DBG_LOOKUP) log_debug("\tsubkey not yet valid\n");
        continue;
      }

      if (DBG_LOOKUP) log_debug("\tsubkey might be fine\n");
  ...
  if ((!latest_key && !want_exact) || foundk == keyblock || req_prim) {
    ...
    pk = keyblock->pkt->pkt.public_key;
    if (!pk->flags.valid) {
      if (DBG_LOOKUP) log_debug("\tprimary key not valid\n");
    } else if (!((pk->pubkey_usage & USAGE_MASK) & req_usage)) {
      if (DBG_LOOKUP)
        log_debug(
            "\tprimary key usage does not match: "
            "want=%x have=%x\n",
            req_usage, pk->pubkey_usage);
    } else if (pk->flags.revoked) {
      if (DBG_LOOKUP) log_debug("\tprimary key has been revoked\n");
    } else if (pk->has_expired) {
      if (DBG_LOOKUP) log_debug("\tprimary key has expired\n");
    } else /* Okay.  */
    {
  ...
  if (!latest_key) {
    if (DBG_LOOKUP) log_debug("\tno suitable key found -  giving up\n");
    if (r_flags) *r_flags |= LOOKUP_NOT_SELECTED;
...
found:
  ...
  if (latest_key != keyblock && opt.verbose) {
    char *tempkeystr = xstrdup(keystr_from_pk(latest_key->pkt->pkt.public_key));
...
static int lookup(ctrl_t ctrl, getkey_ctx_t ctx, int want_secret,
  ...
  if (ret_keyblock) *ret_keyblock = NULL;

  for (;;) {
    rc = keydb_search(ctx->kr_handle, ctx->items.data(), ctx->items.size(),
                      NULL);
    if (rc) break;
     ...
     * merge_selfsigs.  */
    merge_selfsigs(ctrl, keyblock);
    found_key = finish_lookup(keyblock, ctx->req_usage, ctx->exact, &infoflags);
    print_status_key_considered(keyblock, infoflags);
    if (found_key) {
      no_suitable_key = 0;
      goto found;
...
gpg_error_t enum_secret_keys(ctrl_t ctrl, void **context, PKT_public_key *sk) {
  ...
  if (!c) {
    ...
    if (!c) return gpg_error_from_syserror();
    *context = c;
  }
    ...
    while (!c->keyblock) {
      ...
      do {
        ...
        switch (c->state) {
          ...
          {
            std::vector<std::string> namelist;
            err = getkey_bynames(ctrl, &c->ctx, NULL, namelist, 1, &keyblock);
            if (err) {
              release_kbnode(keyblock);
              keyblock = NULL;
              getkey_end(ctrl, c->ctx);
...
gpg_error_t get_seckey_default_or_card(ctrl_t ctrl, PKT_public_key *pk,
  ...
  if (!fpr_card ||
  ...
  else { /* Default key is specified and card key is also available.  */
    ...
    if (!err)
      for (k = keyblock; k; k = k->next) {
        ...
        if (!pk_candidate->flags.valid) continue;
...
static char *get_user_id_string(ctrl_t ctrl, u32 *keyid, int mode,
                                size_t *r_len) {
  user_id_db_t r;
  keyid_list_t a;
  int pass = 0;
      ...
      for (a = r->keyids; a; a = a->next) {
        if (a->keyid[0] == keyid[0] && a->keyid[1] == keyid[1]) {
          if (mode == 2) {
            ...
            if (r_len) *r_len = r->len;
          } else {
            if (mode)
              p = xasprintf("%08lX%08lX %.*s", (unsigned long)keyid[0],
                            (unsigned long)keyid[1], r->len, r->name);
            else
              p = xasprintf("%s %.*s", keystr(keyid), r->len, r->name);
            if (r_len) *r_len = strlen(p);
          }
  ...
  if (mode == 2)
    ...
    p = xasprintf("%08lX%08lX [?]", (unsigned long)keyid[0],
...
/* Return the database handle used by this context.  The context still
   owns the handle.  */
KEYDB_HANDLE
get_ctx_handle(GETKEY_CTX ctx) { return ctx->kr_handle; }
