 ...
 */
static void mk_notation_policy_etc(PKT_signature *sig, PKT_public_key *pk,
                                   PKT_public_key *pksk) {
  const char *string;
  char *p = NULL;
  ...
  if (nd) {
    ...
    for (item = nd; item; item = item->next) {
      ...
      if (!item->altvalue)
        log_error(
            _("WARNING: unable to %%-expand notation "
  ...
  if (policy_url) {
    for (auto &&pu_ : *policy_url) {
      ...
      if (!p) {
        log_error(
            _("WARNING: unable to %%-expand policy URL "
              "(too large).  Using unexpanded.\n"));
        p = xstrdup(string);
      }

      build_sig_subpkt(
          sig, (sigsubpkttype_t)((SIGSUBPKT_POLICY |
 ...
 */
static void hash_uid(gcry_md_hd_t md, int sigversion, const PKT_user_id *uid) {
  byte buf[5];
 ...
 */
static void hash_sigversion_to_magic(gcry_md_hd_t md,
                                     const PKT_signature *sig) {
  byte buf[6];
  size_t n;
...
static int do_sign(ctrl_t ctrl, PKT_public_key *pksk, PKT_signature *sig,
  ...
  if (pksk->timestamp > sig->timestamp) {
    ...
    log_info(ngettext("key %s was created %lu second"
                      ...
                      " in the future (time warp or clock problem)\n",
                      d),
             keystr_from_pk(pksk), d);
    if (!opt.ignore_time_conflict) return GPG_ERR_TIME_CONFLICT;
  }

  print_pubkey_algo_note((pubkey_algo_t)(pksk->pubkey_algo));

  if (!mdalgo) mdalgo = gcry_md_get_algo(md);
  ...
  if (!err) {
    ...
    err = agent_pksign(NULL /*ctrl*/, cache_nonce, hexgrip, desc, pksk->keyid,
                       pksk->main_keyid, pksk->pubkey_algo, dp,
                       gcry_md_get_algo_dlen(mdalgo), mdalgo, &s_sigval);
    xfree(desc);
  ...
  if (err)
    ...
    if (opt.verbose) {
      ...
      log_info(_("%s/%s signature from: \"%s\"\n"),
               ...
               openpgp_md_algo_name(sig->digest_algo), ustr);
      xfree(ustr);
    }
  }
  return err;
...
static int complete_sig(ctrl_t ctrl, PKT_signature *sig, PKT_public_key *pksk,
...
static int match_dsa_hash(unsigned int qbytes) {
  if (qbytes <= 20) return DIGEST_ALGO_SHA1;

  if (qbytes <= 28) return DIGEST_ALGO_SHA224;

  if (qbytes <= 32) return DIGEST_ALGO_SHA256;

  if (qbytes <= 48) return DIGEST_ALGO_SHA384;
...
/*
  ...
  list?
*/
static int hash_for(PKT_public_key *pk) {
...
static void print_status_sig_created(PKT_public_key *pk, PKT_signature *sig,
                                     int what) {
 ...
 */
static int write_onepass_sig_packets(SK_LIST sk_list, IOBUF out, int sigclass) {
  int skcount;
  SK_LIST sk_rover;
 ...
 */
static int write_plaintext_packet(IOBUF out, IOBUF inp, const char *fname,
                                  int ptmode) {
  ...
  if (!iobuf_is_pipe_filename(fname) && *fname) {
    ...
    if (!(tmpsize = iobuf_get_filelength(inp, &overflow)) && !overflow &&
        opt.verbose)
    ...
    if (ptmode == 't' || ptmode == 'u' || ptmode == 'm') filesize = 0;
  } else
    filesize = 0; /* stdin */
 ...
 */
static int write_signature_packets(ctrl_t ctrl, SK_LIST sk_list, IOBUF out,
                                   gcry_md_hd_t hash, int sigclass,
  ...
  for (sk_rover = sk_list; sk_rover; sk_rover = sk_rover->next) {
    ...
    if (!sig) return gpg_error_from_syserror();

    if (duration || !opt.sig_policy_url.empty() || opt.sig_notations ||
        !opt.sig_keyserver_url.empty())
      sig->version = 4;
    else
    ...
    if (duration) sig->expiredate = sig->timestamp + duration;
    sig->sig_class = sigclass;

    if (gcry_md_copy(&md, hash)) BUG();
 ...
 */
int sign_file(ctrl_t ctrl, const std::vector<std::string> &filenames,
              int detached,
  ...
  if (encryptflag == 2 && (rc = setup_symkey(&efx.symkey_s2k, &efx.symkey_dek)))
    goto leave;
  ...
  if (encryptflag && (rc = build_pk_list(ctrl, remusr, &pk_list))) goto leave;
  ...
  } else if ((rc = open_outfile(-1, fname, opt.armor ? 1 : detached ? 2 : 0, 0,
                                &out)))
    goto leave;
  ...
  if (gcry_md_open(&mfx.md, 0, 0)) BUG();
  if (DBG_HASHING) gcry_md_debug(mfx.md, "sign");
  ...
  if (pk_list) {
    if (opt.def_digest_algo) {
      if (!opt.expert &&
          select_algo_from_prefs(pk_list, PREFTYPE_HASH, opt.def_digest_algo,
                                 NULL) != opt.def_digest_algo)
        log_info(_("WARNING: forcing digest algorithm %s (%d)"
                   " violates recipient preferences\n"),
                 gcry_md_algo_name(opt.def_digest_algo), opt.def_digest_algo);
    } else {
      ...
      if (!smartcard || (smartcard && hint.digest_length == 20))
        if ((algo = select_algo_from_prefs(pk_list, PREFTYPE_HASH, -1, &hint)) >
            0)
  ...
  if (!multifile) iobuf_push_filter(inp, md_filter, &mfx);

  if (detached && !encryptflag) afx->what = 2;

  if (opt.armor && !outfile) push_armor_filter(afx, out);
  ...
  if (opt.compress_algo && !outfile && !detached) {
    ...
    if (compr_algo == -1) {
      ...
      if ((compr_algo =
        ...
        compr_algo = default_compress_algo();
               ...
               select_algo_from_prefs(pk_list, PREFTYPE_ZIP, compr_algo,
                                      NULL) != compr_algo)
    ...
    if (compr_algo) push_compress_filter(out, &zfx, compr_algo);
  }
  ...
  if (detached) {
    if (multifile) {
      if (opt.verbose) log_info(_("signing:"));
      for (auto &filename : filenames) {
        ...
        if (opt.verbose) log_printf(" '%s'", filename.c_str());
        if (opt.textmode) {
          memset(&tfx, 0, sizeof tfx);
          iobuf_push_filter(inp, text_filter, &tfx);
        }
    ...
    rc = write_plaintext_packet(
        out, inp, fname,
        opt.textmode && !outfile ? (opt.mimemode ? 'm' : 't') : 'b');
  }
  ...
  rc = write_signature_packets(ctrl, sk_list, out, mfx.md,
                               opt.textmode && !outfile ? 0x01 : 0x00, 0,
                               duration, detached ? 'D' : 'S', NULL);
  if (rc) goto leave;
...
leave:
  if (rc)
    ...
    iobuf_close(out);
    if (encryptflag) write_status(STATUS_END_ENCRYPTION);
  }
  iobuf_close(inp);
...
int clearsign_file(
  ...
  if (gcry_md_open(&textmd, 0, 0)) BUG();
  for (sk_rover = sk_list; sk_rover; sk_rover = sk_rover->next)
    gcry_md_enable(textmd, hash_for(sk_rover->pk));

  if (DBG_HASHING) gcry_md_debug(textmd, "clearsign");
  ...
  /* Write the signatures.  */
  rc = write_signature_packets(ctrl, sk_list, out, textmd, 0x01, 0, duration,
                               'C', NULL);
  if (rc) goto leave;
...
int sign_symencrypt_file(
  ...
  /* prepare to calculate the MD over the input */
  if (opt.textmode) iobuf_push_filter(inp, text_filter, &tfx);
  if (gcry_md_open(&mfx.md, 0, 0)) BUG();
  if (DBG_HASHING) gcry_md_debug(mfx.md, "symc-sign");
  ...
  if (opt.armor) push_armor_filter(afx, out);
  ...
  /*(current filters: zip - encrypt - armor)*/
  rc = write_signature_packets(ctrl, sk_list, out, mfx.md,
                               opt.textmode ? 0x01 : 0x00, 0, duration, 'S',
                               NULL);
  if (rc) goto leave;
 ...
 * MKSUBPKT.
 */
int make_keysig_packet(ctrl_t ctrl, PKT_signature **ret_sig, PKT_public_key *pk,
  ...
  sigversion = 4;
  if (sigversion < pksk->version) sigversion = pksk->version;
  ...
  if (gcry_md_open(&md, digest_algo, 0)) BUG();
  ...
  if (mksubpkt) rc = (*mksubpkt)(sig, opaque);
 ...
 */
gpg_error_t update_keysig_packet(ctrl_t ctrl, PKT_signature **ret_sig,
                                 PKT_signature *orig_sig, PKT_public_key *pk,
                                 PKT_user_id *uid, PKT_public_key *subpk,
                                 PKT_public_key *pksk,
