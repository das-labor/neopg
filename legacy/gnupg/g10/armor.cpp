...
static const char *tail_strings[] = {
    "END PGP MESSAGE",          "END PGP PUBLIC KEY BLOCK",
    "END PGP SIGNATURE",        "END dummy",
    "END PGP ARMORED FILE",     "END PGP PRIVATE KEY BLOCK",
    "END PGP SECRET KEY BLOCK", NULL};

static int armor_filter(void *opaque, int control, iobuf_t chain, byte *buf,
                        size_t *ret_len);
 ...
 */
static int is_armored(const byte *buf) {
  int ctb, pkttype;
     ...
     OpenPGP data.  */
  {
    ...
    if (indeterminate_length) return 1;
  }
...
int use_armor_filter(IOBUF a) {
  ...
  if (n == -1) return 0; /* EOF, doesn't matter whether armored or not */
  if (!n) return 1;      /* can't check it: try armored */
  if (n != 2) return 0;  /* short buffer */
  return is_armored(buf);
}

static void invalid_armor(void) {
...
static int parse_hash_header(const char *line) {
  ...
  if (memcmp(line, "Hash:", 5)) return 0; /* invalid header */
...
static int is_armor_tag(const char *line) {
  if (strncmp(line, "Version", 7) == 0 || strncmp(line, "Comment", 7) == 0 ||
      strncmp(line, "MessageID", 9) == 0 || strncmp(line, "Hash", 4) == 0 ||
 ...
 * armor header.
 */
static int is_armor_header(byte *line, unsigned len) {
  const char *s;
  byte *save_p, *p;
  int save_c;
  int i;

  if (len < 15) return -1;                 /* too short */
  if (memcmp(line, "-----", 5)) return -1; /* no */
  ...
  *save_p = 0;
  ...
  for (i = 0; (s = head_strings[i]); i++)
    if (!strcmp(s, (const char *)(p))) break;
  *save_p = save_c;
  if (!s) return -1; /* unknown armor line */

  if (opt.verbose > 1) log_info(_("armor: %s\n"), head_strings[i]);
  return i;
}
 ...
 *	 >0: Good header line
 */
static int parse_header_line(armor_filter_context_t *afx, byte *line,
                             unsigned int len) {
  byte *p;
  int hashes = 0;
  unsigned int len2;
...
static int check_input(armor_filter_context_t *afx, IOBUF a) {
  int rc = 0;
  int i;
  byte *line;
  unsigned len;
  ...
  maxlen = MAX_LINELEN;
  len = afx->buffer_len =
      iobuf_read_line(a, &afx->buffer, &afx->buffer_size, &maxlen);
  line = afx->buffer;
  if (!maxlen) {
 ...
 */
static int fake_packet(armor_filter_context_t *afx, IOBUF a, size_t *retn,
  ...
  while (!rc && size - len >= (PARTIAL_CHUNK + 1)) {
    ...
    if (afx->faked == 1)
      ...
      while (tempbuf_len < PARTIAL_CHUNK && afx->buffer_pos < afx->buffer_len)
        tempbuf[tempbuf_len++] = afx->buffer[afx->buffer_pos++];
      if (tempbuf_len == PARTIAL_CHUNK) continue;
    }
    ...
    /* Now handle the end-of-line canonicalization */
    {
      ...
      afx->buffer_len = trim_trailing_chars(&p[afx->buffer_pos],
                                            n - afx->buffer_pos, " \t\r\n");
...
static int invalid_crc(void) {
  if (opt.ignore_crc_error) return 0;
  log_inc_errorcount();
  ...
  again:
    if (c == '\n' || c == ' ' || c == '\r' || c == '\t')
      ...
      if (afx->buffer_pos + 2 < afx->buffer_len) {
        ...
        if (isxdigit(cc1) && isxdigit(cc2) && strchr("=\n\r\t ", cc3)) {
 ...
 */
static int armor_filter(void *opaque, int control, IOBUF a, byte *buf,
                        size_t *ret_len) {
  size_t size = *ret_len;
  ...
  if (control == IOBUFCTRL_UNDERFLOW && afx->inp_bypass) {
    ...
    if (afx->buffer_len) {
      ...
      if (afx->buffer_pos >= afx->buffer_len) afx->buffer_len = 0;
    }
    /* If there is still space in BUF, read directly into it.  */
    for (; n < size; n++) {
      if ((c = iobuf_get(a)) == -1) break;
      buf[n] = c & 0xff;
    }
    if (!n) /* We didn't get any data.  EOF.  */
      rc = -1;
    *ret_len = n;
     ...
     * length itself */
    if (size < PARTIAL_CHUNK + 1) BUG(); /* supplied buffer too short */
    ...
    if (afx->faked)
      ...
      if (afx->inp_bypass) {
        ...
        n++;             /* see below */
        memcpy(buf + n, sesmark, sesmarklen);
        n += sesmarklen;
        buf[n++] = CTRLPKT_CLEARSIGN_START;
        buf[n++] = 1; /* dash escaped, sigclass */
        if (hashes & 1) buf[n++] = DIGEST_ALGO_RMD160;
        if (hashes & 2) buf[n++] = DIGEST_ALGO_SHA1;
        if (hashes & 8) buf[n++] = DIGEST_ALGO_SHA224;
        if (hashes & 16) buf[n++] = DIGEST_ALGO_SHA256;
        if (hashes & 32) buf[n++] = DIGEST_ALGO_SHA384;
        if (hashes & 64) buf[n++] = DIGEST_ALGO_SHA512;
        buf[1] = n - 2;
    ...
    *ret_len = n;
    ...
    if (!afx->status) { /* write the header line */
      ...
      if (afx->what >= DIM(head_strings)) log_bug("afx->what=%d", afx->what);
      iobuf_writestr(a, "-----");
    ...
    idx = afx->idx;
    idx2 = afx->idx2;
    for (i = 0; i < idx; i++) radbuf[i] = afx->radbuf[i];
    ...
    for (; size; buf++, size--) {
      ...
      if (idx > 2) {
        ...
        iobuf_put(a, c);
        c = bintoasc[(((radbuf[1] << 2) & 074) | ((radbuf[2] >> 6) & 03)) &
                     077];
        iobuf_put(a, c);
        c = bintoasc[radbuf[2] & 077];
        iobuf_put(a, c);
    ...
    for (i = 0; i < idx; i++) afx->radbuf[i] = radbuf[i];
    afx->idx = idx;
    afx->idx2 = idx2;
    afx->crc = crc;
    ...
    if (afx->cancel)
      ...
      /* may need a linefeed */
      if (idx2) iobuf_writestr(a, (const char *)(afx->eol));
      /* write the CRC */
      iobuf_put(a, '=');
      radbuf[0] = crc >> 16;
      radbuf[1] = crc >> 8;
      ...
      iobuf_writestr(a, (const char *)(afx->eol));
      /* and the trailer */
      if (afx->what >= DIM(tail_strings)) log_bug("afx->what=%d", afx->what);
      iobuf_writestr(a, "-----");
    ...
    if (afx->qp_detected)
      log_error(
          _("quoted printable character in armor - "
...
void unarmor_pump_release(UnarmorPump x) { xfree(x); }
  ...
  switch (x->state) {
    ...
    case STA_compare_header:
      if ("-----BEGIN PGP SIGNATURE-----"[++x->pos] == c) {
        if (x->pos == 28) x->state = STA_found_header_wait_newline;
      } else
    ...
    case STA_read_crc:
      if ((c = asctobin[c]) == 255) {
        ...
        if (x->crc != x->mycrc) {
          log_info(_("CRC error; %06lX - %06lX\n"), (unsigned long)x->crc,
                   (unsigned long)x->mycrc);
          if (invalid_crc()) rval = -3;
        }
