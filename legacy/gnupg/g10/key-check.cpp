...
static int sig_comparison(const void *av, const void *bv) {
...
int key_check_all_keysigs(ctrl_t ctrl, kbnode_t kb, int only_selected,
                          int only_selfsigs) {
  gpg_error_t err;
  PKT_public_key *pk;
  ...
  /* First we look for duplicates.  */
  {
    ...
    if (!nsigs) return 0; /* No signatures at all.  */
    ...
    i = 0;
    for (n = kb; n; n = n->next) {
      if (is_deleted_kbnode(n)) continue;
      ...
      /* They are the same.  Kill the latter.  */
      {
        if (DBG_PACKET) {
          ...
          log_debug(
              ...
              sig->sig_class, keystr(sig->keyid), isotimestamp(sig->timestamp),
              (long long)sig->timestamp, sig->digest_start[0],
              sig->digest_start[1]);
        }
  ...
  for (n_prevp = &kb, n = kb; n;
       /* If we moved n, then n_prevp is need valid.  */
       n_prevp = (n->next == n_next ? &n->next : n_prevp),
      n = n_next) {
    PACKET *p;
    ...
    switch (p->pkttype) {
      case PKT_PUBLIC_KEY:
        ...
        if (DBG_PACKET)
          log_debug("public key %s: timestamp: %s (%lld)\n", pk_keyid_str(pk),
                    isotimestamp(pk->timestamp), (long long)pk->timestamp);
        current_component = n;
        break;
      ...
      case PKT_USER_ID:
        ...
        if (DBG_PACKET)
          log_debug("user id: %s\n",
                    p->pkt.user_id->attrib_data ? "[ photo id ]"
                                                : p->pkt.user_id->name);
        current_component = n;
        break;
      case PKT_SIGNATURE:
        ...
        else /* Issuer is a different key.  */
        {
          ...
          if (err) {
            ...
            issuer = NULL;
            if (DBG_PACKET) {
              if (pending_desc) log_debug("%s", pending_desc);
              log_debug(
                  "    Can't check signature allegedly"
                  " issued by %s: %s\n",
        ...
        if ((err = openpgp_pk_test_algo((pubkey_algo_t)(sig->pubkey_algo)))) {
          if (DBG_PACKET && pending_desc) log_debug("%s", pending_desc);
        ...
        for (n2 = current_component; n2;
          ...
          if (is_deleted_kbnode(n2))
            ...
            err = check_signature_over_key_or_uid(ctrl, issuer, sig, kb,
                                                  n2->pkt, NULL, NULL);
            if (!err) break;
          }
        ...
        if (current_component == n2) {
          ...
          if (DBG_PACKET) {
            ...
            log_debug(
                ...
                n2->pkt->pkttype == PKT_USER_ID
                    ? "user id"
                    : n2->pkt->pkttype == PKT_PUBLIC_SUBKEY ? "subkey"
                                                            : "primary key",
                n2->pkt->pkttype,
                n2->pkt->pkttype == PKT_USER_ID
          ...
          *n_prevp = n->next;
          ...
          if (DBG_PACKET) dump_sig_params = 1;
          ...
          if ((n2 && n2 != last_printed_component) ||
            ...
            if (!modified)
              ...
              tty_print_utf8_string(
                  (const unsigned char *)(last_printed_component->pkt->pkt
                                              .user_id->name),
                  last_printed_component->pkt->pkt.user_id->len);
            } else if (last_printed_component->pkt->pkttype == PKT_PUBLIC_KEY)
              tty_printf(
                  "pub  %s",
                  pk_keyid_str(last_printed_component->pkt->pkt.public_key));
            else
              tty_printf(
                  "sub  %s",
                  pk_keyid_str(last_printed_component->pkt->pkt.public_key));
            ...
            if (modified) {
              if (is_reordered) tty_printf(_(" (reordered signatures follow)"));
              tty_printf("\n");
            }
          }
        ...
        if (dump_sig_params) {
          ...
          for (i = 0; i < pubkey_get_nsig((pubkey_algo_t)(sig->pubkey_algo));
               i++) {
            char buffer[1024];
            size_t len;
            char *printable;
            gcry_mpi_print(GCRYMPI_FMT_USG, (unsigned char *)(buffer),
                           sizeof(buffer), &len, sig->data[i]);
        ...
        break;
      default:
        if (DBG_PACKET) log_debug("unhandled packet: %d\n", p->pkttype);
        break;
    }
  }
  ...
  if (issuer != pk) free_public_key(issuer);
  issuer = NULL;
  ...
  /* Identify keys / uids that don't have a self-sig.  */
  {
    ...
    for (n = kb; n; n = n->next) {
      ...
      switch (p->pkttype) {
        ...
        case PKT_PUBLIC_SUBKEY:
        case PKT_USER_ID:
          if (current_component && !has_selfsig) missing_selfsig++;
          current_component = n;
          has_selfsig = 0;
          break;

        case PKT_SIGNATURE:
          if (!current_component || has_selfsig) break;

          sig = n->pkt->pkt.signature;

          if (!(sig->flags.checked && sig->flags.valid)) break;
