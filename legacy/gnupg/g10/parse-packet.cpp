...
static int parse_onepass_sig(IOBUF inp, int pkttype, unsigned long pktlen,
                             PKT_onepass_sig *ops);
static int parse_key(IOBUF inp, int pkttype, unsigned long pktlen, byte *hdr,
                     int hdrlen, PACKET *packet);
...
static unsigned long read_32(IOBUF inp) {
  unsigned long a;
...
static gcry_mpi_t mpi_read(iobuf_t inp, unsigned int *ret_nread, int secure) {
  ...
  if (!nmax) goto overflow;

  if ((c = c1 = iobuf_get(inp)) == -1) goto leave;
  if (++nread == nmax) goto overflow;
  nbits = c << 8;
  if ((c = c2 = iobuf_get(inp)) == -1) goto leave;
  ++nread;
  nbits |= c;
...
static void unknown_pubkey_warning(int algo) {
...
#if DEBUG_PARSE_PACKET
int dbg_parse_packet(parse_packet_ctx_t ctx, PACKET *pkt, const char *dbg_f,
                     int dbg_l) {
  int skip, rc;
 ...
 */
#if DEBUG_PARSE_PACKET
int dbg_search_packet(parse_packet_ctx_t ctx, PACKET *pkt, off_t *retpos,
                      int with_uid, const char *dbg_f, int dbg_l) {
  int skip, rc;
...
#else  /*!DEBUG_PARSE_PACKET*/
int search_packet(parse_packet_ctx_t ctx, PACKET *pkt, off_t *retpos,
                  int with_uid) {
  int skip, rc;
 ...
 */
#if DEBUG_PARSE_PACKET
int dbg_copy_all_packets(iobuf_t inp, iobuf_t out, const char *dbg_f,
                         int dbg_l) {
  PACKET pkt;
  struct parse_packet_ctx_s parsectx;
  int skip, rc = 0;

  if (!out) log_bug("copy_all_packets: OUT may not be NULL.\n");

  init_parse_packet(&parsectx, inp);

  do {
    init_packet(&pkt);
  } while (!(rc = parse(&parsectx, &pkt, 0, NULL, &skip, out, 0, "copy", dbg_f,
                        dbg_l)));
...
int dbg_copy_some_packets(iobuf_t inp, iobuf_t out, off_t stopoff,
  ...
  do {
    ...
    init_packet(&pkt);
  } while (!(rc = parse(&parsectx, &pkt, 0, NULL, &skip, out, 0, "some", dbg_f,
                        dbg_l)));
 ...
 */
#if DEBUG_PARSE_PACKET
int dbg_skip_some_packets(iobuf_t inp, unsigned n, const char *dbg_f,
                          int dbg_l) {
  int rc = 0;
  int skip;
  PACKET pkt;
  ...
  for (; n && !rc; n--) {
    init_packet(&pkt);
    rc = parse(&parsectx, &pkt, 0, NULL, &skip, NULL, 1, "skip", dbg_f, dbg_l);
  }
...
again:
  ...
  if (out && pkttype) {
    ...
    if (!rc) rc = copy_packet(inp, out, pkttype, pktlen, partial);
    goto leave;
  }
  ...
  if (with_uid && pkttype == PKT_USER_ID)
    ...
    ;
           ...
           || (onlykeypkts && pkttype != PKT_PUBLIC_SUBKEY &&
               pkttype != PKT_PUBLIC_KEY && pkttype != PKT_SECRET_SUBKEY &&
  ...
  switch (pkttype) {
    ...
    case PKT_SECRET_SUBKEY:
      pkt->pkt.public_key =
          (PKT_public_key *)xmalloc_clear(sizeof *pkt->pkt.public_key);
    ...
    case PKT_SIGNATURE:
      pkt->pkt.signature =
          (PKT_signature *)xmalloc_clear(sizeof *pkt->pkt.signature);
      rc = parse_signature(inp, pkttype, pktlen, pkt->pkt.signature);
      break;
    case PKT_ONEPASS_SIG:
      pkt->pkt.onepass_sig =
          (PKT_onepass_sig *)xmalloc_clear(sizeof *pkt->pkt.onepass_sig);
    ...
    case PKT_RING_TRUST: {
      ...
      if (!rc) goto again; /* Directly read the next packet.  */
    } break;
    case PKT_PLAINTEXT:
  ...
  if (!rc && (pkttype == PKT_PUBLIC_KEY || pkttype == PKT_SECRET_KEY ||
              pkttype == PKT_USER_ID || pkttype == PKT_ATTRIBUTE ||
...
leave:
  ...
  if (!rc && iobuf_error(inp)) rc = GPG_ERR_INV_KEYRING;
...
static int copy_packet(IOBUF inp, IOBUF out, int pkttype, unsigned long pktlen,
  ...
  if (partial) {
    while ((n = iobuf_read(inp, buf, sizeof(buf))) != -1)
      if ((rc = iobuf_write(out, buf, n))) return rc; /* write error */
...
/* Read a special size+body from INP.  On success store an opaque MPI
   ...
   PKTLEN.  */
static gpg_error_t read_size_body(iobuf_t inp, int pktlen, size_t *r_nread,
                                  gcry_mpi_t *r_data) {
  char buffer[256];
  char *tmpbuf;
  int i, c, nbytes;
  ...
  *r_data = NULL;
  ...
  if (c < 0) return GPG_ERR_INV_PACKET;
  pktlen--;
  ++*r_nread;
  nbytes = c;
  if (nbytes < 2 || nbytes > 254) return GPG_ERR_INV_PACKET;
  if (nbytes > pktlen) return GPG_ERR_INV_PACKET;
  ...
  if (!tmpbuf) return gpg_error_from_syserror();
...
static int parse_symkeyenc(IOBUF inp, int pkttype, unsigned long pktlen,
  ...
  k = packet->pkt.symkey_enc = (PKT_symkey_enc *)xmalloc_clear(
      sizeof *packet->pkt.symkey_enc + seskeylen - 1);
  k->version = version;
  ...
  if (k->seskeylen) {
    ...
    if (s2kmode != 1 && s2kmode != 3)
      log_info(
          _("WARNING: potentially insecure symmetrically"
  ...
  if (!ndata) {
    ...
    for (i = 0; i < ndata; i++) {
      if (k->pubkey_algo == PUBKEY_ALGO_ECDH && i == 1) {
        ...
        pktlen -= n;
        if (!k->data[i]) rc = GPG_ERR_INV_PACKET;
      }
      if (rc) goto leave;
      if (list_mode) {
        *listfp << "\tdata: ";
...
static void dump_sig_subpkt(int hashed, int type, int critical,
  ...
  switch (type) {
    ...
    case SIGSUBPKT_NOTATION:
      /* minimum length needed, and the subpacket must be well-formed
         ...
         packet. */
      if (n < 8 ||
          8 + ((buffer[4] << 8) | buffer[5]) + ((buffer[6] << 8) | buffer[7]) !=
              n)
        break;
      return 0;
    case SIGSUBPKT_PRIMARY_UID:
...
static int can_handle_critical_notation(const byte *name, size_t len) {
  if (len == 32 && memcmp(name, "preferred-email-encoding@pgp.com", 32) == 0)
    return 1;
  if (len == 21 && memcmp(name, "pka-address@gnupg.org", 21) == 0) return 1;

  return 0;
}

static int can_handle_critical(const byte *buffer, size_t n, int type) {
  switch (type) {
    case SIGSUBPKT_NOTATION:
      if (n >= 8) {
...
const byte *enum_sig_subpkt(const subpktarea_t *pktbuf, sigsubpkttype_t reqtype,
  ...
  if (!critical) critical = &critical_dummy;
  ...
  while (buflen) {
    ...
    {
      if (buflen < 4) goto too_short;
      n = buf32_to_size_t(buffer);
      buffer += 4;
      buflen -= 4;
    } else if (n >= 192) /* 4 byte special encoded length header.  */
    {
      if (buflen < 2) goto too_short;
      n = ((n - 192) << 8) + *buffer + 192;
      buffer++;
      buflen--;
    }
    if (buflen < n) goto too_short;
    type = *buffer;
    if (type & 0x80) {
      type &= 0x7f;
      *critical = 1;
    ...
    if (!(++seq > reqseq))
      ...
      if (*critical) {
        if (n - 1 > buflen + 1) goto too_short;
      ...
      dump_sig_subpkt(reqtype == SIGSUBPKT_LIST_HASHED, type, *critical, buffer,
                      buflen, n);
    else if (type == reqtype) /* Found.  */
    {
      buffer++;
      n--;
      if (n > buflen) goto too_short;
      if (ret_n) *ret_n = n;
...
const byte *parse_sig_subpkt2(PKT_signature *sig, sigsubpkttype_t reqtype) {
  ...
  if (!p) p = parse_sig_subpkt(sig->unhashed, reqtype, NULL);
  return p;
}
...
void parse_revkeys(PKT_signature *sig) {
  ...
  if (sig->sig_class != 0x1F) return;

  while ((revkey = enum_sig_subpkt(sig->hashed, SIGSUBPKT_REV_KEY, &len, &seq,
                                   NULL))) {
    if (/* The only valid length is 22 bytes.  See RFC 4880
        ...
        && (revkey[0] & 0x80)) {
      sig->revkey = (revocation_key *)xrealloc(
          sig->revkey, sizeof(struct revocation_key) * (sig->numrevkeys + 1));
...
int parse_signature(IOBUF inp, int pkttype, unsigned long pktlen,
  ...
  if (is_v4 && sig->pubkey_algo) /* Extract required information.  */
  {
    ...
    if (p && *p == 0) sig->flags.revocable = 0;
    ...
    if (p && len == 2) {
      ...
      /* Only look for a regexp if there is also a trust
         subpacket. */
      sig->trust_regexp = parse_sig_subpkt(sig->hashed, SIGSUBPKT_REGEXP, &len);

      /* If the regular expression is of 0 length, there is no
         regular expression. */
      if (len == 0) sig->trust_regexp = NULL;
    }
    ...
    if (p && *p == 0) sig->flags.exportable = 0;

    /* Find all revocation keys.  */
    if (sig->sig_class == 0x1F) parse_revkeys(sig);
  }
...
static int parse_key(IOBUF inp, int pkttype, unsigned long pktlen, byte *hdr,
  ...
  if (!npkey) {
    ...
    pk->pkey[0] = gcry_mpi_set_opaque(NULL, read_rest(inp, pktlen), pktlen * 8);
    pktlen = 0;
    goto leave;
  } else {
    for (i = 0; i < npkey; i++) {
  ...
  if (list_mode) keyid_from_pk(pk, keyid);
  ...
  if (pkttype == PKT_SECRET_KEY || pkttype == PKT_SECRET_SUBKEY) {
    ...
    if (ski->s2k.mode == 1001 || ski->s2k.mode == 1002) {
      ...
      pk->pkey[npkey] = gcry_mpi_set_opaque(NULL, xstrdup("dummydata"), 10 * 8);
      pktlen = 0;
...
int parse_attribute_subpkts(PKT_user_id *uid) {
  ...
  while (buflen) {
    ...
    if (!n) {
      ...
      if (opt.verbose) log_info("attribute subpacket too short\n");
      break;
    }

    attribs = (user_attribute *)xrealloc(
        attribs, (count + 1) * sizeof(struct user_attribute));
...
static int parse_user_id(IOBUF inp, int pkttype, unsigned long pktlen,
  ...
  packet->pkt.user_id =
      (PKT_user_id *)xmalloc_clear(sizeof *packet->pkt.user_id + pktlen);
  packet->pkt.user_id->len = pktlen;
  packet->pkt.user_id->ref = 1;

  p = (byte *)packet->pkt.user_id->name;
  for (; pktlen; pktlen--, p++) *p = iobuf_get_noeof(inp);
  *p = 0;
...
void make_attribute_uidname(PKT_user_id *uid, size_t max_namelen) {
  ...
  if (uid->numattribs <= 0)
    ...
    sprintf(uid->name, "[%d attributes of size %lu]", uid->numattribs,
            uid->attrib_len);
  else {
...
#define EXTRA_UID_NAME_SPACE 71
  packet->pkt.user_id = (PKT_user_id *)xmalloc_clear(
      sizeof *packet->pkt.user_id + EXTRA_UID_NAME_SPACE);
  packet->pkt.user_id->ref = 1;
...
static int parse_comment(IOBUF inp, int pkttype, unsigned long pktlen,
  ...
  packet->pkt.comment =
      (PKT_comment *)xmalloc(sizeof *packet->pkt.comment + pktlen - 1);
...
static int parse_compressed(IOBUF inp, int pkttype, unsigned long pktlen,
  ...
  zd = pkt->pkt.compressed =
      (PKT_compressed *)xmalloc(sizeof *pkt->pkt.compressed);
...
static int parse_encrypted(IOBUF inp, int pkttype, unsigned long pktlen,
  ...
  ed = pkt->pkt.encrypted =
      (PKT_encrypted *)xmalloc(sizeof *pkt->pkt.encrypted);
  /* ed->len is set below.  */
...
/* Note, that this code is not anymore used in real life because the
   ...
   decrypt_data.  */
static int parse_mdc(IOBUF inp, int pkttype, unsigned long pktlen, PACKET *pkt,
                     int new_ctb) {
  int rc = 0;
  PKT_mdc *mdc;
  byte *p;
...
static int parse_gpg_control(IOBUF inp, int pkttype, unsigned long pktlen,
  ...
  for (i = 0; i < sesmarklen; i++, pktlen--) {
    if (sesmark[i] != iobuf_get_noeof(inp)) goto skipit;
  }
  if (pktlen > 4096)
...
PACKET *create_gpg_control(ctrlpkttype_t type, const byte *data,
                           size_t datalen) {
  PACKET *packet;
  byte *p;
  ...
  packet->pkt.gpg_control =
      (PKT_gpg_control *)xmalloc(sizeof *packet->pkt.gpg_control + datalen - 1);
  packet->pkt.gpg_control->control = type;
  packet->pkt.gpg_control->datalen = datalen;
  p = (byte *)packet->pkt.gpg_control->data;
  for (; datalen; datalen--, p++) *p = *data++;
