...
static int mdc_decode_filter(void *opaque, int control, IOBUF a, byte *buf,
                             size_t *ret_len);
static int decode_filter(void *opaque, int control, IOBUF a, byte *buf,
                         size_t *ret_len);
...
static void release_dfx_context(decode_filter_ctx_t dfx) {
  if (!dfx) return;
...
int decrypt_data(ctrl_t ctrl, void *procctx, PKT_encrypted *ed, DEK *dek) {
  ...
  if (!dfx) return gpg_error_from_syserror();
  dfx->refcount = 1;
  ...
  rc = openpgp_cipher_open(
      &dfx->cipher_hd, dek->algo, GCRY_CIPHER_MODE_CFB,
      (GCRY_CIPHER_SECURE |
       ((ed->mdc_method || dek->algo >= 100) ? 0 : GCRY_CIPHER_ENABLE_SYNC)));
  if (rc) {
    ...
    *ret_len = 0;
    ...
    if (n == 44) {
      ...
      if (dfx->partial) {
        ...
        if (!dfx->length) dfx->eof_seen = 1; /* Normal EOF.  */
      }
  ...
  if (control == IOBUFCTRL_UNDERFLOW && fc->eof_seen) {
    *ret_len = 0;
    ...
    if (fc->partial) {
      ...
      if (!fc->length) fc->eof_seen = 1; /* Normal EOF.  */
    }
    if (n) {
      if (fc->cipher_hd) gcry_cipher_decrypt(fc->cipher_hd, buf, n, NULL, 0);
    } else {
      if (!fc->eof_seen) fc->eof_seen = 1;
      rc = -1; /* Return EOF. */
    }
    *ret_len = n;
  } else if (control == IOBUFCTRL_FREE) {
