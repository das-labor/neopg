...
static gcry_mpi_t my_mpi_copy(gcry_mpi_t a) {
...
void free_symkey_enc(PKT_symkey_enc *enc) { xfree(enc); }

void free_pubkey_enc(PKT_pubkey_enc *enc) {
  int n, i;
  n = pubkey_get_nenc((pubkey_algo_t)(enc->pubkey_algo));
  if (!n) mpi_release(enc->data[0]);
  for (i = 0; i < n; i++) mpi_release(enc->data[i]);
  xfree(enc);
}

void free_seckey_enc(PKT_signature *sig) {
  int n, i;

  n = pubkey_get_nsig((pubkey_algo_t)(sig->pubkey_algo));
  if (!n) mpi_release(sig->data[0]);
  for (i = 0; i < n; i++) mpi_release(sig->data[i]);
...
void free_public_key(PKT_public_key *pk) {
  if (pk) {
...
static subpktarea_t *cp_subpktarea(subpktarea_t *s) {
  subpktarea_t *d;
...
PKT_public_key *copy_public_key(PKT_public_key *d, PKT_public_key *s) {
  ...
  if (!d) d = (PKT_public_key *)xmalloc(sizeof *d);
  memcpy(d, s, sizeof *d);
  d->seckey_info = NULL;
  ...
  if (!n)
    ...
    for (; i < n; i++) d->pkey[i] = my_mpi_copy(s->pkey[i]);
  }
  for (; i < PUBKEY_MAX_NSKEY; i++) d->pkey[i] = NULL;

  if (!s->revkey && s->numrevkeys) BUG();
  if (s->numrevkeys) {
    d->revkey = (revocation_key *)xmalloc(sizeof(struct revocation_key) *
                                          s->numrevkeys);
...
PKT_signature *copy_signature(PKT_signature *d, PKT_signature *s) {
  ...
  if (!d) d = (PKT_signature *)xmalloc(sizeof *d);
  memcpy(d, s, sizeof *d);
 ...
 */
PKT_user_id *scopy_user_id(PKT_user_id *s) {
  if (s) s->ref++;
  return s;
}

void free_comment(PKT_comment *rem) { xfree(rem); }
...
void free_compressed(PKT_compressed *zd) {
  if (!zd) return;
...
void free_encrypted(PKT_encrypted *ed) {
  if (!ed) return;
...
void free_plaintext(PKT_plaintext *pt) {
  if (!pt) return;

  if (pt->buf) { /* We need to skip some bytes.  */
    if (pt->is_partial) {
...
void free_packet(PACKET *pkt, parse_packet_ctx_t parsectx) {
  ...
  if (DBG_MEMORY) log_debug("free_packet() type=%d\n", pkt->pkttype);
...
int cmp_public_keys(PKT_public_key *a, PKT_public_key *b) {
  ...
  if (a->version < 4 && a->expiredate != b->expiredate) return -1;
  if (a->pubkey_algo != b->pubkey_algo) return -1;
  ...
  if (!n) { /* unknown algorithm, rest is in opaque MPI */
    ...
    for (i = 0; i < n; i++) {
      if (mpi_cmp(a->pkey[i], b->pkey[i])) return -1;
    }
  }
...
int cmp_signatures(PKT_signature *a, PKT_signature *b) {
  ...
  if (a->keyid[0] != b->keyid[0]) return -1;
  if (a->keyid[1] != b->keyid[1]) return -1;
  if (a->pubkey_algo != b->pubkey_algo) return -1;

  n = pubkey_get_nsig((pubkey_algo_t)(a->pubkey_algo));
  if (!n) return -1; /* can't compare due to unknown algorithm */
  for (i = 0; i < n; i++) {
    if (mpi_cmp(a->data[i], b->data[i])) return -1;
  }
  return 0;
}
...
int cmp_user_ids(PKT_user_id *a, PKT_user_id *b) {
  ...
  if (a->attrib_data && b->attrib_data) {
    ...
    if (!res) res = memcmp(a->attrib_data, b->attrib_data, a->attrib_len);
  } else if (!a->attrib_data && !b->attrib_data) {
    res = a->len - b->len;
    if (!res) res = memcmp(a->name, b->name, a->len);
  }
