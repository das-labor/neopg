...
static gpg_error_t create_context(ctrl_t ctrl, assuan_context_t *r_ctx) {
  gpg_error_t err;
  assuan_context_t ctx;
...
/* Get a context for accessing dirmngr.  If no context is available a
   ...
   R_CTX on error.  */
static gpg_error_t open_context(ctrl_t ctrl, assuan_context_t *r_ctx) {
  gpg_error_t err;
  dirmngr_local_t dml;
  ...
  *r_ctx = NULL;
  for (;;) {
      ...
      if (!dml->set_keyservers_done) {
        ...
        for (ksi = opt.keyserver; ksi; ksi = ksi->next) {
          ...
          if (!line)
            ...
            err = assuan_transact(dml->ctx, line, NULL, NULL, NULL, NULL, NULL,
                                  NULL);
            xfree(line);
          }
    ...
    if (!dml) return gpg_error_from_syserror();
...
static void close_context(ctrl_t ctrl, assuan_context_t ctx) {
  dirmngr_local_t dml;
...
static void clear_context_flags(ctrl_t ctrl, assuan_context_t ctx) {
  dirmngr_local_t dml;
...
static gpg_error_t ks_status_cb(void *opaque, const char *line) {
  struct ks_status_parm_s *parm = (ks_status_parm_s *)opaque;
  gpg_error_t err = 0;
  const char *s, *s2;
  const char *warn;

  if ((s = has_leading_keyword(line,
                               parm->keyword ? parm->keyword : "SOURCE"))) {
    /* Note that the arg for "S SOURCE" is the URL of a keyserver.  */
    if (!parm->source) {
      parm->source = xtrystrdup(s);
      if (!parm->source) err = gpg_error_from_syserror();
    }
    ...
    if (warn) {
      ...
      if (s2) {
        while (*s2 && !spacep(s2)) s2++;
        while (*s2 && spacep(s2)) s2++;
        if (*s2) print_further_info("%s", s2);
      }
    }
  }
...
gpg_error_t gpg_dirmngr_ks_list(ctrl_t ctrl, char **r_keyserver) {
  ...
  if (r_keyserver) *r_keyserver = NULL;

  err = open_context(ctrl, &ctx);
  if (err) return err;

  err = assuan_transact(ctx, "KEYSERVER", NULL, NULL, NULL, NULL, ks_status_cb,
                        &stparm);
  if (err) goto leave;
  if (!stparm.source) {
...
static gpg_error_t ks_search_data_cb(void *opaque, const void *data,
                                     size_t datalen) {
  gpg_error_t err = 0;
  ...
  if (!data) return 0; /* Ignore END commands.  */
...
again:
  ...
  if ((s = (const char *)memchr(line, '\n', rawlen))) {
    ...
    if (linelen + 1 < sizeof fixedbuf) {
      ...
      fixedbuf[linelen] = 0;
      if (linelen && fixedbuf[linelen - 1] == '\r') fixedbuf[linelen - 1] = 0;
...
gpg_error_t gpg_dirmngr_ks_search(ctrl_t ctrl, const char *searchstr,
  ...
  err = assuan_transact(ctx, line, ks_search_data_cb, &parm, NULL, NULL,
                        ks_status_cb, &stparm);
  if (!err) err = cb(cb_value, 0, NULL); /* Send EOF.  */
...
static gpg_error_t ks_get_data_cb(void *opaque, const void *data,
                                  size_t datalen) {
  gpg_error_t err = 0;
...
gpg_error_t gpg_dirmngr_ks_get(ctrl_t ctrl, char **pattern,
  ...
  *r_fp = NULL;
  ...
  err = assuan_transact(ctx, line, ks_get_data_cb, &parm, NULL, NULL,
                        ks_status_cb, &stparm);
  if (err) goto leave;
...
/* Run the KS_FETCH and pass URL as argument.  On success an estream
   ...
   KS_FETCH command.  */
gpg_error_t gpg_dirmngr_ks_fetch(ctrl_t ctrl, const char *url,
                                 estream_t *r_fp) {
  gpg_error_t err;
  assuan_context_t ctx;
  struct ks_get_parm_s parm;
  ...
  *r_fp = NULL;
  ...
  err =
      assuan_transact(ctx, line, ks_get_data_cb, &parm, NULL, NULL, NULL, NULL);
  if (err) goto leave;
...
static void record_output(estream_t output, pkttype_t type,
                          const char *validity,
                          ...
                          u32 creation_date, u32 expiration_date,
                          const char *userid) {
  ...
  if (pub_key_length > 0) pub_key_length_str = xasprintf("%d", pub_key_length);

  if (pub_key_algo != -1) pub_key_algo_str = xasprintf("%d", pub_key_algo);

  if (keyid)
    keyid_str = xasprintf("%08lX%08lX", (unsigned long)keyid[0],
                          (unsigned long)keyid[1]);

  if (creation_date) creation_date_str = xstrdup(colon_strtime(creation_date));
  ...
  es_fprintf(
      output, "%s:%s:%s:%s:%s:%s:%s:%s:%s:%s:%s:%s:%s:%s:%s:%s:%s\n", type_str,
      validity ?: "", pub_key_length_str ?: "", pub_key_algo_str ?: "",
      keyid_str ?: "", creation_date_str ?: "", expiration_date_str ?: "",
      "" /* Certificate S/N */, "" /* Ownertrust.  */, userid_escaped ?: "",
      "" /* Signature class.  */, "" /* Key capabilities.  */,
      "" /* Issuer certificate fingerprint.  */, "" /* Flag field.  */,
      "" /* S/N of a token.  */, "" /* Hash algo.  */, "" /* Curve name.  */);
...
static gpg_error_t ks_put_inq_cb(void *opaque, const char *line) {
  ...
  if (has_leading_keyword(line, "KEYBLOCK")) {
    ...
    for (node = parm->keyblock; !err && node; node = node->next) {
      switch (node->pkt->pkttype) {
        ...
        case PKT_PUBLIC_SUBKEY: {
          ...
          i = 0;
          if (pk->flags.revoked) validity[i++] = 'r';
          if (pk->has_expired) validity[i++] = 'e';
          validity[i] = '\0';

          keyid_from_pk(pk, NULL);

          record_output(fp, node->pkt->pkttype, validity, nbits_from_pk(pk),
                        pk->pubkey_algo, pk->keyid, pk->timestamp,
                        pk->expiredate, NULL);
        } break;
        ...
        case PKT_USER_ID: {
          ...
          if (!uid->attrib_data) {
            ...
            i = 0;
            if (uid->flags.revoked) validity[i++] = 'r';
            if (uid->flags.expired) validity[i++] = 'e';
            validity[i] = '\0';

            record_output(fp, node->pkt->pkttype, validity, -1, -1, NULL,
                          uid->created, uid->expiredate, uid->name);
          }
        } break;
        ...
        case PKT_SIGNATURE: {
          ...
          if (IS_UID_SIG(sig))
            record_output(fp, node->pkt->pkttype, NULL, -1, -1, sig->keyid,
    ...
    if (!err && parm->keyblock) {
      ...
      if (!err && rc) err = gpg_error_from_syserror();
    }
    es_fclose(fp);
  } else
...
gpg_error_t gpg_dirmngr_ks_put(ctrl_t ctrl, void *data, size_t datalen,
  ...
  if (keyblock) merge_keys_and_selfsig(ctrl, keyblock);
  ...
  err = assuan_transact(ctx, "KS_PUT", NULL, NULL, ks_put_inq_cb, &parm, NULL,
                        NULL);
...
static gpg_error_t wkd_data_cb(void *opaque, const void *data, size_t datalen) {
  ...
  if (!data) return 0;        /* Ignore END commands.  */
  if (!parm->memfp) return 0; /* Data is not required.  */
