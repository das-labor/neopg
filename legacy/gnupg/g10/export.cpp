...
/* Local prototypes.  */
static int do_export(ctrl_t ctrl, const std::vector<std::string> &users,
                     int secret, unsigned int options, export_stats_t stats);
...
void export_release_stats(export_stats_t stats) { xfree(stats); }
...
void export_print_stats(export_stats_t stats) {
  ...
  if (is_status_enabled()) {
    ...
    snprintf(buf, sizeof buf, "%lu %lu %lu", stats->count, stats->secret_count,
             stats->exported);
 ...
 */
int export_pubkeys(ctrl_t ctrl, const std::vector<std::string> &users,
 ...
 */
int export_seckeys(ctrl_t ctrl, const std::vector<std::string> &users,
                   unsigned int options, export_stats_t stats) {
 ...
 */
int export_secsubkeys(ctrl_t ctrl, const std::vector<std::string> &users,
                      unsigned int options, export_stats_t stats) {
...
gpg_error_t export_pubkey_buffer(ctrl_t ctrl, const char *keyspec,
                                 unsigned int options, export_stats_t stats,
                                 kbnode_t *r_keyblock, void **r_data,
                                 size_t *r_datalen) {
  gpg_error_t err;
  iobuf_t iobuf;
  int any;
  ...
  *r_datalen = 0;
  ...
  iobuf = iobuf_temp();
  err = do_export_stream(ctrl, iobuf, helplist, 0, r_keyblock, options, stats,
                         &any);
  if (!err && !any) err = GPG_ERR_NOT_FOUND;
  if (!err) {
    const void *src;
    size_t datalen;
...
static void release_subkey_list(subkey_list_t list) {
  while (list) {
...
static int subkey_in_list_p(subkey_list_t list, KBNODE node) {
  if (node->pkt->pkttype == PKT_PUBLIC_SUBKEY ||
    ...
    for (; list; list = list->next)
      if (list->kid[0] == kid[0] && list->kid[1] == kid[1]) return 1;
  }
  return 0;
}

/* Allocate a new subkey list item from NODE. */
static subkey_list_t new_subkey_list_item(KBNODE node) {
...
/* Helper function to check whether the subkey at NODE actually
   ...
   subkey. */
static int exact_subkey_match_p(KEYDB_SEARCH_DESC *desc, KBNODE node) {
  u32 kid[2];
  ...
  switch (desc->mode) {
    case KEYDB_SEARCH_MODE_SHORT_KID:
      if (desc->u.kid[1] == kid[1]) result = 1;
      break;

    case KEYDB_SEARCH_MODE_LONG_KID:
      if (desc->u.kid[0] == kid[0] && desc->u.kid[1] == kid[1]) result = 1;
      break;

    case KEYDB_SEARCH_MODE_FPR16:
      if (!memcmp(desc->u.fpr, fpr, 16)) result = 1;
      break;

    case KEYDB_SEARCH_MODE_FPR20:
    case KEYDB_SEARCH_MODE_FPR:
      if (!memcmp(desc->u.fpr, fpr, 20)) result = 1;
      break;
...
static gpg_error_t match_curve_skey_pk(gcry_sexp_t s_key, PKT_public_key *pk) {
  ...
  if (flags) {
    for (idx = 1; idx < gcry_sexp_length(flags); idx++) {
      ...
      if (flag && (strcmp("eddsa", flag) == 0)) is_eddsa = 1;
      gcry_free(flag);
    }
  }
...
/* Return a canonicalized public key algoithms.  This is used to
   ...
   considered the same).  */
static enum gcry_pk_algos canon_pk_algo(enum gcry_pk_algos algo) {
  switch (algo) {
    case GCRY_PK_RSA:
    case GCRY_PK_RSA_E:
    case GCRY_PK_RSA_S:
...
static gpg_error_t cleartext_secret_key_to_openpgp(gcry_sexp_t s_key,
                                                   PKT_public_key *pk) {
  gpg_error_t err;
  ...
  if (gcry_sexp_length(top_list) != 2) goto bad_seckey;
  key = gcry_sexp_nth(top_list, 1);
  if (!key) goto bad_seckey;
  ...
  switch (canon_pk_algo(pk_algo)) {
    case GCRY_PK_RSA:
      if (!is_RSA(pk->pubkey_algo)) goto bad_pubkey_algo;
      err = gcry_sexp_extract_param(key, NULL, "ne", &pub_params[0],
                                    &pub_params[1], NULL);
      ...
      if (!err) {
        ...
        err = gcry_sexp_extract_param(key, NULL, "dpqu", &pk->pkey[2],
                                      &pk->pkey[3], &pk->pkey[4], &pk->pkey[5],
                                      NULL);
      }
      if (!err) {
        for (idx = 2; idx < 6; idx++) ski->csum += checksum_mpi(pk->pkey[idx]);
      }
      break;

    case GCRY_PK_DSA:
      if (!is_DSA(pk->pubkey_algo)) goto bad_pubkey_algo;
      err = gcry_sexp_extract_param(key, NULL, "pqgy", &pub_params[0],
                                    &pub_params[1], &pub_params[2],
                                    &pub_params[3], NULL);
      ...
      if (!err) {
        ...
        pk->pkey[4] = NULL;
        err = gcry_sexp_extract_param(key, NULL, "x", &pk->pkey[4], NULL);
      }
      if (!err) ski->csum += checksum_mpi(pk->pkey[4]);
      break;

    case GCRY_PK_ELG:
      if (!is_ELGAMAL(pk->pubkey_algo)) goto bad_pubkey_algo;
      err = gcry_sexp_extract_param(key, NULL, "pgy", &pub_params[0],
                                    &pub_params[1], &pub_params[2], NULL);
      ...
      if (!err) {
        ...
        pk->pkey[3] = NULL;
        err = gcry_sexp_extract_param(key, NULL, "x", &pk->pkey[3], NULL);
      }
      if (!err) ski->csum += checksum_mpi(pk->pkey[3]);
      break;
    ...
    case GCRY_PK_ECC:
      ...
      if (err) goto leave;
      if (!err)
        err = gcry_sexp_extract_param(key, NULL, "q", &pub_params[0], NULL);
      if (!err && (gcry_mpi_cmp(pk->pkey[1], pub_params[0])))
        err = GPG_ERR_BAD_PUBKEY;

      sec_start = 2;
      if (pk->pubkey_algo == PUBKEY_ALGO_ECDH) sec_start += 1;
      if (!err) {
        gcry_mpi_release(pk->pkey[sec_start]);
        pk->pkey[sec_start] = NULL;
        err =
            gcry_sexp_extract_param(key, NULL, "d", &pk->pkey[sec_start], NULL);
      }

      if (!err) ski->csum += checksum_mpi(pk->pkey[sec_start]);
      break;
...
leave:
  ...
  for (idx = 0; idx < DIM(pub_params); idx++) gcry_mpi_release(pub_params[idx]);
  return err;
...
/* Use the key transfer format given in S_PGP to create the secinfo
   ...
   secret parameters.  */
static gpg_error_t transfer_format_to_openpgp(gcry_sexp_t s_pgp,
                                              PKT_public_key *pk) {
  gpg_error_t err;
  gcry_sexp_t top_list;
  ...
  if (nskey > PUBKEY_MAX_NSKEY) goto bad_seckey;
  ...
  if (!gcry_mpi_get_flag(skey[npkey], GCRYMPI_FLAG_OPAQUE)) goto bad_seckey;
  if (npkey + 1 < DIM(skey) && skey[npkey + 1]) goto bad_seckey;

  /* Check that the secret key parameters in PK are all set to NULL. */
  for (idx = npkey; idx < nskey; idx++)
    if (pk->pkey[idx]) goto bad_seckey;
...
static void print_status_exported(PKT_public_key *pk) {
  char *hexfpr;
 ...
 */
gpg_error_t receive_seckey_from_agent(ctrl_t ctrl, int cleartext,
                                      char **cache_nonce_addr,
                                      const char *hexgrip, PKT_public_key *pk) {
  gpg_error_t err = 0;
  ...
  if (!realkeylen) goto unwraperror; /* Invalid csexp.  */
...
unwraperror:
  ...
  if (err) {
    log_error(
        ...
        err == GPG_ERR_FULLY_CANCELED ? "" : _(" - skipped"));
  }
  return err;
}
...
gpg_error_t write_keyblock_to_output(kbnode_t keyblock, int with_armor,
  ...
  if (is_secured_filename(fname)) return GPG_ERR_EPERM;
  ...
  for (node = keyblock; node; node = node->next) {
    if (is_deleted_kbnode(node)) continue;
 ...
 * KEYBLOCK must not have any blocks marked as deleted.
 */
static void apply_drop_subkey_filter(ctrl_t ctrl, kbnode_t keyblock,
                                     recsel_expr_t selector) {
  kbnode_t node;
  struct impex_filter_parm_s parm;
  ...
  for (node = keyblock->next; node; node = node->next) {
    if (node->pkt->pkttype == PKT_PUBLIC_SUBKEY ||
      ...
      if (recsel_select(selector, impex_filter_getval, &parm)) {
         ...
         * next subkey.  */
        delete_kbnode(node);
        for (; node->next && node->next->pkt->pkttype != PKT_PUBLIC_SUBKEY &&
               node->next->pkt->pkttype != PKT_SECRET_SUBKEY;
             node = node->next)
          delete_kbnode(node->next);
      }
...
static gpg_error_t do_export_one_keyblock(ctrl_t ctrl, kbnode_t keyblock,
  ...
  for (kbctx = NULL; (node = walk_kbnode(keyblock, &kbctx, 0));) {
    ...
    if (node->pkt->pkttype == PKT_COMMENT) continue;
        ...
        if (subkey_in_list_p(subkey_list, node))
          ...
          for (j = 0; j < ndesc; j++)
            if (j != descindex && desc[j].exact &&
              ...
              break;
          if (!(j < ndesc)) skip_until_subkey = 1; /* No other one matching. */
        }
      }

      if (skip_until_subkey) continue;
    ...
    if (node->pkt->pkttype == PKT_SIGNATURE) {
      ...
      if (!(options & EXPORT_SENSITIVE_REVKEYS) &&
        ...
        for (i = 0; i < node->pkt->pkt.signature->numrevkeys; i++)
          if ((node->pkt->pkt.signature->revkey[i].klasse & 0x40)) break;
        if (i < node->pkt->pkt.signature->numrevkeys) continue;
      }
    }

    /* Don't export attribs? */
    if (!(options & EXPORT_ATTRIBUTES) && node->pkt->pkttype == PKT_USER_ID &&
    ...
    if (secret && (node->pkt->pkttype == PKT_PUBLIC_KEY ||
      ...
      if (err) {
        log_error(
            ...
            " - skipped\n",
            keystr_with_sub(keyid, subkid), gpg_strerror(err));
        skip_until_subkey = 1;
        err = 0;
        continue;
      }
      ...
      if ((!err && serialno) && secret == 2 &&
        ...
        log_info(_("key %s: key material on-card - skipped\n"),
                 keystr_with_sub(keyid, subkid));
        skip_until_subkey = 1;
      } else if (err == GPG_ERR_NOT_FOUND || (!err && serialno)) {
        /* Create a key stub.  */
        struct seckey_info *ski;
        const char *s;
        ...
        err = receive_seckey_from_agent(ctrl, cleartext, &cache_nonce, hexgrip,
                                        pk);
        if (err) {
          if (err == GPG_ERR_FULLY_CANCELED) goto leave;
          skip_until_subkey = 1;
          err = 0;
        } else {
          if ((options & EXPORT_BACKUP))
  ...
  *any = 0;
  ...
  for (;;) {
    ...
    if ((options & EXPORT_CLEAN))
      clean_key(ctrl, keyblock, opt.verbose, (options & EXPORT_MINIMAL), NULL,
                NULL);
...
leave:
  ...
  if (!*any) log_info(_("WARNING: nothing exported\n"));
  return err;
}

static gpg_error_t key_to_sshblob(membuf_t *mb, const char *identifier, ...) {
  va_list arg_ptr;
  gpg_error_t err = 0;
  ...
  while ((a = va_arg(arg_ptr, gcry_mpi_t))) {
    ...
    if (err) break;
    if (!strcmp(identifier, "ssh-ed25519") && buflen > 5 && buf[4] == 0x40) {
...
gpg_error_t export_ssh_key(ctrl_t ctrl, const char *userid) {
  ...
  if (!err) {
    ...
    err = get_pubkey_byname(ctrl, &getkeyctx, NULL, userid, &keyblock, NULL,
  ...
  if (!latest_key) {
    for (node = keyblock; node; node = node->next) {
      ...
      if (!(pk->pubkey_usage & PUBKEY_USAGE_AUTH)) {
        if (DBG_LOOKUP) log_debug("\tsubkey not usable for authentication\n");
        continue;
      }
      if (!pk->flags.valid) {
    ...
    if (!latest_key && (node = keyblock) &&
      ...
      if (!(pk->pubkey_usage & PUBKEY_USAGE_AUTH)) {
        ...
        if (DBG_LOOKUP) log_debug("\tprimary key not yet valid\n");
      } else {
        if (DBG_LOOKUP) log_debug("\tprimary key is fine\n");
        latest_date = pk->timestamp;
        latest_key = node;
      }
    }
  ...
  switch (pk->pubkey_algo) {
    case PUBKEY_ALGO_DSA:
      identifier = "ssh-dss";
      err = key_to_sshblob(&mb, identifier, pk->pkey[0], pk->pkey[1],
                           pk->pkey[2], pk->pkey[3], NULL);
      break;
  ...
  if (es_ferror(fp))
    ...
    if (es_fclose(fp)) err = gpg_error_from_syserror();
    fp = NULL;
  }
