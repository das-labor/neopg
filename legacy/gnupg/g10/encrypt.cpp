...
static int write_pubkey_enc_from_list(ctrl_t ctrl, PK_LIST pk_list, DEK *dek,
                                      iobuf_t out);
...
static int encrypt_simple(const char *filename, int mode, int use_seskey) {
  ...
  if (inp) iobuf_ioctl(inp, IOBUF_IOCTL_NO_CACHE, 1, NULL);
  ...
  if (mode) {
    ...
    cfx.dek =
        passphrase_to_dek(default_cipher_algo(), s2k, 1, 0, NULL, &canceled);
    ...
    if (use_seskey && s2k->mode != 1 && s2k->mode != 3) {
      use_seskey = 0;
      log_info(
          _("can't use a symmetric ESK packet "
            "due to the S2K mode\n"));
    }
  ...
  if (do_compress && cfx.dek && is_file_compressed(filename, &rc)) {
  ...
  if (s2k) {
    PKT_symkey_enc *enc =
        (PKT_symkey_enc *)xmalloc_clear(sizeof *enc + seskeylen + 1);
    enc->version = 4;
  ...
  if (mode) iobuf_push_filter(out, cipher_filter, &cfx);
  ...
  if (rc)
    ...
    if (mode) write_status(STATUS_END_ENCRYPTION);
  }
  if (pt) pt->buf = NULL;
...
int setup_symkey(STRING2KEY **symkey_s2k, DEK **symkey_dek) {
  ...
  *symkey_dek = passphrase_to_dek(opt.s2k_cipher_algo, *symkey_s2k, 1, 0, NULL,
                                  &canceled);
...
static int write_symkey_enc(STRING2KEY *symkey_s2k, DEK *symkey_dek, DEK *dek,
                            iobuf_t out) {
 ...
 */
int encrypt_crypt(
    ctrl_t ctrl, int filefd, const char *filename,
    const std::vector<std::pair<std::string, unsigned int>> &remusr,
    int use_symkey, pk_list_t provided_keys, int outputfd) {
  iobuf_t inp = NULL;
  iobuf_t out = NULL;
  PACKET pkt;
  PKT_plaintext *pt = NULL;
  ...
  if (filefd != -1 && filename) return GPG_ERR_INV_ARG; /* Both given.  */
  ...
  if (use_symkey && (rc = setup_symkey(&symkey_s2k, &symkey_dek))) {
    release_progress_context(pfx);
    return rc;
  }
...
/* Prepare iobufs. */
  ...
  if (!opt.def_cipher_algo) {
    ...
    if (!opt.expert &&
        (select_algo_from_prefs(pk_list, PREFTYPE_SYM, opt.def_cipher_algo,
  ...
  compliant = gnupg_cipher_is_compliant(
      CO_DE_VS, (cipher_algo_t)(cfx.dek->algo), GCRY_CIPHER_MODE_CFB);

  {
    ...
    for (pkr = pk_list; pkr; pkr = pkr->next) {
      ...
      if (compliant &&
          !gnupg_pk_is_compliant(CO_DE_VS, pk->pubkey_algo, pk->pkey, nbits,
                                 NULL))
        compliant = 0;
    }
  }
  ...
  if (DBG_CRYPTO) log_printhex("DEK is: ", cfx.dek->key, cfx.dek->keylen);
  ...
  if (use_symkey &&
      (rc = write_symkey_enc(symkey_s2k, symkey_dek, cfx.dek, out)))
    goto leave;

  pt = setup_plaintext_name(filename, inp);

  /* Get the size of the file if possible, i.e., if it is a real file.  */
  if (filename && *filename && !iobuf_is_pipe_filename(filename) &&
      !opt.textmode) {
    off_t tmpsize;
    int overflow;
  ...
  if (do_compress) {
    ...
    if (compr_algo == -1) {
      ...
      if (compr_algo == -1) compr_algo = DEFAULT_COMPRESS_ALGO;
 ...
 */
int encrypt_filter(void *opaque, int control, iobuf_t a, byte *buf,
                   size_t *ret_len) {
  size_t size = *ret_len;
  ...
  } else if (control == IOBUFCTRL_FLUSH) /* encrypt */
  {
    if (!efx->header_okay) {
      ...
      if (!opt.def_cipher_algo) {
        ...
        if (opt.verbose && efx->cfx.dek->algo == CIPHER_ALGO_3DES)
        ...
        if (!opt.expert &&
            select_algo_from_prefs(efx->pk_list, PREFTYPE_SYM,
                                   ...
                                   NULL) != opt.def_cipher_algo)
          log_info(
              _("forcing symmetric cipher %s (%d) "
                "violates recipient preferences\n"),
              openpgp_cipher_algo_name((cipher_algo_t)(opt.def_cipher_algo)),
              opt.def_cipher_algo);
      ...
      rc = write_pubkey_enc_from_list(efx->ctrl, efx->pk_list, efx->cfx.dek, a);
      if (rc) return rc;

      if (efx->symkey_s2k && efx->symkey_dek) {
        rc =
            write_symkey_enc(efx->symkey_s2k, efx->symkey_dek, efx->cfx.dek, a);
        if (rc) return rc;
      }
 ...
 */
int write_pubkey_enc(ctrl_t ctrl, PKT_public_key *pk, int throw_keyid, DEK *dek,
                     iobuf_t out) {
  PACKET pkt;
  PKT_pubkey_enc *enc;
  int rc;
  ...
  rc = pk_encrypt((pubkey_algo_t)(pk->pubkey_algo), enc->data, frame, pk,
                  pk->pkey);
  ...
  if (rc)
    ...
    if (opt.verbose) {
      ...
      log_info(_("%s/%s encrypted for: \"%s\"\n"),
               ...
               openpgp_cipher_algo_name((cipher_algo_t)(dek->algo)), ustr);
      xfree(ustr);
    }
    /* And write it. */
    init_packet(&pkt);
...
void encrypt_crypt_files(
    ctrl_t ctrl, int nfiles, char **files,
    const std::vector<std::pair<std::string, unsigned int>> &remusr) {
  int rc = 0;
