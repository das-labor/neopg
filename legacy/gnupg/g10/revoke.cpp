...
/* Outputs a minimal pk (as defined by 2440) from a keyblock.  A
   ...
   possible. */
static int export_minimal_pk(IOBUF out, KBNODE keyblock, PKT_signature *revsig,
                             PKT_signature *revkey) {
  KBNODE node;
  PACKET pkt;
  ...
  while (!selfsig) {
    ...
    if (node->pkt->pkt.user_id->attrib_data) continue;
...
int gen_desig_revoke(
  ...
  if (!rc) rc = keydb_search(kdbhd, &desc, 1, NULL);
  if (rc) {
  ...
  if (!pk->revkey && pk->numrevkeys) BUG();
  ...
  for (i = 0; i < pk->numrevkeys; i++) {
    ...
    if (sk_list) {
      for (list = sk_list; list; list = list->next) {
        ...
        /* Don't get involved with keys that don't have 160
           bit fingerprints */
        if (fprlen != 20) continue;

        if (memcmp(fpr, pk->revkey[i].fpr, 20) == 0) break;
      }
      ...
      rc = get_pubkey_byfprint(ctrl, pk2, NULL, pk->revkey[i].fpr,
                               MAX_FINGERPRINT_LEN);
    }
    ...
    if (!rc) {
      ...
      if (!cpr_get_answer_is_yes("gen_desig_revoke.okay",
                                 _("Create a designated revocation certificate "
                                   "for this key? (y/N) ")))
        continue;
      ...
      if (!opt.armor) tty_printf(_("ASCII armored output forced.\n"));

      if ((rc = open_outfile(-1, NULL, 0, 1, &out))) goto leave;
      ...
      /* create it */
      rc = make_keysig_packet(ctrl, &sig, pk, NULL, NULL, pk2, 0x20, 0, 0, 0,
  ...
  if (!any) log_error(_("no revocation keys found for \"%s\"\n"), uname);
...
static int create_revocation(ctrl_t ctrl, const char *filename,
                             struct revocation_reason_info *reason,
                             PKT_public_key *psk, kbnode_t keyblock,
  ...
  if ((rc = open_outfile(-1, filename, suffix, 1, &out))) goto leave;

  if (leadintext) iobuf_writestr(out, leadintext);

  afx->what = 1;
  afx->hdrlines = "Comment: This is a revocation certificate\n";
  push_armor_filter(afx, out);

  rc = make_keysig_packet(ctrl, &sig, psk, NULL, NULL, psk, 0x20, 0, 0, 0,
...
int gen_standard_revoke(ctrl_t ctrl, PKT_public_key *psk,
  ...
  reason.desc = NULL;
  rc = create_revocation(ctrl, fname, &reason, psk, NULL,
                         (const char *)(leadin), 3, cache_nonce);
  if (!rc && !opt.quiet)
...
int gen_revoke(ctrl_t ctrl, const char *uname) {
  ...
  /* Ambiguous.  */
  {
    ...
    while (!rc) {
      ...
      rc = keydb_search(kdbhd, &desc, 1, NULL);
      if (!rc) rc = keydb_get_keyblock(kdbhd, &keyblock);
    }
...
struct revocation_reason_info *ask_revocation_reason(int key_rev, int cert_rev,
  ...
  do {
    ...
    if (key_rev) tty_printf("  2 = %s\n", text_2);
    if (key_rev) tty_printf("  3 = %s\n", text_3);
    if (cert_rev) tty_printf("  4 = %s\n", text_4);
    tty_printf("  Q = %s\n", _("Cancel"));
    if (hint) tty_printf(_("(Probably you want to select %d here)\n"), hint);

    while (code == -1) {
      int n;
      char *answer =
          cpr_get("ask_revocation_reason.code", _("Your decision? "));
      trim_spaces(answer);
      cpr_kill_prompt();
      if (*answer == 'q' || *answer == 'Q') return NULL; /* cancel */
      if (hint && !*answer)
        n = hint;
    ...
    tty_printf(
        _("Enter an optional description; "
