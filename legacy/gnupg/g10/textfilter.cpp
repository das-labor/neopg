...
static unsigned len_without_trailing_chars(byte *line, unsigned len,
...
static int standard(text_filter_context_t *tfx, IOBUF a, byte *buf, size_t size,
                    size_t *ret_len) {
  int rc = 0;
  size_t len = 0;
  unsigned maxlen;
  ...
  while (!rc && len < size) {
    ...
    tfx->buffer_pos = 0;
    tfx->buffer_len =
        iobuf_read_line(a, &tfx->buffer, &tfx->buffer_size, &maxlen);
    if (!maxlen) tfx->truncated++;
    if (!tfx->buffer_len) {
      if (!len) rc = -1; /* eof */
      break;
    }
 ...
 */
int text_filter(void *opaque, int control, IOBUF a, byte *buf,
                size_t *ret_len) {
  size_t size = *ret_len;
  ...
  if (control == IOBUFCTRL_UNDERFLOW) {
    ...
    mem2str((char *)(buf), "text_filter", *ret_len);
  return rc;
}
...
int copy_clearsig_text(IOBUF out, IOBUF inp, gcry_md_hd_t md) {
  ...
  for (;;) {
    ...
    if (!n) break; /* read_line has returned eof */
