...
static int reply(const char *fmt, ...) {
  int result;
  va_list ap;
...
static char *get_passphrase(const char *fname) {
  ...
  fclose(sink);
  if (remove(fname)) {
    fprintf(stderr, "Failed to remove %s: %s", fname, strerror(errno));
    exit(1);
  }

  if (rename(fname_new, fname)) {
    fprintf(stderr, "Failed to rename %s to %s: %s", fname, fname_new,
            strerror(errno));
    exit(1);
  }
...
#define whitespacep(p) \
  (*(p) == ' ' || *(p) == '\t' || *(p) == '\r' || *(p) == '\n')

/* rstrip line.  */
static void rstrip(char *buffer) {
  char *p;
  if (!*buffer) return; /* This is to avoid p = buffer - 1 */
  for (p = buffer + strlen(buffer) - 1; p >= buffer; p--) {
    if (!whitespacep(p)) break;
    *p = 0;
  }
}
...
char *skip_options(const char *line) {
  while (whitespacep(line)) line++;
  while (*line == '-' && line[1] == '-') {
    while (*line && !whitespacep(line)) line++;
    while (whitespacep(line)) line++;
  }
  return (char *)line;
}
...
char *option_value(const char *line, const char *name) {
  ...
  s = strstr(line, name);
  if (s && s >= skip_options(line)) return NULL;
  if (s && (s == line || whitespacep(s - 1)) && s[n] &&
      (whitespacep(s + n) || s[n] == '=')) {
    s += n + 1;
    s += strspn(s, " ");
    if (*s && !whitespacep(s)) return s;
  }
  return NULL;
}
...
restart:
  ...
  while (!feof(stdin)) {
    ...
    if (log_stream) fprintf(log_stream, "< %s", buffer);
