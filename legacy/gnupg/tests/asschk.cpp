...
typedef enum { VARTYPE_SIMPLE = 0, VARTYPE_FD, VARTYPE_COUNTER } VARTYPE;
...
static int writen(int fd, const char *buffer, size_t length) {
  while (length) {
...
static char *read_assuan(int fd) {
  ...
  while (nleft > 0) {
    ...
    if (pending_len) {
      if (pending_len >= nleft) die_0("received line too large");
  ...
  if (!nleft) die_0("received line too large");
...
static void write_assuan(int fd, const char *line) {
  char buffer[1026];
  size_t n = strlen(line);

  if (n > 1024) die_0("line too long for Assuan protocol");
  strcpy(buffer, line);
  if (!n || buffer[n - 1] != '\n') buffer[n++] = '\n';
...
static void start_server(const char *pgmname) {
  int rp[2];
  int wp[2];
  pid_t pid;

  if (pipe(rp) < 0) die_1("pipe creation failed: %s", strerror(errno));
  if (pipe(wp) < 0) die_1("pipe creation failed: %s", strerror(errno));

  fflush(stdout);
  fflush(stderr);
  pid = fork();
  if (pid < 0) die_0("fork failed");
  ...
  if (!pid) {
    ...
    if (!opt_verbose) {
      ...
      if (fd == -1) die_1("can't open '/dev/null': %s", strerror(errno));
  ...
  if (recv_type != LINE_OK) die_0("no greating message");
}
...
static void unset_var(const char *name) {
  ...
  if (var->type == VARTYPE_FD && var->value) {
    ...
    fd = atoi(var->value);
    if (fd != -1 && fd != 0 && fd != 1 && fd != 2) close(fd);
  }
...
static void set_type_var(const char *name, const char *value, VARTYPE type) {
  VARIABLE var;
...
static const char *get_var(const char *name) {
  ...
  if (!var) return NULL;
  if (var->type == VARTYPE_COUNTER &&
      var->value) { /* Use the scratch space allocated by set_var. */
...
static void inc_counter(const char *name) {
  ...
  for (var = variable_list; var; var = var->next) {
    if (var->type == VARTYPE_COUNTER && var->value && !strcmp(var->value, name))
      var->count++;
  }
}
...
/* Expand variables in LINE and return a new allocated buffer if
   ...
   fits into it. */
static char *expand_line(char *buffer) {
  char *line = buffer;
  char *p, *pend;
  const char *value;
  size_t valuelen, n;
  ...
  while (*line) {
    p = strchr(line, '$');
    if (!p) return result; /* nothing more to expand */
    ...
    if (valuelen <= pend - p) {
      ...
      p += valuelen;
      n = pend - p;
      if (n) memmove(p, p + n, strlen(p + n) + 1);
      line = p;
    } else {
...
static int eval_boolean(const char *cond) {
  int true = 1;

  for (; *cond == '!'; cond++) true = !true;
  if (!*cond || (*cond == '0' && !cond[1])) return !true;
  return true;
}

static void cmd_let(const char *assign_to, char *arg) {
  set_var(assign_to, arg);
}

static void cmd_echo(const char *assign_to, char *arg) {
  (void)assign_to;
  if (!opt_no_echo) printf("%s\n", arg);
}

static void cmd_send(const char *assign_to, char *arg) {
  (void)assign_to;
  if (opt_verbose) fprintf(stderr, "sending '%s'\n", arg);
  write_assuan(server_send_fd, arg);
}

static void handle_status_line(char *arg) {
  char *p;
...
static void cmd_expect_ok(const char *assign_to, char *arg) {
  (void)assign_to;
  (void)arg;
  ...
  do {
    ...
    if (opt_verbose > 1) fprintf(stderr, "got line '%s'\n", recv_line);
    if (recv_type == LINE_STAT) handle_status_line(p);
  } while (recv_type != LINE_OK && recv_type != LINE_ERR);
  if (recv_type != LINE_OK) die_1("expected OK but got '%s'", recv_line);
}

static void cmd_expect_err(const char *assign_to, char *arg) {
  (void)assign_to;
  (void)arg;

  if (opt_verbose) fprintf(stderr, "expecting ERR\n");
  do {
    char *p = read_assuan(server_recv_fd);
    if (opt_verbose > 1) fprintf(stderr, "got line '%s'\n", recv_line);
    if (recv_type == LINE_STAT) handle_status_line(p);
  } while (recv_type != LINE_OK && recv_type != LINE_ERR);
  if (recv_type != LINE_ERR) die_1("expected ERR but got '%s'", recv_line);
}

static void cmd_count_status(const char *assign_to, char *arg) {
  char *p;
  ...
  if (*p) {
    for (*p++ = 0; spacep(p); p++)
      ;
    if (*p) die_0("cmpfiles: syntax error");
  }
  set_type_var(assign_to, arg, VARTYPE_COUNTER);
}

static void cmd_openfile(const char *assign_to, char *arg) {
  int fd;
  char numbuf[20];
  ...
  if (fd == -1) die_2("error opening '%s': %s", arg, strerror(errno));

  sprintf(numbuf, "%d", fd);
  set_type_var(assign_to, numbuf, VARTYPE_FD);
}

static void cmd_createfile(const char *assign_to, char *arg) {
  int fd;
  char numbuf[20];
  ...
  while (fd == -1 && errno == EINTR);
  if (fd == -1) die_2("error creating '%s': %s", arg, strerror(errno));

  sprintf(numbuf, "%d", fd);
  set_type_var(assign_to, numbuf, VARTYPE_FD);
}

static void cmd_pipeserver(const char *assign_to, char *arg) {
  (void)assign_to;

  if (!*arg) die_0("syntax error: servername missing");

  start_server(arg);
}

static void cmd_quit_if(const char *assign_to, char *arg) {
  (void)assign_to;

  if (eval_boolean(arg)) exit(0);
}

static void cmd_fail_if(const char *assign_to, char *arg) {
  (void)assign_to;

  if (eval_boolean(arg)) exit(1);
}

static void cmd_cmpfiles(const char *assign_to, char *arg) {
  char *p = arg;
  char *second;
  FILE *fp1, *fp2;
  ...
  if (feof(fp1) && feof(fp2) && !rc) {
    if (opt_verbose) err("files match");
    set_var(assign_to, "1");
  } else if (!rc)
...
static void cmd_getenv(const char *assign_to, char *arg) {
  const char *s;
  s = *arg ? getenv(arg) : "";
...
static int interpreter(char *line) {
  static struct {
    const char *name;
  ...
  for (i = 0; cmdtbl[i].name && strcmp(stmt, cmdtbl[i].name); i++)
    ;
  if (!cmdtbl[i].name) {
    if (!assign_to) die_1("invalid statement '%s'\n", stmt);
    if (save_p) *save_p = save_c;
    set_var(assign_to, stmt);
    free(must_free);
    return 0;
  }

  if (cmdtbl[i].fnc) cmdtbl[i].fnc(assign_to, p);
  free(must_free);
...
int main(int argc, char **argv) {
  ...
  if (argc) die("usage: asschk [--verbose] {-D<name>[=<value>]}");

  while (fgets(buffer, sizeof buffer, stdin)) {
    p = strchr(buffer, '\n');
    if (!p) die_0("incomplete script line");
    *p = 0;
    if (interpreter(buffer)) break;
    fflush(stdout);
  }
  return 0;
}
