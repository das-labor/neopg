...
static gpg_error_t app_new_register(int slot, ctrl_t ctrl, const char *name,
  ...
  if (!want_undefined) {
    ...
    if (!err) err = iso7816_select_file(slot, 0x2F02, 0);
    if (!err) err = iso7816_read_binary(slot, 0, 0, &result, &resultlen);
    if (!err) {
      size_t n;
      const unsigned char *p;

      p = find_tlv_unchecked(result, resultlen, 0x5A, &n);
      if (p) resultlen -= (p - result);
  ...
  if (err == GPG_ERR_CARD_NOT_PRESENT || err == GPG_ERR_ENODEV) goto leave;
  ...
  if (err && (!name || !strcmp(name, "openpgp"))) err = app_select_openpgp(app);
  if (err && (!name || !strcmp(name, "nks"))) err = app_select_nks(app);
  if (err && (!name || !strcmp(name, "p15"))) err = app_select_p15(app);
  if (err && (!name || !strcmp(name, "dinsig"))) err = app_select_dinsig(app);
  if (err && (!name || !strcmp(name, "sc-hsm"))) err = app_select_sc_hsm(app);
  if (err && name && err != GPG_ERR_OBJ_TERM_STATE) err = GPG_ERR_NOT_SUPPORTED;
  ...
  if (scan || !app_top) {
    ...
    while (1) {
      ...
      if (periodical_check_needed_this < 0) {
        ...
        err = app_new_register(slot, ctrl, name, periodical_check_needed_this);
        if (periodical_check_needed_this) periodical_check_needed = 1;
      }

      if (err) apdu_close_reader(slot);
    }
...
void release_application(app_t app, int locked_already) {
  ...
  if (!locked_already) lock_app(app, NULL);

  if (!app->ref_count)
    log_bug("trying to release an already released context\n");

  --app->ref_count;
  if (!locked_already) unlock_app(app);
}
...
/* Write out the application specifig status lines for the LEARN
   command. */
gpg_error_t app_write_learn_status(app_t app, ctrl_t ctrl, unsigned int flags) {
  gpg_error_t err;

  if (!app) return GPG_ERR_INV_VALUE;
  if (!app->fnc.learn_status) return GPG_ERR_UNSUPPORTED_OPERATION;
...
gpg_error_t app_readcert(app_t app, ctrl_t ctrl, const char *certid,
  ...
  if (!app) return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.readcert) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
...
gpg_error_t app_readkey(app_t app, ctrl_t ctrl, int advanced, const char *keyid,
  ...
  if (!app || !keyid || !pk || !pklen) return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.readkey) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
...
gpg_error_t app_getattr(app_t app, ctrl_t ctrl, const char *name) {
  ...
  if (!app || !name || !*name) return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  ...
  if (!app->fnc.getattr) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
...
gpg_error_t app_setattr(app_t app, ctrl_t ctrl, const char *name,
  ...
  if (!app || !name || !*name || !value) return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.setattr) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
...
gpg_error_t app_sign(app_t app, ctrl_t ctrl, const char *keyidstr, int hashalgo,
                     gpg_error_t (*pincb)(void *, const char *, char **),
                     void *pincb_arg, const void *indata, size_t indatalen,
  ...
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.sign) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
  err = app->fnc.sign(app, keyidstr, hashalgo, pincb, pincb_arg, indata,
                      indatalen, outdata, outdatalen);
  unlock_app(app);
  if (opt.verbose) log_info("operation sign result: %s\n", gpg_strerror(err));
  return err;
}
...
gpg_error_t app_auth(app_t app, ctrl_t ctrl, const char *keyidstr,
  ...
  if (!app || !indata || !indatalen || !outdata || !outdatalen || !pincb)
    return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.auth) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
  err = app->fnc.auth(app, keyidstr, pincb, pincb_arg, indata, indatalen,
                      outdata, outdatalen);
  unlock_app(app);
  if (opt.verbose) log_info("operation auth result: %s\n", gpg_strerror(err));
  return err;
}
...
gpg_error_t app_decipher(app_t app, ctrl_t ctrl, const char *keyidstr,
  ...
  *r_info = 0;
  ...
  if (!app || !indata || !indatalen || !outdata || !outdatalen || !pincb)
    return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.decipher) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
  err = app->fnc.decipher(app, keyidstr, pincb, pincb_arg, indata, indatalen,
                          outdata, outdatalen, r_info);
  unlock_app(app);
  if (opt.verbose)
...
gpg_error_t app_writecert(app_t app, ctrl_t ctrl, const char *certidstr,
                          gpg_error_t (*pincb)(void *, const char *, char **),
                          void *pincb_arg, const unsigned char *data,
                          size_t datalen) {
  gpg_error_t err;

  if (!app || !certidstr || !*certidstr || !pincb) return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.writecert) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
  err =
      app->fnc.writecert(app, ctrl, certidstr, pincb, pincb_arg, data, datalen);
  unlock_app(app);
  if (opt.verbose)
...
gpg_error_t app_writekey(app_t app, ctrl_t ctrl, const char *keyidstr,
                         unsigned int flags,
                         gpg_error_t (*pincb)(void *, const char *, char **),
                         void *pincb_arg, const unsigned char *keydata,
                         size_t keydatalen) {
  gpg_error_t err;

  if (!app || !keyidstr || !*keyidstr || !pincb) return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.writekey) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
  err = app->fnc.writekey(app, ctrl, keyidstr, flags, pincb, pincb_arg, keydata,
                          keydatalen);
  unlock_app(app);
  if (opt.verbose)
...
/* Perform a SETATTR operation.  */
gpg_error_t app_genkey(app_t app, ctrl_t ctrl, const char *keynostr,
                       unsigned int flags, time_t createtime,
                       gpg_error_t (*pincb)(void *, const char *, char **),
                       void *pincb_arg) {
  gpg_error_t err;

  if (!app || !keynostr || !*keynostr || !pincb) return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.genkey) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
  err =
...
gpg_error_t app_get_challenge(app_t app, ctrl_t ctrl, size_t nbytes,
  ...
  if (!app || !nbytes || !buffer) return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  err = lock_app(app, ctrl);
  if (err) return err;
...
gpg_error_t app_change_pin(app_t app, ctrl_t ctrl, const char *chvnostr,
                           int reset_mode,
                           gpg_error_t (*pincb)(void *, const char *, char **),
                           void *pincb_arg) {
  gpg_error_t err;

  if (!app || !chvnostr || !*chvnostr || !pincb) return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.change_pin) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
  err = app->fnc.change_pin(app, ctrl, chvnostr, reset_mode, pincb, pincb_arg);
  unlock_app(app);
  if (opt.verbose)
...
gpg_error_t app_check_pin(app_t app, ctrl_t ctrl, const char *keyidstr,
  ...
  if (!app || !keyidstr || !*keyidstr || !pincb) return GPG_ERR_INV_VALUE;
  if (!app->ref_count) return GPG_ERR_CARD_NOT_INITIALIZED;
  if (!app->fnc.check_pin) return GPG_ERR_UNSUPPORTED_OPERATION;
  err = lock_app(app, ctrl);
  if (err) return err;
...
static void report_change(int slot, int old_status, int cur_status) {
  char *homestr, *envstr;
  char *fname;
  char templ[50];
  FILE *fp;
  ...
  for (a = app_top; a; a = app_next) {
    ...
    if (a->reset_requested)
      ...
      if (sw == SW_HOST_NO_READER) {
        ...
        if (a->periodical_check_needed) periodical_check_needed = 1;
        unlock_app(a);
        continue;
      }
    }
