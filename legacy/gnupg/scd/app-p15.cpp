...
/* fixme: we should move the card detection to a   separate file */
#include "apdu.h"
...
/* The AID of PKCS15. */
static unsigned char const pkcs15_aid[] = {0xA0, 0,    0,    0,    0x63, 0x50,
...
static unsigned char const pkcs15be_aid[] = {
    0xA0, 0, 0, 0x01, 0x77, 0x50, 0x4B, 0x43, 0x53, 0x2D, 0x31, 0x35};
...
static void release_cdflist(cdf_object_t a) {
  while (a) {
...
static void release_prkdflist(prkdf_object_t a) {
  while (a) {
...
void release_aodf_object(aodf_object_t a) {
  if (a) {
    xfree(a->objid);
    xfree(a->authid);
    xfree(a->path);
...
static void release_aodflist(aodf_object_t a) {
  while (a) {
...
static gpg_error_t select_and_read_binary(int slot, unsigned short efid,
...
static gpg_error_t select_ef_by_path(app_t app, const unsigned short *path,
  ...
  if (!pathlen) return GPG_ERR_INV_VALUE;
  ...
  if (app->app_local->direct_path_selection) {
    ...
    if (err) {
      ...
      for (j = 0; j < pathlen; j++) log_printf("%04hX", path[j]);
      log_printf(": %s\n", gpg_strerror(err));
      return err;
    }
  } else {
...
static gpg_error_t parse_certid(app_t app, const char *certid,
  ...
  *r_objidlen = 0;
  ...
  if (strncmp(certid, tmpbuf, strlen(tmpbuf))) {
    if (!strncmp(certid, "P15.", 4) ||
        (!strncmp(certid, "P15-", 4) && hexdigitp(certid + 4) &&
         hexdigitp(certid + 5) && hexdigitp(certid + 6) &&
         hexdigitp(certid + 7) && certid[8] == '.'))
  ...
  for (s = certid, objidlen = 0; hexdigitp(s); s++, objidlen++)
    ;
  if (*s || !objidlen || (objidlen % 2)) return GPG_ERR_INV_ID;
  objidlen /= 2;
  objid = (unsigned char *)xtrymalloc(objidlen);
  if (!objid) return gpg_error_from_syserror();
  for (s = certid, i = 0; i < objidlen; i++, s += 2) objid[i] = xtoi_2(s);
  *r_objid = objid;
  *r_objidlen = objidlen;
  return 0;
}
...
static gpg_error_t cdf_object_from_certid(app_t app, const char *certid,
                                          cdf_object_t *r_cdf) {
  gpg_error_t err;
  size_t objidlen;
  unsigned char *objid;
  ...
  if (!cdf)
    for (cdf = app->app_local->useful_certificate_info; cdf; cdf = cdf->next)
      if (cdf->objidlen == objidlen && !memcmp(cdf->objid, objid, objidlen))
        break;
  xfree(objid);
  if (!cdf) return GPG_ERR_NOT_FOUND;
  *r_cdf = cdf;
  return 0;
}
...
static gpg_error_t prkdf_object_from_keyidstr(app_t app, const char *keyidstr,
  ...
  for (prkdf = app->app_local->private_key_info; prkdf; prkdf = prkdf->next)
    if (prkdf->objidlen == objidlen && !memcmp(prkdf->objid, objid, objidlen))
      break;
  xfree(objid);
  if (!prkdf) return GPG_ERR_NOT_FOUND;
  *r_prkdf = prkdf;
  return 0;
}
...
*/
static gpg_error_t read_ef_odf(app_t app, unsigned short odf_fid) {
  gpg_error_t err;
  ...
  while (buflen && *p && *p != 0xff) {
    if (buflen >= 8 && (p[0] & 0xf0) == 0xA0 &&
        !memcmp(p + 1, "\x06\x30\x04\x04\x02", 5)) {
      offset = 6;
    } else if (buflen >= 12 && (p[0] & 0xf0) == 0xA0 &&
    ...
    switch ((p[0] & 0x0f)) {
      case 0:
        value = app->app_local->odf.private_keys;
        break;
      case 1:
        value = app->app_local->odf.public_keys;
        break;
      case 2:
        value = app->app_local->odf.trusted_public_keys;
        break;
      case 3:
        value = app->app_local->odf.secret_keys;
        break;
      case 4:
        value = app->app_local->odf.certificates;
        break;
      case 5:
        value = app->app_local->odf.trusted_certificates;
        break;
      case 6:
        value = app->app_local->odf.useful_certificates;
        break;
      case 7:
        value = app->app_local->odf.data_objects;
        break;
      case 8:
        value = app->app_local->odf.auth_objects;
        break;
      default:
        value = 0;
        break;
    ...
    switch ((p[0] & 0x0f)) {
      case 0:
        app->app_local->odf.private_keys = value;
        break;
      case 1:
        app->app_local->odf.public_keys = value;
        break;
      case 2:
        app->app_local->odf.trusted_public_keys = value;
        break;
      case 3:
        app->app_local->odf.secret_keys = value;
        break;
      case 4:
        app->app_local->odf.certificates = value;
        break;
      case 5:
        app->app_local->odf.trusted_certificates = value;
        break;
      case 6:
        app->app_local->odf.useful_certificates = value;
        break;
      case 7:
        app->app_local->odf.data_objects = value;
        break;
      case 8:
        app->app_local->odf.auth_objects = value;
        break;
      default:
...
/* Parse the BIT STRING with the keyUsageFlags from the
   CommonKeyAttributes. */
static gpg_error_t parse_keyusage_flags(const unsigned char *der, size_t derlen,
                                        keyusage_flags_t *usageflags) {
  unsigned int bits, mask;
  int i, unused, full;

  memset(usageflags, 0, sizeof *usageflags);
  if (!derlen) return GPG_ERR_INV_OBJ;
  ...
  unused %= 8;
  mask = 0;
  for (i = 1; unused; i <<= 1, unused--) mask |= i;
...
*/
static gpg_error_t read_ef_prkdf(app_t app, unsigned short fid,
                                 prkdf_object_t *result) {
  gpg_error_t err;
  ...
  if (!fid) return GPG_ERR_NO_DATA; /* No private keys. */
  ...
  while (n && *p && *p != 0xff) {
    ...
    err = parse_ber_header(&p, &n, &klasse, &tag, &constructed, &ndef, &objlen,
                           &hdrlen);
    if (!err && (objlen > n || tag != TAG_SEQUENCE)) err = GPG_ERR_INV_OBJ;
    if (err) {
    ...
    where = __LINE__;
    err = parse_ber_header(&pp, &nn, &klasse, &tag, &constructed, &ndef,
                           &objlen, &hdrlen);
    if (!err && (objlen > nn || tag != TAG_SEQUENCE)) err = GPG_ERR_INV_OBJ;
    if (err) goto parse_error;
    {
      ...
      /* Search the optional AuthId.  We need to skip the optional
         ...
         (BITSTRING). */
      where = __LINE__;
      err = parse_ber_header(&ppp, &nnn, &klasse, &tag, &constructed, &ndef,
                             &objlen, &hdrlen);
      if (!err && (objlen > nnn || klasse != CLASS_UNIVERSAL))
        err = GPG_ERR_INV_OBJ;
      if (err == GPG_ERR_EOF) goto no_authid;
      if (err) goto parse_error;
    ...
    if (err) goto parse_error;
    {
      ...
      err = parse_ber_header(&ppp, &nnn, &klasse, &tag, &constructed, &ndef,
                             &objlen, &hdrlen);
      if (!err &&
          (objlen > nnn || klasse != CLASS_UNIVERSAL || tag != TAG_BIT_STRING))
        err = GPG_ERR_INV_OBJ;
      if (err) goto parse_error;
      ...
      err = parse_ber_header(&ppp, &nnn, &klasse, &tag, &constructed, &ndef,
                             &objlen, &hdrlen);
      if (err == GPG_ERR_EOF) goto leave_cki;
      if (!err && objlen > nnn) err = GPG_ERR_INV_OBJ;
      if (err) goto parse_error;
    ...
    err = parse_ber_header(&pp, &nn, &klasse, &tag, &constructed, &ndef,
                           &objlen, &hdrlen);
    if (!err && objlen > nn) err = GPG_ERR_INV_OBJ;
    if (err) goto parse_error;
    ...
    if (klasse == CLASS_UNIVERSAL && tag == TAG_SEQUENCE)
      ; /* RSA */
    else if (klasse == CLASS_CONTEXT) {
      switch (tag) {
        case 0:
          errstr = "EC key objects are not supported";
          break;
        case 1:
          errstr = "DH key objects are not supported";
          break;
        case 2:
          errstr = "DSA key objects are not supported";
          break;
        case 3:
          errstr = "KEA key objects are not supported";
          break;
        default:
          errstr = "unknown privateKeyObject";
          break;
      }
      goto parse_error;
    } else {
    ...
    if (klasse != CLASS_UNIVERSAL || tag != TAG_OCTET_STRING || !objlen ||
        (objlen & 1)) {
    ...
    prkdf =
        (prkdf_object_t)xtrycalloc(1, (sizeof *prkdf - sizeof(unsigned short) +
    ...
    if (nn) {
      ...
      err = parse_ber_header(&pp, &nn, &klasse, &tag, &constructed, &ndef,
                             &objlen, &hdrlen);
      if (!err &&
          (objlen > nn || klasse != CLASS_UNIVERSAL || tag != TAG_INTEGER))
        err = GPG_ERR_INV_OBJ;
      if (err) goto parse_error;
      ...
      err = parse_ber_header(&pp, &nn, &klasse, &tag, &constructed, &ndef,
                             &objlen, &hdrlen);
      if (!err && (objlen > nn || klasse != CLASS_CONTEXT || tag != 0))
        err = GPG_ERR_INV_OBJ;
      if (err) goto parse_error;
    ...
    for (i = 0; i < prkdf->objidlen; i++) log_printf("%02X", prkdf->objid[i]);
    log_printf(" path=");
    for (i = 0; i < prkdf->pathlen; i++) log_printf("%04hX", prkdf->path[i]);
    if (prkdf->have_off) log_printf("[%lu/%lu]", prkdf->off, prkdf->len);
    if (prkdf->authid) {
      log_printf(" authid=");
  ...
  parse_error:
    log_error("error parsing PrKDF record (%d): %s - skipped\n", where,
              errstr ? errstr : gpg_strerror(err));
    if (prkdf) {
      xfree(prkdf->objid);
      xfree(prkdf->authid);
      xfree(prkdf);
...
static gpg_error_t read_ef_cdf(app_t app, unsigned short fid,
                               cdf_object_t *result) {
  gpg_error_t err;
  unsigned char *buffer = NULL;
  size_t buflen;
  ...
  if (!fid) return GPG_ERR_NO_DATA; /* No certificates. */
  ...
  while (n && *p && *p != 0xff) {
    ...
    err = parse_ber_header(&pp, &nn, &klasse, &tag, &constructed, &ndef,
                           &objlen, &hdrlen);
    if (!err &&
        (objlen > nn || klasse != CLASS_UNIVERSAL || tag != TAG_SEQUENCE))
      err = GPG_ERR_INV_OBJ;
    if (err) goto parse_error;
    nn = objlen;
    ...
    cdf = (cdf_object_t)xtrycalloc(1, (sizeof *cdf - sizeof(unsigned short) +
                                       objlen / 2 * sizeof(unsigned short)));
    if (!cdf) {
      err = gpg_error_from_syserror();
      goto leave;
    ...
    for (i = 0; i < cdf->objidlen; i++) log_printf("%02X", cdf->objid[i]);
    log_printf(" path=");
    for (i = 0; i < cdf->pathlen; i++) log_printf("%04hX", cdf->path[i]);
    if (cdf->have_off) log_printf("[%lu/%lu]", cdf->off, cdf->len);
    log_printf("\n");
...
/* Read and parse an Authentication Object Directory File identified
   ...
   error code is returned and RESULT won't get changed.  */
static gpg_error_t read_ef_aodf(app_t app, unsigned short fid,
                                aodf_object_t *result) {
  gpg_error_t err;
  unsigned char *buffer = NULL;
  size_t buflen;
  const unsigned char *p;
  ...
  if (!fid) return GPG_ERR_NO_DATA; /* No authentication objects. */
  ...
  while (n && *p && *p != 0xff) {
    ...
    if (klasse == CLASS_UNIVERSAL && tag == TAG_SEQUENCE)
      ...
      switch (tag) {
        case 0:
          errstr = "biometric auth types are not supported";
          break;
        case 1:
          errstr = "authKey auth types are not supported";
          break;
        case 2:
          errstr = "external auth type are not supported";
          break;
        default:
          errstr = "unknown privateKeyObject";
          break;
    ...
    err = parse_ber_header(&pp, &nn, &klasse, &tag, &constructed, &ndef,
                           &objlen, &hdrlen);
    if (!err && (objlen > nn || !objlen || klasse != CLASS_UNIVERSAL ||
                 tag != TAG_BIT_STRING))
      err = GPG_ERR_INV_OBJ;
    if (err) goto parse_error;

    {
      ...
      if ((bits & 0x20)) aodf->pinflags.change_disabled = 1;
      if ((bits & 0x10)) aodf->pinflags.unblock_disabled = 1;
      if ((bits & 0x08)) aodf->pinflags.initialized = 1;
      if ((bits & 0x04)) aodf->pinflags.needs_padding = 1;
      if ((bits & 0x02)) aodf->pinflags.unblocking_pin = 1;
      if ((bits & 0x01)) aodf->pinflags.so_pin = 1;
      /* The second octet. */
      bits = 0;
      if (objlen) {
        bits = *pp++;
      ...
      if ((bits & 0x80)) aodf->pinflags.disable_allowed = 1;
      if ((bits & 0x40)) aodf->pinflags.integrity_protected = 1;
      if ((bits & 0x20)) aodf->pinflags.confidentiality_protected = 1;
      if ((bits & 0x10)) aodf->pinflags.exchange_ref_data = 1;
      /* Skip remaining bits. */
      pp += objlen;
      nn -= objlen;
    }
    ...
    err = parse_ber_header(&pp, &nn, &klasse, &tag, &constructed, &ndef,
                           &objlen, &hdrlen);
    if (!err &&
        (objlen > nn || klasse != CLASS_UNIVERSAL || tag != TAG_ENUMERATED))
      err = GPG_ERR_INV_OBJ;
    if (!err && objlen > sizeof(ul)) err = GPG_ERR_UNSUPPORTED_ENCODING;
    if (err) goto parse_error;
    ...
    if (klasse == CLASS_UNIVERSAL && tag == TAG_GENERALIZED_TIME) {
      pp += objlen;
      nn -= objlen;
    ...
    if (klasse == CLASS_UNIVERSAL || tag == TAG_SEQUENCE) {
      ...
      if (nnn) {
        ...
        err = parse_ber_header(&ppp, &nnn, &klasse, &tag, &constructed, &ndef,
                               &objlen, &hdrlen);
        if (!err &&
            (objlen > nnn || klasse != CLASS_UNIVERSAL || tag != TAG_INTEGER))
          err = GPG_ERR_INV_OBJ;
        if (err) goto parse_error;
        ...
        err = parse_ber_header(&ppp, &nnn, &klasse, &tag, &constructed, &ndef,
                               &objlen, &hdrlen);
        if (!err && (objlen > nnn || klasse != CLASS_CONTEXT || tag != 0))
          err = GPG_ERR_INV_OBJ;
        if (err) goto parse_error;
  ...
  ready:
    ...
    for (i = 0; i < aodf->objidlen; i++) log_printf("%02X", aodf->objid[i]);
    if (aodf->authid) {
      log_printf(" authid=");
      for (i = 0; i < aodf->authidlen; i++) log_printf("%02X", aodf->authid[i]);
    }
    log_printf(" flags=");
    s = "";
    if (aodf->pinflags.case_sensitive)
      log_printf("%scase_sensitive", s), s = ",";
    if (aodf->pinflags.local) log_printf("%slocal", s), s = ",";
    ...
    if (aodf->pinflags.initialized) log_printf("%sinitialized", s), s = ",";
    if (aodf->pinflags.needs_padding) log_printf("%sneeds_padding", s), s = ",";
    if (aodf->pinflags.unblocking_pin)
      log_printf("%sunblocking_pin", s), s = ",";
    if (aodf->pinflags.so_pin) log_printf("%sso_pin", s), s = ",";
    ...
    if (aodf->max_length_valid) log_printf(" max=%lu", aodf->max_length);
    if (aodf->pad_char_valid) log_printf(" pad=0x%02x", aodf->pad_char);
    if (aodf->pin_reference_valid)
      log_printf(" pinref=0x%02lX", aodf->pin_reference);
    if (aodf->pathlen) {
      log_printf(" path=");
      for (i = 0; i < aodf->pathlen; i++) log_printf("%04hX", aodf->path[i]);
      if (aodf->have_off) log_printf("[%lu/%lu]", aodf->off, aodf->len);
    }
    log_printf("\n");
 ...
 */
static gpg_error_t read_ef_tokeninfo(app_t app) {
  gpg_error_t err;
  unsigned char *buffer = NULL;
  size_t buflen;
  const unsigned char *p;
  ...
  err =
      select_and_read_binary(app->slot, 0x5032, "TokenInfo", &buffer, &buflen);
  if (err) return err;
  ...
  err = parse_ber_header(&p, &n, &klasse, &tag, &constructed, &ndef, &objlen,
                         &hdrlen);
  if (!err && (objlen > n || tag != TAG_INTEGER)) err = GPG_ERR_INV_OBJ;
  if (err) goto leave;
...
static gpg_error_t read_p15_info(app_t app) {
  gpg_error_t err;
...
/* Helper to do_learn_status: Send information about all certificates
   ...
   certificate. */
static gpg_error_t send_certinfo(app_t app, ctrl_t ctrl, const char *certtype,
  ...
  for (; certinfo; certinfo = certinfo->next) {
    ...
    p = stpcpy(buf, "P15");
    if (app->app_local->home_df) {
      snprintf(p, 6, "-%04X", (unsigned int)(app->app_local->home_df & 0xffff));
      p += 5;
    }
    p = stpcpy(p, ".");
    bin2hex(certinfo->objid, certinfo->objidlen, p);

    send_status_info(ctrl, "CERTINFO", certtype, strlen(certtype), buf,
                     strlen(buf), NULL, (size_t)0);
    xfree(buf);
  }
  return 0;
}
...
/* Get the keygrip of the private key object PRKDF.  On success the
   ...
   R_GRIPSTR. */
static gpg_error_t keygripstr_from_prkdf(app_t app, prkdf_object_t prkdf,
                                         char *r_gripstr) {
  gpg_error_t err;
  cdf_object_t cdf;
  unsigned char *der;
  ...
  if (!err) err = ksba_cert_init_from_mem(cert, der, derlen);
  xfree(der);
  if (!err) err = app_help_get_keygrip_string(cert, r_gripstr);
...
/* Helper to do_learn_status: Send information about all known
   ...
   send_certinfo(). */
static gpg_error_t send_keypairinfo(app_t app, ctrl_t ctrl,
  ...
  for (; keyinfo; keyinfo = keyinfo->next) {
    ...
    if (err) {
      ...
      send_status_info(ctrl, "KEYPAIRINFO", gripstr, 40, buf, strlen(buf), NULL,
                       (size_t)0);
    }
    xfree(buf);
  }
...
static gpg_error_t do_learn_status(app_t app, ctrl_t ctrl, unsigned int flags) {
  ...
  if (!err) err = send_keypairinfo(app, ctrl, app->app_local->private_key_info);
  ...
  *r_certlen = 0;
  ...
  if (cdf->image) {
    *r_cert = (unsigned char *)xtrymalloc(cdf->imagelen);
    if (!*r_cert) return gpg_error_from_syserror();
  ...
  if (!err && (!buflen || *buffer == 0xff)) err = GPG_ERR_NOT_FOUND;
  if (err) {
  ...
  if (!rootca && klasse == CLASS_UNIVERSAL && tag == TAG_OBJECT_ID &&
      !constructed) {
...
static gpg_error_t do_readcert(app_t app, const char *certid,
  ...
  *r_certlen = 0;
  ...
  if (!err) err = readcert_by_cdf(app, cdf, r_cert, r_certlen);
  return err;
}
...
static gpg_error_t do_getattr(app_t app, ctrl_t ctrl, const char *name) {
  ...
  if (!strcmp(name, "$AUTHKEYID")) {
    ...
    /* We return the ID of the first private keycapable of
       signing. */
    for (prkdf = app->app_local->private_key_info; prkdf; prkdf = prkdf->next)
      if (prkdf->usageflags.sign) break;
    if (prkdf) {
    ...
    if (app->app_local->card_type == CARD_TYPE_BELPIC) {
      ...
      if (!err) err = iso7816_read_binary(app->slot, 0, 0, &buffer, &buflen);
      if (err) {
...
/* Micardo cards require special treatment. This is a helper for the
   ...
   selected key. */
static gpg_error_t micardo_mse(app_t app, unsigned short fid) {
  gpg_error_t err;
  int recno;
  ...
  for (recno = 1, se_num = -1;; recno++) {
    ...
    if (err == GPG_ERR_NOT_FOUND) break; /* ready */
    if (err) {
...
static gpg_error_t do_sign(app_t app, const char *keyidstr, int hashalgo,
  ...
  if (!aodf) {
    ...
    return GPG_ERR_INV_CARD;
  }
  if (aodf->authid) {
    log_error(
        "PIN verification is protected by an "
  ...
  if (app->app_local->card_type == CARD_TYPE_BELPIC) {
    ...
    err = iso7816_manage_security_env(app->slot, 0x41, 0xB6, mse, sizeof mse);
    no_data_padding = 1;
    mse_done = 1;
  }
  if (err) {
  ...
  if (1) {
    ...
    if (errstr) {
      ...
      xfree(pinvalue);
      return err ? err : GPG_ERR_BAD_PIN_METHOD;
    }
  ...
  if (indatalen == 36) {
    ...
    if (hashalgo != MD_USER_TLS_MD5SHA1) return GPG_ERR_UNSUPPORTED_ALGORITHM;
  ...
  if (hashalgo == MD_USER_TLS_MD5SHA1)
    ...
    err =
        iso7816_compute_ds(app->slot, 0, data + 15, 20, 0, outdata, outdatalen);
  else
...
static gpg_error_t do_auth(app_t app, const char *keyidstr,
  ...
  return do_sign(app, keyidstr, algo, pincb, pincb_arg, indata, indatalen,
                 outdata, outdatalen);
}
...
static unsigned short read_home_df(int slot, int *r_belpic) {
  gpg_error_t err;
  unsigned char *buffer;
  const unsigned char *p, *pp;
  size_t buflen, n, nn;
...
gpg_error_t app_select_p15(app_t app) {
  ...
  rc = iso7816_select_application(slot, (const char *)(pkcs15_aid),
                                  sizeof pkcs15_aid, 0);
  if (rc) { /* Not found: Try to locate it from 2F00.  We use direct path
  ...
  if (rc) { /* Still not found:  Try the default DF. */
    def_home_df = 0x5015;
