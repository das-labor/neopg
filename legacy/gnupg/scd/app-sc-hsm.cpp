...
static unsigned char const sc_hsm_aid[] = {0xE8, 0x2B, 0x06, 0x01, 0x04, 0x01,
                                           0x81, 0xC3, 0x1F, 0x02, 0x01};
...
typedef enum { KEY_TYPE_RSA, KEY_TYPE_ECC } key_type_t;
 ...
 */
static gpg_error_t list_ef(int slot, unsigned char **result,
                           size_t *resultlen) {
  int sw;

  if (!result || !resultlen) return GPG_ERR_INV_VALUE;
  *result = NULL;
  *resultlen = 0;

  sw = apdu_send_le(slot, 1, 0x80, 0x58, 0x00, 0x00, -1, NULL, 65536, result,
                    resultlen);
  if (sw != SW_SUCCESS) {
...
static gpg_error_t select_and_read_binary(int slot, unsigned short efid,
  ...
  sw = apdu_send_le(slot, 1, 0x00, 0xB1, efid >> 8, efid & 0xFF, 4,
                    (const char *)(cdata), maxread, buffer, buflen);

  if (sw == SW_EOF_REACHED) sw = SW_SUCCESS;
...
static gpg_error_t parse_certid(const char *certid, unsigned char **r_objid,
                                size_t *r_objidlen) {
  const char *s;
  size_t objidlen;
  unsigned char *objid;
  ...
  *r_objidlen = 0;
  ...
  if (strncmp(certid, "HSM.", 4)) return GPG_ERR_INV_ID;
  certid += 4;
  ...
  A1 [ CONTEXT 1 ] IMPLICIT SEQUENCE SIZE( 10 )
    SEQUENCE SIZE( 8 )
      ...
      INTEGER SIZE( 2 )                   -- modulus size in bits
        0000  08 00
*/
static gpg_error_t read_ef_prkd(app_t app, unsigned short fid,
                                prkdf_object_t *prkdresult,
  ...
  where = __LINE__;
  err = parse_ber_header(&pp, &nn, &klasse, &tag, &constructed, &ndef, &objlen,
                         &hdrlen);
...

  {
    ...
    err = parse_ber_header(&ppp, &nnn, &klasse, &tag, &constructed, &ndef,
                           &objlen, &hdrlen);
    if (!err &&
        (objlen > nnn || klasse != CLASS_UNIVERSAL || tag != TAG_OCTET_STRING))
      err = GPG_ERR_INV_OBJ;
    if (err) goto parse_error;
...
leave:
  ...
  if (err) {
    if (prkdf) {
      if (prkdf->objid) xfree(prkdf->objid);
      xfree(prkdf);
    }
    if (cdf) {
      if (cdf->objid) xfree(cdf->objid);
      xfree(cdf);
    }
  } else {
    if (prkdf) prkdf->next = *prkdresult;
    *prkdresult = prkdf;
    if (cdf) {
      cdf->next = *cdresult;
      *cdresult = cdf;
    ...
    SEQUENCE SIZE( 6 )
      SEQUENCE SIZE( 4 )
        OCTET-STRING SIZE( 2 )                  -- path
          0000  CA 00                                            ..
 */
static gpg_error_t read_ef_cd(app_t app, unsigned short fid,
                              cdf_object_t *result) {
  gpg_error_t err;
  unsigned char *buffer = NULL;
  size_t buflen;
  ...
  err = parse_ber_header(&pp, &nn, &klasse, &tag, &constructed, &ndef, &objlen,
                         &hdrlen);
  if (!err && (objlen > nn || klasse != CLASS_UNIVERSAL || tag != TAG_SEQUENCE))
    err = GPG_ERR_INV_OBJ;
  if (err) goto parse_error;
  nn = objlen;
  ...
  if (klasse != CLASS_UNIVERSAL || tag != TAG_OCTET_STRING || (objlen & 1)) {
...
7F21 [ APPLICATION 33 ] IMPLICIT SEQUENCE SIZE( 226 )
  ...
  5F37 [ APPLICATION 55 ] SIZE( 64 )                            -- Signature
   ...
   are truncated.
 */
static gpg_error_t read_serialno(app_t app) {
  gpg_error_t err;
  unsigned char *buffer = NULL;
  size_t buflen;
  ...
  err = select_and_read_binary(app->slot, 0x2F02, "EF.C_DevAut", &buffer,
                               &buflen, 512);
  if (err) return err;
  ...
  err = parse_ber_header(&p, &n, &klasse, &tag, &constructed, &ndef, &objlen,
                         &hdrlen);
  if (!err && (objlen > n || tag != 0x21)) err = GPG_ERR_INV_OBJ;
  if (err) {
...
static gpg_error_t read_meta(app_t app) {
  gpg_error_t err;
...
/* Helper to do_learn_status: Send information about all certificates
   ...
   certificate. */
static gpg_error_t send_certinfo(ctrl_t ctrl, const char *certtype,
...
static gpg_error_t do_learn_status(app_t app, ctrl_t ctrl, unsigned int flags) {
  ...
  if ((flags & 1))
    ...
    if (!err)
      err =
          send_certinfo(ctrl, "101", app->app_local->trusted_certificate_info);
  }
...
static gpg_error_t readcert_by_cdf(app_t app, cdf_object_t cdf,
  ...
  *r_certlen = 0;
  ...
  err =
      select_and_read_binary(app->slot, cdf->fid, "CD", &buffer, &buflen, 4096);
  if (err) {
...
static void apply_PKCS_padding(const unsigned char *dig, int diglen,
                               const unsigned char *prefix, int prefixlen,
                               unsigned char *buff, int bufflen) {
  int i, n_ff;

  /* Caller must ensure a sufficient buffer.  */
  if (diglen + prefixlen + 4 > bufflen) return;
  n_ff = bufflen - diglen - prefixlen - 3;

  *buff++ = 0x00;
  *buff++ = 0x01;
  for (i = 0; i < n_ff; i++) *buff++ = 0xFF;
  *buff++ = 0x00;

  if (prefix) memcpy(buff, prefix, prefixlen);
  buff += prefixlen;
...
static int hash_from_digestinfo(const unsigned char *di, size_t dilen,
  ...
  err = parse_ber_header(&pp, &nn, &klasse, &tag, &constructed, &ndef, &objlen,
                         &hdrlen);
  if (!err && (objlen > nn || tag != TAG_OCTET_STRING)) err = GPG_ERR_INV_OBJ;
  if (err) return err;

  if (*hashlen < objlen) return GPG_ERR_TOO_SHORT;
 ...
 */
static gpg_error_t verify_pin(app_t app,
  ...
  sw = apdu_send_simple(app->slot, 0, 0x00, ISO7816_VERIFY, 0x00, 0x81, -1,
                        NULL);

  if (sw == SW_SUCCESS) return 0; /* PIN already verified */
...
static gpg_error_t do_sign(app_t app, const char *keyidstr, int hashalgo,
  ...
  if (prkdf->keytype == KEY_TYPE_RSA) {
    ...
    if (indatalen != 20 && indatalen != 28 && indatalen != 32 &&
      ...
      err = hash_from_digestinfo((const unsigned char *)(indata), indatalen,
                                 cdsblk, &cdsblklen);
      if (err) {
 ...
 * buffer.  */
static gpg_error_t strip_PKCS15_padding(unsigned char *src, int srclen,
                                        unsigned char **dst, size_t *dstlen) {
  unsigned char *p;

  if (srclen < 2) return GPG_ERR_DECRYPT_FAILED;
  if (*src++ != 0x00) return GPG_ERR_DECRYPT_FAILED;
  if (*src++ != 0x02) return GPG_ERR_DECRYPT_FAILED;
  srclen -= 2;
  while ((srclen > 0) && *src) {
    src++;
    srclen--;
...
static gpg_error_t do_decipher(app_t app, const char *keyidstr,
  ...
  if (prkdf->keytype != KEY_TYPE_RSA) return GPG_ERR_NOT_SUPPORTED;

  p1blklen = prkdf->keysize >> 3;
  if (!p1blklen) p1blklen = 256;
  ...
  if (!err) *r_info |= APP_DECIPHER_INFO_NOPAD;
...
gpg_error_t app_select_sc_hsm(app_t app) {
  ...
  rc = iso7816_select_application(slot, (const char *)(sc_hsm_aid),
                                  sizeof sc_hsm_aid, 0);
  if (!rc) {
    app->apptype = "SC-HSM";
