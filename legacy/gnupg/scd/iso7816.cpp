...
static gpg_error_t map_sw(int sw) {
  gpg_error_t ec;
  ...
  switch (sw) {
    case SW_EEPROM_FAILURE:
      ...
      break;
    case SW_TERM_STATE:
      ec = GPG_ERR_OBJ_TERM_STATE;
      break;
    case SW_WRONG_LENGTH:
      ec = GPG_ERR_INV_VALUE;
      break;
    case SW_SM_NOT_SUP:
      ec = GPG_ERR_NOT_SUPPORTED;
      break;
    case SW_CC_NOT_SUP:
      ec = GPG_ERR_NOT_SUPPORTED;
      break;
    case SW_CHV_WRONG:
      ...
      break;
    case SW_CHV_BLOCKED:
      ec = GPG_ERR_PIN_BLOCKED;
      break;
    case SW_USE_CONDITIONS:
      ec = GPG_ERR_USE_CONDITIONS;
      break;
    ...
    case SW_HOST_INV_VALUE:
      ec = GPG_ERR_INV_VALUE;
      break;
    case SW_HOST_INCOMPLETE_CARD_RESPONSE:
      ec = GPG_ERR_CARD;
      break;
...
gpg_error_t iso7816_select_application(int slot, const char *aid, size_t aidlen,
                                       unsigned int flags) {
  int sw;
...
gpg_error_t iso7816_select_file(int slot, int tag, int is_dir) {
  ...
  sw = apdu_send_simple(slot, 0, 0x00, CMD_SELECT_FILE, p0, p1, 2,
                        (char *)tagbuf);
  return map_sw(sw);
}

/* Do a select file command with a direct path. */
gpg_error_t iso7816_select_path(int slot, const unsigned short *path,
                                size_t pathlen) {
  int sw, p0, p1;
  unsigned char buffer[100];
  int buflen;

  if (pathlen / 2 >= sizeof buffer) return GPG_ERR_TOO_LARGE;
  ...
  p0 = 0x08;
  p1 = 0x0c; /* No FC return. */
  sw = apdu_send_simple(slot, 0, 0x00, CMD_SELECT_FILE, p0, p1, buflen,
                        (char *)buffer);
  return map_sw(sw);
}
...
gpg_error_t iso7816_list_directory(int slot, int list_dirs,
  ...
  *resultlen = 0;
  ...
  sw = apdu_send(slot, 0, 0x80, 0xAA, list_dirs ? 1 : 2, 0, -1, NULL, result,
                 resultlen);
  if (sw != SW_SUCCESS) {
...
/* This function sends an already formatted APDU to the card.  With
   ...
   buffer.  */
gpg_error_t iso7816_apdu_direct(int slot, const void *apdudata,
                                size_t apdudatalen, int handle_more,
                                unsigned char **result, size_t *resultlen) {
  int sw;
  ...
  *resultlen = 0;
  ...
  sw = apdu_send_direct(slot, 0, (const unsigned char *)(apdudata), apdudatalen,
                        handle_more, result, resultlen);
  if (!sw) {
    if (*resultlen < 2)
...
gpg_error_t iso7816_verify(int slot, int chvno, const char *chv,
                           size_t chvlen) {
  int sw;
...
gpg_error_t iso7816_change_reference_data(int slot, int chvno,
  ...
  if ((!oldchv && oldchvlen) || (oldchv && !oldchvlen) || !newchv || !newchvlen)
    return GPG_ERR_INV_VALUE;

  buf = (char *)xtrymalloc(oldchvlen + newchvlen);
  if (!buf) return gpg_error_from_errno(errno);
  if (oldchvlen) memcpy(buf, oldchv, oldchvlen);
...
gpg_error_t iso7816_reset_retry_counter_with_rc(int slot, int chvno,
  ...
  if (!data || !datalen) return GPG_ERR_INV_VALUE;

  sw = apdu_send_simple(slot, 0, 0x00, CMD_RESET_RETRY_COUNTER, 0, chvno,
                        datalen, data);
  return map_sw(sw);
}

gpg_error_t iso7816_reset_retry_counter(int slot, int chvno, const char *newchv,
                                        size_t newchvlen) {
  int sw;

  sw = apdu_send_simple(slot, 0, 0x00, CMD_RESET_RETRY_COUNTER, 2, chvno,
                        newchvlen, newchv);
  return map_sw(sw);
}
...
gpg_error_t iso7816_get_data(int slot, int extended_mode, int tag,
  ...
  *resultlen = 0;
  ...
  sw =
      apdu_send_le(slot, extended_mode, 0x00, CMD_GET_DATA, ((tag >> 8) & 0xff),
                   (tag & 0xff), -1, NULL, le, result, resultlen);
  if (sw != SW_SUCCESS) {
    /* Make sure that pending buffers are released. */
    xfree(*result);
    *result = NULL;
...
gpg_error_t iso7816_manage_security_env(int slot, int p1, int p2,
  ...
  if (p1 < 0 || p1 > 255 || p2 < 0 || p2 > 255) return GPG_ERR_INV_VALUE;

  sw = apdu_send_simple(slot, 0, 0x00, CMD_MSE, p1, p2, data ? datalen : -1,
                        (const char *)data);
  return map_sw(sw);
}
...
gpg_error_t iso7816_compute_ds(int slot, int extended_mode,
                               ...
                               int le, unsigned char **result,
                               size_t *resultlen) {
  int sw;

  if (!data || !datalen || !result || !resultlen) return GPG_ERR_INV_VALUE;
  *result = NULL;
  *resultlen = 0;
  ...
  sw = apdu_send_le(slot, extended_mode, 0x00, CMD_PSO, 0x9E, 0x9A, datalen,
...
gpg_error_t iso7816_decipher(int slot, int extended_mode,
                             ...
                             int padind, unsigned char **result,
                             size_t *resultlen) {
  int sw;
  unsigned char *buf;
  ...
  *resultlen = 0;
  ...
  if (padind >= 0) {
    ...
    *buf = padind; /* Padding indicator. */
    ...
    sw = apdu_send_le(slot, extended_mode, 0x00, CMD_PSO, 0x80, 0x86,
                      datalen + 1, (char *)buf, le, result, resultlen);
    xfree(buf);
  } else {
    sw = apdu_send_le(slot, extended_mode, 0x00, CMD_PSO, 0x80, 0x86, datalen,
                      (const char *)data, le, result, resultlen);
  }
  if (sw != SW_SUCCESS) {
    /* Make sure that pending buffers are released. */
...
gpg_error_t iso7816_internal_authenticate(int slot, int extended_mode,
  ...
  *resultlen = 0;
  ...
  sw = apdu_send_le(slot, extended_mode, 0x00, CMD_INTERNAL_AUTHENTICATE, 0, 0,
                    datalen, (const char *)data, le, result, resultlen);
  if (sw != SW_SUCCESS) {
    /* Make sure that pending buffers are released. */
    xfree(*result);
    *result = NULL;
...
static gpg_error_t do_generate_keypair(int slot, int extended_mode,
                                       int read_only, const char *data,
                                       size_t datalen, int le,
                                       unsigned char **result,
                                       size_t *resultlen) {
  int sw;
  ...
  *resultlen = 0;
  ...
  sw = apdu_send_le(slot, extended_mode, 0x00, CMD_GENERATE_KEYPAIR,
                    read_only ? 0x81 : 0x80, 0, datalen, data,
                    le >= 0 && le < 256 ? 256 : le, result, resultlen);
  if (sw != SW_SUCCESS) {
    /* Make sure that pending buffers are released. */
    xfree(*result);
    *result = NULL;
...
gpg_error_t iso7816_generate_keypair(int slot, int extended_mode,
                                     ...
                                     unsigned char **result,
                                     size_t *resultlen) {
  return do_generate_keypair(slot, extended_mode, 0, data, datalen, le, result,
                             resultlen);
}
...
gpg_error_t iso7816_read_public_key(int slot, int extended_mode,
                                    const char *data, size_t datalen, int le,
                                    unsigned char **result, size_t *resultlen) {
  return do_generate_keypair(slot, extended_mode, 1, data, datalen, le, result,
                             resultlen);
}

gpg_error_t iso7816_get_challenge(int slot, int length, unsigned char *buffer) {
  int sw;
  unsigned char *result;
  size_t resultlen, n;

  if (!buffer || length < 1) return GPG_ERR_INV_VALUE;

  do {
    result = NULL;
    n = length > 254 ? 254 : length;
    sw = apdu_send_le(slot, 0, 0x00, CMD_GET_CHALLENGE, 0, 0, -1, NULL, n,
                      &result, &resultlen);
    if (sw != SW_SUCCESS) {
      /* Make sure that pending buffers are released. */
      xfree(result);
...
gpg_error_t iso7816_read_binary(int slot, size_t offset, size_t nmax,
  ...
  *resultlen = 0;
  ...
  if (offset > 32767) return GPG_ERR_INV_VALUE;

  do {
    buffer = NULL;
    bufferlen = 0;
    n = read_all ? 0 : nmax;
    sw = apdu_send_le(slot, 0, 0x00, CMD_READ_BINARY, ((offset >> 8) & 0xff),
                      (offset & 0xff), -1, NULL, n, &buffer, &bufferlen);
    ...
    {
      unsigned char *p =
          (unsigned char *)xtryrealloc(*result, *resultlen + bufferlen);
      if (!p) {
...
gpg_error_t iso7816_read_record(int slot, int recno, int reccount, int short_ef,
  ...
  *resultlen = 0;
  ...
  /* We can only encode 15 bits in p0,p1 to indicate an offset. Thus
     we check for this limit. */
  if (recno < 0 || recno > 255 || reccount != 1 || short_ef < 0 ||
      short_ef > 254)
    return GPG_ERR_INV_VALUE;

  buffer = NULL;
  bufferlen = 0;
  sw = apdu_send_le(slot, 0, 0x00, CMD_READ_RECORD, recno,
                    short_ef ? short_ef : 0x04, -1, NULL, 0, &buffer,
                    &bufferlen);
