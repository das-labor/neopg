...
static gpg_error_t do_learn_status(app_t app, ctrl_t ctrl, unsigned int flags) {
  ...
  if (!len) return 0; /* Card has not been personalized. */
  ...
  sprintf(id_buf, "DINSIG.%04X", fid);
  send_status_info(ctrl, "KEYPAIRINFO", hexkeygrip, 40, id_buf, strlen(id_buf),
                   NULL, (size_t)0);
  return 0;
}
...
static gpg_error_t verify_pin(app_t app,
  ...
  if (!opt.disable_pinpad &&
    ...
    rc = pincb(pincb_arg, _("||Please enter your PIN at the reader's pinpad"),
               NULL);
    if (rc) {
  ...
  } else /* No Pinpad.  */
  {
    ...
    if (rc == GPG_ERR_INV_VALUE) {
      ...
      while (i < sizeof paddedpin) paddedpin[i++] = 0xff;
...
static gpg_error_t do_sign(app_t app, const char *keyidstr, int hashalgo,
  ...
  if (strncmp(keyidstr, "DINSIG.", 7)) return GPG_ERR_INV_ID;
  keyidstr += 7;
