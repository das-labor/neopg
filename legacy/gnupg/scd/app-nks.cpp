...
static unsigned char const aid_nks[] = {0xD2, 0x76, 0x00, 0x00,
                                        0x03, 0x01, 0x02};
static unsigned char const aid_sigg[] = {0xD2, 0x76, 0x00, 0x00, 0x66, 0x01};
...
static int all_zero_p(void *buffer, size_t length) {
  char *p;
...
static gpg_error_t keygripstr_from_pk_file(app_t app, int fid,
                                           char *r_gripstr) {
  gpg_error_t err;
  ...
  if (app->app_local->nks_version < 3) {
    ...
    for (i = 0; i < 2; i++) {
      while (buflen[i] - offset[i] > 1 && !buffer[i][offset[i]] &&
  ...
  if (!err)
    err = gcry_sexp_build(&sexp, NULL, "(public-key (rsa (n %b) (e %b)))",
...
static int get_chv_status(app_t app, int sigg, int pwid) {
  ...
  if (apdu_send_direct(app->slot, 0, (unsigned char *)command, 4, 0, &result,
                       &resultlen))
    rc = -1; /* Error. */
  else if (resultlen < 2)
    rc = -1; /* Error. */
...
static gpg_error_t do_getattr(app_t app, ctrl_t ctrl, const char *name) {
  static struct {
    ...
    int special;
      ...
      {"$AUTHKEYID", 1}, {"NKS-VERSION", 2}, {"CHV-STATUS", 3}, {NULL, 0}};
  gpg_error_t err = 0;
  int idx;
  char buffer[100];
  ...
  switch (table[idx].special) {
    ...
    case 2: /* NKS-VERSION */
      ...
      send_status_info(ctrl, table[idx].name, buffer, strlen(buffer), NULL, 0);
      break;
    ...
    case 3: /* CHV-STATUS */
    {
      ...
      snprintf(buffer, sizeof buffer, "%d %d %d %d", tmp[0], tmp[1], tmp[2],
               tmp[3]);
      send_status_info(ctrl, table[idx].name, buffer, strlen(buffer), NULL, 0);
    } break;
...
static void do_learn_status_core(app_t app, ctrl_t ctrl, unsigned int flags,
                                 int is_sigg) {
  gpg_error_t err;
  ...
  for (i = 0; filelist[i].fid; i++) {
    if (filelist[i].nks_ver > app->app_local->nks_version) continue;

    if (!!filelist[i].is_sigg != !!is_sigg) continue;

    if (filelist[i].certtype && !(flags & 1)) {
      size_t len;

      len = app_help_read_length_of_cert(app->slot, filelist[i].fid, NULL);
      if (len) {
        /* FIXME: We should store the length in the application's
           context so that a following readcert does only need to
           read that many bytes. */
        snprintf(ct_buf, sizeof ct_buf, "%d", filelist[i].certtype);
        snprintf(id_buf, sizeof id_buf, "NKS-%s.%04X", tag, filelist[i].fid);
        send_status_info(ctrl, "CERTINFO", ct_buf, strlen(ct_buf), id_buf,
                         strlen(id_buf), NULL, (size_t)0);
      }
      ...
      if (err)
        ...
        snprintf(id_buf, sizeof id_buf, "NKS-%s.%04X", tag, filelist[i].fid);
        send_status_info(ctrl, "KEYPAIRINFO", gripstr, 40, id_buf,
                         strlen(id_buf), NULL, (size_t)0);
      }
    }
  }
...
static gpg_error_t do_learn_status(app_t app, ctrl_t ctrl, unsigned int flags) {
  ...
  if (err) return 0; /* Silently ignore if we can't switch to SigG.  */
...
static gpg_error_t do_readcert(app_t app, const char *certid,
  ...
  *certlen = 0;
  ...
  certid += 9;
  if (!hexdigitp(certid) || !hexdigitp(certid + 1) || !hexdigitp(certid + 2) ||
      !hexdigitp(certid + 3) || certid[4])
    return GPG_ERR_INV_ID;
  fid = xtoi_4(certid);
  for (i = 0; filelist[i].fid; i++)
    if ((filelist[i].certtype || filelist[i].iskeypair) &&
        filelist[i].fid == fid)
      break;
  if (!filelist[i].fid) return GPG_ERR_NOT_FOUND;
  ...
  if (filelist[i].iskeypair) fid = filelist[i].iskeypair;
...
static gpg_error_t do_readkey(app_t app, int advanced, const char *keyid,
  ...
  if (advanced) return GPG_ERR_NOT_SUPPORTED;
  ...
  if (pk && pklen) {
    *pk = make_canon_sexp_from_rsa_pk(buffer[0], buflen[0], buffer[1],
                                      buflen[1], pklen);
    if (!*pk) err = gpg_error_from_syserror();
  }
...
static gpg_error_t do_writekey(
  ...
  if (!force && !do_readkey(app, 0, keyid, NULL, NULL)) return GPG_ERR_EEXIST;

  /* Parse the S-expression.  */
  err = get_rsa_pk_from_canon_sexp(keydata, keydatalen, &rsa_n, &rsa_n_len,
                                   &rsa_e, &rsa_e_len);
  if (err) goto leave;
...
static gpg_error_t basic_pin_checks(const char *pinvalue, int minlen,
                                    int maxlen) {
...
static gpg_error_t do_sign(app_t app, const char *keyidstr, int hashalgo,
  ...
  if (!hexdigitp(keyidstr) || !hexdigitp(keyidstr + 1) ||
      !hexdigitp(keyidstr + 2) || !hexdigitp(keyidstr + 3) || keyidstr[4])
    return GPG_ERR_INV_ID;
  fid = xtoi_4(keyidstr);
  for (i = 0; filelist[i].fid; i++)
    if (filelist[i].iskeypair && filelist[i].fid == fid) break;
  if (!filelist[i].fid) return GPG_ERR_NOT_FOUND;
  if (!filelist[i].issignkey) return GPG_ERR_INV_ID;
  kid = filelist[i].kid;

  /* Prepare the DER object from INDATA.  */
  if (app->app_local->nks_version > 2 &&
      (indatalen == 35 || indatalen == 47 || indatalen == 51 ||
       indatalen == 67 || indatalen == 83)) {
  ...
  if (app->app_local->nks_version > 2) {
    ...
    mse[4] = 1;
    mse[5] = kid;
    rc = iso7816_manage_security_env(app->slot, 0x41, 0xB6, mse, sizeof mse);
  }
  /* Verify using PW1.CH.  */
  if (!rc) rc = verify_pin(app, 0, NULL, pincb, pincb_arg);
  /* Compute the signature.  */
  if (!rc)
    rc =
        iso7816_compute_ds(app->slot, 0, data, datalen, 0, outdata, outdatalen);
  return rc;
}
...
static gpg_error_t do_decipher(app_t app, const char *keyidstr,
  ...
  for (i = 0; filelist[i].fid; i++)
    if (filelist[i].iskeypair && filelist[i].fid == fid) break;
  if (!filelist[i].fid) return GPG_ERR_NOT_FOUND;
  if (!filelist[i].isenckey) return GPG_ERR_INV_ID;
  kid = filelist[i].kid;
  ...
  if (app->app_local->nks_version > 2) {
    ...
    mse[4] = 1;
    mse[5] = kid;
    rc = iso7816_manage_security_env(app->slot, 0x41, 0xB8, mse, sizeof mse);
  } else {
    static const unsigned char mse[] = {
        0x80, 1, 0x10, /* Select algorithm RSA. */
        0x84, 1, 0x81  /* Select local secret key 1 for decryption. */
    };
    rc = iso7816_manage_security_env(app->slot, 0xC1, 0xB8, mse, sizeof mse);
  }
 ...
 */
static const char *parse_pwidstr(const char *pwidstr, int new_mode, int *r_sigg,
                                 int *r_pwid) {
  const char *desc;
  ...
  if (!pwidstr)
    ...
    *r_pwid = 0x00;
    ...
    desc = (new_mode ? _("|N|Please enter a new PIN for the standard keys.")
                     : _("||Please enter the PIN for the standard keys."));
  } else if (!strcmp(pwidstr, "PW2.CH")) {
    *r_pwid = 0x01;
    desc = (new_mode ? _("|NP|Please enter a new PIN Unblocking Code (PUK) "
    ...
    *r_pwid = 0x81;
    *r_sigg = 1;
    desc = (new_mode ? _("|N|Please enter a new PIN for the key to create "
...
static gpg_error_t do_change_pin(
    app_t app, ctrl_t ctrl, const char *pwidstr, unsigned int flags,
    gpg_error_t (*pincb)(void *, const char *, char **), void *pincb_arg) {
  gpg_error_t err;
  char *newpin = NULL;
  char *oldpin = NULL;
  ...
  if (!newdesc) return GPG_ERR_INV_ID;
  ...
  if ((flags & APP_CHANGE_FLAG_RESET)) {
    ...
    err = iso7816_reset_retry_counter_with_rc(app->slot, pwid, data, datalen);
    wipememory(data, datalen);
    xfree(data);
  } else
    err = iso7816_change_reference_data(app->slot, pwid, oldpin, oldpinlen,
                                        newpin, newpinlen);
leave:
  xfree(oldpin);
  xfree(newpin);
...
static gpg_error_t do_check_pin(app_t app, const char *pwidstr,
                                gpg_error_t (*pincb)(void *, const char *,
                                                     char **),
                                void *pincb_arg) {
  gpg_error_t err;
...
static int get_nks_version(int slot) {
  unsigned char *result = NULL;
  size_t resultlen;
  int type;

  if (iso7816_apdu_direct(slot, "\x80\xaa\x06\x00\x00", 5, 0, &result,
                          &resultlen))
...
static gpg_error_t switch_application(app_t app, int enable_sigg) {
  ...
  if (enable_sigg)
    err = iso7816_select_application(app->slot, (const char *)(aid_sigg),
                                     sizeof aid_sigg, 0);
  else
    err = iso7816_select_application(app->slot, (const char *)(aid_nks),
                                     sizeof aid_nks, 0);
  ...
  if (!err && enable_sigg && app->app_local->nks_version >= 3 &&
    ...
    if (!err) err = iso7816_read_record(app->slot, 1, 1, 0, &buffer, &buflen);
    if (!err) {
      tmpl = find_tlv(buffer, buflen, 0x7a, &tmpllen);
      if (tmpl && tmpllen == 12 &&
          !memcmp(tmpl, "\x93\x02\x00\x01\xA4\x06\x83\x01\x81\x83\x01\x83", 12))
...
gpg_error_t app_select_nks(app_t app) {
  ...
  rc = iso7816_select_application(slot, (const char *)(aid_nks), sizeof aid_nks,
                                  0);
  if (!rc) {
    app->apptype = "NKS";
