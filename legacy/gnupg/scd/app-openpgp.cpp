...
static gpg_error_t do_auth(app_t app, const char *keyidstr,
                           ...
                           void *pincb_arg, const void *indata,
                           size_t indatalen, unsigned char **outdata,
                           size_t *outdatalen);
static void parse_algorithm_attribute(app_t app, int keyno);
static gpg_error_t change_keyattr_from_string(
    app_t app, gpg_error_t (*pincb)(void *, const char *, char **),
    void *pincb_arg, const void *value, size_t valuelen);

/* Deconstructor. */
static void do_deinit(app_t app) {
...
static gpg_error_t get_cached_data(app_t app, int tag, unsigned char **result,
                                   size_t *resultlen, int get_immediate,
                                   int try_extlen) {
  gpg_error_t err;
  int i;
  unsigned char *p;
    ...
    for (c = app->app_local->cache; c; c = c->next)
      if (c->tag == tag) {
        if (c->length) {
          ...
          if (!p) return gpg_error_from_errno(errno);
  ...
  *resultlen = len;
  ...
  if (get_immediate) return 0;

  for (i = 0; data_objects[i].tag; i++)
    if (data_objects[i].tag == tag) {
      if (data_objects[i].dont_cache) return 0;
      break;
    }

  /* Okay, cache it. */
  for (c = app->app_local->cache; c; c = c->next) assert(c->tag != tag);
...
static void flush_cache_item(app_t app, int tag) {
  struct cache_s *c, *cprev;
  int i;

  if (!app->app_local) return;
...
/* Flush all entries from the cache which might be out of sync after
   an error. */
static void flush_cache_after_error(app_t app) {
  int i;
...
static void flush_cache(app_t app) {
  if (app && app->app_local) {
    struct cache_s *c, *c2;
...
static void *get_one_do(app_t app, int tag, unsigned char **result,
                        size_t *nbytes, int *r_rc) {
  int rc, i;
  ...
  *r_rc = 0;
  ...
  if (data_objects[i].tag && data_objects[i].get_from) {
    rc = get_cached_data(
        app, data_objects[i].get_from, &buffer, &buflen,
...
static void dump_all_do(int slot) {
  int rc, i, j;
  unsigned char *buffer;
  size_t buflen;

  for (i = 0; data_objects[i].tag; i++) {
    if (data_objects[i].get_from) continue;
    ...
    if (rc == GPG_ERR_NO_OBJ)
      ...
      if (data_objects[i].binary) {
        ...
        log_info("DO '%s': '%.*s'\n", data_objects[i].desc, (int)buflen,
                 buffer); /* FIXME: sanitize */
      ...
      if (data_objects[i].constructed) {
        for (j = 0; data_objects[j].tag; j++) {
          ...
          if (j == i || data_objects[i].tag != data_objects[j].get_from)
            continue;
          value = find_tlv_unchecked(buffer, buflen, data_objects[j].tag,
                                     &valuelen);
          if (!value)
            ; /* not found */
            ...
            if (data_objects[j].binary) {
              ...
              log_info("DO '%s': '%.*s'\n", data_objects[j].desc, (int)valuelen,
                       value); /* FIXME: sanitize */
          }
        }
      }
    }
...
static unsigned int count_bits(const unsigned char *a, size_t len) {
...
*/
static void parse_login_data(app_t app) {
    ...
    if (buflen > 1 && *buffer == 'F' && buffer[1] == '=') {
      ...
      if (buflen) {
        if (digitp(buffer)) {
          ...
          if (q >= (char *)buffer + buflen || *q == '\x18' || *q == '\n')
            m = n;
          else {
            if (*q++ != ',' || !digitp(q)) goto next;
            m = strtol(q, &q, 10);
          }

          if (buflen < ((unsigned char *)q - buffer)) break;

          buflen -= ((unsigned char *)q - buffer);
          buffer = (unsigned char *)q;

          if (buflen && !(*buffer == '\n' || *buffer == '\x18')) goto next;
...
static gpg_error_t store_fpr(app_t app, int keynumber, u32 timestamp,
  ...
  for (i = 0; i < argc; i++) {
    ...
    if (algo == PUBKEY_ALGO_RSA || i == 1) n += 2;
    n += mlen[i];
  }
  va_end(ap);
...
static void send_fpr_if_not_null(ctrl_t ctrl, const char *keyword, int number,
  ...
  for (i = 0; i < 20 && !fpr[i]; i++)
    ;
  if (i == 20) return; /* All zero. */
  bin2hex(fpr, 20, buf);
  if (number == -1)
    *numbuf = 0; /* Don't print the key number */
  else
    sprintf(numbuf, "%d", number);
  send_status_info(ctrl, keyword, numbuf, (size_t)strlen(numbuf), buf,
                   (size_t)strlen(buf), NULL, 0);
}

static void send_fprtime_if_not_null(ctrl_t ctrl, const char *keyword,
  ...
  send_status_info(ctrl, keyword, numbuf1, (size_t)strlen(numbuf1), numbuf2,
                   (size_t)strlen(numbuf2), NULL, 0);
}

static void send_key_data(ctrl_t ctrl, const char *name, const unsigned char *a,
                          size_t alen) {
  char *buffer, *buf;
  size_t buflen;
  ...
  for (; buflen > 768; buflen -= 768, buf += 768)
    send_status_info(ctrl, "KEY-DATA", "-", 1, buf, 768, NULL, 0);
  send_status_info(ctrl, "KEY-DATA", name, (size_t)strlen(name), buf, buflen,
                   NULL, 0);
  xfree(buffer);
}

static void send_key_attr(ctrl_t ctrl, app_t app, const char *keyword,
                          int keyno) {
  char buffer[200];
  ...
  if (app->app_local->keyattr[keyno].key_type == KEY_TYPE_RSA)
    snprintf(buffer, sizeof buffer, "%d 1 rsa%u %u %d", keyno + 1,
    ...
    snprintf(buffer, sizeof buffer, "%d %d %s", keyno + 1,
...
static int determine_rsa_response(app_t app, int keyno) {
  int size;
...
static gpg_error_t do_getattr(app_t app, ctrl_t ctrl, const char *name) {
  ...
  for (idx = 0; table[idx].name && strcmp(table[idx].name, name); idx++)
    ;
  if (!table[idx].name) return GPG_ERR_INV_NAME;
  ...
  if (table[idx].special == -2) {
    ...
    snprintf(
        ...
        app->app_local->extcap.get_challenge, app->app_local->extcap.key_import,
        app->app_local->extcap.change_force_chv,
        app->app_local->extcap.private_dos,
        app->app_local->extcap.max_certlen_3,
        app->app_local->extcap.algo_attr_change,
        (app->app_local->extcap.sm_supported
             ? (app->app_local->extcap.sm_algo == 0
                    ? CIPHER_ALGO_3DES
                    : (app->app_local->extcap.sm_algo == 1
                           ? CIPHER_ALGO_AES
                           : CIPHER_ALGO_AES256))
             : 0),
        app->app_local->status_indicator, app->app_local->extcap.has_decrypt,
  ...
  if (table[idx].special == -5) {
    for (i = 0; i < 3; i++) send_key_attr(ctrl, app, table[idx].name, i);
    return 0;
  }
  ...
  if (relptr) {
    if (table[idx].special == 1) {
      ...
      send_status_info(ctrl, table[idx].name, numbuf, strlen(numbuf), NULL, 0);
...
static char *get_disp_name(app_t app) {
  int rc;
  void *relptr;
  unsigned char *value;
  size_t valuelen;
 ...
 * returned.  */
static char *get_disp_serialno(app_t app) {
  char *serial = app_get_serialno(app);
...
static int get_remaining_tries(app_t app, int adminpw) {
  void *relptr;
  unsigned char *value;
  size_t valuelen;
  int remaining;
...
/* Retrieve the fingerprint from the card inserted in SLOT and write
   ...
   error code. */
#if GNUPG_MAJOR_VERSION > 1
static gpg_error_t retrieve_fpr_from_card(app_t app, int keyno, char *fpr) {
  gpg_error_t err = 0;
  void *relptr;
  unsigned char *value;
  size_t valuelen;
...
#if GNUPG_MAJOR_VERSION > 1
static gpg_error_t retrieve_key_material(FILE *fp, const char *hexkeyid,
  ...
  for (;;) {
    ...
    if (!nfields) continue; /* No fields at all - skip line.  */
    ...
    if (strcmp(fields[0], "pkd")) continue; /* Not a key data record.  */
    if (nfields < 4 || (i = atoi(fields[1])) < 0 || i > 1 || (!i && m_new) ||
        (i && e_new)) {
      err = GPG_ERR_GENERAL;
...
static gpg_error_t rsa_read_pubkey(app_t app, ctrl_t ctrl, u32 created_at,
                                   int keyno, const unsigned char *data,
                                   size_t datalen, gcry_sexp_t *r_sexp) {
  gpg_error_t err;
  ...
  if (ctrl) {
    ...
    err = store_fpr(app, keyno, created_at, fprbuf, PUBKEY_ALGO_RSA, m, mlen, e,
                    elen);
    if (err) return err;
  ...
  err = gcry_sexp_build(r_sexp, NULL, "(public-key(rsa(n%b)(e%b)))", (int)mlen,
                        mbuf, (int)elen, ebuf);
leave:
  xfree(mbuf);
  xfree(ebuf);
...
static const unsigned char *ecdh_params(const char *curve) {
  unsigned int nbits;
...
static gpg_error_t ecc_read_pubkey(app_t app, ctrl_t ctrl, u32 created_at,
                                   int keyno, const unsigned char *data,
                                   size_t datalen, gcry_sexp_t *r_sexp) {
  gpg_error_t err;
  ...
  if ((app->app_local->keyattr[keyno].ecc.flags &
       ECC_FLAG_DJB_TWEAK)) { /* Prepend 0x40 prefix.  */
    *qbuf = 0x40;
  ...
  if (keyno == 1) {
    if (ctrl) send_key_data(ctrl, "kdf/kek", ecdh_params(curve), (size_t)4);
  ...
  if (ctrl) {
    ...
    err = store_fpr(app, keyno, created_at, fprbuf, algo, oidbuf, oid_len, qbuf,
                    ecc_q_len, ecdh_params(curve), (size_t)4);
    if (err) goto leave;
 ...
 */
static gpg_error_t read_public_key(app_t app, ctrl_t ctrl, u32 created_at,
                                   int keyno, const unsigned char *buffer,
                                   size_t buflen) {
  gpg_error_t err;
  ...
  if (app->app_local->keyattr[keyno].key_type == KEY_TYPE_RSA)
    err = rsa_read_pubkey(app, ctrl, created_at, keyno, data, datalen, &s_pkey);
  else if (app->app_local->keyattr[keyno].key_type == KEY_TYPE_ECC)
    err = ecc_read_pubkey(app, ctrl, created_at, keyno, data, datalen, &s_pkey);
  else
...
#if GNUPG_MAJOR_VERSION > 1
static gpg_error_t get_public_key(app_t app, int keyno) {
  gpg_error_t err = 0;
  unsigned char *buffer;
  const unsigned char *m, *e;
  size_t buflen;
  ...
  if (keyno < 0 || keyno > 2) return GPG_ERR_INV_ID;

  /* Already cached? */
  if (app->app_local->pk[keyno].read_done) return 0;
  ...
  if (app->card_version > 0x0100) {
    ...
    err = iso7816_read_public_key(
        app->slot, exmode, (keyno == 0 ? "\xB6" : keyno == 1 ? "\xB8" : "\xA4"),
...
static gpg_error_t send_keypair_info(app_t app, ctrl_t ctrl, int key) {
  int keyno = key - 1;
  gpg_error_t err = 0;
/* Note that GnuPG 1.x does not need this and it would be too time
  ...
  assert(keyno >= 0 && keyno <= 2);
  if (!app->app_local->pk[keyno].key) goto leave; /* No such key - ignore. */

  err = keygrip_from_canon_sexp(app->app_local->pk[keyno].key,
                                app->app_local->pk[keyno].keylen, grip);
  if (err) goto leave;

  bin2hex(grip, 20, gripstr);

  sprintf(idbuf, "OPENPGP.%d", keyno + 1);
  send_status_info(ctrl, "KEYPAIRINFO", gripstr, 40, idbuf, strlen(idbuf), NULL,
                   (size_t)0);
...
static gpg_error_t do_learn_status(app_t app, ctrl_t ctrl, unsigned int flags) {
  (void)flags;
  ...
  if (app->card_version > 0x0100) do_getattr(app, ctrl, "KEY-TIME");
  ...
  if (app->app_local->extcap.private_dos) {
    ...
    if (app->did_chv2) do_getattr(app, ctrl, "PRIVATE-DO-3");
    if (app->did_chv3) do_getattr(app, ctrl, "PRIVATE-DO-4");
  }
...
/* Handle the READKEY command for OpenPGP.  On success a canonical
   ...
   returned.  */
static gpg_error_t do_readkey(app_t app, int advanced, const char *keyid,
  ...
  if (!buf) return GPG_ERR_NO_PUBKEY;
...
static gpg_error_t do_readcert(app_t app, const char *certid,
  ...
  *cert = NULL;
  *certlen = 0;
  if (strcmp(certid, "OPENPGP.3")) return GPG_ERR_INV_ID;
  if (!app->app_local->extcap.is_v2) return GPG_ERR_NOT_FOUND;

  relptr = get_one_do(app, 0x7F21, &buffer, &buflen, NULL);
  if (!relptr) return GPG_ERR_NOT_FOUND;
...
static int check_pinpad_request(app_t app, pininfo_t *pininfo, int admin_pin) {
...
static char *get_prompt_info(app_t app, int chvno, unsigned long sigcount,
                             int remaining) {
  ...
  if (chvno == 1) {
    ...
    result =
        xtryasprintf(_("%s"
                       ...
                       "Counter\x1f: %lu"
                       "%s"),
                     "\x1e", serial, disp_name ? disp_name : "", sigcount, "");
  } else {
    result = xtryasprintf(_("%s"
                            "Number\x1f: %s%%0A"
                            "Holder\x1f: %s"
                            "%s"),
                          "\x1e", serial, disp_name ? disp_name : "", "");
  }
  xfree(disp_name);
  xfree(serial);
...
/* Verify a CHV either using the pinentry or if possible by
   ...
   */
static gpg_error_t verify_a_chv(
    app_t app, gpg_error_t (*pincb)(void *, const char *, char **),
    void *pincb_arg, int chvno, unsigned long sigcount, char **pinvalue) {
  int rc = 0;
  ...
  *pinvalue = NULL;
  ...
  if (remaining == -1) return GPG_ERR_CARD;

  if (chvno == 2 && app->app_local->flags.def_chv2) {
    /* Special case for def_chv2 mechanism. */
    if (opt.verbose) log_info(_("using default PIN as %s\n"), "CHV2");
...

  {
    ...
    char *infoblock =
        get_prompt_info(app, chvno, sigcount, remaining < 3 ? remaining : -1);
  ...
  if (!opt.disable_pinpad &&
    ...
    rc = iso7816_verify(app->slot, 0x80 + chvno, *pinvalue, strlen(*pinvalue));
  }
...
static gpg_error_t verify_chv2(app_t app,
  ...
  if (app->did_chv2) return 0; /* We already verified CHV2.  */
  ...
  if (!app->did_chv1 && !app->force_chv1 && pinvalue) {
    ...
    if (rc == GPG_ERR_BAD_PIN) rc = GPG_ERR_PIN_NOT_SYNCED;
    if (rc) {
      log_error(_("verify CHV%d failed: %s\n"), 1, gpg_strerror(rc));
      flush_cache_after_error(app);
    } else
...
static gpg_error_t build_enter_admin_pin_prompt(app_t app, char **r_prompt) {
  int remaining;
  char *prompt;
  char *infoblock;
  ...
  *r_prompt = NULL;
  ...
  prompt =
      strconcat(_("|A|Please enter the Admin PIN"), "%0A%0A", infoblock, NULL);
  xfree(infoblock);
  if (!prompt) return gpg_error_from_syserror();
...
static gpg_error_t verify_chv3(app_t app,
                               gpg_error_t (*pincb)(void *, const char *,
                                                    char **),
                               void *pincb_arg) {
  int rc = 0;
...
/* Handle the SETATTR operation. All arguments are already basically
   checked. */
static gpg_error_t do_setattr(app_t app, const char *name,
                              gpg_error_t (*pincb)(void *, const char *,
                                                   char **),
                              void *pincb_arg, const unsigned char *value,
                              size_t valuelen) {
  gpg_error_t rc;
  int idx;
  static struct {
  ...
  if (table[idx].need_v2 && !app->app_local->extcap.is_v2)
    return GPG_ERR_NOT_SUPPORTED; /* Not yet supported.  */
...
/* Handle the WRITECERT command for OpenPGP.  This rites the standard
   ...
   callback.  */
static gpg_error_t do_writecert(app_t app, ctrl_t ctrl, const char *certidstr,
                                gpg_error_t (*pincb)(void *, const char *,
                                                     char **),
                                void *pincb_arg, const unsigned char *certdata,
                                size_t certdatalen) {
  (void)ctrl;
#if GNUPG_MAJOR_VERSION > 1
  if (strcmp(certidstr, "OPENPGP.3")) return GPG_ERR_INV_ID;
  if (!certdata || !certdatalen) return GPG_ERR_INV_ARG;
  if (!app->app_local->extcap.is_v2) return GPG_ERR_NOT_SUPPORTED;
 ...
 */
static gpg_error_t do_change_pin(
    app_t app, ctrl_t ctrl, const char *chvnostr, unsigned int flags,
  ...
  if (!app->app_local->extcap.is_v2) {
    ...
    if (!opt.disable_pinpad &&
        !iso7816_check_pinpad(app->slot, ISO7816_CHANGE_REFERENCE_DATA,
                              &pininfo) &&
  ...
  if (resetcode) {
    ...
    if (!buffer)
      ...
      rc = iso7816_reset_retry_counter_with_rc(app->slot, 0x81, buffer,
                                               strlen(buffer));
    ...
    if (strlen(pinvalue) < 8) {
      ...
      rc = iso7816_put_data(app->slot, 0, 0xD3, pinvalue, strlen(pinvalue));
  } else if (reset_mode) {
    rc = iso7816_reset_retry_counter(app->slot, 0x81, pinvalue,
                                     strlen(pinvalue));
    if (!rc && !app->app_local->extcap.is_v2)
      rc = iso7816_reset_retry_counter(app->slot, 0x82, pinvalue,
                                       strlen(pinvalue));
  } else if (!app->app_local->extcap.is_v2) {
    /* Version 1 cards.  */
    if (chvno == 1 || chvno == 2) {
      rc = iso7816_change_reference_data(app->slot, 0x81, NULL, 0, pinvalue,
                                         strlen(pinvalue));
      if (!rc)
        rc = iso7816_change_reference_data(app->slot, 0x82, NULL, 0, pinvalue,
                                           strlen(pinvalue));
    } else /* CHVNO == 3 */
    {
    ...
    if (use_pinpad) {
      rc =
          pincb(pincb_arg,
                chvno == 3 ? _("||Please enter the Admin PIN and New Admin PIN")
                           : _("||Please enter the PIN and New PIN"),
                NULL);
      if (rc) {
        log_info(_("PIN callback returned error: %s\n"), gpg_strerror(rc));
        goto leave;
      ...
      rc = iso7816_change_reference_data(app->slot, 0x80 + chvno, oldpinvalue,
                                         strlen(oldpinvalue), pinvalue,
                                         strlen(pinvalue));
  }
  ...
  if (rc) flush_cache_after_error(app);
...
static gpg_error_t does_key_exist(app_t app, int keyidx, int generating,
                                  int force) {
  const unsigned char *fpr;
  unsigned char *buffer;
  size_t buflen, n;
...
static size_t add_tlv(unsigned char *buffer, unsigned int tag, size_t length) {
...
static gpg_error_t build_privkey_template(
    app_t app, int keyno, const unsigned char *rsa_n, size_t rsa_n_len,
    const unsigned char *rsa_e, size_t rsa_e_len, const unsigned char *rsa_p,
    size_t rsa_p_len, const unsigned char *rsa_q, size_t rsa_q_len,
    const unsigned char *rsa_u, size_t rsa_u_len, const unsigned char *rsa_dp,
    size_t rsa_dp_len, const unsigned char *rsa_dq, size_t rsa_dq_len,
  ...
  *tp++ = 0;
  ...
  template_size = (1 + 3 /* 0x4d and len. */
                   + exthdr_len + privkey_len + suffix_len + datalen);
  tp = template_x = (unsigned char *)xtrymalloc_secure(template_size);
  if (!template_x) return gpg_error_from_syserror();
...
static gpg_error_t build_ecc_privkey_template(
    app_t app, int keyno, const unsigned char *ecc_d, size_t ecc_d_len,
    const unsigned char *ecc_q, size_t ecc_q_len, unsigned char **result,
    size_t *resultlen) {
  ...
  *tp++ = 0;
  ...
  template_size = (1 + 1 /* 0x4d and len. */
                   + exthdr_len + privkey_len + suffix_len + datalen);
  if (exthdr_len + privkey_len + suffix_len + datalen >= 128) template_size++;
  tp = template_x = (unsigned char *)xtrymalloc_secure(template_size);
  if (!template_x) return gpg_error_from_syserror();
...
static gpg_error_t change_keyattr(
    app_t app, int keyno, const unsigned char *buf, size_t buflen,
    gpg_error_t (*pincb)(void *, const char *, char **), void *pincb_arg) {
  gpg_error_t err;
...
static gpg_error_t change_rsa_keyattr(app_t app, int keyno, unsigned int nbits,
                                      gpg_error_t (*pincb)(void *, const char *,
                                                           char **),
                                      void *pincb_arg) {
  gpg_error_t err = 0;
...
static gpg_error_t change_keyattr_from_string(
  ...
  string = (char *)xtrymalloc(valuelen + 1);
  if (!string) return gpg_error_from_syserror();
  memcpy(string, value, valuelen);
  string[valuelen] = 0;
  ...
  if (keyno < 0 || keyno > 2)
    ...
    err = change_keyattr(app, keyno, (const unsigned char *)(string), oid_len,
                         pincb, pincb_arg);
    gcry_mpi_release(oid);
  } else
...
static gpg_error_t rsa_writekey(
    app_t app, gpg_error_t (*pincb)(void *, const char *, char **),
    void *pincb_arg, int keyno, const unsigned char *buf, size_t buflen,
    int depth) {
  gpg_error_t err;
  ...
  while (!(err = parse_sexp(&buf, &buflen, &depth, &tok, &toklen)) && depth &&
         depth >= last_depth1) {
    if (tok) {
      err = GPG_ERR_UNKNOWN_SEXP;
      goto leave;
    }
    if ((err = parse_sexp(&buf, &buflen, &depth, &tok, &toklen))) goto leave;
    if (tok && toklen == 1) {
  ...
  if (opt.verbose) log_info("RSA modulus size is %u bits\n", nbits);
  if (nbits && nbits != maxbits && app->app_local->extcap.algo_attr_change) {
    /* Try to switch the key to a new length.  */
    err = change_rsa_keyattr(app, keyno, nbits, pincb, pincb_arg);
    if (!err) maxbits = app->app_local->keyattr[keyno].rsa.n_bits;
  }
  if (nbits != maxbits) {
  ...
  if (app->app_local->extcap.is_v2) {
    ...
    err = build_privkey_template(app, keyno, rsa_n, rsa_n_len, rsa_e, rsa_e_len,
                                 rsa_p, rsa_p_len, rsa_q, rsa_q_len, rsa_u,
                                 rsa_u_len, rsa_dp, rsa_dp_len, rsa_dq,
                                 rsa_dq_len, &template_x, &template_len);
    xfree(rsa_u);
    xfree(rsa_dp);
    xfree(rsa_dq);
    ...
    if (app->app_local->cardcap.ext_lc_le && template_len > 254)
      ...
      exmode = 0;
    err = iso7816_put_data_odd(app->slot, exmode, 0x3fff, template_x,
                               template_len);
  } else {
    ...
    */
    assert(rsa_e_len <= 4);
    template_len = (1 + 1 + 4 + 1 + 1 + rsa_p_len + 1 + 1 + rsa_q_len);
  ...
  err = store_fpr(app, keyno, created_at, fprbuf, PUBKEY_ALGO_RSA, rsa_n,
                  rsa_n_len, rsa_e, rsa_e_len);
  if (err) goto leave;
...
static gpg_error_t ecc_writekey(
    app_t app, gpg_error_t (*pincb)(void *, const char *, char **),
    void *pincb_arg, int keyno, const unsigned char *buf, size_t buflen,
    int depth) {
  gpg_error_t err;
  ...
  while (!(err = parse_sexp(&buf, &buflen, &depth, &tok, &toklen)) && depth &&
    ...
    if (tok && toklen == 5 && !memcmp(tok, "curve", 5)) {
      ...
      if ((err = parse_sexp(&buf, &buflen, &depth, &tok, &toklen))) goto leave;
      if (tok && buf2) {
        if (!native) /* Strip off leading zero bytes and save. */
          for (; toklen && !*tok; toklen--, tok++)
            ;
  ...
  if (app->app_local->extcap.is_v2) {
    ...
    err = build_ecc_privkey_template(app, keyno, ecc_d, ecc_d_len, ecc_q,
                                     ecc_q_len, &template_x, &template_len);
    if (err) goto leave;
  ...
  err = store_fpr(app, keyno, created_at, fprbuf, algo, oidbuf, oid_len, ecc_q,
                  ecc_q_len, ecdh_params(curve), (size_t)4);
...
static gpg_error_t do_writekey(
    app_t app, ctrl_t ctrl, const char *keyid, unsigned int flags,
    gpg_error_t (*pincb)(void *, const char *, char **), void *pincb_arg,
...
static gpg_error_t do_genkey(app_t app, ctrl_t ctrl, const char *keynostr,
  ...
  if (app->app_local->keyattr[keyno].key_type == KEY_TYPE_RSA) {
    ...
    if (keybits > 4096) return GPG_ERR_TOO_LARGE;
  ...
  send_status_info(ctrl, "KEY-CREATED-AT", numbuf, (size_t)strlen(numbuf), NULL,
                   0);
...
static gpg_error_t compare_fingerprint(app_t app, int keyno,
...
/* If a fingerprint has been specified check it against the one on the
   ...
   that this is okay. */
static gpg_error_t check_against_given_fingerprint(app_t app, const char *fpr,
                                                   int key) {
  ...
  for (s = fpr, n = 0; n < 20; s += 2, n++) tmp[n] = xtoi_2(s);
...
*/
static gpg_error_t do_sign(app_t app, const char *keyidstr, int hashalgo,
                           gpg_error_t (*pincb)(void *, const char *, char **),
                           void *pincb_arg, const void *indata,
                           size_t indatalen, unsigned char **outdata,
                           size_t *outdatalen) {
  ...
  if (!keyidstr || !*keyidstr) return GPG_ERR_INV_VALUE;
...
#define X(a, b, c, d)                                   \
  ...
  if (indatalen == 20)
    ...
    X(SHA1, sha1, 20, 1)
  else X(RMD160, rmd160, 20, 1) else X(
      SHA224, sha224, 28,
      app->app_local->extcap
          .is_v2) else X(SHA256, sha256, 32,
                         app->app_local->extcap
                             .is_v2) else X(SHA384, sha384, 48,
                                            app->app_local->extcap
                                                .is_v2) else X(SHA512, sha512,
                                                               64,
                                                               app->app_local
                                                                   ->extcap
                                                                   .is_v2) else if ((indatalen ==
                                                                                         28 ||
                                                                                     indatalen ==
                                                                                         32 ||
                                                                                     indatalen ==
                                                                                    ...
                                                                                    app->app_local
                                                                                        ->extcap
                                                                                        .is_v2); /* Assume a plain SHA-3 digest has been given.  */
  else {
...
#undef X
  ...
  if (!strcmp(keyidstr, "OPENPGP.1"))
    ...
    for (s = keyidstr, n = 0; n < 16; s += 2, n++) tmp_sn[n] = xtoi_2(s);

    if (app->serialnolen != 16) return GPG_ERR_INV_CARD;
    if (memcmp(app->serialno, tmp_sn, 16)) return GPG_ERR_WRONG_CARD;
  }
...
#define X(a, b, d)                                       \
  ...
  if (use_auth ||
    ...
    X(SHA1, sha1, 1)
    else X(RMD160, rmd160, 1) else X(SHA224, sha224, app->app_local->extcap.is_v2) else X(
        SHA256, sha256,
        app->app_local->extcap
            .is_v2) else X(SHA384, sha384,
                           app->app_local->extcap
                               .is_v2) else X(SHA512, sha512,
                                              app->app_local->extcap
                                                  .is_v2) else return GPG_ERR_UNSUPPORTED_ALGORITHM;
  } else {
    datalen = indatalen;
...
#undef X
  ...
  if (use_auth) {
    return do_auth(app, "OPENPGP.3", pincb, pincb_arg, data, datalen, outdata,
                   outdatalen);
  }
  ...
  rc = iso7816_compute_ds(app->slot, exmode, data, datalen, le_value, outdata,
                          outdatalen);
  return rc;
}
...
static gpg_error_t do_auth(app_t app, const char *keyidstr,
  ...
  if (!rc) {
    ...
    rc = iso7816_internal_authenticate(
        app->slot, exmode, (const unsigned char *)(indata), indatalen, le_value,
        outdata, outdatalen);
  }
  return rc;
}

static gpg_error_t do_decipher(app_t app, const char *keyidstr,
                               gpg_error_t (*pincb)(void *, const char *,
                                                    char **),
                               void *pincb_arg, const void *indata,
                               size_t indatalen, unsigned char **outdata,
                               size_t *outdatalen, unsigned int *r_info) {
  int rc;
  unsigned char tmp_sn[20]; /* actually 16 but we use it also for the fpr. */
  const char *s;
  int n;
  ...
  if (!keyidstr || !*keyidstr || !indatalen) return GPG_ERR_INV_VALUE;
  ...
  if ((indatalen == 16 + 1 || indatalen == 32 + 1) &&
    ...
    else if (!*(const char *)indata &&
             (indatalen == 129 || indatalen == 193 || indatalen == 257 ||
              indatalen == 385 || indatalen == 513))
      fixuplen = -1;
    else
      fixuplen = 0;
    ...
    if (fixuplen > 0) {
      ...
      if (!fixbuf) return gpg_error_from_syserror();
    ...
    if (old_format_len) {
      ...
      memcpy(fixbuf + fixuplen + 32 - old_format_len, indata, old_format_len);
    } else {
      memcpy(fixbuf + fixuplen, indata, indatalen);
    }
    indata = fixbuf;
  ...
  rc = iso7816_decipher(app->slot, exmode, (const unsigned char *)(indata),
                        indatalen, le_value, padind, outdata, outdatalen);
  xfree(fixbuf);
  ...
  if (rc == GPG_ERR_CARD /* actual SW is 0x640a */
      && app->app_local->manufacturer == 5 && app->card_version == 0x0200)
    log_info(
...
static gpg_error_t do_check_pin(app_t app, const char *keyidstr,
                                gpg_error_t (*pincb)(void *, const char *,
                                                     char **),
                                void *pincb_arg) {
  ...
  if (admin_pin) {
    ...
    if (!count) {
      ...
      log_info(
          _("verification of Admin PIN is currently prohibited "
...
static void show_caps(struct app_local_s *s) {
...
/* Parse the historical bytes in BUFFER of BUFLEN and store them in
   APPLOC.  */
static void parse_historical(struct app_local_s *apploc,
...
static void parse_algorithm_attribute(app_t app, int keyno) {
  ...
  if (opt.verbose) log_info("Key-Attr-%s ..: ", desc[keyno]);
  if (*buffer == PUBKEY_ALGO_RSA && (buflen == 5 || buflen == 6)) {
      ...
      app->app_local->keyattr[keyno].rsa.format =
          (buffer[5] == 0
               ...
               : buffer[5] == 1
                     ...
                     : buffer[5] == 2
                           ? RSA_CRT
                           : buffer[5] == 3 ? RSA_CRT_N : RSA_UNKNOWN_FMT);
    ...
    if (opt.verbose)
      log_printf(
          ...
          app->app_local->keyattr[keyno].rsa.format == RSA_STD
              ? "std"
              : app->app_local->keyattr[keyno].rsa.format == RSA_STD_N
                    ? "std+n"
    ...
    if (buffer[buflen - 1] == 0x00 ||
        buffer[buflen - 1] ==
            0xff) { /* Found "pubkey required"-byte for private key template. */
      oidlen--;
    ...
    if (!curve)
      ...
      if (*buffer == PUBKEY_ALGO_EDDSA ||
          (*buffer == PUBKEY_ALGO_ECDH &&
           !strcmp(app->app_local->keyattr[keyno].ecc.curve, "Curve25519")))
        app->app_local->keyattr[keyno].ecc.flags |= ECC_FLAG_DJB_TWEAK;
      if (opt.verbose)
        log_printf(
            "ECC, curve=%s%s\n", app->app_local->keyattr[keyno].ecc.curve,
...
gpg_error_t app_select_openpgp(app_t app) {
  ...
  /* Note that the card can't cope with P2=0xCO, thus we need to pass a
     special flag value. */
  rc =
      iso7816_select_application(slot, (const char *)(aid), sizeof aid, 0x0001);
  if (!rc) {
    ...
    if (app->card_version >= 0x0200) app->app_local->extcap.is_v2 = 1;
    ...
    if (buflen >= 10) {
      ...
      app->app_local->extcap.max_get_challenge = (buffer[2] << 8 | buffer[3]);
