...
static int make_find(struct cdb_make *cdbmp, const void *key, cdbi_t klen,
                     cdbi_t hval, struct cdb_rl **rlp);
static int make_write(struct cdb_make *cdbmp, const char *ptr, cdbi_t len);
...
/* Read data from cdb file, starting at position pos of length len,
   ...
   error. */
int cdb_read(const struct cdb *cdbp, void *buf, unsigned len, cdbi_t pos) {
...
int cdb_find(struct cdb *cdbp, const void *key, cdbi_t klen) {
...
int cdb_findinit(struct cdb_find *cdbfp, struct cdb *cdbp, const void *key,
  ...
  if (key) {
    ...
    if (!n) return 0;           /* The hash table is empry. */
    pos = cdb_unpack(cdbfp->cdb_htp);
    if (n > (cdbp->cdb_fsize >> 3) || pos > cdbp->cdb_fsize ||
    ...
    while (cdbfp->cdb_httodo) {
      ...
      if (n) {
        ...
        if (cdb_unpack(cdbp->cdb_mem + pos) == cdbfp->cdb_klen) {
          ...
          if (memcmp(cdbfp->cdb_key, cdbp->cdb_mem + pos + 8,
                     cdbfp->cdb_klen) == 0) {
            n = cdb_unpack(cdbp->cdb_mem + pos + 4);
  ...
  } else /* Walk over all entries. */
  {
    do {
      while (cdbfp->cdb_htp >= cdbfp->cdb_htend) {
        if (cdbfp->cdb_hval > 255) return 0; /* No more items. */
        ...
        if (n > (cdbp->cdb_fsize / 8) || pos > cdbp->cdb_fsize ||
            cdbfp->cdb_httodo > cdbp->cdb_fsize - pos) {
          gpg_err_set_errno(EPROTO);
          return -1;
        }
...
int cdb_seek(int fd, const void *key, unsigned klen, cdbi_t *dlenp) {
  ...
  if (lseek(fd, pos, SEEK_SET) < 0 || cdb_bread(fd, rbuf, 8) < 0) return -1;
  if ((htsize = cdb_unpack(rbuf + 4)) == 0) return 0;
  hti = (hval >> 8) % htsize; /* start position in hash table */
  httodo = htsize;
  htstart = cdb_unpack(rbuf);

  for (;;) {
    if (needseek && lseek(fd, htstart + (hti << 3), SEEK_SET) < 0) return -1;
    if (cdb_bread(fd, rbuf, 8) < 0) return -1;
    ...
    if (cdb_unpack(rbuf) != hval) /* hash value not matched */
      ...
      if (cdb_unpack(rbuf) == klen) { /* key length matches */
        ...
        if (*dlenp) *dlenp = cdb_unpack(rbuf + 4); /* save value length */
        for (;;) {
          if (!l) /* the whole key read and matches, return */
            return 1;
          c = l > sizeof(rbuf) ? sizeof(rbuf) : l;
          if (cdb_bread(fd, rbuf, c) < 0) return -1;
...
/* Add record with key (KEY,KLEN) and value (VAL,VLEN) to a database.
   ...
   indicator. */
int cdb_make_add(struct cdb_make *cdbmp, const void *key, cdbi_t klen,
...
int cdb_make_put(struct cdb_make *cdbmp, const void *key, cdbi_t klen,
                 const void *val, cdbi_t vlen, int flags) {
  unsigned char rlen[8];
...
static int match(int fd, cdbi_t pos, const char *key, cdbi_t klen) {
  unsigned char buf[64]; /*XXX cdb_buf may be used here instead */
  if (lseek(fd, pos, SEEK_SET) < 0 || read(fd, buf, 8) != 8) return -1;
  if (cdb_unpack(buf) != klen) return 0;

  while (klen > sizeof(buf)) {
    if (read(fd, buf, sizeof(buf)) != sizeof(buf)) return -1;
    if (memcmp(buf, key, sizeof(buf)) != 0) return 0;
    key += sizeof(buf);
    klen -= sizeof(buf);
  }
  if (klen) {
    if (read(fd, buf, klen) != klen) return -1;
    if (memcmp(buf, key, klen) != 0) return 0;
  }
  return 1;
}
...
static int make_find(struct cdb_make *cdbmp, const void *key, cdbi_t klen,
  ...
  while (rl) {
    for (i = rl->cnt - 1; i >= 0; --i) { /* search backward */
      if (rl->rec[i].hval != hval) continue;
      ...
      if (!r) continue;
      if (r < 0) return -1;
      if (lseek(cdbmp->cdb_fd, cdbmp->cdb_dpos, SEEK_SET) < 0) return -1;
      if (rlp) *rlp = rl;
      return i + 1;
    }
    rl = rl->next;
  }
  if (sought && lseek(cdbmp->cdb_fd, cdbmp->cdb_dpos, SEEK_SET) < 0) return -1;
  return 0;
}

int cdb_make_exists(struct cdb_make *cdbmp, const void *key, cdbi_t klen) {
...
static int ewrite(int fd, const char *buf, int len) {
  while (len) {
    int l = write(fd, buf, len);
    if (l < 0 && errno != EINTR) return -1;
    if (l > 0) {
      len -= l;
      buf += l;
    }
...
static int make_write(struct cdb_make *cdbmp, const char *ptr, cdbi_t len) {
  ...
  if (len > l) {
    ...
    if (l) {
      ...
      if (ewrite(cdbmp->cdb_fd, ptr, l) < 0) return -1;
      ptr += l;
      len -= l;
    }
    cdbmp->cdb_bpos = cdbmp->cdb_buf;
...
static int cdb_make_finish_internal(struct cdb_make *cdbmp) {
  ...
  for (t = 0; t < 256; ++t) {
    ...
    for (i = 0; i < len; ++i) htab[i].hval = htab[i].rpos = 0;
  ...
  free(p);
  if (cdbmp->cdb_bpos != cdbmp->cdb_buf &&
      ewrite(cdbmp->cdb_fd, cdbmp->cdb_buf, cdbmp->cdb_bpos - cdbmp->cdb_buf) !=
          0)
    return -1;
