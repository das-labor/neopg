...
/* Return the current cache object or bail out if it is has not yet
   been initialized.  */
static crl_cache_t get_current_cache(void) {
  if (!current_cache) log_fatal("CRL cache has not yet been initialized\n");
 ...
 */
static int create_directory_if_needed(const char *name) {
  DIR *dir;
  char *fname;
...
static int cleanup_cache_dir(int force) {
  char *dname = make_filename(opt.homedir_cache, DBDIR_D, NULL);
  DIR *dir;
  struct dirent *de;
  int problem = 0;

  if (!force) { /* Very minor sanity checks. */
...
static char *next_line_from_file(estream_t fp, gpg_error_t *r_err) {
  char buf[300];
  char *largebuf = NULL;
  size_t buflen;
  size_t len = 0;
...
static void release_one_cache_entry(crl_cache_entry_t entry) {
  if (entry) {
    if (entry->cdb) {
...
static void release_cache(crl_cache_t cache) {
...
/* Open the dir file FNAME or create a new one if it does not yet
   exist. */
static estream_t open_dir_file(const char *fname) {
  estream_t fp;
  ...
  if (!fp) {
    ...
    if (create_directory_if_needed(NULL) || create_directory_if_needed(DBDIR_D))
      return NULL;
    fp = es_fopen(fname, "w");
    if (!fp) {
...
/* Helper for open_dir. */
static gpg_error_t check_dir_version(estream_t *fpadr, const char *fname,
                                     unsigned int *lineno,
...
static gpg_error_t open_dir(crl_cache_t *r_cache) {
  crl_cache_t cache;
  char *fname;
  char *line = NULL;
  gpg_error_t lineerr = 0;
  ...
  if (!cache) return gpg_error_from_syserror();
  ...
  while ((line = next_line_from_file(fp, &lineerr))) {
    ...
    if (*line == 'c' || *line == 'u' || *line == 'i') {
      ...
      if (*line == 'i') {
        ...
        if (entry->invalid < 1) entry->invalid = 1;
      } else if (*line == 'u')
      ...
      for (fieldno = 1, p = line; p; p = endp, fieldno++) {
        ...
        switch (fieldno) {
          ...
          case 2:
            ...
            break;
          case 3:
            entry->issuer = unpercent_string(p);
            break;
          case 4:
            entry->url = unpercent_string(p);
            break;
          case 5:
          ...
          case 7:
            ...
            break;
          case 8:
            if (*p) entry->crl_number = p;
            break;
          case 9:
            if (*p) entry->authority_issuer = unpercent_string(p);
            break;
          case 10:
            if (*p) entry->authority_serialno = unpercent_string(p);
            break;
          case 11:
            if (*p) entry->check_trust_anchor = xtrystrdup(p);
            break;
          default:
            if (*p)
...
static void write_percented_string(const char *s, estream_t fp) {
  for (; *s; s++)
    if (*s == ':')
      es_fputs("%3A", fp);
    else if (*s == '\n')
...
static void write_dir_line_crl(estream_t fp, crl_cache_entry_t e) {
  if (e->invalid)
  ...
  es_putc(':', fp);
  if (e->crl_number) es_fputs(e->crl_number, fp);
  es_putc(':', fp);
  if (e->authority_issuer) write_percented_string(e->authority_issuer, fp);
  es_putc(':', fp);
  if (e->authority_serialno) es_fputs(e->authority_serialno, fp);
  es_putc(':', fp);
...
static gpg_error_t update_dir(crl_cache_t cache) {
  char *fname = NULL;
  char *tmpfname = NULL;
  char *line = NULL;
  gpg_error_t lineerr = 0;
  ...
  for (e = cache->entries; e; e = e->next) e->mark = 1;
  ...
  /* Create a temporary DIR file. */
  {
    ...
    gpgrt_asprintf(&tmpbuf, "DIR-tmp-%s-%u-%p.txt.tmp", nodename,
                   (unsigned int)getpid(), &tmpbuf);
    if (!tmpbuf) {
      err = gpg_error_from_errno(errno);
      ...
      if (endp) {
        ...
        *endp = ':'; /* Restore original line. */
        if (e && e->deleted) {
          ...
          e->mark = 0;
        } else { /* We ignore entries we don't have in our cache
  ...
  if (!es_ferror(fp) && !es_ferror(fpout) && !lineerr) {
    ...
    for (e = cache->entries; e; e = e->next)
      if (e->mark) {
        if (!e->deleted) write_dir_line_crl(fpout, e);
        e->mark = 0;
      }
  }
  if (lineerr) {
...
leave:
  ...
  if (fpout) {
    es_fclose(fpout);
    if (err && tmpfname) gnupg_remove(tmpfname);
  }
  xfree(tmpfname);
  return err;
}
...
static char *make_db_file_name(const char *issuer_hash) {
  char bname[50];
...
/* Hash the file FNAME and return the MD5 digest in MD5BUFFER. The
   ...
   on success. */
static int hash_dbfile(const char *fname, unsigned char *md5buffer) {
  estream_t fp;
  char *buffer;
  size_t n;
  gcry_md_hd_t md5;
...
static int check_dbfile(const char *fname, const char *md5hexvalue) {
...
static struct cdb *lock_db_file(crl_cache_t cache, crl_cache_entry_t entry) {
  char *fname;
  int fd;
  int open_count;
  crl_cache_entry_t e;
  ...
  if (opt.verbose) log_info(_("opening cache file '%s'\n"), fname);

  if (!entry->dbfile_checked) {
    if (!check_dbfile(fname, entry->dbfile_hash)) entry->dbfile_checked = 1;
...
static void unlock_db_file(crl_cache_t cache, crl_cache_entry_t entry) {
  if (!entry->cdb)
  ...
  if (!entry->cdb_use_count && entry->deleted) {
    ...
    enext = entry->next;
    for (eprev = cache->entries; eprev && eprev->next != entry;
         eprev = eprev->next)
      ;
    assert(eprev);
...
/* Check whether the certificate identified by ISSUER_HASH and
   ...
   and over.  */
static crl_cache_result_t cache_isvalid(ctrl_t ctrl, const char *issuer_hash,
  ...
  if (!cdb) return CRL_CACHE_DONTKNOW; /* Hmmm, not the best error code. */
...
crl_cache_result_t crl_cache_isvalid(ctrl_t ctrl, const char *issuer_hash,
  ...
  std::vector<uint8_t> sn =
      Botan::hex_decode(serialno, strlen(serialno), false);
  result =
      cache_isvalid(ctrl, issuer_hash, sn.data(), sn.size(), force_refresh);
  return result;
}
...
gpg_error_t crl_cache_cert_isvalid(ctrl_t ctrl, ksba_cert_t cert,
  ...
  /* Check the cache.  */
  result =
      cache_isvalid(ctrl, issuerhash_hex.c_str(), sn, snlen, force_refresh);
  switch (result) {
...
static gpg_error_t start_sig_check(ksba_crl_t crl, gcry_md_hd_t *md,
                                   int *algo) {
  gpg_error_t err;
  const char *algoid;
  ...
  *algo = gcry_md_map_name(algoid);
  ...
  if (DBG_HASHING) gcry_md_debug(*md, "hash.cert");
...
static gpg_error_t finish_sig_check(ksba_crl_t crl, gcry_md_hd_t md, int algo,
  ...
  if (DBG_X509) dump_cert("crl_issuer_cert", issuer_cert);
  ...
  err = gcry_sexp_build(&s_hash, NULL, "(data(flags pkcs1)(hash %s %b))",
                        algoname, gcry_md_get_algo_dlen(algo),
  ...
  if (DBG_X509) log_debug("gcry_pk_verify: %s\n", gpg_strerror(err));
...
/* Call this to match a start_sig_check that can not be completed
   normally.  */
static void abort_sig_check(ksba_crl_t crl, gcry_md_hd_t md) {
  (void)crl;
  gcry_md_close(md);
}
...
*/
static int crl_parse_insert(ctrl_t ctrl, ksba_crl_t crl, struct cdb_make *cdb,
                            const char *fname, char **r_crlissuer,
                            ksba_isotime_t thisupdate,
                            ksba_isotime_t nextupdate, char **r_trust_anchor) {
  gpg_error_t err;
      ...
      case KSBA_SR_READY: {
        ...
        *r_crlissuer = crlissuer; /* (Do it here so we don't need
        ...
        if (!ksba_crl_get_auth_key_id(crl, &keyid, &authid, &authidsn)) {
          ...
          if (opt.verbose)
            log_info(
                _("locating CRL issuer certificate by "
          ...
          if (!crlissuer_cert && keyid)
            crlissuer_cert = find_cert_bysubject(ctrl, crlissuer, keyid);
...
/* Return the crlNumber extension as an allocated hex string or NULL
   if there is none. */
static char *get_crl_number(ksba_crl_t crl) {
  gpg_error_t err;
  ksba_sexp_t number;
  char *string;
...
/* Return the authorityKeyIdentifier or NULL if it is not available.
   ...
   0x01. */
static char *get_auth_key_id(ksba_crl_t crl, char **serialno) {
  gpg_error_t err;
  ksba_name_t name;
  ksba_sexp_t sn;
  int idx;
...
gpg_error_t crl_cache_insert(ctrl_t ctrl, const char *url,
  ...
  /* Create a temporary cache file to load the CRL into. */
  {
    ...
    gpgrt_asprintf(&tmpfname, "crl-tmp-%s-%u-%p.db.tmp", nodename,
                   (unsigned int)getpid(), &tmpfname);
    if (!tmpfname) {
      err = gpg_error_from_syserror();
      goto leave;
    }
  ...
  err = crl_parse_insert(ctrl, crl, &cdb, fname, &issuer, thisupdate,
                         nextupdate, &trust_anchor);
  if (err) {
  ...
  if (strcmp(nextupdate, current_time) < 0) {
    if (opt.force)
      ...
      if (!err2) err2 = GPG_ERR_CRL_TOO_OLD;
      invalidate_crl |= 1;
    }
  }
  ...
  for (idx = 0;
       !(err = ksba_crl_get_extension(crl, idx, &oid, &critical, NULL, NULL));
       idx++) {
    if (!critical || !strcmp(oid, oidstr_authorityKeyIdentifier) ||
        !strcmp(oid, oidstr_crlNumber))
      continue;
    log_error(_("unknown critical CRL extension %s\n"), oid);
    if (!err2) err2 = GPG_ERR_INV_CRL;
    invalidate_crl |= 2;
  }
  if (err == GPG_ERR_EOF || err == GPG_ERR_NO_DATA) err = 0;
  if (err) {
  ...
  entry->release_ptr =
      (char *)xtrymalloc(strlen(issuer_hash) + 1 + strlen(issuer) + 1 +
                         strlen(url) + 1 + strlen(checksum) + 1);
  ...
  if (opt.verbose) log_info(_("creating cache file '%s'\n"), newfname);
  ...
  xfree(fname);
  fname = NULL; /*(let the cleanup code not try to remove it)*/
  ...
  if (err) {
    log_error(
        _("updating the DIR file failed - "
...
static gpg_error_t list_one_crl_entry(crl_cache_t cache, crl_cache_entry_t e,
  ...
  while (!rc && (rc = cdb_findnext(&cdbfp)) > 0) {
    ...
    n = cdb_keylen(cdb);
    if (n > sizeof keyrecord) n = sizeof keyrecord;
...
gpg_error_t crl_cache_reload_crl(ctrl_t ctrl, ksba_cert_t cert) {
  ...
  seq = 0;
  while (!(err = ksba_cert_get_crl_dist_point(cert, seq++, &distpoint,
                                              &issuername, NULL))) {
    int name_seq;
    ...
    for (name_seq = 0; ksba_name_enum(distpoint, name_seq); name_seq++) {
      ...
      if (!distpoint_uri) continue;

      if (!strncmp(distpoint_uri, "http:", 5) ||
          !strncmp(distpoint_uri, "https:", 6)) {
        if (opt.ignore_http_dp) continue;
      } else
        continue; /* Skip unknown schemes. */

      any_dist_point = 1;

      if (opt.verbose) log_info("fetching CRL from '%s'\n", distpoint_uri);
