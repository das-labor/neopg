...
/* Return false if both serial numbers match.  Can't be used for
   sorting. */
static int compare_serialno(ksba_sexp_t serial1, ksba_sexp_t serial2) {
...
unsigned char *cert_compute_fpr(ksba_cert_t cert, unsigned char *digest) {
  ...
  if (err) log_fatal("gcry_md_open failed: %s\n", gpg_strerror(err));
...
static void clean_cache_slot(cert_item_t ci) {
  ksba_cert_t cert;

  if (!ci->cert) return; /* Already cleaned.  */
 ...
 * GPG_ERR_DUP_VALUE.  */
static gpg_error_t put_cert(ksba_cert_t cert, int permanent,
                            unsigned int trustclass, void *fpr_buffer) {
  ...
  if (!permanent && total_nonperm_certificates >= MAX_NONPERM_CACHED_CERTS) {
    ...
    for (i = idx; drop_count; i = ((i + 1) % 256)) {
      ...
      for (ci = cert_cache[i]; ci; ci = ci->next)
        if (ci->cert && !ci->permanent) ci_mark = ci;
      if (ci_mark) {
  ...
  for (ci = cert_cache[*fpr]; ci; ci = ci->next)
    if (ci->cert && !memcmp(ci->fpr, fpr, 20)) return GPG_ERR_DUP_VALUE;
  /* Try to reuse an existing entry.  */
  for (ci = cert_cache[*fpr]; ci; ci = ci->next)
    if (!ci->cert) break;
  if (!ci) { /* No: Create a new entry.  */
    ci = (cert_item_t)xtrycalloc(1, sizeof *ci);
    if (!ci) return gpg_error_from_errno(errno);
  ...
  if (!permanent) total_nonperm_certificates++;
...
static gpg_error_t load_certs_from_dir(const char *dirname,
                                       unsigned int trustclass) {
  gpg_error_t err;
  DIR *dir;
  struct dirent *ep;
  char *p;
  ...
  while ((ep = readdir(dir))) {
    ...
    if (!err) err = ksba_cert_read_der(cert, reader);
 ...
 * information message.  */
static gpg_error_t load_certs_from_file(const char *fname,
                                        unsigned int trustclasses,
                                        int no_error) {
  gpg_error_t err;
  estream_t fp = NULL;
  ...
  err = gnupg_ksba_create_reader(
      &ioctx, (GNUPG_KSBA_IO_AUTODETECT | GNUPG_KSBA_IO_MULTIPEM), fp, &reader);
  if (err) {
 ...
 * this function.  */
static void load_certs_from_w32_store(const char *storename) {
  ...
  if (!init_done) {
    ...
    pCertOpenSystemStore =
        (CERTOPENSYSTEMSTORE)GetProcAddress(hCrypt32, "CertOpenSystemStoreA");
    pCertEnumCertificatesInStore = (CERTENUMCERTIFICATESINSTORE)GetProcAddress(
        hCrypt32, "CertEnumCertificatesInStore");
    pCertCloseStore =
        (CERTCLOSESTORE)GetProcAddress(hCrypt32, "CertCloseStore");
    if (!pCertOpenSystemStore || !pCertEnumCertificatesInStore ||
        !pCertCloseStore) {
      log_error("can't load crypt32.dll: %s\n", "missing function");
  ...
  if (!pCertOpenSystemStore) return; /* Not initialized.  */
  ...
  while ((w32cert = pCertEnumCertificatesInStore(w32store, w32cert))) {
    if (w32cert->dwCertEncodingType == X509_ASN_ENCODING) {
      ...
      err = ksba_cert_new(&cert);
      if (!err)
        err = ksba_cert_init_from_mem(cert, w32cert->pbCertEncoded,
...
static gpg_error_t load_certs_from_system(void) {
#ifdef HAVE_W32_SYSTEM
...
void cert_cache_init(const std::vector<std::string> &hkp_cacerts) {
  char *fname;
...
void cert_cache_print_stats(void) {
      ...
      if (ci->cert) {
        ...
        if (ci->trustclasses) {
          ...
          if ((ci->trustclasses & CERTTRUST_CLASS_CONFIG))
            n_trustclass_config++;
          if ((ci->trustclasses & CERTTRUST_CLASS_HKP)) n_trustclass_hkp++;
  ...
  log_info(_("           trusted certificates: %u (%u,%u,%u,%u)\n"), n_trusted,
           n_trustclass_system, n_trustclass_config, n_trustclass_hkp,
           n_trustclass_hkpspool);
}
...
ksba_cert_t get_cert_byhexfpr(const char *string) {
  ...
  if (strchr(string, ':')) {
    for (s = string, i = 0; i < 20 && hexdigitp(s) && hexdigitp(s + 1);) {
      if (s[2] && s[2] != ':') break; /* Invalid string. */
      fpr[i++] = xtoi_2(s);
      s += 2;
      if (i != 20 && *s == ':') s++;
    }
...
static enum pattern_class classify_pattern(const char *pattern,
                                           size_t *r_offset,
                                           size_t *r_sn_offset) {
  ...
  *r_offset = *r_sn_offset = 0;
  ...
  switch (*s) {
    ...
    case '#': /* Serial number or issuer DN. */
    {
      ...
      } else { /* Serialnumber + optional issuer ID. */
        for (si = s; *si && *si != '/'; si++)
          if (!strchr("01234567890abcdefABCDEF", *si)) break;
        if (*si && *si != '/')
    ...
    default:
      ...
      if (hexlength == 8 || (!hexprefix && hexlength == 9 && *s == '0')) {
        ...
        if (hexlength == 59 && (!s[hexlength] || spacep(s + hexlength))) {
          ...
          for (i = 0; i < 20; i++, s += 3) {
            c = hextobyte(s);
            if (c == -1 || (i < 19 && s[2] != ':')) break;
          }
          if (i == 20) result = PATTERN_FINGERPRINT20;
        }
  ...
  if (result != PATTERN_UNKNOWN) *r_offset = s - pattern;
  return result;
}
...
gpg_error_t get_certs_bypattern(const char *pattern,
  ...
  if (!pattern || !retfnc) return GPG_ERR_INV_ARG;
  ...
  switch (klasse) {
    ...
    case PATTERN_ISSUER:
      for (seq = 0, err = 0; !err && (cert = get_cert_byissuer(pattern, seq));
           seq++) {
        err = retfnc(retfnc_data, cert);
        ksba_cert_release(cert);
        cert = NULL;
      }
      if (!err && !seq) err = GPG_ERR_NOT_FOUND;
      break;

    case PATTERN_SUBJECT:
      for (seq = 0, err = 0; !err && (cert = get_cert_bysubject(pattern, seq));
           seq++) {
        err = retfnc(retfnc_data, cert);
        ksba_cert_release(cert);
        cert = NULL;
  ...
  if (!err && cert) err = retfnc(retfnc_data, cert);
  ksba_cert_release(cert);
  xfree(serialno);
  return err;
}
...
ksba_cert_t find_cert_bysn(ctrl_t ctrl, const char *issuer_dn,
  ...
  if (DBG_LOOKUP)
    log_debug(
        "find_cert_bysn: certificate not returned by caller"
        " - doing lookup\n");
 ...
 * resources.  */
ksba_cert_t find_cert_bysubject(ctrl_t ctrl, const char *subject_dn,
                                ksba_sexp_t keyid) {
  gpg_error_t err;
  int seq;
  ksba_cert_t cert = NULL;
  ...
  for (seq = 0; (cert = get_cert_bysubject(subject_dn, seq)); seq++) {
    if (!keyid) break; /* No keyid requested, so return the first one found. */
  ...
  if (cert) return cert; /* Done.  */

  if (DBG_LOOKUP) log_debug("find_cert_bysubject: certificate not in cache\n");
  ...
  if (DBG_LOOKUP)
    log_debug(
        "find_cert_bysubject: certificate not returned by caller"
        " - doing lookup\n");
...
gpg_error_t is_trusted_cert(ksba_cert_t cert, unsigned int trustclasses) {
  unsigned char fpr[20];
  cert_item_t ci;
 ...
 * GPG_ERR_NOT_FOUND.  */
gpg_error_t find_issuing_cert(ctrl_t ctrl, ksba_cert_t cert,
                              ksba_cert_t *r_cert) {
  gpg_error_t err;
  char *issuer_dn;
  ...
  *r_cert = NULL;
  ...
  if (err || !issuer_cert) {
    ...
    if (issuer_cert) err = 0;
  }

leave:
  if (!err && !issuer_cert) err = GPG_ERR_NOT_FOUND;
...
gpg_error_t read_certlist_from_stream(certlist_t *r_certlist, estream_t fp) {
  ...
  *r_certlist = NULL;
  ...
  err = gnupg_ksba_create_reader(
      &ioctx, (GNUPG_KSBA_IO_PEM | GNUPG_KSBA_IO_MULTIPEM), fp, &reader);
  if (err) goto leave;
