...
/*   rc = gnutls_dh_params_import_pkcs3 (dh_params, &datum,
 * GNUTLS_X509_FMT_PEM); */
/*   if (rc < 0) */
...
static gpg_error_t verify_callback(http_t hd, http_session_t session,
                                   int reserved) {
  (void)hd;
  (void)reserved;
...
int main(int argc, char **argv) {
  ...
  if (!cafile) cafile = prepend_srcdir("tls-ca.pem");

  /* http.c makes use of the assuan socket wrapper.  */
  assuan_sock_init();

  rc = gnutls_global_init();
  if (rc) log_error("gnutls_global_init failed: %s\n", gnutls_strerror(rc));

  http_register_tls_callback(verify_callback);
  http_register_tls_ca(cafile);

  err = http_session_new(
      &session, NULL, ((no_crl ? HTTP_FLAG_NO_CRL : 0) | HTTP_FLAG_TRUST_DEF),
      NULL, NULL);
  if (err) log_error("http_session_new failed: %s\n", gpg_strerror(err));
  ...
  if (tls_dbg) gnutls_global_set_log_level(tls_dbg);
  ...
  if (uri->opaque)
    ...
    printf("Flags :%s%s%s%s\n", uri->is_http ? " http" : "",
           uri->opaque ? " opaque" : "", uri->v6lit ? " v6lit" : "",
           uri->onion ? " onion" : "");
    printf("TLS   : %s\n",
           uri->use_tls
               ? "yes"
  ...
  if (session) http_session_set_timeout(session, timeout);

  rc = http_open_document(&hd, *argv, NULL, my_http_flags, NULL, session, NULL,
                          NULL);
  if (rc) {
