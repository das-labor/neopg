...
char *get_fingerprint_hexstring(ksba_cert_t cert) {
  ...
  if (rc) log_fatal(_("gcry_md_open failed: %s\n"), gpg_strerror(rc));
  ...
  buf = (char *)xmalloc(41);
  *buf = 0;
  for (i = 0; i < 20; i++) sprintf(buf + strlen(buf), "%02X", digest[i]);
  return buf;
}
...
void cert_log_cb(http_session_t sess, gpg_error_t err, const char *hostname,
  ...
  if (!err) return; /* No error - no need to log anything  */

  log_debug("expected hostname: %s\n", hostname);
  for (n = 0; certs[n]; n++) {
    err = ksba_cert_new(&cert);
    if (!err) err = ksba_cert_init_from_mem(cert, certs[n], certlens[n]);
    if (err)
 ...
 */
static int remove_percent_escapes(unsigned char *string) {
  int n = 0;
  unsigned char *p, *s;
...
static int my_estream_ksba_reader_cb(void *cb_value, char *buffer, size_t count,
  ...
  if (!fp) return GPG_ERR_INV_VALUE;
  ...
  *r_nread = es_fread(buffer, 1, count, fp);
  if (!*r_nread) return -1; /* EOF or error.  */
  return 0;                 /* Success.  */
}
...
gpg_error_t create_estream_ksba_reader(ksba_reader_t *r_reader, estream_t fp) {
  ...
  *r_reader = NULL;
  ...
  if (!err) err = ksba_reader_set_cb(reader, my_estream_ksba_reader_cb, fp);
  if (err) {
...
gpg_error_t copy_stream(estream_t in, estream_t out) {
  ...
  while (!es_read(in, buffer, sizeof buffer, &nread)) {
    if (!nread) return 0; /* EOF */
    if (es_write(out, buffer, nread, NULL)) break;
  }
