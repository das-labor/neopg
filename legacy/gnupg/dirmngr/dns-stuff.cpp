...
static gpg_error_t get_dns_cert_standard(const char *name, int want_certtype,
                                         void **r_key, size_t *r_keylen,
                                         unsigned char **r_fpr,
                                         size_t *r_fprlen, char **r_url) {
  ...
  if (!answer) return gpg_error_from_syserror();

  err = GPG_ERR_NOT_FOUND;
  r = res_query(name, C_IN, (want_certtype < DNS_CERTTYPE_RRBASE
                                 ? T_CERT
  ...
  if (r >= sizeof(HEADER) && r <= 65536 &&
    ...
    while (count-- > 0 && pt < emsg) {
      ...
      if (want_certtype >= DNS_CERTTYPE_RRBASE &&
          type == (want_certtype - DNS_CERTTYPE_RRBASE) && r_key) {
        *r_key = xtrymalloc(dlen);
        ...
        if (want_certtype && want_certtype != ctype)
          ...
          *r_key = xtrymalloc(dlen);
          ...
          goto leave;
        } else if (ctype == DNS_CERTTYPE_IPGP && dlen && dlen < 1023 &&
                   dlen >= pt[0] + 1) {
          /* IPGP type */
          *r_fprlen = pt[0];
          if (*r_fprlen) {
...
gpg_error_t get_dns_cert(const char *name, int want_certtype, void **r_key,
                         size_t *r_keylen, unsigned char **r_fpr,
                         size_t *r_fprlen, char **r_url) {
  gpg_error_t err;

  if (r_key) *r_key = NULL;
  if (r_keylen) *r_keylen = 0;
  *r_fpr = NULL;
  *r_fprlen = 0;
  *r_url = NULL;

  err = get_dns_cert_standard(name, want_certtype, r_key, r_keylen, r_fpr,
                              r_fprlen, r_url);
