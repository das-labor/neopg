...
static void check_header_constants(void) {
...
#undef X
#define X                                                    \
  (VALIDATE_FLAG_TRUST_SYSTEM | VALIDATE_FLAG_TRUST_CONFIG | \
...
/* Check whether CERT contains critical extensions we don't know
   about.  */
static gpg_error_t unknown_criticals(ksba_cert_t cert) {
  static const char *known[] = {"2.5.29.15", /* keyUsage */
  ...
  for (idx = 0;
    ...
    /* If this critical extension is not supported, check the list
       ...
       is supported.  */
    if (unsupported && opt.ignored_cert_extensions.count(oid) > 0)
      unsupported = 0;

    if (unsupported) {
      log_error(_("critical certificate extension %s is not supported"), oid);
...
static gpg_error_t check_cert_policy(ksba_cert_t cert) {
  static const char *allowed[] = {"2.289.9.9", NULL};
  gpg_error_t err;
  int idx;
  char *p, *haystack;
  char *policies;
  ...
  if (opt.verbose > 1) log_info("certificate's policy list: %s\n", policies);
...
static gpg_error_t allowed_ca(ksba_cert_t cert, int *chainlen) {
  gpg_error_t err;
  int flag;
  ...
  if (!flag) {
    if (!is_trusted_cert(cert, CERTTRUST_CLASS_CONFIG)) {
      ...
      if (chainlen) *chainlen = 3; /* That is what the SigG implements. */
      if (opt.verbose) log_info(_("accepting root CA not marked as a CA"));
    } else {
...
static gpg_error_t check_revocations(ctrl_t ctrl, chain_item_t chain) {
  gpg_error_t err = 0;
  int any_revoked = 0;
  int any_no_crl = 0;
  ...
  for (ci = chain; ci; ci = ci->next) {
    ...
    if (ci == chain) {
      ...
      if (opt.verbose) cert_log_name(_("not checking CRL for"), ci->cert);
      continue;
    }

    if (opt.verbose) cert_log_name(_("checking CRL for"), ci->cert);
    err = crl_cache_cert_isvalid(ctrl, ci->cert, 0);
    if (err == GPG_ERR_NO_CRL_KNOWN) {
      err = crl_cache_reload_crl(ctrl, ci->cert);
      if (!err) err = crl_cache_cert_isvalid(ctrl, ci->cert, 0);
    }
    switch (err) {
      case 0:
        err = 0;
...
gpg_error_t validate_cert_chain(ctrl_t ctrl, ksba_cert_t cert,
                                ksba_isotime_t r_exptime, unsigned int flags,
                                char **r_trust_anchor) {
  gpg_error_t err = 0;
  int depth, maxdepth;
  char *issuer = NULL;
  ...
  *exptime = 0;
  ...
  if (r_trust_anchor) *r_trust_anchor = NULL;

  if (DBG_X509) dump_cert("subject", cert);
  ...
  if (!r_exptime) {
    ...
    err = ksba_cert_get_user_data(cert, "validated_at", &validated_at,
                                  sizeof(validated_at), &buflen);
    if (err || buflen != sizeof(validated_at) || !validated_at)
      ...
      if (validated_at < gnupg_get_time() + (30 * 60)) {
        if (opt.verbose) log_info("certificate is good (cached)\n");
  ...
  for (;;) {
    ...
    if (is_root_cert(subject_cert, issuer, subject)) {
      ...
      if (check_cert_sig(subject_cert, subject_cert)) {
        ...
        err = depth ? GPG_ERR_BAD_CERT_CHAIN : GPG_ERR_BAD_CERT;
        goto leave;
      }

      /* Is this certificate allowed to act as a CA.  */
      err = allowed_ca(subject_cert, NULL);
      if (err) goto leave; /* No. */

      err = is_trusted_cert(subject_cert, (flags & VALIDATE_FLAG_MASK_TRUST));
      if (!err)
        ; /* Yes we trust this cert.  */
      else if (err == GPG_ERR_NOT_TRUSTED) {
        char *fpr;
    ...
    if (err) {
      ...
      err = GPG_ERR_MISSING_ISSUER_CERT;
      goto leave;
    }
  ...
  if (!err && !(flags & VALIDATE_FLAG_NOCRLCHECK) &&
      !((flags & VALIDATE_FLAG_CRL) &&
        !(flags &
          VALIDATE_FLAG_RECURSIVE))) { /* Now that everything is fine, walk the
                                        * chain and check each
                                        * certificate for revocations.
                                        *
                                        * 1. item in the chain  - The root
                                        * certificate.
                                        * 2. item               - the CA below
                                        * the root
                                        * last item             - the target
                                        * certificate.
                                        *
                                        * Now for each certificate in the chain
                                        * check whether it has
                                        * been included in a CRL and thus be
                                        * revoked.  We don't do OCSP
                                        * here because this does not seem to
                                        * make much sense.  This
                                        * might become a recursive process and
                                        * we should better cache
                                        * our validity results to avoid double
                                        * work.  Far worse a
                                        * catch-22 may happen for an improper
                                        * setup hierarchy and we
                                        * need a way to break up such a
                                        * deadlock.  */
...
leave:
  if (!err && !(r_trust_anchor && *r_trust_anchor)) {
    ...
    for (citem = chain; citem; citem = citem->next) {
      err = ksba_cert_set_user_data(citem->cert, "validated_at", &validated_at,
                                    sizeof(validated_at));
      if (err) {
  ...
  ksba_free(subject);
  ksba_cert_release(issuer_cert);
  if (subject_cert != cert) ksba_cert_release(subject_cert);
  while (chain) {
    chain_item_t ci_next = chain->next;
    if (chain->cert) ksba_cert_release(chain->cert);
    xfree(chain);
    chain = ci_next;
  }
  if (err && r_trust_anchor && *r_trust_anchor) {
...
static gpg_error_t check_cert_sig(ksba_cert_t issuer_cert, ksba_cert_t cert) {
  ...
  if (DBG_CRYPTO) {
    ...
    for (j = 0; j < n; j++) log_printf(" %02X", p[j]);
    log_printf("\n");
  }
...
static gpg_error_t check_cert_usage(ksba_cert_t cert,
                                    enum cert_usage_modes mode) {
  gpg_error_t err;
  unsigned int use;
  char *extkeyusages;
  ...
  if (err == GPG_ERR_NO_DATA) err = 0; /* No policy given. */
  if (!err) {
    ...
    if (extkeyusages) {
      ...
      while (p && (pend = strchr(p, ':'))) {
        *pend++ = 0;
          ...
          if (!strcmp(p, oid_kp_serverAuth))
            extusemask |=
                (KSBA_KEYUSAGE_DIGITAL_SIGNATURE |
                 KSBA_KEYUSAGE_KEY_ENCIPHERMENT | KSBA_KEYUSAGE_KEY_AGREEMENT);
          else if (!strcmp(p, oid_kp_clientAuth))
            extusemask |=
                (KSBA_KEYUSAGE_DIGITAL_SIGNATURE | KSBA_KEYUSAGE_KEY_AGREEMENT);
        ...
        if (!strcmp(p, oid_kp_ocspSigning)) have_ocsp_signing = 1;
      ...
      if (!any_critical) extusemask = ~0; /* Reset to the don't care mask. */
    }
    ...
    if (err == GPG_ERR_NO_DATA) {
      err = 0;
      if (opt.verbose &&
          (mode == CERT_USAGE_MODE_SIGN || mode == CERT_USAGE_MODE_ENCR))
  ...
  switch (mode) {
    ...
    case CERT_USAGE_MODE_CERT:
      if ((use & (KSBA_KEYUSAGE_KEY_CERT_SIGN))) return 0;
      log_info(
          _("certificate should not have "
    ...
    case CERT_USAGE_MODE_OCSP:
      if (use != ~0 &&
          (have_ocsp_signing ||
           (use & (KSBA_KEYUSAGE_KEY_CERT_SIGN | KSBA_KEYUSAGE_CRL_SIGN))))
        return 0;
      log_info(
          _("certificate should not have "
    ...
    case CERT_USAGE_MODE_CRL:
      if ((use & (KSBA_KEYUSAGE_CRL_SIGN))) return 0;
      log_info(
          _("certificate should not have "
