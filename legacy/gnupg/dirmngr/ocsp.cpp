...
static gpg_error_t read_response(estream_t fp, unsigned char **r_buffer,
                                 size_t *r_buflen) {
  gpg_error_t err;
  ...
  *r_buflen = 0;
  ...
  for (;;) {
    ...
    if (!(nread == bufsize - nbytes &&
          !es_feof(fp))) { /* Response successfully received. */
      nbytes += nread;
      *r_buffer = buffer;
      *r_buflen = nbytes;
      return 0;
...
static gpg_error_t do_ocsp_request(ctrl_t ctrl, ksba_ocsp_t ocsp,
...

  {
    ...
    if (n > sizeof nonce) n = sizeof nonce;
  ...
  switch (response_status) {
    case KSBA_OCSP_RSPSTATUS_SUCCESS:
      t = "success";
      break;
    case KSBA_OCSP_RSPSTATUS_MALFORMED:
      t = "malformed";
      break;
    case KSBA_OCSP_RSPSTATUS_INTERNAL:
      t = "internal error";
      break;
    case KSBA_OCSP_RSPSTATUS_TRYLATER:
      t = "try later";
      break;
    case KSBA_OCSP_RSPSTATUS_SIGREQUIRED:
      t = "must sign request";
      break;
    case KSBA_OCSP_RSPSTATUS_UNAUTHORIZED:
      t = "unauthorized";
      break;
    case KSBA_OCSP_RSPSTATUS_REPLAYED:
      t = "replay detected";
      break;
    case KSBA_OCSP_RSPSTATUS_OTHER:
      t = "other (unknown)";
      break;
    case KSBA_OCSP_RSPSTATUS_NONE:
      t = "no status";
      break;
    default:
      t = "[unknown status]";
...
static gpg_error_t validate_responder_cert(ctrl_t ctrl, ksba_cert_t cert,
...
static int check_signature_core(ctrl_t ctrl, ksba_cert_t cert,
                                gcry_sexp_t s_sig, gcry_sexp_t s_hash,
                                fingerprint_list_t signer_fpr_list) {
  gpg_error_t err;
  ksba_sexp_t pubkey;
  ...
  xfree(pubkey);
  if (!err) err = gcry_pk_verify(s_sig, s_hash, s_pkey);
  if (!err) err = validate_responder_cert(ctrl, cert, signer_fpr_list);
  if (!err) {
...
/* Check the signature of an OCSP repsonse.  OCSP is the context,
   ...
   the fingerprints. */
static gpg_error_t check_signature(ctrl_t ctrl, ksba_ocsp_t ocsp,
                                   gcry_sexp_t s_sig, gcry_md_hd_t md,
                                   fingerprint_list_t signer_fpr_list) {
  gpg_error_t err;
  int algo, cert_idx;
  gcry_sexp_t s_hash;
  ...
  if (signer_fpr_list && !signer_fpr_list->next) {
    ...
    if (cert) {
      err = check_signature_core(ctrl, cert, s_sig, s_hash, signer_fpr_list);
...
gpg_error_t ocsp_isvalid(ctrl_t ctrl, ksba_cert_t cert, const char *cert_fpr,
  ...
  url = NULL;
  for (idx = 0;
       !url && !opt.ignore_ocsp_service_url && !force_default_responder &&
       !(err = ksba_cert_get_authority_info_access(cert, idx, &oid, &name));
       idx++) {
  ...
  if (!url) {
    ...
    if (opt.verbose) log_info(_("using default OCSP responder '%s'\n"), url);
  } else {
    if (opt.verbose) log_info(_("using OCSP responder '%s'\n"), url);
  }
  ...
  if ((err = canon_sexp_to_gcry(sigval, &s_sig))) goto leave;
  xfree(sigval);
  sigval = NULL;
  ...
  err = ksba_ocsp_get_status(ocsp, cert, &status, this_update, next_update,
  ...
  if (status == KSBA_STATUS_REVOKED) {
    ...
    err = ksba_cert_set_user_data(cert, "validated_at", &validated_at,
                                  sizeof(validated_at));
    if (err) {
  ...
  if (opt.verbose) {
    log_info(_("certificate status is: %s  (this=%s  next=%s)\n"),
             status == KSBA_STATUS_GOOD
                 ? _("good")
                 : status == KSBA_STATUS_REVOKED
                       ? _("revoked")
                       : status == KSBA_STATUS_UNKNOWN
                             ? _("unknown")
                             : status == KSBA_STATUS_NONE ? _("none") : "?",
             this_update, next_update);
    if (status == KSBA_STATUS_REVOKED)
      log_info(
          _("certificate has been revoked at: %s due to: %s\n"),
          revocation_time,
  ...
  if (strcmp(this_update, current_time) > 0) {
    ...
    if (!err) err = GPG_ERR_TIME_CONFLICT;
  }
