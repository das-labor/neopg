...
int _keybox_read_blob(KEYBOXBLOB *r_blob, FILE *fp, int *skipped_deleted) {
  ...
  if (skipped_deleted) *skipped_deleted = 0;
again:
  if (r_blob) *r_blob = NULL;
  off = ftello(fp);
  if (off == (off_t)-1) return gpg_error_from_syserror();

  if ((c1 = getc(fp)) == EOF || (c2 = getc(fp)) == EOF ||
      (c3 = getc(fp)) == EOF || (c4 = getc(fp)) == EOF ||
      (type = getc(fp)) == EOF) {
    if (c1 == EOF && !ferror(fp)) return -1; /* eof */
    if (!ferror(fp)) return GPG_ERR_TOO_SHORT;
    return gpg_error_from_syserror();
  }

  imagelen = ((unsigned int)c1 << 24) | (c2 << 16) | (c3 << 8) | c4;
  if (imagelen < 5) return GPG_ERR_TOO_SHORT;

  if (!type) {
    /* Special treatment for empty blobs. */
    if (fseek(fp, imagelen - 5, SEEK_CUR)) return gpg_error_from_syserror();
    if (skipped_deleted) *skipped_deleted = 1;
    goto again;
  }
  ...
  if (!image) return gpg_error_from_syserror();
...
int _keybox_write_blob(KEYBOXBLOB blob, FILE *fp) {
  ...
  if (length > IMAGELEN_LIMIT) return GPG_ERR_TOO_LARGE;

  if (fwrite(image, length, 1, fp) != 1) return gpg_error_from_syserror();
  return 0;
}

/* Write a fresh header type blob. */
int _keybox_write_header_blob(FILE *fp, int for_openpgp) {
  ...
  image[5] = 1;                     /* Version */
  if (for_openpgp) image[7] = 0x02; /* OpenPGP data may be available.  */
  ...
  if (fwrite(image, 32, 1, fp) != 1) return gpg_error_from_syserror();
  return 0;
}
