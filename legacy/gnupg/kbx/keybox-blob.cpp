...
static void put8(struct membuf *mb, byte a) { put_membuf(mb, &a, 1); }

static void put16(struct membuf *mb, u16 a) {
  unsigned char tmp[2];
  tmp[0] = a >> 8;
  tmp[1] = a;
  put_membuf(mb, tmp, 2);
...
static void put32(struct membuf *mb, u32 a) {
  unsigned char tmp[4];
  tmp[0] = a >> 24;
  tmp[1] = a >> 16;
  tmp[2] = a >> 8;
...
static void add_fixup(KEYBOXBLOB blob, u32 off, u32 val) {
  struct fixup_list *fl;

  if (blob->fixup_out_of_core) return;
...
static int pgp_temp_store_kid(KEYBOXBLOB blob,
  ...
  blob->temp_kids = k;
  for (r = k; r; r = r->next) k->seqno++;

  return k->seqno;
}

/* Helper for pgp_create_key_part.  */
static gpg_error_t pgp_create_key_part_single(
    KEYBOXBLOB blob, int n, struct _keybox_openpgp_key_info *kinfo) {
  size_t fprlen;
  int off;

  fprlen = kinfo->fprlen;
  if (fprlen > 20) fprlen = 20;
  ...
  if (fprlen != 20) /* v3 fpr - shift right and fill with zeroes. */
  {
    ...
    if (off == -1) return gpg_error_from_syserror();
...
static gpg_error_t pgp_create_key_part(KEYBOXBLOB blob,
  ...
  if (info->nsubkeys)
    for (kinfo = &info->subkeys; kinfo; kinfo = kinfo->next)
      if ((err = pgp_create_key_part_single(blob, n++, kinfo))) return err;

  assert(n == blob->nkeys);
  return 0;
}

static void pgp_create_uid_part(KEYBOXBLOB blob, keybox_openpgp_info_t info) {
  int n = 0;
...
static void pgp_create_sig_part(KEYBOXBLOB blob, u32 *sigstatus) {
  int n;
...
static int pgp_create_blob_keyblock(KEYBOXBLOB blob, const unsigned char *image,
                                    size_t imagelen) {
...
static int x509_create_blob_cert(KEYBOXBLOB blob, ksba_cert_t cert) {
  struct membuf *a = blob->buf;
  ...
  if (!image) return GPG_ERR_GENERAL;
...
static void write_stored_kid(KEYBOXBLOB blob, int seqno) {
  struct keyid_list *r;
...
static void release_kid_list(struct keyid_list *kl) {
...
static int create_blob_header(KEYBOXBLOB blob, int blobtype, int as_ephemeral) {
  struct membuf *a = blob->buf;
  int i;
  ...
  if (blob->serial) put_membuf(a, blob->serial, blob->seriallen);
  ...
  if (blobtype == KEYBOX_BLOBTYPE_X509) {
    ...
    for (i = 0; i < blob->nuids; i++) {
      if (blob->uids[i].name) { /* this is a v3 one */
...
static int create_blob_trailer(KEYBOXBLOB blob) {
  (void)blob;
  return 0;
}

static int create_blob_finish(KEYBOXBLOB blob) {
  struct membuf *a = blob->buf;
  unsigned char *p;
  unsigned char *pp;
  size_t n;
...
gpg_error_t _keybox_create_openpgp_blob(KEYBOXBLOB *r_blob,
                                        keybox_openpgp_info_t info,
                                        const unsigned char *image,
                                        size_t imagelen, int as_ephemeral) {
  gpg_error_t err;
  KEYBOXBLOB blob;
  ...
  *r_blob = NULL;
  ...
  if (!blob) return gpg_error_from_syserror();
...
static char *x509_email_kludge(const char *name) {
  const char *p, *string;
  unsigned char *buf;
  int n;
...
leave:
  ...
  blob->temp_kids = NULL;
  if (names) {
    for (i = 0; i < blob->nuids; i++) xfree(names[i]);
    xfree(names);
  }
  if (rc) {
    _keybox_release_blob(blob);
...
int _keybox_new_blob(KEYBOXBLOB *r_blob, unsigned char *image, size_t imagelen,
                     off_t off) {
  KEYBOXBLOB blob;
...
void _keybox_release_blob(KEYBOXBLOB blob) {
  ...
  xfree(blob->keys);
  xfree(blob->serialbuf);
  for (i = 0; i < blob->nuids; i++) xfree(blob->uids[i].name);
  xfree(blob->uids);
  xfree(blob->sigs);
  xfree(blob->blob);
  xfree(blob);
...
off_t _keybox_get_blob_fileoffset(KEYBOXBLOB blob) { return blob->fileoffset; }
