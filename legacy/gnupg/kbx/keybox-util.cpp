...
void *_keybox_malloc(size_t n) { return alloc_func(n); }

void *_keybox_realloc(void *a, size_t n) { return realloc_func(a, n); }
...
gpg_error_t keybox_tmp_names(const char *filename, int for_keyring,
  ...
  *r_tmpname = NULL;
   ...
   * on a modern file system and append the suffix.  */
  {
    ...
    if (strlen(ext) != 4 || strlen(b_ext) != 4) BUG();
    repl =
        (strlen(filename) > 4 && !strcmp(filename + strlen(filename) - 4, ext));
    bak_name = xtrymalloc(strlen(filename) + (repl ? 0 : 4) + 1);
    if (!bak_name) return gpg_error_from_syserror();
