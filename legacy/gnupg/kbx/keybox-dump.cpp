...
static int print_checksum(const byte *buffer, size_t length, size_t unhashed,
                          FILE *fp) {
  const byte *p;
  int i;
  int hashlen;
  ...
  for (i = 0; i < hashlen; p++, i++) fprintf(fp, "%02x", *p);
...
static int dump_header_blob(const byte *buffer, size_t length, FILE *fp) {
  unsigned long n;
...
int _keybox_dump_blob(KEYBOXBLOB blob, FILE *fp) {
  ...
  if (rawdata_off > length || rawdata_len > length ||
      rawdata_off + rawdata_len > length || rawdata_len + 4 > length ||
  ...
  if (!nkeys) fprintf(fp, "[Error: no keys]\n");
  ...
  for (n = 0; n < nkeys; n++, p += keyinfolen) {
    ...
    for (i = 0; i < 20; i++) fprintf(fp, "%02X", p[i]);
    kidoff = get32(p + 20);
    fprintf(fp, "\nKey-Kid-Off[%lu]: %lu\n", n, kidoff);
    fprintf(fp, "Key-Kid[%lu]: ", n);
    /* fixme: check bounds */
    for (i = 0; i < 8; i++) fprintf(fp, "%02X", buffer[kidoff + i]);
    kflags = get16(p + 24);
    fprintf(fp, "\nKey-Flags[%lu]: %04lX\n", n, kflags);
  }
  ...
  if (!nserial)
    ...
    for (; nserial; nserial--, p++) fprintf(fp, "%02X", *p);
  }
  putc('\n', fp);
...
/* Compute the SHA-1 checksum of the rawdata in BLOB and put it into
   DIGEST. */
static int hash_blob_rawdata(KEYBOXBLOB blob, unsigned char *digest) {
...
static int update_stats(KEYBOXBLOB blob, struct file_stats_s *s) {
  const unsigned char *buffer;
  size_t length;
  int type;
  unsigned long n;
  ...
  n = get16(buffer + 6);
  if (n) {
    if ((n & 1)) s->secret_flagged++;
    if ((n & 2)) s->ephemeral_flagged++;
  } else
    s->non_flagged++;
...
static FILE *open_file(const char **filename, FILE *outfp) {
  FILE *fp;
...
int _keybox_dump_file(const char *filename, int stats_only, FILE *outfp) {
  ...
  if (!(fp = open_file(&filename, outfp))) return gpg_error_from_syserror();
  ...
  if (stats_only) {
    fprintf(outfp,
            ...
            stats.total_blob_count, stats.header_blob_count,
            stats.empty_blob_count, stats.pgp_blob_count, stats.x509_blob_count,
            stats.non_flagged, stats.secret_flagged, stats.ephemeral_flagged);
...
static int cmp_dupitems(const void *arg_a, const void *arg_b) {
...
int _keybox_dump_cut_records(const char *filename, unsigned long from,
  ...
  if (!(fp = open_file(&filename, stderr))) return gpg_error_from_syserror();

  while (!(rc = _keybox_read_blob(&blob, fp, NULL))) {
    if (recno > to) break; /* Ready.  */
    if (recno >= from) {
