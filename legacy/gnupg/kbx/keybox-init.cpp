...
gpg_error_t keybox_register_file(const char *fname, int secret,
  ...
  *r_token = NULL;
  ...
  if (!kr) return gpg_error_from_syserror();
...
static KEYBOX_HANDLE do_keybox_new(KB_NAME resource, int secret,
                                   int for_openpgp) {
  KEYBOX_HANDLE hd;
  int idx;
  ...
  if (hd) {
    ...
    if (!resource->handle_table) {
      ...
      resource->handle_table = (keybox_handle **)xtrycalloc(
          resource->handle_table_size, sizeof *resource->handle_table);
      if (!resource->handle_table) {
...
void keybox_release(KEYBOX_HANDLE hd) {
  ...
  if (hd->kb->handle_table) {
    ...
    for (idx = 0; idx < hd->kb->handle_table_size; idx++)
      if (hd->kb->handle_table[idx] == hd) hd->kb->handle_table[idx] = NULL;
  }
...
void _keybox_close_file(KEYBOX_HANDLE hd) {
  ...
  if (!hd || !hd->kb || !hd->kb->handle_table) return;
