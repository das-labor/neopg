...
static inline unsigned int blob_get_blob_flags(KEYBOXBLOB blob) {
  const unsigned char *buffer;
  size_t length;

  buffer = _keybox_get_blob_image(blob, &length);
  if (length < 8) return 0; /* oops */
...
/* Return the first keyid from the blob.  Returns true if
   available.  */
static int blob_get_first_keyid(KEYBOXBLOB blob, u32 *kid) {
  const unsigned char *buffer;
  size_t length, nkeys, keyinfolen;

  buffer = _keybox_get_blob_image(blob, &length);
  if (length < 48) return 0; /* blob too short */

  nkeys = get16(buffer + 16);
  keyinfolen = get16(buffer + 18);
  if (!nkeys || keyinfolen < 28) return 0; /* invalid blob */
...
gpg_error_t _keybox_get_flag_location(const unsigned char *buffer,
                                      size_t length, int what, size_t *flag_off,
                                      size_t *flag_size) {
  size_t pos;
  ...
  switch (what) {
    case KEYBOX_FLAG_BLOB:
      if (length < 8) return GPG_ERR_INV_OBJ;
      *flag_off = 6;
      *flag_size = 2;
      break;
    ...
    case KEYBOX_FLAG_SIG_INFO:
      if (length < 20) return GPG_ERR_INV_OBJ;
      /* Key info. */
      nkeys = get16(buffer + 16);
      keyinfolen = get16(buffer + 18);
      if (keyinfolen < 28) return GPG_ERR_INV_OBJ;
      pos = 20 + keyinfolen * nkeys;
      if (pos + 2 > length) return GPG_ERR_INV_OBJ; /* Out of bounds. */
      /* Serial number. */
      nserial = get16(buffer + pos);
      pos += 2 + nserial;
      if (pos + 4 > length) return GPG_ERR_INV_OBJ; /* Out of bounds. */
      /* User IDs. */
      nuids = get16(buffer + pos);
      pos += 2;
      uidinfolen = get16(buffer + pos);
      pos += 2;
      if (uidinfolen < 12) return GPG_ERR_INV_OBJ;
      pos += uidinfolen * nuids;
      if (pos + 4 > length) return GPG_ERR_INV_OBJ; /* Out of bounds. */
      /* Signature info. */
      siginfooff = pos;
      nsigs = get16(buffer + pos);
      pos += 2;
      siginfolen = get16(buffer + pos);
      pos += 2;
      if (siginfolen < 4) return GPG_ERR_INV_OBJ;
...
static gpg_error_t get_flag_from_image(const unsigned char *buffer,
                                       size_t length, int what,
                                       unsigned int *value) {
  gpg_error_t ec;
  size_t pos, size;
  ...
  if (!ec) switch (size) {
      case 1:
        *value = buffer[pos];
        break;
      case 2:
        *value = get16(buffer + pos);
        break;
      case 4:
        *value = get32(buffer + pos);
        break;
      default:
        ec = GPG_ERR_BUG;
        break;
...
static int blob_cmp_sn(KEYBOXBLOB blob, const unsigned char *sn, int snlen) {
  const unsigned char *buffer;
  size_t length;
  size_t pos, off;
  size_t nkeys, keyinfolen;
  ...
  buffer = _keybox_get_blob_image(blob, &length);
  if (length < 40) return 0; /* blob too short */

  /*keys*/
  nkeys = get16(buffer + 16);
  keyinfolen = get16(buffer + 18);
  if (keyinfolen < 28) return 0; /* invalid blob */
  pos = 20 + keyinfolen * nkeys;
  if (pos + 2 > length) return 0; /* out of bounds */

  /*serial*/
  nserial = get16(buffer + pos);
  off = pos + 2;
  if (off + nserial > length) return 0; /* out of bounds */
...
static int blob_cmp_fpr(KEYBOXBLOB blob, const unsigned char *fpr) {
  const unsigned char *buffer;
  size_t length;
  size_t pos, off;
  size_t nkeys, keyinfolen;
  ...
  if (keyinfolen < 28) return 0; /* invalid blob */
  pos = 20;
  if (pos + keyinfolen * nkeys > length) return 0; /* out of bounds */

  for (idx = 0; idx < nkeys; idx++) {
    off = pos + idx * keyinfolen;
    if (!memcmp(buffer + off, fpr, 20)) return idx + 1; /* found */
  }
  return 0; /* not found */
}

static int blob_cmp_fpr_part(KEYBOXBLOB blob, const unsigned char *fpr,
  ...
  for (idx = 0; idx < nkeys; idx++) {
    off = pos + idx * keyinfolen;
    if (!memcmp(buffer + off + fproff, fpr, fprlen)) return idx + 1; /* found */
  }
  return 0; /* not found */
}

static int blob_cmp_name(KEYBOXBLOB blob, int idx, const char *name,
                         size_t namelen, int substr, int x509) {
  const unsigned char *buffer;
  size_t length;
  size_t pos, off, len;
  size_t nkeys, keyinfolen;
  ...
  nserial = get16(buffer + pos);
  pos += 2 + nserial;
  if (pos + 4 > length) return 0; /* out of bounds */
  ...
  if (pos + uidinfolen * nuids > length) return 0; /* out of bounds */

  if (idx <
      0) { /* Compare all names.  Note that for X.509 we start with index 1
    ...
    if (idx > nuids) return 0; /* no user ID with that idx */
    pos += idx * uidinfolen;
    off = get32(buffer + pos);
    len = get32(buffer + pos + 4);
    if (off + len > length) return 0; /* out of bounds */
    if (len < 1) return 0;            /* empty name */
...
static int blob_cmp_mail(KEYBOXBLOB blob, const char *name, size_t namelen,
                         int substr, int x509) {
  const unsigned char *buffer;
  size_t length;
  size_t pos, off, len;
...
static int blob_x509_has_grip(KEYBOXBLOB blob, const unsigned char *grip) {
  int rc;
  const unsigned char *buffer;
  size_t length;
  ...
  buffer = _keybox_get_blob_image(blob, &length);
  if (length < 40) return 0; /* Too short. */
  cert_off = get32(buffer + 8);
  cert_len = get32(buffer + 12);
  if (cert_off + cert_len > length) return 0; /* Too short.  */

  rc = ksba_reader_new(&reader);
  if (rc) return 0; /* Problem with ksba. */
  ...
  gcry_sexp_release(s_pkey);
  if (!rcp) goto failed; /* Can't calculate keygrip. */
...
*/
static inline int has_short_kid(KEYBOXBLOB blob, u32 lkid) {
  unsigned char buf[4];
  buf[0] = lkid >> 24;
  buf[1] = lkid >> 16;
  buf[2] = lkid >> 8;
...
static inline int has_long_kid(KEYBOXBLOB blob, u32 mkid, u32 lkid) {
  unsigned char buf[8];
  buf[0] = mkid >> 24;
  buf[1] = mkid >> 16;
  buf[2] = mkid >> 8;
...
static inline int has_fingerprint(KEYBOXBLOB blob, const unsigned char *fpr) {
  return blob_cmp_fpr(blob, fpr);
}

static inline int has_keygrip(KEYBOXBLOB blob, const unsigned char *grip) {
  if (blob_get_type(blob) == KEYBOX_BLOBTYPE_X509)
    return blob_x509_has_grip(blob, grip);
  return 0;
}

static inline int has_issuer(KEYBOXBLOB blob, const char *name) {
  size_t namelen;

  return_val_if_fail(name, 0);

  if (blob_get_type(blob) != KEYBOX_BLOBTYPE_X509) return 0;

  namelen = strlen(name);
  return blob_cmp_name(blob, 0 /* issuer */, name, namelen, 0, 1);
}

static inline int has_issuer_sn(KEYBOXBLOB blob, const char *name,
...
static inline int has_sn(KEYBOXBLOB blob, const unsigned char *sn, int snlen) {
  return_val_if_fail(sn, 0);
...
static inline int has_subject(KEYBOXBLOB blob, const char *name) {
  size_t namelen;
...
static inline int has_username(KEYBOXBLOB blob, const char *name, int substr) {
  size_t namelen;
  int btype;
  ...
  if (btype != KEYBOX_BLOBTYPE_PGP && btype != KEYBOX_BLOBTYPE_X509) return 0;

  namelen = strlen(name);
  return blob_cmp_name(blob, -1 /* all subject/user names */, name, namelen,
                       substr, (btype == KEYBOX_BLOBTYPE_X509));
}

static inline int has_mail(KEYBOXBLOB blob, const char *name, int substr) {
  size_t namelen;
  int btype;
  ...
  namelen = strlen(name);
  if (namelen && name[namelen - 1] == '>') namelen--;
  return blob_cmp_mail(blob, name, namelen, substr,
                       (btype == KEYBOX_BLOBTYPE_X509));
}

static void release_sn_array(struct sn_array_s *array, size_t size) {
  size_t n;

  for (n = 0; n < size; n++) xfree(array[n].sn);
  xfree(array);
}

/* Helper to open the file.  */
static gpg_error_t open_file(KEYBOX_HANDLE hd) {
...
gpg_error_t keybox_search_reset(KEYBOX_HANDLE hd) {
  if (!hd) return GPG_ERR_INV_VALUE;
...
gpg_error_t keybox_search(KEYBOX_HANDLE hd, KEYBOX_SEARCH_DESC *desc,
                          size_t ndesc, keybox_blobtype_t want_blobtype,
  ...
  if (hd->error) return hd->error; /* still in error state */
  if (hd->eof) return -1;          /* still EOF */
  ...
  for (n = 0; n < ndesc; n++) {
    ...
    if (desc[n].skipfnc) any_skip = 1;
    if (desc[n].snlen == -1 && !sn_array) {
      sn_array = (sn_array_s *)xtrycalloc(ndesc, sizeof *sn_array);
      if (!sn_array) return (hd->error = gpg_error_from_syserror());
    }
  }
  ...
  for (;;) {
    ...
    if (blobtype == KEYBOX_BLOBTYPE_HEADER) continue;
    if (want_blobtype && blobtype != want_blobtype) continue;
    ...
    for (n = 0; n < ndesc; n++) {
      switch (desc[n].mode) {
        ...
        case KEYDB_SEARCH_MODE_ISSUER:
          if (has_issuer(blob, desc[n].u.name)) goto found;
          break;
        ...
        case KEYDB_SEARCH_MODE_SUBJECT:
          if (has_subject(blob, desc[n].u.name)) goto found;
          break;
        ...
        case KEYDB_SEARCH_MODE_KEYGRIP:
          if (has_keygrip(blob, desc[n].u.grip)) goto found;
          break;
        case KEYDB_SEARCH_MODE_FIRST:
          goto found;
          break;
  ...
  found:
    ...
    if (r_descindex) *r_descindex = n;
    for (n = any_skip ? 0 : ndesc; n < ndesc; n++) {
      u32 kid[2];

      if (desc[n].skipfnc && blob_get_first_keyid(blob, kid) &&
          desc[n].skipfnc(desc[n].skipfncvalue, kid, uid_no))
        break;
    }
    if (n == ndesc) break; /* got it */
  }
  ...
  if (sn_array) release_sn_array(sn_array, ndesc);
...
gpg_error_t keybox_get_keyblock(KEYBOX_HANDLE hd, iobuf_t *r_iobuf,
  ...
  *r_iobuf =
      iobuf_temp_with_content((const char *)(buffer + image_off), image_len);
  return 0;
}
 ...
 */
int keybox_get_cert(KEYBOX_HANDLE hd, ksba_cert_t *r_cert) {
  const unsigned char *buffer;
  size_t length;
  size_t cert_off, cert_len;
  ksba_reader_t reader = NULL;
  ...
  cert_off = get32(buffer + 8);
  cert_len = get32(buffer + 12);
  if (cert_off + cert_len > length) return GPG_ERR_TOO_SHORT;
