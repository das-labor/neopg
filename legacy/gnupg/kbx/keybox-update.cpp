...
static int create_tmp_file(const char *tempel, char **r_bakfname,
                           char **r_tmpfname, FILE **r_fp) {
  gpg_error_t err;
...
static int rename_tmp_file(const char *bakfname, const char *tmpfname,
...
static int blob_filecopy(int mode, const char *fname, KEYBOXBLOB blob,
  ...
  if (mode == FILECOPY_INSERT && !fp && errno == ENOENT) {
    ...
    if (!newfp) return gpg_error_from_syserror();
    ...
    if (fclose(newfp)) return gpg_error_from_syserror();
  ...
  if (mode == FILECOPY_INSERT) {
    ...
    while ((nread = fread(buffer, 1, DIM(buffer), fp)) > 0) {
      if (first_record && for_openpgp && buffer[4] == KEYBOX_BLOBTYPE_HEADER) {
        first_record = 0;
  ...
  if (mode == FILECOPY_DELETE || mode == FILECOPY_UPDATE) {
    ...
    while (current < start_offset) {
      nbytes = DIM(buffer);
      if (current + nbytes > start_offset) nbytes = start_offset - current;
...
gpg_error_t keybox_insert_keyblock(KEYBOX_HANDLE hd, const void *image,
                                   size_t imagelen) {
  gpg_error_t err;
  const char *fname;
  KEYBOXBLOB blob;
  ...
  if (!hd->kb) return GPG_ERR_INV_HANDLE;
  fname = hd->kb->fname;
  if (!fname) return GPG_ERR_INV_HANDLE;
  ...
  err = _keybox_parse_openpgp((const unsigned char *)(image), imagelen,
                              &nparsed, &info);
  if (err) return err;
  assert(nparsed <= imagelen);
  err = _keybox_create_openpgp_blob(
      &blob, &info, (const unsigned char *)(image), imagelen, hd->ephemeral);
...
/* Update the current key at HD with the given OpenPGP keyblock in
   {IMAGE,IMAGELEN}.  */
gpg_error_t keybox_update_keyblock(KEYBOX_HANDLE hd, const void *image,
                                   size_t imagelen) {
  gpg_error_t err;
  const char *fname;
  off_t off;
  ...
  if (!hd || !image || !imagelen) return GPG_ERR_INV_VALUE;
  ...
  if (off == (off_t)-1) return GPG_ERR_GENERAL;
...
int keybox_set_flags(KEYBOX_HANDLE hd, int what, int idx, unsigned int value) {
  ...
  if (fclose(fp)) {
    if (!ec) ec = gpg_error_from_syserror();
  }
...
int keybox_compress(KEYBOX_HANDLE hd) {
  ...
  if (!hd) return GPG_ERR_INV_HANDLE;
  if (!hd->kb) return GPG_ERR_INV_HANDLE;
  if (hd->secret) return GPG_ERR_NOT_IMPLEMENTED;
  fname = hd->kb->fname;
  if (!fname) return GPG_ERR_INV_HANDLE;
  ...
  for (rc = 0; !(read_rc = _keybox_read_blob(&blob, fp, &skipped_deleted));
    ...
    if (skipped_deleted) any_changes = 1;
    buffer = _keybox_get_blob_image(blob, &length);
    if (first_blob) {
      first_blob = 0;
      if (length > 4 && buffer[4] == KEYBOX_BLOBTYPE_HEADER) {
    ...
    if (_keybox_get_flag_location(buffer, length, KEYBOX_FLAG_BLOB, &pos,
                                  &size) ||
        size != 2) {
      rc = GPG_ERR_BUG;
      break;
    ...
    if ((blobflags & KEYBOX_FLAG_BLOB_EPHEMERAL)) {
      ...
      if (_keybox_get_flag_location(buffer, length, KEYBOX_FLAG_CREATED_AT,
                                    &pos, &size) ||
          size != 4)
        created_at = 0; /* oops. */
      else
  ...
  /* Close both files. */
  if (fclose(fp) && !rc) rc = gpg_error_from_syserror();
  if (fclose(newfp) && !rc) rc = gpg_error_from_syserror();
