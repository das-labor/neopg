...
static void try_make_homedir(const char *fname) {
  ...
  if (opt.dry_run || opt.no_homedir_creation) return;
...
static gpg_error_t maybe_create_keybox(char *filename, int force,
                                       int *r_created) {
  ...
  if (!force) return GPG_ERR_ENOENT;
  ...
  *last_slash_in_filename = save_slash;
  ...
  if (!lockhd) {
    ...
    if (opt.verbose) log_info("can't allocate lock for '%s'\n", filename);
  ...
  if (!opt.quiet) log_info(_("keybox '%s' created\n"), filename);
  if (r_created) *r_created = 1;
...
gpg_error_t sm_keydb_add_resource(ctrl_t ctrl, const char *url, int force,
  ...
  if (auto_created) *auto_created = 0;
...
leave:
  if (err) {
    ...
    gpgsm_status_with_error(ctrl, STATUS_ERROR, "add_keyblock_resource", err);
  } else
    any_registered = 1;
  xfree(filename);
  return err;
...
gpg_error_t sm_keydb_lock(KEYDB_HANDLE hd) {
  if (!hd) return GPG_ERR_INV_HANDLE;
  if (hd->locked) return 0; /* Already locked. */
  return lock_all(hd);
}
  ...
  for (i = hd->used - 1; i >= 0; i--) {
    switch (hd->active[i].type) {
      case KEYDB_RESOURCE_TYPE_NONE:
        break;
      case KEYDB_RESOURCE_TYPE_KEYBOX:
        if (hd->active[i].lockhandle) dotlock_release(hd->active[i].lockhandle);
        break;
    }
  }
  hd->locked = 0;
...
int sm_keydb_get_cert(KEYDB_HANDLE hd, ksba_cert_t *r_cert) {
  ...
  if (hd->found < 0 || hd->found >= hd->used) return -1; /* nothing found */
...
gpg_error_t sm_keydb_get_flags(KEYDB_HANDLE hd, int which, int idx,
  ...
  if (hd->found < 0 || hd->found >= hd->used) return GPG_ERR_NOTHING_FOUND;
...
gpg_error_t sm_keydb_set_flags(KEYDB_HANDLE hd, int which, int idx,
  ...
  if (!hd->locked) return GPG_ERR_NOT_LOCKED;
    ...
    switch (hd->active[hd->current].type) {
      ...
      case KEYDB_RESOURCE_TYPE_KEYBOX:
        rc = keybox_search(hd->active[hd->current].u.kr, desc, ndesc,
                           KEYBOX_BLOBTYPE_X509, NULL, &skipped);
        break;
    }
    if (rc == -1 || rc == GPG_ERR_EOF) { /* EOF -> switch to next resource */
      hd->current++;
    } else if (!rc)
      hd->found = hd->current;
  }
...
int sm_keydb_search_issuer_sn(ctrl_t ctrl, KEYDB_HANDLE hd, const char *issuer,
  ...
  s = serial;
  if (*s != '(') return GPG_ERR_INV_VALUE;
  s++;
  for (desc.snlen = 0; digitp(s); s++) desc.snlen = 10 * desc.snlen + atoi_1(s);
  if (*s != ':') return GPG_ERR_INV_VALUE;
  desc.sn = s + 1;
  desc.u.name = issuer;
  rc = sm_keydb_search(ctrl, hd, &desc, 1);
...
int sm_keydb_search_subject(ctrl_t ctrl, KEYDB_HANDLE hd, const char *name) {
  KEYDB_SEARCH_DESC desc;
  int rc;
...
int sm_keydb_store_cert(ctrl_t ctrl, ksba_cert_t cert, int ephemeral,
  ...
  if (!ephemeral) sm_keydb_set_ephemeral(kh, 0);
...
gpg_error_t sm_keydb_set_cert_flags(ctrl_t ctrl, ksba_cert_t cert,
  ...
  if (ephemeral) sm_keydb_set_ephemeral(kh, 1);
...
void sm_keydb_clear_some_cert_flags(ctrl_t ctrl, strlist_t names) {
  ...
  if (rc && rc != -1) log_error("keydb_search failed: %s\n", gpg_strerror(rc));
