...
static int do_encode_md(gcry_md_hd_t md, int algo, int pkalgo,
                        unsigned int nbits, gcry_sexp_t pkey,
                        gcry_mpi_t *r_val) {
  int n;
  size_t nframe;
  unsigned char *frame;
  ...
  if (pkalgo == GCRY_PK_DSA || pkalgo == GCRY_PK_ECDSA) {
    ...
    if (nframe < qbits / 8) {
      log_error(_("a %u bit hash is not valid for a %u bit %s key\n"),
                (unsigned int)nframe * 8, gcry_pk_get_nbits(pkey),
                gcry_pk_algo_name(pkalgo));
      /* FIXME: we need to check the requirements for ECDSA.  */
      if (nframe < 20 || pkalgo == GCRY_PK_DSA) return GPG_ERR_INTERNAL;
    }
  ...
  if (DBG_CRYPTO) {
    ...
    for (j = 0; j < nframe; j++) log_printf(" %02X", frame[j]);
    log_printf("\n");
  }
...
int gpgsm_check_cert_sig(ksba_cert_t issuer_cert, ksba_cert_t cert) {
  ...
  if (gcry_sexp_build(&s_hash, NULL, "%m", frame)) BUG();
...
int gpgsm_check_cms_signature(ksba_cert_t cert, ksba_const_sexp_t sigval,
  ...
  if (DBG_CRYPTO) log_printhex("public key: ", p, n);
  ...
  if (r_pkalgo) *r_pkalgo = pkalgo;
  rc = do_encode_md(md, mdalgo, pkalgo, gcry_pk_get_nbits(s_pkey), s_pkey,
                    &frame);
  if (rc) {
    gcry_sexp_release(s_sig);
    gcry_sexp_release(s_pkey);
...
int gpgsm_create_cms_signature(ctrl_t ctrl, ksba_cert_t cert, gcry_md_hd_t md,
                               int mdalgo, unsigned char **r_sigval) {
  int rc;
  char *grip, *desc;
  size_t siglen;

  grip = gpgsm_get_keygrip_hexstring(cert);
  if (!grip) return GPG_ERR_BAD_CERT;

  desc = gpgsm_format_keydesc(cert);

  rc = gpgsm_agent_pksign(ctrl, grip, desc, gcry_md_read(md, mdalgo),
                          gcry_md_get_algo_dlen(mdalgo), mdalgo, r_sigval,
                          &siglen);
  xfree(desc);
  xfree(grip);
  return rc;
