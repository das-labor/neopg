...
static int hash_data(int fd, gcry_md_hd_t md) {
  estream_t fp;
  char buffer[4096];
  int nread;
  int rc = 0;
...
static int hash_and_copy_data(int fd, gcry_md_hd_t md, ksba_writer_t writer) {
  gpg_error_t err;
  estream_t fp;
  char buffer[4096];
  int nread;
...
static ksba_cert_t get_default_signer(ctrl_t ctrl) {
...
/* Depending on the options in CTRL add the certificate CERT as well as
   ...
   object. */
static int add_certificate_list(ctrl_t ctrl, ksba_cms_t cms, ksba_cert_t cert) {
  gpg_error_t err;
  int rc = 0;
  ...
  if (n < 0 || n > 50) n = 50; /* We better apply an upper bound */
...
int gpgsm_sign(ctrl_t ctrl, certlist_t signerlist, int data_fd, int detached,
               estream_t out_fp) {
  int i, rc;
  gpg_error_t err;
  ...
  if (!err) err = ksba_cms_set_content_type(cms, 1, KSBA_CT_DATA);
  if (err) {
  ...
  if (!signerlist) {
    ...
    if (!rc) rc = gpgsm_validate_chain(ctrl, cert, "", NULL, 0, NULL, 0, NULL);
    if (rc) {
      char *tmpfpr;

      tmpfpr = gpgsm_get_fingerprint_hexstring(cert, 0);
      gpgsm_status2(ctrl, STATUS_INV_SGNR, get_inv_recpsgnr_code(rc), tmpfpr,
                    NULL);
      xfree(tmpfpr);
      goto leave;
    }
  ...
  for (i = 0, cl = signerlist; cl; cl = cl->next, i++) {
    ...
    /* Check compliance.  */
    if (!gnupg_digest_is_allowed(opt.compliance, 1,
                                 (digest_algo_t)(cl->hash_algo))) {
...

    {
      ...
      if (!gnupg_pk_is_allowed(opt.compliance, PK_USE_SIGNING, pk_algo, NULL,
                               nbits, NULL)) {
        log_error(
  ...
  if (opt.verbose) {
    for (i = 0, cl = signerlist; cl; cl = cl->next, i++)
      log_info(_("hash algorithm used for signer %d: %s (%s)\n"), i,
               gcry_md_algo_name(cl->hash_algo), cl->hash_algo_oid);
  }
  ...
  if (DBG_HASHING) gcry_md_debug(data_md, "sign.data");
  ...
  if (detached) { /* We hash the data right now so that we can store the message
                     digest.  ksba_cms_build() takes this as an flag that
                     detached
                     data is expected. */
  ...
  */
  ...
  do {
    ...
    if (stopreason == KSBA_SR_BEGIN_DATA) {
      ...
      if (DBG_HASHING) gcry_md_debug(md, "sign.attr");
      ksba_cms_set_hash_function(cms, HASH_FNC, md);
      for (cl = signerlist, signer = 0; cl; cl = cl->next, signer++) {
        unsigned char *sigval = NULL;
        char *buf, *fpr;

        if (signer) gcry_md_reset(md);
        {
          certlist_t cl_tmp;
        ...
        rc = gpgsm_create_cms_signature(ctrl, cl->cert, md, cl->hash_algo,
                                        &sigval);
        if (rc) {
          gcry_md_close(md);
          goto leave;
        ...
        rc = 0;
        {
          ...
          buf = xtryasprintf("%c %d %d 00 %s %s", detached ? 'D' : 'S', pkalgo,
                             cl->hash_algo, signed_at, fpr);
          if (!buf) rc = gpg_error_from_syserror();
        }
        xfree(fpr);
        if (rc) {
          gcry_md_close(md);
...
leave:
  if (rc) log_error("error creating signature: %s\n", gpg_strerror(rc));
  if (release_signerlist) gpgsm_release_certlist(signerlist);
