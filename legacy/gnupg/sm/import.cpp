...
static void print_imported_status(ctrl_t ctrl, ksba_cert_t cert, int new_cert) {
  char *fpr;

  fpr = gpgsm_get_fingerprint_hexstring(cert, GCRY_MD_SHA1);
  if (new_cert) gpgsm_status2(ctrl, STATUS_IMPORTED, fpr, "[X.509]", NULL);

  gpgsm_status2(ctrl, STATUS_IMPORT_OK, new_cert ? "1" : "0", fpr, NULL);
...
*/
static void print_import_problem(ctrl_t ctrl, ksba_cert_t cert, int reason) {
  char *fpr = NULL;
  char buf[25];
  int i;
...
void print_imported_summary(ctrl_t ctrl, struct stats_s *stats) {
  char buf[14 * 25];
  ...
  sprintf(buf, "%lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu",
          stats->count, 0l /*stats->no_user_id*/, stats->imported,
          0l /*stats->imported_rsa*/, stats->unchanged, 0l /*stats->n_uids*/,
          0l /*stats->n_subk*/, 0l /*stats->n_sigs*/, 0l /*stats->n_revoc*/,
          stats->secret_read, stats->secret_imported, stats->secret_dups,
          0l /*stats->skipped_new_keys*/, stats->not_imported);
  gpgsm_status(ctrl, STATUS_IMPORT_RES, buf);
}

static void check_and_store(ctrl_t ctrl, struct stats_s *stats,
  ...
  */
  ...
  if (!rc || (!ctrl->with_validation && (rc == GPG_ERR_MISSING_CERT ||
    ...
    print_import_problem(
        ...
        rc == GPG_ERR_MISSING_ISSUER_CERT
            ? 2
            : rc == GPG_ERR_MISSING_CERT ? 2 : rc == GPG_ERR_BAD_CERT ? 1 : 0);
  }
}

static int import_one(ctrl_t ctrl, struct stats_s *stats, int in_fd) {
  int rc;
  ...
  do {
    ...
    if (ct == KSBA_CT_SIGNED_DATA) { /* This is probably a signed-only message -
                                        import the certs */
      ...
      if (!rc) any = 1;
               ...
               KSBA_CT_NONE) { /* Failed to identify this message - assume a
                                  certificate */
...
/* Re-import certifciates.  IN_FD is a list of linefeed delimited
   ...
   the ephemeral flag.  */
static int reimport_one(ctrl_t ctrl, struct stats_s *stats, int in_fd) {
  gpg_error_t err = 0;
  estream_t fp = NULL;
...
static gpg_error_t rsa_key_check(struct rsa_secret_key_s *skey) {
  int err = 0;
  ...
  if (err) log_info("RSA secret key check failed\n");
...
/* Helper to store the DER encoded certificate CERTDATA of length
   CERTDATALEN.  */
static void store_cert_cb(void *opaque, const unsigned char *certdata,
                          size_t certdatalen) {
...
static gpg_error_t parse_p12(ctrl_t ctrl, ksba_reader_t reader,
  ...
  if (!err) {
    ...
    if (!p12buffer) err = gpg_error_from_syserror();
  }
  if (err) {
  ...
  if (p12buflen > 29 && !memcmp(p12buffer, "The passphrase is ", 18)) {
    for (p12bufoff = 18; p12bufoff < p12buflen && p12buffer[p12bufoff] != '\n';
         p12bufoff++)
      ;
    p12bufoff++;
  ...
  kparms = p12_parse((const unsigned char *)(p12buffer + p12bufoff),
                     p12buflen - p12bufoff, passphrase, store_cert_cb,
                     &store_cert_parm, &bad_pass);
  ...
  for (i = 0; i < 8; i++) gcry_mpi_release(kparms[i]);
  gcry_free(kparms);
  kparms = NULL;
  if (err) {
  ...
  if (!err && store_cert_parm.err) err = store_cert_parm.err;
...
leave:
  ...
  if (bad_pass) {
    ...
    gpgsm_status_with_err_code(ctrl, STATUS_ERROR, "import.parsep12",
