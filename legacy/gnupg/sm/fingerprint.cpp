...
unsigned char *gpgsm_get_fingerprint(ksba_cert_t cert, int algo,
  ...
  if (!algo) algo = GCRY_MD_SHA1;

  len = gcry_md_get_algo_dlen(algo);
  assert(len);
  if (!array) array = (unsigned char *)xmalloc(len);
  ...
  if (algo == GCRY_MD_SHA1) {
    ...
    assert(len >= 20);
    if (!ksba_cert_get_user_data(cert, "sha1-fingerprint", array, len,
                                 &buflen) &&
        buflen == 20)
      return array;
  }
...
unsigned long gpgsm_get_short_fingerprint(ksba_cert_t cert,
  ...
  if (r_high) *r_high = buf32_to_ulong(digest + 12);
...
unsigned char *gpgsm_get_keygrip(ksba_cert_t cert, unsigned char *array) {
  ...
  if (!p) return NULL; /* oops */

  if (DBG_X509) log_debug("get_keygrip for public key\n");
  ...
  if (DBG_X509) log_printhex("keygrip=", array, 20);
...
char *gpgsm_get_keygrip_hexstring(ksba_cert_t cert) {
  ...
  if (!gpgsm_get_keygrip(cert, grip)) return NULL;
  buf = (char *)xtrymalloc(20 * 2 + 1);
  if (buf) bin2hex(grip, 20, buf);
  return buf;
}
...
int gpgsm_get_key_algo_info(ksba_cert_t cert, unsigned int *nbits) {
  ...
  if (nbits) *nbits = gcry_pk_get_nbits(s_pkey);
  ...
  if (name) {
    if (n > sizeof namebuf - 1) n = sizeof namebuf - 1;
...
char *gpgsm_get_certid(ksba_cert_t cert) {
  ...
  if (!serial) return NULL; /* oops: no serial number */
  p = (char *)serial;
  if (*p != '(') {
