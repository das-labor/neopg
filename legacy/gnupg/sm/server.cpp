...
static void strcpy_escaped_plus(char *d, const char *s) {
  while (*s) {
    if (*s == '%' && s[1] && s[2]) {
      s++;
      *d++ = xtoi_2(s);
...
static void close_message_fd(ctrl_t ctrl) {
...
static gpg_error_t option_handler(assuan_context_t ctx, const char *key,
  ...
  if (!strcmp(key, "lc-ctype")) {
    ...
    if (!opt.lc_ctype) err = gpg_error_from_syserror();
  } else if (!strcmp(key, "lc-messages")) {
    xfree(opt.lc_messages);
    opt.lc_messages = xtrystrdup(value);
    if (!opt.lc_messages) err = gpg_error_from_syserror();
...
static gpg_error_t input_notify(assuan_context_t ctx, char *line) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
...
static gpg_error_t output_notify(assuan_context_t ctx, char *line) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
...
static gpg_error_t cmd_recipient(assuan_context_t ctx, char *line) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
  int rc;
...
static const char hlp_signer[] =
    ...
    "operation.";
static gpg_error_t cmd_signer(assuan_context_t ctx, char *line) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
  int rc;

  rc = gpgsm_add_to_certlist(ctrl, line, 1, &ctrl->server_local->signerlist, 0);
  if (rc) {
    gpgsm_status2(ctrl, STATUS_INV_SGNR, get_inv_recpsgnr_code(rc), line, NULL);
...
static const char hlp_encrypt[] =
    ...
    "pipes are closed.";
static gpg_error_t cmd_encrypt(assuan_context_t ctx, char *line) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
  certlist_t cl;
  int inp_fd, out_fd;
  estream_t out_fp;
  ...
  if (inp_fd == -1) return set_error(GPG_ERR_ASS_NO_INPUT, NULL);
  out_fd = translate_sys2libc_fd(assuan_get_output_fd(ctx), 1);
  if (out_fd == -1) return set_error(GPG_ERR_ASS_NO_OUTPUT, NULL);

  out_fp = es_fdopen_nc(out_fd, "w");
  if (!out_fp) return set_error(gpg_error_from_syserror(), "fdopen() failed");
  ...
  if (!rc)
    rc = gpgsm_encrypt((ctrl_t)(assuan_get_pointer(ctx)),
                       ctrl->server_local->recplist, inp_fd, out_fp);
  es_fclose(out_fp);
...
static gpg_error_t cmd_decrypt(assuan_context_t ctx, char *line) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
  int inp_fd, out_fd;
  estream_t out_fp;
  int rc;
...
static gpg_error_t cmd_verify(assuan_context_t ctx, char *line) {
  int rc;
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
  ...
  if (fd == -1) return set_error(GPG_ERR_ASS_NO_INPUT, NULL);
...
static const char hlp_sign[] =
    ...
    "created (surprise).";
static gpg_error_t cmd_sign(assuan_context_t ctx, char *line) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
  int inp_fd, out_fd;
  estream_t out_fp;
  int detached;
  ...
  out_fp = es_fdopen_nc(out_fd, "w");
  if (!out_fp) return set_error(GPG_ERR_ASS_GENERAL, "fdopen() failed");
...
static gpg_error_t cmd_import(assuan_context_t ctx, char *line) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
  int rc;
  int fd = translate_sys2libc_fd(assuan_get_input_fd(ctx), 0);
...
static gpg_error_t cmd_export(assuan_context_t ctx, char *line) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
  char *p;
  strlist_t list, sl;
  int use_data;
  ...
  if (opt_secret) {
    if (!list || !*list->d) return set_error(GPG_ERR_NO_DATA, "No key given");
    if (list->next) return set_error(GPG_ERR_TOO_MANY, "Only one key allowed");
  }
  ...
  if (use_data) {
    ...
    if (!stream) {
      free_strlist(list);
      return set_error(GPG_ERR_ASS_GENERAL, "error setting up a data stream");
    }
    if (opt_secret)
      gpgsm_p12_export(ctrl, list->d, stream, opt_raw ? 2 : opt_pkcs12 ? 0 : 1);
    else
    ...
    if (opt_secret)
      gpgsm_p12_export(ctrl, list->d, out_fp, opt_raw ? 2 : opt_pkcs12 ? 0 : 1);
    else
...
static gpg_error_t cmd_delkeys(assuan_context_t ctx, char *line) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
  char *p;
  strlist_t list, sl;
  int rc;
...
static int do_listkeys(assuan_context_t ctx, char *line, int mode) {
  ctrl_t ctrl = (ctrl_t)assuan_get_pointer(ctx);
  estream_t fp;
  char *p;
  strlist_t list, sl;
  ...
  if (ctrl->server_local->list_to_output) {
    ...
    if (outfd == -1) return set_error(GPG_ERR_ASS_NO_OUTPUT, NULL);
    fp = es_fdopen_nc(outfd, "w");
    if (!fp) return set_error(gpg_error_from_syserror(), "es_fdopen() failed");
  } else {
  ...
  ctrl->with_colons = 1;
  listmode = mode;
  if (ctrl->server_local->list_internal) listmode |= (1 << 6);
  if (ctrl->server_local->list_external) listmode |= (1 << 7);
  err = gpgsm_list_keys((ctrl_t)(assuan_get_pointer(ctx)), list, fp, listmode);
  free_strlist(list);
  es_fclose(fp);
  if (ctrl->server_local->list_to_output) assuan_close_output_fd(ctx);
  return err;
}

static gpg_error_t cmd_listkeys(assuan_context_t ctx, char *line) {
  return do_listkeys(ctx, line, 3);
}

static gpg_error_t cmd_dumpkeys(assuan_context_t ctx, char *line) {
  return do_listkeys(ctx, line, 259);
}

static gpg_error_t cmd_listsecretkeys(assuan_context_t ctx, char *line) {
  return do_listkeys(ctx, line, 2);
}

static gpg_error_t cmd_dumpsecretkeys(assuan_context_t ctx, char *line) {
...
static gpg_error_t cmd_genkey(assuan_context_t ctx, char *line) {
  ...
  if (!in_stream) return set_error(GPG_ERR_ASS_GENERAL, "es_fdopen failed");
...
static int command_has_option(const char *cmd, const char *cmdopt) {
  if (!strcmp(cmd, "IMPORT")) {
    if (!strcmp(cmdopt, "re-import")) return 1;
  }
...
static int register_commands(assuan_context_t ctx) {
  ...
  } table[] = {{"RECIPIENT", cmd_recipient, hlp_recipient},
...
void gpgsm_server(certlist_t default_recplist) {
  ...
  static const char hello[] =
      ("GNU Privacy Guard's S/M server " VERSION " ready");
...
#define SERVER_STDOUT 1
  ...
  if (opt.verbose || opt.debug) {
    ...
    if (asprintf(&tmp,
                 ...
                 "Config: %s\n"
                 "%s",
                 gnupg_homedir(), opt.config_filename, hello) > 0) {
...
gpg_error_t gpgsm_proxy_pinentry_notify(ctrl_t ctrl,
  ...
  if (!ctrl || !ctrl->server_local) return 0;
  return assuan_inquire(ctrl->server_local->assuan_ctx, (const char *)(line),
                        NULL, NULL, 0);
}
