...
static int init_dek(DEK dek) {
  ...
  if (!dek->keylen || dek->keylen > sizeof(dek->key)) return GPG_ERR_BUG;

  dek->ivlen = gcry_cipher_get_algo_blklen(dek->algo);
  if (!dek->ivlen || dek->ivlen > sizeof(dek->iv)) return GPG_ERR_BUG;
...
static int encode_session_key(DEK dek, gcry_sexp_t *r_data) {
  gcry_sexp_t data;
  char *p;
  int rc;
...
static int encrypt_dek(const DEK dek, ksba_cert_t cert,
...
static int encrypt_cb(void *cb_value, char *buffer, size_t count,
                      size_t *nread) {
  ...
  *nread = 0;
  ...
  if (parm->ready) return -1;

  if (count < blklen) BUG();

  if (!parm->eof_seen) { /* fillup the buffer */
    p = parm->buffer;
  ...
  } else if (parm->eof_seen) { /* no complete block but eof: add padding */
...
int gpgsm_encrypt(ctrl_t ctrl, certlist_t recplist, int data_fd,
                  estream_t out_fp) {
  int rc = 0;
  ...
  if (err) rc = err;
  if (!rc) rc = ksba_reader_set_cb(reader, encrypt_cb, &encparm);
  if (rc) goto leave;
  ...
  for (recpno = 0, cl = recplist; cl; recpno++, cl = cl->next) {
    ...
    if (!gnupg_pk_is_allowed(opt.compliance, PK_USE_ENCRYPTION, pk_algo, NULL,
                             nbits, NULL)) {
      log_error(
