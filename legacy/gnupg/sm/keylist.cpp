...
#define OIDSTR_restriction "1.3.36.8.3.8"
...
/* Return the description for OID; if no description is available
   NULL is returned. */
static const char *get_oid_desc(const char *oid, unsigned int *flag) {
  int i;

  if (oid)
    for (i = 0; oidtranstbl[i].oid; i++)
      if (!strcmp(oidtranstbl[i].oid, oid)) {
        if (flag) *flag = oidtranstbl[i].flag;
...
static void print_key_data(ksba_cert_t cert, estream_t fp) {
#if 0
...
static void print_capabilities(ksba_cert_t cert, estream_t fp) {
  gpg_error_t err;
  unsigned int use;
  size_t buflen;
  char buffer[1];
  ...
  if ((use & (KSBA_KEYUSAGE_DIGITAL_SIGNATURE | KSBA_KEYUSAGE_NON_REPUDIATION)))
    es_putc('s', fp);
  if ((use & KSBA_KEYUSAGE_KEY_CERT_SIGN)) es_putc('c', fp);
  if ((use &
       (KSBA_KEYUSAGE_KEY_ENCIPHERMENT | KSBA_KEYUSAGE_DATA_ENCIPHERMENT)))
    es_putc('E', fp);
  if ((use & (KSBA_KEYUSAGE_DIGITAL_SIGNATURE | KSBA_KEYUSAGE_NON_REPUDIATION)))
...
static void print_time(gnupg_isotime_t t, estream_t fp) {
  if (!t || !*t)
    ;
  else
    es_fputs(t, fp);
...
static char *email_kludge(const char *name) {
  const char *p, *string;
  unsigned char *buf;
  int n;
  ...
  if (!buf) return NULL; /* oops, out of core */
  *buf = '<';
  for (n = 1, p = name; hexdigitp(p); p += 2, n++) buf[n] = xtoi_2(p);
  buf[n++] = '>';
  buf[n] = 0;
  return (char *)buf;
}
...
static void print_compliance_flags(int algo, unsigned int nbits, estream_t fp) {
  if (gnupg_pk_is_compliant(CO_DE_VS, algo, NULL, nbits, NULL))
    es_fputs(gnupg_status_compliance_flag(CO_DE_VS), fp);
}

/* List one certificate in colon mode */
static void list_cert_colon(ctrl_t ctrl, ksba_cert_t cert,
                            unsigned int validity, estream_t fp,
                            int have_secret) {
  int rc;
  int idx;
  char truststring[2];
  ...
  truststring[0] = 0;
  truststring[1] = 0;
  if ((validity & VALIDITY_REVOKED) || valerr == GPG_ERR_CERT_REVOKED)
    *truststring = 'r';
    ...
    *truststring = 'e';
    ...
    if (!opt.ignore_expiration && !ksba_cert_get_validity(cert, 1, not_after) &&
  ...
  if (*truststring) es_fputs(truststring, fp);
  ...
  if ((sexp = ksba_cert_get_serial(cert))) {
    ...
    if (*s == '(') {
      ...
      for (len = 0; *s && *s != ':' && digitp(s); s++)
        len = len * 10 + atoi_1(s);
      if (*s == ':')
        for (s++; len; len--, s++) es_fprintf(fp, "%02X", *s);
    }
    xfree(sexp);
  }
  es_putc(':', fp);
  ...
  if (chain_id) es_fputs(chain_id, fp);
  es_putc(':', fp);
  es_putc('\n', fp);
  xfree(fpr);
  fpr = NULL;
  chain_id = NULL;
  xfree(chain_id_buffer);
  chain_id_buffer = NULL;
  ...
  for (idx = 0; (p = ksba_cert_get_subject(cert, idx)); idx++) {
    /* In the case that the same email address is in the subject DN
       ...
       a second time. */
    if (kludge_uid && !strcmp(kludge_uid, p)) continue;
    ...
    if (!idx) {
      ...
      if (kludge_uid) {
        es_fprintf(fp, "uid:%s::::::::", truststring);
        es_write_sanitized(fp, kludge_uid, strlen(kludge_uid), ":", NULL);
        es_putc(':', fp);
        es_putc(':', fp);
        es_putc('\n', fp);
      }
...
static void print_name_raw(estream_t fp, const char *string) {
  if (!string)
    es_fputs("[error]", fp);
  else
...
static void print_names_raw(estream_t fp, int indent, ksba_name_t name) {
  int idx;
  const char *s;
  int indent_all;
...
static void print_utf8_extn_raw(estream_t fp, int indent,
...
static void print_utf8_extn(estream_t fp, int indent, const unsigned char *der,
                            size_t derlen) {
  gpg_error_t err;
  int klasse, tag, constructed, ndef;
  size_t objlen, hdrlen;
...
static void list_cert_raw(ctrl_t ctrl, KEYDB_HANDLE hd, ksba_cert_t cert,
                          estream_t fp, int have_secret, int with_validation) {
  gpg_error_t err;
  size_t off, len;
  ...
  if (err != GPG_ERR_NO_DATA) {
    if (err)
      ...
      if ((kusage & KSBA_KEYUSAGE_KEY_AGREEMENT)) es_fputs(" keyAgreement", fp);
      if ((kusage & KSBA_KEYUSAGE_KEY_CERT_SIGN)) es_fputs(" certSign", fp);
      if ((kusage & KSBA_KEYUSAGE_CRL_SIGN)) es_fputs(" crlSign", fp);
      if ((kusage & KSBA_KEYUSAGE_ENCIPHER_ONLY)) es_fputs(" encipherOnly", fp);
      if ((kusage & KSBA_KEYUSAGE_DECIPHER_ONLY)) es_fputs(" decipherOnly", fp);
    }
    es_putc('\n', fp);
  } else
    es_fputs(" [none]\n", fp);
      ...
      while (p && (pend = strchr(p, ':'))) {
        *pend++ = 0;
        for (i = 0; key_purpose_map[i].oid; i++)
          if (!strcmp(key_purpose_map[i].oid, p)) break;
        es_fputs(key_purpose_map[i].oid ? key_purpose_map[i].name : p, fp);
        p = pend;
        if (*p != 'C') es_fputs(" (suggested)", fp);
        if ((p = strchr(p, '\n'))) {
          p++;
          es_fputs("\n               ", fp);
        }
  ...
  for (idx = 0;
       !(err = ksba_cert_get_crl_dist_point(cert, idx, &name, &name2,
    ...
    if (reason) {
      es_fputs("               reason: ", fp);
      if ((reason & KSBA_CRLREASON_UNSPECIFIED)) es_fputs(" unused", fp);
      ...
      if ((reason & KSBA_CRLREASON_SUPERSEDED)) es_fputs(" superseded", fp);
  ...
  if (err && err != GPG_ERR_EOF && err != GPG_ERR_NO_VALUE)
  ...
  /* authorityInfoAccess. */
  for (idx = 0;
       !(err = ksba_cert_get_authority_info_access(cert, idx, &string, &name));
       idx++) {
  ...
  /* subjectInfoAccess. */
  for (idx = 0;
       !(err = ksba_cert_get_subject_info_access(cert, idx, &string, &name));
       idx++) {
  ...
  for (idx = 0;
       !(err = ksba_cert_get_extension(cert, idx, &oid, &i, &off, &len));
       idx++) {
    unsigned int flag;

    s = get_oid_desc(oid, &flag);
    if ((flag & OID_FLAG_SKIP)) continue;

    es_fprintf(fp, "     %s: %s%s%s%s  [%d octets]\n",
               i ? "critExtn" : "    extn", oid, s ? " (" : "", s ? s : "",
               s ? ")" : "", (int)len);
    if ((flag & OID_FLAG_UTF8)) {
      if (!cert_der) cert_der = ksba_cert_get_image(cert, NULL);
      assert(cert_der);
...
static void list_cert_std(ctrl_t ctrl, ksba_cert_t cert, estream_t fp,
                          int have_secret, int with_validation) {
  gpg_error_t err;
  ksba_sexp_t sexp;
  char *dn;
  ksba_isotime_t t;
  ...
  /* Print restrictions.  */
  for (idx = 0;
       !(err = ksba_cert_get_extension(cert, idx, &oid, NULL, &off, &len));
       idx++) {
...
static void list_cert_chain(ctrl_t ctrl, KEYDB_HANDLE hd, ksba_cert_t cert,
                            int raw_mode, estream_t fp, int with_validation) {
 ...
 */
static gpg_error_t list_internal_keys(ctrl_t ctrl, strlist_t names,
  ...
  if (!want_ephemeral && names && ndesc) {
    ...
    for (i = 0; (i < ndesc && (desc[i].mode == KEYDB_SEARCH_MODE_FPR ||
                               desc[i].mode == KEYDB_SEARCH_MODE_FPR20 ||
                               desc[i].mode == KEYDB_SEARCH_MODE_FPR16 ||
                               desc[i].mode == KEYDB_SEARCH_MODE_KEYGRIP));
         i++)
      ;
    if (i == ndesc) want_ephemeral = 1;
  }

  if (want_ephemeral) sm_keydb_set_ephemeral(hd, 1);
  ...
  while (!(rc = sm_keydb_search(ctrl, hd, desc, ndesc))) {
    ...
    if (!names) desc[0].mode = KEYDB_SEARCH_MODE_NEXT;
    ...
    if (lastresname != resname) {
      ...
      if (ctrl->no_server) {
        es_fprintf(fp, "%s\n", resname);
        for (i = strlen(resname); i; i--) es_putc('-', fp);
        es_putc('\n', fp);
        lastresname = resname;
      }
    }
    ...
    if (!mode || ((mode & 1) && !have_secret) || ((mode & 2) && have_secret)) {
      if (ctrl->with_colons)
        list_cert_colon(ctrl, cert, validity, fp, have_secret);
      else if (ctrl->with_chain)
        list_cert_chain(ctrl, hd, cert, raw_mode, fp, ctrl->with_validation);
      else {
        if (raw_mode)
          list_cert_raw(ctrl, hd, cert, fp, have_secret, ctrl->with_validation);
        else
          list_cert_std(ctrl, cert, fp, have_secret, ctrl->with_validation);
        es_putc('\n', fp);
      }
    }
  ...
  if (rc == GPG_ERR_EOF || rc == -1) rc = 0;
  if (rc) log_error("keydb_search failed: %s\n", gpg_strerror(rc));
...
static void list_external_cb(void *cb_value, ksba_cert_t cert) {
  ...
  if (parm->print_header) {
    ...
    for (i = strlen(resname); i; i--) es_putc('-', parm->fp);
    es_putc('\n', parm->fp);
    parm->print_header = 0;
  }
...
/* List external keys similar to internal one.  Note: mode does not
   ...
   keys */
static gpg_error_t list_external_keys(ctrl_t ctrl, strlist_t names,
                                      estream_t fp, int raw_mode) {
  int rc;
  struct list_external_parm_s parm;

  parm.fp = fp;
  parm.ctrl = ctrl, parm.print_header = ctrl->no_server;
  ...
  if (rc == GPG_ERR_EOF || rc == -1 || rc == GPG_ERR_NOT_FOUND)
    rc = 0; /* "Not found" is not an error here. */
  if (rc) log_error("listing external keys failed: %s\n", gpg_strerror(rc));
  return rc;
}
 ...
 */
gpg_error_t gpgsm_list_keys(ctrl_t ctrl, strlist_t names, estream_t fp,
                            unsigned int mode) {
  gpg_error_t err = 0;
