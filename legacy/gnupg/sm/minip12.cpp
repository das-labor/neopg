...
static unsigned char const oid_data[9] = {0x2A, 0x86, 0x48, 0x86, 0xF7,
                                          0x0D, 0x01, 0x07, 0x01};
static unsigned char const oid_encryptedData[9] = {0x2A, 0x86, 0x48, 0x86, 0xF7,
                                                   0x0D, 0x01, 0x07, 0x06};
...
static unsigned char const oid_pkcs5PBKDF2[9] = {0x2A, 0x86, 0x48, 0x86, 0xF7,
                                                 0x0D, 0x01, 0x05, 0x0C};
static unsigned char const oid_pkcs5PBES2[9] = {0x2A, 0x86, 0x48, 0x86, 0xF7,
                                                0x0D, 0x01, 0x05, 0x0D};
static unsigned char const oid_aes128_CBC[9] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                                0x03, 0x04, 0x01, 0x02};

static unsigned char const oid_rsaEncryption[9] = {0x2A, 0x86, 0x48, 0x86, 0xF7,
                                                   0x0D, 0x01, 0x01, 0x01};

static unsigned char const data_3desiter2048[30] = {
    0x30, 0x1C, 0x06, 0x0A, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D,
    0x01, 0x0C, 0x01, 0x03, 0x30, 0x0E, 0x04, 0x08, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x02, 0x02, 0x08, 0x00};
#define DATA_3DESITER2048_SALT_OFF 18

static unsigned char const data_rc2iter2048[30] = {
    0x30, 0x1C, 0x06, 0x0A, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D,
    0x01, 0x0C, 0x01, 0x06, 0x30, 0x0E, 0x04, 0x08, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x02, 0x02, 0x08, 0x00};
#define DATA_RC2ITER2048_SALT_OFF 18

static unsigned char const data_mactemplate[51] = {
    0x30, 0x31, 0x30, 0x21, 0x30, 0x09, 0x06, 0x05, 0x2b, 0x0e, 0x03,
    0x02, 0x1a, 0x05, 0x00, 0x04, 0x14, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0x04, 0x08, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0x02, 0x02, 0x08, 0x00};
#define DATA_MACTEMPLATE_MAC_OFF 17
#define DATA_MACTEMPLATE_SALT_OFF 39

static unsigned char const
    data_attrtemplate[106] = {0x31, 0x7c, 0x30, 0x55, 0x06, 0x09, 0x2a, 0x86,
...
static int parse_tag(unsigned char const **buffer, size_t *size,
                     struct tag_info *ti) {
  int c;
  unsigned long tag;
  ...
  /* Get the length */
  if (!length) return -1; /* prematureeof */
  c = *buf++;
  length--;
  ti->nhdr++;
  ...
  if (!(c & 0x80))
    ...
    for (; count; count--) {
      len <<= 8;
      if (!length) return -1; /* premature_eof */
      c = *buf++;
      length--;
      ti->nhdr++;
      len |= c & 0xff;
  ...
  if (ti->klasse == UNIVERSAL && !ti->tag) ti->length = 0;
...
static unsigned char *cram_octet_string(const unsigned char *input,
  ...
  for (;;) {
    if (parse_tag(&s, &n, &ti)) goto bailout;
    if (ti.klasse == UNIVERSAL && ti.tag == TAG_OCTET_STRING && !ti.ndef &&
        !ti.is_constructed) {
  ...
  *length = d - output;
  if (input_consumed) *input_consumed += s - input;
  return output;
...
static int string_to_key(int id, char *salt, size_t saltlen, int iter,
  ...
  p = buf_i;
  for (i = 0; i < 64; i++) *p++ = salt[i % saltlen];
  for (i = j = 0; i < 64; i += 2) {
    *p++ = 0;
    *p++ = pw[j];
  ...
  for (;;) {
    ...
    /* need more bytes. */
    for (i = 0; i < 64; i++) buf_b[i] = hash[i % 20];
...
static int set_key_iv(gcry_cipher_hd_t chd, char *salt, size_t saltlen,
                      int iter, const char *pw, int keybytes) {
  unsigned char keybuf[24];
  int rc;

  assert(keybytes == 5 || keybytes == 24);
  if (string_to_key(1, salt, saltlen, iter, pw, keybytes, keybuf)) return -1;
  ...
  if (string_to_key(2, salt, saltlen, iter, pw, 8, keybuf)) return -1;
...
static int set_key_iv_pbes2(gcry_cipher_hd_t chd, char *salt, size_t saltlen,
                            int iter, const void *iv, size_t ivlen,
                            const char *pw, int algo) {
  unsigned char *keybuf;
  size_t keylen;
  int rc;
  ...
  rc = gcry_kdf_derive(pw, strlen(pw), GCRY_KDF_PBKDF2, GCRY_MD_SHA1, salt,
                       saltlen, iter, keylen, keybuf);
  if (rc) {
...
static void crypt_block(unsigned char *buffer, size_t length, char *salt,
                        size_t saltlen, int iter, const void *iv, size_t ivlen,
...
/* Decrypt a block of data and try several encodings of the key.
   ...
   succeeded. */
static void decrypt_block(const void *ciphertext, unsigned char *plaintext,
                          size_t length, char *salt, size_t saltlen, int iter,
...
/* Return true if the decryption of an bag_encrypted_data object has
   likely succeeded.  */
static int bag_decrypted_data_p(const void *plaintext, size_t length) {
  struct tag_info ti;
  ...
  if (parse_tag(&p, &n, &ti)) return 0;
  if (ti.klasse || ti.tag != TAG_SEQUENCE) return 0;
  if (parse_tag(&p, &n, &ti)) return 0;
...
static int parse_bag_encrypted_data(
    const unsigned char *buffer, size_t length, int startoffset,
    size_t *r_consumed, const char *pw,
    void (*certcb)(void *, const unsigned char *, size_t), void *certcbarg,
    gcry_mpi_t **r_result, int *r_badpass) {
  struct tag_info ti;
  ...
  if (r_result) *r_result = NULL;
  where = "start";
  if (parse_tag(&p, &n, &ti)) goto bailout;
  if (ti.klasse != ASNCONTEXT || ti.tag) goto bailout;
  if (parse_tag(&p, &n, &ti)) goto bailout;
  if (ti.tag != TAG_SEQUENCE) goto bailout;

  where = "bag.encryptedData.version";
  if (parse_tag(&p, &n, &ti)) goto bailout;
  if (ti.tag != TAG_INTEGER || ti.length != 1 || *p != 0) goto bailout;
  p++;
  n--;
  if (parse_tag(&p, &n, &ti)) goto bailout;
  if (ti.tag != TAG_SEQUENCE) goto bailout;
  ...
  if (parse_tag(&p, &n, &ti)) goto bailout;
  if (ti.klasse || ti.tag != TAG_SEQUENCE) goto bailout;
  if (parse_tag(&p, &n, &ti)) goto bailout;
  ...
  if (is_pbes2) {
    ...
    if (ti.klasse || ti.tag != TAG_SEQUENCE) goto bailout;
    if (parse_tag(&p, &n, &ti)) goto bailout;
    if (!(!ti.klasse && ti.tag == TAG_OCTET_STRING && ti.length >= 8 &&
          ti.length < sizeof salt))
    ...
    n -= saltlen;
    if (parse_tag(&p, &n, &ti)) goto bailout;
    if (ti.klasse || ti.tag != TAG_INTEGER || !ti.length) goto bailout;
    for (iter = 0; ti.length; ti.length--) {
      iter <<= 8;
      iter |= (*p++) & 0xff;
      n--;
  ...
  if (ti.klasse == ASNCONTEXT && ti.tag == 0 && ti.is_constructed && ti.ndef) {
    ...
    if (!cram_buffer) goto bailout;
    p = p_start = cram_buffer;
    if (r_consumed) *r_consumed = consumed;
  ...
  decrypt_block(p, plain, ti.length, salt, saltlen, iter, iv, is_pbes2 ? 16 : 0,
                pw,
  ...
  while (n) {
    ...
    if (parse_tag(&p, &n, &ti)) goto bailout;
    if (ti.klasse || ti.tag != TAG_OBJECT_ID) goto bailout;
    ...
    if (iscrlbag) {
      ...
      if (len < ti.length) goto bailout;
      len -= ti.length;
      if (n < len) goto bailout;
      p += len;
      n -= len;
      ...
      if (ti.klasse != ASNCONTEXT || ti.tag) goto bailout;
      if (parse_tag(&p, &n, &ti)) goto bailout;
      if (ti.klasse || ti.tag != TAG_OCTET_STRING || ti.ndef) goto bailout;

      /* Return the certificate. */
      if (certcb) certcb(certcbarg, p, ti.length);
    ...
    if (n) {
      ...
      if (!ti.klasse && ti.tag == TAG_SEQUENCE)
        ; /* No attributes. */
      else if (!ti.klasse && ti.tag == TAG_SET &&
               !ti.ndef) { /* The optional SET. */
        p += ti.length;
        n -= ti.length;
        if (n <= (is_pbes2 ? 16 : 8)) n = 0;
        if (n && parse_tag(&p, &n, &ti)) goto bailout;
      } else
        goto bailout;
    }
  }
...
bailout:
  if (result) {
    ...
    for (i = 0; result[i]; i++) gcry_mpi_release(result[i]);
    gcry_free(result);
  }
  if (r_consumed) *r_consumed = consumed;
  gcry_free(plain);
  ...
  log_error("encryptedData error at \"%s\", offset %u\n", where,
            (unsigned int)((p - p_start) + startoffset));
  if (bad_pass) {
...
/* Return true if the decryption of a bag_data object has likely
   succeeded.  */
static int bag_data_p(const void *plaintext, size_t length) {
  struct tag_info ti;
  const unsigned char *p = (const unsigned char *)plaintext;
  size_t n = length;
  ...
  if (parse_tag(&p, &n, &ti) || ti.klasse || ti.tag != TAG_SEQUENCE) return 0;
  if (parse_tag(&p, &n, &ti) || ti.klasse || ti.tag != TAG_INTEGER ||
      ti.length != 1 || *p)
    return 0;
...
static gcry_mpi_t *parse_bag_data(const unsigned char *buffer, size_t length,
  ...
  if (ti.klasse != ASNCONTEXT || ti.tag) goto bailout;
  if (parse_tag(&p, &n, &ti)) goto bailout;
  if (ti.klasse || ti.tag != TAG_OCTET_STRING) goto bailout;
  ...
  if (parse_tag(&p, &n, &ti)) goto bailout;
  if (ti.klasse || ti.tag != TAG_OCTET_STRING || !ti.length) goto bailout;

  log_info("%lu bytes of %s encrypted text\n", ti.length,
           is_pbes2 ? "AES128" : "3DES");
  ...
  decrypt_block(p, plain, ti.length, salt, saltlen, iter, iv, is_pbes2 ? 16 : 0,
                pw, is_pbes2 ? GCRY_CIPHER_AES128 : GCRY_CIPHER_3DES,
                bag_data_p);
  n = ti.length;
  startoffset = 0;
  p_start = p = plain;
  ...
  for (result_count = 0; len && result_count < 9;) {
    ...
    if (!result_count && ti.length == 1 && !*p)
      ...
      rc = gcry_mpi_scan(result + result_count, GCRYMPI_FMT_USG, p, ti.length,
                         NULL);
      if (rc) {
        log_error("error parsing key parameter: %s\n", gpg_strerror(rc));
        goto bailout;
...
bailout:
  ...
  log_error("data error at \"%s\", offset %u\n", where,
            (unsigned int)((p - buffer) + startoffset));
  if (r_consumed) *r_consumed = consumed;
  return NULL;
}
...
gcry_mpi_t *p12_parse(const unsigned char *buffer, size_t length,
  ...
  *r_badpass = 0;
  ...
  where = "pfxVersion";
  if (parse_tag(&p, &n, &ti)) goto bailout;
  if (ti.tag != TAG_INTEGER || ti.length != 1 || *p != 3) goto bailout;
  p++;
  n--;
  ...
  if (ti.klasse != ASNCONTEXT || ti.tag) goto bailout;
  if (parse_tag(&p, &n, &ti)) goto bailout;
  if (ti.klasse != UNIVERSAL || ti.tag != TAG_OCTET_STRING) goto bailout;
  ...
  where = "bags";
  if (parse_tag(&p, &n, &ti)) goto bailout;
  if (ti.klasse != UNIVERSAL || ti.tag != TAG_SEQUENCE) goto bailout;
  bagseqndef = ti.ndef;
  ...
  while (bagseqlength || bagseqndef) {
    ...
    if (!bagseqndef) {
      if (bagseqlength < ti.nhdr) goto bailout;
    ...
    if (ti.tag == TAG_OBJECT_ID && ti.length == DIM(oid_encryptedData) &&
      ...
      if (!lenndef) len -= DIM(oid_encryptedData);
      where = "bag.encryptedData";
      if (parse_bag_encrypted_data(p, n, (p - p_start), &consumed, pw, certcb,
                                   certcbarg, result ? NULL : &result,
                                   r_badpass))
        goto bailout;
      if (lenndef) len += consumed;
...
/* Create the final PKCS-12 object from the sequences contained in
   ...
   object. */
static unsigned char *create_final(struct buffer_s *sequences, const char *pw,
                                   size_t *r_length) {
  int i;
  size_t needed = 0;
  size_t len[8], n;
  ...
  /* 7. All the buffers. */
  for (i = 0; sequences[i].buffer; i++) needed += sequences[i].length;
...
static unsigned char *build_key_sequence(gcry_mpi_t *kparms, int mode,
                                         size_t *r_length) {
  int rc, i;
  size_t needed, n;
...
static unsigned char *build_key_bag(unsigned char *buffer, size_t buflen,
                                    char *salt, const unsigned char *sha1hash,
                                    const char *keyidstr, size_t *r_length) {
  ...
  if (sha1hash) needed += DIM(data_attrtemplate) + 20;
  ...
  p = store_tag_length(p, TAG_OBJECT_ID, DIM(oid_pkcs_12_pkcs_8ShroudedKeyBag));
...
static unsigned char *build_cert_bag(unsigned char *buffer, size_t buflen,
                                     char *salt, size_t *r_length) {
  size_t len[9], needed;
...
static unsigned char *build_cert_sequence(const unsigned char *buffer,
                                          size_t buflen,
  ...
  p = store_tag_length(p, TAG_OBJECT_ID, DIM(oid_x509Certificate_for_pkcs_12));
...
unsigned char *p12_build(gcry_mpi_t *kparms, const void *cert, size_t certlen,
  ...
  *keyidstr = 0;
  ...
  if (cert && certlen) {
    ...
    gcry_free(buffer);
    buffer = NULL;
    if (!seqlist[seqlistidx].buffer) goto failure;
    seqlistidx++;
  }
...
unsigned char *p12_raw_build(gcry_mpi_t *kparms, int rawmode,
                             size_t *r_length) {
  unsigned char *buffer;
  size_t buflen;
...
static void cert_cb(void *opaque, const unsigned char *cert, size_t certlen) {
...
/*
Local Variables:
compile-command: "gcc -Wall -O0 -g -DTEST=1 -o minip12 minip12.c
