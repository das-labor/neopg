...
static void init_membuf(struct membuf *mb, int initiallen) {
  mb->len = 0;
  mb->size = initiallen;
  mb->out_of_core = 0;
  mb->buf = (char *)xtrymalloc(initiallen);
  if (!mb->buf) mb->out_of_core = 1;
}

static void put_membuf(struct membuf *mb, const void *buf, size_t len) {
  if (mb->out_of_core) return;
...
static void *get_membuf(struct membuf *mb, size_t *len) {
  char *p;
...
static gpg_error_t start_dirmngr_ext(ctrl_t ctrl, assuan_context_t *ctx_r) {
  gpg_error_t err;
  assuan_context_t ctx;

  if (opt.disable_dirmngr || ctrl->offline) return GPG_ERR_NO_DIRMNGR;
...
static int start_dirmngr(ctrl_t ctrl) {
  gpg_error_t err;
  ...
  if (!dirmngr_ctx) dirmngr_ctx_locked = 0;
  return err;
}

static void release_dirmngr(ctrl_t ctrl) {
  (void)ctrl;
...
static int start_dirmngr2(ctrl_t ctrl) {
  gpg_error_t err;
  ...
  if (!dirmngr2_ctx) dirmngr2_ctx_locked = 0;
  return err;
}

static void release_dirmngr2(ctrl_t ctrl) {
  (void)ctrl;
...
static gpg_error_t inq_certificate(void *opaque, const char *line) {
  ...
  if ((s = has_leading_keyword(line, "SENDCERT"))) {
    ...
    for (s = line, n = 0; hexdigitp(s); s++, n++)
      ;
    if (*s || n != 40) return GPG_ERR_ASS_PARAMETER;
  ...
  if (!*line) { /* Send the current certificate. */
    ...
    log_error(
        "sending specific issuer certificate back "
        "is not yet implemented\n");
    rc = GPG_ERR_ASS_UNKNOWN_INQUIRE;
  } else { /* Send the given certificate. */
    int err;
    ksba_cert_t cert;
...
static gpg_error_t isvalid_status_cb(void *opaque, const char *line) {
 ...
 */
int gpgsm_dirmngr_isvalid(ctrl_t ctrl, ksba_cert_t cert,
                          ksba_cert_t issuer_cert, int use_ocsp) {
  ...
  if (!did_options) {
    if (opt.force_crl_refresh)
      assuan_transact(dirmngr_ctx, "OPTION force-crl-refresh=1", NULL, NULL,
                      NULL, NULL, NULL, NULL);
    did_options = 1;
  }
  ...
  if (!rc && stparm.seen) {
    ...
    if (stparm.seen != 1) {
      ...
      if (get_cached_cert(dirmngr_ctx, stparm.fpr, &rspcert)) {
        ...
        kh = sm_keydb_new();
        if (!kh) rc = GPG_ERR_ENOMEM;
        if (!rc) rc = sm_keydb_search_fpr(ctrl, kh, stparm.fpr);
        if (!rc) rc = sm_keydb_get_cert(kh, &rspcert);
        if (rc) {
          log_error(
              "unable to find the certificate used "
              "by the dirmngr: %s\n",
              gpg_strerror(rc));
          rc = GPG_ERR_INV_CRL;
        }
...
/* Lookup helpers*/
static gpg_error_t lookup_cb(void *opaque, const void *buffer, size_t length) {
...
static char *pattern_from_strlist(strlist_t names) {
  strlist_t sl;
  int n;
  const char *s;
  char *pattern, *p;
...
static gpg_error_t lookup_status_cb(void *opaque, const char *line) {
  struct lookup_parm_s *parm = (lookup_parm_s *)opaque;
  const char *s;
...
int gpgsm_dirmngr_lookup(ctrl_t ctrl, strlist_t names, int cache_only,
  ...
  snprintf(line, DIM(line), "LOOKUP%s %s", cache_only ? " --cache-only" : "",
           pattern);
  xfree(pattern);
  ...
  rc = assuan_transact(ctx, line, lookup_cb, &parm, NULL, NULL,
                       lookup_status_cb, &parm);
...
static gpg_error_t get_cached_cert_data_cb(void *opaque, const void *buffer,
                                           size_t length) {
  struct membuf *mb = (membuf *)opaque;

  if (buffer) put_membuf(mb, buffer, length);
  return 0;
}
...
static gpg_error_t get_cached_cert(assuan_context_t ctx,
  ...
  *r_cert = NULL;
  ...
  init_membuf(&mb, 4096);
  err = assuan_transact(ctx, line, get_cached_cert_data_cb, &mb, NULL, NULL,
                        NULL, NULL);
...
static gpg_error_t run_command_cb(void *opaque, const void *buffer,
                                  size_t length) {
  (void)opaque;
...
static gpg_error_t run_command_inq_cb(void *opaque, const char *line) {
  struct run_command_parm_s *parm = (run_command_parm_s *)opaque;
  const char *s;
  int rc = 0;

  if ((s = has_leading_keyword(line,
                               "SENDCERT"))) { /* send the given certificate */
    int err;
    ksba_cert_t cert;
    const unsigned char *der;
    size_t derlen;
    ...
    line = s;
    if (!*line) return GPG_ERR_ASS_PARAMETER;
  ...
  } else if ((s = has_leading_keyword(line,
                                      "PRINTINFO"))) { /* Simply show the
                                                          message given in the
                                                          argument. */
    line = s;
    log_info("dirmngr: %s\n", line);
  } else {
...
static gpg_error_t run_command_status_cb(void *opaque, const char *line) {
...
int gpgsm_dirmngr_run_command(ctrl_t ctrl, const char *command, int argc,
                              char **argv) {
  int rc;
  int i;
  const char *s;
  ...
  *p = 0;
  ...
  rc = assuan_transact(dirmngr_ctx, line, run_command_cb, NULL,
                       run_command_inq_cb, &parm, run_command_status_cb, ctrl);
  xfree(line);
  log_info("response of dirmngr: %s\n", rc ? gpg_strerror(rc) : "okay");
  release_dirmngr(ctrl);
  return rc;
