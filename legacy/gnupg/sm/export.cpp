...
static gpg_error_t export_p12(ctrl_t ctrl, const unsigned char *certimg,
                              size_t certimglen, const char *prompt,
                              const char *keygrip, int rawmode, void **r_result,
                              size_t *r_resultlen);

/* Create a table used to indetify duplicated certificates. */
static duptable_t *create_duptable(void) {
  return (duptable_s **)xtrycalloc(DUPTABLE_SIZE, sizeof(duptable_t));
}

static void destroy_duptable(duptable_t *table) {
  int idx;
  duptable_t t, t2;
...
static gpg_error_t insert_duptable(duptable_t *table, unsigned char *fpr,
                                   int *exists) {
  size_t idx;
  duptable_t t;
...
#error cannot handle a table larger than 16 bits or smaller than 8 bits
  ...
  for (t = table[idx]; t; t = t->next)
    if (!memcmp(t->fpr, fpr + 1, 19)) break;
  if (t) {
    *exists = 1;
    return 0;
  }
  ...
  if (!t) return gpg_error_from_syserror();
...
void gpgsm_export(ctrl_t ctrl, strlist_t names, estream_t stream) {
  ...
  if (names && ndesc) {
    for (i = 0; (i < ndesc && (desc[i].mode == KEYDB_SEARCH_MODE_FPR ||
         ...
         i++)
      ;
    if (i == ndesc) sm_keydb_set_ephemeral(hd, 1);
  }
...
void gpgsm_p12_export(ctrl_t ctrl, const char *name, estream_t stream,
  ...
  err = export_p12(ctrl, image, imagelen, prompt, keygrip, rawmode, &data,
                   &datalen);
  xfree(prompt);
  if (err) goto leave;
...
static gcry_mpi_t *sexp_to_kparms(gcry_sexp_t sexp) {
  gcry_sexp_t list, l2;
  const char *name;
  const char *s;
  size_t n;
  ...
  for (idx = 0, s = elems; *s; s++, idx++) {
    ...
    {
      for (idx = 0; array[idx]; idx++) gcry_mpi_release(array[idx]);
      xfree(array);
      gcry_sexp_release(list);
      return NULL;
    }
...
static gpg_error_t export_p12(ctrl_t ctrl, const unsigned char *certimg,
  ...
  *r_result = NULL;
  ...
  err = gpgsm_agent_export_key(ctrl, keygrip, prompt, &key, &keylen);
  if (err) goto leave;
...
leave:
  ...
  gcry_sexp_release(s_skey);
  if (kparms) {
    for (i = 0; kparms[i]; i++) gcry_mpi_release(kparms[i]);
    xfree(kparms);
  }
