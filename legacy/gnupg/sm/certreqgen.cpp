...
static int proc_parameters(ctrl_t ctrl, struct para_data_s *para,
                           estream_t out_fp, struct reqgen_ctrl_s *outctrl);
static int create_request(ctrl_t ctrl, struct para_data_s *para,
                          const char *carddirect, ksba_const_sexp_t public_x,
                          ksba_const_sexp_t sigkey, ksba_writer_t writer);
...
static struct para_data_s *get_parameter(struct para_data_s *para,
                                         enum para_name key, int seq) {
  struct para_data_s *r;

  for (r = para; r; r = r->next)
    if (r->key == key && !seq--) return r;
  return NULL;
}
...
static int get_parameter_algo(struct para_data_s *para, enum para_name key) {
  ...
  if (!r) return -1;
  if (digitp(r->u.value)) return atoi(r->u.value);
...
static int parse_parameter_usage(struct para_data_s *para, enum para_name key) {
  struct para_data_s *r = get_parameter(para, key, 0);
  char *p, *pn;
  unsigned int use;
  ...
  while ((p = gpg_strsep(&pn, " \t,"))) {
    ...
    else if (!ascii_strcasecmp(p, "encrypt") || !ascii_strcasecmp(p, "encr"))
...
static unsigned int get_parameter_uint(struct para_data_s *para,
  ...
  if (r->key == pKEYUSAGE) return r->u.usage;
...
static int read_parameters(ctrl_t ctrl, estream_t fp, estream_t out_fp) {
  static struct {
    const char *name;
    enum para_name key;
    int allow_dups;
  ...
  while (es_fgets(line, DIM(line) - 1, fp)) {
    ...
    for (i = 0;
         (keywords[i].name && ascii_strcasecmp(keywords[i].name, keyword)); i++)
      ;
    if (!keywords[i].name) {
      err = "unknown keyword";
      break;
...
static int has_invalid_email_chars(const char *s) {
  int at_seen = 0;
...
static int proc_parameters(ctrl_t ctrl, struct para_data_s *para,
  ...
  if (!i && (s = get_parameter_value(para, pKEYTYPE, 0)) && *s) {
    ...
    if (!strncmp(s, "card:", 5) && s[5]) cardkeyid = xtrystrdup(s + 5);
  }
  ...
  for (seq = 0; (s = get_parameter_value(para, pNAMEEMAIL, seq)); seq++) {
    if (has_invalid_email_chars(s) || *s == '@' || s[strlen(s) - 1] == '@' ||
        s[strlen(s) - 1] == '.' || strstr(s, "..")) {
  ...
  /* Check the optional hash-algo.  */
  {
    ...
    if (string &&
        !((mdalgo = gcry_md_map_name(string)) &&
          (mdalgo == GCRY_MD_SHA1 || mdalgo == GCRY_MD_SHA256 ||
           mdalgo == GCRY_MD_SHA384 || mdalgo == GCRY_MD_SHA512))) {
  ...
  if (cardkeyid) /* Take the key from the current smart card. */
  {
    ...
    if (rc) {
      ...
      log_error(_("line %d: error reading key '%s' from card: %s\n"), r->lnr,
                cardkeyid, gpg_strerror(rc));
      xfree(sigkey);
      xfree(cardkeyid);
      return rc;
    }
  } else if ((s = get_parameter_value(para, pKEYGRIP,
                                      0))) /* Use existing key.*/
  {
  ...
  } else if (!outctrl->dryrun) /* Generate new key.  */
  {
    ...
    snprintf((char *)keyparms, DIM(keyparms), "(6:genkey(3:rsa(5:nbits%d:%s)))",
...
static int create_request(ctrl_t ctrl, struct para_data_s *para,
  ...
  if (DBG_HASHING) gcry_md_debug(md, "cr.cri");
  ...
  if (string) {
    ...
    /* Figure out the signing algorithm.  If no sigkey has been
       ...
       certificate. */
    if (!sigkey) sigkey = public_x;

    {
      unsigned char *siginfo;

      err = transform_sigval(sigkey, gcry_sexp_canon_len(sigkey, 0, NULL, NULL),
                             mdalgo, &siginfo, NULL);
      if (!err) {
    ...
    if (string) {
      ...
      hexbuf[3] = len;
      err = ksba_certreq_add_extension(cr, oidstr_authorityKeyIdentifier, 0,
                                       hexbuf, 4 + len);
      xfree(hexbuf);
      if (err) {
    ...
    for (seq = 0; (string = get_parameter_value(para, pEXTENSION, seq));
         seq++) {
      char *hexbuf;
      char *oidstr;
      int crit = 0;
      ...
      for (p = hexbuf, len = 0; p[0] && p[1]; p += 2)
        ((unsigned char *)hexbuf)[len++] = xtoi_2(p);
      err = ksba_certreq_add_extension(cr, oidstr, crit, hexbuf, len);
      xfree(oidstr);
      xfree(hexbuf);
    }
  } else
  ...
  do {
    ...
    if (stopreason == KSBA_SR_NEED_SIG) {
      ...
      if (carddirect)
        rc = gpgsm_scd_pksign(ctrl, carddirect, NULL, gcry_md_read(md, mdalgo),
