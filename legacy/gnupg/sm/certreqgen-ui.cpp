...
static void ask_mb_lines(membuf_t *mb, const char *prefix) {
  char *answer = NULL;
...
void store_key_value_lf(membuf_t *mb, const char *key, const char *value) {
...
/* Helper tp store a membuf create by mb_ask_lines into MB.  Returns
   -1 on error. */
int store_mb_lines(membuf_t *mb, membuf_t *lines) {
  char *p;
...
/* Chech whether we have a key for the key with HEXGRIP.  Returns NULL
   ...
   etc..).  */
static const char *check_keygrip(ctrl_t ctrl, const char *hexgrip) {
  gpg_error_t err;
...
/* This function is used to create a certificate request from the
   ...
   re-implementation.  */
void gpgsm_gencertreq_tty(ctrl_t ctrl, estream_t output_stream) {
  gpg_error_t err;
  char *answer;
  int selection;
  estream_t fp = NULL;
...
again:
  ...
  selfsigned =
      tty_get_answer_is_yes(_("Create self-signed certificate? (y/N) "));
  ...
  if (keygrip) store_key_value_lf(&mb_result, "Key-Grip: ", keygrip);
  store_key_value_lf(&mb_result, "Key-Usage: ", keyusage);
  if (selfsigned) store_key_value_lf(&mb_result, "Serial: ", "random");
  store_key_value_lf(&mb_result, "Name-DN: ", subject_name);
  if (store_mb_lines(&mb_result, &mb_email)) goto mem_error;
  if (store_mb_lines(&mb_result, &mb_dns)) goto mem_error;
  if (store_mb_lines(&mb_result, &mb_uri)) goto mem_error;
  ...
  if (!tty_get_answer_is_yes("Proceed with creation? (y/N) ")) goto leave;
