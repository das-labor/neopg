...
static int is_root_cert(ksba_cert_t cert, const char *issuerdn,
...
static int already_asked_marktrusted(ksba_cert_t cert) {
  unsigned char fpr[20];
  ...
  for (r = marktrusted_info; r; r = r->next)
    if (!memcmp(r->fpr, fpr, 20)) return 1;
  return 0;
}

/* Flag certificate CERT as already asked whether it shall be marked
   as trusted.  */
static void set_already_asked_marktrusted(ksba_cert_t cert) {
  unsigned char fpr[20];
  struct marktrusted_info_s *r;

  gpgsm_get_fingerprint(cert, GCRY_MD_SHA1, fpr, NULL);
  for (r = marktrusted_info; r; r = r->next)
    if (!memcmp(r->fpr, fpr, 20)) return; /* Already marked. */
...
static void do_list(int is_error, int listmode, estream_t fp,
                    const char *format, ...) {
  va_list arg_ptr;
...
static int compare_certs(ksba_cert_t a, ksba_cert_t b) {
...
static int has_validation_model_chain(ksba_cert_t cert, int listmode,
                                      estream_t listfp) {
  gpg_error_t err;
  int idx, yes;
  const char *oid;
  ...
  for (idx = 0;
       !(err = ksba_cert_get_extension(cert, idx, &oid, NULL, &off, &derlen));
       idx++)
    if (!strcmp(oid, "1.3.6.1.4.1.8301.3.5")) break;
  if (err) return 0; /* Not found.  */
  der = ksba_cert_get_image(cert, NULL);
  if (!der) {
    err = GPG_ERR_INV_OBJ; /* Oops  */
    goto leave;
  }
  der += off;

  err = parse_ber_header(&der, &derlen, &klasse, &tag, &constructed, &ndef,
                         &objlen, &hdrlen);
  if (!err && (objlen > derlen || tag != TAG_SEQUENCE)) err = GPG_ERR_INV_OBJ;
  if (err) goto leave;
  derlen = objlen;
  err = parse_ber_header(&der, &derlen, &klasse, &tag, &constructed, &ndef,
                         &objlen, &hdrlen);
  if (!err && (objlen > derlen || tag != TAG_OBJECT_ID)) err = GPG_ERR_INV_OBJ;
  if (err) goto leave;
  ...
  if (opt.verbose)
    do_list(0, listmode, listfp,
            ...
            !strcmp(oidbuf, "1.3.6.1.4.1.8301.3.5.1")
                ? _("chain")
...
static int unknown_criticals(ksba_cert_t cert, int listmode, estream_t fp) {
  ...
  for (idx = 0;
       !(err = ksba_cert_get_extension(cert, idx, &oid, &crit, NULL, NULL));
       idx++) {
    if (!crit) continue;
    ...
    if (unsupported && opt.ignored_cert_extensions) {
      for (sl = opt.ignored_cert_extensions; sl && strcmp(sl->d, oid);
           sl = sl->next)
        ;
      if (sl) unsupported = 0;
    }
    if (unsupported) {
      do_list(1, listmode, fp,
              _("critical certificate extension %s is not supported"), oid);
      rc = GPG_ERR_UNSUPPORTED_CERT;
    }
  }
  /* We ignore the error codes EOF as well as no-value. The later will
     occur for certificates with no extensions at all. */
  if (err && err != GPG_ERR_EOF && err != GPG_ERR_NO_VALUE) rc = err;
...
static int allowed_ca(ctrl_t ctrl, ksba_cert_t cert, int *chainlen,
                      int listmode, estream_t fp) {
  gpg_error_t err;
  int flag;
...
static int check_cert_policy(ksba_cert_t cert, int listmode, estream_t fplist) {
  gpg_error_t err;
  char *policies;
  FILE *fp;
  int any_critical;
  ...
  if (err == GPG_ERR_NO_DATA) return 0; /* No policy given. */
  if (err) return err;
  ...
  if (!opt.policy_file) {
    ...
    if (any_critical) {
      ...
      return GPG_ERR_NO_POLICY_MATCH;
    }
    return 0;
  }
  ...
  if (!fp) {
    ...
    do_list(1, listmode, fplist, _("certificate policy not allowed"));
    return GPG_ERR_NO_POLICY_MATCH;
  }
  ...
  for (;;) {
    ...
    do {
      ...
      if (!*line || line[strlen(line) - 1] != '\n') {
        ...
        fclose(fp);
        xfree(policies);
        return *line ? GPG_ERR_LINE_TOO_LONG : GPG_ERR_INCOMPLETE_LINE;
      }
    ...
    *p = 0; /* strip the rest of the line */
    ...
    for (haystack = policies; (p = strstr(haystack, allowed));
         haystack = p + 1) {
      if (!(p == policies || p[-1] == '\n'))
        continue; /* Does not match the begin of a line. */
      if (p[strlen(allowed)] != ':') continue; /* The length does not match. */
...
static int find_up_search_by_keyid(ctrl_t ctrl, KEYDB_HANDLE kh,
...
static void find_up_store_certs_cb(void *cb_value, ksba_cert_t cert) {
...
/* Helper for find_up().  Locate the certificate for ISSUER using an
   ...
   the AKI or NULL.  */
static int find_up_external(ctrl_t ctrl, KEYDB_HANDLE kh, const char *issuer,
                            ksba_sexp_t keyid) {
  int rc;
  ...
  if (opt.verbose) log_info(_("looking up issuer at external location\n"));
...
/* Helper for find_up().  Ask the dirmngr for the certificate for
   ...
   in the ephemeral DB.  */
static int find_up_dirmngr(ctrl_t ctrl, KEYDB_HANDLE kh, ksba_sexp_t serialno,
                           const char *issuer, int subject_mode) {
  int rc;
  strlist_t names = NULL;
  struct find_up_store_certs_s find_up_store_certs_parm;
  char *pattern;
  ...
  if (opt.verbose) log_info(_("looking up issuer from the Dirmngr cache\n"));
  if (subject_mode) {
    pattern = (char *)xtrymalloc(strlen(issuer) + 2);
    if (pattern) strcpy(stpcpy(pattern, "/"), issuer);
  } else if (serialno)
...
static int find_up(ctrl_t ctrl, KEYDB_HANDLE kh, ksba_cert_t cert,
                   const char *issuer, int find_next) {
  ksba_name_t authid;
  ksba_sexp_t authidno;
  ksba_sexp_t keyid;
  int rc = -1;

  if (DBG_X509) log_debug("looking for parent certificate\n");
  if (!ksba_cert_get_auth_key_id(cert, &keyid, &authid, &authidno)) {
    const char *s = ksba_name_enum(authid, 0);
    if (s && *authidno) {
      rc = sm_keydb_search_issuer_sn(ctrl, kh, s, authidno);
      if (rc) sm_keydb_search_reset(kh);

      if (!rc && DBG_X509) log_debug("  found via authid and sn+issuer\n");
      ...
      if (rc == -1 && !find_next) find_up_dirmngr(ctrl, kh, authidno, s, 0);
      ...
      if (rc) rc = -1; /* Need to make sure to have this error code. */
    }
    ...
    if (rc == -1 && keyid && !find_next) {
      ...
      rc = find_up_search_by_keyid(ctrl, kh, issuer, keyid);
      if (!rc && DBG_X509) log_debug("  found via authid and keyid\n");
      if (rc) {
        int old = sm_keydb_set_ephemeral(kh, 1);
        if (!old) rc = find_up_search_by_keyid(ctrl, kh, issuer, keyid);
        if (!rc && DBG_X509)
  ...
  if (rc == -1 && !find_next) {
    ...
    if (!rc && DBG_X509) log_debug("  found via issuer\n");
  }

  /* Still not found.  If enabled, try an external lookup.  */
  if (rc == -1 && opt.auto_issuer_key_retrieve && !find_next) {
    rc = find_up_external(ctrl, kh, issuer, NULL);
    if (!rc && DBG_X509) log_debug("  found via issuer and external lookup\n");
  }
...
int gpgsm_walk_cert_chain(ctrl_t ctrl, ksba_cert_t start, ksba_cert_t *r_next) {
  int rc = 0;
  char *issuer = NULL;
  char *subject = NULL;
...
static int is_root_cert(ksba_cert_t cert, const char *issuerdn,
  ...
  if (!issuerdn || !subjectdn) return 0; /* No.  */

  if (strcmp(issuerdn, subjectdn)) return 0; /* No.  */
  ...
  if (ak_name_str && !strcmp(ak_name_str, issuerdn) &&
...
static gpg_error_t is_cert_still_valid(ctrl_t ctrl, int force_ocsp, int lm,
                                       estream_t fp, ksba_cert_t subject_cert,
                                       ksba_cert_t issuer_cert,
                                       int *any_revoked, int *any_no_crl,
                                       int *any_crl_too_old) {
  gpg_error_t err;
  ...
  err = gpgsm_dirmngr_isvalid(ctrl, subject_cert, issuer_cert,
                              force_ocsp ? 2 : !!ctrl->use_ocsp);
  if (err) {
    if (!lm) gpgsm_cert_log_name(NULL, subject_cert);
    switch (err) {
      ...
      default:
        do_list(1, lm, fp, _("checking the CRL failed: %s"), gpg_strerror(err));
        return err;
    }
  }
  return 0;
...
static gpg_error_t check_validity_period(ksba_isotime_t current_time,
                                         ksba_cert_t subject_cert,
                                         ksba_isotime_t exptime, int listmode,
                                         estream_t listfp, int depth) {
  gpg_error_t err;
  ksba_isotime_t not_before, not_after;

  err = ksba_cert_get_validity(subject_cert, 0, not_before);
  if (!err) err = ksba_cert_get_validity(subject_cert, 1, not_after);
  if (err) {
    do_list(1, listmode, listfp, _("certificate with invalid validity: %s"),
            gpg_strerror(err));
  ...
  if (*not_before && strcmp(current_time, not_before) < 0) {
    do_list(1, listmode, listfp,
            depth == 0
                ...
                : depth == -1
                      ? _("root certificate not yet valid")
                      :
  ...
  if (*not_after && strcmp(current_time, not_after) > 0) {
    do_list(opt.ignore_expiration ? 0 : 1, listmode, listfp,
            depth == 0
                ...
                : depth == -1
                      ? _("root certificate has expired")
                      :
...
static gpg_error_t check_validity_period_cm(ksba_isotime_t current_time,
  ...
  if (strcmp(not_before, not_after) > 0) {
    do_list(1, listmode, listfp, _("certificate with invalid validity"));
    log_info("  (valid from ");
    dump_isotime(not_before);
  ...
  if (*check_time && (strcmp(check_time, not_before) < 0 ||
    ...
    do_list(
        opt.ignore_expiration ? 0 : 1, listmode, listfp,
        depth == 0
...
static int ask_marktrusted(ctrl_t ctrl, ksba_cert_t cert, int listmode) {
  ...
  if (!rc) {
    ...
    log_info(
        "after checking the fingerprint, you may want "
        "to add it manually to the list of trusted certificates.\n");
  }

  if (rc == GPG_ERR_NOT_SUPPORTED) {
    if (!no_more_questions)
      log_info(
          _("interactive marking as trusted "
...
*/
static int do_validate_chain(ctrl_t ctrl, ksba_cert_t cert,
                             const ksba_isotime_t checktime_arg,
                             ksba_isotime_t r_exptime, int listmode,
                             estream_t listfp, unsigned int flags,
  ...
  *exptime = 0;
  ...
  if (DBG_X509 && !listmode) gpgsm_dump_cert("target", cert);
  ...
  for (;;) {
    ...
    if (is_root) {
      ...
      if (gpgsm_cert_has_well_known_private_key(subject_cert)) {
        ...
        istrusted_rc =
            ((flags & VALIDATE_FLAG_STEED) ? 0 : GPG_ERR_NOT_TRUSTED);
      } else
        istrusted_rc =
            gpgsm_agent_istrusted(ctrl, subject_cert, NULL, rootca_flags);
    ...
    if ((flags & VALIDATE_FLAG_CHAIN_MODEL))
      ...
      rc = check_validity_period(current_time, subject_cert, exptime, listmode,
    ...
    if (is_root) {
      if (!istrusted_rc)
        ...
        rc = depth ? GPG_ERR_BAD_CERT_CHAIN : GPG_ERR_BAD_CERT;
        goto leave;
      }
      ...
      if ((flags & VALIDATE_FLAG_NO_DIRMNGR))
        ...
        rc = is_cert_still_valid(ctrl, (flags & VALIDATE_FLAG_CHAIN_MODEL),
                                 listmode, listfp, subject_cert, subject_cert,
  ...
  try_another_cert:
      ...
      if (rc == GPG_ERR_BAD_SIGNATURE) {
        ...
        if (!rc) {
          ...
          if (rc || !compare_certs(issuer_cert, tmp_cert)) {
            ...
            do_list(0, listmode, listfp, _("found another possible matching "
      ...
      if (rc) {
        ...
        if (is_root) {
          if (gpgsm_cert_has_well_known_private_key(issuer_cert)) {
            ...
            istrusted_rc =
                gpgsm_agent_istrusted(ctrl, issuer_cert, NULL, rootca_flags);
    ...
    if (!listmode) {
      ...
      if (rc) {
        char numbuf[50];
        sprintf(numbuf, "%d", rc);
        gpgsm_status2(ctrl, STATUS_ERROR, "certcert.issuer.keyusage", numbuf,
                      NULL);
        goto leave;
      }
    }
    ...
    if ((flags & VALIDATE_FLAG_NO_DIRMNGR))
      ...
      rc = is_cert_still_valid(ctrl, (flags & VALIDATE_FLAG_CHAIN_MODEL),
                               listmode, listfp, subject_cert, issuer_cert,
                               &any_revoked, &any_no_crl, &any_crl_too_old);
    if (rc) goto leave;

    if (opt.verbose && !listmode)
      log_info(depth == 0
                   ? _("certificate is good\n")
  ...
  if (!rc) { /* If we encountered an error somewhere during the checks, set
    ...
    if (any_revoked)
      ...
      rc = GPG_ERR_NO_CRL_KNOWN;
    else if (any_crl_too_old)
      rc = GPG_ERR_CRL_TOO_OLD;
    else if (any_no_policy_match)
      rc = GPG_ERR_NO_POLICY_MATCH;
  }
...
leave:
  ...
  if (chain && chain->is_root) {
    ...
    for (ci = chain; ci; ci = ci->next) {
      ...
      if (!ci->next && err == GPG_ERR_NOT_FOUND)
        ;
      else if (err)
  ...
  if (r_exptime) gnupg_copy_time(r_exptime, exptime);
  xfree(issuer);
  xfree(subject);
  sm_keydb_release(kh);
  while (chain) {
...
int gpgsm_validate_chain(ctrl_t ctrl, ksba_cert_t cert,
  ...
  if (!retflags) retflags = &dummy_retflags;
  ...
  *retflags = (flags & VALIDATE_FLAG_CHAIN_MODEL);
  ...
  if (opt.verbose)
    do_list(0, listmode, listfp, _("validation model used: %s"),
            (*retflags & VALIDATE_FLAG_STEED)
                ? "steed"
                : (*retflags & VALIDATE_FLAG_CHAIN_MODEL) ? _("chain")
                                                          : _("shell"));
...
int gpgsm_basic_cert_check(ctrl_t ctrl, ksba_cert_t cert) {
  ...
  if (is_root_cert(cert, issuer, subject)) {
    ...
    if (opt.verbose) log_info(_("certificate is good\n"));
  }
