...
void gpgsm_print_serial(estream_t fp, ksba_const_sexp_t sn) {
...
void gpgsm_dump_serial(ksba_const_sexp_t sn) {
  const char *p = (const char *)sn;
  unsigned long n;
  char *endp;
  ...
  if (!p)
    ...
    if (*p != ':')
      ...
      for (p++; n; n--, p++) log_printf("%02X", *(const unsigned char *)p);
    }
  }
}

char *gpgsm_format_serial(ksba_const_sexp_t sn) {
  const char *p = (const char *)sn;
  unsigned long n;
  char *endp;
  char *buffer;
  ...
  if (*p != '(') BUG(); /* Not a valid S-expression. */
...
void gpgsm_print_time(estream_t fp, ksba_isotime_t t) {
  if (!t || !*t)
    ...
    es_fprintf(fp, "%.4s-%.2s-%.2s %.2s:%.2s:%s", t, t + 4, t + 6, t + 9,
               t + 11, t + 13);
}
...
static const unsigned char *parse_dn_part(struct dn_array_s *array,
  ...
  static struct {
    const char *label;
    const char *oid;
  } label_map[] = {/* Warning: When adding new labels, make sure that the buffer
  ...
  if (*string == '#') { /* hexstring */
    ...
    n = s - string;
    if (!n || (n & 1)) return NULL; /* Empty or odd number of digits. */
    n /= 2;
...
static void print_dn_part(estream_t stream, struct dn_array_s *dn,
                          const char *key, int translate) {
  ...
  for (; dn->key; dn++) {
    if (!dn->done && !strcmp(dn->key, key)) {
      /* Forward to the last multi-valued RDN, so that we can
         ...
         RDNs. */
      while (dn->multivalued && dn[1].key) dn++;
    next:
      if (!dn->done && dn->value && *dn->value) {
        ...
        if (translate)
          ...
          es_write_sanitized(stream, dn->value, strlen(dn->value), "/", NULL);
      }
      dn->done = 1;
...
static void print_dn_parts(estream_t stream, struct dn_array_s *dn,
                           int translate) {
  const char *stdpart[] = {"CN", "OU", "O",     "STREET", "L",
                           "ST", "C",  "EMail", NULL};
  int i;

  for (i = 0; stdpart[i]; i++) print_dn_part(stream, dn, stdpart[i], translate);

  /* Now print the rest without any specific ordering */
  for (; dn->key; dn++) print_dn_part(stream, dn, dn->key, translate);
}
...
static void pretty_es_print_sexp(estream_t fp, const unsigned char *buf,
                                 size_t buflen) {
  size_t len;
  gcry_sexp_t sexp;
  char *result, *p;
...
void gpgsm_es_print_name2(estream_t fp, const char *name, int translate) {
...
void gpgsm_es_print_name(estream_t fp, const char *name) {
...
static gpgrt_ssize_t format_name_writer(void *cookie, const void *buffer,
                                        size_t size) {
