...
/* Return 0 if the cert is usable for encryption.  A MODE of 0 checks
   ...
   response signing. */
static int cert_usage_p(ksba_cert_t cert, int mode) {
  gpg_error_t err;
  unsigned int use;
  char *extkeyusages;
  ...
  if (mode == 5) {
    if (use != ~0 && (have_ocsp_signing || (use & (KSBA_KEYUSAGE_KEY_CERT_SIGN |
  ...
  if ((use & ((mode & 1) ? (KSBA_KEYUSAGE_KEY_ENCIPHERMENT |
...
int gpgsm_cert_use_sign_p(ksba_cert_t cert) { return cert_usage_p(cert, 0); }

/* Return 0 if the cert is usable for encryption */
int gpgsm_cert_use_encrypt_p(ksba_cert_t cert) { return cert_usage_p(cert, 1); }

int gpgsm_cert_use_verify_p(ksba_cert_t cert) { return cert_usage_p(cert, 2); }

int gpgsm_cert_use_decrypt_p(ksba_cert_t cert) { return cert_usage_p(cert, 3); }

int gpgsm_cert_use_cert_p(ksba_cert_t cert) { return cert_usage_p(cert, 4); }

int gpgsm_cert_use_ocsp_p(ksba_cert_t cert) { return cert_usage_p(cert, 5); }
...
int gpgsm_cert_has_well_known_private_key(ksba_cert_t cert) {
  ...
  for (idx = 0; !ksba_cert_get_extension(cert, idx, &oid, NULL, NULL, NULL);
       idx++)
    if (!strcmp(oid, "1.3.6.1.4.1.11591.2.2.2")) return 1; /* Yes.  */
  return 0;                                                /* No.  */
}

static int same_subject_issuer(const char *subject, const char *issuer,
                               ksba_cert_t cert) {
  char *subject2 = ksba_cert_get_subject(cert, 0);
  char *issuer2 = ksba_cert_get_issuer(cert, 0);
  int tmp;

  tmp = (subject && subject2 && !strcmp(subject, subject2) && issuer &&
         issuer2 && !strcmp(issuer, issuer2));
  xfree(subject2);
  xfree(issuer2);
  return tmp;
}
...
static int is_cert_in_certlist(ksba_cert_t cert, certlist_t certlist) {
  const unsigned char *img_a, *img_b;
  size_t len_a, len_b;
...
int gpgsm_add_cert_to_certlist(ctrl_t ctrl, ksba_cert_t cert,
  ...
  if (!is_cert_in_certlist(cert, *listaddr)) {
    ...
    if (!cl) return gpg_error_from_syserror();
    cl->cert = cert;
    ksba_cert_ref(cert);
    cl->next = *listaddr;
    ...
    get_next:
      ...
      if (!rc) {
        ...
        if (rc == GPG_ERR_WRONG_KEY_USAGE) {
          ...
          if (!wrong_usage) { /* save the first match */
            wrong_usage = rc;
            ksba_cert_release(cert);
            cert = NULL;
            goto get_next;
          } else if (same_subject_issuer(first_subject, first_issuer, cert)) {
            wrong_usage = rc;
            ksba_cert_release(cert);
            cert = NULL;
            goto get_next;
      ...
      if (rc && wrong_usage) rc = wrong_usage;
      ...
      next_ambigious:
        ...
        if (rc == -1)
          ...
          if (!dup_certs) gpgsm_add_cert_to_certlist(ctrl, cert, &dup_certs, 0);
          ...
          if (!sm_keydb_get_cert(kh, &cert2)) {
            int tmp =
                (same_subject_issuer(first_subject, first_issuer, cert2) &&
                 ((secret ? gpgsm_cert_use_sign_p(cert2)
                          : gpgsm_cert_use_encrypt_p(cert2)) ==
                  GPG_ERR_WRONG_KEY_USAGE));
            if (tmp)
              gpgsm_add_cert_to_certlist(ctrl, cert2, &dup_certs, 0);
            else {
              if (is_cert_in_certlist(cert2, dup_certs)) tmp = 1;
            }

            ksba_cert_release(cert2);
            if (tmp) goto next_ambigious;
          }
      ...
      if (!rc && !is_cert_in_certlist(cert, *listaddr)) {
        if (!rc && secret) {
          ...
          if (p) {
            if (!gpgsm_agent_havekey(ctrl, p)) rc = 0;
            xfree(p);
          }
        }
        if (!rc)
