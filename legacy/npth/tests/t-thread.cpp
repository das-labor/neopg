...
static void *thread_two(void *arg) {
  ...
  while (!thread_twoone_ready) npth_sleep(0);
...
int thread_main(int argc, char *argv[]) {
  ...
  if (argc >= 2 && !strcmp(argv[1], "--verbose")) opt_verbose = 1;
  ...
  fail_if_err(rc);
  if (state != NPTH_CREATE_JOINABLE) fail_msg("new tattr is not joinable");
  ...
  if (counter != 100) fail_msg("counter value not as expected");
