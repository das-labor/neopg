...
static int map_error(int winerr) {
  /* FIXME */
  return EIO;
}

static int wait_for_single_object(HANDLE obj, DWORD msecs) {
  DWORD res;
...
static void enter_npth(const char *function) {
  int res;
...
static void leave_npth(const char *function) {
...
static void dequeue_thread(npth_impl_t thread) {
...
/* Enqueue THREAD to come after the thread whose next pointer is
   prev_ptr.  */
static void enqueue_thread(npth_impl_t thread, npth_impl_t *prev_ptr) {
  if (*prev_ptr) (*prev_ptr)->prev_ptr = &thread->next;
  thread->prev_ptr = prev_ptr;
  thread->next = *prev_ptr;
  *prev_ptr = thread;
}

static int find_thread(npth_t thread_id, npth_impl_t *thread) {
  if (thread_id < 1 || thread_id >= MAX_THREADS) return ESRCH;

  if (!thread_table[thread_id]) return ESRCH;

  *thread = thread_table[thread_id];
  return 0;
}

static int new_thread(npth_t *thread_id) {
  npth_impl_t thread;
  int id;

  /* ID 0 is never allocated.  */
  for (id = 1; id < MAX_THREADS; id++)
    if (!thread_table[id]) break;
  if (id == MAX_THREADS) return EAGAIN;

  thread = malloc(sizeof(*thread));
  if (!thread) return errno;
...
static void free_thread(npth_t thread_id) {
...
static void deref_thread(npth_t thread_id) {
  npth_impl_t thread = thread_table[thread_id];

  thread->refs -= 1;
  if (thread->refs == 0) free_thread(thread_id);
}
...
int npth_init(void) {
  ...
  tls_index = TlsAlloc();
  if (tls_index == TLS_OUT_OF_INDEXES) return map_error(GetLastError());
...
int npth_attr_init(npth_attr_t *attr_r) {
  npth_attr_t attr;

  attr = malloc(sizeof *attr);
  if (!attr) return errno;
...
static DWORD thread_start(void *arg) {
...
int npth_create(npth_t *newthread, const npth_attr_t *user_attr,
  ...
  handle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)thread_start,
                        (void *)thread_id, CREATE_SUSPENDED, NULL);
  if (handle == NULL) {
...
err_out:
  if (thread_id) free_thread(thread_id);
  if (attr_allocated) npth_attr_destroy(&attr);
...
static int npth_tryjoin_np(npth_t thread_id, void **thread_return) {
  int err;
  npth_impl_t thread;
  int res;
  ...
  if (thread->detached) return EINVAL;
  ...
  if (thread_return) *thread_return = thread->result;
  deref_thread(thread_id);

  return 0;
}

int npth_join(npth_t thread_id, void **thread_return) {
  int err;
  npth_impl_t thread;
  int res;
...
int npth_key_create(npth_key_t *key, void (*destr_function)(void *)) {
  DWORD idx;

  if (destr_function) return EOPNOTSUPP;

  idx = TlsAlloc();
  if (idx == TLS_OUT_OF_INDEXES) return map_error(GetLastError());
...
int npth_key_delete(npth_key_t key) {
  ...
  res = TlsFree(key);
  if (res == 0) return map_error(GetLastError());
  return 0;
}
...
int npth_mutexattr_init(npth_mutexattr_t *attr_r) {
...
int npth_mutexattr_gettype(const npth_mutexattr_t *attr, int *kind) {
  *kind = (*attr)->kind;
  return 0;
}
...
int npth_mutex_init(npth_mutex_t *mutex_r, const npth_mutexattr_t *mutex_attr) {
  npth_mutex_t mutex;

  /* We can not check *mutex_r here, as it may contain random data.  */
  mutex = malloc(sizeof(*mutex));
  if (!mutex) return errno;
...
static int mutex_init_check(npth_mutex_t *mutex) {
  int err;
  npth_mutexattr_t attr;
  int kind;
...
int npth_rwlockattr_init(npth_rwlockattr_t *attr_r) {
...
int npth_rwlockattr_gettype_np(const npth_rwlockattr_t *attr, int *kind) {
  *kind = (*attr)->kind;
  return 0;
}
...
int npth_rwlock_init(npth_rwlock_t *rwlock_r,
  ...
  rwlock->prefer_writer =
      (attr->kind == NPTH_RWLOCK_PREFER_WRITER_NONRECURSIVE_NP);
...
err_out:
  if (attr_allocated) npth_rwlockattr_destroy(&attr);
  return err;
}

/* Must be called with global lock held.  */
static int rwlock_init_check(npth_rwlock_t *rwlock) {
  int err;
  npth_rwlockattr_t attr;
  int kind;

  if (*rwlock == 0) return EINVAL;

  if ((*rwlock) == NPTH_RWLOCK_INITIALIZER) kind = NPTH_RWLOCK_PREFER_READER_NP;
...
int npth_rwlock_destroy(npth_rwlock_t *rwlock) {
  ...
  if (*rwlock == 0 || *rwlock == NPTH_RWLOCK_INITIALIZER) return EINVAL;

  if ((*rwlock)->nr_writers || (*rwlock)->nr_readers ||
      (*rwlock)->nr_writers_queued || (*rwlock)->nr_readers_queued)
    return EBUSY;
...
int npth_rwlock_unlock(npth_rwlock_t *rwlock) {
  ...
  if ((*rwlock)->nr_writers) /* We are the writer.  */
    (*rwlock)->nr_writers = 0;
  else
    /* We are the reader.  */
    (*rwlock)->nr_readers--;
...
pid_t npth_waitpid(pid_t pid, int *status, int options) { return EOPNOTSUPP; }
...
int npth_recvmsg(int fd, struct msghdr *msg, int flags) { return EOPNOTSUPP; }
...
void npth_unprotect(void) {
  ...
  if (initialized_or_any_threads) ENTER();
}

void npth_protect(void) {
  /* See npth_unprotect for commentary.  */
  if (initialized_or_any_threads) LEAVE();
}

int npth_is_protected(void) { return got_sceptre; }
