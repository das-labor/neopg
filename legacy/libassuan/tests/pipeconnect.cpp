...
static gpg_error_t cmd_cat(assuan_context_t ctx, char *line) {
  ...
  if (fd == ASSUAN_INVALID_FD) return gpg_error(GPG_ERR_ASS_NO_INPUT);
  fdout = assuan_get_output_fd(ctx);
  if (fdout == ASSUAN_INVALID_FD) return gpg_error(GPG_ERR_ASS_NO_OUTPUT);
  fp = fdopen(fd, "r");
  if (!fp) {
...
static gpg_error_t server_register_commands(assuan_context_t ctx) {
  static struct {
    const char *name;
...
static void run_server(int enable_debug) {
  int rc;
  ...
  rc = assuan_new(&ctx);
  if (rc) log_fatal("assuan_new failed: %s\n", gpg_strerror(rc));

  rc = assuan_init_pipe_server(ctx, filedes);
  if (rc) log_fatal("assuan_init_pipe_server failed: %s\n", gpg_strerror(rc));

  rc = server_register_commands(ctx);
  if (rc) log_fatal("register_commands failed: %s\n", gpg_strerror(rc));

  if (enable_debug) assuan_set_log_stream(ctx, stderr);

  for (;;) {
    rc = assuan_accept(ctx);
    if (rc) {
      if (rc != -1) log_error("assuan_accept failed: %s\n", gpg_strerror(rc));
      break;
    }
    ...
    if (rc) log_error("assuan_process failed: %s\n", gpg_strerror(rc));
  }
...
static gpg_error_t data_cb(void *opaque, const void *buffer, size_t length) {
  ...
  if (buffer) printf("Received data `%.*s'\n", (int)length, (char *)buffer);
  return 0;
}

static void run_client(const char *servername) {
  gpg_error_t err;
  assuan_context_t ctx;
  ...
  err = assuan_new(&ctx);
  if (err) log_fatal("assuan_new failed: %s\n", gpg_strerror(err));

  err = assuan_pipe_connect(ctx, servername, arglist, no_close_fds, NULL, NULL,
                            0);
  if (err) {
  ...
  err = assuan_transact(ctx,
                        "ECHO Your lucky number is 3552664958674928.  "
...
static void parse_std_file_handles(int *argcp, char ***argvp) {
  ...
  if (fixup) {
    ...
    *argcp = argc;
    ...
    argv = *argvp;
    for (i = 1; i < argc; i++) argv[i] = argv[i + fixup];
    for (; i < argc + fixup; i++) argv[i] = NULL;
  }
#else
  (void)argcp;
  (void)argvp;
...
/*
     M A I N
 */
int pipeconnect_main(int argc, char **argv) {
  gpg_error_t err;
  ...
  log_set_prefix(
      xstrconcat(log_get_prefix(), server ? ".server" : ".client", NULL));
  assuan_set_assuan_log_prefix(log_get_prefix());

  err = assuan_sock_init();
  if (err) log_fatal("socket init failed: %s\n", gpg_strerror(err));

  if (server) {
    log_info("server started\n");
    if (debug && !silent_server) assuan_set_assuan_log_stream(stderr);
    run_server(debug && !silent_server);
    log_info("server finished\n");
  } else {
    log_info("client started\n");
    if (debug && !silent_client) assuan_set_assuan_log_stream(stderr);
    run_client(myname);
