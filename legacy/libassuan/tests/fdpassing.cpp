...
static gpg_error_t cmd_echo(assuan_context_t ctx, char *line) {
  ...
  if (fd == -1) return GPG_ERR_ASS_NO_INPUT;
  fp = fdopen(fd, "r");
  if (!fp) {
...
static gpg_error_t register_commands(assuan_context_t ctx) {
  static struct {
    ...
    gpg_error_t (*handler)(assuan_context_t, char *line);
      ...
      {"ECHO", cmd_echo}, {"INPUT", NULL}, {"OUTPUT", NULL}, {NULL, NULL}};
  int i;
  gpg_error_t rc;
...
static int client(assuan_context_t ctx, const char *fname) {
  int rc;
  FILE *fp;
  int i;
...
int fdpassing_main(int argc, char **argv) {
  ...
  if (is_server) {
    ...
    err = assuan_pipe_connect(ctx, with_exec ? "./fdpassing" : NULL,
                              with_exec ? arglist : &loc, no_close_fds, NULL,
                              NULL, 1);
    if (err) {
