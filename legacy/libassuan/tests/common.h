...
#define _log_enter()                      \
  do {                                    \
    EnterCriticalSection(&_log_critsect); \
  } while (0)
#define _log_leave()                      \
  do {                                    \
    LeaveCriticalSection(&_log_critsect); \
  } while (0)
#else
...
static void *xstrdup(const char *string) {
  char *p = (char *)xmalloc(strlen(string) + 1);
  strcpy(p, string);
  return p;
}

static void log_set_prefix(const char *s) {
...
static const char *log_get_prefix(void) { return log_prefix ? log_prefix : ""; }
...
static void log_printhex(const char *text, const void *buffer, size_t length) {
...
static char *xstrconcat(const char *s1, ...) {
  va_list arg_ptr;
  char *result;
