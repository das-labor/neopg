...
gpg_error_t assuan_new_ext(assuan_context_t *r_ctx,
  ...
  /* Need a new block for the trace macros to work.  */
  {
    TRACE_BEG6(&wctx, ASSUAN_LOG_CTX, "assuan_new_ext", r_ctx,
               ...
               malloc_hooks, malloc_hooks->malloc, malloc_hooks->realloc,
               malloc_hooks->free, log_cb, log_cb_data);

    *r_ctx = NULL;
    ctx = (assuan_context_t)_assuan_malloc(&wctx, sizeof(*ctx));
    if (!ctx) return TRACE_ERR(gpg_error_from_syserror());
...
gpg_error_t assuan_new(assuan_context_t *r_ctx) {
  return assuan_new_ext(r_ctx, &_assuan_default_malloc_hooks,
                        _assuan_default_log_cb, _assuan_default_log_cb_data);
}
