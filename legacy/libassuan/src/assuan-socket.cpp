...
static int is_cygwin_fd(assuan_fd_t sockfd) {
  int ret = 0;
  int i;
...
static int insert_cygwin_fd(assuan_fd_t sockfd) {
  int ret = 0;
  int mark = -1;
  int i;
...
static void delete_cygwin_fd(assuan_fd_t sockfd) {
  int i;
...
static int get_nonce(char *buffer, size_t nbytes) {
  HCRYPTPROV prov;
  int ret = -1;
...
static int read_port_and_nonce(const char *fname, unsigned short *port,
                               char *nonce, int *cygwin) {
  FILE *fp;
  char buffer[50], *p;
  size_t nread;
  int aval;
  ...
  *cygwin = 0;
  ...
  if (!strncmp(buffer, "!<socket >", 10)) {
    ...
    if (sscanf(buffer + 10, "%u s %08x-%08x-%08x-%08x", &u0, narr + 0, narr + 1,
               narr + 2, narr + 3) != 5 ||
        u0 < 1 || u0 > 65535) {
...
static struct sockaddr_un *eval_redirection(const char *fname,
                                            int *r_redirect) {
  FILE *fp;
...
#ifdef HAVE_W32_SYSTEM
  assuan_fd_t res;
  if (domain == AF_UNIX || domain == AF_LOCAL) domain = AF_INET;
...
int _assuan_sock_set_flag(assuan_context_t ctx, assuan_fd_t sockfd,
...
int _assuan_sock_get_flag(assuan_context_t ctx, assuan_fd_t sockfd,
...
/* Read NBYTES from SOCKFD into BUFFER.  Return 0 on success.  Handle
   EAGAIN and EINTR.  */
static int do_readn(assuan_context_t ctx, assuan_fd_t sockfd, void *buffer,
                    size_t nbytes) {
  char *p = (char *)buffer;
  ssize_t n;
...
static int do_writen(assuan_context_t ctx, assuan_fd_t sockfd,
...
static int socks5_connect(assuan_context_t ctx, assuan_fd_t sock,
...
int _assuan_sock_connect(assuan_context_t ctx, assuan_fd_t sockfd,
  ...
  if (addr->sa_family == AF_LOCAL || addr->sa_family == AF_UNIX) {
    ...
    if (read_port_and_nonce(unaddr->sun_path, &port, nonce, &cygwin)) return -1;
  ...
  if (addr->sa_family == AF_LOCAL || addr->sa_family == AF_UNIX) {
    ...
    unaddr = (struct sockaddr_un *)addr;
    if (!stat(unaddr->sun_path, &statbuf) && !S_ISSOCK(statbuf.st_mode) &&
...
assuan_fd_t _assuan_sock_connect_byname(assuan_context_t ctx, const char *host,
  ...
  if (fd == ASSUAN_INVALID_FD) return fd;
  ...
  if (socks5_connect(ctx, fd, socksport, credentials, host ? host : "", port,
                     NULL, 0)) {
...
int _assuan_sock_bind(assuan_context_t ctx, assuan_fd_t sockfd,
  ...
  if (addr->sa_family == AF_LOCAL || addr->sa_family == AF_UNIX) {
    ...
    if (get_nonce(nonce.data, 16)) return -1;
    ...
    filehd = CreateFileA(unaddr->sun_path, GENERIC_WRITE, FILE_SHARE_READ, NULL,
                         CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
    if (filehd == INVALID_HANDLE_VALUE) {
      if (GetLastError() == ERROR_FILE_EXISTS) gpg_err_set_errno(EADDRINUSE);
      return -1;
    }

    rc = bind(HANDLE2SOCKET(sockfd), (struct sockaddr *)&myaddr, len);
    if (!rc)
      rc = getsockname(HANDLE2SOCKET(sockfd), (struct sockaddr *)&myaddr, &len);
    if (rc) {
      int save_e = errno;
      CloseHandle(filehd);
    ...
    if (is_cygwin_fd(sockfd)) {
      snprintf(tmpbuf, sizeof tmpbuf, "!<socket >%d s %08x-%08x-%08x-%08x",
               ntohs(myaddr.sin_port), nonce.aint[0], nonce.aint[1],
               nonce.aint[2], nonce.aint[3]);
      len = strlen(tmpbuf) + 1;
    } else {
    ...
    if (res < 0) gpg_err_set_errno(_assuan_sock_wsa2errno(WSAGetLastError()));
    return res;
  }
#else
...
int _assuan_sock_set_sockaddr_un(const char *fname, struct sockaddr *addr,
  ...
  if (r_redirected) *r_redirected = 0;

#if !defined(HAVE_W32_SYSTEM) && defined(HAVE_STAT)
  if (r_redirected && !stat(fname, &statbuf) && !S_ISSOCK(statbuf.st_mode) &&
      S_ISREG(statbuf.st_mode)) {
...
int _assuan_sock_check_nonce(assuan_context_t ctx, assuan_fd_t fd,
  ...
  if (do_readn(ctx, fd, buffer, 16)) return -1;
  ...
  if (is_cygwin_fd(fd)) {
    ...
    if (do_writen(ctx, fd, buffer, 16)) return -1;
    ...
    if (do_readn(ctx, fd, buffer, 8)) return -1;
    /* Send our credentials: We use the uid and gid we received but
       our own pid.  */
    n = getpid();
    memcpy(buffer, &n, 4);
    if (do_writen(ctx, fd, buffer, 8)) return -1;
  }
...
void assuan_sock_deinit() {
  if (sock_ctx == NULL) return;
...
#ifdef HAVE_W32_SYSTEM
  if (fd != ASSUAN_INVALID_FD) delete_cygwin_fd(fd);
#endif
...
int assuan_sock_connect(assuan_fd_t sockfd, struct sockaddr *addr,
                        int addrlen) {
...
assuan_fd_t assuan_sock_connect_byname(const char *host, unsigned short port,
                                       ...
                                       unsigned int flags) {
  return _assuan_sock_connect_byname(sock_ctx, host, port, reserved,
                                     credentials, flags);
}
