...
void assuan_set_log_stream(assuan_context_t ctx, FILE *fp) {
  if (ctx) {
    if (ctx->log_fp) fflush(ctx->log_fp);
    ctx->log_fp = fp;
    if (!_assuan_log) assuan_set_assuan_log_stream(fp);
  }
}
...
const char *assuan_get_assuan_log_prefix(void) { return prefix_buffer; }
...
int _assuan_log_handler(assuan_context_t ctx, void *hook, unsigned int cat,
  ...
  /* For now.  */
  if (msg == NULL) return TEST_LOG_CAT(cat);
  ...
  if (*prf) fprintf(fp, "%s[%u]: ", prf, (unsigned int)getpid());
...
void _assuan_log_control_channel(assuan_context_t ctx, int outbound,
  ...
  if (!ctx || !ctx->log_cb || ctx->flags.no_logging ||
...
#define TOHEX(val) (((val) < 10) ? ((val) + '0') : ((val)-10 + 'a'))
  ...
  if (string) {
    ...
    for (n = length1, s = (const unsigned char *)buffer1; n; n--, s++)
      if ((!isascii(*s) || iscntrl(*s) || !isprint(*s) || !*s) && !(*s >= 0x80))
        break;
    if (!n && buffer2) {
    ...
    if (!n && (length1 && *(const char *)buffer1 != '[')) {
      ...
      res = gpgrt_asprintf(&outbuf, "chan_" CHANNEL_FMT " %s %.*s%.*s\n",
                           ...
                           (int)length1, (const char *)buffer1, (int)length2,
                           buffer2 ? (const char *)buffer2 : "");
    } else {
      ...
      if (!(outbuf = (char *)malloc(50 + 3 * nbytes + 60 + 3 + 1)))
        ...
        res = 0;
        hp = outbuf;
        snprintf(hp, 50, "chan_" CHANNEL_FMT " %s [", ctx->inbound.fd,
                 outbound ? "->" : "<-");
        hp += strlen(hp);
        n = 0;
        for (s = (const unsigned char *)buffer1, x = 0;
             x < length1 && n < nbytes; x++, n++) {
          *hp++ = ' ';
          *hp++ = TOHEX(*s >> 4);
          *hp++ = TOHEX(*s & 0x0f);
          s++;
        }
        for (s = (const unsigned char *)buffer2, x = 0;
             x < length2 && n < nbytes; x++, n++) {
          *hp++ = ' ';
          *hp++ = TOHEX(*s >> 4);
          *hp++ = TOHEX(*s & 0x0f);
          s++;
