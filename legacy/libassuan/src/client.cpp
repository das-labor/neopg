...
gpg_error_t assuan_client_read_response(assuan_context_t ctx, char **line_r,
                                        int *linelen_r) {
  gpg_error_t rc;
  char *line = NULL;
  int linelen = 0;
  ...
  *linelen_r = 0;
  ...
  if (linelen >= 1 && line[0] == 'D' && line[1] == ' ') {
    ...
    for (s = d = line; linelen; linelen--) {
      if (*s == '%' && linelen > 2) { /* handle escaping */
        s++;
        *d++ = xtoi_2(s);
        s += 2;
        linelen -= 2;
...
gpg_error_t assuan_client_parse_response(assuan_context_t ctx, char *line,
  ...
  *off = 0;
  ...
  if (linelen >= 1 && line[0] == 'D' && line[1] == ' ') {
    ...
    *off = 1;
    while (line[*off] == ' ') ++*off;
  } else if (linelen >= 2 && line[0] == 'O' && line[1] == 'K' &&
             (line[2] == '\0' || line[2] == ' ')) {
    *response = ASSUAN_RESPONSE_OK;
    *off = 2;
    while (line[*off] == ' ') ++*off;
  } else if (linelen >= 3 && line[0] == 'E' && line[1] == 'R' &&
             line[2] == 'R' && (line[3] == '\0' || line[3] == ' ')) {
    *response = ASSUAN_RESPONSE_ERROR;
    *off = 3;
    while (line[*off] == ' ') ++*off;
  } else if (linelen >= 7 && line[0] == 'I' && line[1] == 'N' &&
    ...
    *off = 7;
    while (line[*off] == ' ') ++*off;
  } else if (linelen >= 3 && line[0] == 'E' && line[1] == 'N' &&
             line[2] == 'D' && (line[3] == '\0' || line[3] == ' ')) {
...
again:
  ...
  if (rc) return rc; /* error reading from server */
  ...
  if (response == ASSUAN_RESPONSE_ERROR)
    ...
    if (status_cb) rc = status_cb(status_cb_arg, line);
    if (!rc) goto again;
  } else if (response == ASSUAN_RESPONSE_COMMENT &&
             ctx->flags.convey_comments) {
