...
#define MY_ALIGN(n) \
  ((((n)) + sizeof(size_t) - 1) & (size_t) ~(sizeof(size_t) - 1))
#ifndef CMSG_SPACE
...
static ssize_t uds_reader(assuan_context_t ctx, void *buf, size_t buflen) {
#ifndef HAVE_W32_SYSTEM
  int len = 0;
  ...
  int res =
      recvfrom(HANDLE2SOCKET(ctx->inbound.fd), buf, buflen, 0, NULL, NULL);
...
static ssize_t uds_writer(assuan_context_t ctx, const void *buf,
                          size_t buflen) {
#ifndef HAVE_W32_SYSTEM
  struct msghdr msg;
  struct iovec iovec;
...
#else  /*HAVE_W32_SYSTEM*/
  int res =
      sendto(HANDLE2SOCKET(ctx->outbound.fd), buf, buflen, 0,
             (struct sockaddr *)&ctx->serveraddr, sizeof(struct sockaddr_in));
  if (res < 0) gpg_err_set_errno(_assuan_sock_wsa2errno(WSAGetLastError()));
  return res;
#endif /*HAVE_W32_SYSTEM*/
}

static gpg_error_t uds_sendfd(assuan_context_t ctx, assuan_fd_t fd) {
#ifdef USE_DESCRIPTOR_PASSING
  struct msghdr msg;
  struct iovec iovec;
  union {
  ...
  if (len < 0) {
    ...
    TRACE1(ctx, ASSUAN_LOG_SYSIO, "uds_sendfd", ctx, "uds_sendfd: %s",
           strerror(errno));
    errno = saved_errno;
...
static gpg_error_t uds_receivefd(assuan_context_t ctx, assuan_fd_t *fd) {
#ifdef USE_DESCRIPTOR_PASSING
  int i;
...
void _assuan_uds_deinit(assuan_context_t ctx) { _assuan_uds_close_fds(ctx); }
