...
static gpg_error_t dummy_handler(assuan_context_t ctx, char *line) {
  return PROCESS_DONE(ctx, set_error(ctx, GPG_ERR_ASSUAN_SERVER_FAULT,
...
static gpg_error_t std_handler_nop(assuan_context_t ctx, char *line) {
...
static const char std_help_option[] =
    ...
    "<VALUE> is given.";
static gpg_error_t std_handler_option(assuan_context_t ctx, char *line) {
  char *key, *value, *p;
  ...
  if (!*key)
    return PROCESS_DONE(
        ctx, set_error(ctx, GPG_ERR_ASS_SYNTAX, "argument required"));
  if (*key == '=')
    return PROCESS_DONE(
        ctx, set_error(ctx, GPG_ERR_ASS_SYNTAX, "no option name given"));
  for (value = key; *value && !spacep(value) && *value != '='; value++)
    ;
  if (*value) {
    if (spacep(value)) *value++ = 0; /* terminate key */
    ...
    if (*value == '=') {
      *value++ = 0; /* terminate key */
      for (; spacep(value); value++)
        ;
      if (!*value)
        return PROCESS_DONE(ctx, set_error(ctx, GPG_ERR_ASS_SYNTAX,
                                           "option argument expected"));
    }
    if (*value) {
      for (p = value + strlen(value) - 1; p > value && spacep(p); p--)
        ;
      if (p > value) *++p = 0; /* strip trailing spaces */
    }
  }
...
static gpg_error_t std_handler_auth(assuan_context_t ctx, char *line) {
  return PROCESS_DONE(ctx, set_error(ctx, GPG_ERR_NOT_IMPLEMENTED, NULL));
}
...
static gpg_error_t std_handler_reset(assuan_context_t ctx, char *line) {
  gpg_error_t err = 0;

  if (ctx->reset_notify_fnc) err = ctx->reset_notify_fnc(ctx, line);
  if (!err) {
    assuan_close_input_fd(ctx);
    assuan_close_output_fd(ctx);
...
static const char std_help_help[] =
    ...
    "that command.";
static gpg_error_t std_handler_help(assuan_context_t ctx, char *line) {
  unsigned int i;
  ...
  if (!n) {
    ...
    for (i = 0; i < ctx->cmdtbl_used; i++) {
      ...
      if (helpstr && !strncmp(ctx->cmdtbl[i].name, helpstr, n) &&
          (!helpstr[n] || helpstr[n] == '\n' || helpstr[n] == ' ') &&
          (n = strcspn(helpstr, "\n")))
    ...
    for (i = 0; ctx->cmdtbl[i].name; i++)
      if (!my_strcasecmp(line, ctx->cmdtbl[i].name)) break;
    line[n] = c;
...
static gpg_error_t std_handler_end(assuan_context_t ctx, char *line) {
  return PROCESS_DONE(ctx, set_error(ctx, GPG_ERR_NOT_IMPLEMENTED, NULL));
}
...
static const char std_help_input[] =
    ...
    "assuan_sendfd.";
static gpg_error_t std_handler_input(assuan_context_t ctx, char *line) {
  gpg_error_t rc;
  assuan_fd_t fd, oldfd;

  rc = assuan_command_parse_fd(ctx, line, &fd);
  if (rc) return PROCESS_DONE(ctx, rc);

  if (ctx->input_notify_fnc) {
    oldfd = ctx->input_fd;
    ctx->input_fd = fd;
    rc = ctx->input_notify_fnc(ctx, line);
    if (rc) ctx->input_fd = oldfd;
  } else if (!rc)
    ctx->input_fd = fd;
...
static const char std_help_output[] =
    ...
    "server opens the file descriptor just sent by the client using\n"
    "assuan_sendfd.";
static gpg_error_t std_handler_output(assuan_context_t ctx, char *line) {
  gpg_error_t rc;
  assuan_fd_t fd, oldfd;
...
} std_cmd_table[] = {{"NOP", std_handler_nop, std_help_nop, 1},
 ...
 **/
gpg_error_t assuan_register_command(assuan_context_t ctx, const char *cmd_name,
                                    assuan_handler_t handler,
                                    const char *help_string) {
  int i, cmd_index = -1;
  const char *s;

  if (cmd_name && !*cmd_name) cmd_name = NULL;

  if (!cmd_name) return GPG_ERR_ASS_INV_VALUE;

  if (!handler) { /* find a default handler. */
    for (i = 0; (s = std_cmd_table[i].name) && strcmp(cmd_name, s); i++)
      ;
    if (!s) { /* Try again but case insensitive. */
      for (i = 0; (s = std_cmd_table[i].name) && my_strcasecmp(cmd_name, s);
           i++)
        ;
    }
    if (s) handler = std_cmd_table[i].handler;
    if (!handler)
  ...
  if (!ctx->cmdtbl) {
    ctx->cmdtbl_size = 50;
    ctx->cmdtbl =
        (cmdtbl_s *)_assuan_calloc(ctx, ctx->cmdtbl_size, sizeof *ctx->cmdtbl);
    if (!ctx->cmdtbl) return gpg_error_from_syserror();
    ctx->cmdtbl_used = 0;
  } else if (ctx->cmdtbl_used >= ctx->cmdtbl_size) {
    struct cmdtbl_s *x;

    x = (cmdtbl_s *)_assuan_realloc(ctx, ctx->cmdtbl,
                                    (ctx->cmdtbl_size + 10) * sizeof *x);
    if (!x) return gpg_error_from_syserror();
    ctx->cmdtbl = x;
    ctx->cmdtbl_size += 50;
  }
  ...
  if (cmd_index == -1) cmd_index = ctx->cmdtbl_used++;
...
static gpg_error_t handle_data_line(assuan_context_t ctx, char *line,
                                    int linelen) {
...
static gpg_error_t dispatch_command(assuan_context_t ctx, char *line,
                                    int linelen) {
  gpg_error_t err;
  char *p;
  const char *s;
  ...
  if (*p) { /* Skip over leading WS after the keyword */
    *p++ = 0;
    while (*p == ' ' || *p == '\t') p++;
  }
  shift = p - line;
  ...
  for (i = 0; (s = ctx->cmdtbl[i].name); i++) {
    if (!strcmp(line, s)) break;
  }
  if (!s) { /* and try case insensitive */
    for (i = 0; (s = ctx->cmdtbl[i].name); i++) {
      if (!my_strcasecmp(line, s)) break;
    }
  }
  if (!s)
...
gpg_error_t assuan_process_done(assuan_context_t ctx, gpg_error_t rc) {
  if (!ctx->in_command) return GPG_ERR_ASS_GENERAL;

  if (ctx->flags.force_close) ctx->process_complete = 1;
  ...
  if (ctx->outbound.data.fp) {
    ...
    if (!rc && ctx->outbound.data.error) rc = ctx->outbound.data.error;
  } else {
...
static gpg_error_t process_next(assuan_context_t ctx) {
  gpg_error_t rc;
  ...
  if (_assuan_error_is_eagain(ctx, rc)) return 0;
  if (rc == GPG_ERR_EOF) {
    ctx->process_complete = 1;
    return 0;
  }
  ...
  if (!ctx->in_command) {
    ...
    /* Should not happen.  The client is sending data while we are
       ...
       and discard it.  */
    TRACE0(ctx, ASSUAN_LOG_DATA, "process_next", ctx, "unexpected client data");
    rc = 0;
  }
...
gpg_error_t assuan_process_next(assuan_context_t ctx, int *done) {
  ...
  if (done) *done = !!ctx->process_complete;

  return rc;
}

static gpg_error_t process_request(assuan_context_t ctx) {
  gpg_error_t rc;

  if (ctx->in_inquire) return GPG_ERR_ASS_NESTED_COMMANDS;
...
static ssize_t fun2_cookie_write(void *cookie, const char *buffer,
                                 size_t orig_size) {
  return _assuan_cookie_write_data(cookie, buffer, orig_size);
}
...
gpg_error_t assuan_write_status(assuan_context_t ctx, const char *keyword,
  ...
  if (!ctx || !keyword) return GPG_ERR_ASS_INV_VALUE;
  if (!text) text = "";
