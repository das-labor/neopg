...
static GPG_ERR_INLINE assuan_fd_t assuan_fd_from_posix_fd(int fd) {
  if (fd < 0)
...
#define ASSUAN_INVALID_PID ((pid_t)-1)
static GPG_ERR_INLINE assuan_fd_t assuan_fd_from_posix_fd(int fd) { return fd; }
#endif
...
void assuan_set_io_monitor(assuan_context_t ctx, assuan_io_monitor_t io_monitor,
                           void *hook_data);
...
struct assuan_system_hooks {
  ...
  ssize_t (*read)(assuan_context_t ctx, assuan_fd_t fd, void *buffer,
                  size_t size);
  ssize_t (*write)(assuan_context_t ctx, assuan_fd_t fd, const void *buffer,
                   size_t size);
  ...
  pid_t (*waitpid)(assuan_context_t ctx, pid_t pid, int action, int *status,
                   int options);
  ...
  int (*connect)(assuan_context_t ctx, int sock, struct sockaddr *addr,
                 socklen_t length);
};
...
gpg_error_t assuan_client_read_response(assuan_context_t ctx, char **line,
                                        int *linelen);

gpg_error_t assuan_client_parse_response(assuan_context_t ctx, char *line,
                                         int linelen,
                                         assuan_response_t *response, int *off);

/*-- assuan-client.c --*/
gpg_error_t assuan_transact(
    assuan_context_t ctx, const char *command,
...
int __assuan_socketpair(assuan_context_t ctx, int _namespace, int style,
                        int protocol, assuan_fd_t filedes[2]);
int __assuan_socket(assuan_context_t ctx, int _namespace, int style,
                    int protocol);
...
#define ASSUAN_SYSTEM_NPTH_IMPL                                              \
  ...
  struct assuan_system_hooks _assuan_system_npth = {                         \
      ...
      _assuan_npth_read,                                                     \
      _assuan_npth_write,                                                    \
      _assuan_npth_recvmsg,                                                  \
      _assuan_npth_sendmsg,                                                  \
      __assuan_spawn,                                                        \
      _assuan_npth_waitpid,                                                  \
      __assuan_socketpair,                                                   \
      __assuan_socket,                                                       \
