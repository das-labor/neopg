...
assuan_fd_t assuan_fdopen(int fd) {
  ...
  if (!DuplicateHandle(GetCurrentProcess(), ifd, GetCurrentProcess(), &ofd, 0,
...
int __assuan_pipe(assuan_context_t ctx, assuan_fd_t fd[2], int inherit_idx) {
  ...
  if (!CreatePipe(&rh, &wh, &sec_attr, 0)) {
    TRACE1(ctx, ASSUAN_LOG_SYSIO, "__assuan_pipe", ctx, "CreatePipe failed: %s",
           _assuan_w32_strerror(ctx, -1));
    gpg_err_set_errno(EIO);
    return -1;
  }

  if (!DuplicateHandle(GetCurrentProcess(), (inherit_idx == 0) ? rh : wh,
                       GetCurrentProcess(), &th, 0, TRUE,
...
int __assuan_close(assuan_context_t ctx, assuan_fd_t fd) {
  ...
  if (rc) gpg_err_set_errno(_assuan_sock_wsa2errno(WSAGetLastError()));
...
static int is_socket(HANDLE hd) {
  ...
  if (GetFileType(hd) == FILE_TYPE_PIPE &&
      !GetNamedPipeInfo(hd, &dummyflags, &dummyoutsize, &dummyinsize,
                        &dummyinst))
...
ssize_t __assuan_read(assuan_context_t ctx, assuan_fd_t fd, void *buffer,
                      size_t size) {
  int res;
  int ec = 0;
...
ssize_t __assuan_write(assuan_context_t ctx, assuan_fd_t fd, const void *buffer,
                       size_t size) {
  int res;
  int ec = 0;
...
int __assuan_recvmsg(assuan_context_t ctx, assuan_fd_t fd, assuan_msghdr_t msg,
                     int flags) {
  gpg_err_set_errno(ENOSYS);
  return -1;
}

int __assuan_sendmsg(assuan_context_t ctx, assuan_fd_t fd, assuan_msghdr_t msg,
                     int flags) {
  gpg_err_set_errno(ENOSYS);
  return -1;
}
...
static int build_w32_commandline(assuan_context_t ctx, const char *const *argv,
                                 char **cmdline) {
  int i, n;
  const char *s;
  char *buf, *p;
...
int __assuan_spawn(assuan_context_t ctx, pid_t *r_pid, const char *name,
  ...
  if (build_w32_commandline(ctx, argv, &cmdline)) return -1;
  ...
  /*                       name, cmdline); */
  if (!CreateProcess(
          name,      /* Program to start.  */
          cmdline,   /* Command line arguments.  */
          &sec_attr, /* Process security attributes.  */
          &sec_attr, /* Thread security attributes.  */
          TRUE,      /* Inherit handles.  */
          (CREATE_DEFAULT_ERROR_MODE | ((flags & 128) ? DETACHED_PROCESS : 0) |
    ...
    _assuan_free(ctx, cmdline);
    if (nullfd != INVALID_HANDLE_VALUE) CloseHandle(nullfd);
...
int __assuan_socketpair(assuan_context_t ctx, int namespace_x, int style,
                        int protocol, assuan_fd_t filedes[2]) {
  gpg_err_set_errno(ENOSYS);
  return -1;
}

int __assuan_socket(assuan_context_t ctx, int domain, int type, int proto) {
  int res;

  res = socket(domain, type, proto);
  if (res == -1) gpg_err_set_errno(_assuan_sock_wsa2errno(WSAGetLastError()));
  return res;
}

int __assuan_connect(assuan_context_t ctx, int sock, struct sockaddr *addr,
                     socklen_t length) {
  int res;
...
struct assuan_system_hooks _assuan_system_hooks = {ASSUAN_SYSTEM_HOOKS_VERSION,
                                                   __assuan_usleep,
                                                   __assuan_pipe,
                                                   __assuan_close,
                                                   __assuan_read,
