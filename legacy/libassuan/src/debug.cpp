...
void _assuan_debug(assuan_context_t ctx, unsigned int cat, const char *format,
  ...
  if (!ctx || !ctx->log_cb || !(*ctx->log_cb)(ctx, ctx->log_cb_data, cat, NULL))
    return;
...
void _assuan_debug_begin(assuan_context_t ctx, void **line, unsigned int cat,
                         const char *format, ...) {
  va_list arg_ptr;
  int res;
...
void _assuan_debug_add(assuan_context_t ctx, void **line, const char *format,
                       ...) {
  va_list arg_ptr;
  char *toadd;
  char *result;
...
/* Finish construction of *LINE and send it to the debug output
   stream.  */
void _assuan_debug_end(assuan_context_t ctx, void **line, unsigned int cat) {
  if (!*line) return;
