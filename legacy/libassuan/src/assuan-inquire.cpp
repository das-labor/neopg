...
static void init_membuf(assuan_context_t ctx, struct membuf *mb, int initiallen,
                        size_t maxlen) {
  mb->len = 0;
  mb->size = initiallen;
  mb->out_of_core = 0;
...
static void put_membuf(assuan_context_t ctx, struct membuf *mb, const void *buf,
                       size_t len) {
  if (mb->out_of_core || mb->too_large) return;
...
static void *get_membuf(assuan_context_t ctx, struct membuf *mb, size_t *len) {
  char *p;
...
static void free_membuf(assuan_context_t ctx, struct membuf *mb) {
  _assuan_free(ctx, mb->buf);
  mb->buf = NULL;
}
...
gpg_error_t assuan_inquire(assuan_context_t ctx, const char *keyword,
  ...
  if (r_buffer) *r_buffer = NULL;
  if (r_length) *r_length = 0;

  if (!ctx || !keyword || (10 + strlen(keyword) >= sizeof(cmdbuf)))
    return GPG_ERR_ASS_INV_VALUE;
  nodataexpected = !r_buffer && !r_length && !maxlen;
  if (!nodataexpected && (!r_buffer || !r_length)) return GPG_ERR_ASS_INV_VALUE;
  if (!ctx->is_server) return GPG_ERR_ASS_NOT_A_SERVER;
  ...
  for (;;) {
    ...
    if ((line[0] == 'E' || line[0] == 'e') &&
        (line[1] == 'N' || line[1] == 'n') &&
        (line[2] == 'D' || line[2] == 'd') && (!line[3] || line[3] == ' '))
    ...
    if (mb.too_large) continue; /* Need to read up the remaining data.  */
  ...
  if (!nodataexpected) {
    if (mb.too_large)
      ...
      *r_buffer = (unsigned char *)get_membuf(ctx, &mb, r_length);
      if (!*r_buffer) rc = gpg_error_from_syserror();
    }
  }

out:
  if (!nodataexpected) free_membuf(ctx, &mb);
  ctx->in_inquire = 0;
  return rc;
}
...
gpg_error_t assuan_inquire_ext(assuan_context_t ctx, const char *keyword,
  ...
  if (!mb) return gpg_error_from_syserror();
  init_membuf(ctx, mb, maxlen ? maxlen : 1024, maxlen);
