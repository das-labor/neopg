...
void _assuan_usleep(assuan_context_t ctx, unsigned int usec) {
  TRACE1(ctx, ASSUAN_LOG_SYSIO, "_assuan_usleep", ctx, "usec=%u", usec);
...
int _assuan_pipe(assuan_context_t ctx, assuan_fd_t fd[2], int inherit_idx) {
  ...
  TRACE_BEG2(ctx, ASSUAN_LOG_SYSIO, "_assuan_pipe", ctx,
             "inherit_idx=%i (Assuan uses it for %s)", inherit_idx,
             inherit_idx ? "reading" : "writing");

  err = (ctx->system.pipe)(ctx, fd, inherit_idx);
  if (err) return TRACE_SYSRES(err);
...
int _assuan_close(assuan_context_t ctx, assuan_fd_t fd) {
  TRACE1(ctx, ASSUAN_LOG_SYSIO, "_assuan_close", ctx, "fd=0x%x", fd);
...
int _assuan_close_inheritable(assuan_context_t ctx, assuan_fd_t fd) {
  TRACE1(ctx, ASSUAN_LOG_SYSIO, "_assuan_close_inheritable", ctx, "fd=0x%x",
         fd);
  ...
  if (res > 0) {
    ...
    if (cmptr) {
      ...
      TRACE_LOG5(
          ...
          cmptr->cmsg_len, cmptr->cmsg_len - (((char *)data) - ((char *)cmptr)),
...
int _assuan_spawn(assuan_context_t ctx, pid_t *r_pid, const char *name,
  ...
  TRACE_BEG6(ctx, ASSUAN_LOG_CTX, "_assuan_spawn", ctx,
             ...
             name ? name : "(null)", fd_in, fd_out, atfork, atforkvalue, flags);
...
pid_t _assuan_waitpid(assuan_context_t ctx, pid_t pid, int action, int *status,
                      int options) {
#if DEBUG_SYSIO
  ssize_t res;
  TRACE_BEG4(ctx, ASSUAN_LOG_SYSIO, "_assuan_waitpid", ctx,
             "pid=%i, action=%i, status=%p, options=%i", pid, action, status,
             options);
...
int _assuan_socketpair(assuan_context_t ctx, int namespace_x, int style,
  ...
  TRACE_BEG4(ctx, ASSUAN_LOG_SYSIO, "_assuan_socketpair", ctx,
             "namespace=%i,style=%i,protocol=%i,filedes=%p", namespace_x, style,
             protocol, filedes);

  res = (ctx->system.socketpair)(ctx, namespace_x, style, protocol, filedes);
  if (res == 0) TRACE_LOG2("filedes = { 0x%x, 0x%x }", filedes[0], filedes[1]);
...
int _assuan_socket(assuan_context_t ctx, int namespace_x, int style,
  ...
  TRACE_BEG3(ctx, ASSUAN_LOG_SYSIO, "_assuan_socket", ctx,
             "namespace=%i,style=%i,protocol=%i", namespace_x, style, protocol);
