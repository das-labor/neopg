...
int __assuan_close(assuan_context_t ctx, assuan_fd_t fd) { return close(fd); }
...
#ifdef __linux__
  {
    ...
    if (dir) {
      while ((dir_entry = readdir(dir))) {
        ...
        if (*s < '0' || *s > '9') continue;
        x = atoi(s);
        if (x > max_fds) max_fds = x;
      }
      closedir(dir);
    }
    if (max_fds != -1) return max_fds + 1;
  }
#endif /* __linux__ */

#ifdef RLIMIT_NOFILE
  if (!getrlimit(RLIMIT_NOFILE, &rl)) max_fds = rl.rlim_max;
#endif

#ifdef RLIMIT_OFILE
  if (max_fds == -1 && !getrlimit(RLIMIT_OFILE, &rl)) max_fds = rl.rlim_max;
...
#ifdef INT32_MAX
  if (max_fds == INT32_MAX) max_fds = 256;
#endif
...
int __assuan_spawn(assuan_context_t ctx, pid_t *r_pid, const char *name,
  ...
  if (pid == 0) {
    ...
    if (atfork) atfork(atforkvalue, 0);
    ...
    if (fd_out != STDOUT_FILENO) {
      if (dup2(fd_out == ASSUAN_INVALID_FD ? fdnul : fd_out, STDOUT_FILENO) ==
          -1) {
        TRACE1(ctx, ASSUAN_LOG_SYSIO, "__assuan_spawn", ctx,
    ...
    if (fd_in != STDIN_FILENO) {
      if (dup2(fd_in == ASSUAN_INVALID_FD ? fdnul : fd_in, STDIN_FILENO) ==
          -1) {
        TRACE1(ctx, ASSUAN_LOG_SYSIO, "__assuan_spawn", ctx,
               "dup2 failed in child: %s", strerror(errno));
        _exit(4);
    ...
    for (i = 0; i < n; i++) {
      ...
      fdp = fd_child_list;
      if (fdp) {
        while (*fdp != -1 && *fdp != i) fdp++;
      }

      if (!(fdp && *fdp != -1)) close(i);
    }
    gpg_err_set_errno(0);
    ...
    snprintf(errbuf, sizeof(errbuf) - 1, "ERR %d can't exec `%s': %.50s\n",
             GPG_ERR_ASS_SERVER_START, name, strerror(errno));
...
pid_t __assuan_waitpid(assuan_context_t ctx, pid_t pid, int nowait, int *status,
  ...
  if (!nowait) return waitpid(pid, NULL, 0);
  return 0;
}
...
int __assuan_socket(assuan_context_t ctx, int namespace_x, int style,
                    int protocol) {
