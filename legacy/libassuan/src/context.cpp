...
void assuan_set_pointer(assuan_context_t ctx, void *user_pointer) {
  TRACE1(ctx, ASSUAN_LOG_CTX, "assuan_set_pointer", ctx, "user_pointer=%p",
         user_pointer);

  if (ctx) ctx->user_pointer = user_pointer;
}
...
void assuan_set_flag(assuan_context_t ctx, assuan_flag_t flag, int value) {
  TRACE2(ctx, ASSUAN_LOG_CTX, "assuan_set_flag", ctx, "flag=%i,value=%i", flag,
         value);
...
int assuan_get_flag(assuan_context_t ctx, assuan_flag_t flag) {
  int res = 0;
  TRACE_BEG1(ctx, ASSUAN_LOG_CTX, "assuan_get_flag", ctx, "flag=%i", flag);
...
void assuan_ctx_set_system_hooks(assuan_context_t ctx,
  ...
  TRACE2(ctx, ASSUAN_LOG_CTX, "assuan_set_system_hooks", ctx,
         "system_hooks=%p (version %i)", system_hooks, system_hooks->version);
...
gpg_error_t assuan_set_error(assuan_context_t ctx, gpg_error_t err,
                             const char *text) {
  TRACE3(ctx, ASSUAN_LOG_CTX, "assuan_set_error", ctx, "err=%i (%s),text=%s",
         err, gpg_strerror(err), text ? text : "(none)");
...
pid_t assuan_get_pid(assuan_context_t ctx) {
  TRACE1(ctx, ASSUAN_LOG_CTX, "assuan_get_pid", ctx, "pid=%i",
         ctx ? ctx->pid : -1);
